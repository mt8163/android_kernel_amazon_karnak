...
void OpenAnalogHeadphone(bool bEnable)
{
	...
	if (bEnable) {
		...
		/*When the system start, the function is called, and the sampling frequency of play is set 44.1k,
		  it fail to play 48k audio files by tinyplay tool.So we mark this row.*/
...
static int mt63xx_codec_startup(struct snd_pcm_substream *substream, struct snd_soc_dai *Daiport)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE && substream->runtime->rate) {
		...
		/*When the system start, the sampling frequency of record will be set 16k,
		and it fail to record 48k audio file by tinycap tool.So we mark this row.*/
...
static int Ext_Headphone_Amp_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s()\n", __func__);
	ucontrol->value.integer.value[0] =
	    mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTHPAMP];
	return 0;
}

static int Ext_Headphone_Amp_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	mutex_lock(&Ana_Ctrl_Mutex);
	pr_debug("%s()\n", __func__);
	if ((ucontrol->value.integer.value[0] == true) &&
	    (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTHPAMP] == false)) {
...
"-16Db", "-14Db", "-12Db", "-10Db", "-8Db", "-6Db", "-4Db", "-2Db", "0Db", "2Db",
"4Db", "6Db", "8Db"};
...
static const struct snd_kcontrol_new mt6323_snd_controls[] = {
	...
	SOC_ENUM_EXT("Ext_Speaker_Amp_Switch", Audio_DL_Enum[5], Ext_Speaker_Amp_Get,
		     Ext_Speaker_Amp_Set),
	SOC_ENUM_EXT("Ext_Headphone_Amp_Switch", Audio_DL_Enum[6], Ext_Headphone_Amp_Get,
		     Ext_Headphone_Amp_Set),
	SOC_ENUM_EXT("Headphone_Speaker_Mux", Audio_DL_Enum[7], Headphone_Speaker_Mux_Get,
		      Headphone_Speaker_Mux_Set),
	SOC_ENUM_EXT("Headset_PGAL_GAIN", Audio_DL_Enum[8], Headset_PGAL_Get, Headset_PGAL_Set),
	SOC_ENUM_EXT("Headset_PGAR_GAIN", Audio_DL_Enum[9], Headset_PGAR_Get, Headset_PGAR_Set),
	SOC_ENUM_EXT("Handset_PGA_GAIN", Audio_DL_Enum[10], Handset_PGA_Get, Handset_PGA_Set),
	SOC_ENUM_EXT("Lineout_PGAR_GAIN", Audio_DL_Enum[11], Lineout_PGAR_Get, Lineout_PGAR_Set),
	SOC_ENUM_EXT("Lineout_PGAL_GAIN", Audio_DL_Enum[12], Lineout_PGAL_Get, Lineout_PGAL_Set),
	SOC_ENUM_EXT("Ext_Amp_Gain", Audio_DL_Enum[13], Ext_Amp_Gain_Get, Ext_Amp_Gain_Set),
	SOC_ENUM_EXT("AUD_CLK_BUF_Switch", Audio_DL_Enum[14], Aud_Clk_Buf_Get, Aud_Clk_Buf_Set),
#ifdef CONFIG_AMP_AW8737S
...
static const struct soc_enum Audio_UL_Enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(Audio_AnalogMic_Mode), Audio_AnalogMic_Mode),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(Audio_AnalogMic_Mode), Audio_AnalogMic_Mode),
#ifdef CONFIG_VOICE_DSP_MICBIAS0
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(ADC_function), ADC_function),
#endif
};

#ifdef CONFIG_VOICE_DSP_MICBIAS0
static int audio_amic_on = 0;
static int Audio_AMIC_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = audio_amic_on;
	return 0;
}

static int Audio_AMIC_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.integer.value[0] == audio_amic_on) {
		pr_info("AMIC_BIAS is the same as you want to set.");
		// not returned just avoid other control such as Audio_ADC1_Set changing MICBIAS before.
		// ensure this call is executed successfully.
		// return 0;
	}
	audio_amic_on = ucontrol->value.integer.value[0];
...
static void ADC_LOOP_DAC_Off(void)
{
	...
	if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTHPAMP] == true) {
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTHPAMP] = false;
		Ext_Headphone_Amp_Change(false);
	}
...
static void ADC_LOOP_DAC_Func(int mode)
{
	...
	case AUDIO_ANALOG_HEADSET_MIC_LOOP_HP:
		...
		if (mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTHPAMP] == false) {
			Ext_Headphone_Amp_Change(true);
			mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_EXTHPAMP] = true;
		}
		break;
...
static const char * const Pmic_Loopback_Mode[] = {
	"OFF", "AMIC_TO_HP", "AMIC_TO_SPK", "HEADSET_MIC_TO_HP", "HEADSET_MIC_TO_SPK"};
...
#ifdef CONFIG_VOICE_DSP_MICBIAS0
	SOC_ENUM_EXT("Audio_AMIC_Switch", Audio_UL_Enum[18], Audio_AMIC_Get, Audio_AMIC_Set),
#endif
};
