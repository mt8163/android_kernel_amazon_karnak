...
static int Audio_I2S0dl1_lr_switch_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int Audio_I2S0dl1_lr_switch_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_kcontrol_chip(kcontrol);
	struct mt_pcm_I2S0dl1_priv *priv = dev_get_drvdata(component->dev);
	if ((priv->prepared) && (priv->lr_switch != ucontrol->value.integer.value[0])) {
...
enum mt_I2S0dl1_volume_key_switch {
	VOLKEY_NORMAL = 0,
	VOLKEY_SWAP
};

static const char *const mt_I2S0dl1_volume_key_switch[] = { "VOLKEY_NORMAL", "VOLKEY_SWAP" };

extern void set_kpd_swap_vol_key(bool flag);
extern bool get_kpd_swap_vol_key(void);

static int Audio_I2S0dl1_volkey_switch_get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
...
static int Audio_I2S0dl1_volkey_switch_set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
#ifdef CONFIG_KPD_VOLUME_KEY_SWAP
...
static const struct soc_enum mt_I2S0dl1_volkey_control_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(mt_I2S0dl1_volume_key_switch),
		mt_I2S0dl1_volume_key_switch),
};

static const struct snd_kcontrol_new mt_I2S0dl1_volkey_controls[] = {
	SOC_ENUM_EXT("VOLKEY_SWITCH", mt_I2S0dl1_volkey_control_enum[0],
		Audio_I2S0dl1_volkey_switch_get, Audio_I2S0dl1_volkey_switch_set),
};
				     ...
				     struct snd_pcm_hw_params *hw_params)
{
	...
	pr_info("%s dma_bytes = %zu, dma_area = %p, dma_addr = 0x%lx\n", __func__,
...
static int mtk_pcm_I2S0dl1_open(struct snd_pcm_substream *substream)
{
	...
	if (GetSramState() == SRAM_STATE_FREE || GetSramState() == SRAM_STATE_CAPTUREPARTIAL) {
		mtk_I2S0dl1_hardware.buffer_bytes_max = GetPLaybackSramPartial();
	...
	pr_info("mtk_I2S0dl1_hardware.buffer_bytes_max = %zu, mPlaybackSramState = %d\n",
...
static int mtk_I2S0dl1_probe(struct platform_device *pdev)
{
	...
	priv = devm_kzalloc(dev, sizeof(struct mt_pcm_I2S0dl1_priv), GFP_KERNEL);
	if (unlikely(!priv)) {
...
static int mtk_afe_I2S0dl1_probe(struct snd_soc_platform *platform)
{
	...
	snd_soc_add_platform_controls(platform, mt_I2S0dl1_volkey_controls,
				      ARRAY_SIZE(mt_I2S0dl1_volkey_controls));
	/* allocate dram */
