				  ...
				  void __user *dst, snd_pcm_uframes_t count)
{
	...
	if (DMA_Read_Ptr + read_size <= Awb_Block->u4BufferSize) {
		...
		PRINTK_AUD_AWB
			("%s finish 2, copy size_1:%x, u4DMAReadIdx:%x, u4WriteIdx:%x, u4DataRemained:%x\n",
		...
		PRINTK_AUD_AWB
			("%s finish 3, copy size_2:%x, u4DMAReadIdx:%x, u4WriteIdx:%x u4DataRemained:%x\n",
