	...
	struct snd_pcm_substream *substream)
{
	...
	if (GetMemoryPathEnable(Soc_Aud_Digital_Block_MEM_AWB) == true) {
		...
		PRINTK_AUD_AWB("mtk_awb_pcm_pointer =0x%x mem_index = 0x%x\n",
	 ...
	 struct snd_pcm_hw_params *hw_params)
{
	...
	Afe_Set_Reg(AFE_AWB_END, pblock->pucPhysBufAddr +
...
static int mtk_mrgrx_awb_pcm_open(struct snd_pcm_substream *substream)
{
	...
	Mrgrx_AWB_Control_context = Get_Mem_ControlT(
...
static int mtk_capture_mrgrx_pcm_trigger(
				  ...
				  void __user *dst, snd_pcm_uframes_t count)
{
	...
	PRINTK_AUD_AWB("rp = 0x%x, size = 0x%x\n",
	...
	if (DMA_Read_Ptr + read_size <= Awb_Block->u4BufferSize) {
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			PRINTK_AUD_AWB
			("%s 1, size:%zu, remained:%x, DMA_Ptr:%zu, rp:%x\n",
			__func__, read_size,
			Awb_Block->u4DataRemained, DMA_Read_Ptr,
			Awb_Block->u4DMAReadIdx);
		}
		...
		PRINTK_AUD_AWB
		("%s finish 1, size:%x,rp:%x, wp:%x,",
		__func__, read_size,
		Awb_Block->u4DMAReadIdx, Awb_Block->u4WriteIdx);
		PRINTK_AUD_AWB("remained:%x, Size:%x\n",
		Awb_Block->u4DataRemained,
		Get_Mem_CopySizeByStream(
		...
		if (DMA_Read_Ptr != Awb_Block->u4DMAReadIdx) {
			PRINTK_AUD_AWB
			("%s 2, size1:%x, remained:%x, DMAortr:%zu, rp:%x\n",
			__func__, size_1,
			Awb_Block->u4DataRemained, DMA_Read_Ptr,
			Awb_Block->u4DMAReadIdx);
		}
		...
		if (copy_to_user((void __user *)(Read_Data_Ptr + size_1),
			...
			pr_err("%s Fail3 cp to user Rdp:%p,Addr:%p,",
				__func__, Read_Data_Ptr,
				Awb_Block->pucVirtBufAddr);
			pr_err("DMARdId:0x%x, DMARdp:%zu,rd_sz:%zu\n",
				Awb_Block->u4DMAReadIdx,
				 DMA_Read_Ptr, read_size);
			return read_count << 2;
		}
		...
		PRINTK_AUD_AWB
		("%s finish 3, size_2:%x, rp:%x, wp:%x remained:%x\n",
...
static struct page *mtk_mrgrx_capture_pcm_page(
