...
enum audio_system_clock_type {
	...
	CLOCK_AUDIO_AFE,
	CLOCK_AUDIO_DAC,
	CLOCK_AUDIO_DAC_PREDIS,
	CLOCK_AUDIO_ADC,
	CLOCK_AUDIO_22M,
	CLOCK_AUDIO_24M,
	CLOCK_AUDIO_I2S,
	CLOCK_AUDIO_APLL_TUNER,
	CLOCK_AUDIO_APLL2_TUNER,
#endif
	CLOCK_CLK26M,
	CLOCK_NUM
};
...
#ifdef CONFIG_SND_I2S_MCLK
	[CLOCK_TOP_APLL1_CK] = {"apll1_clk", false, false, NULL},
	[CLOCK_TOP_APLL2_CK] = {"apll2_clk", false, false, NULL},
	[CLOCK_AUDIO_AFE] = {"aud_afe", true, false, NULL},
	[CLOCK_AUDIO_DAC] = {"aud_dac", true, false, NULL},
	[CLOCK_AUDIO_DAC_PREDIS] = {"aud_dac_predis", true, false, NULL},
	[CLOCK_AUDIO_ADC] = {"aud_adc", true, false, NULL},
	[CLOCK_AUDIO_22M] = {"aud_22m", false, false, NULL},
	[CLOCK_AUDIO_24M] = {"aud_24m", false, false, NULL},
	[CLOCK_AUDIO_I2S] = {"aud_i2s", true, false, NULL},
	[CLOCK_AUDIO_APLL_TUNER] = {"aud_apll_tnr", false, false, NULL},
	[CLOCK_AUDIO_APLL2_TUNER] = {"aud_apll2_tnr", false, false, NULL},
#else
...
#ifdef CONFIG_SND_I2S_MCLK
		ret = clk_enable(aud_clks[CLOCK_AUDIO_AFE].clock);
		if (ret)
			pr_err("%s failed to prepare and enable CLOCK_AUDIO_AFE: %d\n",
				__func__, ret);

		ret = clk_enable(aud_clks[CLOCK_AUDIO_DAC].clock);
		if (ret)
			pr_err("%s failed to prepare and enable CLOCK_AUDIO_DAC: %d\n",
				__func__, ret);

		ret = clk_enable(aud_clks[CLOCK_AUDIO_DAC_PREDIS].clock);
		if (ret)
			pr_err("%s failed to prepare and enable CLOCK_AUDIO_DAC_PREDIS: %d\n",
				__func__, ret);
#else
...
#if defined(COMMON_CLOCK_FRAMEWORK_API)
#ifdef CONFIG_SND_I2S_MCLK
		if (aud_clks[CLOCK_AUDIO_DAC_PREDIS].clk_prepared)
			clk_disable(aud_clks[CLOCK_AUDIO_DAC_PREDIS].clock);
		if (aud_clks[CLOCK_AUDIO_DAC].clk_prepared)
			clk_disable(aud_clks[CLOCK_AUDIO_DAC].clock);
		if (aud_clks[CLOCK_AUDIO_AFE].clk_prepared)
			clk_disable(aud_clks[CLOCK_AUDIO_AFE].clock);
#else
...
#if defined(COMMON_CLOCK_FRAMEWORK_API)
#ifdef CONFIG_SND_I2S_MCLK
		ret = clk_enable(aud_clks[CLOCK_AUDIO_ADC].clock);
		if (ret)
			pr_err("%s: failed to enable CLOCK_ADC: %d\n", __func__, ret);
#else
...
#if defined(COMMON_CLOCK_FRAMEWORK_API)
#ifdef CONFIG_SND_I2S_MCK
		if (aud_clks[CLOCK_AUDIO_ADC].clk_prepared)
			clk_disable(aud_clks[CLOCK_AUDIO_ADC].clock);
#else
...
void AudDrv_ADC2_Clk_On(void)
{
	...
	Aud_ADC2_Clk_cntr++;
	spin_unlock_irqrestore(&auddrv_Clk_lock, flags);
	PRINTK_AUDDRV("-AudDrv_ADC2_Clk_On, Aud_ADC2_Clk_cntr:%d\n", Aud_ADC2_Clk_cntr);
}
...
void AudDrv_ADC2_Clk_Off(void)
{
	...
	spin_unlock_irqrestore(&auddrv_Clk_lock, flags);
	PRINTK_AUDDRV("-AudDrv_ADC2_Clk_Off, Aud_ADC2_Clk_cntr:%d\n", Aud_ADC2_Clk_cntr);
}
...
#ifdef CONFIG_SND_I2S_MCLK
		ret = clk_prepare_enable(aud_clks[CLOCK_AUDIO_22M].clock);
		if (ret)
			pr_err("%s, failed to prepare and enable CLOCK_AUDIO_22M: %d\n",
				__func__, ret);
#else
...
#if defined(COMMON_CLOCK_FRAMEWORK_API)
#ifdef CONFIG_SND_I2S_MCLK
		clk_disable_unprepare(aud_clks[CLOCK_AUDIO_22M].clock);
#else
...
void AudDrv_APLL24M_Clk_On(void)
{
	...
	if (Aud_APLL24M_Clk_cntr == 0) {
		...
		if (ret)
			pr_err("%s, failed to prepare and enable CLOCK_AUDIO_24M: %d\n",
				__func__, ret);
#else
...
#if defined(COMMON_CLOCK_FRAMEWORK_API)
#ifdef CONFIG_SND_I2S_MCK
		clk_disable_unprepare(aud_clks[CLOCK_AUDIO_24M].clock);
#else
...
#if defined(COMMON_CLOCK_FRAMEWORK_API)
#ifdef CONFIG_SND_I2S_MCLK
		ret = clk_enable(aud_clks[CLOCK_AUDIO_I2S].clock);
		if (ret)
			pr_err("%s, failed to enable CLOCK_AUDIO_I2S: %d\n",
				__func__, ret);
#else
...
#if defined(COMMON_CLOCK_FRAMEWORK_API)
#ifdef CONFIG_SND_I2S_MCLK
		clk_disable(aud_clks[CLOCK_AUDIO_I2S].clock);
#else
...
void AudDrv_Core_Clk_On(void)
{
	...
	if (Aud_Core_Clk_cntr == 0) {
		...
		ret = clk_enable(aud_clks[CLOCK_AUDIO_AFE].clock);
		if (ret)
			pr_err("%s, failed to enable CLOCK_AUDIO_AFE: %d\n", __func__, ret);
#else
		Afe_Set_Reg(AUDIO_TOP_CON0, 0 << 2, 1 << 2); /* enable AFE */
#endif
#else
...
void AudDrv_APLL1Tuner_Clk_On(void)
{
	...
	if (Aud_APLL1_Tuner_cntr == 0) {
		ret = clk_prepare(aud_clks[CLOCK_AUDIO_APLL_TUNER].clock);
		if (ret)
			pr_err("%s, failed to prepare CLOCK_AUDIO_APLL_TUNER: %d\n",
				__func__, ret);
		else
			aud_clks[CLOCK_AUDIO_APLL_TUNER].clk_prepared = true;
	}
#endif
	spin_lock_irqsave(&auddrv_Clk_lock, flags);
	if (Aud_APLL1_Tuner_cntr == 0) {
...
#if defined(COMMON_CLOCK_FRAMEWORK_API)
#ifdef CONFIG_SND_I2S_MCLK
		ret = clk_enable(aud_clks[CLOCK_AUDIO_APLL_TUNER].clock);
		if (ret) {
			pr_err("%s, failed to enable CLOCK_AUDIO_APLL_TUNER: %d\n",
				__func__, ret);
		}
#else
...
void AudDrv_APLL1Tuner_Clk_Off(void)
{
	unsigned long flags;
#ifdef CONFIG_SND_I2S_MCLK
	bool bClockDisableActionFlag = false;
#endif
	spin_lock_irqsave(&auddrv_Clk_lock, flags);
	Aud_APLL1_Tuner_cntr--;
	if (Aud_APLL1_Tuner_cntr == 0) {
#if defined(COMMON_CLOCK_FRAMEWORK_API)
#ifdef CONFIG_SND_I2S_MCLK
		clk_disable(aud_clks[CLOCK_AUDIO_APLL_TUNER].clock);
		bClockDisableActionFlag = true;
...
#ifdef CONFIG_SND_I2S_MCLK
	if (bClockDisableActionFlag)
		if (aud_clks[CLOCK_AUDIO_APLL_TUNER].clk_prepared) {
			clk_unprepare(aud_clks[CLOCK_AUDIO_APLL_TUNER].clock);
			aud_clks[CLOCK_AUDIO_APLL_TUNER].clk_prepared = false;
		}
#endif
}
...
void AudDrv_APLL2Tuner_Clk_On(void)
{
	...
	if (Aud_APLL2_Tuner_cntr == 0) {
		ret = clk_prepare(aud_clks[CLOCK_AUDIO_APLL2_TUNER].clock);
		if (ret)
			pr_err("%s, failed to prepare CLOCK_AUDIO_APLL2_TUNER: %d\n",
				__func__, ret);
		else
			aud_clks[CLOCK_AUDIO_APLL2_TUNER].clk_prepared = true;
	}
#endif
	spin_lock_irqsave(&auddrv_Clk_lock, flags);
	if (Aud_APLL2_Tuner_cntr == 0) {
...
#if defined(COMMON_CLOCK_FRAMEWORK_API)
#ifdef CONFIG_SND_I2S_MCLK
		ret = clk_enable(aud_clks[CLOCK_AUDIO_APLL2_TUNER].clock);
		if (ret)
			pr_err("%s, failed to enable CLOCK_AUDIO_APLL2_TUNER: %d\n", __func__, ret);
#else
...
#if defined(COMMON_CLOCK_FRAMEWORK_API)
#ifdef CONFIG_SND_I2S_MCLK
		clk_disable(aud_clks[CLOCK_AUDIO_APLL2_TUNER].clock);
		bClockDisableActionFlag = true;
#else
...
#ifdef CONFIG_SND_I2S_MCLK
	if (bClockDisableActionFlag)
		if (aud_clks[CLOCK_AUDIO_APLL2_TUNER].clk_prepared) {
			clk_unprepare(aud_clks[CLOCK_AUDIO_APLL2_TUNER].clock);
			aud_clks[CLOCK_AUDIO_APLL2_TUNER].clk_prepared = false;
		}
#endif
}
...
#ifdef CONFIG_SND_I2S_MCLK
			clk_disable_unprepare(aud_clks[CLOCK_AUDIO_22M].clock);
			clk_disable_unprepare(aud_clks[CLOCK_AUDIO_APLL_TUNER].clock);
#else
...
#ifdef CONFIG_SND_I2S_MCLK
			clk_disable_unprepare(aud_clks[CLOCK_AUDIO_24M].clock);
			clk_disable_unprepare(aud_clks[CLOCK_AUDIO_APLL2_TUNER].clock);
#else
			Afe_Set_Reg(AUDIO_TOP_CON0, 1 << 9, 1 << 9); /* disable 24M */
...
void AudDrv_Suspend_Clk_On(void)
{
	...
	if (Aud_Core_Clk_cntr > 0) {
		...
		if (Aud_I2S_Clk_cntr > 0)
			...
			ret = clk_enable(aud_clks[CLOCK_AUDIO_I2S].clock);
			if (ret) {
				pr_err("%s, failed to prepare and enable CLOCK_AUDIO_I2S: %d\n",
					__func__, ret);
			}
#else
		...
		if (Aud_ADC_Clk_cntr > 0)
			...
			ret = clk_enable(aud_clks[CLOCK_AUDIO_ADC].clock);
			if (ret) {
				pr_err("%s, failed to prepare and enable CLOCK_AUDIO_ADC: %d\n",
					__func__, ret);
			}
#else
			Afe_Set_Reg(AUDIO_TOP_CON0, 0 << 24, 1 << 24);
		...
		if (Aud_APLL22M_Clk_cntr > 0) {
			...
			ret = clk_prepare_enable(aud_clks[CLOCK_AUDIO_APLL_TUNER].clock);
			if (ret) {
				pr_err("%s, failed to prepare and enable CLOCK_AUDIO_APLL_TUNER: %d\n",
					__func__, ret);
			}
#else
			Afe_Set_Reg(AUDIO_TOP_CON0, 0 << 8, 1 << 8); /* enable 22M */
		...
		if (Aud_APLL24M_Clk_cntr > 0) {
			...
			ret = clk_prepare_enable(aud_clks[CLOCK_AUDIO_APLL2_TUNER].clock);
			if (ret) {
				pr_err("%s, failed to prepare and enable CLOCK_AUDIO_APLL2_TUNER: %d\n",
					__func__, ret);
			}
#else
			Afe_Set_Reg(AUDIO_TOP_CON0, 0 << 9, 1 << 9); /* enable 24M */
