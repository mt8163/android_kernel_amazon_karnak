...
#if !defined(CONFIG_GTP_REQUEST_FW_UPDATE) && !defined(CONFIG_GTP_HEADER_FW_UPDATE)
...
int gt1x_update_firmware(void *filename)
{
	...
	for (i = 1; i < update_info.firmware_info->subsystem_count; i++) {
		GTP_INFO("subsystem: %d", update_info.firmware_info->subsystem[i].type);
		GTP_INFO("Length: %d", update_info.firmware_info->subsystem[i].length);
		GTP_INFO("Address: %d", update_info.firmware_info->subsystem[i].address);
...
int gt1x_check_firmware(void)
{
	...
	update_info.firmware_info->pid[5] = 0;

	p = &p[FW_HEAD_OFFSET_SUBSYSTEM_INFO_BASE];
	firmware_temp = update_info.firmware_info;
	offset = FW_HEAD_SIZE;
	for (i = 0; i < firmware_temp->subsystem_count; i++) {
		firmware_temp->subsystem[i].type = p[i * FW_HEAD_SUBSYSTEM_INFO_SIZE];
		firmware_temp->subsystem[i].length = getU16(&p[i * FW_HEAD_SUBSYSTEM_INFO_SIZE + 1]);
		firmware_temp->subsystem[i].address = getU16(&p[i * FW_HEAD_SUBSYSTEM_INFO_SIZE + 3]) * 256;
		firmware_temp->subsystem[i].offset = offset;
		offset += firmware_temp->subsystem[i].length;
	}
	...
	GTP_INFO("Firmware product: GT%s", update_info.firmware_info->pid);
	GTP_INFO("Firmware patch: %02X%02X%02X", update_info.firmware_info->version[0], update_info.firmware_info->version[1], update_info.firmware_info->version[2]);
	GTP_INFO("Firmware chip: 0x%02X", update_info.firmware_info->chip_type);
	GTP_INFO("Subsystem count: %d", update_info.firmware_info->subsystem_count);
	for (i = 0; i < update_info.firmware_info->subsystem_count; i++) {
		GTP_DEBUG("------------------------------------------");
		GTP_DEBUG("Subsystem: %d", i);
		GTP_DEBUG("Type: %d", update_info.firmware_info->subsystem[i].type);
		GTP_DEBUG("Length: %d", update_info.firmware_info->subsystem[i].length);
		GTP_DEBUG("Address: 0x%08X", update_info.firmware_info->subsystem[i].address);
		GTP_DEBUG("Offset: %d", update_info.firmware_info->subsystem[i].offset);
	}
...
int gt1x_error_erase(void)
{
	...
	fw = gt1x_get_fw_data(update_info.firmware_info->subsystem[0].offset,
