...
#define GTP_MAX_KEY_NUM  \
	(ARRY_SIZE(touch_key_array)/sizeof(touch_key_array[0]))

#if GTP_DEBUG_ON
static const int  key_codes[] = {KEY_HOME,
	KEY_BACK, KEY_MENU, KEY_SEARCH};
static char *key_names[] = {"Key_Home",
	"Key_Back", "Key_Menu", "Key_Search"};
#endif
...
static ssize_t gt91xx_fw_version_read_proc(struct file *file,
	char __user *buffer, size_t size, loff_t *ppos);

static struct proc_dir_entry *gt91xx_fw_version_proc = NULL;
static const struct file_operations fw_version_proc_ops = {
	.owner = THIS_MODULE,
...
static DEVICE_ATTR(suspend, 0200, NULL, gtp_suspend_store);
...
{
	u8  end_cmd[3] = {GTP_READ_COOR_ADDR
		>> 8, GTP_READ_COOR_ADDR & 0xFF, 0};
	...
	"%s slide to light up the screen!",
				...
				/*  clear 0x814B */
				doze_buf[2] = 0x00;
				gtp_i2c_write(i2c_connect_client,
				doze_buf, 3);
	...
	/*  GT9XXF reques event */
	if ((finger == 0x00) && (ts->chip_type
				== CHIP_TYPE_GT9F)) {   /*  request arrived */
		ret = gtp_i2c_read(ts->client, rqst_buf, 3);

		if (ret < 0) {
			GTP_ERROR(
	"Read request status error!");
			goto exit_work_func;
		}
	...
	"Request for config responded!");
		...
		case GTP_RQST_BAK_REF:
			GTP_INFO(
	"Request for backup reference.");
			ts->rqst_processing = 1;
			ret = gtp_bak_ref_proc(ts,
			GTP_BAK_REF_SEND);
...
static enum hrtimer_restart
	goodix_ts_timer_handler(struct hrtimer *timer)
{
 ...
 */
static irqreturn_t
	goodix_ts_irq_handler(int irq, void *dev_id)
{
 ...
 ******************************************************
 */
void gtp_reset_guitar(struct i2c_client *client,
	s32 ms)
{
#if GTP_COMPATIBLE_MODE
	struct goodix_ts_data *ts
...
static s8 gtp_enter_doze(struct goodix_ts_data *ts)
{
	...
	while (retry++ < 5) {
		...
		if (ret < 0) {
			GTP_DEBUG(
	"failed to set doze flag into 0x8046, %d", retry);
			continue;
		}
...
#ifdef GTP_CONFIG_OF
	GTP_DEBUG(
	"Get config dat from device tree.");
...
#else
	GTP_DEBUG(
	"Get config data from header file.");
...
/* You need to check you header file CFG_GROUP section! */
	...
	"Config Version: %d, 0x%02X; IC Config Version: %d, 0x%02X",
			...
			if (flash_cfg_version
			< 90 && flash_cfg_version
			> drv_cfg_version)
	...
	if (ts->chip_type == CHIP_TYPE_GT9F) {
		...
		if (ts->is_950 == 1) {
			driver_num =
			config[GTP_REG_MATRIX_DRVNUM -
				GTP_REG_CONFIG_DATA + 2];
			sensor_num =
			config[GTP_REG_MATRIX_SENNUM -
				GTP_REG_CONFIG_DATA + 2];

			if (have_key)
				driver_num--;

			ts->bak_ref_len = (driver_num *
					(sensor_num - 1) + 2) * 2 * 6;
		} else {
			driver_num = (config[CFG_LOC_DRVA_NUM]
			& 0x1F) +
			...
			sensor_num = (config[CFG_LOC_SENS_NUM]
			& 0x0F)
				+ ((config[CFG_LOC_SENS_NUM] >> 4)
				& 0x0F);
			ts->bak_ref_len = (driver_num
			* (sensor_num - 2)
					+ 2) * 2;
		...
		char __user *buffer, size_t size, loff_t *ppos)
{
	...
	int i;
	int len,err = -1;
	int gtp_data_each_line_cnt = 8;
	int gtp_data_len_cnt = 5;
	char s1[] = "==== GT9XX config init value====\n";
	char s2[] = "==== GT9XX config real value====\n";
	...
	len = (strlen(s1)) + (strlen(s2) + 1) +
			(GTP_CONFIG_MAX_LENGTH/gtp_data_each_line_cnt) *
	...
	gtp_i2c_read(i2c_connect_client, temp_data,
			GTP_CONFIG_MAX_LENGTH + 2);
 ...
 ******************************************************
 */
s32 gtp_read_version(struct i2c_client *client,
	u16 *version)
{
	s32 ret = -1;
	u8 buf[8] = {GTP_REG_VERSION >> 8,
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
	u8 test[3] = {GTP_REG_CONFIG_DATA >> 8,
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	if (ret < 0) {
		...
		GTP_GPIO_AS_INT(gtp_int_gpio);
		ts->client->irq
		= gpio_to_irq(gtp_int_gpio);
	}

	ret = GTP_GPIO_REQUEST(gtp_rst_gpio,
	"GTP RST PORT");
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
	...
	GTP_DEBUG_FUNC();
	GTP_DEBUG(
	"INT trigger type:%x", ts->int_trigger_type);
...
static s32 gtp_bak_ref_proc(struct goodix_ts_data *ts,
	u8 mode)
{
	s32 ret = 0;
	s32 i = 0;
	...
	ts->ref_chk_fs_times, GTP_CHK_FS_MNT_MAX);
		...
		GTP_INFO(
		"check /data mount timeout...");
	} else
		GTP_INFO("/data mounted!!!(%d/%d)",
				ts->ref_chk_fs_times,
				GTP_CHK_FS_MNT_MAX);

	p_bak_ref = kzalloc(ts->bak_ref_len,
	GFP_KERNEL);

	if (p_bak_ref == NULL) {
		GTP_ERROR(
	"Allocate memory for p_bak_ref failed!");
		return FAIL;
	}
	...
	ref_filp = filp_open(GTP_BAK_REF_PATH,
	O_RDWR | O_CREAT, 0666);

	if (IS_ERR(ref_filp)) {
		GTP_ERROR(
	"Failed to open/create %s.", GTP_BAK_REF_PATH);
	...
	case GTP_BAK_REF_SEND:
		...
		if (ret < 0) {
			GTP_ERROR(
	"failed to read bak_ref info from file, sending default bak_ref");
			goto bak_ref_default;
		}
		...
		for (j = 0; j < ref_grps; ++j) {
			...
			learn_cnt = (p_bak_ref[j
			* ref_seg_len
					+ ref_seg_len
					- 4] << 8) +
				(p_bak_ref[j * ref_seg_len
				 + ref_seg_len
				 - 3]);
			chksum = (p_bak_ref[j
			* ref_seg_len
	...
	"wrong chksum for bak_ref, reset to 0x00 bak_ref");
				...
				* ref_seg_len],
				0, ref_seg_len);
				p_bak_ref[ref_seg_len
				+ j * ref_seg_len - 1] = 0x01;
			} else {
				if (j == (ref_grps - 1))
					GTP_INFO(
	...
	default:
		GTP_ERROR(
	"invalid backup-reference request");
		break;
	}
...
bak_ref_default:
	...
	if (!IS_ERR(ref_filp)) {
		GTP_INFO(
	"write backup -reference data into %s",
		GTP_BAK_REF_PATH);
		ref_filp->f_op->llseek(ref_filp,
		0, SEEK_SET);
		ref_filp->f_op->write(ref_filp,
...
static s32 gtp_main_clk_proc(struct goodix_ts_data *ts)
{
	...
	clk_filp = filp_open(GTP_MAIN_CLK_PATH,
			O_RDWR | O_CREAT, 0666);
...
#endif
	GTP_INFO(
	"calibrate main clock: %d", ret);

	if (ret < 50 || ret > 120) {
		GTP_ERROR(
	"wrong main clock: %d", ret);
		goto exit_main_clk;
	}
	...
	if (!IS_ERR(clk_filp)) {
		GTP_DEBUG(
	"write main clock data into %s", GTP_MAIN_CLK_PATH);
		clk_filp->f_op->llseek(clk_filp,
		0, SEEK_SET);
...
update_main_clk:
	ret = i2c_write_bytes(ts->client,
	GTP_REG_MAIN_CLK, p_main_clk, 6);

	if (ret == FAIL) {
		GTP_ERROR(
	"update main clock failed!");
		return FAIL;
	}
...
#ifdef GTP_CONFIG_OF
/*
 * gtp_parse_dt - parse platform infomration form devices tree.
 */
static void gtp_parse_dt(struct device *dev)
{
	struct device_node *np = dev->of_node;

	gtp_int_gpio = of_get_named_gpio(np,
	"goodix,irq-gpio", 0);
	gtp_rst_gpio = of_get_named_gpio(np,
	"goodix,rst-gpio", 0);
 ...
 */
int gtp_parse_dt_cfg(struct device *dev,
	u8 *cfg, int *cfg_len, u8 sid)
{
	struct device_node *np = dev->of_node;
	...
	"goodix,cfg-group%d", sid);
	prop = of_find_property(np,
	cfg_name, cfg_len);

	if (prop && prop->value && *cfg_len
		!= 0 && *cfg_len
		< GTP_CONFIG_MAX_LENGTH) {
 ...
 * return: 0-succeed, -1-faileds
 * notice:   Set vdd_ana to global variant because
 * we need to regulator_put this voltage
 * if we encounter an I2C communication error.
 * That's because the MB has connected another
 * touch IC panel, focaltech maybe.
 */
...
static int gtp_power_switch(struct i2c_client *client, int on)
{
	...
	if (!vdd_ana) {
		vdd_ana = regulator_get(&client->dev,
		"vdd_ana");

		if (IS_ERR(vdd_ana)) {
			GTP_ERROR(
			"regulator get of vdd_ana failed");
			ret = PTR_ERR(vdd_ana);
			vdd_ana = NULL;
			return ret;
		}
	...
	if (!vcc_i2c) {
		vcc_i2c = regulator_get(&client->dev,
		"vcc_i2c");

		if (IS_ERR(vcc_i2c)) {
			GTP_ERROR(
			"regulator get of vcc_i2c failed");
			vcc_i2c = NULL;
		}
	}

	regulator_set_voltage(vdd_ana, 3000000, 3000000);
	/* there are 2 kind of tp design, the one needs both vcc_i2c(1V8) and vdd_ana(3V3)
	 * the other one needs vdd_ana only, and we need to make it compatible
	 */
	if(!vcc_i2c)
		GTP_INFO("no vcc_i2c in dts, no need to control vcc_i2c in this project");
	else
		...
		const struct i2c_device_id *id)
{
	...
	GTP_INFO("GTP Driver Version: %s",
	GTP_DRIVER_VERSION);

	GTP_INFO(
	"GTP Driver Built@08:00:00,Nov 1 2015");
	...
	if (ts == NULL) {
		GTP_ERROR(
		"Alloc GFP_KERNEL memory failed.");
		return -ENOMEM;
	}
...
/*  HZ: clock ticks in 1 second generated by system */
	...
	GTP_DEBUG(
	"Clock ticks for an esd cycle: %d", ts->clk_tick_cnt);
	...
	gt91xx_fw_version_proc
		= proc_create(GT91XX_FW_VERSION_PROC_FILE,
...
{
#if   defined(CONFIG_FB)
	ts->notifier.notifier_call
	= gtp_fb_notifier_callback;
	fb_register_client(&ts->notifier);

#elif defined(CONFIG_HAS_EARLYSUSPEND)
	ts->early_suspend.level
	= EARLY_SUSPEND_LEVEL_BLANK_SCREEN + 1;
	ts->early_suspend.suspend
	= goodix_ts_early_suspend;
	ts->early_suspend.resume
...
#if GTP_ESD_PROTECT
s32 gtp_i2c_read_no_rst(struct i2c_client *client,
	u8 *buf, s32 len)
{
	struct i2c_msg msgs[2];
	s32 ret = -1;
...
s32 gtp_i2c_write_no_rst(struct i2c_client *client,
	u8 *buf, s32 len)
{
	struct i2c_msg msg;
	s32 ret = -1;
...
static s32 gtp_init_ext_watchdog(struct i2c_client *client)
{
	...
	return gtp_i2c_write_no_rst(client,
	opr_buffer, 3);
}
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	for (i = 0; i < 3; i++) {
		ret = gtp_i2c_read_no_rst(ts->client,
		esd_buf, 4);
		GTP_DEBUG(
...
/* IC works normally, Write 0x8040 0xAA, feed the dog */
	...
	"IC working abnormally! Process esd recovery.");
				...
				esd_buf[3] = 0x01;
				esd_buf[4] = 0x01;
				gtp_i2c_write_no_rst(ts->client,
				esd_buf, 5);
				msleep(50);
#ifdef GTP_CONFIG_OF
