...
static s32 gup_init_panel(struct goodix_ts_data *ts)
{
	...
	&config[GTP_ADDR_LENGTH], &ts->gtp_cfg_len, sensor_id);
	...
	memset(&config[GTP_ADDR_LENGTH], 0,
...
static u8 gup_get_ic_msg(struct i2c_client *client,
	u16 addr, u8 *msg, s32 len)
{
	s32 i = 0;
	...
	for (i = 0; i < 5; i++) {
		if (gup_i2c_read(client, msg,
		GTP_ADDR_LENGTH + len) > 0)
			break;
	}

	if (i >= 5) {
		GTP_ERROR("Read data from 0x%02x%02x failed!",
		msg[0], msg[1]);
		return FAIL;
	}
...
static u8 gup_set_ic_msg(struct i2c_client *client,
	u16 addr, u8 val)
{
	s32 i = 0;
	u8 msg[3];
	...
	for (i = 0; i < 5; i++) {
		if (gup_i2c_write(client, msg,
		GTP_ADDR_LENGTH + 1) > 0)
			break;
	}

	if (i >= 5) {
		GTP_ERROR("Set data to 0x%02x%02x failed!",
		msg[0], msg[1]);
		return FAIL;
	}
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
	...
	if (ret == FAIL) {
		GTP_ERROR(
		"[get_ic_fw_msg]get hw_info failed,exit");
		return FAIL;
	}
	...
	GTP_DEBUG("IC force update flag:0x%x",
	...
	if (ret == FAIL) {
		GTP_ERROR(
	"[get_ic_fw_msg]get pid & vid failed,exit");
		return FAIL;
	}

	memset(update_msg.ic_fw_msg.pid, 0,
	sizeof(update_msg.ic_fw_msg.pid));
	memcpy(update_msg.ic_fw_msg.pid,
	&buf[GTP_ADDR_LENGTH], 4);
	GTP_DEBUG("IC Product id:%s",
	...
	if (update_msg.ic_fw_msg.pid[0] != 0) {
		if (!memcmp(update_msg.ic_fw_msg.pid, "9111", 4)) {
			GTP_DEBUG(
			"IC Mapping Product id:%s",
			update_msg.ic_fw_msg.pid);
			memcpy(update_msg.ic_fw_msg.pid, "9110P", 5);
		}
	}

	update_msg.ic_fw_msg.vid = buf[GTP_ADDR_LENGTH
	+ 4] + (buf[GTP_ADDR_LENGTH + 5] << 8);
	GTP_DEBUG("IC version id:%04x",
...
s32 gup_enter_update_mode(struct i2c_client *client)
{
	...
	while (retry++ < 200) {
		...
		/* step5:Confirm hold */
		ret = gup_get_ic_msg(client,
		_rRW_MISCTL__SWRST_B0_, rd_buf, 1);
...
static u8 gup_enter_update_judge(struct st_fw_head *fw_head)
{
	...
	if (!memcmp(fw_head->hw_info,
	update_msg.ic_fw_msg.hw_info,
...
static s8 gup_update_config(struct i2c_client *client)
{
	...
	file_len =
	update_msg.cfg_file->f_op->llseek(update_msg.cfg_file,
	0, SEEK_END);
	...
	file_config = kzalloc(chip_cfg_len
	+ GTP_ADDR_LENGTH, GFP_KERNEL);
	update_msg.cfg_file->f_op->llseek(update_msg.cfg_file, 0, SEEK_SET);
	GTP_DEBUG("[update_cfg]Read config from file.");
	ret = vfs_read(update_msg.cfg_file,
	...
	for (i = 0, file_cfg_len = GTP_ADDR_LENGTH;
	i < count; i += 5) {
	...
	while (i++ < 5) {
		ret = gup_i2c_write(client, file_config,
		file_cfg_len);
...
#if (GTP_AUTO_UPDATE && (!GTP_REQUEST_FW || GTP_AUTO_UPDATE_CFG))
static void gup_search_file(s32 search_type)
{
	...
	for (i = 0; i < GUP_SEARCH_FILE_TIMES; ++i) {
		...
		if (search_type & AUTO_SEARCH_BIN) {
			GTP_DEBUG(
			"Search for %s, %s for fw update.(%d/%d)",
			UPDATE_FILE_PATH_1, UPDATE_FILE_PATH_2,
			i + 1, GUP_SEARCH_FILE_TIMES);
			pfile = filp_open(UPDATE_FILE_PATH_1,
			O_RDONLY, 0);

			if (IS_ERR(pfile)) {
				pfile = filp_open(UPDATE_FILE_PATH_2,
				O_RDONLY, 0);
...
/*		GTP_INFO("Firmware size: %d", update_msg.fw->size); */
		...
		GTP_DEBUG("Firmware actual size: %d(%dK)",
			  ...
			  update_msg.fw_total_len / 1024);
		memcpy(fw_head, &update_msg.fw->data[0],
		FW_HEAD_LENGTH);
		...
		for (i = 0; i < update_msg.fw_total_len; i += 2) {
			fw_checksum +=
			    (update_msg.fw->data[FW_HEAD_LENGTH
				+ i] << 8) +
			    update_msg.fw->data[FW_HEAD_LENGTH
				+ i + 1];
		}

		GTP_DEBUG("firmware checksum:%x",
		fw_checksum & 0xFFFF);
	...
	update_msg.fw_total_len = update_msg.file->f_op->llseek(update_msg.file,
	0, SEEK_END);
	...
	* 4 + FW_DSP_ISP_LENGTH + FW_DSP_LENGTH + FW_BOOT_LENGTH)) {
	...
	update_msg.fw_total_len, update_msg.fw_total_len / 1024);
	update_msg.file->f_op->llseek(update_msg.file,
	0, SEEK_SET);
	ret = vfs_read(update_msg.file,
...
static u8 gup_burn_proc(struct i2c_client *client,
...
{
#if (GTP_AUTO_UPDATE && GTP_REQUEST_FW)
	...
	u8 *chk_src, u16 start_rd_addr, u16 chk_length)
{
	...
	while (recall_length < chk_length) {
		...
		ret = gup_get_ic_msg(client, recall_addr,
		rd_buf, frame_length);
	...
	u8 *fw_section, u16 start_addr, u8 bank_cmd)
{
	...
	if (ret <= 0) {
		GTP_ERROR(
		"[burn_fw_section]hold ss51 & dsp fail.");
		return FAIL;
	}

	/* step2:set scramble */
	ret = gup_set_ic_msg(client,
	_rRW_MISCTL__BOOT_OPT_B0_, 0x00);

	if (ret <= 0) {
		GTP_ERROR(
	"[burn_fw_section]set scramble fail.");
		return FAIL;
	}
	...
	ret = gup_set_ic_msg(client,
	_rRW_MISCTL__BOOT_CTL_, bank_cmd & 0x0f);
	...
	do {
		ret = gup_get_ic_msg(client,
		_rRW_MISCTL__BOOT_CTL_, rd_buf, 1);
...
/* GTP_DEBUG("[burn_fw_section]Get burn state:%d.", rd_buf[GTP_ADDR_LENGTH]); */
	...
	if (ret <= 0) {
		GTP_ERROR(
	"[burn_fw_section]select bank %d fail.", (bank_cmd >> 4) & 0x0F);
		return FAIL;
	}
...
static u8 gup_burn_dsp_isp(struct i2c_client *client)
{
	...
	if (retry >= 5) {
		GTP_ERROR(
	"[burn_dsp_isp]Alloc memory fail,exit.");
		return FAIL;
	}
	...
	GTP_DEBUG(
	"[burn_dsp_isp]step5:set boot from sram");
	ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOTCTL_B0_, 0x02);

	if (ret <= 0) {
		GTP_ERROR(
	"[burn_dsp_isp]set boot from sram fail.");
		ret = FAIL;
		goto exit_burn_dsp_isp;
	}
	...
	ret = gup_set_ic_msg(client,
	_bWO_MISCTL__CPU_SWRST_PULSE, 0x01);
	...
	ret = gup_burn_proc(client,
	fw_dsp_isp, 0xC000, FW_DSP_ISP_LENGTH);
	...
	+= FW_DSP_ISP_LENGTH;
	GTP_DEBUG("[burn_dsp_isp]Burned length:%d",
	update_msg.fw_burned_len);
	ret = SUCCESS;
exit_burn_dsp_isp:
	kfree(fw_dsp_isp);
...
static u8 gup_burn_fw_ss51(struct i2c_client *client)
{
	...
	GTP_INFO(
	"[burn_fw_ss51]Begin burn ss51 firmware---->>");
	/* step1:alloc memory */
	...
	ret = gup_set_ic_msg(client,
	_rRW_MISCTL__BOOT_CTL_, 0x00);

	if (ret <= 0) {
		GTP_ERROR(
	"[burn_fw_ss51]clear control flag fail.");
		ret = FAIL;
	...
	ret = gup_burn_fw_section(client,
	fw_ss51, 0xC000, 0x13);
	...
	update_msg.fw_burned_len
	+= (FW_SECTION_LENGTH * 4);
	GTP_DEBUG("[burn_fw_ss51]Burned length:%d",
	update_msg.fw_burned_len);
	ret = SUCCESS;
exit_burn_fw_ss51:
	kfree(fw_ss51);
...
static u8 gup_burn_fw_dsp(struct i2c_client *client)
{
	...
	if (retry >= 5) {
		GTP_ERROR(
		"[burn_fw_dsp]Alloc memory fail,exit.");
		return FAIL;
	}
	...
	if (ret <= 0) {
		GTP_ERROR(
	"[burn_fw_dsp]hold ss51 & dsp fail.");
		ret = FAIL;
		goto exit_burn_fw_dsp;
	}
	...
	GTP_DEBUG(
	"[burn_fw_dsp]step6: release ss51 & dsp");
	ret = gup_set_ic_msg(client,
	_rRW_MISCTL__SWRST_B0_, 0x04);                 /* 20121211 */

	if (ret <= 0) {
		GTP_ERROR(
	"[burn_fw_dsp]release ss51 & dsp fail.");
		ret = FAIL;
		goto exit_burn_fw_dsp;
	}
	...
	GTP_DEBUG(
	"[burn_fw_dsp]step7: burn 4k dsp firmware");
	ret = gup_burn_proc(client,
	fw_dsp, 0x9000, FW_DSP_LENGTH);

	if (ret == FAIL) {
		GTP_ERROR(
	"[burn_fw_dsp]burn fw_section fail.");
		goto exit_burn_fw_dsp;
	}
	...
	ret = gup_set_ic_msg(client,
	_rRW_MISCTL__BOOT_CTL_, 0x05);

	if (ret <= 0) {
		GTP_ERROR(
	"[burn_fw_dsp]send burn cmd fail.");
		goto exit_burn_fw_dsp;
	}

	GTP_DEBUG(
	"[burn_fw_dsp]Wait for the burn is complete......");
	...
	do {
		...
		if (ret <= 0) {
			GTP_ERROR(
	"[burn_fw_dsp]Get burn state fail");
			goto exit_burn_fw_dsp;
		}
...
static u8 gup_burn_fw_boot(struct i2c_client *client)
{
	...
	while (retry++ < 5) {
		fw_boot = kzalloc(FW_BOOT_LENGTH,
		GFP_KERNEL);
	...
	if (retry >= 5) {
		GTP_ERROR(
	"[burn_fw_boot]Alloc memory fail,exit.");
		return FAIL;
	}

	/* step2:load firmware bootloader */
	GTP_DEBUG(
	"[burn_fw_boot]step2:load firmware bootloader");
	...
	/* step3:hold ss51 & dsp */
	GTP_DEBUG(
	"[burn_fw_boot]step3:hold ss51 & dsp");
	ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x0C);

	if (ret <= 0) {
		GTP_ERROR(
	"[burn_fw_boot]hold ss51 & dsp fail.");
		ret = FAIL;
		goto exit_burn_fw_boot;
	}
	...
	/* step4:set scramble */
	GTP_DEBUG(
	"[burn_fw_boot]step4:set scramble");
	ret = gup_set_ic_msg(client,
	_rRW_MISCTL__BOOT_OPT_B0_, 0x00);

	if (ret <= 0) {
		GTP_ERROR(
	"[burn_fw_boot]set scramble fail.");
		ret = FAIL;
		goto exit_burn_fw_boot;
	}
	...
	GTP_DEBUG(
	"[burn_fw_boot]step5:hold ss51 & release dsp");
	ret = gup_set_ic_msg(client,
	_rRW_MISCTL__SWRST_B0_, 0x04);                 /* 20121211 */

	if (ret <= 0) {
		GTP_ERROR(
	"[burn_fw_boot]release ss51 & dsp fail.");
		ret = FAIL;
		goto exit_burn_fw_boot;
	}
	...
	if (ret <= 0) {
		GTP_ERROR(
	"[burn_fw_boot]select bank3 fail.");
		ret = FAIL;
		goto exit_burn_fw_boot;
	}
	...
	ret = gup_burn_proc(client,
	fw_boot, 0x9000, FW_BOOT_LENGTH);

	if (ret == FAIL) {
		GTP_ERROR(
	"[burn_fw_boot]burn fw_boot fail.");
		goto exit_burn_fw_boot;
	}
	...
	update_msg.fw_burned_len
	+= FW_BOOT_LENGTH;
	GTP_DEBUG("[burn_fw_boot]Burnedlength:%d",
	update_msg.fw_burned_len);
	ret = SUCCESS;
exit_burn_fw_boot:
	kfree(fw_boot);
...
static u8 gup_burn_fw_boot_isp(struct i2c_client *client)
{
	...
	while (retry++ < 5) {
		fw_boot_isp = kzalloc(FW_BOOT_ISP_LENGTH,
		GFP_KERNEL);
	...
	/* step3:hold ss51 & dsp */
	GTP_DEBUG(
	"[burn_fw_boot_isp]step3:hold ss51 & dsp");
	ret = gup_set_ic_msg(client,
	_rRW_MISCTL__SWRST_B0_, 0x0C);

	if (ret <= 0) {
		GTP_ERROR(
	"[burn_fw_boot_isp]hold ss51 & dsp fail.");
		ret = FAIL;
		goto exit_burn_fw_boot_isp;
	}
	...
	/* step4:set scramble */
	GTP_DEBUG(
	"[burn_fw_boot_isp]step4:set scramble");
	ret = gup_set_ic_msg(client,
	_rRW_MISCTL__BOOT_OPT_B0_, 0x00);

	if (ret <= 0) {
		GTP_ERROR(
	"[burn_fw_boot_isp]set scramble fail.");
		ret = FAIL;
		goto exit_burn_fw_boot_isp;
	}
	...
	/* step6:select bank3 */
	GTP_DEBUG(
	"[burn_fw_boot_isp]step6:select bank3");
	ret = gup_set_ic_msg(client,
	_bRW_MISCTL__SRAM_BANK, 0x03);

	if (ret <= 0) {
		GTP_ERROR(
	"[burn_fw_boot_isp]selectbank3 fail.");
		ret = FAIL;
		goto exit_burn_fw_boot_isp;
	}
	...
	ret = gup_burn_proc(client,
	fw_boot_isp, 0x9000, FW_BOOT_ISP_LENGTH);
	...
	ret = gup_set_ic_msg(client,
	_rRW_MISCTL__BOOT_CTL_, 0x07);

	if (ret <= 0) {
		GTP_ERROR(
	"[burn_fw_boot_isp]send burn cmd fail.");
		goto exit_burn_fw_boot_isp;
	}
...
static u8 gup_burn_fw_link(struct i2c_client *client)
{
	...
	>= update_msg.fw_total_len) {
		GTP_DEBUG(
	"No need to upgrade the link code");
		return SUCCESS;
	}

	GTP_INFO(
	"[burn_fw_link]Begin burn link firmware---->>");
	/* step1:Alloc memory */
	GTP_DEBUG("[burn_fw_link]step1:Alloc memory");

	while (retry++ < 5) {
		fw_link = kzalloc(FW_SECTION_LENGTH,
		GFP_KERNEL);
	...
	if (retry >= 5) {
		GTP_ERROR(
	"[burn_fw_link]Alloc memory fail, exit.");
	...
	offset = update_msg.fw_burned_len
	- FW_DSP_ISP_LENGTH;
	ret = gup_load_section_file(fw_link,
	offset, FW_SECTION_LENGTH, SEEK_SET);
	...
	fw_link, 0x9000, FW_SECTION_LENGTH, 0x38);
	...
	ret = gup_load_section_file(fw_link,
	offset, FW_GLINK_LENGTH - FW_SECTION_LENGTH, SEEK_SET);
	...
	ret = gup_burn_fw_gwake_section(client,
	fw_link, 0x9000, FW_GLINK_LENGTH
	- FW_SECTION_LENGTH, 0x39);
	...
	GTP_DEBUG("[burn_fw_link]Burned length : %d",
	update_msg.fw_burned_len);
	ret = SUCCESS;
exit_burn_fw_link:
	kfree(fw_link);
...
static u8 gup_burn_fw_gwake(struct i2c_client *client)
{
	...
	+ FW_DSP_LENGTH + FW_BOOT_LENGTH + FW_BOOT_ISP_LENGTH
	...
	ret = gup_burn_fw_gwake_section(client,
	fw_gwake, 0x9000, FW_SECTION_LENGTH, 0x3C);
	...
	ret = gup_burn_fw_gwake_section(client,
	fw_gwake, 0x9000, FW_SECTION_LENGTH, 0x3D);
	...
	GTP_DEBUG("[burn_fw_gwake]Burned length : %d",
	update_msg.fw_burned_len);
	ret = SUCCESS;
exit_burn_fw_gwake:
	kfree(fw_gwake);
...
static u8 gup_burn_fw_finish(struct i2c_client *client)
{
	...
	GTP_DEBUG(
	"[burn_fw_finish]step2 : burn ss51 first 8K.");
	...
	GTP_DEBUG(
	"[burn_fw_finish]step6 : release ss51 & hold dsp");
...
s32 gup_update_proc(void *dir)
{
	...
	if (searching_file) {
		...
		GTP_INFO(
	"Exiting searching .bin update file...");
	...
	if (ts->chip_type == CHIP_TYPE_GT9F)
		return gup_fw_download_proc(dir,
	GTP_FL_FW_BURN);
...
#if (GTP_AUTO_UPDATE && GTP_AUTO_UPDATE_CFG && GTP_REQUEST_FW)
...
u8 gup_check_fs_mounted(char *path_name)
{
	...
	err = kern_path("/", LOOKUP_FOLLOW,
	&root_path);

	if (err) {
		GTP_DEBUG("\"/\" NOT Mounted: %d",
		err);
		return FAIL;
	}

	err = kern_path(path_name,
	LOOKUP_FOLLOW, &path);

	if (err) {
		GTP_DEBUG("%s NOT Mounted: %d",
		path_name, err);
		err = FAIL;
		goto exit_chk;
	}
...
s32 i2c_write_bytes(struct i2c_client *client,
	u16 addr, u8 *buf, s32 len)
{
	...
	while (len > 0) {
		...
		for (retry = 0; retry < 5; ++retry) {
			...
			i2c_opr_buf, write_bytes + GTP_ADDR_LENGTH);
...
s32 i2c_read_bytes(struct i2c_client *client,
	u16 addr, u8 *buf, s32 len)
{
	s32 ret = 0;
	s32 read_bytes = 0;
...
static void gup_clk_count_init(s32 bCh, s32 bCNT)
{
	...
	_bRW_MISCTL__MEA_MODE, &buf, 1);
	/* _bRW_MISCTL__MEA_SRCSEL = 8 + bCh;  From GIO1 */
	buf = 8 + bCh;
	i2c_write_bytes(i2c_connect_client,
	_bRW_MISCTL__MEA_SRCSEL, &buf, 1);

	buf = bCNT;
	i2c_write_bytes(i2c_connect_client,
	_wRW_MISCTL__MEA_MAX_NUM, &buf, 1);
...
s32 gup_hold_ss51_dsp(struct i2c_client *client)
{
	...
	while (retry++ < 200) {
		...
		if (ret <= 0) {
			GTP_DEBUG(
	"Hold ss51 & dsp I2C error,retry:%d", retry);
			continue;
		}
	...
	if (ret <= 0) {
		GTP_ERROR(
	"[enter_update_mode]disable wdt fail.");
		return FAIL;
	}
	...
	if (ret <= 0) {
		GTP_ERROR(
	"[enter_update_mode]clear cache enable fail.");
		return FAIL;
	}

	/* set boot from sram */
	ret = gup_set_ic_msg(client,
	_rRW_MISCTL__BOOTCTL_B0_, 0x02);

	if (ret <= 0) {
		GTP_ERROR(
	"[enter_update_mode]set boot from sram fail.");
		return FAIL;
	}
	...
	if (ret <= 0) {
		GTP_ERROR(
	"[enter_update_mode]software reboot fail.");
		return FAIL;
	}
...
s32 gup_enter_update_mode_fl(struct i2c_client *client)
{
	...
	if (ret <= 0) {
		GTP_ERROR(
	"[enter_update_mode]hold ss51 & dsp failed.");
		return FAIL;
	}
	...
	if (ret <= 0) {
		GTP_ERROR(
	"[enter_update_mode]clear control flag fail.");
		return FAIL;
	}
	...
	if (ret <= 0) {
		GTP_ERROR(
	"[enter_update_mode]set scramble fail.");
		return FAIL;
	}
	...
	if (ret <= 0) {
		GTP_ERROR(
	"[enter_update_mode]enable accessing code fail.");
		return FAIL;
	}
	...
	u8 dwn_mode)
{
	...
	if (dwn_mode == GTP_FL_FW_BURN) {
		GTP_INFO(
	"[download_fw_dsp]Begin download dsp fw---->>");

		if (ret <= 0) {
			GTP_ERROR(
	"[download_fw_dsp]select bank2 fail.");
			return FAIL;
		}
		...
		if (ret == FAIL) {
			GTP_ERROR(
	"[download_fw_dsp]download FW dsp fail.");
			return FAIL;
		}
...
static s32 gup_burn_fw_proc(struct i2c_client *client,
	u16 start_addr, s32 start_index, s32 burn_len)
{
	s32 ret = 0;
...
static s32 gup_check_and_repair(struct i2c_client *client,
	u16 start_addr, s32 start_index, s32 chk_len)
{
	s32 ret = 0;
	s32 cmp_len = 0;
	...
	GTP_DEBUG("checked firmware: start 0x%04X, %d bytes",
	start_addr, chk_len);

	while ((chk_len > 0) && (checked_times < GTP_CHK_FW_MAX)) {
		if (chk_len >= GUP_FW_CHK_SIZE)
			cmp_len = GUP_FW_CHK_SIZE;
		else
			cmp_len = chk_len;

		ret = i2c_read_bytes(client, cmp_addr,
		chk_cmp_buf, cmp_len);
		...
		for (i = 0; i < cmp_len; ++i) {
			if (chk_cmp_buf[i] != gtp_default_FW_fl[FW_HEAD_LENGTH
			+ start_index + i]) {
				chk_fail = 1;
				...
				GTP_ERROR(
	"Checked failed index : %d(%d != %d),redownload chuck",
	i, chk_cmp_buf[i], gtp_default_FW_fl[FW_HEAD_LENGTH
					+ start_index + i]);
				break;
			}
		}
...
static u8 gup_download_fw_ss51(struct i2c_client *client, u8 dwn_mode)
{
	...
	for (section = 1; section <= 4; section += 2) {
		...
		* FW_SS51_SECTION_LEN;
		GTP_DEBUG(
	"download firmware ss51: select bank%d", bank);
		ret = gup_set_ic_msg(client,
		_bRW_MISCTL__SRAM_BANK, bank);

		if (dwn_mode == GTP_FL_FW_BURN) {
			GTP_DEBUG(
	"download firmware ss51 section%d & %d", section, section + 1);
			ret = gup_burn_fw_proc(client,
			burn_addr, start_index, 2 * FW_SS51_SECTION_LEN);

			if (ret == FAIL) {
				GTP_ERROR(
	"download fw ss51 section%d & %d failed!", section, section + 1);
				return FAIL;
			}

			GTP_DEBUG(
	"check firmware ss51 section%d & %d", section, section + 1);
			ret = gup_check_and_repair(client,
			burn_addr, start_index, 2 * FW_SS51_SECTION_LEN);

			if (ret == FAIL) {
				GTP_ERROR(
	"check ss51 section%d & %d failed!", section, section + 1);
				return FAIL;
			}
		} else if (dwn_mode == GTP_FL_ESD_RECOVERY) {
	...
	section, section + 1);
				...
				if (ret == FAIL) {
					GTP_ERROR(
	"download fw ss51 section%d failed!", section);
					return FAIL;
				}
			}
		} else {
...
static s32 gup_prepare_fl_fw(char *path, struct st_fw_head *fw_head)
{
	...
	"Force terminate auto update for GT9XXF...");
			...
			GTP_DEBUG(
	"Search for %s, %s for fw update.(%d/%d)", FL_UPDATE_PATH,
			FL_UPDATE_PATH_SD, i + 1, GUP_SEARCH_FILE_TIMES);
			update_msg.file = filp_open(FL_UPDATE_PATH,
			O_RDONLY, 0);
	...
	sizeof(gtp_default_FW_fl), sizeof(gtp_default_FW_fl) / 1024,
	update_msg.fw_total_len, update_msg.fw_total_len / 1024);
		set_fs(update_msg.old_fs);
		_CLOSE_FILE(update_msg.file);
		return FAIL;
	...
	if (ret < 0) {
		GTP_ERROR(
		"read %s failed, err-code: %d", path, ret);
		return FAIL;
	}
	...
	struct st_fw_head *fw_head, char *path)
{
	...
	for (i = FW_HEAD_LENGTH; i
	< update_msg.fw_total_len; i += 2)
		fw_checksum += (gtp_default_FW_fl[i] << 8)
	+ gtp_default_FW_fl[i + 1];
	...
	u8 dwn_mode)
{
	...
	if (dir == NULL) {
		if (dwn_mode == GTP_FL_FW_BURN)
			GTP_INFO(
	"[fw_download_proc]Begin fw download ......");
	...
	"[fw_download_proc]Being fw repair check......");
		...
		GTP_INFO(
	"[fw_download_proc]Begin firmware update by bin file");
	...
	if (ret == FAIL) {
		GTP_ERROR(
	"[fw_download_proc]check update file fail.");
		goto file_fail;
	}
	...
	if (ret == FAIL) {
		GTP_ERROR(
	"[fw_download_proc]enter update mode fail.");
		goto download_fail;
	}

	while (retry++ < 5) {
		ret = gup_download_fw_ss51(i2c_connect_client,
		dwn_mode);
		show_len = 60;

		if (ret == FAIL) {
			GTP_ERROR(
	"[fw_download_proc]burn ss51 firmware fail.");
			continue;
		}
		...
		if (ret == FAIL) {
			GTP_ERROR(
	"[fw_download_proc]burn dsp firmware fail.");
			continue;
		}

		GTP_INFO(
	"[fw_download_proc]UPDATE SUCCESS.");
		break;
	}

	if (retry >= 5) {
		GTP_ERROR(
	"[fw_download_proc]retry timeout,UPDATE FAIL.");
		goto download_fail;
	}
