...
/*if original value is not the value needed,write the register to change*/
		...
		for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++) {
			for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++) {
				...
				RawDataMin =
					g_stCfg_MCap_DetailThreshold.RawDataTest_High_Min[iRow]
					[iCol];
				RawDataMax =
					g_stCfg_MCap_DetailThreshold.RawDataTest_High_Max[iRow]
					[iCol];
...
/*if original value is not the value needed,write the register to change*/
		...
		if (g_stCfg_FT5X46_BasicThreshold.RawDataTest_SetLowFreq) {
			 ...
			 */
			for (index = 0, nRawDataOK = 0; index < 10 && nRawDataOK < 3;
				 ++index) {
				ReCode = GetRawData();
				if (ReCode != ERROR_CODE_OK) {
					FTS_TEST_DBG
			...
			for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++) {
				...
				for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++) {
					if (g_stCfg_MCap_DetailThreshold.InvalidNode[iRow][iCol] ==
						...
						continue;	/*Invalid Node */
					RawDataMin =
						g_stCfg_MCap_DetailThreshold.RawDataTest_Low_Min[iRow]
						[iCol];
					RawDataMax =
						g_stCfg_MCap_DetailThreshold.RawDataTest_Low_Max[iRow]
						[iCol];
					iValue = m_RawData[iRow][iCol];
					if (iValue < RawDataMin || iValue > RawDataMax) {
						btmpresult = false;
						FTS_TEST_DBG
							("rawdata test failure. Node=(%d, %d), Get_value=%d, Set_Range=(%d, %d) ",
							 iRow + 1, iCol + 1, iValue, RawDataMin,
							 RawDataMax);
					}
				}
			}
...
/*	int ibiggerValue = 0;*/
	...
	TestResultLen +=
		sprintf(TestResult + TestResultLen, " SCap RawData Test is %s. \n\n",
				(btmpresult ? "OK" : "NG"));
...
unsigned char FT5X46_TestItem_SCapCbTest(bool *bTestResult)
{
	...
	for (i = 0; i < 3; i++) {
		...
		ReCode =
			GetTxSC_CB(g_ScreenSetParam.iTxNum + g_ScreenSetParam.iRxNum + 128,
					   m_ucTempData);
		if (ReCode != ERROR_CODE_OK) {
						  ...
						  int ByteNum, int *pRevBuffer)
{
	...
	if (ReCode == ERROR_CODE_OK) {
		focal_msleep(10);
		ReCode =
			Comm_Base_IIC_IO(I2C_wBuffer, 1, m_ucTempData, BytesNumInTestMode1);
		if (ReCode != ERROR_CODE_OK) {
	...
	for (i = 1; i < iReadNum; i++) {
		...
		if (i == iReadNum - 1) {	/*last packet */
			focal_msleep(10);
			ReCode =
				Comm_Base_IIC_IO(NULL, 0, m_ucTempData + BYTES_PER_TIME * i,
	...
	if (ReCode == ERROR_CODE_OK) {
		for (i = 0; i < (ByteNum >> 1); i++) {
			pRevBuffer[i] =
				(m_ucTempData[i << 1] << 8) + m_ucTempData[(i << 1) + 1];
...
static unsigned char GetRawData(void)
{
	...
	for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++) {
		for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++) {
			m_RawData[iRow][iCol] =
				m_iTempRawData[iRow * g_ScreenSetParam.iRxNum + iCol];
		}
	}
	return ReCode;
}
...
unsigned char FT5X46_TestItem_PanelDifferTest(bool *bTestResult)
{
	...
	for (i = 0; i < g_ScreenSetParam.iTxNum; i++) {
		...
		for (j = 0; j < g_ScreenSetParam.iRxNum; j++) {
			...
			if (NODE_AST_TYPE == g_stCfg_MCap_DetailThreshold.InvalidNode[i][j]
				|| NODE_INVALID_TYPE ==
	...
	AvgValue =
		AvgValue / (g_ScreenSetParam.iTxNum * g_ScreenSetParam.iRxNum -
					InvalidNum);
	FTS_TEST_DBG("PanelDiffer:Max: %d, Min: %d, Avg: %d ", maxValue, minValue,
				 AvgValue);
	...
	TestResultLen +=
		sprintf(TestResult + TestResultLen, " Panel Differ Test is %s. \n\n",
				(btmpresult ? "OK" : "NG"));
