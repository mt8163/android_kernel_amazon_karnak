...
int malloc_struct_DetailThreshold(void)
{
	...
	g_stCfg_Incell_DetailThreshold.InvalidNode =
		(unsigned char (*)[RX_NUM_MAX])fts_malloc(NUM_MAX *
												  sizeof(unsigned char));
	...
	g_stCfg_Incell_DetailThreshold.CBTest_Min =
		(int (*)[RX_NUM_MAX])fts_malloc(NUM_MAX * sizeof(int));
	if (NULL == g_stCfg_Incell_DetailThreshold.CBTest_Min) {
		FTS_TEST_DBG("CBTest_Min. \n");
		goto ERR;
	}
	g_stCfg_Incell_DetailThreshold.CBTest_Max =
		(int (*)[RX_NUM_MAX])fts_malloc(NUM_MAX * sizeof(int));
...
void OnGetTestItemParam(char *strItemName, char *strIniFile, int iDefautValue)
{
	...
	GetPrivateProfileString("Basic_Threshold", strItemName, str_tmp, strValue,
							strIniFile);
...
void OnInit_InvalidNode(char *strIniFile)
{
	...
	for (i = 0; i < TX_NUM_MAX; i++) {
		for (j = 0; j < RX_NUM_MAX; j++) {
			...
			GetPrivateProfileString("INVALID_NODE", strTemp, "1", str,
									strIniFile);
			if (fts_atoi(str) == 0) {
	...
	for (i = 0; i < 2; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			...
			GetPrivateProfileString("INVALID_NODES", strTemp, "1", str,
									strIniFile);
			if (fts_atoi(str) == 0) {
...
void OnInit_DThreshold_RawDataTest(char *strIniFile)
{
	...
	if ((g_ScreenSetParam.iSelectedIC >> 4 == IC_FT8716 >> 4)
		|| (g_ScreenSetParam.iSelectedIC >> 4 == IC_FT8736 >> 4)
		|| (g_ScreenSetParam.iSelectedIC >> 4 == IC_FTE716 >> 4)) {
		return;
	}

/****************RawData Test**********************************************/
	GetPrivateProfileString("Basic_Threshold", "RawDataTest_Max", "10000", str,
							strIniFile);
...
/*******************FTS_TEST_DBG("GetPrivateProfileString = %d ", dividerPos);*/
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.RawDataTest_Max[i][k] =
					(short)(fts_atoi(str_tmp));
				g_stCfg_Incell_DetailThreshold.RawDataTest_Max[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	GetPrivateProfileString("Basic_Threshold", "RawDataTest_Min", "7000", str,
							strIniFile);
	...
	for (i = 0; i < g_ScreenSetParam.iUsedMaxTxNum; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.RawDataTest_Min[i][k] =
					(short)(fts_atoi(str_tmp));
				g_stCfg_Incell_DetailThreshold.RawDataTest_Min[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
/********************RawData Test Low****************************************/
	...
	for (i = 0; i < g_ScreenSetParam.iUsedMaxTxNum; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.RawDataTest_Low_Max[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < g_ScreenSetParam.iUsedMaxTxNum; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.RawDataTest_Low_Min[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
/******************RawData Test High******************************************************/
	...
	for (i = 0; i < g_ScreenSetParam.iUsedMaxTxNum; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.RawDataTest_High_Max[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < g_ScreenSetParam.iUsedMaxTxNum; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.RawDataTest_High_Min[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_SCapRawDataTest(char *strIniFile)
{
	...
	/*********OFF***********/
	GetPrivateProfileString("Basic_Threshold", "SCapRawDataTest_OFF_Min", "150",
							str, strIniFile);
	MinValue = fts_atoi(str);
	...
	for (i = 0; i < 2; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			g_stCfg_MCap_DetailThreshold.SCapRawDataTest_OFF_Max[i][j] =
				MaxValue;
		}
	}
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.SCapRawDataTest_OFF_Max[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 2; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			g_stCfg_MCap_DetailThreshold.SCapRawDataTest_OFF_Min[i][j] =
				MinValue;
		}
	}
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.SCapRawDataTest_OFF_Min[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	/******************ON********************/
	GetPrivateProfileString("Basic_Threshold", "SCapRawDataTest_ON_Min", "150",
							str, strIniFile);
	MinValue = fts_atoi(str);
	GetPrivateProfileString("Basic_Threshold", "SCapRawDataTest_ON_Max", "1000",
							str, strIniFile);
	MaxValue = fts_atoi(str);
	...
	for (i = 0; i < 2; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			g_stCfg_MCap_DetailThreshold.SCapRawDataTest_ON_Max[i][j] =
				MaxValue;
		}
	}
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.SCapRawDataTest_ON_Max[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 2; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			g_stCfg_MCap_DetailThreshold.SCapRawDataTest_ON_Min[i][j] =
				MinValue;
		}
	}
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.SCapRawDataTest_ON_Min[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_SCapCbTest(char *strIniFile)
{
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.SCapCbTest_ON_Max[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.SCapCbTest_ON_Min[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.SCapCbTest_OFF_Max[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 2; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.SCapCbTest_OFF_Min[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_PanelDifferTest(char *strIniFile)
{
	...
	for (i = 0; i < TX_NUM_MAX; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.PanelDifferTest_Max[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < TX_NUM_MAX; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.PanelDifferTest_Min[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_RxLinearityTest(char *strIniFile)
{
	...
	/*Rx_Linearity Test */
	GetPrivateProfileString("Basic_Threshold", "RxLinearityTest_Max", "50", str,
							strIniFile);
	MaxValue = fts_atoi(str);
	...
	for (i = 0; i < g_ScreenSetParam.iUsedMaxTxNum; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.RxLinearityTest_Max[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_TxLinearityTest(char *strIniFile)
{
	...
	/*Tx_Linearity Test */
	GetPrivateProfileString("Basic_Threshold", "TxLinearityTest_Max", "50", str,
							strIniFile);
	MaxValue = fts_atoi(str);
	...
	for (i = 0; i < g_ScreenSetParam.iUsedMaxTxNum; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.TxLinearityTest_Max[i][k] =
					(short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_ForceTouch_SCapRawDataTest(char *strIniFile)
{
	...
	MinValue = fts_atoi(str);
	GetPrivateProfileString("Basic_Threshold",
							"ForceTouch_SCapRawDataTest_OFF_Max", "1000", str,
							strIniFile);
	MaxValue = fts_atoi(str);
	...
	for (i = 0; i < 1; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			g_stCfg_MCap_DetailThreshold.ForceTouch_SCapRawDataTest_OFF_Max[i]
				[j] = MaxValue;
		}
	}
	for (i = 0; i < 1; i++) {
	...
	for (i = 0; i < 1; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			g_stCfg_MCap_DetailThreshold.ForceTouch_SCapRawDataTest_OFF_Min[i]
				[j] = MinValue;
		}
	}
	for (i = 0; i < 1; i++) {
	...
	MinValue = fts_atoi(str);
	GetPrivateProfileString("Basic_Threshold",
							"ForceTouch_SCapRawDataTest_ON_Max", "1000", str,
							strIniFile);
	MaxValue = fts_atoi(str);
	...
	for (i = 0; i < 1; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			g_stCfg_MCap_DetailThreshold.ForceTouch_SCapRawDataTest_ON_Max[i][j]
				= MaxValue;
		}
	}
	for (i = 0; i < 1; i++) {
		...
		dividerPos =
			GetPrivateProfileString("SpecialSet", str, "NULL", strTemp,
									strIniFile);
		/*FTS_TEST_DBG("%s:%s\r", str, strTemp); */
		sprintf(strValue, "%s", strTemp);
		if (0 == dividerPos)
			continue;
		index = 0;
	...
	for (i = 0; i < 1; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			g_stCfg_MCap_DetailThreshold.ForceTouch_SCapRawDataTest_ON_Min[i][j]
				= MinValue;
		}
	}
	for (i = 0; i < 1; i++) {
...
void OnInit_DThreshold_ForceTouch_SCapCbTest(char *strIniFile)
{
	...
	FTS_TEST_FUNC_ENTER();
	 ...
	 /*ON*/
		GetPrivateProfileString("Basic_Threshold",
								...
								strIniFile);
	MinValue = fts_atoi(str);
	GetPrivateProfileString("Basic_Threshold", "ForceTouch_SCapCbTest_ON_Max",
							"240", str, strIniFile);
	MaxValue = fts_atoi(str);
	...
	for (i = 0; i < 1; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			g_stCfg_MCap_DetailThreshold.ForceTouch_SCapCbTest_ON_Max[i][j] =
				MaxValue;
		}
	}
	for (i = 0; i < 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.ForceTouch_SCapCbTest_ON_Max[i][k]
					= (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 1; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			g_stCfg_MCap_DetailThreshold.ForceTouch_SCapCbTest_ON_Min[i][j] =
				MinValue;
		}
	}
	for (i = 0; i < 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.ForceTouch_SCapCbTest_ON_Min[i][k]
					= (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	 ...
	 /*OFF*/
		GetPrivateProfileString("Basic_Threshold",
								...
								strIniFile);
	MinValue = fts_atoi(str);
	GetPrivateProfileString("Basic_Threshold", "ForceTouch_SCapCbTest_OFF_Max",
							"240", str, strIniFile);
	MaxValue = fts_atoi(str);
	/*Max */
	for (i = 0; i < 1; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			g_stCfg_MCap_DetailThreshold.ForceTouch_SCapCbTest_OFF_Max[i][j] =
				MaxValue;
		}
	}
	for (i = 0; i < 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.ForceTouch_SCapCbTest_OFF_Max[i][k]
					= (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
	...
	for (i = 0; i < 1; i++) {
		for (j = 0; j < g_ScreenSetParam.iUsedMaxRxNum; j++) {
			g_stCfg_MCap_DetailThreshold.ForceTouch_SCapCbTest_OFF_Min[i][j] =
				MinValue;
		}
	}
	for (i = 0; i < 1; i++) {
		...
		for (j = 0; j < dividerPos; j++) {
			if (',' == strValue[j]) {
				g_stCfg_MCap_DetailThreshold.ForceTouch_SCapCbTest_OFF_Min[i][k]
					= (short)(fts_atoi(str_tmp));
				index = 0;
				memset(str_tmp, 0x00, sizeof(str_tmp));
				k++;
...
void OnInit_DThreshold_CBTest(char *strIniFile)
{
	...
	GetPrivateProfileString("Basic_Threshold", "CBTest_Max_Vkey", "100", str,
							strIniFile);
	MaxValue_Vkey = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "ChannelNumTest_ChannelX", "15",
							str, strIniFile);
	ChannelNumTest_ChannelXNum = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "ChannelNumTest_ChannelY", "24",
							str, strIniFile);
...
void OnThreshold_VkAndVaRawDataSeparateTest(char *strIniFile)
{
	...
	GetPrivateProfileString("Basic_Threshold", "RawDataTest_Max", "1100", str,
							strIniFile);
	MaxValue = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "RawDataTest_Max_VKey", "1100",
							str, strIniFile);
	MaxValue_Vkey = fts_atoi(str);
	...
	for (i = 0; i < TX_NUM_MAX; i++) {
		...
		if (i == ChannelNumTest_ChannelXNum) {
			for (j = 0; j < RX_NUM_MAX; j++) {
				g_stCfg_Incell_DetailThreshold.RawDataTest_Max[i][j] =
					MaxValue_Vkey;
			}
		}
	}
...
/*FTS_TEST_DBG("node (%d, %d) value = %d \n", (i+1), (k+1), g_stCfg_Incell_DetailThreshold.RawDataTest_Max[i][k]);*/
	...
	GetPrivateProfileString("Basic_Threshold", "RawDataTest_Min", "5000", str,
							strIniFile);
	MinValue = fts_atoi(str);

	GetPrivateProfileString("Basic_Threshold", "RawDataTest_Min_VKey", "5000",
							str, strIniFile);
	MinValue_Vkey = fts_atoi(str);
	...
	for (i = 0; i < TX_NUM_MAX; i++) {
		...
		if (i == ChannelNumTest_ChannelXNum) {
			for (j = 0; j < RX_NUM_MAX; j++) {
				g_stCfg_Incell_DetailThreshold.RawDataTest_Min[i][j] =
					MinValue_Vkey;
			}
		}
	}
