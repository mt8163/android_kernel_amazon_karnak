...
*******************************************************************************/
#define FOCALTECH_TEST_INFO "test library Ver: V1.1.0 2016-08-01"
...
int fts_test_init(struct i2c_client *client)
{
	...
	if (0 != err) {
		FTS_TEST_DBG("[focal] %s() - ERROR: sysfs_create_group() failed.",
					 __func__);
		sysfs_remove_group(&client->dev.kobj, &fts_test_attribute_group);
		return -EIO;
	} else {
		FTS_TEST_DBG("[focal] %s() - sysfs_create_group() succeeded.",
					 __func__);
	}
...
/*default print: current function name and line number*/
	FTS_TEST_DBG("");
	sysfs_remove_group(&client->dev.kobj, &fts_test_attribute_group);
