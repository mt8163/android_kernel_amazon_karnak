...
*****************************************************************************/
#define FOCALTECH_IC_TABLE_INFO "focaltech_ic_table.c Ver: V1.0.0 2016-03-22"
...
unsigned int fts_ic_table_get_ic_code_from_ic_name(char *strIcName)
{
	...
	if (strncmp(strIcName, "FTE736", 6) == 0)
		return IC_FTE736;
	printk(KERN_ERR "<<-FTS-ERROR->>%s.Can't get ic code! \n", __func__);
	return 0xff;
}
...
/****************************************************************************/
void fts_ic_table_get_ic_name_from_ic_code(unsigned int ucIcCode,
										   char *strIcName)
{
	if (NULL == strIcName)
		return;
...
/***************************************************************************/
unsigned int fts_ic_table_get_ic_code_from_chip_id(unsigned char chip_id,
												   unsigned char chip_id2)
{
	...
	case 0x58:
		ic_code = IC_FT5822;
		break;
/*case 0x59:Does not support the 5926 series*/
...
/***************************************************************************/
unsigned int fts_ic_table_get_chip_id_from_ic_code(unsigned int ic_code,
...
int fts_ic_table_need_chip_id2(unsigned int chip_id)
{
	...
	case 0xE7:
		b_need_id2 = 0;
		printk(KERN_INFO "b_need_id2 = %d, chip_id = 0x%02x\n", b_need_id2,
			   chip_id);
		break;
	default:
		b_need_id2 = -1;
