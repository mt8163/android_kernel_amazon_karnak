...
static ssize_t gt91xx_fw_version_read_proc(struct file *, char __user *, size_t, loff_t *);
...
exit_with_irq_en:
	if (ts->use_irq && !atomic_read(&ts->gtp_is_suspend))
		gtp_irq_enable(ts);
}
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	gt91xx_fw_version_proc = proc_create(GT91XX_FW_VERSION_PROC_FILE, 0444, NULL, &fw_version_proc_ops);

	if (gt91xx_fw_version_proc == NULL)
		GTP_ERROR("create_proc_entry %s failed\n", GT91XX_FW_VERSION_PROC_FILE);
	else
		GTP_INFO("create proc entry %s success", GT91XX_FW_VERSION_PROC_FILE);
...
static void goodix_ts_suspend(struct goodix_ts_data *ts)
{
	...
	atomic_set(&ts->gtp_is_suspend, 1);
#if GTP_ESD_PROTECT
	gtp_esd_switch(ts->client, SWITCH_OFF);
#endif
...
static void goodix_ts_resume(struct goodix_ts_data *ts)
{
	...
	atomic_set(&ts->gtp_is_suspend, 0);
	if (ts->use_irq)
		gtp_irq_enable(ts);
	else
		hrtimer_start(&ts->timer, ktime_set(1, 0), HRTIMER_MODE_REL);
...
static void gtp_esd_check_func(struct work_struct *work)
{
	...
	if (atomic_read(&ts->gtp_is_suspend) || ts->enter_update) {
