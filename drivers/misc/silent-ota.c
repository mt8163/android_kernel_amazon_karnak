...
static void __silent_ota_psy_work(struct work_struct *dummy)
{
	...
	if (silent_ota_inst.is_silent && psy){
		ret = psy->desc->get_property(psy, POWER_SUPPLY_PROP_PRESENT, &present);
		if (ret)
			return;

		if (!strncmp(psy->desc->name, "ac", 2) && (silent_ota_inst.ac_psy_present != present.intval)) {
			pr_info("silent: power supply %s present status changed from %d to %d\n", psy->desc->name, silent_ota_inst.ac_psy_present, present.intval);
			silent_ota_inst.ac_psy_present = present.intval;
			__silent_ota_turn_on_bl(1);
		}

		if (!strncmp(psy->desc->name, "usb", 3) && (silent_ota_inst.usb_psy_present != present.intval)) {
			pr_info("silent: power supply %s present status changed from %d to %d\n", psy->desc->name, silent_ota_inst.usb_psy_present, present.intval);
				   ...
				   unsigned long event, void *data)
{
	...
	if (silent_ota_inst.is_silent && event == PSY_EVENT_PROP_CHANGED && psy && psy->desc->name) {
		while (i < array_len) {
			if (!strncmp(psy->desc->name, psy_list[i], strlen(psy_list[i]))) {
...
static int silent_ota_probe(struct platform_device *pdev)
{
	...
	if (!psy)
		return -EPROBE_DEFER;
	ret = psy->desc->get_property(psy, POWER_SUPPLY_PROP_PRESENT, &present);
	if ((ret)||(-1 == present.intval)){
		pr_info("EPROBE_DEFER for POWER_SUPPLY_PROP_PRESENT (usb:present.intval:%d) \n",present.intval);
		return -EPROBE_DEFER;
	}
	silent_ota_inst.usb_psy_present = present.intval;
	...
	ret = psy->desc->get_property(psy, POWER_SUPPLY_PROP_PRESENT, &present);
	if ((ret) ||(-1 == present.intval)){
		pr_info("EPROBE_DEFER for POWER_SUPPLY_PROP_PRESENT (ac: present.intval:%d) \n",present.intval);
		return -EPROBE_DEFER;
	}
	silent_ota_inst.ac_psy_present = present.intval;
