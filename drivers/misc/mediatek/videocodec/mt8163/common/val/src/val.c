	...
	unsigned int a_u4ParamSize)
{
	...
	if (pWaitQueue != VAL_NULL) {
		...
		pr_err("[VCODEC][ERROR] Event wait Queue failed to create\n");
	}
	if (pFlag != VAL_NULL) {
		a_prParam->pvReserved = (void *) pFlag;
		*((unsigned char *) a_prParam->pvReserved) = VAL_FALSE;
	} else {
		pr_err("[VCODEC][ERROR] Event flag failed to create\n");
	}
	...
	unsigned int a_u4ParamSize)
{
	...
	if (a_prParam->pvReserved != VAL_NULL)
		*((unsigned char *) a_prParam->pvReserved) = VAL_TRUE;
		...
		pr_err("[VCODEC][ERROR]Event flag should not be null\n");

	if (pWaitQueue != VAL_NULL)
		wake_up_interruptible(pWaitQueue);
	else
		pr_err("[VCODEC][ERROR]Wait Queue should not be null\n");
	...
	unsigned int a_u4ParamSize)
{
	...
	if (pLock != VAL_NULL)
		...
		pr_err("[VCODEC][ERROR]Enable to create mutex!\n");
...
enum VAL_RESULT_T eVideoMemSet(struct VAL_MEMORY_T *a_prParam,
	unsigned int a_u4ParamSize, signed int a_u4Value,
