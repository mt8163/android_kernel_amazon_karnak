 ...
 *
 * Currently no lock for mfg, Mali driver will take. And MFG power domain
 * (MTCMOS) is assumed that enabled before calling MFG functions.
 */
...
do { \
	...
	if (exp_val != con) { \
		dev_info(&mfg_dev->dev, \
			"MFG %s #%d read con: %#x, expect %#x\n", \
...
static void mtk_mfg_wait_gpu_idle(void)
{
	...
	/* Polling the MFG_DEBUG_REG for checking GPU IDLE
	 * before MTCMOS power off (0.1ms)
	 */
	...
	if (polling_count <= 0)
		pr_err("[MALI]: MFG is still BUSY! polling_count=%d\n",
			polling_count);
}
...
/* In suspend mode, pm_runtime_get_sync() will return -13,
 * which indicate no premission. When return fail in
 * mtk_smi_larb_clock_on(), not call mtk_smi_larb_clock_off()
 * next time. Solve the issue in ALPS02813524.
 */
static int iSmiFail;
...
static int pm_callback_power_on(struct kbase_device *kbdev)
{
	...
	*/
	 ...
	 * GPU state (if the below HARD_RESET code is active)
	 */
	return 1;
}
...
static void pm_callback_power_off(struct kbase_device *kbdev)
{
	...
	/* Cause a GPU hard reset to test whether we have actually
	 * idled the GPU and that we properly reconfigure the GPU
	 * on power up. Usually this would be dangerous, but if the
	 * GPU is working correctly it should be completely safe as
	 * the GPU should not be active at this point. However this
	 * is disabled normally because it will most likely interfere
	 * with bus logging etc.
	 */
	kbase_os_reg_write(kbdev,
		GPU_CONTROL_REG(GPU_COMMAND), GPU_COMMAND_HARD_RESET);
	mtk_mfg_wait_gpu_idle();
#endif
