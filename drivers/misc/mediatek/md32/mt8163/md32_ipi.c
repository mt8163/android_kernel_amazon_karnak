...
void md32_ipi_handler(void)
{
	...
	if (ipi_desc[md32_rcv_obj->id].handler) {
		memcpy_from_md32(md32_recv_buff,
				 (void *)md32_rcv_obj->share_buf,
				 md32_rcv_obj->len);
			      ...
			      unsigned int wait)
{
	...
	if (sw_rstn == 0x0) {
		pr_err("md32_ipi_send: MD32 not enabled\n");
		return ERROR;
	}
	...
	if (id < MD32_NR_IPI) {
		...
		memcpy_to_md32((void *)md32_send_obj->share_buf,
			       md32_send_buff, len);
		md32_send_obj->len = len;
		md32_send_obj->id = id;
		/*
		 * make sure that ipi_md2host will be run after
		 * data prepared
		 */
		mb();
		ipi_md2host();
