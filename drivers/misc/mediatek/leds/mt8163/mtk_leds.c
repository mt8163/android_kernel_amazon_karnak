		...
		for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
			...
			if (!led_node) {
				...
				if (!ret) {
					...
					LEDS_DEBUG
					    ("can not get pwm config data.\n");

#if defined(CONFIG_ENABLE_BACKLIGHT_FACTOR)
                ret = of_property_read_u32_array(led_node,
                        "brightness_normalize_param",
                        normalize_param,
                        ARRAY_SIZE(normalize_param));
                if (!ret && normalize_param[0]>0 && normalize_param[1]>0) {
                    mtk_leds_base = normalize_param[0];
                    mtk_leds_scale= normalize_param[1];
                    LEDS_DEBUG("The %s's brightness normalization param: base/scale=%d/%d\n",
                         pled_dtsi[i].name, mtk_leds_base, mtk_leds_scale);
                } else {
                    LEDS_DEBUG("led dts can not get normalization param from dts\n");
                }
#endif
				...
				case MT65XX_LED_MODE_CUST_LCM:
					...
					LEDS_DEBUG
					    ("backlight hw mode is LCM.\n");
					break;
				case MT65XX_LED_MODE_CUST_BLS_PWM:
					pled_dtsi[i].data =
					    (long)disp_bls_set_backlight;
					LEDS_DEBUG
					    ("backlight hw mode is BLS.\n");
					break;
				default:
					break;
				}
...
int mt_led_set_pwm(int pwm_num, struct nled_setting *led)
{
	...
	LEDS_DEBUG("led_set_pwm: mode=%d,pwm_no=%d\n", led->nled_mode,
		   pwm_num);
	/* We won't choose 32K to be the clock src of
	 * old mode because of system performance.
	 */
	...
	/* the setting still can not to turn off NLED.
	 * We should disable PWM to turn off NLED.
	 */
...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting *led)
{
	...
	/*pmic_set_register_value(PMIC_RG_DRV_32K_CK_PDN, 0x0);	*/
	/* Disable power down */
	switch (pmic_type) {
	case MT65XX_LED_PMIC_NLED_ISINK0:
#ifdef PMIC_MT6325
			 ...
			 struct PWM_config *config_data)
{
	...
	if (BacklightLevelSupport == BACKLIGHT_LEVEL_PWM_256_SUPPORT)
		...
		/* New mode fifo and periodical mode */
...
#endif
			LEDS_DEBUG
			    ("Set Backlight val %d at %lu, map level is %d\n",
			     led_data->level, jiffies, level);
			disp_aal_notify_backlight_changed((((1 <<
				MT_LED_INTERNAL_LEVEL_BIT_CNT) - 1)
				 * level + 127) / 255);
		}
	}
#else
			...
			unsigned long *delay_on, unsigned long *delay_off)
{
	...
	if (*delay_on != led_data->delay_on
		...
		if (led_data->delay_on && led_data->delay_off) {
			/* when enable blink  then to set the level  (255) */
			led_data->level = 255;
