...
#ifdef CONFIG_MTK_LEDS_DEFAULT_BRIGHTNESS
				ret = of_property_read_u8(led_node,
							   "default_brightness",
							   &default_brightness);
				if (!ret) {
					LEDS_DEBUG("The %s's default brightness is %d\n",
						   pled_dtsi[i].name,
						   default_brightness);
					pled_dtsi[i].default_brightness = default_brightness;
				} else {
					LEDS_DEBUG("led dts has no default brightness\n");
					pled_dtsi[i].default_brightness = -1;
				}
#endif
		...
		if (strcmp(led_data->cust.name, "lcd-backlight") != 0) {
			...
			if (mtk_leds_scale_enable == true) {
				disp_aal_notify_backlight_changed(((((1 <<
							    ...
							    - 1) * level +
							   127) / 255) * mtk_leds_base / mtk_leds_scale);
			} else {
				disp_aal_notify_backlight_changed((((1 <<
							     MT_LED_INTERNAL_LEVEL_BIT_CNT)
							    - 1) * level +
