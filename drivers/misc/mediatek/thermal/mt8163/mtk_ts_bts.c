...
static INT16 mtkts_bts_thermistor_conver_temp(INT32 Res)
{
	...
	/* xlog_printk(ANDROID_LOG_INFO, "Power/BTS_Thermal",
	   "mtkts_bts_thermistor_conver_temp() : asize = %d, Res = %d\n",asize,Res); */
	...
	xlog_printk(ANDROID_LOG_INFO, "Power/BTS_Thermal",
		    "mtkts_bts_thermistor_conver_temp() : Res = %d\n", Res);
	xlog_printk(ANDROID_LOG_INFO, "Power/BTS_Thermal",
		    "mtkts_bts_thermistor_conver_temp() : RES1 = %d\n", RES1);
	xlog_printk(ANDROID_LOG_INFO, "Power/BTS_Thermal",
		    "mtkts_bts_thermistor_conver_temp() : RES2 = %d\n", RES2);
	xlog_printk(ANDROID_LOG_INFO, "Power/BTS_Thermal",
		    "mtkts_bts_thermistor_conver_temp() : TMP1 = %d\n", TMP1);
	xlog_printk(ANDROID_LOG_INFO, "Power/BTS_Thermal",
		    "mtkts_bts_thermistor_conver_temp() : TMP2 = %d\n", TMP2);
#endif
...
static int mtkts_bts_get_temp(struct thermal_zone_device *thermal, unsigned long *t)
{
	*t = mtkts_bts_get_hw_temp();
	...
	/* if ((int) *t > 52000) */
	/* xlog_printk(ANDROID_LOG_INFO, "Power/BTS_Thermal", "T=%d\n", (int) *t); */
			       ...
			       loff_t *data)
{
	...
	if (sscanf
		...
		mtkts_bts_dprintk("[mtkts_bts_write] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,",
g_THERMAL_TRIP[0], g_THERMAL_TRIP[1]);
		mtkts_bts_dprintk("g_THERMAL_TRIP_2=%d,g_THERMAL_TRIP_3=%d,g_THERMAL_TRIP_4=%d,",
g_THERMAL_TRIP[2], g_THERMAL_TRIP[3], g_THERMAL_TRIP[4]);
		mtkts_bts_dprintk("g_THERMAL_TRIP_5=%d,g_THERMAL_TRIP_6=%d,g_THERMAL_TRIP_7=%d,",
g_THERMAL_TRIP[5], g_THERMAL_TRIP[6], g_THERMAL_TRIP[7]);
		mtkts_bts_dprintk("g_THERMAL_TRIP_8=%d,g_THERMAL_TRIP_9=%d,\n",
g_THERMAL_TRIP[8], g_THERMAL_TRIP[9]);
		mtkts_bts_dprintk("[mtkts_bts_write] cooldev0=%s,cooldev1=%s,", g_bind0, g_bind1);
		mtkts_bts_dprintk("cooldev2=%s,cooldev3=%s,cooldev4=%s,cooldev5=%s,",
g_bind2, g_bind3, g_bind4, g_bind5);
		mtkts_bts_dprintk("cooldev6=%s,cooldev7=%s,cooldev8=%s,cooldev9=%s\n",
g_bind6, g_bind7, g_bind8, g_bind9);
		...
		mtkts_bts_dprintk("[mtkts_bts_write] trip_0_temp=%d,trip_1_temp=%d,",
trip_temp[0], trip_temp[1]);
		mtkts_bts_dprintk("trip_2_temp=%d,trip_3_temp=%d,trip_4_temp=%d,",
trip_temp[2], trip_temp[3], trip_temp[4]);
		mtkts_bts_dprintk("trip_5_temp=%d,trip_6_temp=%d,trip_7_temp=%d,",
trip_temp[5], trip_temp[6], trip_temp[7]);
		mtkts_bts_dprintk("trip_8_temp=%d,trip_9_temp=%d,time_ms=%d\n",
...
void mtkts_bts_copy_table(struct BTS_TEMPERATURE *des, struct BTS_TEMPERATURE *src)
{
	...
	/* xlog_printk(ANDROID_LOG_INFO, "Power/BTS_Thermal", "mtkts_bts_copy_table() : j = %d\n",j); */
	for (i = 0; i < j; i++)
		des[i] = src[i];
}
