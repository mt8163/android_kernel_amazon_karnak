...
struct thermal_dev_node_names cpu_node_names = {
	.offset_name = "thermal_cpu,offset",
	.alpha_name = "thermal_cpu,alpha",
	.weight_name = "thermal_cpu,weight",
...
static int mtktscpu_probe(struct platform_device *pdev)
{
	...
	struct thermal_dev_params *cpu_params;
	int ret = 0;
	...
	platform_set_drvdata(pdev, virtual_sensor);
        ...
        if (virtual_sensor->therm_fw) {
                ...
                virtual_sensor->therm_fw->dev_ops = &mtktscpu_sensor_fops;
                virtual_sensor->therm_fw->tdp = thermal_sensor_dt_to_params(&pdev->dev,
						cpu_params, &cpu_node_names);
...
#ifdef CONFIG_OF
static const struct of_device_id mtkscpu_of_match[] = {
	{.compatible = "amazon,mtkscpu_sensor", },
	{},
};
MODULE_DEVICE_TABLE(of, mtkscpu_of_match);
#endif
...
#ifdef CONFIG_OF
		.of_match_table = mtkscpu_of_match,
#endif
		.owner = THIS_MODULE,
	},
};
