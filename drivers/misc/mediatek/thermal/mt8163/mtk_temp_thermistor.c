...
struct thermal_dev_node_names thermistor_node_names = {
	...
	.weight_invert_name = "thermal_thermistor,weight_invert",
...
#ifdef CONFIG_AMAZON_THERMAL
static int thermistor_get_temp(struct virtual_sensor_temp_sensor *virt_sensor)
{
	return mtktsthermistor_read_temp(virt_sensor->therm_fw);
}

static int mtktsthermistor_get_temp(void *data, int *t)
{
	*t = thermistor_get_temp(data);
	return 0;
}
