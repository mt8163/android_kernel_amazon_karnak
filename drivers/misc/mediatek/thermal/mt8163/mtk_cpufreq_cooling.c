/*
 * mtk_cpufreq_cooling.c - MTK cpufreq works as cooling device.
 *
...
static int mtk_cpufreq_cooling_probe(struct platform_device *pdev)
{
...
static int mtk_cpufreq_cooling_remove(struct platform_device *pdev)
{
...
static int mtk_cpufreq_cooling_suspend(struct platform_device *pdev,
		pm_message_t state)
{
	return 0;
}

static int mtk_cpufreq_cooling_resume(struct platform_device *pdev)
{
	return 0;
}

static struct platform_driver mtk_cpufreq_cooling_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = "mtk-cpufreq-cooling",
	},
	.probe = mtk_cpufreq_cooling_probe,
	.suspend = mtk_cpufreq_cooling_suspend,
	.resume = mtk_cpufreq_cooling_resume,
	.remove = mtk_cpufreq_cooling_remove,
};

static struct platform_device mtk_cpufreq_cooling_device = {
	.name = "mtk-cpufreq-cooling",
	.id = -1,
};
...
static int __init mtk_cpufreq_cooling_init(void)
{
	...
	ret = platform_driver_register(&mtk_cpufreq_cooling_driver);
	if (ret) {
		pr_err("Fail: mtk_cpufreq cooling driver (%d)\n", ret);
		return ret;
	}
	ret = platform_device_register(&mtk_cpufreq_cooling_device);
	if (ret) {
		pr_err("Fail: mtk_cpufreq cooling device (%d)\n", ret);
		return ret;
	}
	return 0;
}

static void __exit mtk_cpufreq_cooling_exit(void)
{
	platform_driver_unregister(&mtk_cpufreq_cooling_driver);
	platform_device_unregister(&mtk_cpufreq_cooling_device);
}

module_init(mtk_cpufreq_cooling_init);
module_exit(mtk_cpufreq_cooling_exit);

MODULE_AUTHOR("Akwasi Boateng <boatenga@amazon.com>");
MODULE_DESCRIPTION("MTK cpufreq cooling driver");
MODULE_LICENSE("GPL");
