...
/*
 * .get_trend wrapper: get the current trendof the thermal zone.
 */
static int mtk_thermal_wrapper_get_trend
(struct thermal_zone_device *thermal, int trip, enum thermal_trend *trend) {
	int ret = -EINVAL;
	struct thermal_zone_device_ops *ops;

	THRML_LOG("[.get_trend] tz: %s trip: %d\n", thermal->type, trip);

	ops = getClientZoneOps(thermal);

	if (!ops)
		THRML_ERROR_LOG("[.get_trend] tz: %s unregistered.\n",
				thermal->type);
	else if (ops->get_trend)
		ret = ops->get_trend(thermal, trip, trend);
...
#ifdef CONFIG_AMAZON_THERMAL
static int mtk_thermal_wrapper_set_trip_temp(
struct thermal_zone_device *thermal, int trip, int temperature) {
	int ret = 0;
	struct thermal_zone_device_ops *ops;
	...
	if (!ops) {
		THRML_ERROR_LOG("[.set_trip_temp] tz: %s unregistered.\n",
				thermal->type);
		return 1;
	}
	if (ops->set_trip_temp)
		ret = ops->set_trip_temp(thermal, trip, temperature);

	THRML_LOG("[.set_trip_temp] tz: %s trip: %d temp: %d\n", thermal->type,
		  trip, (int)temperature);
	THRML_STORAGE_LOG(THRML_LOGGER_MSG_TRIP_POINT, set_trip_temp,
			  thermal->type, trip, temperature);
	return ret;
}

static int mtk_thermal_wrapper_get_trip_hyst(
struct thermal_zone_device *thermal, int trip, int *hyst) {
	int ret = 0;
	struct thermal_zone_device_ops *ops;
	...
	if (!ops) {
		THRML_ERROR_LOG("[.get_trip_hyst] tz: %s unregistered.\n",
				thermal->type);
		return 1;
	}

	if (ops->get_trip_hyst)
		ret = ops->get_trip_hyst(thermal, trip, hyst);

	THRML_LOG("[.get_trip_hyst] tz: %s trip: %d temp: %d\n", thermal->type,
		  trip, (int)*hyst);
	THRML_STORAGE_LOG(THRML_LOGGER_MSG_TRIP_POINT, get_trip_hyst,
			  thermal->type, trip, *hyst);

	return ret;
}

static int mtk_thermal_wrapper_set_trip_hyst(
struct thermal_zone_device *thermal, int trip, int hyst) {
	int ret = 0;
	struct thermal_zone_device_ops *ops;
	...
	ops = getClientZoneOps(thermal);
	if (!ops) {
		THRML_ERROR_LOG("[.set_trip_hyst] tz: %s unregistered.\n",
				thermal->type);
		return 1;
	}

	if (ops->set_trip_hyst)
		ret = ops->set_trip_hyst(thermal, trip, hyst);

	THRML_LOG("[.set_trip_hyst] tz: %s trip: %d temp: %d\n", thermal->type,
		  trip, (int)hyst);
	THRML_STORAGE_LOG(THRML_LOGGER_MSG_TRIP_POINT, set_trip_hyst,
			  thermal->type, trip, hyst);
...
#ifdef CONFIG_AMAZON_THERMAL
	.set_trip_temp = mtk_thermal_wrapper_set_trip_temp,
	.set_trip_hyst = mtk_thermal_wrapper_set_trip_hyst,
	.get_trip_hyst = mtk_thermal_wrapper_get_trip_hyst,
	.get_trend = mtk_thermal_wrapper_get_trend,
#endif
