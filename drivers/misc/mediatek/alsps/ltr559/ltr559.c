...
static ssize_t ltr559_show_reg(struct device_driver *ddri, char *buf)
{
	...
	int reg[] =
	    ...
	    0x8b, 0x8c,
		0x8d, 0x8e, 0x8f, 0x90, 0x91, 0x92, 0x93, 0x94, 0x95, 0x97,
		    0x98, 0x99, 0x9a, 0x9e
	};
	for (i = 0; i < 27; i++) {
		len +=
...
/*----------------------------------------------------------------------------*/
static ssize_t ltr559_store_reg(struct device_driver *ddri, const char *buf,
				size_t count)
{
	int ret, value;
	u8 reg;
...
static inline int32_t ltr559_als_get_data_avg(int sSampleNo)
{
	...
	printk("[%s]: Ignore first als value:%d\n", __func__, als_reading);

	while (DataCount < sSampleNo) {
		msleep(50);
		result = ltr559_als_read(obj->client, &als_reading);
		printk(KERN_ERR "%s: [#23][LTR]als code = %d\n", __func__,
		       als_reading);
						 ...
						 unsigned int value)
{
	...
	mm_segment_t fs;
	char w_buf[LTR_DATA_BUF_NUM * sizeof(unsigned int) * 2 + 1] =
	    { 0 }, r_buf[LTR_DATA_BUF_NUM * sizeof(unsigned int) * 2 + 1] = {
	0};
	int i;
	char *dest = w_buf;
...
static ssize_t ltr559_als_enable_show(struct device_driver *ddri, char *buf)
{
	int32_t enabled = 0;
	int32_t ret = 0;
	u8 regdata = 0;
...
static ssize_t ltr559_als_enable_store(struct device_driver *ddri,
...
static ssize_t ltr559_als_lux_show(struct device_driver *ddri, char *buf)
{
...
static ssize_t ltr559_als_lux_threshold_show(struct device_driver *ddri,
					     char *buf)
{
...
static ssize_t ltr559_als_lux_threshold_store(struct device_driver *ddri,
					      const char *buf, size_t size)
{
...
//kaka
static ssize_t ltr559_als_transmittance_show(struct device_driver *ddri,
					     char *buf)
{
...
static ssize_t ltr559_als_transmittance_store(struct device_driver *ddri,
					      const char *buf, size_t size)
{
	unsigned long value = 0;
	int ret;
...
static ssize_t als_cali_Light_show(struct device_driver *ddri, char *buf)
{
	int32_t als_reading;
	...
	bool result = false;
	printk(KERN_ERR "%s:[#23][LTR]Start Cali light...\n", __func__);
	...
	if (((als_value_cali * ltr559_obj->transmittance) /
		...
		ltr559_obj->transmittance = (als_value_cali * ltr559_obj->transmittance) / (ltr559_obj->lux_threshold);	//transmittance for cali
...
#endif
static DRIVER_ATTR(status, 0664, ltr559_show_status, NULL);
static DRIVER_ATTR(reg, 0664, ltr559_show_reg, ltr559_store_reg);
/*----------------------------------------------------------------------------*/
//For Diag to Calibrate the ALS
static DRIVER_ATTR(enable, S_IWUSR | S_IRUGO, ltr559_als_enable_show, ltr559_als_enable_store);
static DRIVER_ATTR(lux, S_IWUSR | S_IRUGO, ltr559_als_lux_show, NULL);
static DRIVER_ATTR(lux_threshold, S_IWUSR | S_IRUGO, ltr559_als_lux_threshold_show, ltr559_als_lux_threshold_store);
static DRIVER_ATTR(transmittance, S_IWUSR | S_IRUGO, ltr559_als_transmittance_show, ltr559_als_transmittance_store);
static DRIVER_ATTR(cali_Light, S_IWUSR | S_IRUGO, als_cali_Light_show, NULL);
...
/* eint request */
	if (ltr559_obj->irq_node) {
		...
		if (request_irq
		    (ltr559_obj->irq, ltr559_eint_handler, IRQF_TRIGGER_FALLING,
		     "ALS-eint", NULL)) {
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (als_cal > 0 && als_cal <= 65535)
		ltr559_obj->transmittance = als_cal;
	else
