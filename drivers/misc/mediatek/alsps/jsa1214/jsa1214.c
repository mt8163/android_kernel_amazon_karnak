...
#define POWER_NONE_MACRO	MT65XX_POWER_NONE
	...
	#define GPIO_ALS_EINT_PIN_M_EIN     2   /* eint mode */
...
#endif
static u8 ratio = 1;
static u16 als_data = 0;
static u32 als_data_buffer = 0;
...
#define APS_DBG(fmt, args...)           pr_info(fmt, ##args)
/******************************************************************************
 * extern functions
 ******************************************************************************/
...
struct stk3x1x_priv {
	...
	struct delayed_work     work_als_polling;
	struct stk3x1x_i2c_addr addr;	/* i2c address group */
	/* misc */
	atomic_t    trace;
	atomic_t    i2c_retry;
...
int stk3x1x_read_als(struct i2c_client *client, u16 *data)
{
	...
	als_data_buffer = (als_data * ratio) ;
	/* show data formation */
	*data = (als_data * ratio);
#else
	struct stk3x1x_priv *obj = i2c_get_clientdata(client);
...
#ifdef CONFIG_MTK_JSA1214_SWITCH_RANGE_AUTO
	u8 thres[3];
	thres[0] = 0x32;	/* ALS low threshold is set 50 */
...
#ifdef CONFIG_MTK_JSA1214_SWITCH_RANGE_AUTO
	err = stk3x1x_master_send(client, 0x05, &thres[0], 1);
	if (err < 0) {
		APS_ERR("i2c write Lowthres error = %d\n", err);
		return err;
	}
	err = stk3x1x_master_send(client, 0x06, &thres[1], 1);
	if (err < 0) {
		APS_ERR("i2c write midthres error = %d\n", err);
		return err;
	}
	err = stk3x1x_master_send(client, 0x07, &thres[2], 1);
	if (err < 0) {
		APS_ERR("i2c write highthres error = %d\n", err);
		return err;
	}
#endif
	err = stk3x1x_write_als(client, 0x84);
	if (err) {
...
#endif
	return als_data_buffer;
}

static inline uint32_t stk3x1x_get_als_reading_AVG(struct stk3x1x_priv *ps_data, int SampleNo)
{
	uint32_t ALSData = 0;
	uint32_t DataCount = 0;
	uint32_t SaveAlsData = 0;
	while (DataCount < SampleNo) {
		msleep(100);
		ALSData = stk3x1x_get_als_reading(ps_data);
		APS_ERR("%s: [#23][STK]als code = %d\n", __func__, ALSData);
		SaveAlsData +=  ALSData;
		DataCount++;
	}
	SaveAlsData /= SampleNo;
...
static char als_read_cali_transmittance_file(void){
	...
	char *alscal_capcolor = NULL;

	APS_LOG("%s enter", __func__);
	cali_file = filp_open(STK3X1X_CAL_FILE, O_CREAT | O_RDWR, 0777);
	...
	read_color = buf;
	alscal_capcolor = strstr(read_color,substr);
...
static ssize_t stk3x1x_store_config(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d %d %d %d %d %d", &retry, &als_deb, &mask, &hthres, &lthres, &ps_deb) == 6) {
...
static ssize_t stk3x1x_show_allreg(struct device_driver *ddri, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE,
			"[0]%2X [1]%2X [2]%2X [3]%2X [4]%2X [5]%2X [6/7 HTHD]%2X, \
			%2X [8/9 LTHD]%2X, %2X [A]%2X [B]%2X [C]%2X [D]%2X [E/F Aoff]%2X,%2X, \
			[10]%2X [11/12 PS]%2X,%2X [13]%2X [14]%2X [15/16 Foff]%2X, \
			%2X [17]%2X [18]%2X [3E]%2X [3F]%2X\n",
...
static ssize_t stk3x1x_show_status(struct device_driver *ddri, char *buf)
{
	...
	len += scnprintf(buf+len, PAGE_SIZE-len,
			...
			atomic_read(&stk3x1x_obj->ps_high_thd_val),
			atomic_read(&stk3x1x_obj->ps_low_thd_val),
			stk3x1x_obj->enable, stk3x1x_obj->pending_intr);
	...
	len += scnprintf(buf+len, PAGE_SIZE-len,
			"[PS=%2X] [ALS=%2X] [WAIT=%4Xms] [EN_ASO=%2X] [EN_AK=%2X] \
			[NEAR/FAR=%2X] [FLAG_OUI=%2X] [FLAG_PSINT=%2X] [FLAG_ALSINT=%2X]\n",
			rbuf[0]&0x01, (rbuf[0]&0x02)>>1, ((rbuf[0]&0x04)>>2)*rbuf[5]*6,
			(rbuf[0]&0x20)>>5,(rbuf[0]&0x40)>>6, rbuf[16]&0x01, (rbuf[16]&0x04)>>2,
...
static int read_int_from_buf(struct stk3x1x_priv *obj, const char *buf, size_t count, u32 data[], int len)
{
	int idx = 0;
...
static ssize_t stk3x1x_store_alslv(struct device_driver *ddri, const char *buf, size_t count)
{
	if (!stk3x1x_obj) {
		...
		memcpy(stk3x1x_obj->als_level, stk3x1x_obj->hw->als_level, sizeof(stk3x1x_obj->als_level));
               ...
               read_int_from_buf(stk3x1x_obj, buf, count,
                                 stk3x1x_obj->hw->als_level,
...
static ssize_t stk_als_enable_show(struct device_driver *ddri, char *buf)
{
	...
	if (enable != ret)
		APS_ERR("%s: driver and sensor mismatch! driver_enable=0x%x, sensor_enable=%x\n",
			__func__, enable, ret);
...
static ssize_t stk_als_enable_alsdata_polling_store(struct device_driver *ddri, const char *buf, size_t size)
{
	uint8_t en;
	if (sysfs_streq(buf, "1"))
		en = 1;
	...
	APS_LOG("%s: Enable ALS polling: %d\n", __func__, en);
	if (en) {
		if (!stk3x1x_obj->als_polling_enable) {
			stk3x1x_obj->als_polling_enable = 1;
			schedule_delayed_work(&stk3x1x_obj->work_als_polling, 220*HZ/1000);
		}
	}
	else
		stk3x1x_obj->als_polling_enable = 0;
...
static ssize_t als_CCI_test_Light_show(struct device_driver *ddri, char *buf)
{
	...
	APS_LOG("%s:[#23][STK]Start testing light...\n", __func__);
	msleep(150);
	als_reading = stk3x1x_get_als_reading_AVG(stk3x1x_obj, 5);
	mutex_lock(&stk3x1x_obj->io_lock);
	APS_LOG("%s:[#23][STK]als_reading = %d\n", __func__, als_reading);
	cci_als_value = stk_alscode2lux(stk3x1x_obj, als_reading);
	mutex_unlock(&stk3x1x_obj->io_lock);
	APS_LOG("%s:[#23][STK]Start testing light done!!! cci_als_value = %d lux, \
		cci_als_test_adc = %d \n",
...
static ssize_t als_CCI_cali_Light_show(struct device_driver *ddri, char *buf)
{
	...
	bool result = false;
	APS_LOG("%s:[#23][STK]Start Cali light...\n", __func__);
	msleep(150);
	als_reading = stk3x1x_get_als_reading_AVG(stk3x1x_obj, 5);
...
{
	if (sysfs_streq(buf, "1"))
		capcolor = capcolor_to_bin = '1';
	else if (sysfs_streq(buf, "0"))
		capcolor = capcolor_to_bin = default_black_color;
	else {
		APS_ERR("%s, invalid value %d\n", __func__, *buf);
		return -EINVAL;
	}
...
static ssize_t stk_all_reg_show(struct device_driver *ddri, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "[0]%2X [1]%2X [2]%2X [3]%2X [4]%2X [5]%2X [6/7 HTHD]%2X, \
			%2X [8/9 LTHD]%2X, %2X [A]%2X [B]%2X [C]%2X [D]%2X [E/F Aoff]%2X,%2X, \
			[10]%2X [11/12 PS]%2X,%2X [13]%2X [14]%2X [15/16 Foff]%2X, \
			%2X [17]%2X [18]%2X [3E]%2X [3F]%2X\n",
			rbuf[0], rbuf[1], rbuf[2], rbuf[3], rbuf[4], rbuf[5], rbuf[6],
			rbuf[7], rbuf[8],rbuf[9], rbuf[10], rbuf[11], rbuf[12], rbuf[13],
			rbuf[14], rbuf[15], rbuf[16], rbuf[17],rbuf[18], rbuf[19], rbuf[20],
			rbuf[21], rbuf[22], rbuf[23], rbuf[24], rbuf[25], rbuf[26]);
}
...
static ssize_t pin_sensor_show(struct device_driver *ddri, char *buf)
{
	...
	if (ret >= 0) {
		APS_LOG("STK check PID = %d success\n", ret);
		return scnprintf(buf, PAGE_SIZE, "[stk] I2C Work, ID = %x\n", ret);
	} else {
		APS_ERR("STK check PID = %d fail\n", ret);
...
static int stk3x1x_get_als_value(struct stk3x1x_priv *obj, u16 als)
{
	...
	if (!invalid) {
		...
		APS_DBG("ALS: %d [%d, %d] => %d [%d, %d] \n", als,
			level_low, level_high, value, value_low, value_high);
		return value;
#endif
...
static void stk3x1x_diag_eint_work(struct work_struct *work)
{
	int value, status;
	while (stk3x1x_obj->als_polling_enable) {
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct regulator *als_vcn_ldo = NULL;
	APS_LOG("%s: driver version: %s\n", __FUNCTION__, DRIVER_VERSION);
	als_vcn_ldo = devm_regulator_get(&client->dev, "vcn28");
	if (IS_ERR(als_vcn_ldo)) {
		err = PTR_ERR(als_vcn_ldo);
		dev_err(&client->dev, "failed to get als_vcn_ldo, %d\n", err);
		goto exit;
	}
	err = regulator_enable(als_vcn_ldo);
	if (!regulator_is_enabled(als_vcn_ldo)) {
		err = regulator_enable(als_vcn_ldo);
	if (err != 0)
		APS_ERR("ALS: fail to enable supply als_vcn_ldo\n");
	}
	...
	INIT_DELAYED_WORK(&obj->work_als_polling, stk3x1x_diag_eint_work);
	...
	APS_LOG("%s: state_val=0x%x, psctrl_val=0x%x, alsctrl_val=0x%x, \
            ...
            atomic_read(&obj->alsctrl_val),obj->ledctrl_val,
            obj->wait_val, obj->int_val);

	APS_LOG("stk3x1x_i2c_probe() OK!\n");
	obj->als_polling_enable = 0;
	obj->enable = 0;
	obj->pending_intr = 0;
...
static int stk3x1x_local_init(void)
{
	...
	if (ret) {
		APS_ERR("add driver error,ret:%d\n",ret);
		return ret;
	}
	if (stk3x1x_init_flag == -1) {
		ret = -1;
        return ret;
	}
