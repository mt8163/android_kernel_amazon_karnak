...
unsigned int cci_als_value;//lux value in test level
...
static int stk3x1x_enable_ps(struct i2c_client *client, int enable)
{
	...
	if(enable)
	{
		...
		else
		{
			...
			else if(stk3x1x_obj->hw->polling_mode_ps == 0) //stk3x1x_obj.hw->polling_mode_ps == 0
			{
...
void stk3x1x_eint_func(void)
{
	...
	int_top_time = sched_clock();
        //mt_eint_mask(CUST_EINT_STK3X1X_NUM);
...
static int stk3x1x_i2c_write_data(struct i2c_client *client, unsigned char command, int length, unsigned char *values)
{
    ...
    for (retry = 0; retry < 5; retry++)
    {
        err = i2c_master_send(client, data, msg.len);
...
static ssize_t stk_als_enable_store(struct device_driver *ddri, const char *buf,size_t size)
{
    ...
    mutex_lock(&stk3x1x_obj->io_lock);
    //stk3x1x_enable_als(stk3x1x_obj, en);//ps_data
    stk3x1x_enable_als(stk3x1x_obj->client, en);//ps_data
    mutex_unlock(&stk3x1x_obj->io_lock);
    return size;
}
...
static ssize_t stk_als_code_show(struct device_driver *ddri, char *buf)
{
    ...
    reading = stk3x1x_get_als_reading(stk3x1x_obj);//ps_data
    return scnprintf(buf, PAGE_SIZE, "%d\n", reading);
}
*/
...
static ssize_t als_CCI_cali_Light_show(struct device_driver *ddri, char *buf)
{
    ...
    if(((cci_als_value_cali * stk3x1x_obj->als_transmittance)/400) > 0 && (cci_als_value_cali_adc <=65535)){
        cci_transmittance_cali = (cci_als_value_cali * stk3x1x_obj->als_transmittance)/400; //transmittance for cali
	stk3x1x_obj->als_transmittance = cci_transmittance_cali; // writ back to als_transmittance
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			if((buff_out == NULL) || (size_out< sizeof(sensor_data))) //hwm_sensor_data
			{
				APS_ERR("get sensor data parameter error!\n");
				err = -EINVAL;
			}
...
static int als_get_data(int* value, int* status)
{
	...
	else
	{
		*value = stk_alscode2lux(obj, obj->als);
		//*value = stk3x1x_get_als_value(obj, reading_lux);
		*status = SENSOR_STATUS_ACCURACY_MEDIUM;
	}
#endif //#ifdef CUSTOM_KERNEL_SENSORHUB
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	stk3x1x_obj = obj;
	obj->hw = &alsps_cust; //get_cust_alsps_hw();
	...
	ps_lock = wakeup_source_register("alps lock");
