...
#define PFX "OV9724_camera_sensor"
#define LOG_1 LOG_INF("OV9724,MIPI 2LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV9724_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h

    .checksum_value = 0x8790037C,        //checksum value for Camera Auto Test

    .pre = {
        .pclk = 36600000,                //record different mode's pclk
        .linelength = 0x0628,                //record different mode's linelength
        .framelength = 0x02f8,            //record different mode's framelength
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
    ...
    if (gain < 2 * BASEGAIN) {
        reg_gain = 16 * (gain - BASEGAIN) / BASEGAIN;
    }else if (gain < 4 * BASEGAIN) {
        reg_gain |= 0x10;
        reg_gain |= 8 * (gain - 2 * BASEGAIN) / BASEGAIN;
    }else if (gain < 8 * BASEGAIN) {
        reg_gain |= 0x30;
        reg_gain |= 4 * (gain - 4 * BASEGAIN) / BASEGAIN;
    }else if (gain < 16 * BASEGAIN) {
        reg_gain |= 0x70;
        reg_gain |= 2 * (gain - 8 * BASEGAIN) / BASEGAIN;
    }else if (gain < 32 * BASEGAIN) {
        reg_gain |= 0xF0;
        reg_gain |= (gain - 16 * BASEGAIN) / BASEGAIN;
    }else {
    }
...
static void sensor_init(void)
{
   ...
   /*****************************************************************************
    ...
    sysclk = 160 MHZ
    */
	   write_cmos_sensor(0x0103, 0x01);//pll
	   write_cmos_sensor(0x3210, 0x43);//
	   write_cmos_sensor(0x0307, 0x3d);//pll
	   write_cmos_sensor(0x5780, 0x3e);//
	   write_cmos_sensor(0x3606, 0x75);//
	   write_cmos_sensor(0x3705, 0x41);//
	   write_cmos_sensor(0x3601, 0x34);//
	   write_cmos_sensor(0x3607, 0x94);//
	   write_cmos_sensor(0x3608, 0x38);//
	   write_cmos_sensor(0x3712, 0xb4);//
	   write_cmos_sensor(0x370d, 0xcc);//
	   write_cmos_sensor(0x4010, 0x08);//
	   write_cmos_sensor(0x4000, 0x01);//
	   
	   write_cmos_sensor(0x0340, 0x02);//frame length  760
	   write_cmos_sensor(0x0341, 0xf8);//
	   
	   write_cmos_sensor(0x0342, 0x06);//line length 1576
	   write_cmos_sensor(0x0343, 0x28);//
	   
	   write_cmos_sensor(0x0202, 0x02);// exposure high bit
	   write_cmos_sensor(0x0203, 0xf0);// exposure low bit
	   
	   write_cmos_sensor(0x4800, 0x64);//mipi control
	   
	   write_cmos_sensor(0x4801, 0x0f);//mipi control
	   write_cmos_sensor(0x4801, 0x8f);//
	   write_cmos_sensor(0x4814, 0x2b);//mipi
	   write_cmos_sensor(0x0101, 0x01);//mirror & flip
	   write_cmos_sensor(0x5110, 0x09);//
	   write_cmos_sensor(0x4307, 0x3a);//
	   write_cmos_sensor(0x5000, 0x00);//BPC off;  on:0x06
	   
	   write_cmos_sensor(0x5001, 0x73);//blc & manual wb gain
	   
	   write_cmos_sensor(0x0205, 0x3f);// gain control
