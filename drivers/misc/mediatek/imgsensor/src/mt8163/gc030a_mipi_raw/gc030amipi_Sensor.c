...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC030A_SENSOR_ID,	//record sensor id defined in Kd_imgsensor.h
...
#define ANALOG_GAIN_6 355		// 5.54x
#define ANALOG_GAIN_7 485		// 7.57x
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if ((ANALOG_GAIN_1 <= iReg) && (iReg < ANALOG_GAIN_2))
	{
		...
		LOG_INF("GC030AMIPI analogic gain 1x, GC030AMIPI add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_2 <= iReg) && (iReg < ANALOG_GAIN_3))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("GC030AMIPI analogic gain 1.42x , GC030AMIPI add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_3 <= iReg) && (iReg < ANALOG_GAIN_4))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("GC030AMIPI analogic gain 1.97x , GC030AMIPI add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_4 <= iReg) && (iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("GC030AMIPI analogic gain 2.78x , GC030AMIPI add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_5 <= iReg) && (iReg < ANALOG_GAIN_6))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("GC030AMIPI analogic gain 3.78x , GC030AMIPI add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_6 <= iReg) && (iReg < ANALOG_GAIN_7))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("GC030AMIPI analogic gain 5.54x , GC030AMIPI add pregain = %d\n",
			temp);
	} else
	{
		//analog gain
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("GC030AMIPI analogic gain 7.57x , GC030AMIPI add pregain = %d\n",
			temp);
	}
	return gain;
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff)
	{
		...
		do
		{
			*sensor_id = return_sensor_id();
			...
			{
				printk
					("GC030A get_imgsensor_id i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			printk
				("GC030A get_imgsensor_id Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		}
		while (retry > 0);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff)
	{
		...
		do
		{
			...
			LOG_INF
				("GC030A open Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		}
		while (retry > 0);
...
UINT32 GC030AMIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
