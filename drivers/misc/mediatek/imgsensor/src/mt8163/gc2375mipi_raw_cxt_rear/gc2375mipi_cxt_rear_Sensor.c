...
#include "gc2375mipi_cxt_rear_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "gc2375_cxt_rear_camera_sensor"
#define LOG_1 LOG_INF("gc2375_cxt_rear,MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC2375MIPI_CXT_REAR_SENSOR_ID,	//record sensor id defined in Kd_imgsensor.h
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	} else if ((ANALOG_GAIN_2 <= iReg) && (iReg < ANALOG_GAIN_3))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("gc2375MIPI analogic gain 1.43x , gc2375MIPI add pregain = %d\n",
			temp);
	...
	} else if ((ANALOG_GAIN_4 <= iReg) && (iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("gc2375MIPI analogic gain 2.84x , gc2375MIPI add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_5 <= iReg) && (iReg < ANALOG_GAIN_6))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("gc2375MIPI analogic gain 3.97x , gc2375MIPI add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_6 <= iReg) && (iReg < ANALOG_GAIN_7))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("gc2375MIPI analogic gain 5.68x , gc2375MIPI add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_7 <= iReg) && (iReg < ANALOG_GAIN_8))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("gc2375MIPI analogic gain 8.14x , gc2375MIPI add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_8 <= iReg) && (iReg < ANALOG_GAIN_9))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("gc2375MIPI analogic gain 11.34x , gc2375MIPI add pregain = %d\n",
			temp);
	} else
	{
		write_cmos_sensor(0x20, 0x0c);
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("gc2375MIPI analogic gain 16.23x , gc2375MIPI add pregain = %d\n",
			temp);
	}
...
extern int get_adc_id_status(void);
...
static kal_uint32 get_imgsensor_id(UINT32 * sensor_id)
{
	...
	/* The ID pin voltage values of the two modules are 0mv and 900mv.
	   To prevent voltage fluctuations from causing other problems,
	   So we chose 200mv as the criterion.
	*/
	adc_val = get_adc_id_status();
	if (adc_val < 200)
	{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff)
	{
		...
		do
		{
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id - 3)
			{
				printk
					("GC2375MIPI_CXT_REAR i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, (*sensor_id + 3));
				return ERROR_NONE;
			}
			printk
				("GC2375MIPI_CXT_REAR Read sensor id fail, write id: 0x%x, id: 0x%x\n",
...
static kal_uint32 open(void)
{
	...
	if ((imgsensor_info.sensor_id - 3) != sensor_id)
		return ERROR_SENSOR_CONNECT_FAIL;
...
UINT32 GC2375MIPI_CXT_REAR_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT
	**pfFunc)
{
	...
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}								/*    GC2375MIPI_CXT_REAR_RAW_SensorInit    */
