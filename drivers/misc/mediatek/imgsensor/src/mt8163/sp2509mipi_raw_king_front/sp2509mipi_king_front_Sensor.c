...
#include "sp2509mipi_king_front_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "SP2509_king_front_camera_sensor"
#define LOG_1 LOG_INF("SP2509_KING_FRONT,MIPI 1LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = SP2509MIPI_KING_FRONT_SENSOR_ID,	//record sensor id defined in Kd_imgsensor.h
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain >= BASEGAIN && gain <= 15 * BASEGAIN)
	{
		...
		if (iReg <= 0x10)
		{
			...
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("SP2509MIPI_KING_FRONT_SetGain = 16");
		} else if (iReg >= 0xa0)
		{
			write_cmos_sensor(0xfd, 0x01);
			write_cmos_sensor(0x24, 0xa0);
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("SP2509MIPI_KING_FRONT_SetGain = 160");
		} else
		{
			write_cmos_sensor(0xfd, 0x01);
			write_cmos_sensor(0x24, (kal_uint8) iReg);
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("SP2509MIPI_KING_FRONT_SetGain = %d\n", iReg);
		}
	} else
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x10, 0xb2);
	write_cmos_sensor(0x11, 0xb2);
	write_cmos_sensor(0x13, 0xb2);
	write_cmos_sensor(0x14, 0xb2);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff)
	{
		...
		do
		{
			*sensor_id = return_sensor_id();
			...
			printk
				("SP2509mipi_king_front Read sensor id fail, write id: 0x%x, id: 0x%x\n",
...
/* This Function not used after ROM */
...
UINT32 SP2509MIPI_KING_FRONT_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT
	**pfFunc)
{
