...
#if DEBUG
#define LOG_INF(format, args...)    pr_debug(ANDROID_LOG_INFO   , PFX, "[%s] " format, __func__, ##args)

#define LOG_1 LOG_INF("GC2235 RAW,MIPI 2LANE\n")
#define LOG_2 LOG_INF("preview 1280*960@30fps,864Mbps/lane;"
					"ideo 5M@30fps,864Mbps/lane; capture 5M@30fps,864Mbps/lane;"
					"HS HD0@120fps,864Mbps/lane; slim HD@30fps,864Mbps/lane\n")
#else
...
static imgsensor_info_struct imgsensor_info = {
	...
	.checksum_value = 0x9f254a0a,	/*original: 0x603c4250 */

	.pre = {
		.pclk = 24000000,	/*record different mode's pclk */
		.linelength = 1030,	/*record different mode's linelength */
		.framelength = 1258,	/*record different mode's framelength */
		.startx = 8,	/*record different mode's startx of grabwindow */
		.starty = 6,	/*record different mode's starty of grabwindow */
		.grabwindow_width = 1584,	/*record different mode's width of grabwindow */
		.grabwindow_height = 1188,	/*record different mode's height of grabwindow */
	...
	.cap1 = {		/*capture for PIP 24fps relative information, capture1 mode must use same framelength,
		 ...
		 .grabwindow_height = 1188,
		 .mipi_data_lp2hs_settle_dc = 85,	/*unit , ns */
		 .max_framerate = 185,	/*less than 13M(include 13M),cap1 max framerate is 24fps,
	...
	.max_frame_length = 8191,	/*max framelength */
	.ae_shut_delay_frame = 0,	/*shutter delay frame for AE cycle,
							2 frame with isp_gain_delay-shut_delay=2-0=2 */
	.ae_sensor_gain_delay_frame = 1,	/*sensor gain delay frame for AE cycle,
						2 frame with isp_gain_delay-sensor_gain_delay=2-0=2 */
	.ae_isp_gain_delay_frame = 3,	/*isp gain delay frame for AE cycle */
...
static imgsensor_struct imgsensor = {
	...
	record current sensor mode,such as: INIT, Preview, Capture, Video,High Speed Video, Slim Video */
	.shutter = 0x3D0,	/*current shutter */
	.gain = 0x100,		/*current gain */
	.dummy_pixel = 0,	/*current dummypixel */
	.dummy_line = 0,	/*current dummyline */
	.current_fps = 185,	/*full size current fps : 24fps for PIP, 30fps for Normal or ZSD */
	.autoflicker_en = KAL_FALSE,	/*auto flicker enable:
	KAL_FALSE for disable auto flicker, KAL_TRUE for enable auto flicker */
	.test_pattern = KAL_FALSE,	/*test pattern mode or not. KAL_FALSE 
	for in test pattern mode, KAL_TRUE for normal output */
...
{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0, 0, 1600, 1200, 8, 6, 1584, 1188},	/*hight speed video */
{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0, 0, 1600, 1200, 8, 6, 1584, 1188}, /* * slim video */ */
{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0, 0, 1600, 1200, 8, 6, 1584, 1188},	/*custom1 */
{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0, 0, 1600, 1200, 8, 6, 1584, 1188},	/*custom2 */
{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0, 0, 1600, 1200, 8, 6, 1584, 1188},	/*custom3 */
{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0, 0, 1600, 1200, 8, 6, 1584, 1188},	/* custom4 */
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (256 > gain) {
		...
		write_cmos_sensor(0xb1, temp);	/* only digital gain 12.13 */
	} else {
		/* analogic gain */
		temp = 64 * gain / 256;
		/* temp = iReg - 256 + 64; */
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable) {
		...
		/* write_cmos_sensor(0x8b,0xb0); //bit[4]: 1 enable test pattern, 0 disable test pattern */
		write_cmos_sensor(0xfe, 0x00);
		...
		/* 0x5E00[8]: 1 enable,  0 disable */
		/* 0x5E00[1:0]; 00 Color bar, 01 Random Data, 10 Square, 11 BLACK */
		write_cmos_sensor(0x8b, 0xa0);	/* bit[4]: 1 enable test pattern, 0 disable test pattern */
	}
	spin_lock(&imgsensor_drv_lock);
			   ...
			   MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	/* LOG_INF("sensor mode num: sensor_info->SensorModeNum:%d, imgsensor_info.sensor_mode_num:%d\n", sensor_info->SensorModeNum, imgsensor_info.sensor_mode_num); */
