...
#include "gc2385mipi_chxt_rear_Sensor.h"

/*************** Modify Following Strings for Debug *********/
#define PFX "GC2385_chxt_rear_camera_sensor"
...
kal_bool GC2385_Rear_DuringTestPattern = KAL_FALSE;

static struct imgsensor_info_struct imgsensor_info = {
	/* record sensor id defined in Kd_imgsensor.h */
	.sensor_id = GC2385MIPI_CHXT_REAR_SENSOR_ID,
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if ((iReg >= ANALOG_GAIN_1) && (iReg < ANALOG_GAIN_2)) {
		...
		log_pr_debug("GC2385MIPI_CHXT_REAR analogic gain 1x, GC2385MIPI_CHXT_REAR add pregain = %d\n",
		     temp);
	}
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		log_pr_debug("GC2385MIPI_CHXT_REAR analogic gain 1.43x , GC2385MIPI_CHXT_REAR add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		log_pr_debug("GC2385MIPI_CHXT_REAR analogic gain 2x , GC2385MIPI_CHXT_REAR add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		log_pr_debug("GC2385MIPI_CHXT_REAR analogic gain 2.84x , GC2385MIPI_CHXT_REAR add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		log_pr_debug("GC2385MIPI_CHXT_REAR analogic gain 3.97x , GC2385MIPI_CHXT_REAR add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		log_pr_debug("GC2385MIPI_CHXT_REAR analogic gain 5.68x , GC2385MIPI_CHXT_REAR add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		log_pr_debug("GC2385MIPI_CHXT_REAR analogic gain 8.14x , GC2385MIPI_CHXT_REAR add pregain = %d\n",
		     temp);
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		log_pr_debug("GC2385MIPI_CHXT_REAR analogic gain 11.34x , GC2385MIPI_CHXT_REAR add pregain = %d\n",
		     temp);
	} else {
		write_cmos_sensor(0xfe, 0x00);
		...
		write_cmos_sensor(0xb1, temp >> 8);
		write_cmos_sensor(0xb2, temp & 0xff);
		log_pr_debug("GC2385MIPI_CHXT_REAR analogic gain 16.23x , GC2385MIPI_CHXT_REAR add pregain = %d\n",
		     temp);
	}
	return gain;
		...
		do {
			*sensor_id = return_sensor_id() & 0xffef;
			if (*sensor_id == imgsensor_info.sensor_id) {
				log_pr_info("i2c write id: 0x%x, sensor id: 0x%x\n",
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			log_pr_info("Read sensor id fail, write id: 0x%x, id: 0x%x\n",
			     imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
	...
	spin_unlock(&imgsensor_drv_lock);
	GC2385_Rear_DuringTestPattern = KAL_FALSE;
...
UINT32 GC2385MIPI_CHXT_REAR_RAW_SensorInit(
	struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	...
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}				/*    GC2385MIPI_CHXT_REAR_RAW_SensorInit    */
