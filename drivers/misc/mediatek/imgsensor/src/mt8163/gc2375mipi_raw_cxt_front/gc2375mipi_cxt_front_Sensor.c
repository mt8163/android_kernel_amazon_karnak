...
#include "gc2375mipi_cxt_front_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "GC2375_cxt_front_camera_sensor"
#define LOG_1 LOG_INF("GC2375_CXT_Front,MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC2375MIPI_CXT_FRONT_SENSOR_ID,	//record sensor id defined in Kd_imgsensor.h
	...
	.cap_delay_frame = 2,		//einter capture delay frame num
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0, 0, 1600, 1200}	// slim vide
};
...
static void gc2375_gcore_identify_otp(void)
{
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xfe, 0x00);
...
/*No Need to implement this function, mybe used for Dynamic frame adjustment later*/
}								/*    set_dummy  */
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if ((ANALOG_GAIN_1 <= iReg) && (iReg < ANALOG_GAIN_2))
	{
		...
		LOG_INF
			("GC2375MIPI_CXT_FRONT analogic gain 1x, GC2375MIPI_CXT_FRONT add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_2 <= iReg) && (iReg < ANALOG_GAIN_3))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("GC2375MIPI_CXT_FRONT analogic gain 1.43x , GC2375MIPI_CXT_FRONT add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_3 <= iReg) && (iReg < ANALOG_GAIN_4))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("GC2375MIPI_CXT_FRONT analogic gain 2x , GC2375MIPI_CXT_FRONT add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_4 <= iReg) && (iReg < ANALOG_GAIN_5))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("GC2375MIPI_CXT_FRONT analogic gain 2.84x , GC2375MIPI_CXT_FRONT add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_5 <= iReg) && (iReg < ANALOG_GAIN_6))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("GC2375MIPI_CXT_FRONT analogic gain 3.97x , GC2375MIPI_CXT_FRONT add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_6 <= iReg) && (iReg < ANALOG_GAIN_7))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("GC2375MIPI_CXT_FRONT analogic gain 5.68x , GC2375MIPI_CXT_FRONT add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_7 <= iReg) && (iReg < ANALOG_GAIN_8))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("GC2375MIPI_CXT_FRONT analogic gain 8.14x , GC2375MIPI_CXT_FRONT add pregain = %d\n",
			temp);
	} else if ((ANALOG_GAIN_8 <= iReg) && (iReg < ANALOG_GAIN_9))
	{
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("GC2375MIPI_CXT_FRONT analogic gain 11.34x , GC2375MIPI_CXT_FRONT add pregain = %d\n",
			temp);
	} else
	{
		write_cmos_sensor(0x20, 0x0c);
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF
			("GC2375MIPI_CXT_FRONT analogic gain 16.23x , GC2375MIPI_CXT_FRONT add pregain = %d\n",
			temp);
	}
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x92, STARTX);
	write_cmos_sensor(0x94, STARTY);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff)
	{
		...
		do
		{
			*sensor_id = return_sensor_id();
			...
			{
				printk
					("GC2375MIPI_CXT_FRONT i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, (*sensor_id + 2));
				return ERROR_NONE;
			}
			printk
				("GC2375MIPI_CXT_FRONT Read sensor id fail, write id: 0x%x, id: 0x%x\n",
...
UINT32 GC2375MIPI_CXT_FRONT_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT
	**pfFunc)
{
	...
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}								/*    GC2375MIPI_CXT_FRONT_RAW_SensorInit    */
