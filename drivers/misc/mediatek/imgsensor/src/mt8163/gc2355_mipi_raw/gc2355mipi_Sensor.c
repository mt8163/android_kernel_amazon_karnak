...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0000, 0000, 1600,
		1200, 0, 0, 1600, 1200},	/* Preview */
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0000, 0000, 1600,
		1200, 0, 0, 1600, 1200},	/* capture */
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0000, 0000, 1600,
		1200, 0, 0, 1600, 1200},	/* video */
	{1600, 1200, 0, 0, 1600, 1200, 640, 480, 0000, 0000, 640,
		480, 0, 0, 640, 480},	/* hight speed video */
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0000, 0000, 1600,
		1200, 0, 0, 1280, 720}
};				/* slim video */
...
static void set_shutter(kal_uint16 shutter)
{
	...
	shutter = min((kal_uint32)shutter,
...
#define ANALOG_GAIN_7 470	/* 7.15x */
	...
	if ((iReg >= ANALOG_GAIN_1) && (iReg < ANALOG_GAIN_2)) {
		...
		LOG_INF(
			"GC2355 analogic gain 1x, add pregain = %d\n", temp);
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF(
			"GC2355 analogic gain 1.45x, add pregain = %d\n", temp);
		...
		write_cmos_sensor(0xb2, (temp << 2) & 0xfc);
		LOG_INF(
			"GC2355 analogic gain 2.02x, add pregain = %d\n", temp);
	}
	/* else if(iReg >= ANALOG_GAIN_4) */
	else {
		/* analog gain */
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xe4, 0x01);
	write_cmos_sensor(0xe5, 0xff);	/*add by lucifer*/

	 /* MIPI */
	write_cmos_sensor(0xfe, 0x03);
	write_cmos_sensor(0x10, 0x81);
	write_cmos_sensor(0x01, 0x87);
	write_cmos_sensor(0x02, 0x01); //JIRA1108 0x11
	...
	write_cmos_sensor(0x22, 0x02); //JIRA1087 0x03
	write_cmos_sensor(0x23, 0x20);
	write_cmos_sensor(0x24, 0x02);
	write_cmos_sensor(0x25, 0x10);
	write_cmos_sensor(0x26, 0x05); //JIRA1087 0x08
	write_cmos_sensor(0x27, 0x06); //JIRA1108
	write_cmos_sensor(0x29, 0x02);	/* 04 */
	write_cmos_sensor(0x2a, 0x0a);
	write_cmos_sensor(0x2b, 0x05); //JIRA1108 0x08
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
	...
	if (enable)
		...
		/* bit[4]: 1 enable test pattern, 0 disable test pattern */
	else
		write_cmos_sensor(0x8b, 0xa2);
		/* bit[4]: 1 enable test pattern, 0 disable test pattern */
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			pr_err("%s fail, write id: 0x%x, id: 0x%x\n",
 ...
 *	*sensor_config_data : address pointer of line numbers in one
 *	*period of VSYNC
 *
 * RETURNS
 *	None
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
		LOG_INF("GET_DEFAULT_FRAME_RATE_BY_SCENARIO scenarioId:%d\n",
			(UINT32) * feature_data);
...
UINT32 GC2355_MIPI_RAW_SensorInit(
