...
void ovl_set_status(enum DISP_OVL1_STATUS status)
{
	DDPMSG("cascade, set_ovl1 from %s to %s!\n",
	       ovl_get_status_name(ovl1_status), ovl_get_status_name(status));
...
int ovl_is_idle(enum DISP_MODULE_ENUM module)
{
	...
	if (((DISP_REG_GET(idx_offset + DISP_REG_OVL_FLOW_CTRL_DBG) & 0x3ff) !=
	     0x1) &&
	    ((DISP_REG_GET(idx_offset + DISP_REG_OVL_FLOW_CTRL_DBG) & 0x3ff) !=
	     0x2)) {
		return 0;
	} else {
		return 1;
...
static int
ovl_layer_config(enum DISP_MODULE_ENUM module, unsigned int layer,
		 ...
		 unsigned char alpha, unsigned int sur_aen,
		 unsigned int src_alpha, unsigned int dst_alpha,
		 unsigned int constant_color, unsigned int yuv_range,
		 ...
		 void *handle)
{
...
#endif
	if (is_rotation &&
	    (get_boot_mode() == META_BOOT || get_boot_mode() == FACTORY_BOOT))
		is_rotation = false;
	...
	DDPDBG(
		"ovl%d, layer=%d, source=%s, off(x=%d, y=%d), dst(%d, %d, %d, %d),pitch=%d, ",
	       idx, layer, (source == 0) ? "memory" : "dim", src_x, src_y,
	       dst_x, dst_y, dst_w, dst_h, src_pitch);
	DDPDBG(
		"fmt=%s, addr=%lx, keyEn=%d, key=%d, aen=%d, alpha=%d,sur_aen=%d, ",
	       ovl_intput_format_name(fmt, input_swap), addr, key_en, key, aen,
	       alpha, sur_aen);
	DDPDBG("sur_alpha=0x%x, constant_color=0x%x, yuv_range=%d\n",
	       dst_alpha << 2 | src_alpha, constant_color, yuv_range);

	if (source == OVL_LAYER_SOURCE_RESERVED) {
		/* ==1, means constant color */
		if (aen == 0)
	...
	/*
	 * DISP_REG_SET_DIRTY(handle, DISP_REG_OVL_RDMA0_CTRL+layer_offset,
	 * 0x1);
	 */
	...
	DISP_REG_SET_DIRTY(handle, DISP_REG_OVL_L0_CLR + idx_offset + layer * 4,
			   constant_color);
	...
	if (is_rotation) {
		...
		if (module == DISP_MODULE_OVL1 &&
		    (ovl_get_status() == DDP_OVL1_STATUS_PRIMARY ||
		     ovl_get_status() == DDP_OVL1_STATUS_SUB_REQUESTING))
			bg_h = DISP_REG_GET(DISP_REG_OVL_ROI_SIZE);
		bg_w = bg_h & 0xFFFF;
		bg_h = bg_h >> 16;
		DISP_REG_SET_DIRTY(
			handle, DISP_REG_OVL_L0_OFFSET + layer_offset,
			(bg_h - dst_h - dst_y) << 16 | (bg_w - dst_w - dst_x));
		offset = (src_x + dst_w) * bpp +
			 (src_y + dst_h - 1) * src_pitch - 1;
	} else {
		DISP_REG_SET_DIRTY(handle,
				   DISP_REG_OVL_L0_OFFSET + layer_offset,
				   dst_y << 16 | dst_x);
	...
	if (!is_engine_sec) {
		...
		if (sec != DISP_SECURE_BUFFER) {
			...
			/*
			 * we need to tell cmdq to help map non-sec mva to sec
			 * mva
			 */
			if (is_rotation)
				cmdqRecWriteSecure(
					...
					disp_addr_convert(DISP_REG_OVL_L0_ADDR +
							  layer_offset),
					CMDQ_SAM_NMVA2_MVA_REVERSE,
					addr + offset, 0, size, m4u_port);
			else
				cmdqRecWriteSecure(
					handle,
					disp_addr_convert(DISP_REG_OVL_L0_ADDR +
							  layer_offset),
					CMDQ_SAM_NMVA_2_MVA, addr + offset, 0,
					size, m4u_port);
			...
			/*offset = src_x * bpp + src_y * src_pitch;*/
			cmdqRecWriteSecure(
				handle, disp_addr_convert(DISP_REG_OVL_L0_ADDR +
							  layer_offset),
	...
	if (key_en == 1)
		DISP_REG_SET_DIRTY(handle,
				   DISP_REG_OVL_L0_SRCKEY + layer_offset, key);

	value = (((sur_aen & 0x1) << 15) | ((dst_alpha & 0x3) << 6) |
		 ((dst_alpha & 0x3) << 4) | ((src_alpha & 0x3) << 2) |
		 (src_alpha & 0x3));
	...
	if (idx == 0) {
		if (primary_display_is_decouple_mode() == 0) {
			if (DISP_REG_GET(DISP_REG_OVL_RDMA0_MEM_GMC_SETTING +
					 layer_offset) != 0x6070)
				DISP_REG_SET(
					handle,
...
int ovl_clock_on(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	if (idx == 0)
		ddp_module_clock_enable(MM_CLK_DISP_OVL0, true);
	else
		ddp_module_clock_enable(MM_CLK_DISP_OVL1, true);
...
int ovl_clock_off(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	if (idx == 0)
		ddp_module_clock_enable(MM_CLK_DISP_OVL0, false);
	else
		ddp_module_clock_enable(MM_CLK_DISP_OVL1, false);
		...
		for (i = 0; i < OVL_LAYER_NUM_PER_OVL; i++) {
			...
			DDPMSG(
				"ovl_get_info:layer%d,en %d,w %d,h %d,bpp %d,addr %lu\n",
			       i, p->layer_en, p->src_w, p->src_h, p->bpp,
			       p->addr);
		}
	} else {
...
#if 0
/* dynamic switch between ovl0 and ovl0+ovl1, not used now */
static int ovl_layer_select(enum DISP_MODULE_ENUM module,
			    struct disp_ddp_path_config  *pConfig,
			    unsigned int *layer_min,
			    unsigned int *layer_max, void *handle)
{
	unsigned int i = 0;
	unsigned int cnt = 0;
	...
	if (cnt <= 4) {
		*layer_min = 0;
		*layer_max = 4;
	} else if (cnt > 4 && module == DISP_MODULE_OVL0) {
		*layer_min = 4;
		*layer_max = OVL_LAYER_NUM;
	} else if (cnt > 4 && module == DISP_MODULE_OVL1) {
		*layer_min = 0;
		*layer_max = 4;
	}
	/* is cmdq enabled, this dose not work */
	if (cnt > 4 && (DISP_REG_GET(DISP_OVL_INDEX_OFFSET +
					DISP_REG_OVL_EN) == 0)
	    && handle == NULL) {
		DDPERR(
			"layer cnt=%d, bigger than 4, but ovl1 not enabled!\n",
			cnt);
	}
	return 0;
}
...
#ifdef CONFIG_MTK_HDMI_SUPPORT
	else if (module == DISP_MODULE_OVL1 && is_hdmi_active() &&
		 !extd_disp_hdmi_is_decouple())
		mode = CMDQ_DISP_VIDEO_MODE;
#endif
...
static void
ovl_cmdq_insert_wait_frame_done_token_mira(enum DISP_MODULE_ENUM module,
					   void *handle)
{
	if ((module == DISP_MODULE_OVL0 ||
	     (module == DISP_MODULE_OVL1 &&
			...
			struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	if (!pConfig->ovl_dirty) {
		/*
		 * DDPERR("abnormal, called into ovl_config_l() but
		 * pConfig->ovl_dirty is 0!\n");
		 */
		return 0;
	}
	...
	if (has_sec_layer) {
		...
		if (ovl_is_sec[ovl_idx] == 1) {
			...
			if (ret)
				DDPAEE(
					"[SVP]fail to create disable handle %s ret=%d\n",
				       __func__, ret);
			 ...
			 * nonsec_switch_handle);
			 */
			ovl_cmdq_insert_wait_frame_done_token_mira(
				module, nonsec_switch_handle);
			cmdqRecSetSecure(nonsec_switch_handle, 1);
			cmdqRecSetSecureMode(nonsec_switch_handle, mode);

			/*
			 * we should disable ovl before new (nonsec) setting
			 * take effect
			 * or translation fault may happen,
			 * if we switch ovl to nonsec BUT its setting is still
			 * sec
			 */
	...
	for (i = layer_min; i < layer_max; i++) {
		if (pConfig->ovl_config[i].layer_en != 0) {
			...
			if (module == DISP_MODULE_OVL0 &&
			    DISP_REG_GET(DISP_REG_OVL_EN) == 0) {
				/*
				 * DDPERR("OVL0 EN=0 while config, force set to
				 * 1!\n");
				 */
			...
			if (module == DISP_MODULE_OVL1 &&
				...
				/*
				 * DDPERR("OVL1 EN=0 while config, force set to
				 * 1!\n");
				 */
				DISP_REG_SET(handle,
					     DISP_REG_OVL_EN +
			...
			DDPDBG(
				"O%d/L%d/S%d/%s/0x%lx/(%d,%d)/P%d/(%d,%d,%d,%d).\n",
			       module - DISP_MODULE_OVL0, i,
			       pConfig->ovl_config[i].source,
			       ovl_intput_format_name(
				       ovl_input_fmt_convert(
					       pConfig->ovl_config[i].fmt),
				       ovl_input_fmt_byte_swap(
					       ovl_input_fmt_convert(
						       pConfig->ovl_config[i]
							       .fmt))),
			       pConfig->ovl_config[i].addr,
			       ...
			       pConfig->ovl_config[i].dst_w,
			       pConfig->ovl_config[i].dst_h);
			mmprofile_log_ex(ddp_mmp_get_events()->ovl_enable,
			...
			if (DISP_REG_GET(DISP_REG_OVL_SRC_CON +
				...
				/* DDPMLOG("disable L%d.\n",i%4); */
				mmprofile_log_ex(
					ddp_mmp_get_events()->ovl_disable,
					MMPROFILE_FLAG_PULSE,
					((module - DISP_MODULE_OVL0) << 4) +
						(i % 4),
					0);
	...
	DISP_REG_SET(handle, DISP_REG_OVL_SRC_CON +
				     (module - DISP_MODULE_OVL0) *
					     DISP_OVL_INDEX_OFFSET,
		     layer_enable);
		...
		if (state == CMDQ_CHECK_IDLE_AFTER_STREAM_EOF) {
			if (module == DISP_MODULE_OVL0) {
				ret = cmdqRecPoll(cmdq_trigger_handle,
						  DISP_REG_OVL0_STATE_PA, 2,
						  0x3ff);
			} else if (module == DISP_MODULE_OVL1) {
				ret = cmdqRecPoll(cmdq_trigger_handle,
						  DISP_REG_OVL1_STATE_PA, 2,
						  0x3ff);
			} else {
...
void ovl_dump_reg(enum DISP_MODULE_ENUM module)
{
	...
	if (src_on & 0x1) {
		DDPDUMP(
			"OVL:0x030=0x%08x,0x034=0x%08x,0x038=0x%08x,0x03c=0x%08x\n",
		...
		DDPDUMP(
			"OVL:0xf40=0x%08x,0x044=0x%08x,0x0c0=0x%08x,0x0c8=0x%08x,0x0d0=0x%08x\n",
	...
	if (src_on & 0x2) {
		DDPDUMP(
			"OVL:0x050=0x%08x,0x054=0x%08x,0x058=0x%08x,0x05c=0x%08x\n",
		...
		DDPDUMP(
			"OVL:0xf60=0x%08x,0x064=0x%08x,0x0e0=0x%08x,0x0e8=0x%08x,0x0f0=0x%08x\n",
	...
	if (src_on & 0x4) {
		...
		DDPDUMP(
			"OVL:0xf80=0x%08x,0x084=0x%08x,0x100=0x%08x,0x110=0x%08x\n",
	...
	if (src_on & 0x8) {
		DDPDUMP(
			"OVL:0x090=0x%08x,0x094=0x%08x,0x098=0x%08x,0x09c=0x%08x\n",
		...
		DDPDUMP(
			"OVL:0xfa0=0x%08x,0x0a4=0x%08x,0x120=0x%08x,0x130=0x%08x\n",
...
static void ovl_printf_status(int idx, unsigned int status)
{
	...
	DDPDUMP(
		"rdma3_idle:%d,rdma2_idle:%d,rdma1_idle:%d, rdma0_idle:%d,rst:%d\n",
		(status >> 16) & (0x1),
		(status >> 17) & (0x1),
...
void ovl_dump_analysis(enum DISP_MODULE_ENUM module)
{
	...
	DDPDUMP(
		"ovl_en=%d,layer_enable(%d,%d,%d,%d),bg(w=%d, h=%d),cur_pos(x=%d,y=%d)\n",
		DISP_REG_GET(DISP_REG_OVL_EN + offset),
		  ...
		  void *cmdq)
{
	...
	case DISP_IOCTL_OVL_ENABLE_CASCADE: {
		...
		/*
		 * when add ovl1 to path, dst_dirty may be not 1, so we have to
		 * call ovl_roi
		 */
