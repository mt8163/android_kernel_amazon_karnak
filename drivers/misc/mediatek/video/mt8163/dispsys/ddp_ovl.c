...
static int ovl_layer_config(DISP_MODULE_ENUM module,
				unsigned int layer, unsigned int source,
				DpColorFormat format, unsigned long addr,
				...
				DISP_BUFFER_TYPE sec, unsigned int is_engine_sec, void *handle)
{
...
#endif
#endif
	if (is_rotation && (get_boot_mode() == META_BOOT || get_boot_mode() == FACTORY_BOOT))
		is_rotation = false;
	...
	DDPDBG("ovl%d, layer=%d, source=%s, off(x=%d, y=%d), dst(%d, %d, %d, %d),pitch=%d, ",
	       ...
	       dst_h,
	       src_pitch);
	DDPDBG("fmt=%s, addr=%lx, keyEn=%d, key=%d, aen=%d, alpha=%d,sur_aen=%d, ",
		...
		if (sec != DISP_SECURE_BUFFER) {
			...
			if (is_rotation)
				cmdqRecWriteSecure(handle,
					   ...
					   CMDQ_SAM_NMVA2_MVA_REVERSE, addr + offset, 0, size, m4u_port);
			else
				cmdqRecWriteSecure(handle,
					   disp_addr_convert(DISP_REG_OVL_L0_ADDR + layer_offset),
...
unsigned int ddp_ovl_get_cur_addr(bool rdma_mode, int layerid)
{
	...
	if (layerid < 4 &&
		...
		|| ovl_get_status() == DDP_OVL1_STATUS_SUB_REQUESTING)) {
		if (DISP_REG_GET(DISP_REG_OVL_RDMA0_CTRL + DISP_OVL_INDEX_OFFSET +
			layerid * 0x20) & 0x1)
			return (DISP_REG_GET(DISP_REG_OVL_L0_ADDR + DISP_OVL_INDEX_OFFSET +
					 layerid * 0x20));
			else
				return 0;
		} else {	/* OVL 0 */
...
static int ovl_layer_select(DISP_MODULE_ENUM module,
			    disp_ddp_path_config  *pConfig,
			    unsigned int *layer_min, unsigned int *layer_max, void *handle)
{
	unsigned int i = 0;
	unsigned int cnt = 0;
	...
	if (cnt > 4 && (DISP_REG_GET(DISP_OVL_INDEX_OFFSET + DISP_REG_OVL_EN) == 0)
	    && handle == NULL) {
		DDPERR("layer cnt=%d, bigger than 4, but ovl1 not enabled!\n", cnt);
	}
	return 0;
}
#endif
static int ovl_get_sec_mode(DISP_MODULE_ENUM module)
{
	...
	#ifdef CONFIG_MTK_HDMI_SUPPORT
	else if (module == DISP_MODULE_OVL1 && is_hdmi_active() && !extd_disp_hdmi_is_decouple())
			mode = CMDQ_DISP_VIDEO_MODE;
	#endif
...
static void ovl_cmdq_insert_wait_frame_done_token_mira(DISP_MODULE_ENUM module, void *handle)
{
	if ((module == DISP_MODULE_OVL0 ||
		(module == DISP_MODULE_OVL1 &&
		(ovl_get_status() == DDP_OVL1_STATUS_PRIMARY ||
		...
		cmdqRecWaitNoClear(handle, CMDQ_EVENT_MUTEX0_STREAM_EOF + extd_disp_get_mutex_id());
#endif
}
...
static int ovl_config_l(DISP_MODULE_ENUM module, disp_ddp_path_config *pConfig, void *handle)
{
	...
	if (!pConfig->ovl_dirty) {
		/* DDPERR("abnormal, called into ovl_config_l() but pConfig->ovl_dirty is 0!\n"); */
		return 0;
	}
	...
	if (has_sec_layer) {
		...
		if (ovl_is_sec[ovl_idx] == 1) {
			...
			ovl_cmdq_insert_wait_frame_done_token_mira(module, nonsec_switch_handle);
			...
			for (i = layer_min; i < layer_max; i++) {
				...
				ovl_layer_switch(module, i % 4, pConfig->ovl_config[i].layer_en, nonsec_switch_handle);
	...
	for (i = layer_min; i < layer_max; i++) {
		if (pConfig->ovl_config[i].layer_en != 0) {
			...
			ovl_layer_config(module, i % 4, pConfig->ovl_config[i].source,
				...
				DDPDBG("O%d/L%d/S%d/%s/0x%lx/(%d,%d)/P%d/(%d,%d,%d,%d).\n",
