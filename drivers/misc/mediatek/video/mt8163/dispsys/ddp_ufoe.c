...
static int ufoe_dump(enum DISP_MODULE_ENUM module, int level)
{
	pr_debug("==DISP UFOE REGS==\n");
	pr_debug("(0x000)UFOE_START =0x%x\n", DISP_REG_GET(DISP_REG_UFO_START));
	pr_debug("(0x000)UFOE_CFG0 =0x%x\n", DISP_REG_GET(DISP_REG_UFO_CFG_0B));
	pr_debug("(0x000)UFOE_CFG1 =0x%x\n", DISP_REG_GET(DISP_REG_UFO_CFG_1B));
	pr_debug("(0x000)UFOE_WIDTH =0x%x\n",
		 DISP_REG_GET(DISP_REG_UFO_FRAME_WIDTH));
	pr_debug("(0x000)UFOE_HEIGHT =0x%x\n",
		 DISP_REG_GET(DISP_REG_UFO_FRAME_HEIGHT));
	pr_debug("(0x000)UFOE_PAD  =0x%x\n",
...
int ufoe_start(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	if (ufoe_enable)
		DISP_REG_SET_FIELD(cmdq_handle, START_FLD_DISP_UFO_START,
				   DISP_REG_UFO_START, 1);
...
{
	DISP_REG_SET_FIELD(cmdq_handle, START_FLD_DISP_UFO_START,
			   DISP_REG_UFO_START, 0);
	DDPMSG("ufoe_stop, ufoe_start:0x%x\n",
	       DISP_REG_GET(DISP_REG_UFO_START));
	return 0;
}

static int ufoe_config(enum DISP_MODULE_ENUM module,
		       struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	if (lcm_config->ufoe_enable) {
		ufoe_enable = true;
		DISP_REG_SET_FIELD(handle, START_FLD_DISP_UFO_BYPASS,
				   DISP_REG_UFO_START,
				   0); /* disable BYPASS ufoe */

		if (lcm_config->ufoe_params.lr_mode_en == 1) {
			DISP_REG_SET_FIELD(handle, START_FLD_DISP_UFO_LR_EN,
					   DISP_REG_UFO_START, 1);
		} else {
			DISP_REG_SET_FIELD(handle, START_FLD_DISP_UFO_LR_EN,
					   DISP_REG_UFO_START, 0);
			if (lcm_config->ufoe_params.compress_ratio == 3) {
				unsigned int internal_width =
					...
					disp_if_config->width % 4;
				DISP_REG_SET_FIELD(
					handle,
					CFG_0B_FLD_DISP_UFO_CFG_COM_RATIO,
			...
			if (lcm_config->ufoe_params.vlc_disable) {
				DISP_REG_SET_FIELD(
					handle, CFG_0B_FLD_DISP_UFO_CFG_VLC_EN,
					DISP_REG_UFO_CFG_0B, 0);
				...
				DISP_REG_SET_FIELD(
					...
					DISP_REG_UFO_CFG_0B, 1);
				DISP_REG_SET(
					handle, DISP_REG_UFO_CFG_1B,
					(lcm_config->ufoe_params.vlc_config ==
					 0)
						? 5
						: lcm_config->ufoe_params
							  .vlc_config);
			}
		}
		DISP_REG_SET(handle, DISP_REG_UFO_FRAME_WIDTH,
			     disp_if_config->width);
		DISP_REG_SET(handle, DISP_REG_UFO_FRAME_HEIGHT,
			     disp_if_config->height);
	}
	/* ufoe_dump(); */
	return 0;
}
...
{
	DISP_REG_SET_FIELD(handle, START_FLD_DISP_UFO_SW_RST_ENGINE,
			   DISP_REG_UFO_START, 1);
	DISP_REG_SET_FIELD(handle, START_FLD_DISP_UFO_SW_RST_ENGINE,
			   DISP_REG_UFO_START, 0);
