...
int ufoe_start(DISP_MODULE_ENUM module, void *cmdq_handle)
{
	if (ufoe_enable)
		DISP_REG_SET_FIELD(cmdq_handle, START_FLD_DISP_UFO_START, DISP_REG_UFO_START, 1);
...
static int ufoe_config(DISP_MODULE_ENUM module, disp_ddp_path_config *pConfig, void *handle)
{
	...
	if (lcm_config->ufoe_enable) {
		...
		DISP_REG_SET_FIELD(handle, START_FLD_DISP_UFO_BYPASS,
			DISP_REG_UFO_START, 0);/* disable BYPASS ufoe */
