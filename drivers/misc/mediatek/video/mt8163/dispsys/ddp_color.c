...
void DpEngine_COLORonInit(DISP_MODULE_ENUM module, void *__cmdq)
{
	...
	if (g_color_bypass == 0) {
		...
		DISP_REG_SET_FIELD(cmdq, CFG_MAIN_FLD_COLOR_DBUF_EN, DISP_COLOR_CFG_MAIN + offset, 0x1);
		DISP_REG_SET_FIELD(cmdq, START_FLD_DISP_COLOR_START, DISP_COLOR_START + offset, 0x1);
	}
			    ...
			    void *__cmdq)
{
	...
	DISP_REG_SET(cmdq, DISP_COLOR_G_PIC_ADJ_MAIN_2, (0x200<<16) |
...
static void ddp_color_set_window(DISP_PQ_WIN_PARAM *win_param, void *__cmdq)
{
	...
	DISP_REG_MASK(cmdq, DISP_COLOR_DBG_CFG_MAIN + offset, (g_split_en<<3), 0x00000008);
	DISP_REG_SET(cmdq, DISP_COLOR_WIN_X_MAIN + offset, g_split_window_x);
	DISP_REG_SET(cmdq, DISP_COLOR_WIN_Y_MAIN + offset, g_split_window_y);
}
...
static unsigned long color_pa2va(unsigned int addr)
{
	...
	for (i = 0; i < DISP_REG_NUM; i++) {
		if ((addr >= ddp_reg_pa_base[i]) &&
			...
			COLOR_ERR("color_pa2va(), COLOR PA:0x%x, PABase[0x%x], VABase[0x%lx]\n",
...
static int _color_bypass(DISP_MODULE_ENUM module, int bypass)
{
	...
	if (bypass) {
		...
		DISP_REG_SET(NULL, DISP_COLOR_CFG_MAIN + offset, (0<<7)); /*color enable*/
		DISP_REG_SET(NULL, DISP_COLOR_START + offset, 0x00000001); /*color start*/

		/* enable R2Y/Y2R in Color Wrapper*/
		DISP_REG_SET(NULL, DISP_COLOR_CM1_EN + offset, 0x01);
		DISP_REG_SET(NULL, DISP_COLOR_CM2_EN + offset, 0x11);   /*also set no rounding on Y2R*/
	}
