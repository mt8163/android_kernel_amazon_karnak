...
enum DSI_LANE_NUM { ONE_LANE = 1, TWO_LANE = 2, THREE_LANE = 3, FOUR_LANE = 4 };
...
#define DISP_RDMA_INDEX_OFFSET                                                 \
	(dispsys_reg[DISP_REG_RDMA1] - dispsys_reg[DISP_REG_RDMA0])
#define DISP_OVL_INDEX_OFFSET                                                  \
	(dispsys_reg[DISP_REG_OVL1] - dispsys_reg[DISP_REG_OVL0])
#define DISP_WDMA_INDEX_OFFSET                                                 \
	(dispsys_reg[DISP_REG_WDMA1] - dispsys_reg[DISP_REG_WDMA0])
...
#define DDP_REG_IO_DRIVING dispsys_reg[DISP_REG_IO_DRIVING]
...
#define DISPSYS_REG_ADDR_MAX                                                   \
	0xffffffffffffffff
/* (dispsys_reg[0]+0x200000) */
...
#define READ_REGISTER_UINT32(reg) (*(/* volatile */ unsigned long *const)(reg))

#define WRITE_REGISTER_UINT32(reg, val)                                        \
	((*(/* volatile */ unsigned long *const)(reg)) = (val))

#define READ_REGISTER_UINT16(reg) (*(/* volatile */ uint16_t * const)(reg))

#define WRITE_REGISTER_UINT16(reg, val)                                        \
	((*(/* volatile */ uint16_t * const)(reg)) = (val))

#define READ_REGISTER_UINT8(reg) (*(/* volatile */ uint8_t * const)(reg))
...
#define OUTREG32(x, y)                                                         \
	WRITE_REGISTER_UINT32((unsigned long *)(x), (uint32_t)(y))
...
#define OUTREG32(x, y)                                                         \
	WRITE_REGISTER_UINT32((uint32_t *)((void *)(x)), (uint32_t)(y))
#define MASKREG32(x, y, z) OUTREG32(x, (INREG32(x) & ~(y)) | (z))
...
#define REG_FLD_MASK(field)                                                    \
	(((unsigned int)(1 << REG_FLD_WIDTH(field)) - 1)                       \
	 << REG_FLD_SHIFT(field))
...
#define DISP_CPU_REG_SET(reg32, val)                                           \
	do {                                                                   \
		...
		mt_reg_sync_writel(val, (/* volatile */ \
					unsigned long *)(reg32));    \
	} while (0)
...
#define DISP_REG_MASK(handle, reg32, val, mask)                                \
	do {                                                                   \
		...
		if (handle == NULL)                                            \
			mt_reg_sync_writel(                                    \
				(unsigned int)(INREG32(reg32) & ~(mask)) |     \
					(val),                                 \
				(/* volatile */ unsigned long *)(reg32));      \
		else                                                           \
			cmdqRecWrite((struct cmdqRecStruct *)handle,           \
...
extern void _disp_primary_path_exit_idle(const char *caller,
...
#define DISP_REG_SET_FIELD(handle, field, reg32, val)                          \
	do {                                                                   \
		...
		if (handle == 0) {                                             \
			mt_reg_sync_writel(                                    \
				(__raw_readl(reg32) & ~REG_FLD_MASK(field)) |  \
					REG_FLD_VAL((field), (val)),           \
				(reg32));                                      \
		} else {                                                       \
			dprec_reg_op((void *)handle, (unsigned long)(reg32),   \
				     val << REG_FLD_SHIFT(field),              \
				     REG_FLD_MASK(field));                     \
			cmdqRecWrite((struct cmdqRecStruct *)handle,           \
				     disp_addr_convert((unsigned long)reg32),  \
...
#define DISP_REG_CMDQ_POLLING(handle, reg32, val, mask)                        \
	do {                                                                   \
		if (handle == 0) {                                             \
			...
			dprec_reg_op((void *)handle, (unsigned long)reg32,     \
				     val, 0x00000000);                         \
			cmdqRecPoll((struct cmdqRecStruct *)handle,            \
				    disp_addr_convert((unsigned long)reg32),   \
				    val, mask);                                \
		}                                                              \
	} while (0)
...
#define DISP_REG_SET_CMDQ(handle, reg32, val)                                  \
	do {                                                                   \
		dprec_reg_op((void *)handle, (unsigned long)reg32, val,        \
			     0x00000000);                                      \
		cmdqRecWrite((struct cmdqRecStruct *)handle,                   \
			     disp_addr_convert((unsigned long)reg32), val,     \
			     ~0);                                              \
	} while (0)
...
#define DISP_REG_SET_CMDQ_MODULE(handle, reg32, val, module)                   \
	do {                                                                   \
		dprec_reg_op((void *)handle, (unsigned long)reg32, val,        \
			     0x00000000);                                      \
		cmdqRecWrite((struct cmdqRecStruct *)handle,                   \
			     ddp_reg_pa_base[module] + (reg32 & 0xfffl), val,  \
			     ~0);                                              \
	} while (0)
#define DISP_REG_SET_FIELD_CMDQ_MODULE(handle, field, reg32, val, module)      \
	do {                                                                   \
		dprec_reg_op((void *)handle, (unsigned long)(reg32),           \
			     ...
			     REG_FLD_MASK(field));                             \
		cmdqRecWrite((struct cmdqRecStruct *)handle,                   \
			     ddp_reg_pa_base[module] + (reg32 & 0xfffl),       \
			     val << REG_FLD_SHIFT(field),                      \
			     REG_FLD_MASK(field));                             \
	} while (0)
...
#define DISP_REG_CONFIG_DPI1_SEL_IN (DISPSYS_CONFIG_BASE + 0x068)
...
#define DISP_REG_CLK_CFG_0_CLR                                                 \
	(DISPSYS_CONFIG_BASE + 0x48) /* BIT31  1 clear for mms */
#define DISP_REG_CLK_CFG_1_CLR                                                 \
	(DISPSYS_CONFIG_BASE + 0x58) /* bit7 1 clear for pwm */
#define DISP_REG_CLK_CFG_6_DPI (DISPSYS_CONFIG_BASE + 0xA0) /* bit7 0 */
#define DISP_REG_CLK_CFG_6_CLR                                                 \
	(DISPSYS_CONFIG_BASE + 0xA8) /* bit7 1 clear for dpi */
...
#define DPI0_SEL_IN_FLD_DPI1_SEL_IN REG_FLD(2, 16)
...
#define L_CON_FLD_LARC                                                         \
	REG_FLD(2, 28) /* layer source 0:mem, 1:constant color */
...
/* DSC*/
#define DISP_REG_DSC_CON (DISPSYS_DSC_BASE + 0x000)
#define DISP_REG_DSC_INTEN (DISPSYS_DSC_BASE + 0x004)
#define DISP_REG_DSC_INTSTA (DISPSYS_DSC_BASE + 0x008)
#define DISP_REG_DSC_INTACK (DISPSYS_DSC_BASE + 0x00C)
#define DISP_REG_DSC_STA (DISPSYS_DSC_BASE + 0x010)
#define DISP_REG_DSC_PIC_W (DISPSYS_DSC_BASE + 0x018)
#define DISP_REG_DSC_PIC_H (DISPSYS_DSC_BASE + 0x01C)
#define DISP_REG_DSC_SLICE_W (DISPSYS_DSC_BASE + 0x020)
#define DISP_REG_DSC_SLICE_H (DISPSYS_DSC_BASE + 0x024)
#define DISP_REG_DSC_CHUNK_SIZE (DISPSYS_DSC_BASE + 0x028)
#define DISP_REG_DSC_BUF_SIZE (DISPSYS_DSC_BASE + 0x02C)
#define DISP_REG_DSC_MODE (DISPSYS_DSC_BASE + 0x030)
#define DISP_REG_DSC_CFG (DISPSYS_DSC_BASE + 0x034)
#define DISP_REG_DSC_DBG_CON (DISPSYS_DSC_BASE + 0x060)
#define DISP_REG_DSC_CKSM_MON0 (DISPSYS_DSC_BASE + 0x064)
#define DISP_REG_DSC_CKSM_MON1 (DISPSYS_DSC_BASE + 0x068)
#define DISP_REG_DSC_RESV (DISPSYS_DSC_BASE + 0x06C)
#define DISP_REG_DSC_PPS0 (DISPSYS_DSC_BASE + 0x080)
#define DISP_REG_DSC_PPS1 (DISPSYS_DSC_BASE + 0x084)
#define DISP_REG_DSC_PPS2 (DISPSYS_DSC_BASE + 0x088)
#define DISP_REG_DSC_PPS3 (DISPSYS_DSC_BASE + 0x08C)
#define DISP_REG_DSC_PPS4 (DISPSYS_DSC_BASE + 0x090)
#define DISP_REG_DSC_PPS5 (DISPSYS_DSC_BASE + 0x094)
#define DISP_REG_DSC_PPS6 (DISPSYS_DSC_BASE + 0x098)
#define DISP_REG_DSC_PPS7 (DISPSYS_DSC_BASE + 0x09C)
#define DISP_REG_DSC_PPS8 (DISPSYS_DSC_BASE + 0x0A0)
#define DISP_REG_DSC_PPS9 (DISPSYS_DSC_BASE + 0x0A4)
#define DISP_REG_DSC_PPS10 (DISPSYS_DSC_BASE + 0x0A8)
#define DISP_REG_DSC_PPS11 (DISPSYS_DSC_BASE + 0x0AC)
#define DISP_REG_DSC_PPS12 (DISPSYS_DSC_BASE + 0x0B0)
#define DISP_REG_DSC_PPS13 (DISPSYS_DSC_BASE + 0x0B4)
#define DISP_REG_DSC_PPS14 (DISPSYS_DSC_BASE + 0x0B8)
#define DISP_REG_DSC_PPS15 (DISPSYS_DSC_BASE + 0x0BC)
#define DISP_REG_DSC_PPS16 (DISPSYS_DSC_BASE + 0x0C0)
#define DISP_REG_DSC_PPS17 (DISPSYS_DSC_BASE + 0x0C4)
#define DISP_REG_DSC_PPS18 (DISPSYS_DSC_BASE + 0x0C8)
#define DISP_REG_DSC_PPS19 (DISPSYS_DSC_BASE + 0x0CC)

#define START_FLD_DISP_DSC_START REG_FLD(1, 0)
#define START_FLD_DISP_DSC_IN_SEL REG_FLD(1, 3)
#define START_FLD_DISP_DSC_BYPASS REG_FLD(1, 4)
#define START_FLD_DISP_DSC_RELAY REG_FLD(1, 5)
#define START_FLD_DISP_DSC_SW_RESET REG_FLD(1, 8)
#define START_FLD_DISP_DSC_UFOE_SEL REG_FLD(1, 16)
