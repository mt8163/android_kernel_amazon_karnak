...
static void _DSI_INTERNAL_IRQ_Handler(DISP_MODULE_ENUM module, unsigned int param)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (status.RD_RDY) {
			waitRDDone = true;
			/* clear interrupt */
			DSI_OUTREGBIT(NULL, struct DSI_INT_STATUS_REG, DSI_REG[d]->DSI_INTSTA,
					      RD_RDY, 0);
		...
		if (status.TE_RDY) {
			...
			DSI_OUTREGBIT(NULL, struct DSI_INT_STATUS_REG, DSI_REG[i]->DSI_INTSTA,
					      TE_RDY, 0);
	...
	int DSI_WaitVMDone(DISP_MODULE_ENUM module)
{
		...
		/*...dsi video is always in busy state... */
		if (_dsi_is_video_mode(module)) {
			pr_debug("DSI_WaitVMDone error: should set DSI to CMD mode firstly\n");
	...
	DSI_STATUS DSI_RestoreRegisters(DISP_MODULE_ENUM module, cmdqRecHandle cmdq)
{
	...
	int DSI_BIST_Pattern_Test(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, bool enable,
				  unsigned int color) {
		int i = 0;
...
/* unsigned int fmod = 30;//Fmod = 30KHz by default */
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
			...
			MIPITX_OUTREGBIT(struct MIPITX_DSI_CLOCK_LANE_REG,
					 DSI_PHY_REG[i]->MIPITX_DSI_CLOCK_LANE, RG_DSI_LNTC_RT_CODE,
					 0x8);
			MIPITX_OUTREGBIT(struct MIPITX_DSI_DATA_LANE3_REG,
					 DSI_PHY_REG[i]->MIPITX_DSI_DATA_LANE3, RG_DSI_LNT3_RT_CODE,
					 0x8);
			MIPITX_OUTREGBIT(struct MIPITX_DSI_DATA_LANE2_REG,
					 DSI_PHY_REG[i]->MIPITX_DSI_DATA_LANE2, RG_DSI_LNT2_RT_CODE,
					 0x8);
			MIPITX_OUTREGBIT(struct MIPITX_DSI_DATA_LANE1_REG,
					 DSI_PHY_REG[i]->MIPITX_DSI_DATA_LANE1, RG_DSI_LNT1_RT_CODE,
					 0x8);
			MIPITX_OUTREGBIT(struct MIPITX_DSI_DATA_LANE0_REG,
					 DSI_PHY_REG[i]->MIPITX_DSI_DATA_LANE0, RG_DSI_LNT0_RT_CODE,
					 0x8);
			...
			if (0 != data_Rate) {
				...
				if (1 != dsi_params->ssc_disable) {
					...
					ASSERT(delta1 <= 8);
					pdelta1 =
					    (delta1 * data_Rate * txdiv * 262144 + 281664) / 563329;
					...
					/* DSI_OUTREGBIT(MIPITX_DSI_PLL_CON1_REG,DSI_PHY_REG->MIPITX_DSI_PLL_CON1,
...
#define NS_TO_CYCLE(n, c)	((n) / (c))
		...
		/* -------------------------------------------------------------- */
		timcon1.DA_HS_EXIT =
		    (dsi_params->DA_HS_EXIT == 0) ? NS_TO_CYCLE(0xC8,
								cycle_time) :
		    dsi_params->DA_HS_EXIT;

		timcon2.CLK_TRAIL =
		    ((dsi_params->CLK_TRAIL == 0) ? NS_TO_CYCLE(0x64,
								cycle_time) : dsi_params->CLK_TRAIL)
		    + 0x01;
		...
		timcon3.CLK_HS_EXIT =
		    (dsi_params->CLK_HS_EXIT == 0) ? NS_TO_CYCLE(0xC8,
								 cycle_time) :
		    dsi_params->CLK_HS_EXIT;
		timcon3.CLK_HS_POST =
		    (dsi_params->CLK_HS_POST == 0) ? NS_TO_CYCLE((0x3c + 0x34 * ui),
								 cycle_time) :
		...
		if (on) {
			...
			for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
				...
				MIPITX_OUTREGBIT(struct MIPITX_DSI_BG_CON_REG,
						 DSI_PHY_REG[i]->MIPITX_DSI_BG_CON, RG_DSI_BG_CKEN,
						 0);
				MIPITX_OUTREGBIT(struct MIPITX_DSI_BG_CON_REG,
	...
	void DSI_set_cmdq_V3(DISP_MODULE_ENUM module, cmdqRecHandle cmdq,
		...
		do {
			...
			if (data_id == REGFLAG_ESCAPE_ID && cmd == REGFLAG_DELAY_MS_V3) {
				udelay(1000 * count);
				pr_debug("DISP/DSI " "DSI_set_cmdq_V3[%d]. Delay %d (ms)\n", index,
					count);
			...
			if (0 != DSI_REG->DSI_MODE_CTRL.MODE) {	/* not in cmd mode */
				...
				if (count > 1) {
					...
					for (i = 0; i < count; i++) {
						goto_addr =
						    (UINT32) (&DSI_VM_CMD_REG->data[0].byte1) + i;
						mask_para = (0xFF << ((goto_addr & 0x3) * 8));
						set_para =
...
#endif
			{
				DSI_WaitForNotBusy(module, cmdq);
				{
					...
					if (count > 1) {
						...
						DSI_OUTREG32(cmdq, &DSI_CMDQ_REG[0]->data[0].byte0,
							     AS_UINT32(&t2));

						goto_addr =
						    (unsigned long)(&DSI_CMDQ_REG[0]->
								    data[1].byte0);
						mask_para = (0xFFu << ((goto_addr & 0x3u) * 8));
						set_para = (cmd << ((goto_addr & 0x3u) * 8));
						DSI_MASKREG32(cmdq,
					...
					for (i = 0; i < count; i++) {
						...
						DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_CMDQ_SIZE,
							     2 + (count) / 4);
					} else {
						t0.CONFG = 0;
						t0.Data0 = cmd;
					if (count) {
						...
						DSI_OUTREG32(cmdq, &DSI_CMDQ_REG[0]->data[0],
							     AS_UINT32(&t0));
						DSI_OUTREG32(cmdq, &DSI_REG[0]->DSI_CMDQ_SIZE, 1);
					}

	/* for (i = 0; i < AS_UINT32(&DSI_REG->DSI_CMDQ_SIZE); i++) */
	/* pr_debug("DISP/DSI " "DSI_set_cmdq_V3[%d]. DSI_CMDQ+%04x : 0x%08x\n",
			index, i*4, INREG32(DSI_BASE + 0x180 + i*4)); */

					if (force_update) {
						MMProfileLog(MTKFB_MMP_Events.DSICmd,
							     MMProfileFlagStart);
						DSI_Start(module, cmdq);
						DSI_WaitForNotBusy(module, cmdq);
						MMProfileLog(MTKFB_MMP_Events.DSICmd,
	...
	static void lcm_mdelay(uint32_t ms)
{
	...
	void DSI_set_cmdq_V2_DSI0(void *cmdq, unsigned cmd, unsigned char count,
		...
		/*cmdqRecHandle cmdq_hd =(cmdqRecHandle)cmdq;for_kernel_warning */
	...
	void DSI_ChangeClk(DISP_MODULE_ENUM module, uint32_t clk)
{
	...
	int ddp_dsi_init(DISP_MODULE_ENUM module, void *cmdq_handle)
{
		...
		node = of_find_compatible_node(NULL, NULL, "mediatek,mt8163-apmixedsys");
		if (!node)
		...
		/* pr_debug("dsi_init: dsi = 0x%lx, mipi = 0x%lx, dsi_cmdq = 0x%lx\n",DSI_REG[0],
		...
		ddp_set_mipi26m(1);
			...
			if (module == DISP_MODULE_DSI0 /* || module == DISP_MODULE_DSIDUAL */) {
				...
				if (ret > 0)
					pr_debug("DISP/DSI DSI0 power manager API return FALSE\n");
...
force_config:
	...
	int ddp_dsi_start(DISP_MODULE_ENUM module, void *cmdq_handle)
{
		int i = 0;
		...
		if (module == DISP_MODULE_DSIDUAL) {
			...
			DSI_OUTREGBIT(cmdq, struct DSI_COM_CTRL_REG, DSI_REG[0]->DSI_COM_CTRL, DSI_DUAL_EN,
				      1);
			DSI_OUTREGBIT(cmdq, Dstruct SI_COM_CTRL_REG, DSI_REG[1]->DSI_COM_CTRL, DSI_DUAL_EN,
				      1);
...
#endif
		{
			DSI_Send_ROI(module, cmdq_handle, 0, 0, _dsi_context[i].lcm_width,
				     _dsi_context[i].lcm_height);
			DSI_SetMode(module, cmdq_handle, _dsi_context[i].dsi_params.mode);
	...
	int ddp_dsi_clk_off(DISP_MODULE_ENUM module, void *cmdq_handle, unsigned int level)
{
		...
		case DDP_CABC_MODE:
			{
				...
				DSI_OUTREGBIT(cmdq_handle, struct DSI_MODE_CTRL_REG, DSI_REG[0]->DSI_MODE_CTRL,
				      MIX_MODE, 1); /*send command in MIX MODE*/
	...
	int ddp_dsi_trigger(DISP_MODULE_ENUM module, void *cmdq_handle)
{
		int i = 0;
		unsigned int data_array[16];
	...
	void PanelMaster_DSI_set_timing(uint32_t dsi_index, MIPI_TIMING timing)
{
		...
		case HS_PRPR:
			...
			/* OUTREGBIT(DSI_PHY_TIMCON0_REG, struct DSI_REG->DSI_PHY_TIMECON0,HS_PRPR,timing.value); */
			break;
		case HS_ZERO:
			if (dsi_index == PM_DSI0) {
				DSI_OUTREGBIT(NULL, struct DSI_PHY_TIMCON0_REG,
	...
	int DSI_test_roi(int x, int y)
{
		...
		if (ret == 0)
			...
			DSI_Send_ROI(DISP_MODULE_DSI0, NULL, 0, 0, _dsi_context[0].lcm_width,
				     _dsi_context[0].lcm_height);
		}
