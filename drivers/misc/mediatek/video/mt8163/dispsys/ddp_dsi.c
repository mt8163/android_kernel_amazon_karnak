...
#define DSI_CLRREGBIT(cmdq, TYPE, REG, bit, value)              \
			do {												\
				TYPE r; 										\
				mt_reg_sync_writel(0xFFFFFFFF, &r);				\
				r.bit = (value);								\
				DISP_REG_SET(cmdq, &REG, INREG32(&r));			\
			} while (0)
...
#define MIPITX_Write60384(slave_addr, write_addr, write_data)                  \
	{                                                                      \
		...
		mt_reg_sync_writel(((unsigned int)slave_addr << 0x1),          \
				   MIPITX_BASE + 0x04);                        \
		...
		mt_reg_sync_writel(0x1, MIPITX_BASE + 0x14);                   \
		mt_reg_sync_writel(0x1, MIPITX_BASE + 0x18);                   \
		mt_reg_sync_writel(((unsigned int)slave_addr << 0x1) + 1,      \
				   MIPITX_BASE + 0x04);                        \
		mt_reg_sync_writel(0x1, MIPITX_BASE + 0x24);                   \
		while (INREG32(MIPITX_BASE + 0xC) & 0x1 != 0x1)                \
			;                                                      \
...
#define MIPITX_OUTREGBIT(TYPE, REG, bit, value)                                \
	{                                                                      \
		do {                                                           \
			...
			*(unsigned long *)(&r) = (0x00000000UL);               \
			r.bit = value;                                         \
			MIPITX_OUTREG32(&REG, AS_UINT32(&r));                  \
		} while (0);                                                   \
	}

#define MIPITX_MASKREG32(x, y, z)                                              \
	MIPITX_OUTREG32(x, (MIPITX_INREG32(x) & ~(y)) | (z))
#else
#define MIPITX_INREG32(addr)                                                   \
	({                                                                     \
		unsigned int val = 0;                                          \
...
unsigned long long cmd_done_times;
unsigned long long te_done_times;
#endif
				      ...
				      unsigned int param)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (status.RD_RDY) {
			waitRDDone = true;
			/* clear interrupt */
			DSI_CLRREGBIT(NULL, struct DSI_INT_STATUS_REG,
...
{
	int i = 0;
	/*DSI_MODE_CTRL_REG tmpreg;for_kernel_warning */
...
#if 0
static void DSI_WaitForNotBusy(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq)
{
	int i = 0;
	...
	if (cmdq) {
		for (i = DSI_MODULE_BEGIN(module);
			...
			DSI_POLLREG32(cmdq,
				&DSI_REG[i]->DSI_INTSTA, 0x80000000, 0x0);
		return;
	}
					 ...
					 unsigned int mode)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (mode == 0) { /* V2C */
				  ...
				  */
			DSI_OUTREGBIT(cmdq, struct DSI_MODE_CTRL_REG,
				      DSI_REG[i]->DSI_MODE_CTRL, V2C_SWITCH_ON,
				      1);
		} else /* C2V */
			/*
			 * DSI_OUTREGBIT(cmdq,
			 * DSI_MODE_CTRL_REG,
			 * DSI_REG[i]->DSI_MODE_CTRL,V2C_SWITCH_ON,0);
			 */
			DSI_OUTREGBIT(cmdq, struct DSI_MODE_CTRL_REG,
 ...
 * {
 * if(bypass == 0){
 * DSI_OUTREGBIT(cmdq, struct
 * DSI_RACK_REG,DSI_REG[i]->DSI_RACK,DSI_RACK_BYPASS,0);
 * }
 * else
 * DSI_OUTREGBIT(cmdq, struct
 * DSI_RACK_REG,DSI_REG[i]->DSI_RACK,DSI_RACK_BYPASS,1);
 *
 * }
 *
 * return DSI_STATUS_OK;
...
enum DSI_STATUS DSI_DisableClk(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq)
{
#if 0
		int i = 0;
...
void DSI_lane0_ULP_mode(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq,
		bool enter)
{
	int i = 0;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (enter) {
			DSI_OUTREGBIT(cmdq, struct DSI_PHY_LD0CON_REG,
				      DSI_REG[i]->DSI_PHY_LD0CON, L0_HS_TX_EN,
				      0);
			mdelay(1);
			DSI_OUTREGBIT(cmdq, struct DSI_PHY_LD0CON_REG,
				      DSI_REG[i]->DSI_PHY_LD0CON, L0_ULPM_EN,
				      1);
			mdelay(1);
		} else {
			DSI_OUTREGBIT(cmdq, struct DSI_PHY_LD0CON_REG,
			...
			mdelay(1);
			DSI_OUTREGBIT(cmdq, struct DSI_PHY_LD0CON_REG,
				      DSI_REG[i]->DSI_PHY_LD0CON, L0_WAKEUP_EN,
				      1);
			mdelay(1);
			DSI_OUTREGBIT(cmdq, struct DSI_PHY_LD0CON_REG,
				      DSI_REG[i]->DSI_PHY_LD0CON, L0_WAKEUP_EN,
...
void DSI_clk_ULP_mode(enum DISP_MODULE_ENUM module, struct cmdqRecStruct *cmdq,
		      bool enter)
{
	int i = 0;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (enter) {
			...
			DSI_OUTREGBIT(cmdq, struct DSI_PHY_LCCON_REG,
				      DSI_REG[i]->DSI_PHY_LCCON, LC_WAKEUP_EN,
				      1);
			mdelay(1);
...
enum DSI_STATUS DSI_DumpRegisters(enum DISP_MODULE_ENUM module, int level)
{
	...
	if (level >= 0) {
		if (module ==
		    DISP_MODULE_DSI0 /* || module == DISP_MODULE_DSIDUAL */) {
	...
	if (level >= 1) {
		if (module ==
			...
			DDPDUMP("DSI_DBG6_Status :0x%08x\n", DSI_DBG6_Status);
			...
			for (i = 0; i < sizeof(struct DSI_CMDQ_REGS); i += 16) {
				DDPDUMP(
					...
					i, INREG32((DDP_REG_BASE_DSI0 + 0x200 +
						    i)),
			...
			if (module == DISP_MODULE_DSI1 ||
				...
				for (i = 0; i < sizeof(DSI_CMDQ_REGS);
						i += 16) {
					DDPDUMP(
						"DSI_CMD+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n",
						i,
		...
		struct cmdqRecStruct *cmdq)
{
	...
	/*
	 * unsigned int count = 0;
	 * unsigned int tmp = 0; for_kernel_warning
	 */
	static const long WAIT_TIMEOUT = HZ / 10; /* 0.1 sec */
	int ret = 0;

	if (cmdq) {
		/*
		 * for(i = DSI_MODULE_BEGIN(module);i <=
		 * DSI_MODULE_END(module);i++)
		 */
	 ...
	 * INREG32(&DSI_REG[i]->DSI_STATE_DBG6))
	 *
	 * //msleep(1);
	 *
	 * if(count++>1000000000)
...
enum DSI_STATUS DSI_SleepOut(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq)
{
	int i = 0;
	/* wake_up_prd *1024*cycle time > 1ms */
	int wake_up_prd =
		(_dsi_context[i].dsi_params.PLL_CLOCK * 2 * 1000) / (1024 * 8) +
		0x1;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DSI_OUTREGBIT(cmdq, struct DSI_TIME_CON0_REG,
			      ...
			      wake_up_prd); /* cycle to 1ms for 520MHz */
	}

	return DSI_STATUS_OK;
}

enum DSI_STATUS DSI_Wakeup(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq)
{
	int i = 0;
	int ret = 0;
...
enum DSI_STATUS DSI_BackupRegisters(enum DISP_MODULE_ENUM module,
				    struct cmdqRecStruct *cmdq)
{
	int i = 0;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if ((DSI_REG[i]->DSI_MODE_CTRL.V2C_SWITCH_ON == 0x01) || (regs->DSI_MODE_CTRL.V2C_SWITCH_ON == 0x01)) {
			DISPMSG("DSI_BackupRegisters i:%d first DSI+0x14:%08x, real register:%08x DSI+0x0160 : 0x%08x DSI+0x0164 : 0x%08x\n",
				i,AS_UINT32(&regs->DSI_MODE_CTRL),AS_UINT32(&DSI_REG[i]->DSI_MODE_CTRL),AS_UINT32(&DSI_REG[i]->DSI_STATE_DBG6),AS_UINT32(&DSI_REG[i]->DSI_STATE_DBG7));
		}
		...
		pr_debug("DSI_BackupRegisters VM_CMD_EN %d TS_VFP_EN %d\n",
			 DSI_REG[i]->DSI_VM_CMD_CON.VM_CMD_EN,
			 DSI_REG[i]->DSI_VM_CMD_CON.TS_VFP_EN);

		if ((DSI_REG[i]->DSI_MODE_CTRL.V2C_SWITCH_ON == 0x01) || (regs->DSI_MODE_CTRL.V2C_SWITCH_ON == 0x01)) {
			DISPMSG("DSI_BackupRegisters i:%d after DSI+0x14:%08x, real register:%08x DSI+0x0160 : 0x%08x DSI+0x0164 : 0x%08x\n",
				i,AS_UINT32(&regs->DSI_MODE_CTRL),AS_UINT32(&DSI_REG[i]->DSI_MODE_CTRL),AS_UINT32(&DSI_REG[i]->DSI_STATE_DBG6),AS_UINT32(&DSI_REG[i]->DSI_STATE_DBG7));
		}
	}
...
enum DSI_STATUS DSI_RestoreRegisters(enum DISP_MODULE_ENUM module,
				     struct cmdqRecStruct *cmdq)
{
	int i = 0;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if ((DSI_REG[i]->DSI_MODE_CTRL.V2C_SWITCH_ON == 0x01) || (regs->DSI_MODE_CTRL.V2C_SWITCH_ON == 0x01)) {
			DISPMSG("DSI_RestoreRegisters begin i:%d DSI+0x14:%08x,now RealRegister DSI+0x14:%08x, DSI+0x0160 : 0x%08x DSI+0x0164 : 0x%08x",
				i,AS_UINT32(&regs->DSI_MODE_CTRL),AS_UINT32(&DSI_REG[i]->DSI_MODE_CTRL),AS_UINT32(&DSI_REG[i]->DSI_STATE_DBG6),AS_UINT32(&DSI_REG[i]->DSI_STATE_DBG7));
		}
		...
		pr_debug("DSI_RestoreRegisters VM_CMD_EN %d TS_VFP_EN %d\n",
			 regs->DSI_VM_CMD_CON.VM_CMD_EN,
			 regs->DSI_VM_CMD_CON.TS_VFP_EN);
		if ((DSI_REG[i]->DSI_MODE_CTRL.V2C_SWITCH_ON == 0x01) || (regs->DSI_MODE_CTRL.V2C_SWITCH_ON == 0x01)) {
			DISPMSG("DSI_RestoreRegisters after i:%d DSI+0x14:%08x,now RealRegister DSI+0x14:%08x, DSI+0x0160 : 0x%08x DSI+0x0164 : 0x%08x",
				i,AS_UINT32(&regs->DSI_MODE_CTRL),AS_UINT32(&DSI_REG[i]->DSI_MODE_CTRL),AS_UINT32(&DSI_REG[i]->DSI_STATE_DBG6),AS_UINT32(&DSI_REG[i]->DSI_STATE_DBG7));
		}
	}
	return DSI_STATUS_OK;
...
int DSI_BIST_Pattern_Test(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq,
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		if (enable) {
			DSI_OUTREG32(cmdq, &DSI_REG[i]->DSI_BIST_PATTERN,
				     color);
			/*
			 * DSI_OUTREG32(&DSI_REG->DSI_BIST_CON,
			 * AS_UINT32(&temp_reg));
			 */
			/*
			 * DSI_OUTREGBIT(DSI_BIST_CON_REG,
			 * DSI_REG->DSI_BIST_CON, SELF_PAT_MODE, 1);
			 */
			DSI_OUTREGBIT(cmdq, struct DSI_BIST_CON_REG,
				      DSI_REG[i]->DSI_BIST_CON, SELF_PAT_MODE,
				      1);
			...
			/*
			 * if disable dsi pattern, need enable mutex, can't just
			 * start dsi
			 ...
			 * DSI_REG[i]->DSI_BIST_CON, SELF_PAT_MODE, 0);
			 */
		...
		struct LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (dsi_params->mode == SYNC_EVENT_VDO_MODE ||
			...
			ASSERT(dsi_params->horizontal_backporch * dsiTmpBufBpp >
			       9);
			horizontal_backporch_byte =
				(dsi_params->horizontal_backporch *
					 dsiTmpBufBpp -
		...
		struct LCM_DSI_PARAMS *dsi_params, int w, int h)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (dsi_params->ufoe_enable &&
			...
			if (dsi_params->ufoe_params.compress_ratio ==
			    3) { /* 1/3 */
				 ...
				 struct LCM_DSI_PARAMS *dsi_params)
{
	...
	/*bool cksm_en = TRUE;for_kernel_warning */
	/*bool ecc_en = TRUE;for_kernel_warning */
	int lane_num = dsi_params->LANE_NUM;
	int vc_num = 0;
	bool null_packet_en = false;
	/*bool err_correction_en = FALSE;for_kernel_warning */
...
void DSI_PHY_clk_setting(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq,
		struct LCM_DSI_PARAMS *dsi_params)
{
#ifdef CONFIG_FPGA_EARLY_PORTING
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		MIPITX_OUTREGBIT(struct MIPITX_DSI_BG_CON_REG,
				 DSI_PHY_REG[i]->MIPITX_DSI_BG_CON,
				 RG_DSI_BG_FAST_CHARGE, 1);

		/* step 3 */
		mdelay(1);

		/* step 4 */
		MIPITX_OUTREGBIT(struct MIPITX_DSI_TOP_CON_REG,
				 DSI_PHY_REG[i]->MIPITX_DSI_TOP_CON,
				 RG_DSI_LNT_HS_BIAS_EN, 1);

		/* step 5 */
		MIPITX_OUTREGBIT(struct MIPITX_DSI_CON_REG,
		...
		/* step 6 */
		MIPITX_OUTREGBIT(struct MIPITX_DSI_PLL_PWR_REG,
		...
		if (data_Rate != 0) {
			 ...
			 */
			/*
			 * pcw = data_Rate*4*txdiv/(26*2);//Post DIV =4, so need
			 * data_Rate*4
			 */
			...
			if (dsi_params->ssc_disable != 1) {
				...
				ASSERT(delta1 <= 8);
				pdelta1 = (delta1 * data_Rate * txdiv * 262144 +
					   281664) /
					  563329;
				MIPITX_OUTREGBIT(
		...
		MIPITX_OUTREGBIT(struct MIPITX_DSI_PLL_TOP_REG,
				 DSI_PHY_REG[i]->MIPITX_DSI_PLL_TOP,
				 RG_MPPLL_PRESERVE, 2);
		...
		MIPITX_OUTREGBIT(struct MIPITX_DSI_PLL_CHG_REG,
		       ...
		       struct LCM_DSI_PARAMS *dsi_params)
{
	...
	if (dsi_params->PLL_CLOCK != 0) {
		...
		pr_debug(
			"[DISP]DSI_PHY_TIMCONFIG, Cycle Time = %d(ns),Unit I = %d(ns). , lane# = %d\n",
...
/*
 * cycle_time = (1000 * div2 * div1 * pre_div * post_div)/ (fbk_sel *
 * (fbk_div+0x01) * 26) + 1;
 */
/*
 * ui = (1000 * div2 * div1 * pre_div * post_div)/ (fbk_sel * (fbk_div+0x01) *
 * 26 * 2) + 1;
 */
#define NS_TO_CYCLE(n, c) ((n) / (c))
	...
	hs_trail_m = 1;
	hs_trail_n =
		(dsi_params->HS_TRAIL == 0)
			? NS_TO_CYCLE(((hs_trail_m * 0x4) + 0x60), cycle_time)
			: dsi_params->HS_TRAIL;
	/* +3 is recommended from designer becauase of HW latency */
	timcon0.HS_TRAIL =
		((hs_trail_m > hs_trail_n) ? hs_trail_m : hs_trail_n) + 0x01;
	...
	/*
	 * timcon1.TA_SACK         = (dsi_params->TA_SACK == 0) ? 1 :
	 * dsi_params->TA_SACK;
	 */
	timcon1.TA_GET = (dsi_params->TA_GET == 0) ? (0x5 * timcon0.LPX)
						   : dsi_params->TA_GET;
	timcon1.TA_SURE = (dsi_params->TA_SURE == 0) ? (0x3 * timcon0.LPX / 0x2)
						     : dsi_params->TA_SURE;
	...
	timcon1.DA_HS_EXIT = (dsi_params->DA_HS_EXIT == 0)
				     ? NS_TO_CYCLE(0xC8, cycle_time)
				     : dsi_params->DA_HS_EXIT;

	timcon2.CLK_TRAIL =
		((dsi_params->CLK_TRAIL == 0) ? NS_TO_CYCLE(0x64, cycle_time)
	...
	/*
	 * timcon2.LPX_WAIT        = (dsi_params->LPX_WAIT == 0) ? 1 :
	 * dsi_params->LPX_WAIT;
	 */
	...
	timcon3.CLK_HS_EXIT = (dsi_params->CLK_HS_EXIT == 0)
				      ? NS_TO_CYCLE(0xC8, cycle_time)
				      : dsi_params->CLK_HS_EXIT;
	timcon3.CLK_HS_POST =
		(dsi_params->CLK_HS_POST == 0)
			? NS_TO_CYCLE((0x3c + 0x34 * ui), cycle_time)
	...
	DDPMSG(
		"DSI_PHY_TIMCONFIG, HS_TRAIL = %d, HS_ZERO = %d, HS_PRPR = %d, LPX = %d, TA_GET = %d\n",
		timcon0.HS_TRAIL, timcon0.HS_ZERO, timcon0.HS_PRPR, timcon0.LPX,
		timcon1.TA_GET);
	DDPMSG(
		"TA_SURE = %d, TA_GO = %d, CLK_TRAIL = %d, CLK_ZERO = %d, CLK_HS_PRPR = %d\n",
		...
		int on)
{
	...
	if (on) {
		...
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
			...
			MIPITX_OUTREG32(&DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON1,
					0x00000000);
			MIPITX_OUTREG32(&DSI_PHY_REG[i]->MIPITX_DSI_PLL_CON2,
					0x50000000);
			mdelay(1);
		}
	}
...
enum DSI_STATUS DSI_EnableClk(enum DISP_MODULE_ENUM module,
		struct cmdqRecStruct *cmdq)
{
#if 0
		DISPFUNC();
...
#endif
	{
		if (DSI_REG[0]->DSI_MODE_CTRL.V2C_SWITCH_ON) {
			DISPMSG("DSI_Start DSI+0x0014 : 0x%08x DSI+0x0160 : 0x%08x DSI+0x0164 : 0x%08x\n", INREG32(DDP_REG_BASE_DSI0 + 0x0014),
				INREG32(DDP_REG_BASE_DSI0 + 0x0160),INREG32(DDP_REG_BASE_DSI0 + 0x0164));
		}
				 ...
				 uint8_t *buffer, uint8_t buffer_size)
{
	...
	struct DSI_T0_INS t0;
	static const long WAIT_TIMEOUT = HZ / 2; /* 0.5 sec */
	int ret;
	/*int timeout = 0;*/
	...
	for (d = DSI_MODULE_BEGIN(module); d <= DSI_MODULE_END(module); d++) {
		...
		do {
			...
			t0.CONFG = 0x04; /* /BTA */
			/*
			 * 0xB0 is used to distinguish DCS cmd or Gerneric
			 * cmd, is that Right???
			 */
			t0.Data_ID = (cmd < 0xB0)
			...
			/* / 4: read data */
			ret = wait_event_interruptible_timeout(
				_dsi_dcs_read_wait_queue[d],
				!(DSI_REG[d]->DSI_INTSTA.RD_RDY), WAIT_TIMEOUT);
			/*
			 * if (ret > 0) {
			 * do {
			 * timeout++;
			 ...
			 * DSI_REG[d]->DSI_RACK,
			 * DSI_RACK, 1);
			 * } while (DSI_REG[d]->DSI_INTSTA.BUSY && (timeout
			 * < 1000));
			 * }
			 * waitRDDone = false;
			 * if (timeout == 1000)
	...
	for (d = DSI_MODULE_BEGIN(module); d <= DSI_MODULE_END(module); d++) {
		if (DSI_REG[d]->DSI_MODE_CTRL.MODE != 0) { /* not in cmd mode */
			...
			if (cmd < 0xB0) {
				if (count > 1) {
					/*
					 * DSI_CMDQ cmd_data;for_kernel_warning
					 */
					t2.CONFG = 2;
		     ...
		     unsigned char force_update)
{
	...
	do {
		...
		if (data_id == REGFLAG_ESCAPE_ID &&
			...
			udelay(1000 * count);
			pr_debug(
				"DISP/DSI DSI_set_cmdq_V3[%d]. Delay %d (ms)\n",
				index, count);

			continue;
		}
#if 0
			if (DSI_REG->DSI_MODE_CTRL.MODE != 0) {
				DSI_VM_CMD_CON_REG vm_cmdq;

				OUTREG32(&vm_cmdq,
					AS_UINT32(&DSI_REG->DSI_VM_CMD_CON));
				pr_debug("set cmdq in VDO mode\n");
				if (count > 1) {
					vm_cmdq.LONG_PKT = 1;
					vm_cmdq.CM_DATA_ID = data_id;
					vm_cmdq.CM_DATA_0 = count + 1;
					OUTREG32(&DSI_REG->DSI_VM_CMD_CON,
						AS_UINT32(&vm_cmdq));
					...
					set_para = (cmd <<
						((goto_addr & 0x3) * 8));
					MASKREG32(goto_addr & (~0x3),
						mask_para, set_para);

					for (i = 0; i < count; i++) {
						goto_addr =
						    (UINT32) (&DSI_VM_CMD_REG->
							data[0].byte1) + i;
						mask_para = (0xFF << ((
							goto_addr & 0x3) * 8));
						set_para =
				...
				if (force_update) {
					mmprofile_log_ex(
						...
						*(unsigned int *)
						(&DSI_VM_CMD_REG->data[0]),
						*(unsigned int *)
						(&DSI_VM_CMD_REG->data[1]));
					DSI_EnableVM_CMD();
					...
					mmprofile_log_ex(
						...
						MMPROFILE_FLAG_END,
					       *(unsigned int *)
					       (&DSI_VM_CMD_REG->data[2]),
					       *(unsigned int *)
					       (&DSI_VM_CMD_REG->data[3]));
				}
			} else
#endif
		{
			...
			{
				/*
				 * for(i = 0; i < sizeof(DSI_CMDQ_REG->data0) /
				 * sizeof(DSI_CMDQ); i++)
				 */
				/* OUTREG32(&DSI_CMDQ_REG->data0[i], 0); */
				/*
				 * memset(&DSI_CMDQ_REG->data[0], 0,
				 * sizeof(DSI_CMDQ_REG->data[0]));
				 */
				...
				/*
				 * for (i = 0; i <
				 * AS_UINT32(&DSI_REG->DSI_CMDQ_SIZE); i++)
				 */
				/*
				 * pr_debug("DISP/DSI " "DSI_set_cmdq_V3[%d].
				 * DSI_CMDQ+%04x : 0x%08x\n",
				 * index, i*4, INREG32(DSI_BASE +
				 * 0x180 + i*4));
				 */

				if (force_update) {
					mmprofile_log(MTKFB_MMP_Events.DSICmd,
						      MMPROFILE_FLAG_START);
					DSI_Start(module, cmdq);
					DSI_WaitForNotBusy(module, cmdq);
					mmprofile_log(MTKFB_MMP_Events.DSICmd,
		...
		if (DSI_REG[i]->DSI_MODE_CTRL.MODE != 0) {
			...
			if (queue_size > 1) {	/* long packet */
				...
				vm_cmdq.CM_DATA_0 =
					((pdata[0] >> 16) & 0xFF);
				vm_cmdq.CM_DATA_1 = 0;
				OUTREG32(&DSI_REG[i]->DSI_VM_CMD_CON,
					AS_UINT32(&vm_cmdq));
			...
			/* start DSI VM CMDQ */
			if (force_update) {
				mmprofile_log_ex(MTKFB_MMP_Events.DSICmd,
					MMPROFILE_FLAG_START,
					*(unsigned int *)
					(&DSI_VM_CMD_REG->data[0]),
					*(unsigned int *)
			...
			/*
			 * printk("[DISP] - kernel - DSI_set_cmdq. DSI_CMDQ+%04x
			 * : 0x%08x\n",
...
{
	memcpy((struct LCM_DSI_PARAMS *)dst, (struct LCM_DSI_PARAMS *)src,
...
{
	/*
	 * struct cmdqRecStruct *cmdq_hd =
	 * (struct cmdqRecStruct *)cmdq;for_kernel_warning
	 */
...
void DSI_set_cmdq_V2_DSI1(void *cmdq, unsigned int cmd, unsigned char count,
			  unsigned char *para_list, unsigned char force_update)
{
	/*struct cmdqRecStruct *cmdq_hd =(struct cmdqRecStruct *)cmdq; */
...
unsigned int DSI_dcs_read_lcm_reg_v2_wrapper_DSIDUAL(uint8_t cmd,
						     uint8_t *buffer,
						     uint8_t buffer_size)
{
...
#ifdef CONFIG_MTK_LEGACY
	utils->set_gpio_out =
		(int (*)(unsigned int, unsigned int))mt_set_gpio_out;
	utils->set_gpio_mode =
		(int (*)(unsigned int, unsigned int))mt_set_gpio_mode;
	utils->set_gpio_dir =
		(int (*)(unsigned int, unsigned int))mt_set_gpio_dir;
	utils->set_gpio_pull_enable =
		(int (*)(unsigned int, unsigned char))mt_set_gpio_pull_enable;
#else
	/*
	 * TODO: attach replacements of these functions if using kernel
	 * standardization...
	 */
...
int ddp_dsi_init(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	DSI_VM_CMD_REG[1] =
		(struct DSI_VM_CMDQ_REGS *)(DISPSYS_DSI0_BASE + 0x134);
	/*
	 * pr_debug("dsi_init: dsi = 0x%lx, mipi = 0x%lx, dsi_cmdq =
	 * 0x%lx\n",DSI_REG[0],
	 * DSI_PHY_REG[0],DSI_CMDQ_REG[0]);
	 */
	...
	disp_register_module_irq_callback(DISP_MODULE_DSI0,
		...
		disp_register_module_irq_callback(DISP_MODULE_DSI1,
				_DSI_INTERNAL_IRQ_Handler);
		disp_register_module_irq_callback(DISP_MODULE_DSIDUAL,
				_DSI_INTERNAL_IRQ_Handler);
#endif
...
#ifndef CONFIG_FPGA_EARLY_PORTING
	if (MIPITX_IsEnabled(module, cmdq_handle)) {
#endif
		...
		if (module ==
			...
			ret += ddp_module_clock_enable(MM_CLK_DSI_DIGITAL,
						       true);

			if (ret > 0)
				pr_debug(
					"DISP/DSI DSI0 power manager API return FALSE\n"
					);
		}
#endif
		...
		DSI_OUTREGBIT(NULL, struct DSI_INT_ENABLE_REG,
			      DSI_REG[0]->DSI_INTEN, SLEEPOUT_DONE, 1);
					  ...
					  struct LCM_DSI_PARAMS *dsi_params)
{
	...
	struct DSI_PHY_TIMCON3_REG
		timcon3; /* CLK_HS_EXIT, CLK_HS_POST, CLK_HS_PREP */
		   ...
		   struct disp_ddp_path_config *config, void *cmdq)
{
	...
	/*int mipitx_config_dirty = 0;for_kernel_warning */
	if (!config->dst_dirty) {
	...
	{
		DISPCHECK(
			"MIPITX is not inited, will config mipitx clock now\n");
...
int ddp_dsi_start(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	int i = 0;
		...
		if (module == DISP_MODULE_DSIDUAL) {
			...
			DSI_OUTREGBIT(cmdq, struct DSI_COM_CTRL_REG,
					DSI_REG[0]->DSI_COM_CTRL, DSI_DUAL_EN,
				      1);
			DSI_OUTREGBIT(cmdq, Dstruct SI_COM_CTRL_REG,
					DSI_REG[1]->DSI_COM_CTRL, DSI_DUAL_EN,
				      1);

			DSI_SetMode(module, cmdq,
					_dsi_context[i].dsi_params.mode);
			DSI_clk_HS_mode(module, cmdq, TRUE);
		} else
#endif
	{
		DSI_Send_ROI(module, cmdq_handle, 0, 0,
			     _dsi_context[i].lcm_width,
			     _dsi_context[i].lcm_height);
		DSI_SetMode(module, cmdq_handle,
			    _dsi_context[i].dsi_params.mode);
		DSI_clk_HS_mode(module, cmdq_handle, true);
	}
...
int ddp_dsi_stop(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (DSI_REG[0]->DSI_MODE_CTRL.V2C_SWITCH_ON) {
		DISPMSG("dsi stop DSI+0x0014 : 0x%08x DSI+0x0160 : 0x%08x DSI+0x0164 : 0x%08x\n", INREG32(DDP_REG_BASE_DSI0 + 0x0014),
			INREG32(DDP_REG_BASE_DSI0 + 0x0160),INREG32(DDP_REG_BASE_DSI0 + 0x0164));
	}
...
int ddp_dsi_switch_lcm_mode(enum DISP_MODULE_ENUM module, void *params)
{
	...
	if (mode == 0) {
		...
		/*
		 * DSI_OUTREGBIT(NULL,
		 * DSI_INT_ENABLE_REG,DSI_REG[i]->DSI_INTEN,EXT_TE,1);
		 */
		...
		wait_event_interruptible(_dsi_wait_vm_cmd_done_queue[i],
					 wait_vm_cmd_done);
#if 0
			DSI_OUTREG32(NULL, (unsigned long)(DSI_REG[i]) + 0x130,
				0x00001539 | (lcm_cmd.addr << 16) |
				(lcm_cmd.val[1] << 24));/* DM = 0 */
			DSI_OUTREGBIT(NULL, DSI_START_REG,
				DSI_REG[i]->DSI_START, VM_CMD_START, 0);
			DSI_OUTREGBIT(NULL, DSI_START_REG,
				DSI_REG[i]->DSI_START, VM_CMD_START, 1);
			wait_vm_cmd_done = false;
			wait_event_interruptible(
				_dsi_wait_vm_cmd_done_queue[i],
				wait_vm_cmd_done);
#endif
	}
	return 0;
...
/*V2C */
		...
		DSI_OUTREGBIT(cmdq_handle, struct DSI_START_REG,
			      DSI_REG[i]->DSI_START, VM_CMD_START, 1);
		DSI_MASKREG32(cmdq_handle, 0xF4020028, 0x1,
			      0x1); /* reset mutex for V2C */
		DSI_MASKREG32(cmdq_handle, 0xF4020028, 0x1, 0x0); /*  */
		DSI_MASKREG32(cmdq_handle, 0xF4020030, 0x1,
			      0x0); /* mutext to cmd  mode */
...
/* DSI_SetBypassRack(module, NULL, 0); */
		...
		DSI_SetSwitchMode(module, cmdq_handle,
				  1); /* EXT TE could not use C2V */
		DSI_MASKREG32(cmdq_handle, 0xF4020030, 0x1,
			      0x1); /* mutext to video mode */
		DSI_OUTREG32(cmdq_handle,
			     (unsigned long)(DSI_REG[i]) + 0x200 + 0,
			     0x00001500 | (lcm_cmd.addr << 16) |
				     (lcm_cmd.val[0] << 24));
		DSI_OUTREG32(cmdq_handle,
			     (unsigned long)(DSI_REG[i]) + 0x200 + 4,
			     0x00000020);
		DSI_OUTREG32(cmdq_handle, (unsigned long)(DSI_REG[i]) + 0x60,
			     2);
		DSI_Start(module,
			  cmdq_handle); /* ???????????????????????????????? */
		DSI_MASKREG32(NULL, 0xF4020020, 0x1,
			      0x1); /* release mutex for video mode */
		cmdqRecFlush(cmdq_handle);
		cmdqRecReset(cmdq_handle);
		cmdqRecWaitNoClear(cmdq_handle, CMDQ_SYNC_TOKEN_STREAM_EOF);
#else
		...
		DSI_SetSwitchMode(module, NULL,
				  1); /* EXT TE could not use C2V */
		DSI_MASKREG32(NULL, 0xF4020030, 0x1,
			      0x1); /* mutext to video mode */
		DSI_OUTREG32(NULL, (unsigned int)(DSI_REG[i]) + 0x200 + 0,
			     0x00001500 | (lcm_cmd.addr << 16) |
				     (lcm_cmd.val[0] << 24));
		DSI_OUTREG32(NULL, (unsigned int)(DSI_REG[i]) + 0x204 + 0,
			     0x00000020);
...
int ddp_dsi_clk_on(enum DISP_MODULE_ENUM module, void *cmdq_handle,
		   unsigned int level)
{
	int ret = 0;
	/* if(level>0) */
	...
	if (module == DISP_MODULE_DSI0 || module == DISP_MODULE_DSIDUAL) {
		...
		if (ret > 0)
			pr_warn(
				"DISP/DSI DSI power manager API return FALSE\n"
				);
	}
	if (level > 0)
		DSI_PHY_clk_switch(module, NULL, true);

	pr_debug("ddp_dsi_clk_on.\n");

	return ret;
}

int ddp_dsi_clk_off(enum DISP_MODULE_ENUM module, void *cmdq_handle,
		    unsigned int level)
{
	int ret = 0;
		  ...
		  unsigned int ioctl_cmd, unsigned long *params)
{
	...
	case DDP_STOP_VIDEO_MODE: {
		/*
		 * ths caller should call wait_event_or_idle for frame stop
		 * event then.
		 */
		DSI_SetMode(module, cmdq_handle, CMD_MODE);
		/* TODO: modify this with wait event */
	...
	case DDP_SWITCH_DSI_MODE: {
		DISPMSG("enter DDP_SWITCH_DSI_MODE \n");
	...
	case DDP_CABC_MODE: {
		...
		pr_err("ADD CMD SIZE CABC MODE = %d\n", mode);
		DSI_OUTREG32(cmdq_handle, &DSI_CMDQ_REG[0]->data[0],
			     AS_UINT32(&t0));
		DSI_OUTREG32(cmdq_handle, &DSI_REG[0]->DSI_CMDQ_SIZE, 1);
		DSI_OUTREGBIT(cmdq_handle, struct DSI_MODE_CTRL_REG,
			      DSI_REG[0]->DSI_MODE_CTRL, MIX_MODE,
			      1); /*send command in MIX MODE*/
...
int ddp_dsi_trigger(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	int i = 0;
	unsigned int data_array[16];
	...
	if (_dsi_context[i].dsi_params.mode == CMD_MODE) {
		data_array[0] = 0x002c3909;
		DSI_set_cmdq(module, cmdq_handle, data_array, 1, 0);
	}
...
int ddp_dsi_power_on(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (!s_isDsiPowerOn) {
		...
		/* DSI_Reset(module, NULL); */
		if (module == DISP_MODULE_DSIDUAL) {
			DSI_OUTREG32(NULL, 0xF401A050,
				     _dsi_context[i].lcm_width);
			DSI_OUTREG32(NULL, 0xF401A054,
				     _dsi_context[i].lcm_height);
...
int ddp_dsi_power_off(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (s_isDsiPowerOn) {
		...
		if (module == DISP_MODULE_DSI0 ||
		    module == DISP_MODULE_DSIDUAL) {
			ret += ddp_module_clock_enable(MM_CLK_DSI_ENGINE,
						       false);
			ret += ddp_module_clock_enable(MM_CLK_DSI_DIGITAL,
						       false);
			if (ret > 0)
...
void dsi_analysis(enum DISP_MODULE_ENUM module)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DISPMSG(
			"DSI%d IRQ,RD_RDY:%d,CMD_DONE:%d,SP_DONE:%d,TE_RDY:%d,VM_CMD_DONE:%d,VM_DONE:%d\n",
	...
	struct ddp_lcm_read_cmd_table *read_table)
{
	...
	DSI_OUTREGBIT(cmdq_trigger_handle, struct DSI_MODE_CTRL_REG,
	  DSI_REG[0]->DSI_MODE_CTRL, MODE, CMD_MODE);
	...
	for (i = 0; i < 1; i++) {
		...
		if (*read_Slot && dsi_i == 0) {
			cmdqRecBackupRegisterToSlot(cmdq_trigger_handle,
							*read_Slot, 2*i,
							0x14012074);
			cmdqRecBackupRegisterToSlot(cmdq_trigger_handle,
							*read_Slot, 2*i+1,
		       ...
		       enum CMDQ_STATE state)
{
	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		if (module == DISP_MODULE_DSI0) {
			...
			ret = cmdqRecClearEventToken(cmdq_trigger_handle,
						     CMDQ_EVENT_DSI_TE);
		...
		/* need waiting te */
		if (module == DISP_MODULE_DSI0)
			ret = cmdqRecPoll(cmdq_trigger_handle, 0x1401200C, 0,
					  0x80000000);

#if 0
			else if (module == DISP_MODULE_DSI1) {
				ret = cmdqRecPoll(cmdq_trigger_handle,
						0x1401c00c, 0, 0x80000000);
			} else if (module == DISP_MODULE_DSIDUAL) {
				ret = cmdqRecPoll(cmdq_trigger_handle,
						0x1401b00c, 0, 0x80000000);
				ret = cmdqRecPoll(cmdq_trigger_handle,
						0x1401c00c, 0, 0x80000000);
			}
#endif
		...
		for (i = 0; i < 3; i++) {
			...
			/*
			 * 1. wait DSI RD_RDY(must clear, in case of cpu RD_RDY
			 * interrupt handler)
			 */
			if (dsi_i == 0) { /* DSI0 */
				ret = cmdqRecPoll(
			...
			if (hSlot) {
				cmdqRecBackupRegisterToSlot(cmdq_trigger_handle,
								hSlot, 2*i,
								disp_addr_convert((unsigned long)&DSI_REG[dsi_i]->DSI_RX_DATA0));
				cmdqRecBackupRegisterToSlot(cmdq_trigger_handle,
								hSlot, 2*i+1,
								disp_addr_convert((unsigned long)&DSI_REG[dsi_i]->DSI_RX_DATA1));

				/*
				 * cmdqRecBackupRegisterToSlot(
				 * cmdq_trigger_handle,
				 * hSlot, i, dsi_i ? 0x1401c074 : 0x1401b074);
				 */
			}

			/* 3. write RX_RACK */
			DSI_OUTREGBIT(cmdq_trigger_handle, struct DSI_RACK_REG,
				      DSI_REG[dsi_i]->DSI_RACK, DSI_RACK, 0);
			DSI_OUTREGBIT(cmdq_trigger_handle, struct DSI_RACK_REG,
		...
		/*
		 * DSI_OUTREGBIT(cmdq_trigger_handle,
		 * DSI_INT_ENABLE_REG,DSI_REG[dsi_i]->DSI_INTEN,RD_RDY,0);
		 */
		...
		for (i = 0; i < 3; i++) {
			...
			if (hSlot) {
				...
				cmdqBackupReadSlot(hSlot, 2*i,
							(uint32_t *)&read_data0);
				cmdqBackupReadSlot(hSlot, 2*i+1,
							(uint32_t *)&read_data1);
			} else {
			...
			mmprofile_log_ex(
				ddp_mmp_get_events()->esd_rdlcm,
				MMPROFILE_FLAG_PULSE, AS_UINT32(&read_data0),
				AS_UINT32(
					&(dsi_params->lcm_esd_check_table[i])));
		...
		DSI_POLLREG32(cmdq_trigger_handle,
			&DSI_REG[i]->DSI_INTSTA,
			0x80000000, 0);

		DSI_SetMode(module, cmdq_trigger_handle, CMD_MODE);

		/* 1. polling dsi not busy */
		if (i == 0) { /* DSI0/DUAL */ /* polling vm done */
			/*
			 * ret = cmdqRecPoll(cmdq_trigger_handle,
			 * disp_addr_convert((unsigned long)(DSI_REG[i])+0xc),
			 * 0, 0x00000010);
			 */
			/* polling dsi busy */
			ret = cmdqRecPoll(
		...
		if (module == DISP_MODULE_DSIDUAL) {
			/*
			 * must set DSI_START to 0 before set dsi_dual_en, don't
			 * know why.2014.02.15
			 */
			DSI_OUTREGBIT(cmdq_trigger_handle, struct DSI_START_REG,
				      DSI_REG[0]->DSI_START, DSI_START, 0);
...
struct DDP_MODULE_DRIVER ddp_driver_dsi0 = {.module = DISP_MODULE_DSI0,
					    .init = ddp_dsi_init,
...
struct DDP_MODULE_DRIVER ddp_driver_dsi1 = {.module = DISP_MODULE_DSI1,
					    .init = ddp_dsi_init,
					    .deinit = ddp_dsi_deinit,
					    .config = ddp_dsi_config,
					    .build_cmdq = ddp_dsi_build_cmdq,
...
struct DDP_MODULE_DRIVER ddp_driver_dsidual = {.module = DISP_MODULE_DSIDUAL,
					       .init = ddp_dsi_init,
					       .deinit = ddp_dsi_deinit,
					       .config = ddp_dsi_config,
					       .build_cmdq = ddp_dsi_build_cmdq,
...
void PanelMaster_DSI_set_timing(uint32_t dsi_index, struct MIPI_TIMING timing)
{
	...
	case HS_PRPR:
		...
		/*
		 * OUTREGBIT(DSI_PHY_TIMCON0_REG, struct
		 * DSI_REG->DSI_PHY_TIMECON0,HS_PRPR,timing.value);
		 */
		break;
	case HS_ZERO:
		if (dsi_index == PM_DSI0) {
		...
		/*
		 * OUTREGBIT(DSI_PHY_TIMCON0_REG,DSI_REG->DSI_PHY_TIMECON0,
	...
	case TA_GO:
		...
		/*
		 * OUTREGBIT(DSI_PHY_TIMCON1_REG,DSI_REG->DSI_PHY_TIMECON1,
		 * TA_GO,timing.value);
		 */
		break;
	case TA_SURE:
	...
	case CONT_DET:
		...
		/*
		 * OUTREGBIT(DSI_PHY_TIMCON2_REG,DSI_REG->DSI_PHY_TIMECON2,
	...
	case CLK_HS_PRPR:
		...
		/*
		 * OUTREGBIT(DSI_PHY_TIMCON3_REG,DSI_REG->DSI_PHY_TIMECON3,
	...
	case HPW:
		if (dsi_params->mode == SYNC_EVENT_VDO_MODE ||
			...
			timing.value =
				(timing.value * fbconfig_dsiTmpBufBpp - 10);
		}
	...
	case HBP:
		if (dsi_params->mode == SYNC_EVENT_VDO_MODE ||
			...
			hbp_byte = ((timing.value +
				     dsi_params->horizontal_sync_active) *
					    fbconfig_dsiTmpBufBpp -
				    10);
		} else {
			/*
			 * hsa_byte = (dsi_params->horizontal_sync_active *
			 * fbconfig_dsiTmpBufBpp - 10);
			 */
...
unsigned char DSCRead(uint8_t cmd)
{
	unsigned char buffer[2], bVal;
	unsigned int array[16];

	DSI_OUTREGBIT(NULL, struct DSI_MODE_CTRL_REG, DSI_REG[0]->DSI_MODE_CTRL,
		      MIX_MODE, 0);
	DSI_BackupRegisters(DISP_MODULE_DSI0, NULL);
	/* set to cmd mode */
	DSI_SetMode(DISP_MODULE_DSI0, NULL, CMD_MODE);

	bVal = 0;
	array[0] = 0x00013700; /* read id return two byte,version and id */
	buffer[0] = buffer[1] = 0xFF; /* Reset buffer */
	DSI_set_cmdq(DISP_MODULE_DSI0, NULL, array, 1, 1);
	DSI_dcs_read_lcm_reg_v2(DISP_MODULE_DSI0, NULL, cmd, buffer, 1);

	DSI_RestoreRegisters(DISP_MODULE_DSI0, NULL);
...
int DSI_set_roi(int x, int y)
{
	...
	DSI_Send_ROI(DISP_MODULE_DSI0, NULL, x, y, _dsi_context[0].lcm_width,
		     _dsi_context[0].lcm_height);

	data_array[0] = 0x00043700; /* read id return two byte,version and id */
	DSI_set_cmdq(DISP_MODULE_DSI0, NULL, data_array, 1, 1);
	int count = DSI_dcs_read_lcm_reg_v2(DISP_MODULE_DSI0, NULL, 0x2a,
					    read_buf, 4);
	...
	count = DSI_dcs_read_lcm_reg_v2(DISP_MODULE_DSI0, NULL, 0x2b, read_buf,
					4);
...
int DSI_test_roi(int x, int y)
{
	...
	if (ret == 0)
		...
		DSI_Send_ROI(DISP_MODULE_DSI0, NULL, 0, 0,
...
int DSI_test_force_te(enum DISP_MODULE_ENUM module)
{
	int ret = 0, r = 0;
	int i = 0;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		r = wait_event_interruptible_timeout(
			_dsi_wait_ext_te[i], !(DSI_REG[i]->DSI_INTSTA.TE_RDY),
			HZ / 50);
		if (r == 0)
			DISPERR("dsi wait not busy timeout\n");
	}
	 ...
	 * i++)
	 * {
	 * r= wait_event_interruptible_timeout(_dsi_wait_ext_te[i],
	 * !(DSI_REG[i]->DSI_INTSTA.EXT_TE), HZ/50);
	 * if (0 == r)
	 * {
	 ...
	 * i++)
	 * {   DSI_OUTREGBIT(NULL,
	 * DSI_RACK_REG,DSI_REG[i]->DSI_RACK,DSI_RACK,1);
	 * r =
...
unsigned int DSI_WaitForNotBusyTimeout(enum DISP_MODULE_ENUM module,
				       struct cmdqRecStruct *cmdq)
{
	int i = 0;
	unsigned int count = 0;
