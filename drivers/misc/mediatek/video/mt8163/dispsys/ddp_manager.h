...
/*
 * create disp path handle , it will assign mutex to this handle, and cache this
 * handle to modules in scenario, and will assign default
 * irq event to this handle.
...
/*
 * set dst module, the default dst module maybe not right, so set real dst
 * module on this path.
 * return 0.
 * dp_handle: disp path handle.
 ...
 *        DISP_MODULE_DPI0
 */
int dpmgr_path_set_dst_module(void *dp_handle,
...
/*
 * init path , it will set mutex according to modules on this path and sof
 * sorce.
...
/*
 * connect path , it will set mutex according to modules on this path and sof
 * sorce.
 * and it will connect path , then initialize modules on this path.
 * return 0.
 * dp_handle: disp path handle.
...
/*
 * power 0ff,  turn off each modules clk, if all hande are closed. top clock
 * will be off.
 * return 0.
 * dp_handle: disp path handle.
 * encmdq: 1 use command queue, 0 not.
...
/*
 * dpmgr_ioctl , it will call ioctl of ddp modules ioctl() to do some special
 * config or setting.
 * return 0.
 * dp_handle: disp path handle.
...
extern int dpmgr_path_get_handle(unsigned long *dp_handle,
				 unsigned long *cmdq_handle);
/*
