...
{
	unsigned int pic_group_width, slice_width, slice_hight, pic_height_ext_num,
	    slice_group_width;
	...
	if (lcm_config->dsc_enable == 1) {
		...
		DISP_REG_SET_FIELD(handle, CFG_FLD_PIC_GROUP_WIDTH_M1, DISP_REG_DSC_PIC_W,
				   (pic_group_width - 1));

		DISP_REG_SET_FIELD(handle, CFG_FLD_PIC_HEIGHT_M1, DISP_REG_DSC_PIC_H,
				   (pConfig->dst_h - 1));
		DISP_REG_SET_FIELD(handle, CFG_FLD_PIC_HEIGHT_EXT_M1, DISP_REG_DSC_PIC_H,
				   (pic_height_ext_num * slice_hight - 1));

		DISP_REG_SET_FIELD(handle, CFG_FLD_SLICE_WIDTH, DISP_REG_DSC_SLICE_W, slice_width);
		DISP_REG_SET_FIELD(handle, CFG_FLD_SLICE_GROUP_WIDTH_M1, DISP_REG_DSC_SLICE_W,
				   (slice_group_width - 1));
		DISP_REG_SET_FIELD(handle, CFG_FLD_SLICE_HEIGHT_M1, DISP_REG_DSC_SLICE_H,
				   (slice_hight - 1));
		DISP_REG_SET_FIELD(handle, CFG_FLD_SLICE_NUM_M1, DISP_REG_DSC_SLICE_H,
				   (pic_height_ext_num - 1));
		DISP_REG_SET_FIELD(handle, CFG_FLD_SLICE_WIDTH_MOD3, DISP_REG_DSC_SLICE_H,
				   (slice_width % 3));

		DISP_REG_SET_FIELD(handle, CFG_FLD_CHUNK_SIZE, DISP_REG_DSC_CHUNK_SIZE,
				   chrunk_size);
		DISP_REG_SET_FIELD(handle, CFG_FLD_BUF_SIZE, DISP_REG_DSC_BUF_SIZE,
				   chrunk_size * slice_hight);
		DISP_REG_SET_FIELD(handle, CFG_FLD_SLICE_MODE, DISP_REG_DSC_MODE,
		...
		DISP_REG_SET_FIELD(handle, CFG_FLD_INIT_DELAY_HEIGHT, DISP_REG_DSC_MODE, 4);

		DISP_REG_SET_FIELD(handle, CFG_FLD_CFG, DISP_REG_DSC_CFG,
				   (lcm_config->dsc_params.dsc_cfg ==
				    0) ? 0x22 : lcm_config->dsc_params.dsc_cfg);
...
void dsc_bypass(DISP_MODULE_ENUM module, void *cmdq)
{
	DISP_REG_SET_FIELD(cmdq, START_FLD_DISP_DSC_BYPASS, DISP_REG_DSC_CON, 1);
