	...
	if (DISP_MODULE_DSI0) {
		...
		for (i = 0; i < 20 * 16; i += 16) {
			pr_debug("DSI1+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n", i,
			       INREG32(0xF401c000 + i), INREG32(0xF401c000 + i + 0x4),
			       INREG32(0xF401c000 + i + 0x8), INREG32(0xF401c000 + i + 0xc));
		}
	} else {
		DDPDUMP("==DISP DSIDUAL REGS==\n");
		for (i = 0; i < 20 * 16; i += 16) {
			pr_debug("DSI0+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n", i,
			       INREG32(0xF401b000 + i), INREG32(0xF401b000 + i + 0x4),
			       INREG32(0xF401b000 + i + 0x8), INREG32(0xF401b000 + i + 0xc));
		}
		for (i = 0; i < 20 * 16; i += 16) {
			pr_debug("DSI1+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n", i,
			       INREG32(0xF401c000 + i), INREG32(0xF401c000 + i + 0x4),
