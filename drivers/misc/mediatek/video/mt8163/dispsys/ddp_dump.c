...
static void mutex_dump_analysis(void)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if (DISP_REG_GET(DISP_REG_CONFIG_MUTEX_MOD(i)) != 0 &&
		    ((DISP_REG_GET(DISP_REG_CONFIG_MUTEX_EN(i) + 0x20 * i) ==
			      1 &&
		      DISP_REG_GET(DISP_REG_CONFIG_MUTEX_SOF(i) + 0x20 * i) !=
			      SOF_SINGLE) ||
		     DISP_REG_GET(DISP_REG_CONFIG_MUTEX_SOF(i) + 0x20 * i) ==
			     SOF_SINGLE)) {
			len = sprintf(
				p, "MUTEX%d :mode=%s,module=(", i,
				ddp_get_fmt_name(
					DISP_MODULE_MUTEX,
					DISP_REG_GET(
			...
			for (j = 6; j <= 18; j++) {
				if ((DISP_REG_GET(
					     DISP_REG_CONFIG_MUTEX_MOD(i)) >>
				     j) &
				    0x1) {
					len = sprintf(
						p, "%s,",
						ddp_get_mutex_module_name(j));
					p += len;
				}
			}
			DDPDUMP("%s)\n", mutex_module);
...
static void mmsys_config_dump_reg(void)
{
	...
	DDPMSG("(0xc )DISP_REG_CONFIG_MJC_APB_TX_CON =0x%x\n",
	       DISP_REG_GET(DISP_REG_CONFIG_MJC_APB_TX_CON));
 ...
 * for the modules:
 * smi_common, larb0, mdp_crop, fake_eng,
 * mutex_32k, pwm0, pwm1, dsi0, dsi1, dpi.
 * Other bits could keep 1.
 * Suggest to keep smi_common and larb0 always clock on.
 * --------valid & ready
 * example:
 * ovl0 -> ovl0_mout_ready=1 means engines
 * after ovl_mout are ready for receiving
 * data
 * ovl0_mout_ready=0 means ovl0_mout can not receive data, maybe ovl0_mout_valid
 * or after engines config error
 * ovl0 -> ovl0_mout_valid=1 means engines before ovl0_mout is OK,
 * ovl0_mout_valid=0 means ovl can not transfer data to ovl0_mout, means
 * ovl0 or before engines are not ready.
 */
...
static void mmsys_config_dump_analysis(void)
{
	...
	pr_debug("larb0_force_on=%d, smi_common_force_on=%d\n",
		 clk_is_force_on(MT_CG_DISP0_SMI_LARB0),
		 clk_is_force_on(MT_CG_DISP0_SMI_COMMON));
#endif
	...
	if (((DISP_REG_GET(DISP_REG_CONFIG_C10) >> 7) & 0x7) == 5 &&
	    ((DISP_REG_GET(DISP_REG_OVL_ROI_SIZE + DISP_OVL_INDEX_OFFSET * 1) &
	      0xfff) > 1200 ||
	     ((DISP_REG_GET(DISP_REG_OVL_ROI_SIZE +
			    DISP_OVL_INDEX_OFFSET * 1) >>
	       16) &
	      0xfff) > 1920)) {
	...
	for (i = 0; i < 5; i++) {
		DDPDUMP(
			"valid0=0x%x, valid1=0x%x, ready0=0x%x, ready1=0x%x, greq=0%x\n",
...
static void gamma_dump_analysis(void)
{
	...
	DDPDUMP(
		"gamma: en=%d, w=%d, h=%d, in_p_cnt=%d, in_l_cnt=%d, out_p_cnt=%d, out_l_cnt=%d\n",
		DISP_REG_GET(DISP_REG_GAMMA_EN),
...
static void color_dump_analysis(enum DISP_MODULE_ENUM module)
{
	...
	DDPDUMP("color%d: bypass=%d, w=%d, h=%d, pixel_cnt=%d, line_cnt=%d,\n",
		index, (DISP_REG_GET(DISP_COLOR_CFG_MAIN) >> 7) & 0x1,
		DISP_REG_GET(DISP_COLOR_INTERNAL_IP_WIDTH),
		DISP_REG_GET(DISP_COLOR_INTERNAL_IP_HEIGHT),
...
#if 0
	DDPDUMP("(0x354)AAL_DRE_GAIN_FILTER_00 =0x%x\n",
...
static void aal_dump_analysis(void)
{
	...
	DDPDUMP(
		"aal: bypass=%d, relay=%d, en=%d, w=%d, h=%d, in(%d,%d),out(%d,%d)\n",
		DISP_REG_GET(DISP_AAL_EN) == 0x0,
		DISP_REG_GET(DISP_AAL_CFG) & 0x01, DISP_REG_GET(DISP_AAL_EN),
...
{
	DDPDUMP(
		"ccorr: en=%d, config=%d, w=%d, h=%d, in_p_cnt=%d, in_l_cnt=%d, out_p_cnt=%d, out_l_cnt=%d\n",
		DISP_REG_GET(DISP_REG_CCORR_EN),
		DISP_REG_GET(DISP_REG_CCORR_CFG),
...
static void dsi_dump_reg(enum DISP_MODULE_ENUM module)
{
	...
	if (DISP_MODULE_DSI0) {
		...
		for (i = 0; i < 20 * 16; i += 16) {
			pr_debug(
				"DSI1+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n",
				i,
				INREG32(0xF401c000 + i),
				INREG32(0xF401c000 + i + 0x4),
				INREG32(0xF401c000 + i + 0x8),
				INREG32(0xF401c000 + i + 0xc));
		}
	} else {
		DDPDUMP("==DISP DSIDUAL REGS==\n");
		for (i = 0; i < 20 * 16; i += 16) {
			pr_debug(
				"DSI0+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n",
				i,
				INREG32(0xF401b000 + i),
				INREG32(0xF401b000 + i + 0x4),
				INREG32(0xF401b000 + i + 0x8),
				INREG32(0xF401b000 + i + 0xc));
		}
		for (i = 0; i < 20 * 16; i += 16) {
			pr_debug(
				"DSI1+%04x : 0x%08x  0x%08x  0x%08x  0x%08x\n",
