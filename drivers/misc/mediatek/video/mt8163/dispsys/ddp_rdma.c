...
static unsigned int
rdma_input_format_byte_swap(enum RDMA_INPUT_FORMAT inputFormat)
{
	int input_swap = 0;
...
static unsigned int
rdma_input_format_color_space(enum RDMA_INPUT_FORMAT inputFormat)
{
	int space = 0;
...
static unsigned int
rdma_input_format_reg_value(enum RDMA_INPUT_FORMAT inputFormat)
{
	int reg_value = 0;
...
int rdma_start(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	DISP_REG_SET(handle,
		     ...
		     0x3E);
	DISP_REG_SET_FIELD(
		handle, GLOBAL_CON_FLD_ENGINE_EN,
...
int rdma_stop(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	DISP_REG_SET(handle,
		     ...
		     0);
	DISP_REG_SET(handle,
		     idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_INT_STATUS,
		     0);
	return 0;
}
...
int rdma_reset(enum DISP_MODULE_ENUM module, void *handle)
{
	...
	DISP_REG_SET_FIELD(
		handle, GLOBAL_CON_FLD_SOFT_RESET,
		idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_GLOBAL_CON, 1);
	while ((DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET +
		...
		if (delay_cnt > 10000) {
			ret = -1;
			DDPERR(
				"rdma%d_reset timeout, stage 1! DISP_REG_RDMA_GLOBAL_CON=0x%x\n",
			       idx, DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET +
						 DISP_REG_RDMA_GLOBAL_CON));
			break;
		}
	}
	...
	while ((DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET +
		...
		if (delay_cnt > 10000) {
			ret = -1;
			DDPERR(
				"rdma%d_reset timeout, stage 2! DISP_REG_RDMA_GLOBAL_CON=0x%x\n",
			       idx, DISP_REG_GET(idx * DISP_RDMA_INDEX_OFFSET +
						 DISP_REG_RDMA_GLOBAL_CON));
			break;
		}
...
void rdma_set_ultra(unsigned int idx, unsigned int width, unsigned int height,
		    unsigned int bpp, unsigned int frame_rate, void *handle)
{
	/* constant */
	static const unsigned int blank_overhead =
		115; /* it is 1.15, need to divide 100 later */
	...
	static const unsigned int fifo_valid_line_ratio =
		125; /* valid size 1/8 line; */
	...
	fifo_valid_size = (width * bpp * fifo_valid_line_ratio) /
			  (rdma_fifo_width * 1000);
	fifo_valid_size = fifo_valid_size > fifo_min_size ? fifo_valid_size
							  : fifo_min_size;

	/* change calculation order to prevent overflow of unsigned int */
	consume_levels_per_sec =
		(width * height * frame_rate * bpp / rdma_fifo_width / 100) *
		blank_overhead;

	/* /1000000 for ultra_low_time in unit of us */
	ultra_low_level = (unsigned int)(ultra_low_time *
					 consume_levels_per_sec / 1000000);
	pre_ultra_low_level = (unsigned int)(pre_ultra_low_time *
					     consume_levels_per_sec / 1000000);
	pre_ultra_high_level = (unsigned int)(pre_ultra_high_time *
					      consume_levels_per_sec / 1000000);
		...
		void *handle) /* ourput setting */
{
	...
	enum RDMA_INPUT_FORMAT inputFormat =
		rdma_input_format_convert(inFormat);
	...
	unsigned int input_format_reg =
		rdma_input_format_reg_value(inputFormat);
	unsigned int color_matrix =
		0x4; /* 0100 MTX_JPEG_TO_RGB (YUV FUll TO RGB) */
	unsigned int idx = rdma_index(module);
	...
	DDPMSG(
		"RDMAConfig idx %d, mode %d, address 0x%lx, inputformat %s, pitch %u, width %u, height %u,sec%d\n",
	       idx, mode, address,
	       rdma_intput_format_name(inputFormat, input_swap), pitch, width,
	       height, sec);
#else
	DDPDBG(
		"RDMAConfig idx %d, mode %d, address 0x%lx, inputformat %s, pitch %u, width %u, height %u,sec%d\n",
	...
	DISP_REG_SET_FIELD(
		handle, GLOBAL_CON_FLD_MODE_SEL,
		idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_GLOBAL_CON, mode);
	/*
	 * FORMAT & SWAP only works when RDMA memory mode, set both to 0 when
	 * RDMA direct link mode.
	 */
	DISP_REG_SET_FIELD(
		handle, MEM_CON_FLD_MEM_MODE_INPUT_FORMAT,
	...
	if (sec != DISP_SECURE_BUFFER) {
		 ...
		 */
		cmdqRecWriteSecure(
			handle, disp_addr_convert(idx * DISP_RDMA_INDEX_OFFSET +
						  DISP_REG_RDMA_MEM_START_ADDR),
			CMDQ_SAM_H_2_MVA, address, 0, size, m4u_port);
	}
	DISP_REG_SET(handle,
		     idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_MEM_SRC_PITCH,
		     pitch);
	/*
	 * DISP_REG_SET(handle,idx*DISP_RDMA_INDEX_OFFSET+
	 * DISP_REG_RDMA_INT_ENABLE, 0x3F);
	 */
	DISP_REG_SET_FIELD(
		handle, SIZE_CON_0_FLD_OUTPUT_FRAME_WIDTH,
	...
	if (ufoe_enable == 0) {
		/*
		 * UFOE bypassed, enable RDMA underflow intr, else disable RDMA
		 * underflow intr
		 */
		DISP_REG_SET_FIELD(handle, FIFO_CON_FLD_FIFO_UNDERFLOW_EN,
				   ...
				   1);
		DISP_REG_SET_FIELD(
			handle, FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD,
		...
		if (fifolen > 0x200)
			DISP_REG_SET_FIELD(
				handle,
				FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD,
				idx * DISP_RDMA_INDEX_OFFSET +
					DISP_REG_RDMA_FIFO_CON,
				0x200);
		else
...
static int rdma_clock_on(enum DISP_MODULE_ENUM module, void *handle)
{
	unsigned int idx = rdma_index(module);
#ifdef ENABLE_CLK_MGR
	if (idx == 0)
		ddp_module_clock_enable(MM_CLK_DISP_RDMA0, true);
	else
		ddp_module_clock_enable(MM_CLK_DISP_RDMA1, true);
#endif
...
static int rdma_clock_off(enum DISP_MODULE_ENUM module, void *handle)
{
	unsigned int idx = rdma_index(module);
#ifdef ENABLE_CLK_MGR
	if (idx == 0)
		ddp_module_clock_enable(MM_CLK_DISP_RDMA0, false);
	else
		ddp_module_clock_enable(MM_CLK_DISP_RDMA1, false);
#endif
...
static int rdma_init(enum DISP_MODULE_ENUM module, void *handle)
{
	rdma_clock_on(module, handle);
	return 0;
}

static int rdma_deinit(enum DISP_MODULE_ENUM module, void *handle)
{
...
void rdma_dump_analysis(enum DISP_MODULE_ENUM module)
{
	...
	DDPDUMP(
		"rdma%d: en=%d, memory mode=%d, w=%d, h=%d, pitch=%d, addr=0x%x, fmt=%s, fifo_min=%d, ",
		idx, DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON +
				  DISP_RDMA_INDEX_OFFSET * idx) &
			     0x1,
		(DISP_REG_GET(DISP_REG_RDMA_GLOBAL_CON +
			      DISP_RDMA_INDEX_OFFSET * idx) &
...
void rdma_get_info(int idx, struct RDMA_BASIC_STRUCT *info)
{
	...
	p->addr = DISP_REG_GET(DISP_REG_RDMA_MEM_START_ADDR +
			       DISP_RDMA_INDEX_OFFSET * idx);
	p->src_w = DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_0 +
				DISP_RDMA_INDEX_OFFSET * idx) &
		   0xfff;
	p->src_h = DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_1 +
				DISP_RDMA_INDEX_OFFSET * idx) &
		   0xfffff,
	p->bpp = rdma_input_format_bpp(
		(DISP_REG_GET(DISP_REG_RDMA_MEM_CON +
			      DISP_RDMA_INDEX_OFFSET * idx) >>
		 4) &
		0xf);
			    ...
			    struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	unsigned int width =
		pConfig->dst_dirty ? pConfig->dst_w : r_config->width;
	unsigned int height =
		pConfig->dst_dirty ? pConfig->dst_h : r_config->height;
	...
	rdma_config(
		module, mode, (mode == RDMA_MODE_DIRECT_LINK)
				      ? 0
				      : r_config->address, /* address */
		(mode == RDMA_MODE_DIRECT_LINK)
			? eRGB888
			: r_config->inputFormat, /* inputFormat */
		(mode == RDMA_MODE_DIRECT_LINK) ? 0
						: r_config->pitch, /* pitch */
		width,
		height, lcm_param->dsi.ufoe_enable, r_config->security, handle);

	return 0;
}

static void rdma_cmdq_insert_wait_frame_done_token_mira(void *handle,
							int rdma_idx)
{
	if (rdma_idx == 0) {
			  ...
			  struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	if (mode == RDMA_MODE_MEMORY) {
		if (security == DISP_SECURE_BUFFER) {
			...
			/* set engine as sec */
			cmdqRecSecureEnablePortSecurity(handle,
							(1LL << cmdq_engine));
			/*
			 * cmdqRecSecureEnableDAPC(handle,
			 * (1LL << cmdq_engine));
			 */
			...
			if (rdma_is_sec[rdma_idx]) {
				/*
				 * rdma is in sec stat, we need to switch it to
				...
				if (ret)
					DDPAEE(
						"[SVP]fail to create disable handle %s\n",
					       __func__);
				 ...
				 * nonsec_switch_handle);
				 */
				rdma_cmdq_insert_wait_frame_done_token_mira(
					nonsec_switch_handle, rdma_idx);
				cmdqRecSetSecure(nonsec_switch_handle, 1);

				/*
				 * ugly work around by kzhang !!. will remove
				 * when cmdq delete disable scenario.
				 * To avoid translation fault like ovl (see
				 * notes in ovl.c)
				 */
				do_rdma_config_l(module, pConfig,
						 nonsec_switch_handle);

				/*
				 * in fact, dapc/port_sec will be disabled by
				 * cmdq
				 */
...
int rdma_resolution_test(enum DISP_MODULE_ENUM module, unsigned int fw,
			 unsigned int fh)
{
	int ret = 0;
	unsigned int idx = rdma_index(module);

	int pw = DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_0 +
			      DISP_RDMA_INDEX_OFFSET * idx) &
		 0xfff;
	int ph = DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_1 +
			      DISP_RDMA_INDEX_OFFSET * idx) &
		 0xfffff;
	...
	DISP_REG_SET_FIELD(
		NULL, SIZE_CON_0_FLD_OUTPUT_FRAME_WIDTH,
		idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_SIZE_CON_0, pw);
	DISP_REG_SET_FIELD(
		NULL, SIZE_CON_1_FLD_OUTPUT_FRAME_HEIGHT,
		idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_SIZE_CON_1, ph);
	DISP_REG_SET_FIELD(
		NULL, SIZE_CON_0_FLD_OUTPUT_FRAME_WIDTH,
		idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_SIZE_CON_0, fw);
	DISP_REG_SET_FIELD(
		NULL, SIZE_CON_1_FLD_OUTPUT_FRAME_HEIGHT,
		idx * DISP_RDMA_INDEX_OFFSET + DISP_REG_RDMA_SIZE_CON_1, fh);

	int w = DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_0 +
			     DISP_RDMA_INDEX_OFFSET * idx) &
		0xfff;
	int h = DISP_REG_GET(DISP_REG_RDMA_SIZE_CON_1 +
			     DISP_RDMA_INDEX_OFFSET * idx) &
		0xfffff;

	if ((w != pw) && (h != ph)) {
		pr_debug("w=%d,h=%d,pw=%d,ph=%d,fw=%d,fh=%d\n", w, h, pw, ph,
			 fw, fh);
		DISP_REG_SET_FIELD(NULL, SIZE_CON_0_FLD_OUTPUT_FRAME_WIDTH,
				   idx * DISP_RDMA_INDEX_OFFSET +
					   DISP_REG_RDMA_SIZE_CON_0,
				   pw);
		DISP_REG_SET_FIELD(NULL, SIZE_CON_1_FLD_OUTPUT_FRAME_HEIGHT,
				   idx * DISP_RDMA_INDEX_OFFSET +
					   DISP_REG_RDMA_SIZE_CON_1,
				   ph);
		ret = -1;
