...
int rdma_config(DISP_MODULE_ENUM module, enum RDMA_MODE mode,
	unsigned long address, DpColorFormat inFormat, unsigned pitch,
	unsigned width, unsigned height, unsigned ufoe_enable,
	DISP_BUFFER_TYPE sec, void *handle)	/* ourput setting */
{
...
#if 1
	if (ufoe_enable == 0) {
		/* UFOE bypassed, enable RDMA underflow intr, else disable RDMA underflow intr */
	...
	if (mode == RDMA_MODE_MEMORY) {
		if (security == DISP_SECURE_BUFFER) {
			...
			if (rdma_is_sec[rdma_idx]) {
				...
				rdma_cmdq_insert_wait_frame_done_token_mira(nonsec_switch_handle, rdma_idx);
