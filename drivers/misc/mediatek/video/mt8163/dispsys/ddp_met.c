	...
	for (i = 0; i < layerCnt; i++) {
		...
		if (ovlInfo[i].layer_en) {
			...
			if (sLayerBufAddr[index][idx] != ovlInfo[i].addr) {
				...
				met_tag_oneshot(DDP_IRQ_LAYER_FPS_ID, tag_name,
						i + 1);
			}
			if (sLayerBufFmt[index][idx] != ovlInfo[i].fmt) {
				sLayerBufFmt[index][idx] = ovlInfo[i].fmt;
				sprintf(tag_name, "OVL%dL%d_Fmt_%s", index, idx,
					parse_color_format(ovlInfo[i].fmt));
				met_tag_oneshot(DDP_IRQ_LAYER_FORMAT_ID,
						tag_name, i + 1);
			}
