...
static unsigned int wdma_index(enum DISP_MODULE_ENUM module)
{
	int idx = 0;
	...
	case DISP_MODULE_WDMA1:
		...
		break;
	default:
		pr_err("[DDP] error: invalid wdma module=%d\n",
...
static int wdma_start(enum DISP_MODULE_ENUM module, void *handle)
{
...
static int wdma_stop(enum DISP_MODULE_ENUM module, void *handle)
{
	unsigned int idx = wdma_index(module);
...
static int wdma_reset(enum DISP_MODULE_ENUM module, void *handle)
{
	unsigned int delay_cnt = 0;
	unsigned int idx = wdma_index(module);

	DISP_REG_SET(handle, idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_RST,
		     0x01); /* trigger soft reset */
	if (!handle) {
	...
	DISP_REG_SET(handle, idx * DISP_WDMA_INDEX_OFFSET + DISP_REG_WDMA_RST,
		     0x0); /* trigger soft reset */

	return 0;
}

#if 0
static int wdma_config_uv(enum DISP_MODULE_ENUM module,
...
static int wdma_config_yuv420(enum DISP_MODULE_ENUM module,
			      enum DP_COLOR_ENUM fmt, unsigned int dstPitch,
...
/* -------------------------------------------------------- */
void wdma_calc_ultra(unsigned int idx, unsigned int width, unsigned int height,
		     unsigned int bpp, unsigned int frame_rate, void *handle)
{
	/* constant */
	unsigned int blank_overhead =
	...
	/*
	 * consume_levels_per_sec = ((long long unsigned
	 * int)width*height*frame_rate*blank_overhead*bpp)/rdma_fifo_width/100;
	 * change calculation order to prevent overflow of unsigned int
	 * bpp/2 for bpp in unit of 0.5 bytes/pixel
	 */
	consume_levels_per_sec = (width * height * frame_rate * bpp / 2 /
				  rdma_fifo_width / 100) *
				 blank_overhead;

	/* /1000000 /2 for ultra_low_time in unit of 0.5 us */
	ultra_low_level = (unsigned int)(ultra_low_time *
					 consume_levels_per_sec / 1000000 / 2);
	pre_ultra_low_level =
		(unsigned int)(pre_ultra_low_time * consume_levels_per_sec /
			       1000000 / 2);
	ultra_high_level = (unsigned int)(ultra_high_time *
					  consume_levels_per_sec / 1000000 / 2);
...
static int wdma_config(enum DISP_MODULE_ENUM module, unsigned int srcWidth,
		       unsigned int srcHeight, unsigned int clipX,
		       unsigned int clipY, unsigned int clipWidth,
		       unsigned int clipHeight, enum DP_COLOR_ENUM out_format,
...
/* DDPMSG("wmda%d_clock_on\n",idx); */
	...
	if (idx == 0)
		ddp_module_clock_enable(MM_CLK_DISP_WDMA0, true);
	else
		ddp_module_clock_enable(MM_CLK_DISP_WDMA1, true);
#endif
	return 0;
}
...
#ifdef ENABLE_CLK_MGR
	if (idx == 0)
		ddp_module_clock_enable(MM_CLK_DISP_WDMA0, false);
	else
		ddp_module_clock_enable(MM_CLK_DISP_WDMA1, false);
#endif
	return 0;
}
	...
	DDPDUMP(
		...
		fmt_string(fmt_type(
			(DISP_REG_GET(DISP_REG_WDMA_CFG + idx_offst) >> 4) &
				0xf,
			(DISP_REG_GET(DISP_REG_WDMA_CFG + idx_offst) >> 11) &
				0x1)));
	DDPDUMP(
		"wdma%d:status=%s,in_req=%d,in_ack=%d,exec=%d,input_pixel=(L:%d,P%d)\n",
		index,
...
static int wdma_check_input_param(struct WDMA_CONFIG_STRUCT *config)
{
	...
	if (unique > WDMA_OUTPUT_FORMAT_YV12 &&
	    unique != WDMA_OUTPUT_FORMAT_NV21) {
...
static void wdma_cmdq_insert_wait_frame_done_token_mira(void *handle,
							int wdma_idx)
{
...
static int wdma_get_sec_mode(int wdma_idx)
{
			 ...
			 struct disp_ddp_path_config *pConfig, void *handle)
{
	...
	enum CMDQ_DISP_MODE mode = wdma_get_sec_mode(wdma_idx);
	...
	if (handle) {
		if (
			...
			unsigned int idx_offst =
				wdma_idx * DISP_WDMA_INDEX_OFFSET;

			DISP_REG_SET(handle, idx_offst + DISP_REG_WDMA_RST,
				     0x01); /* trigger soft reset */
			cmdqRecPoll(
				handle,
				disp_addr_convert(idx_offst +
	...
	if (config->security == DISP_SECURE_BUFFER) {
		...
		if (wdma_is_sec[wdma_idx]) {
			 ...
			 * nonsec_switch_handle);
			 */
			wdma_cmdq_insert_wait_frame_done_token_mira(
				nonsec_switch_handle, wdma_idx);
			cmdqRecSetSecure(nonsec_switch_handle, 1);
			cmdqRecSetSecureMode(handle, mode);
			/*in fact, dapc/port_sec will be disabled by cmdq */
			wdma_config(module, config->srcWidth, config->srcHeight,
				    config->clipX, config->clipY,
				    config->clipWidth, config->clipHeight,
				    config->outputFormat, config->dstAddress,
				    config->dstPitch, config->useSpecifiedAlpha,
				    config->alpha, config->security,
				    nonsec_switch_handle);
...
int wdma_build_cmdq(enum DISP_MODULE_ENUM module, void *cmdq_trigger_handle,
