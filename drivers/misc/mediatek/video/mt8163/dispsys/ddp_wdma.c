...
static void wdma_cmdq_insert_wait_frame_done_token_mira(void *handle, int wdma_idx)
{
...
static int wdma_config_l(DISP_MODULE_ENUM module, disp_ddp_path_config *pConfig, void *handle)
{
	...
	if (config->security == DISP_SECURE_BUFFER) {
		...
		if (wdma_is_sec[wdma_idx]) {
			...
			wdma_cmdq_insert_wait_frame_done_token_mira(nonsec_switch_handle, wdma_idx);
