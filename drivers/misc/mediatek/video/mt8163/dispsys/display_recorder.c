...
static struct reg_base_map reg_map[] = {
	{"MMSYS", (0xf4000000)},     {"OVL0", (0xF400c000)},
	{"OVL1", (0xF400d000)},      {"RDMA0", (0xF400e000)},
	{"RDMA1", (0xF400f000)},     {"RDMA2", (0xF4010000)},
	{"WDMA0", (0xF4011000)},     {"WDMA1", (0xF4012000)},
	{"COLOR0", (0xF4013000)},    {"COLOR1", (0xF4014000)},
	{"AAL", (0xF4015000)},       {"GAMMA", (0xF4016000)},
	{"MERGE", (0xF4017000)},     {"SPLIT0", (0xF4018000)},
	{"SPLIT1", (0xF4019000)},    {"UFOE", (0xF401a000)},
	{"DSI0", (0xF401b000)},      {"DSI1", (0xF401c000)},
	{"DPI", (0xF401d000)},       {"PWM0", (0xF401e000)},
	{"PWM1", (0xF401f000)},      {"MUTEX", (0xF4020000)},
	{"SMI_LARB0", (0xF4021000)}, {"SMI_COMMON", (0xF4022000)},
	{"OD", (0xF4023000)},	{"MIPITX0", (0xF0215000)},
	{"MIPITX1", (0xF0216000)},
};
...
void dprec_to_mmp(unsigned int type_logsrc, enum mmp_log_type mmp_log,
...
const char *_get_event_string(enum DPREC_EVENT event)
{
	int i = 0;
...
static struct dprec_logger logger[DPREC_LOGGER_NUM] = {{0} };
...
/*
 * #define SPLIT_NS(x) (unsigned long long)nsec_high(x), (unsigned
 * long)nsec_low(x)
 * #define SPLIT_MS(x) (unsigned long long)msec_high(x), (unsigned
 * long)msec_low(x)
 */

const char *dprec_logger_spy(enum DPREC_LOGGER_ENUM l)
{
	switch (l) {
	case DPREC_LOGGER_PRIMARY_TRIGGER:
			  ...
			  unsigned int val2)
{
	...
	if (source == DPREC_LOGGER_DSI_EXT_TE) {
		/*
		 * DISPMSG("count=%d, time=%llu, period=%llu,
		 * max=%llu,min=%llu,
		 * start=%llu,trigger=%llu\n", l->count, time, l->period_frame,
		 * l->period_max_frame, l->period_min_frame, l->ts_start,
		 * l->ts_trigger);
		 * DISPMSG("count=%d,period=%lld.%02lums,max=%lld.%02lums\n",
		 * l->count, SPLIT_NS(l->period_frame),
		 * SPLIT_NS(l->period_max_frame));
		 */
	}
		       ...
		       unsigned int val2)
{
	...
	if (source == DPREC_LOGGER_RDMA0_TRANSFER) {
		/*
		 * DISPMSG("done,
		 * count=%llu,period=%llu.%02lums,max=%llu.%02lums\n",
		 * l->count, SPLIT_NS(l->period_frame),
		 * SPLIT_NS(l->period_max_frame));
		 */
	}
	...
	if (p) {
		...
		if (mmp_root)
			p->mmp = mmprofile_register_event(
				(unsigned int)(unsigned long)mmp_root, name);
		else
			p->mmp = mmprofile_register_event(
				  ...
				  unsigned int frm_sequence)
{
	...
	if (dprec_met_info[device_type].begin_frm_seq != frm_sequence) {
		...
		preempt_disable();
		event_trace_printk(
			tracing_mark_write_addr, "S|%d|%s|%d\n", current->tgid,
				...
				unsigned int frm_sequence)
{
	...
	if (dprec_met_info[device_type].end_frm_seq != frm_sequence) {
		...
		preempt_disable();
		event_trace_printk(
			tracing_mark_write_addr, "F|%d|%s|%d\n", current->tgid,
			dprec_met_info[device_type].log_name, frm_sequence);
		preempt_enable();
		 ...
		 unsigned int val2)
{
	if (event) {
		if (event->level & DPREC_LOGGER_LEVEL_MMP)
			mmprofile_log_ex(event->mmp, MMPROFILE_FLAG_START, val1,
					 val2);
		...
		if (event->level & DPREC_LOGGER_LEVEL_MOBILE_LOG) {
			/* pr_debug("DPREC/ "LOG_TAG "DISP/%s start,*/
		...
		unsigned int val2)
{
	if (event) {
		if (event->level & DPREC_LOGGER_LEVEL_MMP)
			mmprofile_log_ex(event->mmp, MMPROFILE_FLAG_END, val1,
					 val2);
		...
		if (event->level & DPREC_LOGGER_LEVEL_MOBILE_LOG) {
			/* pr_debug("DPREC"LOG_TAG "DISP/"LOG_TAG,"*/
			/*done,0x%08x,0x%08x\n",event->name, val1, val2); */
		}
		if (event->level & DPREC_LOGGER_LEVEL_UART_LOG)
		   ...
		   unsigned int val2)
{
	if (event) {
		if (event->level & DPREC_LOGGER_LEVEL_MMP)
			mmprofile_log_ex(event->mmp, MMPROFILE_FLAG_PULSE, val1,
					 val2);
		...
		if (event->level & DPREC_LOGGER_LEVEL_MOBILE_LOG) {
			/* pr_debug("DPREC/"LOG_TAG "%s trigger,0x%08x,*/
			/*0x%08x\n",event->name, val1, val2); */
		}
		if (event->level & DPREC_LOGGER_LEVEL_UART_LOG)
...
unsigned long long ts_dprec_reset;
...
int dprec_logger_get_result_value(enum DPREC_LOGGER_ENUM source,
				  ...
				  unsigned long long *pavg,
				  unsigned long long *pfps,
...
void dprec_stub_event(enum DISP_PATH_EVENT event)
{
	...
	if (event == DISP_PATH_EVENT_IF_VSYNC) {
		vsync_cnt++;
		/*
		 * mmprofile_log_ex(ddp_mmp_get_events()->vsync_count,
		 * MMPROFILE_FLAG_PULSE, vsync_cnt, 0);
		 */
		dprec_start(&dprec_vsync_irq_event, 0, 0);
		dprec_done(&dprec_vsync_irq_event, 0, 0);
		/* ATRACE_COUNTER("Vsyncirq", vsync_cnt); kernel 4.9 */
		/* ACOS_MOD_ONELINE */
	}
}
...
int dprec_state_machine_op(enum DPREC_STM_EVENT op)
{
	return 0;
}

int dprec_mmp_dump_ovl_layer(
	struct OVL_CONFIG_STRUCT *ovl_layer, unsigned int l,
...
unsigned int dprec_logger_error_id;
unsigned int dprec_logger_error_buflen[DPREC_LOGGER_BUFFER_COUNT] = {16 * 1024,
								     16 * 1024};

char dprec_logger_error_buffer[DPREC_LOGGER_BUFFER_COUNT][16 * 1024];

static DEFINE_SPINLOCK(dprec_logger_fence_spinlock);
unsigned int dprec_logger_fence_id;
unsigned int dprec_logger_fence_buflen[DPREC_LOGGER_BUFFER_COUNT] = {16 * 1024,
								     16 * 1024};

char dprec_logger_fence_buffer[DPREC_LOGGER_BUFFER_COUNT][16 * 1024];

static DEFINE_SPINLOCK(dprec_logger_hwop_spinlock);
unsigned int dprec_logger_hwop_id;
unsigned int dprec_logger_hwop_buflen[DPREC_LOGGER_BUFFER_COUNT] = {16 * 1024,
								    16 * 1024};

char dprec_logger_hwop_buffer[DPREC_LOGGER_BUFFER_COUNT][16 * 1024];

char *dprec_logger_find_buf(enum DPREC_LOGGER_PR_TYPE type, unsigned int *len)
{
	char *p = NULL;
	...
	if (type == DPREC_LOGGER_ERROR) {
		...
		if (dprec_logger_error_buflen[dprec_logger_error_id] < 64) {
			...
			dprec_logger_error_id = dprec_logger_error_id %
						DPREC_LOGGER_BUFFER_COUNT;
		}
		...
		if (dprec_logger_fence_buflen[dprec_logger_fence_id] < 64) {
			...
			dprec_logger_fence_id = dprec_logger_fence_id %
						DPREC_LOGGER_BUFFER_COUNT;
			p = dprec_logger_fence_buffer[dprec_logger_fence_id];
			dprec_logger_fence_buflen[dprec_logger_fence_id] =
				16 * 1024;
		}
		...
		if (dprec_logger_hwop_buflen[dprec_logger_hwop_id] < 64) {
			...
			dprec_logger_hwop_id = dprec_logger_hwop_id %
					       DPREC_LOGGER_BUFFER_COUNT;
		}
...
char dprec_logger_buffer[DPREC_LOGGER_PR_NUM][DPREC_LOGGER_BUFFER_COUNT]
			[16 * 1024];
...
int dprec_logger_pr(unsigned int type, char *fmt, ...)
{
	...
	if (dprec_logger_len[type][dprec_logger_id[type]] < 64) {
		...
		dprec_logger_id[type] =
			dprec_logger_id[type] % DPREC_LOGGER_BUFFER_COUNT;
...
char *_logger_pr_type_spy(enum DPREC_LOGGER_PR_TYPE type)
{
	switch (type) {
