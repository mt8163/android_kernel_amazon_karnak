...
static dprec_logger logger[DPREC_LOGGER_NUM] = { {0} };
...
void dprec_logger_trigger(unsigned int type_logsrc, unsigned int val1, unsigned int val2)
{
	...
	if (source == DPREC_LOGGER_DSI_EXT_TE) {
		/* DISPMSG("count=%d, time=%llu, period=%llu, max=%llu,min=%llu,*/
		/*start=%llu,trigger=%llu\n", l->count, time, l->period_frame,*/
		/* l->period_max_frame, l->period_min_frame, l->ts_start, l->ts_trigger); */
			     ...
			     MMP_Event *mmp_root)
{
	if (p) {
		...
		if (mmp_root)
			p->mmp = MMProfileRegisterEvent((unsigned int)(unsigned long)mmp_root, name);
		else
...
void dprec_logger_frame_seq_begin(unsigned int session_id, unsigned frm_sequence)
{
	...
	if (dprec_met_info[device_type].begin_frm_seq != frm_sequence) {
		...
		event_trace_printk(tracing_mark_write_addr, "S|%d|%s|%d\n",
				   current->tgid, dprec_met_info[device_type].log_name,
				   frm_sequence);
		preempt_enable();
...
void dprec_stub_event(DISP_PATH_EVENT event)
{
	...
	if (event == DISP_PATH_EVENT_IF_VSYNC) {
		...
		ATRACE_COUNTER("Vsyncirq", vsync_cnt); /* ACOS_MOD_ONELINE */
	}
}
