	...
	{                                                                      \
		if (dpi_reg_op_debug)                                          \
			pr_debug("[dsi/reg]0x%08x=0x%08x, cmdq:0x%08x\n",      \
				 addr, val, cmdq);                             \
		if (cmdq)                                                      \
			cmdqRecWrite(cmdq, (unsigned int)(addr)&0x1fffffff,    \
				     val, ~0);                                 \
		else                                                           \
 ...
 * AS_UINT32(REG_ADDR(DPI_REG[DPI_IDX(module)],
 * BACKUP_DPI_REG_OFFSETS[i])));
 * }
 * }
 *
 ...
 * for (i = 0; i < ARY_SIZE(BACKUP_DPI_REG_OFFSETS); ++i)
 * {
 * DPI_OUTREG32(0, REG_ADDR(DPI_REG[DPI_IDX(module)],
 * BACKUP_DPI_REG_OFFSETS[i]),
...
enum DPI_STATUS ddp_dpi_ConfigPclk(enum DISP_MODULE_ENUM module,
 ...
 */
	...
	DPI_OUTREG32(cmdq, DDP_REG_TVDPLL_CON1, pcw | (1 << 31));
	/* TVDPLL enable */
	DPI_OUTREG32(cmdq, DDP_REG_TVDPLL_CON0, 0x121);
#endif
#ifdef CONFIG_MTK_CLKMGR
	/*CLK_CFG_7  bit0~2 */
	clkmux_sel(MT_MUX_DPI1, ck_div, "dpi1");
#endif
	/*IO driving setting */
	/*
	 * MASKREG32(DISPSYS_IO_DRIVING, 0x3C00, 0x0);
	 * 0x1400 for 8mA, 0x0 for 4mA
	 */
#else
	if (ck_div == 2)
	...
	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->OUTPUT_SETTING,
		     AS_UINT32(&ctrl));

	clkcon.DPI_CKOUT_DIV = dpickoutdiv;
	clkcon.DPI_CK_POL = dpickpol;
	clkcon.DPI_CK_DUT = dpickdut;
	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->DPI_CLKCON,
		     AS_UINT32(&clkcon));
...
enum DPI_STATUS ddp_dpi_ConfigCLK(enum DISP_MODULE_ENUM module,
				  struct cmdqRecStruct *cmdq,
				  enum DPI_POLARITY polarity, bool LVDSEN)
{
	struct DPI_REG_OUTPUT_SETTING pol =
		DPI_REG[DPI_IDX(module)]->OUTPUT_SETTING;
	...
	DPI_OUTREGBIT(cmdq, struct DPI_REG_DDR_SETTING,
		      DPI_REG[DPI_IDX(module)]->DDR_SETTING, DDR_4PHASE, 0);
	DPI_OUTREGBIT(cmdq, struct DPI_REG_CLKCNTL,
		      DPI_REG[DPI_IDX(module)]->DPI_CLKCON, DPI_CK_DIV, 0);
	if (LVDSEN || (module == DISP_MODULE_DPI1)) {
		DPI_OUTREGBIT(cmdq, struct DPI_REG_CLKCNTL,
			      DPI_REG[DPI_IDX(module)]->DPI_CLKCON,
			      DPI_CKOUT_DIV, 1);
...
enum DPI_STATUS ddp_dpi_ConfigDE(enum DISP_MODULE_ENUM module,
				 struct cmdqRecStruct *cmdq,
				 enum DPI_POLARITY polarity)
{
	struct DPI_REG_OUTPUT_SETTING pol =
...
enum DPI_STATUS ddp_dpi_ConfigVsync(enum DISP_MODULE_ENUM module,
				    struct cmdqRecStruct *cmdq,
				    enum DPI_POLARITY polarity,
				    ...
				    uint32_t frontPorch)
{
	struct DPI_REG_TGEN_VWIDTH_LODD vwidth_lodd =
		DPI_REG[DPI_IDX(module)]->TGEN_VWIDTH_LODD;
	struct DPI_REG_TGEN_VPORCH_LODD vporch_lodd =
		DPI_REG[DPI_IDX(module)]->TGEN_VPORCH_LODD;
	struct DPI_REG_OUTPUT_SETTING pol =
		DPI_REG[DPI_IDX(module)]->OUTPUT_SETTING;
	...
	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->OUTPUT_SETTING,
		     AS_UINT32(&pol));
	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->TGEN_VWIDTH_LODD,
		     AS_UINT32(&vwidth_lodd));
	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->TGEN_VPORCH_LODD,
...
enum DPI_STATUS ddp_dpi_ConfigHsync(enum DISP_MODULE_ENUM module,
				    struct cmdqRecStruct *cmdq,
				    enum DPI_POLARITY polarity,
				    uint32_t pulseWidth, uint32_t backPorch,
				    uint32_t frontPorch)
{
	struct DPI_REG_TGEN_HPORCH hporch =
		DPI_REG[DPI_IDX(module)]->TGEN_HPORCH;
	struct DPI_REG_OUTPUT_SETTING pol =
		DPI_REG[DPI_IDX(module)]->OUTPUT_SETTING;
	...
	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->TGEN_HPORCH,
		     AS_UINT32(&hporch));
	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->OUTPUT_SETTING,
		     AS_UINT32(&pol));
...
enum DPI_STATUS ddp_dpi_ConfigDualEdge(enum DISP_MODULE_ENUM module,
				       struct cmdqRecStruct *cmdq, bool enable,
				       uint32_t mode)
{
	DPI_OUTREGBIT(cmdq, struct DPI_REG_OUTPUT_SETTING,
		      DPI_REG[DPI_IDX(module)]->OUTPUT_SETTING, DUAL_EDGE_SEL,
		      enable);
...
enum DPI_STATUS ddp_dpi_ConfigBG(enum DISP_MODULE_ENUM module,
				 struct cmdqRecStruct *cmdq, bool enable,
				 int BG_W, int BG_H)
{
	if (enable == false) {
		DPI_OUTREGBIT(cmdq, struct DPI_REG_CNTL,
			      DPI_REG[DPI_IDX(module)]->CNTL, BG_EN, 0);
	} else if (BG_W || BG_H) {
		DPI_OUTREGBIT(cmdq, struct DPI_REG_CNTL,
			      DPI_REG[DPI_IDX(module)]->CNTL, BG_EN, 1);
		DPI_OUTREGBIT(cmdq, struct DPI_REG_BG_HCNTL,
			      DPI_REG[DPI_IDX(module)]->BG_HCNTL, BG_RIGHT,
			      BG_W / 4);
		DPI_OUTREGBIT(cmdq, struct DPI_REG_BG_HCNTL,
			      DPI_REG[DPI_IDX(module)]->BG_HCNTL, BG_LEFT,
			      BG_W - BG_W / 4);
		DPI_OUTREGBIT(cmdq, struct DPI_REG_BG_VCNTL,
			      DPI_REG[DPI_IDX(module)]->BG_VCNTL, BG_BOT,
			      BG_H / 4);
		DPI_OUTREGBIT(cmdq, struct DPI_REG_BG_VCNTL,
			      DPI_REG[DPI_IDX(module)]->BG_VCNTL, BG_TOP,
			      BG_H - BG_H / 4);
		DPI_OUTREGBIT(cmdq, struct DPI_REG_CNTL,
			      DPI_REG[DPI_IDX(module)]->CNTL, BG_EN, 1);
...
enum DPI_STATUS ddp_dpi_ConfigSize(enum DISP_MODULE_ENUM module,
				   struct cmdqRecStruct *cmdq, uint32_t width,
				   uint32_t height)
{
...
enum DPI_STATUS ddp_dpi_yuv422_setting(enum DISP_MODULE_ENUM module,
				       struct cmdqRecStruct *cmdq,
				       uint32_t uvsw)
{
	struct DPI_REG_YUV422_SETTING uvset =
		DPI_REG[DPI_IDX(module)]->YUV422_SETTING;

	uvset.UV_SWAP = uvsw;
	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->YUV422_SETTING,
		     AS_UINT32(&uvset));

	return DPI_STATUS_OK;
}

enum DPI_STATUS ddp_dpi_CLPFSetting(enum DISP_MODULE_ENUM module,
				    struct cmdqRecStruct *cmdq,
				    uint8_t clpfType,
				    bool roundingEnable)
{
	struct DPI_REG_CLPF_SETTING setting =
		DPI_REG[DPI_IDX(module)]->CLPF_SETTING;

	setting.CLPF_TYPE = clpfType;
	setting.ROUND_EN = roundingEnable ? 1 : 0;
	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->CLPF_SETTING,
		     AS_UINT32(&setting));

	return DPI_STATUS_OK;
}

enum DPI_STATUS ddp_dpi_ConfigHDMI(enum DISP_MODULE_ENUM module,
				   struct cmdqRecStruct *cmdq,
				   uint32_t yuv422en,
				   uint32_t rgb2yuven, uint32_t ydfpen,
				   uint32_t r601sel, uint32_t clpfen)
{
 ...
 */
enum DPI_STATUS ddp_dpi_ConfigVsync_LEVEN(enum DISP_MODULE_ENUM module,
					  struct cmdqRecStruct *cmdq,
					  uint32_t pulseWidth,
					  uint32_t backPorch,
					  uint32_t frontPorch, bool fgInterlace)
{
	struct DPI_REG_TGEN_VWIDTH_LEVEN vwidth_leven =
		DPI_REG[DPI_IDX(module)]->TGEN_VWIDTH_LEVEN;
	struct DPI_REG_TGEN_VPORCH_LEVEN vporch_leven =
		DPI_REG[DPI_IDX(module)]->TGEN_VPORCH_LEVEN;
	struct DPI_REG_CNTL VS = DPI_REG[DPI_IDX(module)]->CNTL;
	...
	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->TGEN_VWIDTH_LEVEN,
		     AS_UINT32(&vwidth_leven));
	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->TGEN_VPORCH_LEVEN,
...
enum DPI_STATUS ddp_dpi_ConfigVsync_REVEN(enum DISP_MODULE_ENUM module,
					  struct cmdqRecStruct *cmdq,
					  uint32_t pulseWidth,
					  uint32_t backPorch,
					  uint32_t frontPorch, bool fgInterlace)
{
	struct DPI_REG_TGEN_VWIDTH_REVEN vwidth_reven =
		DPI_REG[DPI_IDX(module)]->TGEN_VWIDTH_REVEN;
	struct DPI_REG_TGEN_VPORCH_REVEN vporch_reven =
		DPI_REG[DPI_IDX(module)]->TGEN_VPORCH_REVEN;
	...
	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->TGEN_VWIDTH_REVEN,
		     AS_UINT32(&vwidth_reven));
	DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->TGEN_VPORCH_REVEN,
...
int LVDSTX_IsEnabled(enum DISP_MODULE_ENUM module, struct cmdqRecStruct *cmdq)
{
	...
	if (LVDS_TX_REG != NULL) {
		if ((LVDS_TX_REG->LVDS_OUT_CTRL.LVDS_EN) &&
...
int ddp_dpi_reset(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	DPI_OUTREGBIT(cmdq_handle, struct DPI_REG_RST,
		      DPI_REG[DPI_IDX(module)]->DPI_RST, RST, 1);
	DPI_OUTREGBIT(cmdq_handle, struct DPI_REG_RST,
		      DPI_REG[DPI_IDX(module)]->DPI_RST, RST, 0);
...
int ddp_dpi_trigger(enum DISP_MODULE_ENUM module, void *cmdq)
{
	if (s_isDpiStart == false) {
		...
		/*enable DPI */
		DPI_OUTREG32(cmdq, &DPI_REG[DPI_IDX(module)]->DPI_EN,
			     0x00000001);
...
int ddp_dpi_stop(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	DPI_OUTREG32(cmdq_handle, &DPI_REG[DPI_IDX(module)]->DPI_EN,
		     0x00000000);
...
unsigned int ddp_dpi_get_cur_addr(bool rdma_mode, int layerid)
{
	...
	if (rdma_mode)
		i = (INREG32(DISP_REG_RDMA_MEM_START_ADDR +
			     DISP_INDEX_OFFSET * 2));
	else
		i = (INREG32(DISP_INDEX_OFFSET + DISP_REG_OVL_L0_ADDR +
			     layerid * 0x20));
	return i;
}

#if ENABLE_DPI_INTERRUPT
static irqreturn_t _DPI_InterruptHandler(enum DISP_MODULE_ENUM module,
					 unsigned int param)
{
	static int counter;
...
int ddp_dpi_init(enum DISP_MODULE_ENUM module, void *cmdq)
{
	...
	for (i = 0; i < DPI_INTERFACE_NUM; i++)
		DPI_OUTREGBIT(cmdq, struct DPI_REG_INTERRUPT,
			      DPI_REG[i]->INT_ENABLE, VSYNC, 1);
...
int ddp_dpi_dump(enum DISP_MODULE_ENUM module, int level)
{
	...
	DDPDUMP("DPI+Color Bar : %04x : 0x%08x\n", 0xF00,
		INREG32(DISPSYS_DPI0_BASE + 0xF00));
#if 0
		   ...
		   uint32_t PLL_CLK, uint32_t SSC_range, bool SSC_disable)
{
	...
	pr_warn(
		"DPI0 LVDSPLL_init: pixel_clock = %d,postdiv = 0x%x, pcw = 0x%x, ck_div = 0x%x\n",
		pixel_clock, postdiv, pcw, ck_div);
#if 0
#ifdef CONFIG_MTK_CLKMGR
	/*LVDSPLL_CON0 */
	OUTREG32(clk_apmixed_base + 0x2c0, (1 << 8) |
			(postdiv << 4) | (1 << 0));
	/*LVDSPLL_CON1 */
	OUTREG32(clk_apmixed_base + 0x2c4, pcw);

	/*CLK_CFG_6  bit0~2 */
	clkmux_sel(MT_MUX_DPI0, ck_div, "dpi0");
#endif
#else
...
void LVDS_DIG_Init(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	DPI_OUTREG32(cmdq_handle, DDP_REG_BASE_MMSYS_CONFIG + 0x90c,
		     0x00010000); /*enable LVDS fifo out */
	LVDS_DIG_RST(
		module,
		cmdq_handle); /*ADD for suspend&resume blurred screen issue */
	pr_warn("LVDS_DIG_init finished\n");

#if 0
	/* pattern enable for 800 x 1280 */
	DPI_OUTREGBIT(cmdq, LVDS_REG_RGTST_PAT, LVDS_TX_REG->LVDS_RGTST_PAT,
			RG_TST_PATN_EN, 0x01);
	DPI_OUTREGBIT(cmdq, LVDS_REG_RGTST_PAT, LVDS_TX_REG->LVDS_RGTST_PAT,
			RG_TST_PATN_TYPE,
		      0x02);
	/* pattern width */
	DPI_OUTREGBIT(cmdq, LVDS_REG_PATN_TOTAL, LVDS_TX_REG->LVDS_PATN_TOTAL,
			RG_PTGEN_HTOTAL,
		      0x360);
	DPI_OUTREGBIT(cmdq, LVDS_REG_PATN_ACTIVE, LVDS_TX_REG->LVDS_PATN_ACTIVE,
			RG_PTGEN_HACTIVE,
		      0x320);
	/* pattern height */
	DPI_OUTREGBIT(cmdq, LVDS_REG_PATN_TOTAL, LVDS_TX_REG->LVDS_PATN_TOTAL,
	...
	DPI_OUTREGBIT(cmdq, LVDS_REG_PATN_ACTIVE, LVDS_TX_REG->LVDS_PATN_ACTIVE,
			RG_PTGEN_VACTIVE,
		      0x500);
	/* pattern start */
	DPI_OUTREGBIT(cmdq, LVDS_REG_PATN_START, LVDS_TX_REG->LVDS_PATN_START,
			RG_PTGEN_VSTART,
		      0x08);
	DPI_OUTREGBIT(cmdq, LVDS_REG_PATN_START, LVDS_TX_REG->LVDS_PATN_START,
			RG_PTGEN_HSTART,
		      0x20);
	/* pattern sync width */
	DPI_OUTREGBIT(cmdq, LVDS_REG_PATN_WIDTH, LVDS_TX_REG->LVDS_PATN_WIDTH,
			RG_PTGEN_VWIDTH,
		      0x01);
	DPI_OUTREGBIT(cmdq, LVDS_REG_PATN_WIDTH, LVDS_TX_REG->LVDS_PATN_WIDTH,
			RG_PTGEN_HWIDTH,
...
void ddp_dpi_lvds_config(enum DISP_MODULE_ENUM module,
			 struct LCM_DPI_PARAMS *dpi_config, void *cmdq_handle)
{
...
void ddp_dpi_RGB_set_DrivingCurrent(enum DISP_MODULE_ENUM module,
				    void *cmdq_handle,
				    enum LCM_DRIVING_CURRENT CURRENT)
{
	switch (CURRENT) {
...
{
	DPI_OUTREG32(cmdq_handle, DDP_REG_BASE_MMSYS_CONFIG + 0x90c,
		     0x00000001); /*enable RGB out */
	pr_warn("RGB config finished\n");
}

void ddp_lvds_power_off(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	DPI_OUTREGBIT(cmdq_handle, struct LVDS_ANA_REG_CTL2,
		      LVDS_ANA_REG->LVDSTX_ANA_CTL2, LVDSTX_ANA_LDO_EN, 0x0);
	DPI_OUTREGBIT(cmdq_handle, struct LVDS_ANA_REG_CTL2,
		      LVDS_ANA_REG->LVDSTX_ANA_CTL2, LVDSTX_ANA_BIAS_EN, 0x0);
	DPI_OUTREGBIT(cmdq_handle, struct LVDS_VPLL_REG_CTL2,
		      LVDS_ANA_REG->LVDS_VPLL_CTL2, LVDS_VPLL_EN, 0x0);
	DPI_OUTREGBIT(cmdq_handle, struct LVDS_ANA_REG_CTL3,
		      LVDS_ANA_REG->LVDSTX_ANA_CTL3, LVDSTX_ANA_EXT_EN, 0x00);
	DPI_OUTREGBIT(cmdq_handle, struct LVDS_ANA_REG_CTL3,
		      LVDS_ANA_REG->LVDSTX_ANA_CTL3, LVDSTX_ANA_DRV_EN, 0x00);
		   ...
		   struct disp_ddp_path_config *config, void *cmdq_handle)
{
	if (((s_isDpiConfig == false) && (module == DISP_MODULE_DPI0)) ||
	    ((s_isDpi1Config == false) && (module == DISP_MODULE_DPI1))) {
		...
		if (module == DISP_MODULE_DPI0) {
			...
			if (dpi_config->lvds_tx_en)
				...
				ddp_dpi_RGB_set_DrivingCurrent(
					module, cmdq_handle,
					dpi_config->io_driving_current);
...
int ddp_dpi_power_on(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	pr_warn(
		"DISP/DPI, ddp_dpi_power_on, s_isDpiPowerOn %d, s_isDpi1PowerOn %d\n",
...
int ddp_dpi_power_off(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	pr_warn(
		"DISP/DPI, ddp_dpi_power_off, s_isDpiPowerOn %d, s_isDpi1PowerOn %d\n",
		s_isDpiPowerOn, s_isDpi1PowerOn);
	if ((s_isDpiPowerOn) && (module == DISP_MODULE_DPI0)) {
...
struct DDP_MODULE_DRIVER ddp_driver_dpi0 = {.module = DISP_MODULE_DPI0,
					    .init = ddp_dpi_init,
...
struct DDP_MODULE_DRIVER ddp_driver_dpi1 = {.module = DISP_MODULE_DPI1,
					    .init = ddp_dpi_init,
					    .deinit = ddp_dpi_deinit,
					    .config = ddp_dpi_config,
					    .build_cmdq = ddp_dpi_build_cmdq,
