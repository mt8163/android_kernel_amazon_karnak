 ...
 * PM_lcm_utils_dsi0.DSI_dcs_read_lcm_reg_v2
 */
...
#define MIPI_SET_VM                                                            \
	FBCONFIG_IOW(54, unsigned int) /* mipi video mode timing setting */
#define MIPI_SET_CC                                                            \
	FBCONFIG_IOW(55, unsigned int) /* mipi non-continuous clock */
#define MIPI_SET_SSC FBCONFIG_IOW(56, unsigned int) /* spread frequency */
#define MIPI_SET_CLK_V2                                                        \
	FBCONFIG_IOW(57, unsigned int) /* For div1,div2,fbk_div case */
 ...
 */
/*
 * extern unsigned int fbconfig_get_layer_vaddr(int layer_id,int *
 * layer_size,int * enable);
 */
/*
 * unsigned int fbconfig_get_layer_height(int layer_id,int * layer_size,int *
 * enable,int* height ,int * fmt);
 */
...
static struct PM_TOOL_T pm_params = {
	.dsi_id = PM_DSI0, .pLcm_params = NULL, .pLcm_drv = NULL,
};
...
int fbconfig_get_esd_check(enum DSI_INDEX dsi_id, uint32_t cmd, uint8_t *buffer,
			   uint32_t num)
{
	int array[4];
	int ret;
	...
	ret = DSI_dcs_read_lcm_reg_v2(pm_get_dsi_handle(dsi_id), NULL, cmd,
				      buffer, num);
...
static ssize_t fbconfig_read(struct file *file, char __user *ubuf, size_t count,
			     loff_t *ppos)
{
			   ...
			   unsigned long arg)
{
	...
	case FB_LAYER_GET_EN: {
		...
		struct OVL_BASIC_STRUCT ovl_all[OVL_LAYER_NUM];
	...
	case FB_LAYER_GET_INFO: {
		...
		if (copy_from_user(&layer_info, (void __user *)argp,
				   sizeof(layer_info))) {
			pr_debug(
				"[TE_SET_ENABLE]: copy_from_user failed! line:%d\n",
				__LINE__);
			return -EFAULT;
		}
	...
	case FB_LAYER_DUMP: {
		...
		if ((layer_size != 0) &&
			...
			/*
			 * pr_debug("sxk==>FB_LAYER_DUMP==>layer_size is %d
			 * mva is 0x%x\n",layer_size,mva);
			 */
			...
			if (ret < 0) {
				pr_debug(
					"m4u_mva_map_kernel error: ret=%d 0x%x %d\r\n",
			...
			pr_debug(
				"==> kva is 0x%lx real_mva %x mva %x mmaped size is %dlayer_size is %d\n",
...
{
	ConfigPara_dbgfs = debugfs_create_file("fbconfig", S_IFREG | 0444, NULL,
					       (void *)0, &fbconfig_fops);
