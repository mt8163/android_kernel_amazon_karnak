...
extern int rdma_resolution_test(enum DISP_MODULE_ENUM module, unsigned int fw,
				unsigned int fh);
extern unsigned int DSI_WaitForNotBusyTimeout(enum DISP_MODULE_ENUM module,
...
extern int DSI_test_force_te(enum DISP_MODULE_ENUM module);
...
extern int fbconfig_get_esd_check(enum DSI_INDEX dsi_id, uint32_t cmd,
...
enum DISP_POWER_STATE { DISP_ALIVE = 0xf0, DISP_SLEPT };

enum DISP_FRM_SEQ_STATE { FRM_CONFIG = 0, FRM_TRIGGER, FRM_START, FRM_END };
...
struct display_primary_path_context {
	...
	struct m4u_client_t *framebuffer_client;
	unsigned long framebuffer_pa;
	unsigned int framebuffer_mva;
	...
	unsigned int dc_buf_sec[DISP_INTERNAL_BUFFER_COUNT];
...
int primary_display_config(unsigned long pa, unsigned int mva);
...
int primary_display_trigger(int blocking, void *callback,
