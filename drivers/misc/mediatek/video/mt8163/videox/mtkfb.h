...
extern struct fb_info *mtkfb_get_fb_info(void);
...
#define MTKFB_GET_INTERFACE_TYPE                                               \
	MTK_IOR(20, unsigned long) /* /0 DBI, 1 DPI, 2 MIPI */
#define MTKFB_GET_POWERSTATE                                                   \
	MTK_IOR(21, unsigned long) /* /0: power off  1: power on */
#define MTKFB_GET_DISPLAY_IF_INFORMATION MTK_IOR(22, struct mtk_dispif_info)
/*
 * called before SET_OVERLAY each time, if true, hwc will not use FB_LAYER
 * again
 */
...
/*
 * ----------------------------------------------------------------------FM
 * De-sense for EM and Normal mode
 */
...
enum MTK_DISP_MODE { DISP_DIRECT_LINK_MODE, DISP_DECOUPLE_MODE };
...
enum MTK_DISP_SESSION { /* map sessions to scenairos in kernel driver */
	       ...
	       /*
		* Extension mode, Dst buf is provided by user,for Wifi Display
		* or other purpose
		*/
