...
unsigned long get_dim_layer_mva_addr(void)
{
	...
	if (dim_layer_mva == 0) {
		...
		if (free_fb_buf || pgc->framebuffer_mva == 0) {
			dim_layer_mva = (unsigned long)init_dim_buffer();
		}
		else
	#endif
		{
		...
		DISPMSG("init dim layer mva %lu", dim_layer_mva);
	}
...
int primary_display_free_fb_buf(void)
{
	...
	if (pgc->framebuffer_pa) {
		...
		free_reserved_area((void *)va_start, (void *)va_end, 0, "fbmem");
		//free_fb_mem(pgc->framebuffer_pa, pgc->framebuffer_pa + (unsigned long)vramsize);
...
/* if (_get_init_setting("M4U")) */
	...
	if (1) {
		...
		*mva = pa_start & 0xffffffffULL;
		ret = m4u_alloc_mva(pgc->framebuffer_client, M4U_PORT_DISP_OVL0, 0, sg_table,
		...
		pr_info("[DISPHAL] FB MVA is 0x%lx PA is %pa\n", *mva, &pa_start);
