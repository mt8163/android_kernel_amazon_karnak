...
static unsigned int dim_layer_mva;
/* wdma dump thread */
		    ...
		    struct disp_internal_buffer_info *buf)
{
	if (ctx && head && buf) {
		...
		/*
		 * DISPMSG("enqueue_buffer, head=0x%08x, buf=0x%08x,
		 * mva=0x%08x\n", head, buf, buf->mva);
		 */
	}
}

void reset_buffer(struct display_primary_path_context *ctx,
		  struct disp_internal_buffer_info *buf)
{
	if (ctx && buf) {
...
dequeue_buffer(struct display_primary_path_context *ctx, struct list_head *head)
{
	struct disp_internal_buffer_info *temp = NULL;

	if (ctx && head) {
		if (!list_empty(head)) {
			temp = list_entry(head->prev,
					  struct disp_internal_buffer_info,
					  list);
			/*
			 * DISPMSG("dequeue_buffer, head=0x%08x, buf=0x%08x,
			 * mva=0x%08x\n", head, temp, temp->mva);
			 */
			...
			if (list_empty(head)) {
				/*
				 * DISPMSG("after dequeue_buffer, head:0x%08x is
				 * empty\n", head);
				 */
				/**reserve**/
			}
...
struct disp_internal_buffer_info *
find_buffer_by_mva(struct display_primary_path_context *ctx,
	...
	if (ctx && head && mva) {
		if (!list_empty(head)) {
			list_for_each_entry(temp, head, list) {
				/*
				 * DISPMSG("find buffer: temp=0x%08x,
				 * mva=0x%08x\n", temp, temp->mva);
				 */
				if (mva == temp->mva) {
					buf = temp;
...
primary_display_path_lock(const char *caller)
{
...
int _disp_primary_path_set_vfp(int enter)
{
	...
	if (primary_display_is_video_mode()) {
		...
		if (ret != 0) {
			DISPCHECK(
				"fail to create primary cmdq handle for set vfp\n"
				);
			return -1;
		}
		...
		if (enter) {
			dpmgr_path_ioctl(
				pgc->dpmgr_handle, cmdq_handle_vfp,
				DDP_DSI_VFP_LP,
				(unsigned long *)&(
					lcm_param->dsi.vertical_vfp_lp));
		} else {
			dpmgr_path_ioctl(
				pgc->dpmgr_handle, cmdq_handle_vfp,
				DDP_DSI_VFP_LP,
					...
					unsigned int need_primary_lock)
{
	...
	if (is_hwc_enabled == 0) {
		DISPMSG(
			"primary_display_save_power_for_idle, hwc not enable ,just return !\n"
			);
		return 0;
	}
	...
	if (disp_low_power_enlarge_blanking == 1) {
		...
		if (enter) {
			DISP_REG_SET(pgc->cmdq_handle_config,
				     dispsys_reg[DISP_REG_DSI0] +
				     DISP_DSI_REG_VFP,
				     vfp_for_low_power);
		} else {
			DISP_REG_SET(pgc->cmdq_handle_config,
				     dispsys_reg[DISP_REG_DSI0] +
				     DISP_DSI_REG_VFP, vfp_original);
		}
#else
	...
	if (disp_low_power_disable_ddp_clock == 1) {
		/* open */
		static unsigned int disp_low_power_disable_ddp_clock_cnt;

		DISPMSG(
			"MM clock, disp_low_power_disable_ddp_clock enter %d.\n",
			disp_low_power_disable_ddp_clock_cnt++);
		if ((enter == 1) && (primary_display_is_video_mode() == 0)) {
			/* only for command mode */
			if (isIdlePowerOff == 0) {
				DISPMSG("off MM clock start.\n");
				_disp_primary_path_idle_clock_off(
					0); /* parameter represent level */
				DISPMSG("off MM clock end.\n");

				if (gDumpClockStatus == 1) {
					/*
					 * DISPMSG("---- start dump regs!
					 * clk_stat_bug.\n");
					 */
					/* clk_stat_bug(); */
					DISPMSG(
						"start dump regs! clk_stat_check\n"
						);
#ifdef CONFIG_MTK_CLKMGR
			...
			if (isIdlePowerOff == 1) {
				...
				_disp_primary_path_idle_clock_on(
					0); /* parameter represent level */
...
void _disp_primary_path_exit_idle(const char *caller,
				  unsigned int need_primary_lock)
{
...
static int _disp_primary_path_idle_detect_thread(void *data)
{
	...
	while (1) {
		...
		if (((sched_clock() - last_primary_trigger_time) / 1000) >
			...
			DISPDBG(
				"[ddp_idle_off]idle for 1s, switch to low power mode!\n"
				);
			_primary_path_lock(__func__);
		...
		ret = wait_event_interruptible(
			idle_detect_wq, (atomic_read(&idle_detect_flag) != 0));
		atomic_set(&idle_detect_flag, 0);
		pr_debug("[ddp_idle]ret=%d\n", ret);
		if (kthread_should_stop())
			break;
		pr_debug("[ddp_idle]_disp_primary_path_idle_detect end\n");
	}
	return 0;
}
...
#if 0
static unsigned int get_frm_seq_by_addr(unsigned int addr,
...
static unsigned int release_started_frm_seq(unsigned int addr,
		enum DISP_FRM_SEQ_STATE FRM_END)
{
	int i = 0;
...
int primary_display_get_debug_state(char *stringbuf, int buf_len)
{
	 ...
	 */
	len += scnprintf(stringbuf + len, buf_len - len,
			 "|session_mode is 0x%08x\n", pgc->session_mode);

	len += scnprintf(
		stringbuf + len, buf_len - len,
		"|State=%s\tlcm_fps=%d\tmax_layer=%d\tmode:%d\tvsync_drop=%d\n",
		pgc->state == DISP_ALIVE ? "Alive" : "Slept", pgc->lcm_fps,
...
static void _dump_internal_buffer_into(struct display_primary_path_context *ctx,
				       char *dump_reason)
{
	int n = 0;
	int len = 0;
	...
	if (ctx) {
		...
		if (!list_empty(head)) {
			list_for_each_entry(buf, head, list) {
				n += scnprintf(_dump_buffer_string + n,
						len - n, "\t0x%08x",
					       buf->mva);
			}
		}
...
int32_t decouple_rdma_worker_callback(unsigned long data)
{
	...
	if (!reading_buf) {
		/*
		 * DISPERR("fatal error, we can't find related buffer info with
		 * callback data:0x%08x\n", data);
		 */
		mutex_unlock(&(pgc->dc_lock));
		return 0;
	}
	...
	list_for_each_entry_safe(temp, n, &(pgc->dc_reading_list), list) {
		if (temp && temp->mva != current_reading_addr) {
			DISPMSG(
				"temp=0x%p, temp->mva=0x%08x, temp->list=0x%lx\n",
...
static int _disp_primary_path_decouple_worker_thread(void *data)
{
	...
	while (1) {
		/*
		 * shuold wait FRAME_START here,
		 * but MUTEX1's sof always issued with MUTEX0's sof,
		 * root cause still unkonwn. so use FRAME_COMPLETE instead
		 */
		dpmgr_wait_event(pgc->ovl2mem_path_handle,
		...
		writing_buf = find_buffer_by_mva(pgc, &pgc->dc_writing_list,
				current_writing_mva);
		if (!writing_buf) {
			DISPERR(
			...
			enqueue_buffer(pgc, &(pgc->dc_reading_list),
					writing_buf);
			...
			if (!reading_buf) {
				...
				enqueue_buffer(pgc, &(pgc->dc_writing_list),
						reading_buf);
			}
			disp_sw_mutex_unlock(&(pgc->dc_lock));
		}
		...
		ret = dpmgr_path_config(pgc->ovl2mem_path_handle, pconfig,
				CMDQ_DISABLE);

		memset((void *)&input, 0, sizeof(
		...
		input.isDirty = 1;
		mmprofile_log_ex(ddp_mmp_get_events()->interface_trigger,
				MMPROFILE_FLAG_PULSE,
			       input.addr, pconfig->wdma_config.dstAddress);
		ret = primary_display_config_interface_input(&input);
		ret =
		    _trigger_display_interface(false,
				    decouple_rdma_worker_callback,
...
#endif
static enum DISP_MODULE_ENUM
_get_dst_module_by_lcm(struct disp_lcm_handle *plcm)
{
	if (plcm == NULL) {
...
static int _build_path_direct_link(void)
{
	...
	enum enum DISP_MODULE_ENUM dst_module = 0;
...
void disp_set_sodi(unsigned int enable, void *cmdq_handle)
{
	...
	if (gEnableSODIControl == 1) {
		if (cmdq_handle != NULL) {
			if (enable == 1)
				cmdqRecWrite(cmdq_handle, DISP_REG_SODI_PA, 0,
					     1);
			else
				cmdqRecWrite(cmdq_handle, DISP_REG_SODI_PA, 1,
					     1);
		} else {
			if (enable == 1)
				DISP_REG_SET(0, SPM_PCM_SRC_REQ,
					     DISP_REG_GET(SPM_PCM_SRC_REQ) &
						     (~0x1));
			else
				DISP_REG_SET(0, SPM_PCM_SRC_REQ,
					     DISP_REG_GET(SPM_PCM_SRC_REQ) |
...
static void _cmdq_build_trigger_loop(void)
{
	...
	if (primary_display_is_video_mode()) {
		...
		/*
		 * DSI command mode doesn't have mutex_stream_eof, need use CMDQ
		 * token instead
		 */
		 ...
		 */
		/*
		 * for operations before frame transfer, such as waiting for DSI
		 * TE
		 */
		if (islcmconnected)
			dpmgr_path_build_cmdq(pgc->dpmgr_handle,
		...
		/*
		 * cleat frame done token, now the config thread will not
		 * allowed to config registers.
		 */
		/*
		 * remember that config thread's priority is higher than trigger
		 * thread,
		 */
		/*
		 * so all the config queued before will be applied then
		 * STREAM_EOF token be cleared
		 */
		...
		/*
		 * (unsigned int)(DISP_REG_CONFIG_MUTEX_EN(0))&0x1fffffff, 1,
		 * ~0);
		 */
		...
		/*
		 * waiting for frame done, because we can't use mutex stream eof
		 * here,
		 */
		...
		/*
		 * dsi is not idle rightly after rdma frame done, so we need to
		 * polling
		 */
	...
	/*
	 * dump trigger loop instructions to check whether dpmgr_path_build_cmdq
	 * works correctly
	 */
...
static int _DL_switch_to_DC_fast(void)
{
	...
	dpmgr_modify_path(pgc->dpmgr_handle,
...
/* 4.config rdma from directlink mode to memory mode */
	...
	rdma_config.pitch =
		primary_display_get_width() *
			DP_COLOR_BITS_PER_PIXEL(rdma_config.inputFormat) >>
		3;
	 ...
	 * DISP_PATH_EVENT_FRAME_START, DDP_IRQ_WDMA0_FRAME_COMPLETE);
	 */
	/*
	 * dpmgr_enable_event(pgc->ovl2mem_path_handle,
...
static int _DC_switch_to_DL_fast(void)
{
	...
	dpmgr_modify_path(pgc->dpmgr_handle, DDP_SCENARIO_PRIMARY_DISP,
			  pgc->cmdq_handle_config,
			  primary_display_is_video_mode() ? DDP_VIDEO_MODE
							  : DDP_CMD_MODE);
	...
	/*
	 * use blocking flush to make sure all config is done, then stop/start
	 * trigger loop
	 */
...
static KREE_SECUREMEM_HANDLE
allocate_decouple_sec_buffer(unsigned int buffer_size)
{
	int /*TZ_RESULT*/ ret;
...
static KREE_SECUREMEM_HANDLE
free_decouple_sec_buffer(KREE_SECUREMEM_HANDLE mem_handle)
{
	int /*TZ_RESULT*/ ret;
...
static int init_decouple_buffers(void)
{
	...
	for (i = 0; i < DISP_INTERNAL_BUFFER_COUNT; i++) {
		...
		if (decouple_buffer_info[i] != NULL) {
			...
			dc_vAddr[i] =
				(unsigned long)decouple_buffer_info[i]->va;
		}
	}
#ifdef CONFIG_MTK_SEC_VIDEO_PATH_SUPPORT
	for (i = 0; i < DISP_INTERNAL_BUFFER_COUNT; i++)
...
static int __build_path_decouple(void)
{
	...
	pgc->dpmgr_handle =
		dpmgr_create_path(DDP_SCENARIO_PRIMARY_RDMA0_COLOR0_DISP,
				  pgc->cmdq_handle_config);
	if (pgc->dpmgr_handle) {
	...
	pgc->ovl2mem_path_handle = dpmgr_create_path(
		DDP_SCENARIO_PRIMARY_OVL_MEMOUT, pgc->cmdq_handle_config);
	if (pgc->ovl2mem_path_handle) {
	 ...
	 * FRAME_START will be used in decouple-mirror mode, for post-path fence
	 * release(rdma->dsi)
	 */
	dpmgr_enable_event(pgc->dpmgr_handle, DISP_PATH_EVENT_IF_VSYNC);
	dpmgr_enable_event(pgc->dpmgr_handle, DISP_PATH_EVENT_FRAME_DONE);
...

	{
		...
		size = primary_display_get_width() *
		...
		for (i = 0; i < DISP_INTERNAL_BUFFER_COUNT; i++) {
			buf = kzalloc(sizeof(struct disp_internal_buffer_info),
				      GFP_KERNEL);
			if (buf) {
				DISPMSG("buf=0x%p\n", buf);
				/*
				 ...
				 */
				src_handle =
					ion_alloc(ion_client, size, 0,
						  ION_HEAP_MULTIMEDIA_MASK, 0);
				if (IS_ERR(src_handle)) {
					DISPERR(
						"Fatal Error, ion_alloc for size %d failed\n",
						size);
				...
				memset((void *)&mm_data, 0,
				       sizeof(struct ion_mm_data));
				mm_data.config_buffer_param.kernel_handle =
					src_handle;
				/*
				 * mm_data.config_buffer_param.m4u_port=
				 * M4U_PORT_DISP_OVL0;
				 */
				/*
				 * mm_data.config_buffer_param.prot =
				 * M4U_PROT_READ|M4U_PROT_WRITE;
				 */
				/*
				 * mm_data.config_buffer_param.flags =
				 * M4U_FLAGS_SEQ_ACCESS;
				 */
				mm_data.mm_cmd = ION_MM_CONFIG_BUFFER;
				if (ion_kernel_ioctl(
					    ion_client, ION_CMD_MULTIMEDIA,
					    (unsigned long)&mm_data) < 0) {
					DISPERR(
						"ion_test_drv: Config buffer failed.\n"
						);
					ion_free(ion_client, src_handle);
					ion_client_destroy(ion_client);
					kfree(buf);
				...
				DISPMSG(
					"buf:0x%p, buf->list:0x%lx, buf->mva:0x%08x, buf->size:0x%08x, ",
					buf, (unsigned long)(&buf->list),
					buf->mva, buf->size);
				DISPMSG("buf->handle:0x%p,dc_vaddr = 0x%lx\n",
					buf->handle, dc_vAddr[i]);
				/*
				 * memcpy from FBMEM to DC buffer for video mode
				 * flash
				 */
				if (i == 0) {
					unsigned int line = 0;
...
int disp_fmt_to_hw_fmt(enum DISP_FORMAT src_fmt, unsigned int *ovl_fmt,
			       ...
			       unsigned int userdata)
{
	...
	if (primary_display_is_video_mode() == 1 &&
		...
		cmdqRecBackupRegisterToSlot(pgc->cmdq_handle_config,
					    pgc->ovl_status_info, 0,
					    DISP_REG_OVL0_STATUS_PA);
		cmdqRecBackupRegisterToSlot(pgc->cmdq_handle_config,
					    pgc->ovl_status_info, 1,
	...
	/*
	 * TODO: _is_decouple_mode() shuold be protected by mutex!!!!!!!!when
	 * dynamic switch decouple/directlink
	 */
...
int _trigger_ovl_to_memory(void *disp_handle, struct cmdqRecStruct *cmdq_handle,
			   fence_release_callback callback, unsigned int data)
{
	...
	cmdqRecFlushAsyncCallback(cmdq_handle, (CmdqAsyncFlushCB)callback,
				  data);
	cmdqRecReset(cmdq_handle);
	cmdqRecWait(cmdq_handle, CMDQ_EVENT_DISP_WDMA0_EOF);
	mmprofile_log_ex(ddp_mmp_get_events()->ovl_trigger,
			 MMPROFILE_FLAG_PULSE, 0, data);
	return 0;
}

int _trigger_ovl_to_memory_mirror(void *disp_handle,
		struct cmdqRecStruct *cmdq_handle,
...
static atomic_t esd_ext_te_event =
	ATOMIC_INIT(0); /* For Vdo Mode EXT TE Check */
		...
		if (gpio_mode == GPIO_DSI_TE_PIN_M_DSI_TE) {
			...
			/* if(_need_register_eint()) */
			{
				...
				mt_set_gpio_mode(GPIO_DSI_TE_PIN,
						 GPIO_DSI_TE_PIN_M_GPIO);
#endif
				/* 2.register eint */
				node = of_find_compatible_node(
					0, 0, "mediatek, DSI_TE_1-eint");
				if (node) {
					/* DISPMSG("node 0x%x\n", node); */
					of_property_read_u32_array(
						...
						ARRAY_SIZE(ints));
					/*
					 * mt_gpio_set_debounce(ints[0],
					 * ints[1]);
					 */
					mt_eint_set_hw_debounce(ints[0],
								ints[1]);
...
int primary_display_esd_check(void)
{
		...
		if (primary_display_is_video_mode()) {
			...
			if (_need_register_eint()) {
				...
				if (wait_event_interruptible_timeout(
					...
					DISPCHECK(
						"esd check fail release fence fake\n"
						);
			...
			if (dpmgr_wait_event_timeout(pgc->dpmgr_handle,
						     DISP_PATH_EVENT_IF_VSYNC,
						     HZ / 2) > 0) {
				ret = 0; /* esd check pass */
			} else {
	...
	if (primary_display_cmdq_enabled()) {
		...
		if (ret == 1) {
			if (_need_wait_esd_eof()) {
				/*
				 * Need set esd check eof synctoken to let
				 * trigger loop go.
				 */
				cmdqCoreSetEvent(CMDQ_SYNC_TOKEN_ESD_EOF);
			}
...
static int primary_display_esd_check_worker_kthread(void *data)
{
	...
	while (1) {
...
#if 0
		{
			...
			period = dprec_logger_get_current_hold_period(
		...
		if (ret == 1) {
			DISPCHECK(
				"[ESD]esd check fail, will do esd recovery %d\n",
				ret);
			i = esd_try_cnt;
			while (i--) {
...
static int _disp_primary_path_check_trigger(void *data)
{
	...
	while (1) {
		...
		mmprofile_log_ex(
			ddp_mmp_get_events()->primary_display_aalod_trigger,
			MMPROFILE_FLAG_PULSE, 0, 0);
...
static void _RDMA0_INTERNAL_IRQ_Handler(enum DISP_MODULE_ENUM module,
					unsigned int param)
{
...
static void _WDMA0_INTERNAL_IRQ_Handler(enum DISP_MODULE_ENUM module,
					unsigned int param)
{
	if (param & 0x1) {
		/* WDMA Done */
...
static void _MUTEX_INTERNAL_IRQ_Handler(enum DISP_MODULE_ENUM module,
					unsigned int param)
{
	if (param & 0x1) {
		/* RDMA-->DSI SOF */
...
#endif
	if (primary_display_is_video_mode()) {
		disp_unregister_module_irq_callback(
			DISP_MODULE_RDMA0, _RDMA0_INTERNAL_IRQ_Handler);
		...
		if (_is_decouple_mode(pgc->session_mode)) {
			disp_register_module_irq_callback(
				DISP_MODULE_MUTEX, _MUTEX_INTERNAL_IRQ_Handler);
			disp_register_module_irq_callback(
				DISP_MODULE_WDMA0, _WDMA0_INTERNAL_IRQ_Handler);
		}
	}
}
					  ...
					  unsigned int height)
{
	if (pgc->plcm) {
		DISPMSG(
			...
			width, height);
		/*
		 * align with 4 is the minimal check, to ensure we can boot up
		 * into kernel,
		 */
					  ...
					  unsigned int param)
{
	...
	if ((module == DISP_MODULE_OVL0) &&
	    (_is_decouple_mode(pgc->session_mode) == 0)) {
		if (param & 0x2) {
			/* ov0 frame done */
			atomic_set(&primary_display_frame_update_event, 1);
			wake_up_interruptible(&primary_display_frame_update_wq);
	...
	if ((module == DISP_MODULE_OVL0) &&
	    (_is_decouple_mode(pgc->session_mode) == 1)) {
		if (param & 0x2) {
...
static int _if_fence_release_worker_thread(void *data)
{
	...
	struct sched_param param = {
		.sched_priority = 94/* RTPM_PRIO_SCRN_UPDATE */ };
...
static int _ovl2mem_fence_release_worker_thread(void *data)
{
	...
	while (1) {
		...
		/*
		 * because when CPU read wdma addr, maybe it's
		 * already changed by next request
		 * but luckly currently we will wait rdma frame done
		 * after wdma done(in CMDQ), so it's safe now
		 */
		dpmgr_wait_event(pgc->ovl2mem_path_handle,
			DISP_PATH_EVENT_FRAME_COMPLETE);
		if (_is_mirror_mode(pgc->session_mode)) {
			...
			cmdqBackupReadSlot(pgc->cur_config_fence, layer,
				&fence_idx);
			cmdqBackupReadSlot(pgc->subtractor_when_free, layer,
				&subtractor);
		...
		if (pgc->session_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE) {
			...
			if (ret == 0) {
				...
				_cmdq_insert_wait_frame_done_token_mira(
					cmdq_handle);
				cmdqBackupReadSlot(
					pgc->rdma_buff_info, 0,
					(uint32_t *)&(tmpConfig.address));

				/*
				 * rdma pitch only use bit[15..0], we use
				 * bit[31:30] to store secure Information
				 */
				cmdqBackupReadSlot(
					pgc->rdma_buff_info, 1,
					(uint32_t *)&(rdma_pitch_sec));
				tmpConfig.pitch = rdma_pitch_sec & ~(3 << 30);
				tmpConfig.security = rdma_pitch_sec >> 30;

				_config_rdma_input_data(&tmpConfig,
							pgc->dpmgr_handle,
							cmdq_handle);
...
static int _fence_release_worker_thread(void *data)
{
	...
	while (1) {
		...
		if (ovl_get_status() == DDP_OVL1_STATUS_PRIMARY_RELEASED) {
			ovl_set_status(DDP_OVL1_STATUS_SUB);
			wake_up_interruptible(&ovl1_wait_queue);
		} else if (ovl_get_status() ==
				DDP_OVL1_STATUS_PRIMARY_DISABLE) {
			ovl_set_status(DDP_OVL1_STATUS_IDLE);
			wake_up_interruptible(&ovl1_wait_queue);
		}
...
int primary_display_capture_framebuffer_wdma(void *data)
{
	...
	ret = m4u_alloc_mva(m4uClient, M4U_PORT_DISP_WDMA0,
			    (unsigned long)va[0], NULL, buffer_size,
	...
	if (primary_display_cmdq_enabled()) {
		...
		if (ret != 0) {
			DISPCHECK(
				"wdma dump:Fail to create primary cmdq handle for capture\n"
				);
			ret = -1;
			goto out;
		}
...
int primary_display_switch_wdma_dump(int on)
{
	if (on && (!primary_dump_wdma)) {
		...
		primary_display_wdma_out =
			kthread_create(primary_display_capture_framebuffer_wdma,
				       NULL, "display_wdma_out");
					     ...
					     unsigned long pa)
{
	...
	DISPMSG("framebuffer va %lu, mva %u\n", va, mva);
...
unsigned int primary_display_get_frame_buffer_mva_address(void)
{
...
int is_dim_layer(unsigned int mva)
{
...
unsigned int get_dim_layer_mva_addr(void)
{
	_primary_path_lock(__func__);
...
int primary_display_init(char *lcm_name, unsigned int lcm_fps)
{
	 ...
	 */
	if (ret)
		DISP_FATAL_ERR("DISP_SEC",
			       "fail to create disable handle ret=%d\n", ret);
...
/* dpmgr_path_init(pgc->dpmgr_handle, CMDQ_DISABLE); */
	...
	rdma_set_target_line(DISP_MODULE_RDMA0,
			     primary_display_get_height() * 1 / 2,
			     pgc->cmdq_handle_config);
	rdma_set_target_line(DISP_MODULE_RDMA0,
			     primary_display_get_height() * 1 / 2, NULL);
...
/* this should remove? for video mode when LK has start path */
	...
	primary_display_esd_check_task =
		kthread_create(primary_display_esd_check_worker_kthread, NULL,
			       "display_esd_check");
	...
	if (_need_register_eint()) {
		...
		if (node) {
			...
			if (request_irq(irq, _esd_check_ext_te_irq_handler,
					IRQF_TRIGGER_NONE, "DSI_TE_1-eint",
					NULL)) {
				DISPCHECK(
					"[ESD]EINT IRQ LINE NOT AVAILABLE!!\n");
			} else {
	...
	if (_is_decouple_mode(pgc->session_mode)) {
		if_fence_release_worker_task =
		    kthread_create(_if_fence_release_worker_thread,
				    NULL, "if_fence_worker");
...
int primary_display_resume(void)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_resume,
			 MMPROFILE_FLAG_PULSE, 0, 1);
	DISPCHECK("[POWER]lcm resume_power[begin]\n");
	disp_lcm_resume_power(pgc->plcm);
	DISPCHECK("[POWER]lcm resume_power[end]\n");
...

	{
		...
		if (_is_decouple_mode(pgc->session_mode)) {
			...
			ret = dpmgr_path_config(pgc->ovl2mem_path_handle,
						data_config, NULL);
			mmprofile_log_ex(ddp_mmp_get_events()->primary_resume,
					 MMPROFILE_FLAG_PULSE, 2, 2);
		}
	...
	if (primary_display_is_video_mode()) {
		...
		/*
		 * insert a wait token to make sure first config after resume
		 * will config to HW when HW idle
		 */
		_cmdq_insert_wait_frame_done_token();
	}
	mmprofile_log_ex(ddp_mmp_get_events()->primary_resume,
...
int primary_display_trigger(int blocking, void *callback, unsigned int userdata)
{
	...
	if (is_switched_dst_mode) {
		primary_display_switch_dst_mode(
			1); /* swith to vdo mode if trigger disp */
	...
	if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE) {
		...
		if (pgc->need_trigger_dcMirror_out == 0) {
			DISPPR_ERROR(
				"There is no output config when decouple mirror!!\n"
				);
		} else {
			pgc->need_trigger_dcMirror_out = 0;
			_trigger_ovl_to_memory_mirror(
				pgc->ovl2mem_path_handle,
				pgc->cmdq_handle_ovl1to2_config,
	...
	if (primary_display_use_cmdq == CMDQ_DISABLE)
		dpmgr_path_flush(pgc->dpmgr_handle,
				 primary_display_cmdq_enabled());
				    ...
				    unsigned int userdata)
{
	...
	if (pgc->state == DISP_SLEPT || !_is_mirror_mode(pgc->session_mode)) {
		DISPMSG(
			"mem out trigger is already slept or is not mirror mode(%d)\n",
			pgc->session_mode);
		return 0;
	}
	/* /dprec_logger_start(DPREC_LOGGER_PRIMARY_TRIGGER, 0, 0); */
	...
	if (gDumpMemoutCmdq == 1) {
		DISPMSG(
			"primary_display_mem_out_trigger, dump before flush 1:\n"
			);
		cmdqRecDumpCommand(pgc->cmdq_handle_ovl1to2_config);
	}
	...
	if (gDumpMemoutCmdq == 1) {
		DISPMSG(
			"primary_display_mem_out_trigger, dump before flush 2:\n"
			);
		cmdqRecDumpCommand(pgc->cmdq_handle_ovl1to2_config);
	}
	...
	if (_should_flush_cmdq_config_handle())
		 ...
		 * pgc->cmdq_handle_ovl1to2_config, 0);
		 */
		cmdqRecFlushAsyncCallback(
			pgc->cmdq_handle_ovl1to2_config,
...
int primary_display_config_output(struct disp_mem_output_config *output)
{
	...
	/*
	 * dprec_logger_done(DPREC_LOGGER_PRIMARY_CONFIG, output->src_x,
	 * output->src_y);
	 */
	return ret;
}
...
static int _config_interface_input(struct primary_disp_input_config *input)
{
	...
	dprec_logger_start(DPREC_LOGGER_PRIMARY_CONFIG, input->layer |
			(input->layer_en << 16),
			   input->addr);
...
static int _config_ovl_input(struct disp_session_input_config *session_input)
{
	...
	for (i = 0; i < session_input->config_layer_num; i++) {
		...
		if (session_input->setter != SESSION_USER_AEE) {
			if (isAEEEnabled &&
			    layer == primary_display_get_option(
					     "ASSERT_LAYER")) {
		...
		if ((ovl_cfg->layer == 0) &&
			...
			update_frm_seq_info(
				...
				mtkfb_query_frm_seq_by_addr(pgc->session_id, 0,
							    0),
				FRM_CONFIG);
	...
	if (ovl_get_status() == DDP_OVL1_STATUS_SUB_REQUESTING) {
		...
		if (max_layer_id_configed <
		    OVL_LAYER_NUM_PER_OVL - isAEEEnabled) {
			for (i = OVL_LAYER_NUM_PER_OVL; i < OVL_LAYER_NUM; i++)
				data_config->ovl_config[i].layer_en = 0;

			force_disable_ovl1 = 1;
			DISPMSG(
				"cascade: HWC set %d layers, force disable OVL1 layers\n",
				max_layer_id_configed);
		} else {
			DISPMSG(
				"cascade: try to split ovl1 fail: HWC set %d layers\n",
				max_layer_id_configed);
		}
	}
	if (ovl_get_status() == DDP_OVL1_STATUS_SUB) {
		/*disable ovl layer 4~8 to free ovl1 */
		if (OVL_LAYER_NUM_PER_OVL - isAEEEnabled <=
		    max_layer_id_configed) {
			for (i = OVL_LAYER_NUM_PER_OVL; i < OVL_LAYER_NUM; i++)
				data_config->ovl_config[i].layer_en = 0;
	...
	ret = dpmgr_path_config(disp_handle, data_config,
				primary_display_cmdq_enabled() ? cmdq_handle
							       : NULL);
	...
	if (force_disable_ovl1) {
		for (layer = OVL_LAYER_NUM_PER_OVL; layer < OVL_LAYER_NUM;
		     layer++) {
...
static int _config_ovl_output(struct disp_mem_output_config *output)
{
	...
	ret = dpmgr_path_config(pgc->ovl2mem_path_handle,
			data_config, pgc->cmdq_handle_config);
			      ...
			      void **handle)
{
	...
	ret = _convert_disp_input_to_rdma(
	...
	struct disp_session_input_config *session_input)
{
	...
	if (is_sec && (pgc->dc_buf_sec[0] == 0)) {
		for (i = 0; i < DISP_INTERNAL_BUFFER_COUNT; i++)
			pgc->dc_buf_sec[i] =
				allocate_decouple_sec_buffer(buffer_size);
	} else if (!is_sec && (pgc->dc_buf_sec[0] != 0)) {
		for (i = 0; i < DISP_INTERNAL_BUFFER_COUNT; i++) {
			pgc->dc_buf_sec[i] =
				free_decouple_sec_buffer(pgc->dc_buf_sec[i]);
			pgc->dc_buf_sec[i] = 0;
		}
	}
#endif
...
int primary_display_config_interface_input(
	struct primary_disp_input_config *input)
{
	int ret = 0;
	void *cmdq_handle = NULL;
	...
	ret = _convert_disp_input_to_rdma(&(data_config->rdma_config),
	...
	/*
	 * this is used for decouple mode, to indicate whether we need to
	 * trigger ovl
	 */
	...
	if (handle) {
		if (cmdqRecGetInstructionCount(handle) > cmdqsize) {
			...
			if (pgc->state == DISP_ALIVE) {
				_cmdq_set_config_handle_dirty_mira(handle);
				/*
				 * use non-blocking flush here to avoid primary
				 * path is locked for too long
				 */
				if (_should_flush_cmdq_config_handle())
...
#ifdef MTK_DISP_IDLE_LP
int primary_display_switch_mode_nolock(int sess_mode, unsigned int session,
				       int force)
{
	DISPDBG("primary_display_switch_mode sess_mode %d, session 0x%x\n",
		sess_mode, session);
	...
	if (pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE &&
	    sess_mode == DISP_SESSION_DECOUPLE_MODE) {
		/*
		 * signal frame start event to switch logic in
		 * fence_release_worker_thread
		 */
		/*
		 * dpmgr_signal_event(pgc->dpmgr_handle,
		 * DISP_PATH_EVENT_FRAME_START);
		 */
		_DL_switch_to_DC_fast();
		pgc->session_mode = sess_mode;
		 ...
		 */
		/*
		 * dpmgr_signal_event(pgc->ovl2mem_path_handle,
		 * DISP_PATH_EVENT_FRAME_START);
		 */
		_DC_switch_to_DL_fast();
		pgc->session_mode = sess_mode;
...
int primary_display_get_info(void *info)
{
	...
	case LCM_TYPE_DSI: {
		...
		if (lcm_param->dsi.mode == CMD_MODE) {
			...
			dispif_info->isHwVsyncAvailable = 1;
			/*
			 * DISPMSG("DISP Info: DSI, CMD Mode, HW Vsync
			 * enable\n");
			 */
		} else {
			dispif_info->displayMode = DISP_IF_MODE_VIDEO;
			dispif_info->isHwVsyncAvailable = 1;
			/*
			 * DISPMSG("DISP Info: DSI, VDO Mode, HW Vsync
			 * enable\n");
			 */
		}
...
#error
	{
		...
		if (lcm_drv) {
			...
			DISPMSG(
				"DISP Info: LCM Panel Original Resolution(For DFO Only): %d x %d\n",
				dispif_info->lcmOriginalWidth,
				dispif_info->lcmOriginalHeight);
		} else {
...
{
	/*
	 * TODO: we should store the video/cmd mode in runtime, because we will
	 * support cmd/vdo dynamic switch
	 */
...
int primary_display_cmdq_enabled(void)
{
	return primary_display_use_cmdq;
}

int primary_display_switch_cmdq_cpu(int use_cmdq)
{
	_primary_path_lock(__func__);
...
int _set_backlight_by_cmdq(unsigned int level)
{
	...
	if (primary_display_is_video_mode()) {
		...
		dpmgr_path_ioctl(pgc->dpmgr_handle, cmdq_handle_backlight,
				 DDP_BACK_LIGHT, (unsigned long *)&level);
		...
		cmdqRecClearEventToken(cmdq_handle_backlight,
				       CMDQ_SYNC_TOKEN_CABC_EOF);
		dpmgr_path_ioctl(pgc->dpmgr_handle, cmdq_handle_backlight,
				 DDP_BACK_LIGHT, (unsigned long *)&level);
...
int primary_display_setbacklight(unsigned int level)
{
...
#if 0
	{
		...
		DSI_dcs_read_lcm_reg_v2(DISP_MODULE_DSI0, NULL,
				0x51, buffer, 1);
...
static int _set_backlight_mode_by_cmdq(unsigned int mode)
{
	...
	if (ret != 0) {
		DISPCHECK(
			"fail to create primary cmdq handle for backlight mode\n");
		return -1;
	}
	...
	if (primary_display_is_video_mode()) {
		cmdqRecReset(cmdq_handle);
		_cmdq_insert_wait_frame_done_token_mira(cmdq_handle);
		dpmgr_path_ioctl(pgc->dpmgr_handle, cmdq_handle, DDP_CABC_MODE,
				 (unsigned long *)&mode);
		_cmdq_flush_config_handle_mira(cmdq_handle, 1);
...
int _set_backlight_mode_by_cpu(unsigned int mode)
{
	...
	mmprofile_log_ex(ddp_mmp_get_events()->primary_set_bl,
			 MMPROFILE_FLAG_PULSE, 0, 1);
	if (primary_display_is_video_mode()) {
		disp_lcm_set_backlight_mode(pgc->plcm, mode);
	} else {
		_cmdq_stop_trigger_loop();
		if (dpmgr_path_is_busy(pgc->dpmgr_handle)) {
			ret = dpmgr_wait_event_timeout(
...
int primary_display_setbacklight_mode(unsigned int mode)
{
	...
	_primary_path_lock(__func__);
	if (pgc->state == DISP_SLEPT) {
		DISPCHECK("Slept State set backlight mdoe invalid\n");
		ret = -1;
	} else {
		if (primary_display_cmdq_enabled()) {
			if (primary_display_is_video_mode())
...
int primary_display_set_cmd(int *lcm_cmd, unsigned int cmd_num)
{
	...
	if (pgc->state == DISP_SLEPT) {
		...
		if (primary_display_cmdq_enabled()) { /* cmdq */
			...
			if (ret != 0) {
				DISPCHECK(
					"fail to create primary cmdq handle for cmd\n");
				return -1;
			}
			if (primary_display_is_video_mode()) {
				/* video mode */
				cmdqRecReset(cmdq_handle_cmd);
				disp_lcm_set_cmd(pgc->plcm,
						 (void *)cmdq_handle_cmd,
						 lcm_cmd, cmd_num);
				_cmdq_flush_config_handle_mira(cmdq_handle_cmd,
							       1);
				...
				disp_lcm_set_cmd(pgc->plcm,
						 (void *)cmdq_handle_cmd,
						 lcm_cmd, cmd_num);
				cmdqRecSetEventToken(cmdq_handle_cmd,
						     CMDQ_SYNC_TOKEN_CABC_EOF);
				_cmdq_flush_config_handle_mira(cmdq_handle_cmd,
							       1);
				DISPCHECK("[Display]_set_cmd_by_cmdq ret=%d\n",
...
int primary_display_capture_framebuffer_decouple(unsigned long pbuf,
						 unsigned int format)
{
	unsigned int i = 0;
	int ret = 0;
	...
	DISPMSG("map 0x%08x with %d bytes to 0x%08lx with %d bytes\n", mva,
		buffer_size, va, mapped_size);

	ret = m4u_cache_sync(m4uClient, M4U_PORT_DISP_WDMA0, va, buffer_size,
			     mva, M4U_CACHE_FLUSH_ALL);
#if 1
	{
		unsigned int j = 0;
					    ...
					    unsigned int format)
{
	...
	if (_is_decouple_mode(pgc->session_mode) ||
		...
		memset((void *)pbuf, 0, buffer_size);
		DISPMSG(
			"primary capture: fill black for decouple & mirror mode End\n"
			);
		goto out;
	}
	...
	ret = m4u_alloc_mva(m4uClient, M4U_PORT_DISP_WDMA0, pbuf, NULL,
			    buffer_size, M4U_PROT_READ | M4U_PROT_WRITE, 0,
			    &mva);
	if (ret != 0) {
				  ...
				  unsigned long *va, unsigned int *mva)
{
	...
	page_count = (pa_end - pa_start + 1 + PAGE_SIZE - 1) / PAGE_SIZE;
	pages = kmalloc_array(page_count, sizeof(struct page *), GFP_KERNEL);
	pr_info("[DISPHAL] page count: %d\n", page_count);
	for (i = 0; i < page_count; i++) {
		addr = pa_start + i * PAGE_SIZE;
		pages[i] = pfn_to_page(addr >> PAGE_SHIFT);
	}

	*va = (unsigned long)vmap(pages, page_count, VM_MAP, PAGE_KERNEL);
	kfree(pages);

	/*
	 * *va = (unsigned long)ioremap_nocache(pa_start, pa_end - pa_start +
	 * 1);
	 */
	pr_info("disphal_allocate_fb, pa=%p, va=0x%lx\n", (void *)pa_start,
		*va);
...
/* xuecheng, m4u not enabled now */
	...
	if (1) {
		...
		pgc->framebuffer_client = m4u_create_client();
		if (IS_ERR_OR_NULL(pgc->framebuffer_client))
			DISPMSG("create client fail!\n");

		*mva = pa_start & 0xffffffffULL;
		ret = m4u_alloc_mva(pgc->framebuffer_client, M4U_PORT_DISP_OVL0,
				    0, sg_table, (pa_end - pa_start + 1),
				    M4U_PROT_READ | M4U_PROT_WRITE,
		...
		pr_info("[DISPHAL] FB MVA is 0x%x PA is %pa\n", *mva,
			&pa_start);
...
int primary_display_check_path(char *stringbuf, int buf_len)
{
	...
	if (stringbuf) {
		...
		DISPMSG("COLOR0:\t\t\t%s\t%s\n", IS_READY(READY_BIT0(
						DDP_SIGNAL_COLOR__CCORR)),
			IS_VALID(READY_BIT0(DDP_SIGNAL_COLOR__CCORR)));
		DISPMSG("CCORR:\t\t%s\t%s\n", IS_READY(READY_BIT0(
						DDP_SIGNAL_CCORR__AAL)),
			IS_VALID(READY_BIT0(DDP_SIGNAL_CCORR__AAL)));
		DISPMSG("AAL0:\t\t\t%s\t%s\n", IS_READY(READY_BIT0(
						DDP_SIGNAL_AAL__GAMMA)),
			IS_VALID(READY_BIT0(DDP_SIGNAL_AAL__GAMMA)));
		DISPMSG("GAMMA:\t\t\t%s\t%s\n", IS_READY(READY_BIT1(
						DDP_SIGNAL_GAMMA__DITHER)),
			IS_VALID(READY_BIT1(DDP_SIGNAL_GAMMA__DITHER)));
		DISPMSG("DITHER:\t\t\t%s\t%s\n",
		...
		DISPMSG("UFOE_SEL:\t\t%s\t%s\n", IS_READY(READY_BIT0(
						DDP_SIGNAL_UFOE_SEL__UFOE)),
			IS_VALID(READY_BIT0(DDP_SIGNAL_UFOE_SEL__UFOE)));
		DISPMSG("UFOE:\t\t\t%s\t%s\n", IS_READY(READY_BIT0(
						DDP_SIGNAL_UFOE__UFOE_MOUT)),
			IS_VALID(READY_BIT0(DDP_SIGNAL_UFOE__UFOE_MOUT)));
...
int primary_display_lcm_ATA(void)
{
	...
	if (pgc->state == 0) {
		DISPCHECK(
			"ATA_LCM, primary display path is already slept, skip\n"
			);
		goto done;
	}
...
int primary_display_switch_dst_mode(int mode)
{
	...
	if (lcm_cmd == NULL) {
		DISPCHECK(
			"[primary_display_switch_dst_mode]get lcm cmd fail\n",
			primary_display_cur_dst_mode, mode);
		goto done;
	} else {
		int temp_mode = 0;

		if (dpmgr_path_ioctl(pgc->dpmgr_handle,
					  pgc->cmdq_handle_config,
...
int primary_display_rdma_res_test(void)
{
	...
	if (ret < 0)
		...
		pr_debug(
			"[display_test_result]==>RDMA RESOLUTION SETTING ONCE!\n"
			);
	return ret;
}
