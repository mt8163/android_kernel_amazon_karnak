...
#define DISP_FATAL_ERR(tag, string, args...)                                   \
	do {                                                                   \
		pr_debug("[DISP][%s #%d]ERROR:" string, __func__, __LINE__,    \
			 ##args);                                              \
		if (0)                                                         \
			dprec_error_log_len += scnprintf(                      \
				dprec_error_log_buffer + dprec_error_log_len,  \
...
#define DISPPR(string, args...)                                                \
	do {                                                                   \
		dprec_error_log_len =                                          \
			scnprintf(dprec_error_log_buffer,                      \
				  dprec_error_log_buflen, string, ##args);     \
...
#define DISPPR_ERROR(string, args...)                                          \
	do {                                                                   \
		dprec_logger_pr(DPREC_LOGGER_ERROR, string, ##args);           \
		pr_err("[DISP][%s #%d]ERROR:" string, __func__, __LINE__,      \
		       ##args);                                                \
	} while (0)
#endif
