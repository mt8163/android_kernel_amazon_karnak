...
/**
 * Copy of
 * alps/vendor/mediatek/proprietary_mt8163/hardware/dpframework/inc/DpDataType.h
...
enum DP_PROFILE_ENUM { DP_PROFILE_BT601, DP_PROFILE_BT709, DP_PROFILE_JPEG };
...
#define DP_COLORFMT_PACK(VIDEO, PLANE, COPLANE, HFACTOR, VFACTOR, BITS, GROUP, \
	...
	((VIDEO << 27) | (PLANE << 24) | (COPLANE << 22) | (HFACTOR << 20) |   \
	 (VFACTOR << 18) | (BITS << 8) | (GROUP << 6) | (SWAP_ENABLE << 5) |   \
	 (UNIQUEID << 0))
...
enum DP_COLOR_ENUM {
	...
	DP_COLOR_420_BLKP_UFO = DP_COLORFMT_PACK(
		5, 2, 1, 1, 1, 256, 1, 0, 12), /* Frame mode + Block mode */
	DP_COLOR_420_BLKP = DP_COLORFMT_PACK(1, 2, 1, 1, 1, 256, 1, 0,
					     12), /* Frame mode + Block mode */
	DP_COLOR_420_BLKI = DP_COLORFMT_PACK(3, 2, 1, 1, 1, 256, 1, 0,
					     12), /* Field mode + Block mode */
	DP_COLOR_422_BLKP =
		DP_COLORFMT_PACK(1, 1, 0, 1, 0, 512, 1, 0, 4), /* Frame mode */
...
enum DpInterlaceFormat { eInterlace_None, eTop_Field, eBottom_Field };

enum DpSecure { DP_SECURE_NONE = 0, DP_SECURE = 1, DP_SECURE_SHIFT = 8 };
