...
int ovl2mem_get_info(void *info)
{
	...
	/*
	 * FIXME,  for decouple mode, should dynamic return 4 or 8, please refer
	 * to primary_display_get_info()
	 */
...
static int ovl2mem_callback(unsigned int userdata)
{
	...
	for (layid = 0; layid < (HW_OVERLAY_COUNT + 1); layid++) {
		...
		if (fence_idx < 0) {
			if (fence_idx == -1) {
				/*
				 * DISPPR_ERROR("find fence idx for layer
				 * %d,addr 0x%08x fail,
				 * unregistered addr%d\n", layid, userdata,
				 * fence_idx);
				 */
			...
			if (data_config && (layid >= HW_OVERLAY_COUNT)) {
				wdma_layer.dstAddress = mtkfb_query_buf_mva(
					pgc->session, layid, fence_idx);
...
int ovl2mem_init(unsigned int session)
{
...
#if 0
	ret =
	    cmdqCoreRegisterCB(CMDQ_GROUP_DISP, cmdqDdpClockOn,
	...
	/*
	 * dpmgr_path_set_dst_module(pgc->dpmgr_handle,DISP_MODULE_ENUM
	 * dst_module)
	 */
...
int ovl2mem_input_config(struct ovl2mem_in_config *input)
{
	...
	/*
	 * hope we can use only 1 input struct for input config, just set layer
	 * number
	 */
	for (i = 0; i < HW_OVERLAY_COUNT; i++) {
		...
		if (input[i].dirty) {
			ret = _convert_disp_input_to_ovl(
				&(data_config->ovl_config[input[i].layer]),
...
int ovl2mem_output_config(struct ovl2mem_out_config *out)
{
	int ret = -1;
	struct disp_ddp_path_config *data_config;
	/* /DISPFUNC(); */
...
int ovl2mem_trigger(int blocking, void *callback, unsigned int userdata)
{
	...
	dprec_logger_frame_seq_begin(
		pgc->session, mtkfb_query_frm_seq_by_addr(pgc->session, 0, 0));
...
int ovl2mem_deinit(void)
{
	...
	while (((atomic_read(&g_trigger_ticket) -
		 atomic_read(&g_release_ticket)) != 1) &&
	       (loop_cnt < 10)) {
		usleep_range(5000, 6000); /*wait the last configuration done  */
		loop_cnt++;
	}
