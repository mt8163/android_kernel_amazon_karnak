	...
	case MTKFB_VSYNC_IOCTL: {
		...
		if (arg == MTKFB_VSYNC_SOURCE_HDMI) {
			...
			if (down_interruptible(&mtkfb_vsync_sem)) {
				pr_debug(
					"[mtkfb_vsync_ioctl] can't get semaphore,%d\n",
					__LINE__);
				msleep(20);
				return ret;
			}
			...
			up(&mtkfb_vsync_sem);
			MTKFB_VSYNC_LOG(
				"[MTKFB_VSYNC]: leave MTKFB_VSYNC_IOCTL, %d\n",
				__LINE__);
#else
			MTKFB_VSYNC_LOG(
				"[MTKFB_VSYNC]: NS leave MTKFB_VSYNC_IOCTL, %d\n",
				__LINE__);
			ret = -EFAULT;
#endif
			return ret;
		...
		if (down_interruptible(&mtkfb_vsync_sem)) {
			pr_debug("[mtkfb_vsync_ioctl] can't get semaphore,%d\n",
				 __LINE__);
			msleep(20);
			return ret;
		}
...
static int mtkfb_vsync_probe(struct platform_device *pdev)
{
	...
	if (alloc_chrdev_region(&mtkfb_vsync_devno, 0, 1,
				MTKFB_VSYNC_DEVNAME)) {
