...
uint32_t MFC_Get_Cursor_Offset(void *handle)
{
	...
	uint32_t offset = ctxt->cursor_col * MFC_FONT_WIDTH * MFC_BPP +
			  ctxt->cursor_row * MFC_FONT_HEIGHT * MFC_PITCH;

	return offset;
}

static void _MFC_DrawChar(struct MFC_CONTEXT *ctxt, uint32_t x, uint32_t y,
			  char c)
{
...
enum MFC_STATUS MFC_SetMem(void *handle, const char *str, uint32_t color)
{
	...
	for (j = 0; j < MFC_FONT_HEIGHT; j++) {
		ptr = (uint32_t *)(ctxt->fb_addr + (j + 1) * MFC_PITCH -
				   count * ctxt->fb_bpp);
