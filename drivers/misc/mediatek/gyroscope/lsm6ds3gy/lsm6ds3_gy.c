...
#ifdef CONFIG_OF
static const struct of_device_id i2c_gyro_of_match[] = {
...
#ifdef CONFIG_OF
        .of_match_table = i2c_gyro_of_match,
#endif
    },
...
#define LSM6DS3_GYRO_CALI_IDME           "/proc/idme/sensorcal"
#define LSM6DS3_GYRO_CALI_FILE          "/data/inv_cal_data.bin"
#define LSM6DS3_DATA_BUF_NUM            3
#define LSM6DS3_CALI_FILE_SIZE          20

#define LSM6DS3_CALI_DATA_BYPASS_NUM    10  /*3*/
#define GYRO_MAX_OFFSET_VAL             170  //142 -> 170 for HVT build     // Gyro FS= +/-2000dps
#define GYRO_MIN_OFFSET_VAL             -170  //-142 -> -170    // Gyro FS= +/-2000dps
...
static int gyro_self_test[LSM6DS3_GYRO_AXES_NUM] = {0};
static s16 gyro_xyz_offset[LSM6DS3_GYRO_AXES_NUM] = {0};
u8 gyro_current_reg[CTRL_REG_NUM] = {0};

static int gyro_store_offset_in_file(const char *filename, s16 offset[3], int data_valid);
...
static inline s16 cali_axis_value(s16 raw)
{
	long long temp;

	temp = (long long)raw * LSM6DS3_GYRO_SENSITIVITY_2000DPS * 131;
	return (s16) div_s64(temp, 1000000);
}
...
static int LSM6DS3_ReadGyroData(struct i2c_client *client, char *buf, int bufsize)
{
    char databuf[6];
    int data[3];
    static s16 offset[LSM6DS3_GYRO_AXES_NUM] = {0};
    struct lsm6ds3_gyro_i2c_data *obj = i2c_get_clientdata(client);
...
#endif
	/*report degree/s */
	obj->data[LSM6DS3_AXIS_X] = cali_axis_value(obj->data[LSM6DS3_AXIS_X]);
	obj->data[LSM6DS3_AXIS_Y] = cali_axis_value(obj->data[LSM6DS3_AXIS_Y]);
	obj->data[LSM6DS3_AXIS_Z] = cali_axis_value(obj->data[LSM6DS3_AXIS_Z]);

	offset[0] = cali_axis_value(offset[LSM6DS3_AXIS_X]);
	offset[1] = cali_axis_value(offset[LSM6DS3_AXIS_Y]);
	offset[2] = cali_axis_value(offset[LSM6DS3_AXIS_Z]);
              /*
...
/*
        ...
        cali_file->f_pos=LSM6DS3_DATA_BUF_NUM *sizeof(s16);  //seek to fs pos 3 becuase g sensor uses 0-2
     cali_file->f_op->write(cali_file, (void *)w_buf, sizeof(w_buf), &cali_file->f_pos);
        cali_file->f_pos=LSM6DS3_DATA_BUF_NUM *sizeof(s16);
...
static int gyro_store_offset_in_file(const char *filename, s16 *offset, int data_valid)
{
    ...
    else
    {
        ...
        cali_file->f_pos=LSM6DS3_DATA_BUF_NUM *sizeof(s16)*2;
        cali_file->f_op->write(cali_file, (void *)w_buf, LSM6DS3_DATA_BUF_NUM*sizeof(s16)*2+1, &cali_file->f_pos);
        cali_file->f_pos=LSM6DS3_DATA_BUF_NUM *sizeof(s16)*2;
...
static ssize_t set_gyro_cali(struct device_driver *ddri, char *buf)
{
    ...
    static int xyz[LSM6DS3_CALI_DATA_NUM+LSM6DS3_CALI_DATA_BYPASS_NUM][3] = {{0}};
    ...
    err = LSM6DS3_gyro_SetPowerMode(client, true);
    if (err)
    {
        GYRO_ERR("[Sensor] enable power fail\n");
    ...
    {
        err = hwmsen_read_byte(client, LSM6DS3_OUTX_L_G, &data[i][0]);
        if (err < 0)
            return snprintf(buf, PAGE_SIZE, "read LSM6DS3_OUTX_L_XL fail\n" );
        err = hwmsen_read_byte(client, LSM6DS3_OUTX_H_G, &data[i][1]);
        if (err < 0)
            return snprintf(buf, PAGE_SIZE, "read LSM6DS3_OUTX_H_XL fail\n" );
        err = hwmsen_read_byte(client, LSM6DS3_OUTY_L_G, &data[i][2]);
        if (err < 0)
            return snprintf(buf, PAGE_SIZE, "read LSM6DS3_OUTY_L_XL fail\n" );
        err = hwmsen_read_byte(client, LSM6DS3_OUTY_H_G, &data[i][3]);
        if (err < 0)
            return snprintf(buf, PAGE_SIZE, "read LSM6DS3_OUTY_H_XL fail\n" );
        err = hwmsen_read_byte(client, LSM6DS3_OUTZ_L_G, &data[i][4]);
        if (err < 0)
            return snprintf(buf, PAGE_SIZE, "read LSM6DS3_OUTZ_L_XL fail\n" );
        err = hwmsen_read_byte(client, LSM6DS3_OUTZ_H_G, &data[i][5]);
        if (err < 0)
    ...
    err = LSM6DS3_gyro_SetPowerMode(client, false);
    if (err)
    {
        GYRO_ERR("[Sensor] disable power fail\n");
    ...
    if (avg[0]>GYRO_MAX_OFFSET_VAL || avg[0]<GYRO_MIN_OFFSET_VAL)
    {
        ...
        offset[1] = 0 - avg[1];
        offset[2] = 0 - avg[2];
        //gyro_store_offset_in_file(LSM6DS3_GYRO_CALI_FILE, offset, 0);
        return snprintf(buf, PAGE_SIZE, "[Sensor] %s: X axis out of range %d\n", __FUNCTION__, avg[0]);
    }
    else if (avg[1]>GYRO_MAX_OFFSET_VAL || avg[1]<GYRO_MIN_OFFSET_VAL)
    {
        ...
        offset[2] = 0 - avg[2];
        //gyro_store_offset_in_file(LSM6DS3_GYRO_CALI_FILE, offset, 0);
        return snprintf(buf, PAGE_SIZE, "[Sensor] %s: Y axis out of range %d\n", __FUNCTION__, avg[1]);
    }
    else if (avg[2]>GYRO_MAX_OFFSET_VAL || avg[2]<GYRO_MIN_OFFSET_VAL)
    {
        ...
        offset[2] = 0 - avg[2];
        //gyro_store_offset_in_file(LSM6DS3_GYRO_CALI_FILE, offset, 0);
        return snprintf(buf, PAGE_SIZE, "[Sensor] %s: Z axis out of range %d\n", __FUNCTION__, avg[2]);
    }
    ...
    if(gyro_store_offset_in_file(LSM6DS3_GYRO_CALI_FILE, gyro_xyz_offset, 1))
        {
            return snprintf(buf, PAGE_SIZE, "[Gyro Sensor] set_gyro_cali ERROR %d, %d, %d\n",
        gyro_xyz_offset[0],
        gyro_xyz_offset[1],
        gyro_xyz_offset[2]);
        }

    return snprintf(buf, PAGE_SIZE, "[Gyro Sensor] set_gyro_cali PASS %d, %d, %d\n",
        gyro_xyz_offset[0],
        gyro_xyz_offset[1],
        gyro_xyz_offset[2]);
}
...
int read_gyro_current_reg(void)
{
    int err;
    int ret = -1;
    struct i2c_client *client = lsm6ds3_i2c_client;

    err = hwmsen_read_byte(client, LSM6DS3_CTRL1_XL, &gyro_current_reg[0]);
    if (err < 0)
        return ret;
    err = hwmsen_read_byte(client, LSM6DS3_CTRL2_G, &gyro_current_reg[1]);
    if (err < 0)
        return ret;
    err = hwmsen_read_byte(client, LSM6DS3_CTRL3_C, &gyro_current_reg[2]);
    if (err < 0)
    return ret;
    err = hwmsen_read_byte(client, LSM6DS3_CTRL4_C, &gyro_current_reg[3]);
    if (err < 0)
    return ret;
    err = hwmsen_read_byte(client, LSM6DS3_CTRL5_C, &gyro_current_reg[4]);
    if (err < 0)
        return ret;
    err = hwmsen_read_byte(client, LSM6DS3_CTRL6_G, &gyro_current_reg[5]);
    if (err < 0)
        return ret;
    err = hwmsen_read_byte(client, LSM6DS3_CTRL7_G, &gyro_current_reg[6]);
    if (err < 0)
        return ret;
    err = hwmsen_read_byte(client, LSM6DS3_CTRL8_XL, &gyro_current_reg[7]);
    if (err < 0)
        return ret;
    err = hwmsen_read_byte(client, LSM6DS3_CTRL9_XL, &gyro_current_reg[8]);
    if (err < 0)
        return ret;
    err = hwmsen_read_byte(client, LSM6DS3_CTRL10_C, &gyro_current_reg[9]);
    if (err < 0)
        return ret;
...
int write_gyro_current_reg(void)
{
    int err;
    int ret = -1;
    struct i2c_client *client = lsm6ds3_i2c_client;

    err = hwmsen_write_byte(client, LSM6DS3_CTRL1_XL, gyro_current_reg[0]);
    if (err < 0)
        return ret;
    err = hwmsen_write_byte(client, LSM6DS3_CTRL2_G, gyro_current_reg[1]);
    if (err < 0)
        return ret;
    err = hwmsen_write_byte(client, LSM6DS3_CTRL3_C, gyro_current_reg[2]);
    if (err < 0)
        return ret;
    err = hwmsen_write_byte(client, LSM6DS3_CTRL4_C, gyro_current_reg[3]);
    if (err < 0)
        return ret;
    err = hwmsen_write_byte(client, LSM6DS3_CTRL5_C, gyro_current_reg[4]);
    if (err < 0)
        return ret;
    err = hwmsen_write_byte(client, LSM6DS3_CTRL6_G, gyro_current_reg[5]);
    if (err < 0)
        return ret;
    err = hwmsen_write_byte(client, LSM6DS3_CTRL7_G, gyro_current_reg[6]);
    if (err < 0)
        return ret;
    err = hwmsen_write_byte(client, LSM6DS3_CTRL8_XL, gyro_current_reg[7]);
    if (err < 0)
        return ret;
    err = hwmsen_write_byte(client, LSM6DS3_CTRL9_XL, gyro_current_reg[8]);
    if (err < 0)
        return ret;
    err = hwmsen_write_byte(client, LSM6DS3_CTRL10_C, gyro_current_reg[9]);
    if (err < 0)
        return ret;
...
static ssize_t set_gyro_self_test(struct device_driver *ddri, char *buf)
{
    ...
    err = hwmsen_read_byte(client, LSM6DS3_OUTX_L_G, &data[0]);
    if (err < 0)
        goto lsm6ds3_set_gyro_self_test_mutex_unlock;
    err = hwmsen_read_byte(client, LSM6DS3_OUTX_H_G, &data[1]);
    if (err < 0)
        goto lsm6ds3_set_gyro_self_test_mutex_unlock;
    err = hwmsen_read_byte(client, LSM6DS3_OUTY_L_G, &data[2]);
    if (err < 0)
        goto lsm6ds3_set_gyro_self_test_mutex_unlock;
    err = hwmsen_read_byte(client, LSM6DS3_OUTY_H_G, &data[3]);
    if (err < 0)
        goto lsm6ds3_set_gyro_self_test_mutex_unlock;
    err = hwmsen_read_byte(client, LSM6DS3_OUTZ_L_G, &data[4]);
    if (err < 0)
        goto lsm6ds3_set_gyro_self_test_mutex_unlock;
    err = hwmsen_read_byte(client, LSM6DS3_OUTZ_H_G, &data[5]);
    if (err < 0)
        goto lsm6ds3_set_gyro_self_test_mutex_unlock;
    ...
    return snprintf(buf, PAGE_SIZE, "[Gyro Sensor] set_gyro_self_test PASS\n");
...
lsm6ds3_set_gyro_self_test_mutex_unlock:
    mutex_unlock(&bank_registers_lock);
    return snprintf(buf, PAGE_SIZE, "[Gyro Sensor] exit - Fail , err=%d\n", err);
}
...
static DRIVER_ATTR(gyrogetcali,           S_IRUGO, get_gyro_cali,         NULL);
static DRIVER_ATTR(gyrosetcali,           S_IRUGO, set_gyro_cali,         NULL);
static DRIVER_ATTR(gyrogetselftest,           S_IRUGO, get_gyro_self_test,         NULL);
static DRIVER_ATTR(gyrosetselftest,           S_IRUGO, set_gyro_self_test,         NULL);
...
static int lsm6ds3_gyro_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    obj->hw = get_gyro_dts_func("mediatek,lsm6ds3gy", hw);
    ...
    printk("lsm6ds3_gyro_i2c_probe idmedata %x, %x, %x, %x, %x, %x\n",
...
{
    struct gyro_hw *gy_hw = get_gyro_dts_func("mediatek,lsm6ds3gy", hw);
    GYRO_FUN();
...
static int __init lsm6ds3_gyro_init(void)
{
    /*GYRO_FUN();*/
       ...
       hw = get_gyro_dts_func("mediatek,lsm6ds3gy", hw);

        printk("lsm6ds3_gyro_init enter\n");
