...
#endif
#ifndef CONFIG_MTK_CLKMGR
struct clk *usb11_pll_clk;
struct clk *usb11_mcu_clk;
...
#else
	usb11_pll_clk = devm_clk_get(&pdev->dev, "usbpll");
#endif
	if (IS_ERR(usb11_pll_clk)) {
		WARNING("cannot get usbpll clock\n");
		return PTR_ERR(usb11_pll_clk);
	}

	retval = clk_prepare(usb11_pll_clk);
	if (retval != 0) {
		WARNING("prepare fail\n");
		return retval;
	}

#ifdef CONFIG_OF
	usb11_mcu_clk = of_clk_get_by_name(node, "usbmcu");
#else
	usb11_mcu_clk = devm_clk_get(&pdev->dev, "usbmcu");
#endif
	if (IS_ERR(usb11_mcu_clk)) {
		WARNING("cannot get usbmcu clock\n");
		return PTR_ERR(usb11_mcu_clk);
	}

	retval = clk_prepare(usb11_mcu_clk);
	if (retval != 0) {
		WARNING("prepare fail\n");
		return retval;
	}
#ifdef CONFIG_OF
	usb11_clk = of_clk_get_by_name(node, "icusb");
#else
	usb11_clk = devm_clk_get(&pdev->dev, "icusb");
#endif
	if (IS_ERR(usb11_clk)) {
		WARNING("cannot get icusb clock\n");
		return PTR_ERR(usb11_clk);
	}

	retval = clk_prepare(usb11_clk);
	if (retval != 0) {
		WARNING("prepare fail\n");
		return retval;
	}
...
static int musbfsh_resume(struct device *dev)
{
	...
	clk_prepare(usb11_pll_clk);
	clk_prepare(usb11_mcu_clk);
