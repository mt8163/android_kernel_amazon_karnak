...
static const kal_uint32
	btsco_PacketValidMask[BT_SCO_CVSD_MAX][BT_SCO_CVSD_MAX] = {
...
{20, 9, BT_SCO_PACKET_180 / SCO_TX_ENCODE_SIZE,
	BT_SCO_PACKET_180 / SCO_RX_PLC_SIZE}
};				/*20*/


static struct {
	struct BT_SCO_TX_T *pTX;
	struct BT_SCO_RX_T *pRX;
	kal_uint8 *pStructMemory;
	kal_uint8 *pWorkingMemory;
	kal_uint16 uAudId;
	enum CVSD_STATE  uTXState;
	enum CVSD_STATE  uRXState;
	bool fIsStructMemoryOnMED;
} btsco;

static struct TIME_BUFFER_INFO_T time_buffer_info_rx, time_buffer_info_tx;

static kal_uint32 *bt_hw_REG_PACKET_W,
	*bt_hw_REG_PACKET_R, *bt_hw_REG_CONTROL;
...
static kal_uint64 BT_RX_timestamp, BT_TX_timestamp,
	BT_RX_bufdata_equivalent_time, BT_TX_bufdata_equivalent_time;
...
static int Auddrv_BTCVSD_Address_Map(void)
{
	...
	ret = of_property_read_u32_array(node, "offset", offset,
			ARRAY_SIZE(offset));
	if (ret) {
		pr_debug("%s(), get offest fail, ret %d\n", __func__, ret);
		return ret;
	}
...
{
	kal_uint32 *INFRA_MISC_REGISTER =
		(kal_uint32 *)(INFRA_MISC_ADDRESS);
...
static int AudDrv_btcvsd_Allocate_Buffer(struct file *fp, kal_uint8 isRX)
{
	...
	if (isRX == 1) {
		readFromBT_cnt = 0;
		BT_CVSD_Mem.u4RXBufferSize = sizeof(struct BT_SCO_RX_T);
		...
		if (BT_CVSD_Mem.pucRXVirtBufAddr == NULL) {
			...
			if ((BT_CVSD_Mem.pucRXPhysBufAddr == 0)
				...
				pr_debug
				    ("btcvsd_Allocate_Buffer RX fail\n");
				return -1;
			}
		}
		...
		PRINTK_AUDDRV("BT_CVSD_Mem Allocate RX %p 0x%x\n",
		     BT_CVSD_Mem.pucRXVirtBufAddr,
		     BT_CVSD_Mem.pucRXPhysBufAddr);

		btsco.pRX =
			(struct BT_SCO_RX_T *) (BT_CVSD_Mem.pucRXVirtBufAddr);
		btsco.pRX->u4BufferSize = SCO_RX_PACKER_BUF_NUM *
			(SCO_RX_PLC_SIZE + BTSCO_CVSD_PACKET_VALID_SIZE);
	} else {
		writeToBT_cnt = 0;
		BT_CVSD_Mem.u4TXBufferSize = sizeof(struct BT_SCO_TX_T);
		...
		if (BT_CVSD_Mem.pucTXVirtBufAddr == NULL) {
			...
			if ((BT_CVSD_Mem.pucTXPhysBufAddr == 0)
				...
				pr_debug
				    ("btcvsd_Allocate_Buffer TX fail\n");
				return -1;
			}
		}
		memset((void *)BT_CVSD_Mem.pucTXVirtBufAddr,
			0, BT_CVSD_Mem.u4TXBufferSize);

		PRINTK_AUDDRV("BT_CVSD_Mem Allocate TX 0x%p 0x%x\n",
		     BT_CVSD_Mem.pucTXVirtBufAddr,
		     BT_CVSD_Mem.pucTXPhysBufAddr);

		btsco.pTX =
			(struct BT_SCO_TX_T *) (BT_CVSD_Mem.pucTXVirtBufAddr);
...
static int AudDrv_btcvsd_Free_Buffer(struct file *fp, kal_uint8 isRX)
{
	...
	if (isRX == 1) {
		if (BT_CVSD_Mem.pucRXVirtBufAddr != NULL) {
			PRINTK_AUDDRV("dma_free_coherent %p,%x",
			     BT_CVSD_Mem.pucRXVirtBufAddr,
			     BT_CVSD_Mem.pucRXPhysBufAddr);
			btsco.pRX = NULL;
			...
			PRINTK_AUDDRV("rx can't dma_free_coherent %p,0x%x\n",
				BT_CVSD_Mem.pucRXVirtBufAddr,
				BT_CVSD_Mem.pucRXPhysBufAddr);
			return -1;
		}
		...
		if (BT_CVSD_Mem.pucTXVirtBufAddr != NULL) {
			...
			PRINTK_AUDDRV("tx cannot dma_free_coherent %p,0x%x\n",
				BT_CVSD_Mem.pucTXVirtBufAddr,
				BT_CVSD_Mem.pucTXPhysBufAddr);
			return -1;
		}
 ...
 *****************************************************************************
 */
static long AudDrv_btcvsd_ioctl(struct file *fp,
	unsigned int cmd, unsigned long arg)
{
	...
	case SET_BTCVSD_STATE:{
			...
			if (arg == BT_SCO_TXSTATE_DIRECT_LOOPBACK) {
				...
				btsco.uTXState = arg;
				pr_debug("set btsco.uTXState to %lu\n", arg);
			} else {	/*RX state*/
				btsco.uRXState = arg;
				pr_debug("set btsco.uRXState to %lu\n", arg);
			}
	...
	case GET_BTCVSD_RX_TIME_BUFFER_INFO:{
		...
		time_buffer_info_rx.uDataCountEquiTime =
			BT_RX_bufdata_equivalent_time;
		PRINTK_AUDDRV("GET_BTCVSD_RX_TIMESTAMP %llu,%llu",
			time_buffer_info_rx.uTimestampUS,
			time_buffer_info_rx.uDataCountEquiTime);

		if (copy_to_user((void __user *)arg, &time_buffer_info_rx,
			sizeof(struct TIME_BUFFER_INFO_T))) {
			pr_warn("GET_BTCVSD_RX_TIMESTAMP Fail copy to user Ptr:%p,r_sz:%zu",
				(kal_uint8 *)&time_buffer_info_rx,
				sizeof(struct TIME_BUFFER_INFO_T));
			ret = -1;
		}
			break;
	...
	case GET_BTCVSD_TX_TIME_BUFFER_INFO:{
		...
		time_buffer_info_tx.uDataCountEquiTime =
			BT_TX_bufdata_equivalent_time;
		PRINTK_AUDDRV("GET_BTCVSD_TX_TIMESTAMP %llu,%llu",
			time_buffer_info_tx.uTimestampUS,
			time_buffer_info_tx.uDataCountEquiTime);
...
#ifdef CONFIG_COMPAT
static long AudDrv_btcvsd_compat_ioctl(struct file *fp,
	unsigned int cmd, unsigned long arg)
{
	long ret;
...
static void AudDrv_BTCVSD_DataTransfer(enum BT_SCO_DIRECT uDir, kal_uint8 *pSrc,
	kal_uint8 *pDst, kal_uint32 uBlockSize,
	kal_uint32 uBlockNum, enum CVSD_STATE  uState)
{
	kal_int32 i, j;
	...
	kal_uint32 uBlockSize, kal_uint32 uControl)
{
	...
	PRINTK_AUDDRV
		("AudDrv_BTCVSD_ReadFromBT connsys_addr=0x%lx, 0x%lx\n",
		connsys_addr_rx, ap_addr_rx);
	pSrc = (kal_uint8 *) ap_addr_rx;

	PRINTK_AUDDRV("AudDrv_BTCVSD_ReadFromBT %d, %d, %d\n",
	     uPacketLength, uPacketNumber, btsco.uRXState);
	AudDrv_BTCVSD_DataTransfer(BT_SCO_DIRECT_BT2ARM, pSrc,
		btsco.pRX->TempPacketBuf,
		uPacketLength, uPacketNumber, btsco.uRXState);
	PRINTK_AUDDRV("AudDrv_BTCVSD_ReadFromBT DONE! 0x%x,%d\n",
	     uControl, uLen);
	...
	for (i = 0; i < uBlockSize; i++) {
		...
		pPacketBuf = (kal_uint8 *) btsco.pRX->PacketBuf +
		    ...
		    (SCO_RX_PLC_SIZE + BTSCO_CVSD_PACKET_VALID_SIZE) +
		    SCO_RX_PLC_SIZE;
		memcpy((void *)pPacketBuf,
			(void *)&pv, BTSCO_CVSD_PACKET_VALID_SIZE);
		btsco.pRX->iPacket_w++;
	}
...
static void AudDrv_BTCVSD_WriteToBT(enum BT_SCO_PACKET_LEN uLen,
	kal_uint32 uPacketLength, kal_uint32 uPacketNumber,
	kal_uint32 uBlockSize)
{
	...
	if (btsco.pTX != NULL) {
		for (i = 0; i < uBlockSize; i++) {
			memcpy((void *)(btsco.pTX->TempPacketBuf +
			(SCO_TX_ENCODE_SIZE * i)),
			(void *)(btsco.pTX->PacketBuf[btsco.pTX->iPacket_r &
			SCO_TX_PACKET_MASK]),
			SCO_TX_ENCODE_SIZE);
			btsco.pTX->iPacket_r++;
		}
...
static int AudDrv_BTCVSD_IRQ_handler(void)
{
	...
	if ((btsco.uRXState != BT_SCO_RXSTATE_RUNNING
		...
		pr_debug
		    ("AudDrv_BTCVSD_IRQ_handler in idle state %d,%d\n",
	...
	PRINTK_AUDDRV
		("AudDrv_BTCVSD_IRQ_handler BT uControl =0x%x, %d\n",
	...
	if (btsco.pTX && btsco.uTXState == BT_SCO_TXSTATE_DIRECT_LOOPBACK) {
		...
		ap_addr_tx = (unsigned long)BTSYS_SRAM_BANK2_BASE_ADDRESS +
		    (connsys_addr_tx & 0xFFFF);
		PRINTK_AUDDRV
			("AudDrv_BTCVSD_WriteToBT connsys_addr=0x%lx,0x%lx\n",
			connsys_addr_tx, ap_addr_tx);
		pDst = (kal_uint8 *) ap_addr_tx;
		...
		if (btsco.pRX) {
			if (btsco.uRXState == BT_SCO_RXSTATE_RUNNING
				...
				if (!btsco.pRX->fOverflow &&
					...
					(SCO_RX_PACKER_BUF_NUM -
					uBufferCount_RX))) {
					AudDrv_BTCVSD_ReadFromBT(uPacketType,
						uPacketLength,
						uPacketNumber, uBufferCount_RX,
						uControl);
					readFromBT_cnt++;
				} else {
					btsco.pRX->fOverflow = true;
					pr_debug
					    ("Overflow 0!\n");
				}
			}
		...
		if (btsco.pTX) {
			if (btsco.uTXState == BT_SCO_TXSTATE_RUNNING
				...
				if ((!btsco.pTX->fUnderflow
				    ...
				    ||
				    btsco.uTXState == BT_SCO_TXSTATE_ENDING) {
					AudDrv_BTCVSD_WriteToBT(uPacketType,
						uPacketLength,
						uPacketNumber, uBufferCount_TX);
					writeToBT_cnt++;
				} else {
					btsco.pTX->fUnderflow = true;
					pr_debug
					    ("Underflow 1!\n");
				}
			}
...
static int AudDrv_btcvsd_probe(struct platform_device *dev)
{
...
#endif
	ret =
		request_irq(btcvsd_irq_number,
			(irq_handler_t) AudDrv_BTCVSD_IRQ_handler,
			IRQF_TRIGGER_LOW /*IRQF_TRIGGER_FALLING */,
			"BTCVSD_ISR_Handle", dev);
	...
	/* init */
	memset((void *)&BT_CVSD_Mem, 0, sizeof(struct CVSD_MEMBLOCK_T));
...
#else
	BTSYS_PKV_BASE_ADDRESS =
		ioremap_nocache(AUDIO_BTSYS_PKV_PHYSICAL_BASE, 0x10000);
	BTSYS_SRAM_BANK2_BASE_ADDRESS =
	    ioremap_nocache(AUDIO_BTSYS_SRAM_BANK2_PHYSICAL_BASE, 0x10000);
	bt_hw_REG_PACKET_R =
	    (kal_uint32 *)(BTSYS_PKV_BASE_ADDRESS
	    + CVSD_MCU_READ_OFFSET);
	bt_hw_REG_PACKET_W =
	    (kal_uint32 *)(BTSYS_PKV_BASE_ADDRESS
	    + CVSD_MCU_WRITE_OFFSET);
...
static int AudDrv_btcvsd_open(struct inode *inode, struct file *fp)
{
	PRINTK_AUDDRV(ANDROID_LOG_INFO, "Sound",
		"btcvsd_open do nothing inode:%p, file:%pss\n",
		inode, fp);
	return 0;
}
	...
	while (count) {
		...
		if (copy_size != 0) {
			...
			if (BTSCOTX_WriteIdx + copy_size <
				...
				if (!access_ok(VERIFY_READ,
					...
					pr_debug
					    ("btcvsd_write 0ptr %lx, %d\n",
					    (unsigned long)data_w_ptr,
					    copy_size);
					pr_debug
					    ("btcvsd_write Size=%d, %d\n",
					     btsco.pTX->u4BufferSize,
					     BTSCOTX_WriteIdx);
				} else {
					if (copy_from_user
					...
					copy_size)) {
						pr_debug("btcvsd_write Fail!\n");
						return -1;
					}
				}

				spin_lock_irqsave(&auddrv_BTCVSDTX_lock, flags);
				btsco.pTX->iPacket_w += copy_size /
					SCO_TX_ENCODE_SIZE;
				spin_unlock_irqrestore(&auddrv_BTCVSDTX_lock,
					flags);
				data_w_ptr += copy_size;
				count -= copy_size;
			} else {
				...
				if (!access_ok(VERIFY_READ,
					data_w_ptr, size_1)) {
					pr_debug
					    ("btcvsd_write 1ptr ptr=%lx, %d\n",
					    (unsigned long)data_w_ptr, size_1);
					pr_debug
					    ("btcvsd_write BufSize=%d, %d\n",
					     btsco.pTX->u4BufferSize,
					     BTSCOTX_WriteIdx);
				} else {
					if ((copy_from_user
		...
		if (count != 0) {
			...
			if (ret < 0) {
				...
				pr_debug("%s(), after timeout\n", __func__);
			} else {
				pr_debug("%s(), before timeout\n", __func__);
			}
		}
		/* here need to wait for interrupt handler */
	}
	PRINTK_AUDDRV("btcvsd_write wsize = %d, %llu\n",
				  ...
				  size_t count, loff_t *offset)
{
	...
	int max_timeout_trial = 2;
	unsigned int packet_size = SCO_RX_PLC_SIZE +
	...
	read_timeout_limit = ((kal_uint64) SCO_RX_PACKER_BUF_NUM *
		SCO_RX_PLC_SIZE * 16 * 1000000000) / 2 / 2 / 64000;

	while (count) {
		PRINTK_AUDDRV("btcvsd_read %d, %d,count %zu\n",
		...
		PRINTK_AUDDRV("btcvsd_read read_size=%zu, %zu\n",
		...
		if (BTSCORX_ReadIdx_tmp + read_size < btsco.pRX->u4BufferSize) {
			...
			if (copy_to_user((void __user *)Read_Data_Ptr,
			     ...
			     BTSCORX_ReadIdx_tmp),
			     read_size)) {
				pr_debug("btcvsd_read Fail 1 copy to user %p, %p, %zu, %zu",
				     Read_Data_Ptr,
			...
			count -= read_size;
			PRINTK_AUDDRV
				("btcvsd_read finish1, %zu, 0x%x, %x, %zu\r\n",
			    read_size, btsco.pRX->iPacket_r,
		...
		/* copy twice */
			...
			PRINTK_AUDDRV
				("btcvsd_read 2-2 copy target=%p, size=%zu\n",
				Read_Data_Ptr,
				size_1);
			if (copy_to_user
			   ...
			   (void *)((kal_uint8 *) btsco.pRX->PacketBuf +
			    BTSCORX_ReadIdx_tmp),
			    size_1)) {
				pr_debug("AudDrv_btcvsd_read Fail 2 copy to user R_Ptr:%p, PacketBuf:%p,RIdx_tmp:%zu, r_sz:%zu",
				     Read_Data_Ptr, btsco.pRX->PacketBuf,
				     BTSCORX_ReadIdx_tmp, read_size);
				if (read_count == 0)
					return -1;
				else
		...
		if (count != 0) {
			...
			PRINTK_AUDDRV
				("btcvsd_read WAITING.0x%x, count=%zu\n",
			...
			t2 = sched_clock();
			PRINTK_AUDDRV("btcvsd_read WAKEUP...count=%zu\n",
				count);
			t2 = t2 - t1;	/* in ns (10^9) */
	...
	BT_RX_bufdata_equivalent_time = ((kal_uint64)(btsco.pRX->iPacket_w -
		btsco.pRX->iPacket_r))
	...
	/* return equivalent time(us) to data count */
	BT_RX_bufdata_equivalent_time *= 1000;
	PRINTK_AUDDRV
		("%llu, %llu, iPacket_w:%d, iPacket_r:%d ",
		BT_RX_timestamp, BT_RX_bufdata_equivalent_time,
		btsco.pRX->iPacket_w, btsco.pRX->iPacket_r);
	PRINTK_AUDDRV("btcvsd_read read_count = %zu,read_timeout_limit=%llu\n",
