...
static inline void isp_wr32_mask(void *reg, u32 mask, u32 shift, u32 value)
{
	u32 val;

	pr_err("Joel: writing reg=%p mask=%x shift=%x value=%x\n", reg, mask,
	       shift, value);
...
static signed int EDBufQueRemainNodeCnt; /* record remain node
						     count(success/fail)
						     excludes head when
						     enque/deque control */
...
struct ISP_EDBUF_STRUCT {
	...
	signed int p2dupCQIdx;  /* p2 duplicate CQ index(for recognize
...
static struct ISP_EDBUF_STRUCT
	P2_EDBUF_RingList[_MAX_SUPPORT_P2_FRAME_NUM_];
...
struct ISP_EDBUF_MGR_STRUCT {
	...
	signed int p2dupCQIdx;  /* p2 duplicate CQ index(for recognize
					    belong to which package) */
	signed int dequedNum;   /* number of dequed buffer no matter
					    deque success or fail */
};
...
/* m4u_callback_ret_t ISP_M4U_TranslationFault_callback(int port, unsigned int
 * mva, void* data); */
...
/* static ISP_DEQUE_BUF_INFO_STRUCT g_deque_buf = {0,{}};    // Marked to remove
 * build warning. */
...
*******************************************************************************/
/*
struct ISP_HOLD_INFO_STRUCT_FRMB
{
    atomic_t            HoldEnable;
    atomic_t            WriteEnable;
    enum ISP_HOLD_TIME_ENUM_FRMB  Time;
};

struct ISP_TIME_LOG_STRUCT_FRMB
{
    unsigned int     Vd;
...
enum eLOG_TYPE {
	_LOG_DBG = 0, /* currently, only used at ipl_buf_ctrl. to protect
			 critical section */
	_LOG_INF = 1,
	_LOG_ERR = 2,
	_LOG_MAX = 3,
...
static unsigned int g_ISPIntErr[_IRQ_MAX] = {0}; /* remove volatile */
...
struct UserKeyInfo {
	char userName[USERKEY_STR_LEN]; /* name for the user that register a
					   userKey */
	int userKey;			/* the user key for that user */
};
static struct UserKeyInfo IrqUserKey_UserInfo[IRQ_USER_NUM_MAX]; /* array for
								    recording
								    the user
								    name for a
								    specific
...
struct ISP_IRQ_INFO_STRUCT_FRMB {
	unsigned int Status[IRQ_USER_NUM_MAX]
				    [ISP_IRQ_TYPE_AMOUNT]; /* interrupt status
							      for each user in
							      irqType/irqBit */
	...
	unsigned int MarkedFlag[IRQ_USER_NUM_MAX]
					[ISP_IRQ_TYPE_AMOUNT]; /* flag for
								  ...
								  not */
	unsigned int
		MarkedTime_sec[IRQ_USER_NUM_MAX][ISP_IRQ_TYPE_AMOUNT]
			      [32]; /* time for marking a specific interrupt */
	unsigned int
		MarkedTime_usec[IRQ_USER_NUM_MAX][ISP_IRQ_TYPE_AMOUNT]
			       [32]; /* time for marking a specific interrupt */
	signed int PassedBySigCnt
		[IRQ_USER_NUM_MAX][ISP_IRQ_TYPE_AMOUNT]
		[32]; /* number of a specific signal that passed by */
	unsigned int LastestSigTime_sec[ISP_IRQ_TYPE_AMOUNT]
				       [32]; /* latest occuring time for each
						interrupt, remove volatile */
	unsigned int LastestSigTime_usec[ISP_IRQ_TYPE_AMOUNT]
					[32]; /* latest occuring time for each
						 interrupt, remove volatile */
	struct ISP_EIS_META_STRUCT Eismeta[ISP_IRQ_TYPE_INTB]
					  [EISMETA_RINGSIZE]; /* eis meta only
								 for p1 and
								 p1_d, remove
								 volatile */
};
...
struct ISP_INFO_STRUCT_FRMB {
	spinlock_t
		SpinLockIrq
			[_IRQ_MAX]; /* currently, IRQ and IRQ_D share the same
				       ISR , so share the same key,IRQ. */
...
struct T_STAMP {
	unsigned long long
		T_ns; /* 1st frame start time, accurency in us,unit in ns */
...
*
*******************************************************************************/
static inline unsigned int ISP_GetIRQState_FrmB(enum eISPIrq eIrq,
						unsigned int type,
...
/*  */
static signed int
ISP_RTBC_ENQUE_FRMB(signed int dma,
		    struct ISP_RT_BUF_INFO_STRUCT_FRMB *prt_buf_info)
{
	...
	if (buffer_exist) {
		...
		if (pstRTBuf_FrmB->ring_buf[rt_dma].total_count ==
		    ISP_RT_BUF_SIZE) {
			LOG_ERR("[ENQUE]:[rtbc]:buffer full(%d)",
				pstRTBuf_FrmB->ring_buf[rt_dma].total_count);
		} else {
			/* first time add */
			index = pstRTBuf_FrmB->ring_buf[rt_dma].total_count %
				ISP_RT_BUF_SIZE;
			/*  */
			pstRTBuf_FrmB->ring_buf[rt_dma].data[index].memID =
				prt_buf_info->memID;
			pstRTBuf_FrmB->ring_buf[rt_dma].data[index].size =
				prt_buf_info->size;
			...
			pstRTBuf_FrmB->ring_buf[rt_dma].data[index].bFilled =
				ISP_RTBC_BUF_EMPTY;
			pstRTBuf_FrmB->ring_buf[rt_dma].data[index].bufIdx =
				prt_buf_info->bufIdx;
			/*  */
			pstRTBuf_FrmB->ring_buf[rt_dma].total_count++;
			pstRTBuf_FrmB->ring_buf[rt_dma].empty_count++;
	...
	if (pstRTBuf_FrmB->ring_buf[rt_dma].empty_count == 1) {
		...
		if (IspInfo_FrmB.DebugMask & ISP_DBG_BUF_CTRL) {
			LOG_DBG("[rtbc][ENQUE]:dma(%d),base_pAddr(0x%x)/imgo(0x%x)/img2o(0x%x)empty_count(%d) ",
				rt_dma, pstRTBuf_FrmB->ring_buf[rt_dma]
						.data[index]
						.base_pAddr,
...
static void ISP_FBC_DUMP(unsigned int dma_id, unsigned int VF_1,
			 unsigned int VF_2, unsigned int VF_3,
			 unsigned int VF_4)
{
	unsigned int z;
	char str[128];
...
static signed int
ISP_RTBC_DEQUE_FRMB(signed int dma,
		    struct ISP_DEQUE_BUF_INFO_STRUCT_FRMB *pdeque_buf)
{
	...
	for (i = 0; i < pstRTBuf_FrmB->ring_buf[rt_dma].total_count; i++) {
		/*  */
		index = (pstRTBuf_FrmB->ring_buf[rt_dma].start + i) %
			pstRTBuf_FrmB->ring_buf[rt_dma].total_count;
		/*  */
	...
	if (pdeque_buf->count == 0) {
		...
		/*  */
		for (i = 0;
		     i <= pstRTBuf_FrmB->ring_buf[rt_dma].total_count - 1;
		     i++) {
...
/* do nothing */
#else /* for camsv */
			/* if ( ( _camsv_imgo_  == rt_dma) || (_camsv2_imgo_ ==
			 * rt_dma) ) */
			/* _bFlag = MTRUE; */
			/* else */
			_bFlag = MFALSE;
			...
			if (_bFlag == MTRUE) {
				if ((ISP_RT_BUF_CTRL_ENQUE_FRMB ==
					...
					p1_fbc[_img2o_].Reg_val = ISP_RD32(
				...
				if (CurVF_En) {
...
#endif
						/* copy_from/to_user might sleep
						 * when page-fault, can't call
						 * in atomic contex */
						/* spin_lock_irqsave(&(IspInfo_FrmB.SpinLockIrq[irqT_Lock]),
						 * flags); */
						/**/ if (0 !=
							 rt_buf_ctrl
								 .ex_data_ptr) {
					...
					}
					{
						/*  */
						{
							 ...
							 * PA(0x%x)
							 * VA(0x%x)",rt_dma,i,pstRTBuf_FrmB->ring_buf[rt_dma].data[i].base_pAddr,pstRTBuf_FrmB->ring_buf[rt_dma].data[i].base_vAddr);
							 */
							IRQ_LOG_KEEPER(
								irqT, 0,
								_LOG_DBG,
								...
								deque_buf
									.data[0]
									.base_pAddr);
							/* spin_lock_irqsave(&(IspInfo_FrmB.SpinLockIrq[irqT]),
							 * flags); */
							pstRTBuf_FrmB
								->ring_buf
									[rt_dma]
							...
							/**/ else {
								...
								IRQ_LOG_PRINTER(
									irqT, 0,
									_LOG_DBG);
								LOG_ERR("[rtbc]dma(%d),PA(0x%x),over enque",
									rt_dma,
									pstRTBuf_FrmB
										->ring_buf
											[rt_dma]
...
/* LOG_INF("RTBC_DBG7 e_dma_%d:%d %d
 ...
 */
					...
					/**/		} else {
						...
						/* spin_unlock_irqrestore(&(IspInfo_FrmB.SpinLockIrq[irqT_Lock]),
						 * flags); */
						return -EFAULT;
					}
							...
							if (pstRTBuf_FrmB
								...
								/**/ else {
									...
									IRQ_LOG_PRINTER(
										...
										0,
										_LOG_DBG);
									LOG_ERR("[rtbc]error:dma(%d),PA(0x%x),over enque",
										rt_dma,
										pstRTBuf_FrmB
											->ring_buf
												[rt_dma]
								...
								pstRTBuf_FrmB
									...
									.empty_count++;
#endif
								/* spin_unlock_irqrestore(&(IspInfo_FrmB.SpinLockIrq[irqT]),
								 * flags); */
					...
					/**/				} else {
						/* rcnt_sync only work when
						 * multi-dma ch enabled. but in
						 * order to support multi-enque,
						 * these mech. also to be */
					...
					/**/					} else {
						spin_unlock_irqrestore(
							&(IspInfo_FrmB.SpinLockIrq
								  [irqT_Lock]),
							flags);
						LOG_ERR("[rtbc]error:dma(%d) r not being activated(%d)",
							rt_dma,
							pstRTBuf_FrmB
								->ring_buf
									[rt_dma]
						...
						if ((MTRUE ==
						    ...
						    (MTRUE ==
						     pstRTBuf_FrmB
							     ...
							     .active)) {
							/* record wheather all
							 * enabled dma r alredy
							 * enqued, */
							/* rcnt_inc will only be
							 * pulled to high once
							 * all enabled dma r
							 * enqued. */
					...
					/**/				} else
						...
						/**/ if (_bypass == MFALSE) {
							...
							if ((p1_fbc[rt_dma]
								 ...
								 * write2Phy
								 * directly(%d,%d)",rt_dma,p1_fbc[rt_dma].Bits.FB_NUM,p1_fbc[rt_dma].Bits.FBC_CNT);
								 */
								IRQ_LOG_KEEPER(
									irqT, 0,
									_LOG_DBG,
							...
							/**/ if (_openedDma ==
								...
								IRQ_LOG_KEEPER(
									...
									rt_dma);
					/**/				} else {
						p1_fbc[ch_imgo].Bits.RCNT_INC =
							1;
						ISP_WR32(p1_fbc_reg[ch_imgo],
							 p1_fbc[ch_imgo]
								 .Reg_val);
						p1_fbc[ch_img2o].Bits.RCNT_INC =
							1;
						ISP_WR32(p1_fbc_reg[ch_img2o],
							 p1_fbc[ch_img2o]
								 .Reg_val);
						IRQ_LOG_KEEPER(
							irqT, 0, _LOG_DBG,
...
/* } */
					...
					/**/ if (IspInfo_FrmB.DebugMask &
						 ...
						 */
						LOG_DBG("[rtbc][ENQUE]:dma(%d),PA(0x%x),O(0x%x),2O(0x%x),fps(%d/%d/%d/%d)us,IMD_%d\n",
							rt_dma,
							rt_buf_info.base_pAddr,
							ISP_RD32(
								ISP_REG_ADDR_IMGO_BASE_ADDR),
		...
		case ISP_RT_BUF_CTRL_DEQUE_FRMB:
			...
			/* VF start already */
			spin_lock_irqsave(
				&(IspInfo_FrmB.SpinLockIrq[irqT_Lock]), flags);
			if (reg_val & 0x01) {
				if (_bFlag == MTRUE) {
					deque_buf.count = P1_DEQUE_CNT;
#ifdef _rtbc_buf_que_2_0_
					iBuf = pstRTBuf_FrmB->ring_buf[rt_dma]
						       .read_idx; /* p1_fbc[rt_dma].Bits.WCNT
								     - 1;
								     //WCNT =
								     [1,2,..] */
					pstRTBuf_FrmB->ring_buf[rt_dma]
					...
					/**/ if (IspInfo_FrmB.DebugMask &
						 ISP_DBG_BUF_CTRL) {
						LOG_DBG("[rtbc][DEQUE](%d):d(%d)/id(0x%x)/bs(0x%x)/va(0x%llx)/pa(0x%x)/t(%d.%d)/img(%d,%d,%d,%d,%d,%d,%d,%d)/m(0x%x)/fc(%d)/hrz(%d,%d,%d,%d,%d,%d),dmao(%d,%d,%d,%d),lm#(0x%x)",
							iBuf + i, rt_dma,
							deque_buf.data[i].memID,
							deque_buf.data[i].size,
							deque_buf.data[i]
...
/*  */
						...
						/*  */
						/* tstamp =
						 * deque_buf.data[i].timeStampS*1000000+deque_buf.data[i].timeStampUs;
						 */
						/* if ( (0 != prv_tstamp) &&
						 * (prv_tstamp >= tstamp) ) { */
						/**/ if (0 !=
		...
		case ISP_RT_BUF_CTRL_IS_RDY_FRMB:
			...
			if (IspInfo_FrmB.DebugMask & ISP_DBG_BUF_CTRL) {
				...
				IRQ_LOG_KEEPER(
					...
					pstRTBuf_FrmB->ring_buf[rt_dma]
						.total_count,
					bWaitBufRdy);
				for (z = 0; z < pstRTBuf_FrmB->ring_buf[rt_dma]
							.total_count;
				     z++) {
					IRQ_LOG_KEEPER(
						irqT, 0, _LOG_DBG, "%d_",
			...
			/*  */
			/* spin_unlock_irqrestore(&(IspInfo_FrmB.SpinLockRTBC),
			 * flags); */
			/*  */
			break;
		case ISP_RT_BUF_CTRL_GET_SIZE_FRMB:
			 ...
			 */
			/* } */
			/* if(copy_to_user((void*)rt_buf_ctrl.data_ptr, &size,
			 * sizeof(unsigned int)) != 0) */
			if (copy_to_user((void __user *)rt_buf_ctrl.pExtend,
		...
		case ISP_RT_BUF_CTRL_CLEAR_FRMB:
			...
			case _img2o_:
				...
				memset(IspInfo_FrmB.IrqInfo.LastestSigTime_usec
					       [ISP_IRQ_TYPE_INT],
				       0, sizeof(unsigned int) * 32);
				memset(IspInfo_FrmB.IrqInfo.LastestSigTime_sec
					       [ISP_IRQ_TYPE_INT],
				       0, sizeof(unsigned int) * 32);
				memset(IspInfo_FrmB.IrqInfo
					       .Eismeta[ISP_IRQ_TYPE_INT],
				       0, sizeof(struct ISP_EIS_META_STRUCT) *
						  EISMETA_RINGSIZE);
				gEismetaRIdx = 0;
				gEismetaWIdx = 0;
				gEismetaInSOF = 0;
...
/* remove, cause clear will be involked only when current module r totally
 * stopped */
/* spin_lock_irqsave(&(IspInfo_FrmB.SpinLockIrq[irqT_Lock]), flags); */
...

			{
				...
				if ((pstRTBuf_FrmB->ring_buf[_imgo_].active ==
				     MFALSE) &&
				    (pstRTBuf_FrmB->ring_buf[_img2o_].active ==
				     MFALSE))
					out[0] = _IRQ;
		...
		case ISP_RT_BUF_CTRL_DMA_EN_FRMB: {
			...
			if (copy_from_user(array,
				...
				for (z = 0; z < _rt_dma_max_; z++) {
					pstRTBuf_FrmB->ring_buf[z].active =
						array[z];
					/**/ if (IspInfo_FrmB.DebugMask &
						 ISP_DBG_BUF_CTRL) {
						LOG_INF("[rtbc][DMA_EN]:dma_%d:%d",
							z, array[z]);
					}
				}
			} else {
...
#endif
		case ISP_RT_BUF_CTRL_MAX_FRMB: /* Add this to remove build
						  warning. */
			/* Do nothing. */
			break;
		default:
				       ...
				       unsigned int *pCurr_pa)
{
	...
	if (g1stSof[irqT] == MTRUE) { /* 1st frame of streaming */
		...
		pstRTBuf_FrmB->ring_buf[ch_imgo].start =
			((imgo_fbc.Bits.FBC_CNT + m_P1_RCNT_INC_CNT) % 3 + 1) -
			1;
		pstRTBuf_FrmB->ring_buf[ch_img2o].start =
			((img2o_fbc.Bits.FBC_CNT + m_P1_RCNT_INC_CNT) % 3 + 1) -
			1;
#else
...
/*  */
	...
	if (pstRTBuf_FrmB->dropCnt == 0) {
		...
		/* { */
		/* IRQ_LOG_KEEPER(irqT,m_CurrentPPB,_LOG_INF,"PrvAddr:Last(0x%x)
		 * == Cur(0x%x)\n",PrvAddr[i],curr_pa); */
		/* ISP_DumpReg(); */
		/* } */
		PrvAddr[i] = curr_pa;
		...
		if (curr_pa !=
			...
			for (i = 0;
				...
				if (curr_pa ==
					 ...
					 * case also should not be happened */
					/* imgo_idx  = i; */
					/* rrzo_idx = i; */
		 ...
		 */
		/*  */
		pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].timeStampS =
			sec;
		pstRTBuf_FrmB->ring_buf[ch_imgo].data[imgo_idx].timeStampUs =
			usec;
		pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].timeStampS =
			sec;
		pstRTBuf_FrmB->ring_buf[ch_img2o].data[img2o_idx].timeStampUs =
			usec;
		if (IspInfo_FrmB.DebugMask & ISP_DBG_INT_3) {
			static unsigned int m_sec = 0, m_usec;
			unsigned int _tmp = pstRTBuf_FrmB->ring_buf[ch_imgo]
							    .data[imgo_idx]
							    .timeStampS *
						    1000000 +
		...
		if (irqT == _IRQ) {
			...
			pstRTBuf_FrmB->ring_buf[ch_imgo]
				.data[imgo_idx]
				.image.fmt =
				(ISP_RD32(ISP_REG_ADDR_FMT_SEL_P1) & 0xF000) >>
				12;
			pstRTBuf_FrmB->ring_buf[ch_imgo]
				.data[imgo_idx]
				.image.pxl_id =
			...
			pstRTBuf_FrmB->ring_buf[ch_imgo]
				.data[imgo_idx]
				.dmaoCrop.y =
				(ISP_RD32(ISP_REG_ADDR_IMGO_CROP) >> 16) &
				0x1fff;
			pstRTBuf_FrmB->ring_buf[ch_imgo]
				.data[imgo_idx]
				.dmaoCrop.w =
			...
			pstRTBuf_FrmB->ring_buf[ch_imgo]
				.data[imgo_idx]
				.dmaoCrop.h =
				(ISP_RD32(ISP_REG_ADDR_IMGO_YSIZE) & 0x1FFF) +
				1;
			...
			pstRTBuf_FrmB->ring_buf[ch_img2o]
				.data[img2o_idx]
				.image.w = pstRTBuf_FrmB->ring_buf[ch_imgo]
						   .data[imgo_idx]
						   .image.w;
			pstRTBuf_FrmB->ring_buf[ch_img2o]
				.data[img2o_idx]
				.image.h =
				(ISP_RD32(ISP_REG_ADDR_IMG2O_YSIZE) & 0x1FFF) +
				1;
			pstRTBuf_FrmB->ring_buf[ch_img2o]
				.data[img2o_idx]
				.image.stride =
			...
			pstRTBuf_FrmB->ring_buf[ch_img2o]
				.data[img2o_idx]
				.image.pxl_id = pstRTBuf_FrmB->ring_buf[ch_imgo]
							.data[imgo_idx]
							.image.pxl_id;
			pstRTBuf_FrmB->ring_buf[ch_img2o]
				.data[img2o_idx]
			...
			pstRTBuf_FrmB->ring_buf[ch_img2o]
				.data[img2o_idx]
				.dmaoCrop.y =
				(ISP_RD32(ISP_REG_ADDR_IMG2O_CROP) >> 16) &
				0x1fff;
			pstRTBuf_FrmB->ring_buf[ch_img2o]
				.data[img2o_idx]
				.dmaoCrop.w =
					 ...
					 union CQ_RTBC_FBC *pFbc)
{
	...
	if (_dma_cur_fbc.Bits.WCNT > 0) {
		unsigned int _sw_wcnt =
			((_dma_cur_fbc.Bits.FBC_CNT + m_P1_RCNT_INC_CNT) % 3 +
			 1);
		...
		if (_sw_wcnt >
			...
			if (shiftT > 0)
			   ...
			   (pstRTBuf_FrmB->ring_buf[_working_dma].start + 2)) {
			shiftT = _sw_wcnt + _dma_cur_fbc.Bits.FB_NUM -
				 (pstRTBuf_FrmB->ring_buf[_working_dma].start +
				  2);
			if (IspInfo_FrmB.DebugMask & ISP_DBG_INT_2) {
				IRQ_LOG_KEEPER(
...
#endif
	{
		for (i = 0; i <= 1; i++) {
			...
			while (1) {
				...
				/*  */
				if (pstRTBuf_FrmB->ring_buf[i_dma]
					    .data[curr]
...
/* LOG_INF("RTBC_DBG7 imgo(buf cnt): %d %d
 * %d\n",pstRTBuf_FrmB->ring_buf[_imgo_].data[0].bFilled,pstRTBuf_FrmB->ring_buf[_imgo_].data[1].bFilled,pstRTBuf_FrmB->ring_buf[_imgo_].data[2].bFilled);
 */
/* LOG_INF("RTBC_DBG7 img2o(buf cnt): %d %d
 * %d\n",pstRTBuf_FrmB->ring_buf[_img2o_].data[0].bFilled,pstRTBuf_FrmB->ring_buf[_img2o_].data[1].bFilled,pstRTBuf_FrmB->ring_buf[_img2o_].data[2].bFilled);
 */
#if 0
	if (IspInfo_FrmB.DebugMask & ISP_DBG_INT_2) {
...
static signed int ISP_ED_BufQue_Set_FailNode(ISP_ED_BUF_STATE_ENUM failType, signed int idx)
{
	signed int ret = 0;
...
static signed int ISP_ED_BufQue_Erase(signed int idx, int listTag)
{
	...
	case P2_EDBUF_MLIST_TAG:
		...
		if (P2_EDBUF_MgrList[tmpIdx].p2dupCQIdx == -1) {
			 ...
			 * moment, should update first index to the last node */
			if ((!stop) && (i == cnt)) {
				P2_EDBUF_MList_FirstBufIdx =
					P2_EDBUF_MList_LastBufIdx;
			}
...
*******************************************************************************/
static signed int
ISP_ED_BufQue_Get_FirstMatBuf(struct ISP_ED_BUFQUE_STRUCT_FRMB param,
...
*******************************************************************************/
static signed int
ISP_ED_BufQue_CTRL_FUNC_FRMB(struct ISP_ED_BUFQUE_STRUCT_FRMB param)
{
	signed int ret = 0;
	int i = 0;
	int idx = -1, idx2 = -1;
	...
	switch (param.ctrl) {
	case ISP_ED_BUFQUE_CTRL_ENQUE_FRAME: /* signal that a specific buffer is
						enqueued */
		...
		if (((P2_EDBUF_MList_LastBufIdx + 1) %
			...
			if (
				...
				P2_EDBUF_RingList[P2_EDBUF_RList_FirstBufIdx]
						.bufSts ==
					ISP_ED_BUF_STATE_NONE) { /* all buffer
								    node is
								    empty */
				P2_EDBUF_RList_LastBufIdx =
				...
				P2_EDBUF_RingList[P2_EDBUF_RList_CurBufIdx]
						.bufSts ==
					ISP_ED_BUF_STATE_NONE) { /* first node
								    is not
								    empty, but
								    current/last
								    is empty */
		...
		break;
	case ISP_ED_BUFQUE_CTRL_WAIT_DEQUE: /* a dequeue thread is waiting to do
					       dequeue */
		...
		break;
	case ISP_ED_BUFQUE_CTRL_DEQUE_SUCCESS: /* signal that a buffer is
						  dequeued(success) */
	case ISP_ED_BUFQUE_CTRL_DEQUE_FAIL:    /* signal that a buffer is
						  dequeued(fail) */
		if (IspInfo_FrmB.DebugMask & ISP_DBG_BUF_CTRL) {
		 ...
		 * Rlikst to update the buffer status cuz deque success/fail may
		 * not be the first buffer in Rlist */
		/* //////////////////////////////////////////////////////////////////////
		 */
	...
	case ISP_ED_BUFQUE_CTRL_WAIT_FRAME: /* wait for a specific buffer */
		...
		if (idx == -1) {
			spin_unlock(&(SpinLockEDBufQueList));
			LOG_ERR("could not find match buffer pID/cID (%d/0x%x)",
				param.processID, param.callerID);
			ret = -EFAULT;
			return ret;
		}
		...
		if (P2_EDBUF_MgrList[idx].dequedNum == P2_Support_BurstQNum) {
			...
			/* [3]if not, goto wait event and wait for a signal to
			 * check */
			restTime = wait_event_interruptible_timeout(
		...
		break;
	case ISP_ED_BUFQUE_CTRL_WAKE_WAITFRAME: /* wake all sleeped users to
						   check buffer is dequeued or
						   not */
		wake_up_interruptible(&WaitQueueHead_EDBuf_WaitFrame);
		break;
...
static signed int ISP_REGISTER_IRQ_USERKEY(char *userName)
{
	...
	char
		m_UserName[USERKEY_STR_LEN]; /* local veriable for saving
						Username from user space */
	...
	if (userName == NULL) {
		...
		if (bCopyFromUser == MTRUE) {
			...
			if (length ==
			    USERKEY_STR_LEN) { /* string length too long */
			...
			if (FirstUnusedIrqUserKey >= IRQ_USER_NUM_MAX ||
				...
				for (i = 1; i < FirstUnusedIrqUserKey; i++) {
					if (strcmp(IrqUserKey_UserInfo[i]
							   .userName,
						   m_UserName) == 0) {
...
static signed int ISP_MARK_IRQ(struct ISP_WAIT_IRQ_STRUCT_FRMB irqinfo)
{
	...
	spin_lock_irqsave(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);
	IspInfo_FrmB.IrqInfo.MarkedTime_usec[irqinfo.UserInfo.UserKey]
					    [irqinfo.UserInfo.Type][idx] =
		(unsigned int)(time_usec);
	IspInfo_FrmB.IrqInfo.MarkedTime_sec[irqinfo.UserInfo.UserKey]
					   [irqinfo.UserInfo.Type][idx] =
	...
	spin_lock_irqsave(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);
	IspInfo_FrmB.IrqInfo.PassedBySigCnt[irqinfo.UserInfo.UserKey]
					   [irqinfo.UserInfo.Type][idx] = 0;
	spin_unlock_irqrestore(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);

	LOG_DBG("MARK key/type/sts (%d/%d/0x%x), t(%d us)",
...
*******************************************************************************/
static signed int
ISP_GET_MARKtoQEURY_TIME(struct ISP_WAIT_IRQ_STRUCT_FRMB *irqinfo)
{
	int idx;
	signed int Ret = 0;
	unsigned long flags;
	...
	time_ready2return_usec =
		do_div(time_ready2return_sec, 1000000); /* sec and usec */
	idx = my_get_pow_idx(irqinfo->UserInfo.Status);
	...
	if (irqinfo->UserInfo.Status &
	    IspInfo_FrmB.IrqInfo.MarkedFlag[irqinfo->UserInfo.UserKey]
...
static signed int ISP_WaitIrq_FrmB(struct ISP_WAIT_IRQ_STRUCT_FRMB *WaitIrq)
{
	...
	time_getrequest_usec =
		do_div(time_getrequest_sec, 1000000); /* sec and usec */

	/*struct timeval time_wake;*/
	/*struct timeval time_x;*/
	/*unsigned int t_Wake;*/ /* ns */
	/*  */
	if (IspInfo_FrmB.DebugMask & ISP_DBG_INT) {
	...
	if (WaitIrq->Clear == ISP_IRQ_CLEAR_STATUS_FRMB) {
		 ...
		 * been
		 * cleared",WaitIrq->Clear,WaitIrq->Type,IspInfo_FrmB.IrqInfo.Status[WaitIrq->Type]);
		 */
		IspInfo_FrmB.IrqInfo.Status[WaitIrq->UserInfo.UserKey]
					   [WaitIrq->UserInfo.Type] &=
			(~WaitIrq->UserInfo.Status);
		spin_unlock_irqrestore(&(IspInfo_FrmB.SpinLockIrq[eIrq]),
				       flags);
		return Ret;
	} else {
		if (WaitIrq->UserInfo.Status &
			 ...
			 * before signal occuring, we freese the counting for
			 * passby signal */
			...
			freeze_passbysigcnt = !(ISP_GetIRQState_FrmB(
			...
			if (WaitIrq->Clear == ISP_IRQ_CLEAR_WAIT_FRMB) {
				...
				if (IspInfo_FrmB.IrqInfo
					 ...
					 * Type(%d): IrqStatus(0x%08X) has been
					 * cleared",WaitIrq->Clear,WaitIrq->Type,IspInfo_FrmB.IrqInfo.Status[WaitIrq->Type]
					 * & WaitIrq->Status); */
	...
	Timeout = wait_event_interruptible_timeout(
		...
		ISP_GetIRQState_FrmB(eIrq, WaitIrq->UserInfo.Type,
				     WaitIrq->UserInfo.UserKey,
				     WaitIrq->UserInfo.Status),
	...
	if ((WaitIrq->UserInfo.UserKey == 0) &&
		...
		time_wake_usec =
			do_div(time_wake_sec, 1000000); /* sec and usec */
	...
	if ((Timeout != 0) &&
	    (!ISP_GetIRQState_FrmB(
		    ...
		    WaitIrq->UserInfo.Status))) { /* check if user is
	...
	/* 3. get interrupt and update time related information that would be
	 * return to user */
	/*unsigned long long time_ready2return_sec;*/
	/*unsigned long time_ready2return_usec;*/
	...
	WaitIrq->TimeInfo.tLastSig_sec =
		IspInfo_FrmB.IrqInfo
			.LastestSigTime_sec[WaitIrq->UserInfo.Type][idx];
	WaitIrq->TimeInfo.tLastSig_usec =
		IspInfo_FrmB.IrqInfo
			.LastestSigTime_usec[WaitIrq->UserInfo.Type][idx];
	if ((int)(WaitIrq->UserInfo.Type) < (int)(ISP_IRQ_TYPE_INTB) &&
		...
		WaitIrq->EisMeta.tLastSOF2P1done_sec =
			IspInfo_FrmB.IrqInfo
				.Eismeta[WaitIrq->UserInfo.Type][gEismetaRIdx]
				.tLastSOF2P1done_sec;
...
/*  */
	...
	IspInfo_FrmB.IrqInfo
		...
		(~WaitIrq->UserInfo
			  .Status); /* clear the status if someone get the irq
				       */
	spin_unlock_irqrestore(&(IspInfo_FrmB.SpinLockIrq[eIrq]), flags);
/*  */

EXIT:
	/* 4. clear mark flag / reset marked time / reset time related infor and
...
#define STATUSX_WARNING                                                        \
	...
	/* move into spinlock for protecting "m_P1_RCNT_INC_CNT" & fbc_cnt
	 * racing issue. */
	...
	if (IrqStatus[ISP_IRQ_TYPE_INT] & ISP_IRQ_INT_STATUS_PASS1_TG1_DON_ST) {
		...
		IspInfo_FrmB.IrqInfo.LastestSigTime_usec[ISP_IRQ_TYPE_INT][10] =
			(unsigned int)(usec);
		IspInfo_FrmB.IrqInfo.LastestSigTime_sec[ISP_IRQ_TYPE_INT][10] =
			(unsigned int)(sec);
		gEismetaInSOF = 0;
...
/* IRQ_LOG_KEEPER(_IRQ,m_CurrentPPB,_LOG_INF,"lost p1Done ErrHandle\n"); */
...
#ifdef _rtbc_buf_que_2_0_
		{
			...
			for (z = 0; z <= mFwRcnt.curIdx[_IRQ]; z++) {
				...
				if (mFwRcnt.INC[_IRQ][z] == 1) {
					...
					/* LOG_INF("RTBC_DBG:%d %d %d %d %d %d
					 * %d %d %d
					 * %d",mFwRcnt.INC[_IRQ][0],mFwRcnt.INC[_IRQ][1],mFwRcnt.INC[_IRQ][2],mFwRcnt.INC[_IRQ][3],mFwRcnt.INC[_IRQ][4],\
					 */
		...
		if (IspInfo_FrmB.DebugMask & ISP_DBG_INT) {
			union CQ_RTBC_FBC
				_fbc_chk[2]; /* can chk fbc status compare to
						p1_fbc. (the difference is the
						timing of reading) */

			_fbc_chk[0].Reg_val =
				ISP_RD32(ISP_REG_ADDR_IMGO_FBC); /* in order to
								    log newest
								    fbc
								    condition */
		...
		}
		{
			...
			/* update SOF time stamp for eis user(need match with
			 * the time stamp in image header) */
			IspInfo_FrmB.IrqInfo
				.LastestSigTime_usec[ISP_IRQ_TYPE_INT][12] =
				(unsigned int)(usec);
			IspInfo_FrmB.IrqInfo
				.LastestSigTime_sec[ISP_IRQ_TYPE_INT][12] =
				(unsigned int)(sec);
			IspInfo_FrmB.IrqInfo
				.Eismeta[ISP_IRQ_TYPE_INT][gEismetaWIdx]
			...
			if (sof_pass1done[0] == 1)
				ISP_SOF_Buf_Get_FrmB(_IRQ, sec, usec, MTRUE,
						     p1_fbc, curr_pa);
			else
				ISP_SOF_Buf_Get_FrmB(_IRQ, sec, usec, MFALSE,
						     p1_fbc, curr_pa);
		}
#if CONFIG_K_FOR_SYSTRACE
		_kernel_trace_end();
...
static bool ISP_BufWrite_Alloc_FrmB(void)
{
	...
	for (i = 0; i < ISP_BUF_WRITE_AMOUNT; i++) {
		IspInfo_FrmB.BufInfo.Write[i].Status =
			ISP_BUF_STATUS_EMPTY_FRMB;
		IspInfo_FrmB.BufInfo.Write[i].Size = 0;
		IspInfo_FrmB.BufInfo.Write[i].pData = (unsigned char *)kmalloc(
...
static signed int ISP_open_FrmB(void)
{
	...
	for (q = 0; q < IRQ_USER_NUM_MAX; q++) {
		for (i = 0; i < ISP_IRQ_TYPE_AMOUNT_FRMB; i++) {
			...
			for (p = 0; p < 32; p++) {
				IspInfo_FrmB.IrqInfo.MarkedTime_usec[q][i][p] =
					0;
				IspInfo_FrmB.IrqInfo.PassedBySigCnt[q][i][p] =
					0;
				IspInfo_FrmB.IrqInfo.LastestSigTime_usec[i][p] =
					0;
			}
...
#ifdef KERNEL_LOG
	IspInfo_FrmB.DebugMask =
		(ISP_DBG_INT | ISP_DBG_BUF_CTRL | ISP_DBG_INT_2);
#endif
/*  */
EXIT:
	if (Ret < 0) {
...
static signed int __init ISP_Init_FrmB(void)
{
	...
	for (i = 0; i < LOG_PPNUM; i++) {
		...
		tmp =
			(void *)((char *)tmp + NORMAL_STR_LEN); /* log buffer
								   ,in case of
								   overflow */
	}
