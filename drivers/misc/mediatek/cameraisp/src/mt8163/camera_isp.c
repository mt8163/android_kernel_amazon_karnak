...
/* #include <mach/mt_clkmgr.h> */ /* For clock mgr APIS.
...
#define LOG_INF(format, args...)                                               \
	pr_info(MyTag "[%s]" format "\n", __func__, ##args)
#define LOG_NOTICE(format, args...)                                            \
	pr_notice(MyTag "[%s]" format "\n", __func__, ##args)
#define LOG_WRN(format, args...)                                               \
	pr_warn(MyTag "[%s, line%05d]Warning:" format "\n", __func__,          \
		__LINE__, ##args)
#define LOG_ERR(format, args...)                                               \
	pr_err(MyTag "[%s, line%05d]ERROR:" format "\n", __func__, __LINE__,   \
	       ##args)
#define LOG_AST(format, args...)                                               \
	pr_alert(MyTag "[%s, line%05d]ASSERT:" format "\n", __func__,          \
		 __LINE__, ##args)

#ifdef ISPCNT_DEBUGLOG
#define LOG_DBG_IF(format, args...)                                            \
	pr_info(MyTag "[%s]" format "\n", __func__, ##args)
#else
#define LOG_DBG_IF(format, args...)                                            \
	pr_debug(MyTag "[%s]" format "\n", __func__, ##args)
...
/* #define ISP_WR32(addr, data)    iowrite32(data, addr)	*/ /* For other
								      projects.
								      */
/* #define ISP_WR32(addr, data)    mt65xx_reg_sync_writel(data, addr)    // For
 * 89 Only.NEED_TUNING_BY_PROJECT */
...
#define ISP_REG_RANGE                                                          \
	(0x10000) /* 0x6100,the same with the value in isp_reg.h and           \
		     page-aligned */
/* seninf driver */
#define SENINF_BASE_ADDR                                                       \
	0x15008000 /* the same with the value in                               \
		      seninf_drv.cpp(chip-dependent) */
#define SENINF_REG_RANGE                                                       \
	(0x4000) /* 0x800,the same with the value in seninf_reg.h and          \
		    page-aligned */
/* #define IMGSYS_CG_CLR0_ADDR  0x15000000 //the same with the value in
 * seninf_drv.cpp(chip-dependent) */
/* #define MMSYS_RANGE           (0x1000)   //0x100,the same with the value in
 * seninf_drv.cpp and page-aligned */
#define PLL_BASE_ADDR                                                          \
	0x10000000 /* the same with the value in                               \
		      seninf_drv.cpp(chip-dependent) */
#define PLL_RANGE                                                              \
	(0x1000) /* 0x200,the same with the value in seninf_drv.cpp and        \
		    page-aligned */
#define MIPIRX_CONFIG_ADDR                                                     \
	0x1500C000 /* the same with the value in                               \
		      seninf_drv.cpp(chip-dependent) */
#define MIPIRX_CONFIG_RANGE                                                    \
	(0x100) /* 0x100,the same with the value in seninf_drv.cpp and         \
		   page-aligned */
#define MIPIRX_ANALOG_ADDR                                                     \
	0x10217000 /* the same with the value in                               \
		      seninf_drv.cpp(chip-dependent) */
#define MIPIRX_ANALOG_RANGE (0x3000)
#define GPIO_BASE_ADDR                                                         \
	0x10005000 /* the same with the value in                               \
		      seninf_drv.cpp(chip-dependent) */
#define GPIO_RANGE (0x1000)
#define EFUSE_BASE_ADDR 0x10206000
#define EFUSE_RANGE                                                            \
	(0x1000) /* 0x400,the same with the value in seninf_drv.cpp and        \
		    page-aligned */
/* security concern */
...
/*
   if CONFIG_MTK_SMI_VARIANT is defined, isp scpsys is controlled by smi for
   suspend/resume.
...
struct ISP_CLK_STRUCT ispclk[] = {
	...
	{"MM_SMI_COMMON", NULL}, {"IMG_LARB2_SMI", NULL},
#endif
	{"IMG_CAM_SMI", NULL},   {"IMG_CAM_CAM", NULL},   {"IMG_SEN_TG", NULL},
	{"IMG_SEN_CAM", NULL},   {"IMG_CAM_SV", NULL},
};
...
#define ISP_REG_MASK_INT_STATUS                                                \
	(ISP_IRQ_INT_STATUS_VS1_ST | ISP_IRQ_INT_STATUS_TG1_ST1 |              \
	 ISP_IRQ_INT_STATUS_TG1_ST2 | ISP_IRQ_INT_STATUS_EXPDON1_ST |          \
	 ISP_IRQ_INT_STATUS_PASS1_TG1_DON_ST |                                 \
	 ISP_IRQ_INT_STATUS_SOF1_INT_ST | ISP_IRQ_INT_STATUS_PASS2_DON_ST |    \
	 ISP_IRQ_INT_STATUS_TPIPE_DON_ST | ISP_IRQ_INT_STATUS_AF_DON_ST |      \
	 ISP_IRQ_INT_STATUS_FLK_DON_ST | ISP_IRQ_INT_STATUS_CQ_DON_ST)

/* IRQ Error Mask */
#define ISP_REG_MASK_INT_STATUS_ERR                                            \
	(ISP_IRQ_INT_STATUS_TG1_ERR_ST | ISP_IRQ_INT_STATUS_CQ_ERR_ST |        \
	 ISP_IRQ_INT_STATUS_IMGO_ERR_ST | ISP_IRQ_INT_STATUS_AAO_ERR_ST |      \
	 ISP_IRQ_INT_STATUS_IMG2O_ERR_ST | ISP_IRQ_INT_STATUS_ESFKO_ERR_ST |   \
...
/* IRQ  Mask */
#define ISP_REG_MASK_DMA_INT                                                   \
	(ISP_IRQ_DMA_INT_IMGO_DONE_ST | ISP_IRQ_DMA_INT_IMG2O_DONE_ST |        \
	 ISP_IRQ_DMA_INT_AAO_DONE_ST | ISP_IRQ_DMA_INT_ESFKO_DONE_ST)
/* IRQ Error Mask */
#define ISP_REG_MASK_DMA_INT_ERR                                               \
	(ISP_IRQ_DMA_INT_CQ0_ERR_ST | ISP_IRQ_DMA_INT_TG1_GBERR_ST)

/* ====== CAM_CTL_INTB_STATUS ====== */

/* IRQ  Mask */
#define ISP_REG_MASK_INTB_STATUS                                               \
	(ISP_IRQ_INTB_STATUS_PASS2_DON_ST | ISP_IRQ_INTB_STATUS_TPIPE_DON_ST | \
	 ISP_IRQ_INTB_STATUS_CQ_DON_ST)
/* IRQ Error Mask */
#define ISP_REG_MASK_INTB_STATUS_ERR                                           \
	(ISP_IRQ_INTB_STATUS_CQ_ERR_ST | ISP_IRQ_INTB_STATUS_IMGO_ERR_ST |     \
	 ISP_IRQ_INTB_STATUS_LCSO_ERR_ST | ISP_IRQ_INTB_STATUS_IMG2O_ERR_ST |  \
	 ISP_IRQ_INTB_STATUS_LSC_ERR_ST | ISP_IRQ_INTB_STATUS_LCE_ERR_ST |     \
	 ISP_IRQ_INTB_STATUS_DMA_ERR_ST)
/* CAM_CTL_DMAB_INT */
/* IRQ  Mask */
#define ISP_REG_MASK_DMAB_INT                                                  \
	(ISP_IRQ_DMAB_INT_IMGO_DONE_ST | ISP_IRQ_DMAB_INT_IMG2O_DONE_ST |      \
	 ISP_IRQ_DMAB_INT_AAO_DONE_ST | ISP_IRQ_DMAB_INT_LCSO_DONE_ST |        \
	 ISP_IRQ_DMAB_INT_ESFKO_DONE_ST | ISP_IRQ_DMAB_INT_DISPO_DONE_ST |     \
...
/* IRQ  Mask */
#define ISP_REG_MASK_INTC_STATUS                                               \
	(ISP_IRQ_INTC_STATUS_PASS2_DON_ST | ISP_IRQ_INTC_STATUS_TPIPE_DON_ST | \
	 ISP_IRQ_INTC_STATUS_CQ_DON_ST)
/* IRQ Error Mask */
#define ISP_REG_MASK_INTC_STATUS_ERR                                           \
	(ISP_IRQ_INTC_STATUS_CQ_ERR_ST | ISP_IRQ_INTC_STATUS_IMGO_ERR_ST |     \
	 ISP_IRQ_INTC_STATUS_LCSO_ERR_ST | ISP_IRQ_INTC_STATUS_IMG2O_ERR_ST |  \
	 ISP_IRQ_INTC_STATUS_LSC_ERR_ST | ISP_IRQ_INTC_STATUS_BPC_ERR_ST |     \
	 ISP_IRQ_INTC_STATUS_LCE_ERR_ST | ISP_IRQ_INTC_STATUS_DMA_ERR_ST)
/* ====== CAM_CTL_DMAC_INT ====== */

/* IRQ  Mask */
#define ISP_REG_MASK_DMAC_INT                                                  \
	(ISP_IRQ_DMAC_INT_IMGO_DONE_ST | ISP_IRQ_DMAC_INT_IMG2O_DONE_ST |      \
	 ISP_IRQ_DMAC_INT_AAO_DONE_ST | ISP_IRQ_DMAC_INT_LCSO_DONE_ST |        \
	 ISP_IRQ_DMAC_INT_ESFKO_DONE_ST | ISP_IRQ_DMAC_INT_DISPO_DONE_ST |     \
...
/* IRQ  Mask */
#define ISP_REG_MASK_INTX_STATUS                                               \
	(ISP_IRQ_INTX_STATUS_VS1_ST | ISP_IRQ_INTX_STATUS_TG1_ST1 |            \
	 ISP_IRQ_INTX_STATUS_TG1_ST2 | ISP_IRQ_INTX_STATUS_EXPDON1_ST |        \
	 ISP_IRQ_INTX_STATUS_VS2_ST | ISP_IRQ_INTX_STATUS_TG2_ST1 |            \
	 ISP_IRQ_INTX_STATUS_TG2_ST2 | ISP_IRQ_INTX_STATUS_EXPDON2_ST |        \
	 ISP_IRQ_INTX_STATUS_PASS1_TG1_DON_ST |                                \
	 ISP_IRQ_INTX_STATUS_PASS1_TG2_DON_ST |                                \
	 ISP_IRQ_INTX_STATUS_PASS2_DON_ST | ISP_IRQ_INTX_STATUS_TPIPE_DON_ST | \
	 ISP_IRQ_INTX_STATUS_AF_DON_ST | ISP_IRQ_INTX_STATUS_FLK_DON_ST |      \
	 ISP_IRQ_INTX_STATUS_FMT_DON_ST | ISP_IRQ_INTX_STATUS_CQ_DON_ST)
/* IRQ Error Mask */
#define ISP_REG_MASK_INTX_STATUS_ERR                                           \
	(ISP_IRQ_INTX_STATUS_TG1_ERR_ST | ISP_IRQ_INTX_STATUS_TG2_ERR_ST |     \
	 ISP_IRQ_INTX_STATUS_CQ_ERR_ST | ISP_IRQ_INTX_STATUS_IMGO_ERR_ST |     \
	 ISP_IRQ_INTX_STATUS_AAO_ERR_ST | ISP_IRQ_INTX_STATUS_LCSO_ERR_ST |    \
	 ISP_IRQ_INTX_STATUS_LCSO_ERR_ST | ISP_IRQ_INTX_STATUS_ESFKO_ERR_ST |  \
	 ISP_IRQ_INTX_STATUS_FLK_ERR_ST | ISP_IRQ_INTX_STATUS_LSC_ERR_ST |     \
	 ISP_IRQ_INTX_STATUS_LCE_ERR_ST | ISP_IRQ_INTX_STATUS_DMA_ERR_ST)
/* ISP_IRQ_INTX_STATUS_BPC_ERR_ST    //Vent@20121025: Remove
 * ISP_IRQ_INTX_STATUS_BPC_ERR_ST. From TH Wu's explanation, this bit is not
 * used as an error state anymore. */

#define ISP_REG_MASK_INTX_STATUS_ERR_FRMB                                      \
	(ISP_IRQ_INTX_STATUS_TG1_ERR_ST | ISP_IRQ_INTX_STATUS_CQ_ERR_ST |      \
	 ISP_IRQ_INTX_STATUS_DMA_ERR_ST)

/* ====== CAM_CTL_DMA_INTX ====== */

/* IRQ  Mask */
#define ISP_REG_MASK_DMAX_INT                                                  \
	(ISP_IRQ_DMAX_INT_IMGO_DONE_ST | ISP_IRQ_DMAX_INT_IMG2O_DONE_ST |      \
	 ISP_IRQ_DMAX_INT_AAO_DONE_ST | ISP_IRQ_DMAX_INT_LCSO_DONE_ST |        \
	 ISP_IRQ_DMAX_INT_ESFKO_DONE_ST | ISP_IRQ_DMAX_INT_DISPO_DONE_ST |     \
	 ISP_IRQ_DMAX_INT_VIDO_DONE_ST | ISP_IRQ_DMAX_INT_VRZO_DONE_ST |       \
	 ISP_IRQ_DMAX_INT_NR3O_DONE_ST | ISP_IRQ_DMAX_INT_BUF_OVL_ST)
/* IRQ Error Mask */
#define ISP_REG_MASK_DMAX_INT_ERR                                              \
	(ISP_IRQ_DMAX_INT_NR3O_ERR_ST | ISP_IRQ_DMAX_INT_CQ_ERR_ST |           \
	 ISP_IRQ_DMAX_INT_TG1_GBERR_ST | ISP_IRQ_DMAX_INT_TG2_GBERR_ST)
...
#if (LOG_CONSTRAINT_ADJ == 1)
static bool g_log_def_constraint = 100;
#endif
...
/* static struct ISP_DEQUE_BUF_INFO_STRUCT g_deque_buf = {0,{}};    // Marked to
 * remove build warning.WARNING */
...
#define SIG_TEST                                                               \
	44 /* we choose 44 as our signal number (real-time signals are in the  \
	      range of 33 to 64) */
...
int getTaskInfo(pid_t pid)
{
	...
	/* t = find_task_by_pid_type(PIDTYPE_PID, g_pid);  //find the
...
static signed int ISP_DumpReg(void)
{
	...
	LOG_DBG("start MT");
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x000,
		ISP_RD32((void *)(ISP_ADDR + 0x000)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x004,
		ISP_RD32((void *)(ISP_ADDR + 0x004)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x008,
		ISP_RD32((void *)(ISP_ADDR + 0x008)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x00C,
		ISP_RD32((void *)(ISP_ADDR + 0x00C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x010,
		ISP_RD32((void *)(ISP_ADDR + 0x010)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x018,
		ISP_RD32((void *)(ISP_ADDR + 0x018)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x01C,
		ISP_RD32((void *)(ISP_ADDR + 0x01C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x020,
		ISP_RD32((void *)(ISP_ADDR + 0x020)));

#if 1 /* it may touch ReadClear register */
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x024,
		ISP_RD32((void *)(ISP_ADDR + 0x024)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x028,
		ISP_RD32((void *)(ISP_ADDR + 0x028)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x02C,
		ISP_RD32((void *)(ISP_ADDR + 0x02C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x030,
		ISP_RD32((void *)(ISP_ADDR + 0x030)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x034,
		ISP_RD32((void *)(ISP_ADDR + 0x034)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x038,
	...
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x050,
		ISP_RD32((void *)(ISP_ADDR + 0x050))); /* Tile setting */
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x074,
		ISP_RD32((void *)(ISP_ADDR + 0x074)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x078,
		ISP_RD32((void *)(ISP_ADDR + 0x078)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x07C,
		ISP_RD32((void *)(ISP_ADDR + 0x07C)));
#if 0 /* WO */
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x080,
		ISP_RD32((void *)(ISP_ADDR + 0x080)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x084,
		ISP_RD32((void *)(ISP_ADDR + 0x084)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x088,
		ISP_RD32((void *)(ISP_ADDR + 0x088)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x08C,
		ISP_RD32((void *)(ISP_ADDR + 0x08C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x090,
		ISP_RD32((void *)(ISP_ADDR + 0x090)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x094,
		ISP_RD32((void *)(ISP_ADDR + 0x094)));
#endif
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x0A8,
		ISP_RD32((void *)(ISP_ADDR + 0x0A8))); /* cq0 base */
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x0AC,
		ISP_RD32((void *)(ISP_ADDR + 0x0AC)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x0B0,
		ISP_RD32((void *)(ISP_ADDR + 0x0B0)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x0B4,
		ISP_RD32((void *)(ISP_ADDR + 0x0B4)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x0B8,
		ISP_RD32((void *)(ISP_ADDR + 0x0B8)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x0BC,
		ISP_RD32((void *)(ISP_ADDR + 0x0BC)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x0E8,
		ISP_RD32((void *)(ISP_ADDR + 0x0E8))); /* cq0 cur base */
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x0F4,
		ISP_RD32((void *)(ISP_ADDR + 0x0F4))); /* fbc imgo */
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x0F8,
		ISP_RD32((void *)(ISP_ADDR + 0x0F8))); /* fbc img2o */
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x110,
		ISP_RD32((void *)(ISP_ADDR + 0x110))); /* mdp crop x */
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x114,
		ISP_RD32((void *)(ISP_ADDR + 0x114))); /* mdp crop y */
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x138,
		ISP_RD32((void *)(ISP_ADDR + 0x138)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x13C,
		ISP_RD32((void *)(ISP_ADDR + 0x13C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x144,
		ISP_RD32((void *)(ISP_ADDR + 0x144)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x148,
		ISP_RD32((void *)(ISP_ADDR + 0x148)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x14C,
		ISP_RD32((void *)(ISP_ADDR + 0x14C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x204,
		ISP_RD32((void *)(ISP_ADDR + 0x204)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x20C,
		ISP_RD32((void *)(ISP_ADDR + 0x20C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x22C,
		ISP_RD32((void *)(ISP_ADDR + 0x22C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x230,
		ISP_RD32((void *)(ISP_ADDR + 0x230)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x234,
		ISP_RD32((void *)(ISP_ADDR + 0x234)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x238,
		ISP_RD32((void *)(ISP_ADDR + 0x238)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x23C,
		ISP_RD32((void *)(ISP_ADDR + 0x23C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x240,
		ISP_RD32((void *)(ISP_ADDR + 0x240)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x244,
		ISP_RD32((void *)(ISP_ADDR + 0x244)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x258,
		ISP_RD32((void *)(ISP_ADDR + 0x258)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x260,
		ISP_RD32((void *)(ISP_ADDR + 0x260)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x26C,
		ISP_RD32((void *)(ISP_ADDR + 0x26C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x270,
		ISP_RD32((void *)(ISP_ADDR + 0x270)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x27C,
		ISP_RD32((void *)(ISP_ADDR + 0x27C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x298,
		ISP_RD32((void *)(ISP_ADDR + 0x298)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x2B4,
		ISP_RD32((void *)(ISP_ADDR + 0x2B4)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x2D0,
		ISP_RD32((void *)(ISP_ADDR + 0x2D0)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x2D4,
		ISP_RD32((void *)(ISP_ADDR + 0x2D4)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x2F0,
		ISP_RD32((void *)(ISP_ADDR + 0x2F0)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x2F4,
		ISP_RD32((void *)(ISP_ADDR + 0x2F4)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x300,
		ISP_RD32((void *)(ISP_ADDR + 0x300)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x304,
		ISP_RD32((void *)(ISP_ADDR + 0x304)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x308,
		ISP_RD32((void *)(ISP_ADDR + 0x308)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x30C,
		ISP_RD32((void *)(ISP_ADDR + 0x30C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x310,
		ISP_RD32((void *)(ISP_ADDR + 0x310)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x314,
		ISP_RD32((void *)(ISP_ADDR + 0x314)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x318,
		ISP_RD32((void *)(ISP_ADDR + 0x318)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x31C,
		ISP_RD32((void *)(ISP_ADDR + 0x31C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x320,
		ISP_RD32((void *)(ISP_ADDR + 0x320)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x324,
		ISP_RD32((void *)(ISP_ADDR + 0x324)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x328,
		ISP_RD32((void *)(ISP_ADDR + 0x328)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x32C,
		ISP_RD32((void *)(ISP_ADDR + 0x32C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x330,
		ISP_RD32((void *)(ISP_ADDR + 0x330)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x334,
		ISP_RD32((void *)(ISP_ADDR + 0x334)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x338,
		ISP_RD32((void *)(ISP_ADDR + 0x338)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x33C,
		ISP_RD32((void *)(ISP_ADDR + 0x33C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x350,
		ISP_RD32((void *)(ISP_ADDR + 0x350)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x37C,
		ISP_RD32((void *)(ISP_ADDR + 0x37C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x398,
		ISP_RD32((void *)(ISP_ADDR + 0x398)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x410,
		ISP_RD32((void *)(ISP_ADDR + 0x410)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x414,
		ISP_RD32((void *)(ISP_ADDR + 0x414)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x418,
		ISP_RD32((void *)(ISP_ADDR + 0x418)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x41C,
		ISP_RD32((void *)(ISP_ADDR + 0x41C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x420,
		ISP_RD32((void *)(ISP_ADDR + 0x420)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x448,
		ISP_RD32((void *)(ISP_ADDR + 0x448)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4B0,
		ISP_RD32((void *)(ISP_ADDR + 0x4B0)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4B4,
		ISP_RD32((void *)(ISP_ADDR + 0x4B4)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4B8,
		ISP_RD32((void *)(ISP_ADDR + 0x4B8)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4BC,
		ISP_RD32((void *)(ISP_ADDR + 0x4BC)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4E8,
		ISP_RD32((void *)(ISP_ADDR + 0x4E8)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x534,
		ISP_RD32((void *)(ISP_ADDR + 0x534)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x538,
		ISP_RD32((void *)(ISP_ADDR + 0x538)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x53C,
		ISP_RD32((void *)(ISP_ADDR + 0x53C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x580,
	...
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x884,
		ISP_RD32((void *)(ISP_ADDR + 0x884)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x888,
		ISP_RD32((void *)(ISP_ADDR + 0x888)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x8A0,
		ISP_RD32((void *)(ISP_ADDR + 0x8A0)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x920,
		ISP_RD32((void *)(ISP_ADDR + 0x920)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x924,
		ISP_RD32((void *)(ISP_ADDR + 0x924)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x928,
		ISP_RD32((void *)(ISP_ADDR + 0x928)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x92C,
		ISP_RD32((void *)(ISP_ADDR + 0x92C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x930,
		ISP_RD32((void *)(ISP_ADDR + 0x930)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x934,
		ISP_RD32((void *)(ISP_ADDR + 0x934)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x938,
		ISP_RD32((void *)(ISP_ADDR + 0x938)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x93C,
		ISP_RD32((void *)(ISP_ADDR + 0x93C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x948,
		ISP_RD32((void *)(ISP_ADDR + 0x948))); /* unp offset */
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x960,
		ISP_RD32((void *)(ISP_ADDR + 0x960)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x9C4,
		ISP_RD32((void *)(ISP_ADDR + 0x9C4)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x9E4,
		ISP_RD32((void *)(ISP_ADDR + 0x9E4)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x9E8,
		ISP_RD32((void *)(ISP_ADDR + 0x9E8)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x9EC,
		ISP_RD32((void *)(ISP_ADDR + 0x9EC)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xA00,
		ISP_RD32((void *)(ISP_ADDR + 0xA00)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xA04,
		ISP_RD32((void *)(ISP_ADDR + 0xA04)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xA08,
		ISP_RD32((void *)(ISP_ADDR + 0xA08)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xA0C,
		ISP_RD32((void *)(ISP_ADDR + 0xA0C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xA10,
		ISP_RD32((void *)(ISP_ADDR + 0xA10)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xA14,
		ISP_RD32((void *)(ISP_ADDR + 0xA14)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xA20,
		ISP_RD32((void *)(ISP_ADDR + 0xA20)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xAA0,
		ISP_RD32((void *)(ISP_ADDR + 0xAA0)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xACC,
		ISP_RD32((void *)(ISP_ADDR + 0xACC)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB00,
		ISP_RD32((void *)(ISP_ADDR + 0xB00)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB04,
		ISP_RD32((void *)(ISP_ADDR + 0xB04)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB08,
		ISP_RD32((void *)(ISP_ADDR + 0xB08)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB0C,
		ISP_RD32((void *)(ISP_ADDR + 0xB0C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB10,
		ISP_RD32((void *)(ISP_ADDR + 0xB10)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB14,
		ISP_RD32((void *)(ISP_ADDR + 0xB14)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB18,
		ISP_RD32((void *)(ISP_ADDR + 0xB18)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB1C,
		ISP_RD32((void *)(ISP_ADDR + 0xB1C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB20,
		ISP_RD32((void *)(ISP_ADDR + 0xB20)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB44,
		ISP_RD32((void *)(ISP_ADDR + 0xB44)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB48,
		ISP_RD32((void *)(ISP_ADDR + 0xB48)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB4C,
		ISP_RD32((void *)(ISP_ADDR + 0xB4C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB50,
		ISP_RD32((void *)(ISP_ADDR + 0xB50)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB54,
		ISP_RD32((void *)(ISP_ADDR + 0xB54)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB58,
		ISP_RD32((void *)(ISP_ADDR + 0xB58)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB5C,
		ISP_RD32((void *)(ISP_ADDR + 0xB5C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xB60,
		ISP_RD32((void *)(ISP_ADDR + 0xB60)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xBA0,
		ISP_RD32((void *)(ISP_ADDR + 0xBA0)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xBA4,
		ISP_RD32((void *)(ISP_ADDR + 0xBA4)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xBA8,
		ISP_RD32((void *)(ISP_ADDR + 0xBA8)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xBAC,
		ISP_RD32((void *)(ISP_ADDR + 0xBAC)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xBB0,
		ISP_RD32((void *)(ISP_ADDR + 0xBB0)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xBB4,
		ISP_RD32((void *)(ISP_ADDR + 0xBB4)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xBB8,
		ISP_RD32((void *)(ISP_ADDR + 0xBB8)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xBBC,
		ISP_RD32((void *)(ISP_ADDR + 0xBBC)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xBC0,
		ISP_RD32((void *)(ISP_ADDR + 0xBC0)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xC20,
		ISP_RD32((void *)(ISP_ADDR + 0xC20)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xCC0,
		ISP_RD32((void *)(ISP_ADDR + 0xCC0)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xCE4,
		ISP_RD32((void *)(ISP_ADDR + 0xCE4)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xCE8,
		ISP_RD32((void *)(ISP_ADDR + 0xCE8)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xCEC,
		ISP_RD32((void *)(ISP_ADDR + 0xCEC)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xCF0,
		ISP_RD32((void *)(ISP_ADDR + 0xCF0)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xCF4,
		ISP_RD32((void *)(ISP_ADDR + 0xCF4)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xCF8,
		ISP_RD32((void *)(ISP_ADDR + 0xCF8)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xCFC,
		ISP_RD32((void *)(ISP_ADDR + 0xCFC)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xD24,
		ISP_RD32((void *)(ISP_ADDR + 0xD24)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xD28,
		ISP_RD32((void *)(ISP_ADDR + 0xD28)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xD2C,
		ISP_RD32((void *)(ISP_ADDR + 0xD2c)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xD40,
		ISP_RD32((void *)(ISP_ADDR + 0xD40)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xD64,
		ISP_RD32((void *)(ISP_ADDR + 0xD64)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xD68,
		ISP_RD32((void *)(ISP_ADDR + 0xD68)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xD6C,
		ISP_RD32((void *)(ISP_ADDR + 0xD6c)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xD70,
		ISP_RD32((void *)(ISP_ADDR + 0xD70)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xD74,
		ISP_RD32((void *)(ISP_ADDR + 0xD74)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xD78,
		ISP_RD32((void *)(ISP_ADDR + 0xD78)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xD7C,
		ISP_RD32((void *)(ISP_ADDR + 0xD7C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xDA4,
		ISP_RD32((void *)(ISP_ADDR + 0xDA4)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xDA8,
		ISP_RD32((void *)(ISP_ADDR + 0xDA8)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0xDAC,
		ISP_RD32((void *)(ISP_ADDR + 0xDAC)));
#if 0
	ISP_WR32((void *)(ISP_ADDR + 0x4618), 0x0000FFFF);
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4010,
		ISP_RD32((void *)(ISP_ADDR + 0x4010)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4014,
		ISP_RD32((void *)(ISP_ADDR + 0x4014)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4018,
		ISP_RD32((void *)(ISP_ADDR + 0x4018)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x401C,
		ISP_RD32((void *)(ISP_ADDR + 0x401C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4020,
		ISP_RD32((void *)(ISP_ADDR + 0x4020)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4024,
		ISP_RD32((void *)(ISP_ADDR + 0x4024)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4028,
		ISP_RD32((void *)(ISP_ADDR + 0x4028)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x402C,
		ISP_RD32((void *)(ISP_ADDR + 0x402C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4100,
		ISP_RD32((void *)(ISP_ADDR + 0x4100)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4104,
		ISP_RD32((void *)(ISP_ADDR + 0x4104)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4108,
		ISP_RD32((void *)(ISP_ADDR + 0x4108)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x410C,
		ISP_RD32((void *)(ISP_ADDR + 0x410C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4120,
		ISP_RD32((void *)(ISP_ADDR + 0x4120)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x412C,
		ISP_RD32((void *)(ISP_ADDR + 0x412C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x420C,
		ISP_RD32((void *)(ISP_ADDR + 0x420C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4234,
		ISP_RD32((void *)(ISP_ADDR + 0x4234)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4240,
		ISP_RD32((void *)(ISP_ADDR + 0x4240)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4300,
		ISP_RD32((void *)(ISP_ADDR + 0x4300)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4304,
		ISP_RD32((void *)(ISP_ADDR + 0x4304)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4600,
		ISP_RD32((void *)(ISP_ADDR + 0x4600)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4604,
		ISP_RD32((void *)(ISP_ADDR + 0x4604)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4608,
		ISP_RD32((void *)(ISP_ADDR + 0x4608)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x460C,
		ISP_RD32((void *)(ISP_ADDR + 0x460C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4610,
		ISP_RD32((void *)(ISP_ADDR + 0x4610)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4614,
		ISP_RD32((void *)(ISP_ADDR + 0x4614)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4618,
		ISP_RD32((void *)(ISP_ADDR + 0x4618)));
	ISP_WR32((void *)(ISP_ADDR + 0x461C), 0x10);
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x461C,
		ISP_RD32((void *)(ISP_ADDR + 0x461C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4620,
	...
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4618,
		ISP_RD32((void *)(ISP_ADDR + 0x4618)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4624,
		ISP_RD32((void *)(ISP_ADDR + 0x4624)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4628,
		ISP_RD32((void *)(ISP_ADDR + 0x4628)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x462C,
		ISP_RD32((void *)(ISP_ADDR + 0x462C)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x4630,
	...
	for (i = 0x44; i <= 0x48; i += 4)
		LOG_DBG("0x%08X %08X ", (unsigned int)(ISP_TPIPE_ADDR + i),
			ISP_RD32(ISP_ADDR + i));
...
static int ISPDbgPortDump(int params)
{
	...
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x054,
	...
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x240,
		ISP_RD32((void *)(ISP_ADDR + 0x240)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x248,
		ISP_RD32((void *)(ISP_ADDR + 0x248)));
	LOG_DBG("0x%08X %08X", ISP_TPIPE_ADDR + 0x24C,
		ISP_RD32((void *)(ISP_ADDR + 0x24C)));
	LOG_DBG(" <");
...
int ISPRegDump(int params, int params1)
{
	...
	LOG_DBG("start MT");
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x000, ISP_RD32((void *)(ISP_ADDR + 0x000)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x004, ISP_RD32((void *)(ISP_ADDR + 0x004)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x008, ISP_RD32((void *)(ISP_ADDR + 0x008)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x00C, ISP_RD32((void *)(ISP_ADDR + 0x00C)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x010, ISP_RD32((void *)(ISP_ADDR + 0x010)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x018, ISP_RD32((void *)(ISP_ADDR + 0x018)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x01C, ISP_RD32((void *)(ISP_ADDR + 0x01C)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x050, ISP_RD32((void *)(ISP_ADDR + 0x050)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x054, ISP_RD32((void *)(ISP_ADDR + 0x054)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x078, ISP_RD32((void *)(ISP_ADDR + 0x078)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x110, ISP_RD32((void *)(ISP_ADDR + 0x110)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x114, ISP_RD32((void *)(ISP_ADDR + 0x114)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x22C, ISP_RD32((void *)(ISP_ADDR + 0x22C)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x240, ISP_RD32((void *)(ISP_ADDR + 0x240)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x27C, ISP_RD32((void *)(ISP_ADDR + 0x27C)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x2B4, ISP_RD32((void *)(ISP_ADDR + 0x2B4)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x308, ISP_RD32((void *)(ISP_ADDR + 0x308)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x30C, ISP_RD32((void *)(ISP_ADDR + 0x30C)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x310, ISP_RD32((void *)(ISP_ADDR + 0x310)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x31C, ISP_RD32((void *)(ISP_ADDR + 0x31C)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x328, ISP_RD32((void *)(ISP_ADDR + 0x328)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x32C, ISP_RD32((void *)(ISP_ADDR + 0x32C)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x330, ISP_RD32((void *)(ISP_ADDR + 0x330)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x33C, ISP_RD32((void *)(ISP_ADDR + 0x33C)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x3A8, ISP_RD32((void *)(ISP_ADDR + 0x3A8)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x534, ISP_RD32((void *)(ISP_ADDR + 0x534)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x538, ISP_RD32((void *)(ISP_ADDR + 0x538)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x53C, ISP_RD32((void *)(ISP_ADDR + 0x53C)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x800, ISP_RD32((void *)(ISP_ADDR + 0x800)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x8A0, ISP_RD32((void *)(ISP_ADDR + 0x8A0)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x9C4, ISP_RD32((void *)(ISP_ADDR + 0x9C4)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x9E4, ISP_RD32((void *)(ISP_ADDR + 0x9E4)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x9E8, ISP_RD32((void *)(ISP_ADDR + 0x9E8)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0x9EC, ISP_RD32((void *)(ISP_ADDR + 0x9EC)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0xA20, ISP_RD32((void *)(ISP_ADDR + 0xA20)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0xACC, ISP_RD32((void *)(ISP_ADDR + 0xACC)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0xB00, ISP_RD32((void *)(ISP_ADDR + 0xB00)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0xB04, ISP_RD32((void *)(ISP_ADDR + 0xB04)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0xB08, ISP_RD32((void *)(ISP_ADDR + 0xB08)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0xB0C, ISP_RD32((void *)(ISP_ADDR + 0xB0C)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0xB10, ISP_RD32((void *)(ISP_ADDR + 0xB10)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0xB14, ISP_RD32((void *)(ISP_ADDR + 0xB14)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0xB18, ISP_RD32((void *)(ISP_ADDR + 0xB18)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0xB1C, ISP_RD32((void *)(ISP_ADDR + 0xB1C)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0xB20, ISP_RD32((void *)(ISP_ADDR + 0xB20)));
	LOG_DBG("[ISP/MDP][TPIPE_DumpReg] 0x%08X = 0x%08X",
		ISP_TPIPE_ADDR + 0xF50, ISP_RD32((void *)(ISP_ADDR + 0xF50)));
	LOG_DBG("end MT");
...
static int ISPResetPass2(int params)
{
	...
	ISP_WR32((void *)ISP_REG_ADDR_CTL_EN2,
		 m32RegEn2 & (~ISP_REG_ADDR_CTL_EN2_UV_CRSA_EN_BIT));
	/* LOG_DBG("ISP_REG_ADDR_CTL_EN2>>: 0x%08x ", ISP_RD32((void
	 * *)ISP_REG_ADDR_CTL_EN2)); */
	...
	/* LOG_DBG("ISP_REG_ADDR_CTL_CROP_X>>: 0x%08x ", ISP_RD32((void
	 * *)ISP_REG_ADDR_CTL_CROP_X)); */

	ISP_WR32((void *)ISP_REG_ADDR_CTL_DBG_SET,
		 ISP_REG_ADDR_CTL_DBG_SET_CQ_STS);
	for (i = 0; i < 5000; i++) {
		if ((ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_PORT) & 0x11) ==
		    0x11) {
			break;
		}
	}
...
{
	/* must keep this clk close order: ISP clk -> CG_SCP_SYS_ISP ->
...
void ISP_EnableClock(bool En, unsigned int LogLevel)
{

	if (LogLevel > 0) {
		LOG_INF("En  (%d),g_EnableClkCnt(%d)", En,
...
/*LOG_DBG("Camera clock enbled. g_EnableClkCnt(%d)", g_EnableClkCnt);*/
		...
		/*
		LOG_DBG("[MTCMOS CHECK]===> [0x1000660c]=0x%x ,
		[0x10006610]=0x%x ",
		ISP_RD32(g_scpsys_baseaddr+0x60c),
		ISP_RD32(g_scpsys_baseaddr+0x610));

		if ((ISP_RD32(g_scpsys_baseaddr+0x60c)&0x28) != 0x28)
			LOG_ERR("MTCMOS: DISP-Bit3(%d), ISP-Bit5(%d) is not
		opened !!",
				(ISP_RD32(g_scpsys_baseaddr+0x60c)&0x08),
		(ISP_RD32(g_scpsys_baseaddr+0x60c)&0x20));

		LOG_DBG("[VENCPLL CHECK]===> [0x1000F800]=0x%x ,
		[0x1000F80c]=0x%x ",
		ISP_RD32(g_ddrphy_baseaddr+0x800),
		ISP_RD32(g_ddrphy_baseaddr+0x80c));
		LOG_DBG("[IMG_CG_CON CHECK]===> [0x%lx]=0x%x", ISP_IMGSYS_BASE,
		ISP_RD32(ISP_IMGSYS_BASE));
		*/
		/*LOG_DBG("(0x%08x 0x%08x)", ISP_RD32((void
		...
		if (ISP_RD32((void *)ISP_REG_ADDR_CTL_DBG_SET) !=
			...
			if (LogLevel > 0) {
				LOG_DBG("(0x%08x 0x%08x)",
					ISP_RD32((
						void *)ISP_REG_ADDR_CTL_DBG_SET),
					ISP_RD32((
						void *)ISP_REG_ADDR_CTL_DBG_PORT));
			}
			/* LOG_DBG(" <"); */
		}
	} else { /* disable clock. */
...
static inline void ISP_Reset(void)
{
	...
	/*
	   1500000C    IMG_SW_RST
	   17    16    SENINF    SENINF    R/W    2'b00    "SENINF clock domain
	   hw reset
	...
	/* js_test, remove later once IMGSYS is working. Now it's NOT working on
	 * FPGA. */
...
/* bandwidth limitor for TG */
	...
	ISP_WR32((void *)ISP_REG_ADDR_SW_CTL,
		 ISP_REG_SW_CTL_SW_RST_TRIG | ISP_REG_SW_CTL_HW_RST); /* 0x5 */
...
*
********************************************************************************
static void ISP_BufWrite_Init(void)    //Vent@20121106: Marked to remove build
warning: 'ISP_BufWrite_Init' defined but not used [-Wunused-function]
{
    unsigned int i;
    //
...
static bool ISP_BufWrite_Alloc(void)
{
	...
	for (i = 0; i < ISP_BUF_WRITE_AMOUNT; i++) {
		g_IspInfo.BufInfo.Write[i].Status = ISP_BUF_STATUS_EMPTY;
		g_IspInfo.BufInfo.Write[i].Size = 0;
		g_IspInfo.BufInfo.Write[i].pData = (unsigned char *)kmalloc(
			ISP_BUF_SIZE_WRITE, GFP_ATOMIC);
		if (g_IspInfo.BufInfo.Write[i].pData == NULL) {
...
static signed int ISP_WriteReg(struct ISP_REG_IO_STRUCT *pRegIo)
{
	...
	if (g_IspInfo.DebugMask & ISP_DBG_WRITE_REG) {
		/* LOG_DBG("Data(0x%08X), Count(%d)", (unsigned
		 * int)(pRegIo->Data), (unsigned int)(pRegIo->Count)); */
	...
	if (atomic_read(&(g_IspInfo.HoldInfo.HoldEnable))) {
		if (ISP_BufWrite_Add((pRegIo->Count) *
			...
			TimeVd =
				ISP_JiffiesToMs(jiffies) - g_IspInfo.TimeLog.Vd;
			...
			LOG_ERR("HoldTime(%d),VD(%d ms),Expdone(%d ms),Tasklet(%d ms)",
				g_IspInfo.HoldInfo.Time, TimeVd, TimeExpdone,
				TimeTasklet);
			ISP_BufWrite_Dump();
			ISP_DumpReg();
		...
		/* pData = (unsigned char*)kmalloc((pRegIo->Count)*sizeof(struct
		 * ISP_REG_STRUCT), GFP_ATOMIC); */
		pData = (struct ISP_REG_STRUCT *)kmalloc(
...
{
	LOG_DBG("hold_reg(%d), BufAmount(%d)",
...
static long ISP_REF_CNT_CTRL_FUNC(unsigned long Param)
{
	...
	if (copy_from_user(&ref_cnt_ctrl, (void __user *)Param,
		...
		if (g_IspInfo.DebugMask & ISP_DBG_REF_CNT_CTRL) {
			LOG_DBG("ctrl(%d),id(%d)", ref_cnt_ctrl.ctrl,
				ref_cnt_ctrl.id);
		}
		...
		if (ref_cnt_ctrl.id < ISP_REF_CNT_ID_MAX) {
			...
			case ISP_REF_CNT_DEC_AND_RESET_IF_LAST_ONE:
				...
				break;
			default:
			case ISP_REF_CNT_MAX: /* Add this to remove build
						 warning. */
				/* Do nothing. */
				break;
			}
			...
			if (imem_ref_cnt == 0)
				LOG_DBG_IF("g_imem_ref_cnt[%d]: %d.",
					   ref_cnt_ctrl.id, imem_ref_cnt);

			if ((imem_ref_cnt == 0) &&
			    (ref_cnt_ctrl.ctrl ==
			     ISP_REF_CNT_DEC_AND_RESET_IF_LAST_ONE)) { /* No
									  user
									  left
									  and
									  ctrl
			...
			if (copy_to_user((void __user *)ref_cnt_ctrl.data_ptr,
					 &imem_ref_cnt,
					 sizeof(signed int)) != 0) {
...
/* spin_lock_irqsave(&(g_IspInfo.SpinLockRTBC),g_Flash_SpinLock); */
	...
	if (buffer_exist) {
		...
		if (g_pstRTBuf->ring_buf[rt_dma].total_count ==
		    ISP_RT_BUF_SIZE) {
			LOG_ERR("[rtbc][ENQUE]:buffer full(%d)",
				g_pstRTBuf->ring_buf[rt_dma].total_count);
		} else {
			/* first time add */
			index = g_pstRTBuf->ring_buf[rt_dma].total_count %
				ISP_RT_BUF_SIZE;

			g_pstRTBuf->ring_buf[rt_dma].data[index].memID =
				g_rt_buf_info.memID;
			g_pstRTBuf->ring_buf[rt_dma].data[index].size =
				g_rt_buf_info.size;
			g_pstRTBuf->ring_buf[rt_dma].data[index].base_vAddr =
				g_rt_buf_info.base_vAddr;
			g_pstRTBuf->ring_buf[rt_dma].data[index].base_pAddr =
				g_rt_buf_info.base_pAddr;
			g_pstRTBuf->ring_buf[rt_dma].data[index].bFilled =
				ISP_RTBC_BUF_EMPTY;
			...
			LOG_DBG("[rtbc][ENQUE]:dma(%d),index(%d),PA(0x%x),empty(%d),total(%d)",
				rt_dma, index, g_pstRTBuf->ring_buf[rt_dma]
						       .data[index]
						       .base_pAddr,
				g_pstRTBuf->ring_buf[rt_dma].empty_count,
				g_pstRTBuf->ring_buf[rt_dma].total_count);
...
static signed int ISP_RTBC_DEQUE(signed int dma)
{
	...
	for (i = 0; i < g_pstRTBuf->ring_buf[rt_dma].total_count; i++) {
		index = (g_pstRTBuf->ring_buf[rt_dma].start + i) %
			g_pstRTBuf->ring_buf[rt_dma].total_count;
	...
	if (g_deque_buf.count == 0) {
		/* queue buffer status */
		LOG_DBG("[rtbc][DEQUE]:dma(%d),start(%d),total(%d),empty(%d), g_deque_buf.count(%d)",
			rt_dma, g_pstRTBuf->ring_buf[rt_dma].start,
			g_pstRTBuf->ring_buf[rt_dma].total_count,
			g_pstRTBuf->ring_buf[rt_dma].empty_count,
			g_deque_buf.count);

		for (i = 0; i <= g_pstRTBuf->ring_buf[rt_dma].total_count - 1;
		     i++) {
			LOG_DBG("[rtbc][DEQUE]Buf List:i(%d),memID(%d),"
				"size(0x%x),VA(0x%llx),PA(0x%x),bFilled(%d)",
	...
	if (g_deque_buf.count) {
		...
		for (i = 0; i < g_deque_buf.count; i++) {
			g_deque_buf.data[i].memID = g_pstRTBuf->ring_buf[rt_dma]
							    .data[index + i]
							    .memID;
			g_deque_buf.data[i].size = g_pstRTBuf->ring_buf[rt_dma]
							   .data[index + i]
							   .size;
	...
	if (copy_from_user(&rt_buf_ctrl, (void __user *)Param,
		...
		if (g_IspInfo.DebugMask & ISP_DBG_INT) {
			LOG_DBG("[rtbc]ctrl(0x%x),buf_id(0x%x),data_ptr(0x%p),ex_data_ptr(0x%p)",
				rt_buf_ctrl.ctrl, rt_buf_ctrl.buf_id,
				rt_buf_ctrl.data_ptr, rt_buf_ctrl.ex_data_ptr);
		}
		...
		case ISP_RT_BUF_CTRL_EXCHANGE_ENQUE:
			if (copy_from_user(
				...
				if ((reg_val & 0x01) || (reg_val2 & 0x01)) {
							...
							for (i = 0;
								...
								if (g_pstRTBuf
									...
									g_oldImgoAddr =
										g_rt_buf_info
											.base_pAddr;
									LOG_DBG("[rtbc]dma(%d),old(%d),PA(0x%x),VA(0x%llx)",
										rt_dma,
										i,
										g_pstRTBuf
											->ring_buf
									...
									LOG_DBG("[rtbc]dma(%d),new(%d),PA(0x%x),VA(0x%llx)",
										rt_dma,
										i,
										g_pstRTBuf
											->ring_buf
									...
									if (rt_dma ==
									    _imgo_) {
										curr_pa = ISP_RD32(
											(void *)ISP_REG_ADDR_IMGO_BASE_ADDR);
										LOG_DBG("[rtbc][EXG]o,curr_pa(0x%x),old(0x%x)",
											curr_pa,
											g_rt_buf_info
												.base_pAddr);
										LOG_DBG("[rtbc][EXG]o,NUM(0x%x),CNT(0x%x)",
											imgo_fbc.Bits
												.FB_NUM,
											imgo_fbc.Bits
												.FBC_CNT);
					...
					/**/					} else {	/* timing issue */
						...
						g_newImgoAddr =
							g_pstRTBuf
								...
								.data[i]
								.base_pAddr;
						LOG_DBG("[rtbc][EXG]o,g_old(0x%x),g_new(0x%x)",
							g_oldImgoAddr,
							g_newImgoAddr);
					}
					/**/				} else if (rt_dma ==
										   _img2o_) {
						curr_pa = ISP_RD32((
							void *)ISP_REG_ADDR_IMG2O_BASE_ADDR);
						LOG_DBG("[rtbc][EXG]2o,curr_pa(0x%x),old(0x%x)",
							curr_pa,
							g_rt_buf_info
								.base_pAddr);
						LOG_DBG("[rtbc][EXG]2o,NUM(0x%x),CNT(0x%x)",
					...
					/**/					} else {	/* timing issue */
						...
						g_newImg2oAddr =
							g_pstRTBuf
								...
								.data[i]
								.base_pAddr;
						LOG_DBG("[rtbc][EXG]2o,g_old(0x%x),g_new(0x%x)",
							g_oldImg2oAddr,
							g_newImg2oAddr);
					}
					}
					...
					if (g_IspInfo.DebugMask &
					    ISP_DBG_BUF_CTRL) {
						LOG_DBG("[rtbc][ENQ]:dma(%d),P(0x%x),O(0x%x),2O(0x%x),fps(%d)us",
							rt_dma,
							g_rt_buf_info
								.base_pAddr,
							ISP_RD32((
					...
					if (g_IspInfo.DebugMask &
					    ISP_DBG_BUF_CTRL) {
						LOG_DBG("[rtbc][ENQUE]+:wait enque done.(0x%x)(0x%x)",
							reg_val, reg_val2);
					}
		...
		case ISP_RT_BUF_CTRL_DEQUE:
			...
			if ((reg_val & 0x01) || (reg_val2 & 0x01)) {
				...
				for (i = 0; i < g_deque_buf.count; i++) {
					 ...
					 * ISP_DBG_BUF_CTRL) */
					/* { */
					LOG_DBG("[rtbc][DEQ]:T\"%d.%06d\",co(0x%08lx),c2o(0x%08lx),i(%d),iBuf+i(%d),V(0x%llx),P(0x%x),O(0x%x),2O(0x%x)",
						g_deque_buf.data[i].timeStampS,
						g_deque_buf.data[i].timeStampUs,
						imgo_fbc.Reg_val,
					...
					/* tstamp =
					 * g_deque_buf.data[i].timeStampS*1000000+g_deque_buf.data[i].timeStampUs;
					 */
						...
						if ((g_prv_tstamp_s >
						    ...
						    ((g_prv_tstamp_s ==
						     ...
						     (g_prv_tstamp_us >=
						      g_deque_buf.data[i]
							      .timeStampUs))) {
							LOG_ERR("[rtbc]TS rollback,prv\"%d.%06d\",cur\"%d.%06d\"",
								g_prv_tstamp_s,
								g_prv_tstamp_us,
								g_deque_buf
									.data[i]
		...
		case ISP_RT_BUF_CTRL_IS_RDY:
...
#if defined(_rtbc_use_cq0c_)
			bWaitBufRdy =
				((_imgo_ == rt_dma) ? (imgo_fbc.Bits.FBC_CNT)
						    : (img2o_fbc.Bits.FBC_CNT))
					? 0
					: 1;
#else
			...
			if (g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
				LOG_DBG("[rtbc][IS_RDY]:bWaitBufRdy(%d)",
					bWaitBufRdy);
			}
		...
		case ISP_RT_BUF_CTRL_GET_SIZE:
			...
			if (g_IspInfo.DebugMask & ISP_DBG_INT) {
				LOG_DBG("[rtbc][GET_SIZE]:rt_dma(%d)/size(%d)",
					rt_dma, size);
			}
...
static signed int ISP_SOF_Buf_Get(unsigned long long sec, unsigned long usec)
{
	...
	union CQ_RTBC_FBC imgo_fbc;
	union CQ_RTBC_FBC img2o_fbc;
	unsigned int curr_imgo =
		0; /* (imgo_fbc.Bits.WCNT+imgo_fbc.Bits.FB_NUM-1)%imgo_fbc.Bits.FB_NUM;
		      //[0,1,2,...] */
	unsigned int curr_img2o =
		0; /* (img2o_fbc.Bits.WCNT+img2o_fbc.Bits.FB_NUM-1)%img2o_fbc.Bits.FB_NUM;
		      //[0,1,2,...] */
	...
	if (g_pstRTBuf->dropCnt == 0) {
		...
		if (g_TempAddr == curr_pa) {
			LOG_DBG("[WARNING]g_TempAddr:Last(0x%08X) == Cur(0x%08X)",
				g_TempAddr, curr_pa);
			/* ISP_DumpReg(); */
		}
		 ...
		 */
		if (ISP_RT_CQ0C_BUF_SIZE <
		    g_pstRTBuf->ring_buf[_imgo_].total_count) {
			LOG_ERR("[rtbc]buf cnt(%d)",
				g_pstRTBuf->ring_buf[_imgo_].total_count);
			g_pstRTBuf->ring_buf[_imgo_].total_count =
		...
		if (curr_pa !=
			...
			for (i = 0;
			     i < g_pstRTBuf->ring_buf[_imgo_].total_count;
			     i++) {
				if (curr_pa ==
		...
		g_pstRTBuf->ring_buf[_img2o_].data[curr_img2o].timeStampUs =
			usec;
		...
		if (g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
			LOG_DBG("[rtbc]TStamp(%d.%06d),curr(%d),pa(0x%x/0x%x),o2o(0x%x/0x%x),fbc_o(0x%08x),fbc_2o(0x%08x),cq0c(0x%x)",
				...
				g_pstRTBuf->ring_buf[_imgo_]
					.data[curr_imgo]
					.timeStampUs,
				curr_imgo, g_pstRTBuf->ring_buf[_imgo_]
						   .data[curr_imgo]
						   .base_pAddr,
	...
	if (1000000 < ((sec * 1000000 + usec) - g_start_time)) {
		if (g_sof_count != 0) {
			g_avg_frame_time =
				(sec * 1000000 + usec) - g_start_time;
			g_avg_frame_time =
				(unsigned int)(g_avg_frame_time / g_sof_count);
		}
		g_sof_count = 0;
	} else {
		g_sof_count++;
	...
	for (i = 0; i <= 1; i++) {
		...
		if (g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
			...
			for (i = 0;
			     i <= g_pstRTBuf->ring_buf[i_dma].total_count - 1;
			     i++) {
				LOG_DBG("[rtbc][SOF]Buf List:%d/%d/0x%x/0x%x/0x%x/%d/  ",
					i, g_pstRTBuf->ring_buf[i_dma]
						   .data[i]
						   .memID,
		...
		if (g_pstRTBuf->ring_buf[i_dma].empty_count == 0) {
			/* once if buffer put into queue between SOF and
			 * ISP_DONE. */
			g_pstRTBuf->ring_buf[i_dma].active = MFALSE;

			if (g_IspInfo.DebugMask & ISP_DBG_INT) {
				LOG_DBG("[rtbc][SOF]:dma(%d)real time buffer number empty,start(%d) ",
					i_dma,
			...
			if (g_pstRTBuf->ring_buf[i_dma].empty_count >= 2) {
				/* next buffer */
				next = (g_pstRTBuf->ring_buf[i_dma].start + 1) %
				       g_pstRTBuf->ring_buf[i_dma].total_count;
				...
				if (g_IspInfo.DebugMask & ISP_DBG_INT) {
					LOG_DBG("[rtbc][SOF]:dma(%d)real time buffer number is running out ",
						i_dma);
				}
			}
...
static signed int ISP_DONE_Buf_Time(unsigned long long sec, unsigned long usec)
{
	...
	for (i = 0; i <= 1; i++) {
		i_dma = (i == 0) ? _imgo_ : _img2o_;
		reg_fbc = (_imgo_ == i_dma) ? ISP_REG_ADDR_IMGO_FBC
					    : ISP_REG_ADDR_IMG2O_FBC;

		if (g_pstRTBuf->ring_buf[i_dma].empty_count == 0) {
			if (g_IspInfo.DebugMask & ISP_DBG_INT) {
				LOG_DBG("[rtbc][DONE]:dma(%d)real time buffer number empty,start(%d)",
					i_dma,
					g_pstRTBuf->ring_buf[i_dma].start);
			}
		...
		while (1) {
			...
			if (g_IspInfo.DebugMask & ISP_DBG_INT) {
				LOG_DBG("i_dma(%d),curr(%d),bFilled(%d)", i_dma,
					curr, g_pstRTBuf->ring_buf[i_dma]
						      .data[curr]
						      .bFilled);
			}
			...
			if (loopCount >
				...
				LOG_ERR("Can't find empty dma(%d) buf in total_count(%d)",
					i_dma, g_pstRTBuf->ring_buf[i_dma]
						       .total_count);
				break;
			}
		}
		...
		if (g_pstRTBuf->ring_buf[i_dma].empty_count == 0) {
			...
			if (g_IspInfo.DebugMask & ISP_DBG_BUF_CTRL) {
				LOG_DBG("[rtbc][DONE]:dma(%d),en fbc(0x%x) stalled DMA out",
					i_dma, ISP_RD32((void *)reg_fbc));
			}
		}
		...
		if (g_IspInfo.DebugMask & ISP_DBG_INT) {
			LOG_DBG("[rtbc][DONE]:dma(%d),curr(%d),sec(%lld),usec(%ld) ",
				i_dma, curr, sec, usec);
		}
	}
...
static signed int ISP_WaitIrq(struct ISP_WAIT_IRQ_STRUCT WaitIrq)
{
	...
	while (1) {
		...
		for (i = 0; i < ISP_IRQ_TYPE_AMOUNT; i++) {
			if (g_IspInfo.IrqInfo.ErrMask[i] & IrqStatus[i]) {
				/* LOG_ERR("Error IRQ, Type(%d),
				 * Status(0x%08X)",i,g_IspInfo.IrqInfo.ErrMask[i]
				 * & IrqStatus[i]); */
				/* TODO: Add error handler... */
			}

			if (g_IspInfo.DebugMask & ISP_DBG_INT) {
				LOG_DBG("Type(%d), IrqStatus(0x%08X | 0x%08X)",
					i, g_IspInfo.IrqInfo.Status[i],
					IrqStatus[i]);
				/* LOG_DBG("Mask(0x%08X),
				 * ErrMask(0x%08X)",g_IspInfo.IrqInfo.Mask[i],
	...
	if (Timeout == 0) {
		...
		LOG_ERR("start(%u.%06u),end(%u.%06u)", startTimeS, startTimeUS,
			endTimeS, endTimeUS);
	...
	if (g_IspInfo.DebugMask & ISP_DBG_INT) {
		for (i = 0; i < ISP_IRQ_TYPE_AMOUNT; i++) {
			LOG_DBG("Type(%d), IrqStatus(0x%08X)", i,
				g_IspInfo.IrqInfo.Status[i]);
		}
	}
...
static __tcmfunc irqreturn_t ISP_Irq(signed int Irq, void *DeviceId)
{
	...
	if (!CAM_HAL_VER_IS3) {
		...
		for (i = 0; i < ISP_IRQ_TYPE_AMOUNT; i++) {
			if (g_IspInfo.IrqInfo.ErrMask[i] & IrqStatus[i]) {
				/* ISP_IRQ_INTX_STATUS_IMGO_ERR_ST: on-the-fly
				 * imgo error, not really overrun */
				if (i != ISP_IRQ_TYPE_INTX ||
				    (g_IspInfo.IrqInfo.ErrMask[i] &
				     IrqStatus[i] &
				     (~ISP_IRQ_INTX_STATUS_IMGO_ERR_ST))) {
					if (i !=
					    ISP_IRQ_TYPE_INTX) { /* to reduce
								    isp log */
						LOG_DBG("Error IRQ, Type(%d), Status(0x%08X)",
							i,
							g_IspInfo.IrqInfo.ErrMask
									[i] &
								IrqStatus[i]);
					}
					/* TODO: Add error handler... */
		...
		/*if (
		   IrqStatus[ISP_IRQ_TYPE_INTX]&ISP_IRQ_INTX_STATUS_DMA_ERR_ST )
		   {

		   #if defined(_debug_dma_err_)
		   LOG_ERR("[rtbc]StatusX(0x%08X),
		   DMA_ERR",IrqStatus[ISP_IRQ_TYPE_INTX]);  //IMGO may overrun
		   before setting smi regs

		   //allan
		   DMAErrHandler();

		   LOG_DBG("ERR (8018 0x%08x)(8024 0x%08x)(802C 0x%08x)(4440
		   0x%08x) (810C 0x%08x)",
		   ISP_RD32(0xF5008018),
		   ISP_RD32(0xF5008024),
		   ISP_RD32(0xF500802C),
		...
		/*  */
		/* if (
		 * IrqStatus[ISP_IRQ_TYPE_INT]&ISP_IRQ_INT_STATUS_TG1_ERR_ST ||
		 */
		...
		/* LOG_ERR("[rtbc]dropframe
		 * st(0x%08X),co(0x%08X),c2o(0x%08X)",IrqStatus_fbc_int,ISP_RD32(ISP_REG_ADDR_IMGO_FBC),ISP_RD32(ISP_REG_ADDR_IMG2O_FBC));
		 */
		/* } */
		...
		if (IrqStatus[ISP_IRQ_TYPE_INT] &
			...
			if (g_newImgoAddr != DEFAULT_PA &&
				...
				LOG_DBG("[rtbc][TG1_DON]o,tg_pa(0x%x),old(0x%x)",
					tg_pa, g_oldImgoAddr);
			...
			if (g_newImg2oAddr != DEFAULT_PA &&
				...
				LOG_DBG("[rtbc][TG1_DON]2o,tg_pa(0x%x),old(0x%x)",
					tg_pa, g_oldImg2oAddr);
...
/* what's this? */
		...
		if (IrqStatus[ISP_IRQ_TYPE_INT] &
			...
			if ((sof_pa != 0) && (sof_pa == g_oldImgoAddr) &&
			    ...
			    ((imgo_fbc.Bits.FB_NUM == imgo_fbc.Bits.FBC_CNT) ||
			     (imgo_fbc.Bits.FBC_CNT ==
			      (imgo_fbc.Bits.FB_NUM - 1)))) {
				LOG_DBG("[rtbc][SOF]o,sof_pa(0x%x),old(0x%x)",
					sof_pa, g_oldImgoAddr);
				LOG_DBG("[rtbc][SOF]o,NUM(0x%x),CNT(0x%x)",
					imgo_fbc.Bits.FB_NUM,
					imgo_fbc.Bits.FBC_CNT);
				LOG_DBG("[rtbc][SOF]o,new(0x%x)",
					g_newImgoAddr);
			...
			if ((sof_pa != 0) && (sof_pa == g_oldImg2oAddr) &&
			    ...
			    ((img2o_fbc.Bits.FB_NUM ==
			     ...
			     (img2o_fbc.Bits.FBC_CNT ==
			      (img2o_fbc.Bits.FB_NUM - 1)))) {
				LOG_DBG("[rtbc][SOF]2o,sof_pa(0x%x),old(0x%x)",
					sof_pa, g_oldImg2oAddr);
				LOG_DBG("[rtbc][SOF]2o,NUM(0x%x),CNT(0x%x)",
					img2o_fbc.Bits.FB_NUM,
					img2o_fbc.Bits.FBC_CNT);
				LOG_DBG("[rtbc][SOF]2o,new(0x%x/0x%x)", sof_pa,
					g_newImg2oAddr);
		...
		for (j = 0; j < ISP_IRQ_TYPE_AMOUNT_FRMB; j++) {
			for (i = 0; i < IRQ_USER_NUM_MAX; i++) {
				...
				/* 2. update signal occuring time and passed by
				 * signal count */
				...
				} else { /* no any interrupt is not marked and
		...
		wake_up_interruptible(
			&IspInfo_FrmB.WaitQueueHead); /* could move to
							 ISP_Irq_FrmB */
	}
	 ...
	 * queue function. */
	if (IrqStatus[ISP_IRQ_TYPE_INT] &
	    (ISP_IRQ_INT_STATUS_VS1_ST)) { /* |ISP_IRQ_INT_STATUS_VS2_ST)) */
		g_IspInfo.TimeLog.Vd = ISP_JiffiesToMs(jiffies);
		schedule_work(&g_IspInfo.ScheduleWorkVD);
		tasklet_schedule(&IspTaskletVD);
	}

	/* Tasklet. It is uninterrupted, so there can NOT be "Sleep" in tasklet
	 * function. */
	if (IrqStatus[ISP_IRQ_TYPE_INT] &
	    (ISP_IRQ_INT_STATUS_EXPDON1_ST)) { /* |ISP_IRQ_INT_STATUS_EXPDON2_ST))
						  */
...
static long ISP_ioctl(struct file *pFile, unsigned int Cmd, unsigned long Param)
{
	...
	int userKey = -1;
	/*signed int burstQNum;  unused, mark to fix build error */
	...
	case ISP_WAKELOCK_CTRL: {
		if (copy_from_user(&wakelock_ctrl, (void __user *)Param,
				   sizeof(unsigned int)) != 0) {
	...
	case ISP_READ_REGISTER: {
		if (copy_from_user(&RegIo, (void __user *)Param,
	...
	case ISP_HOLD_REG_TIME: {
		if (copy_from_user(&HoldTime, (void __user *)Param,
	...
	case ISP_HOLD_REG: {
		if (copy_from_user(&HoldEnable, (void __user *)Param,
				   sizeof(bool)) == 0) {
	...
	case ISP_WAIT_IRQ: {
		if (copy_from_user(&WaitIrq, (void __user *)Param,
	...
	case ISP_READ_IRQ: {
		if (copy_from_user(&ReadIrq, (void __user *)Param,
			...
			if (copy_to_user((void __user *)Param, &ReadIrq,
					 sizeof(struct ISP_READ_IRQ_STRUCT)) !=
			    0) {
	...
	case ISP_CLEAR_IRQ: {
		if (copy_from_user(&ClearIrq, (void __user *)Param,
...
#if 0 /* Old method, already move setting pclk to seninf->kd_sensorlist.c:     \
	 setMCLK() */
			unsigned int senFreq = 0;

			if (copy_from_user(&senFreq, (void __user *)Param, sizeof(unsigned int)) == 0) {
	...
	case ISP_SET_CAM_VERSION: {
		...
		if (copy_from_user(&bIspVer, (void __user *)Param,
				   sizeof(bool)) == 0) {
			spin_lock_irqsave(&SpinLockCamHaVer, flags);
	...
	case ISP_WAIT_IRQ_FRMB:
		if (copy_from_user(&Irq_FrmB, (void __user *)Param,
				   sizeof(struct ISP_WAIT_IRQ_STRUCT_FRMB)) ==
		    0) {
			if ((Irq_FrmB.UserInfo.UserKey >= IRQ_USER_NUM_MAX) ||
			    (Irq_FrmB.UserInfo.UserKey <= 0)) {
				/* LOG_ERR("invalid userKey(%d),
				 * max(%d)",WaitIrq_FrmB.UserInfo.UserKey,IRQ_USER_NUM_MAX);
				 */
				Irq_FrmB.UserInfo.UserKey = 0;
			}
			if ((Irq_FrmB.UserInfo.Type >=
	...
	case ISP_ED_QUEBUF_CTRL_FRMB:
		if (copy_from_user(&edQueBuf_FrmB, (void __user *)Param,
				   sizeof(struct ISP_ED_BUFQUE_STRUCT_FRMB)) ==
		    0) {
	...
	case ISP_GET_CUR_SOF_FRMB:
		if (copy_from_user(&DebugFlag[0], (void __user *)Param,
			...
			switch (DebugFlag[0]) {
			case _IRQ:
				/* TG sof update at expo_done, causes potential
				 * timing issue */
				/* DebugFlag[1] =
		...
		if (copy_to_user((void __user *)Param, &DebugFlag[1],
				 sizeof(unsigned int)) != 0) {
	...
	case ISP_GET_INT_ERR_FRMB:
		if (copy_to_user((void __user *)Param, g_ISPIntErr,
	...
	case ISP_REGISTER_IRQ_USER_KEY:
		if (copy_from_user(
			    ...
			    sizeof(struct ISP_REGISTER_USERKEY_STRUCT_FRMB)) ==
		    0) {
	...
	case ISP_FLUSH_IRQ_REQUEST:
		if (copy_from_user(&Irq_FrmB, (void __user *)Param,
			...
			/* if((Irq_FrmB.UserInfo.UserKey>=IRQ_USER_NUM_MAX) ||
			 * (Irq_FrmB.UserInfo.UserKey<1)) */
			/* UserKey(0) is Isp drv, flush ISP EnqueThread SOF
			 * signal */
			...
			LOG_INF("User_%s(%d), type(%d)",
				IrqUserKey_UserInfo[Irq_FrmB.UserInfo.UserKey]
					.userName,
				Irq_FrmB.UserInfo.UserKey,
				Irq_FrmB.UserInfo.Type);
...
static int compat_get_isp_buf_ctrl_frmb_struct_data(
	struct compat_ISP_BUFFER_CTRL_STRUCT_FRMB __user *data32,
	struct ISP_BUFFER_CTRL_STRUCT_FRMB __user *data)
{
	compat_uint_t tmp;
	compat_uptr_t uptr;
	int err = 0;
...
static int compat_get_isp_register_irq_user_key_data(
	struct compat_ISP_REGISTER_USERKEY_STRUCT_FRMB __user *data32,
	struct ISP_REGISTER_USERKEY_STRUCT_FRMB __user *data)
{
	compat_uint_t tmp;
	compat_uptr_t uptr;
	int err = 0;
			     ...
			     unsigned long arg)
{
	...
	case COMPAT_ISP_BUFFER_CTRL: {
		if (CAM_HAL_VER_IS3) {
			struct compat_ISP_BUFFER_CTRL_STRUCT_FRMB __user
				*data32;
			struct ISP_BUFFER_CTRL_STRUCT_FRMB __user *data;
			int err;
			...
			err = compat_get_isp_buf_ctrl_frmb_struct_data(data32,
								       data);
			if (err) {
			...
			ret = filp->f_op->unlocked_ioctl(filp, ISP_BUFFER_CTRL,
							 (unsigned long)data);
			err = compat_put_isp_buf_ctrl_frmb_struct_data(data32,
								       data);
			if (err) {
	...
	case COMPAT_ISP_REGISTER_IRQ_USER_KEY: {
		struct compat_ISP_REGISTER_USERKEY_STRUCT_FRMB __user *data32;
		struct ISP_REGISTER_USERKEY_STRUCT_FRMB *data;
		int err;
	...
	case COMPAT_ISP_SET_USER_PID: {
		ret = filp->f_op->unlocked_ioctl(
			filp, ISP_SET_USER_PID, (unsigned long)compat_ptr(arg));
		return ret;
	}
#endif
	...
	case COMPAT_ISP_DEBUG_FLAG_FRMB: {
		ret = filp->f_op->unlocked_ioctl(
			filp, ISP_DEBUG_FLAG_FRMB,
			(unsigned long)compat_ptr(arg));
		return ret;
	}
	case COMPAT_ISP_SENSOR_FREQ_CTRL: {
		ret = filp->f_op->unlocked_ioctl(
			filp, ISP_SENSOR_FREQ_CTRL,
			(unsigned long)compat_ptr(arg));
		return ret;
	}
	case COMPAT_ISP_GET_CUR_SOF_FRMB: {
		ret = filp->f_op->unlocked_ioctl(
			filp, ISP_GET_CUR_SOF_FRMB,
			(unsigned long)compat_ptr(arg));
		return ret;
	}
	case COMPAT_ISP_GET_INT_ERR_FRMB: {
		ret = filp->f_op->unlocked_ioctl(
			filp, ISP_GET_INT_ERR_FRMB,
			(unsigned long)compat_ptr(arg));
		return ret;
	}
...
static signed int ISP_open(struct inode *pInode, struct file *pFile)
{
	...
	LOG_DBG_IF("+,UserCount(%d), g_EnableClkCnt(%d)", g_IspInfo.UserCount,
		   atomic_read(&g_EnableClkCnt));
	...
	if (g_IspInfo.UserCount > 0) {
		LOG_DBG_IF(
			...
			g_IspInfo.UserCount, current->comm, current->pid,
			current->tgid);
		goto EXIT;
	}
	...
	g_IspInfo.BufInfo.Read.pData =
		(unsigned char *)kmalloc(ISP_BUF_SIZE, GFP_ATOMIC);
...
EXIT:
	...
	if (Ret < 0) {
		...
		if (g_IspInfo.UserCount == 1)
			LOG_DBG_IF(
				...
				g_IspInfo.UserCount, current->comm,
				current->pid, current->tgid);

		ISP_EnableClock(MTRUE, 1);
		LOG_DBG_IF("isp open g_EnableClkCnt:	%d",
			   atomic_read(&g_EnableClkCnt));
	}
	...
	LOG_DBG_IF("-,Ret(%d),UserCount(%d), g_EnableClkCnt(%d)", Ret,
		   g_IspInfo.UserCount, atomic_read(&g_EnableClkCnt));
...
static signed int ISP_release(struct inode *pInode, struct file *pFile)
{
	...
	if (g_IspInfo.UserCount > 0) {
		spin_unlock(&(g_IspInfo.SpinLockIspRef));
		LOG_DBG_IF(
			"+ Curr UserCount(%d->%d), (process, pid, tgid)=(%s, %d, %d), users exist",
			(g_IspInfo.UserCount + 1), g_IspInfo.UserCount,
			current->comm, current->pid, current->tgid);
		goto EXIT;
	} else {
		spin_unlock(&(g_IspInfo.SpinLockIspRef));
	...
	LOG_INF("+ Curr UserCount(%d->%d), (process, pid, tgid)=(%s, %d, %d), last user",
		(g_IspInfo.UserCount + 1), g_IspInfo.UserCount, current->comm,
		current->pid, current->tgid);
...
static signed int mmap_kmem(struct file *filp, struct vm_area_struct *vma)
{
	...
	/* check length - do not allow larger mappings than the number of  pages
	 * allocated */
	...
	if (pVma->vm_pgoff == RT_BUF_TBL_NPAGES) {
		...
		LOG_DBG
		    ("pVma->vm_pgoff(0x%x),phy(0x%x),pVmapVma->vm_start(0x%x),pVma->vm_end(0x%x),length(0x%x)",
		     (unsigned int)(pVma->vm_pgoff), (unsigned
		int)(pVma->vm_pgoff << PAGE_SHIFT),
		     (unsigned int)(pVma->vm_start), (unsigned
		int)(pVma->vm_end),
		     (unsigned int)(length));
		*/
		...
		case MIPIRX_CONFIG_ADDR:
			if (length > MIPIRX_CONFIG_RANGE) {
				LOG_ERR("mmap range error : length(0x%x),MIPIRX_CONFIG_RANGE(0x%x)!",
					(unsigned int)(length),
					MIPIRX_CONFIG_RANGE);
				return -EAGAIN;
			}
		...
		case MIPIRX_ANALOG_ADDR:
			if (length > MIPIRX_ANALOG_RANGE) {
				LOG_ERR("mmap range error : length(0x%x),MIPIRX_ANALOG_RANGE(0x%x)!",
					(unsigned int)(length),
					MIPIRX_ANALOG_RANGE);
				return -EAGAIN;
			}
...
static unsigned long of_get_nodeVA(const char *comp_str)
{
	...
	if (dev_node == NULL) {
		...
		ret_baseaddr =
			(unsigned long)of_iomap(plat_dev->dev.of_node, 0);
		LOG_DBG("(%s)ret_baseaddr PA=0x%lx, VA = 0x%lx", comp_str,
			of_getISPPA(plat_dev->dev.of_node, 0), ret_baseaddr);
	}
	return ret_baseaddr;
}
...
static signed int ISP_probe(struct platform_device *pDev)
{
	...
	for (i = 0; i < ISP_CAM_BASEADDR_NUM; i++) {
		...
		i, pDev->dev.of_node->name, gISPSYS_Reg[i],
		gISPSYS_Reg_FrmB[i]);*/
		LOG_INF("DT, i=%d, PA(0x%lx), map_VA=0x%lx/0x%lx\n", i,
			of_getISPPA(pDev->dev.of_node, i), gISPSYS_Reg[i],
			gISPSYS_Reg_FrmB[i]);
	}
		...
		if (i == ISP_CAM0_IRQ_IDX) {
			Ret =
				request_irq(
					...
					IRQF_TRIGGER_NONE,
					"ISP", NULL); /* IRQF_TRIGGER_NONE dose
							 not take effect here,
							 real trigger mode set
							 in dts file */
		}
		if (Ret) {
			dev_err(&pDev->dev,
	...
	for (i = 0; i < 1;
	     i++) { /* NEED_TUNING_BY_CHIP. 1: Only one IORESOURCE_MEM type
		       resource in kernel\mt_devs.c\mt_resource_isp[]. */
		...
		pRes = request_mem_region(
			pRes->start, pRes->end - pRes->start + 1, pDev->name);
		if (pRes == NULL) {
...
static signed int ISP_suspend(struct platform_device *pDev, pm_message_t Mesg)
{
	...
	LOG_INF("g_bPass1_On_In_Resume_TG1(%d),regTG1Val(0x%08x)",
		g_bPass1_On_In_Resume_TG1, regTG1Val);
...
static signed int ISP_resume(struct platform_device *pDev)
{
	...
	LOG_INF("g_bPass1_On_In_Resume_TG1(%d),regTG1Val(0x%x)",
		g_bPass1_On_In_Resume_TG1, regTG1Val);
...

{
	...
	LOG_DBG("pFile(%p),pBuffer(%p),Count(%ld)", pFile, pBuffer,
		(long int)Count);
	...
	if (sscanf(RegBuf, "%x %x", &Addr, &Data) == 2) {
		if ((ISP_ADDR_CAMINF + Addr >= ISP_ADDR) &&
		    (ISP_ADDR_CAMINF + Addr <
		     (ISP_ADDR_CAMINF + ISP_REG_RANGE))) {
...
*
*******************************************************************************/
static ssize_t CAMIO_RegDebug(struct file *pFile, const char __user *pBuffer,
			      size_t Count, loff_t *ppos)
{
#if 0
	char RegBuf[64];
	unsigned int CopyBufSize = (Count < (sizeof(RegBuf) - 1)) ? (Count) : (sizeof(RegBuf) - 1);
	unsigned int Addr = 0;
	unsigned int Data = 0;
...
#define SENINF1_EN_MASK (1 << SENINF1_MUX_EN_SHIFT)
...
void ISP_set_mclk1(int clkcnt)
{
	int clkr_edge = 0, clk_pol = 0, pad_clkinv = 0;
	int clkf_pol = !(clkcnt & 0x1);
	int clkf_edge = clkcnt > 1 ? ((clkcnt + 1) >> 1) : 1;

	pr_err("camera: setting up mclk\n");
...
{
	LOG_DBG_IF("ISP_MDPClockOnCallback => ISP_EnableClock(ON) +++");
...
{
	LOG_DBG_IF("ISP_MDPClockOffCallback => ISP_EnableClock(OFF) ---");
...
enum m4u_callback_ret_t ISP_M4U_TranslationFault_callback(int port, unsigned int
	mva, void *data)
{
	LOG_DBG("[ISP_M4U]fault call port=%d, mva=0x%x", port, mva);

	LOG_DBG("[TF_%d]0x%08X %08X", port, (unsigned int)(ISP_TPIPE_ADDR +
0x0004),
...
0x007c),
	...
	switch (port) {
	case M4U_PORT_IMGO:
		LOG_DBG("[TF_IMGO]0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR +
0x0300),
...
0x031C),
		...
		break;
	case M4U_PORT_IMG2O:
		LOG_DBG("[TF_IMG2O]0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR +
0x0320),
...
0x033C),
		...
		break;
	case M4U_PORT_LSCI:
		LOG_DBG("[TF_LSCI]0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR +
0x00A8),
...
0x027c),
		...
		break;
	case M4U_PORT_IMGI:
		LOG_DBG("[TF_IMGI]0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR +
0x0050),
...
0x0240),
		...
		break;
	case M4U_PORT_ESFKO:
		LOG_DBG("[TF_ESFKO]0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR +
0x035C),
...
0x037C),
		...
		break;
	case M4U_PORT_AAO:
		LOG_DBG("[TF_AAO]0x%08X %08X", (unsigned int)(ISP_TPIPE_ADDR +
0x0388),
...
static signed int __init ISP_Init(void)
{
	...
	proc_create("driver/isp_reg", 0000, NULL, &g_isp_reg_fops);
	proc_create("driver/camio_reg", 0000, NULL, &g_camio_reg_fops);
#else
...
#endif
	/* allocate a memory area with kmalloc. Will be rounded up to a page
	 * boundary */
	...
	g_pTbl_RTBuf = (signed int *)((((unsigned long)g_pBuf_kmalloc) +
				       PAGE_SIZE - 1) &
				      PAGE_MASK);
	...
	for (i = 0; i < RT_BUF_TBL_NPAGES * PAGE_SIZE; i += PAGE_SIZE) {
		SetPageReserved(
			virt_to_page(((unsigned long)g_pTbl_RTBuf) + i));
	}
...
/*
	...
	and power off(put_sync) in late_init to make sure ISP HW engine is
   workable.
