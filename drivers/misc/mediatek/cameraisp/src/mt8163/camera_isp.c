...
#define LOG_DBG_IF(format, args...)	pr_info(MyTag "[%s]" format "\n", __func__, ##args)
#else
#define LOG_DBG_IF(format, args...)	pr_debug(MyTag "[%s]" format "\n", __func__, ##args)
#endif
/*******************************************************************************
* defined marco
********************************************************************************/
...
static long ISP_REF_CNT_CTRL_FUNC(unsigned long Param)
{
	...
	if (copy_from_user(&ref_cnt_ctrl, (void __user *)Param, sizeof(ISP_REF_CNT_CTRL_STRUCT)) ==
		...
		if (ISP_REF_CNT_ID_MAX > ref_cnt_ctrl.id) {
			...
			if (imem_ref_cnt == 0)
				LOG_DBG_IF("g_imem_ref_cnt[%d]: %d.", ref_cnt_ctrl.id, imem_ref_cnt);
			...
			if ((imem_ref_cnt == 0) && (ref_cnt_ctrl.ctrl == ISP_REF_CNT_DEC_AND_RESET_IF_LAST_ONE)) {	/* No user left and ctrl is RESET_IF_LAST_ONE, do ISP reset. */
				ISP_Reset();
				LOG_DBG_IF("ISP_REF_CNT_DEC_AND_RESET_IF_LAST_ONE. Do ISP_Reset");
			}
...
static MINT32 ISP_open(struct inode *pInode, struct file *pFile)
{
	...
	if (g_IspInfo.UserCount > 0) {
		LOG_DBG_IF("Curr UserCount(%d), (process, pid, tgid)=(%s, %d, %d), users exist",
...
EXIT:
	...
	if (Ret < 0) {
		...
		if (1 == g_IspInfo.UserCount)
			LOG_DBG_IF("Curr UserCount(%d), (process, pid, tgid)=(%s, %d, %d), first user",
			g_IspInfo.UserCount, current->comm, current->pid, current->tgid);
...
static MINT32 ISP_release(struct inode *pInode, struct file *pFile)
{
	...
	if (g_IspInfo.UserCount > 0) {
		spin_unlock(&(g_IspInfo.SpinLockIspRef));
		LOG_DBG_IF("+ Curr UserCount(%d->%d), (process, pid, tgid)=(%s, %d, %d), users exist",
...
static MINT32 ISP_suspend(struct platform_device *pDev, pm_message_t Mesg)
{
	...
	LOG_INF("g_bPass1_On_In_Resume_TG1(%d),regTG1Val(0x%08x)", g_bPass1_On_In_Resume_TG1,
		regTG1Val);
...
static MINT32 ISP_resume(struct platform_device *pDev)
{
	...
	LOG_INF("g_bPass1_On_In_Resume_TG1(%d),regTG1Val(0x%x)", g_bPass1_On_In_Resume_TG1,
		regTG1Val);
...
void ISP_set_mclk1(int clkcnt)
{
	...
	/* set PCEN */
	isp_wr32_mask(SENINF_TG1_PH_CNT, SENINF_TG1_PCEN_MASK, SENINF_TG1_PCEN_SHIFT, 1);

	/* Clear top clock gating in SENINF_TOP_CTRL, the case where pcen = 1  */
	isp_wr32_mask(SENINF_TOP_CTRL, SENINF12_PCLK_MASK, 0, SENINF12_PCLK_VAL);
	...
	/* TGCLK_SEL = 1 */
	isp_wr32_mask(SENINF_TG1_PH_CNT, SENINF_TG1_TGCLK_SEL_MASK, SENINF_TG1_TGCLK_SEL_SHIFT, 1);

	isp_wr32_mask(SENINF_TG1_PH_CNT, SENINF_TG1_CLKFL_POL_MASK, SENINF_TG1_CLKFL_POL_SHIFT, clkf_pol);

	isp_wr32_mask(SENINF_TG1_PH_CNT, SENINF_TG1_PADCLK_INV_MASK, SENINF_TG1_PADCLK_INV_SHIFT, pad_clkinv);

	isp_wr32_mask(SENINF_TG1_PH_CNT, SENINF_TG1_CLK_POL_MASK, SENINF_TG1_CLK_POL_SHIFT, clk_pol);

	/* Set SENINF1_MUX_EN */
	isp_wr32_mask(SENINF1_MUX_CTRL, SENINF1_MUX_EN_MASK, SENINF1_MUX_EN_SHIFT, 1);
