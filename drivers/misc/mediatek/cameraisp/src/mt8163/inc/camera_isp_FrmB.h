...
#define T_STAMP_2_0 /* time stamp workaround method. (increase timestamp       \
		       baseon fix fps, not read at each isr) */
...
struct ISP_IRQ_TIME_STRUCT_FRMB {
	unsigned int
		tLastSig_sec; /* time stamp of the latest occuring signal */
	unsigned int
		tLastSig_usec; /* time stamp of the latest occuring signal */
	unsigned int tMark2WaitSig_sec; /* time period from marking a signal to
					   user try to wait and get the signal
					   */
	unsigned int
		tMark2WaitSig_usec; /* time period from marking a signal to user
				       try to wait and get the signal */
	unsigned int tLastSig2GetSig_sec;  /* time period from latest occuring
					      signal to user try to wait and get
					      the signal */
	unsigned int tLastSig2GetSig_usec; /* time period from latest occuring
					      signal to user try to wait and get
					      the signal */
...
struct ISP_EIS_META_STRUCT {
	unsigned int tLastSOF2P1done_sec;  /* time stamp of the last closest
					      occuring sof signal for pass1 done
					      */
	unsigned int tLastSOF2P1done_usec; /* time stamp of the last closest
					      occuring sof signal for pass1 done
					      */
};
...
struct ISP_WAIT_IRQ_STRUCT_FRMB {
	...
	unsigned int
		Timeout; /* time out for waiting for a specific interrupt */
...
struct ISP_RT_RING_BUF_INFO_STRUCT_FRMB {
	...
	unsigned int
		total_count; /* total buffer number.Include Filled and empty */
	unsigned int empty_count; /* total empty buffer number include current
				     DMA accessing buffer */
...
enum ISP_ED_BUFQUE_CTRL_ENUM {
	...
	ISP_ED_BUFQUE_CTRL_WAIT_DEQUE,    /* 1,a dequeue thread is waiting to do
					     dequeue */
	ISP_ED_BUFQUE_CTRL_DEQUE_SUCCESS, /* 2,signal that a buffer is dequeued
					     (success) */
	ISP_ED_BUFQUE_CTRL_DEQUE_FAIL,    /* 3,signal that a buffer is dequeued
					     (fail) */
	ISP_ED_BUFQUE_CTRL_WAIT_FRAME,    /* 4,wait for a specific buffer */
	ISP_ED_BUFQUE_CTRL_WAKE_WAITFRAME, /* 5,wake all sleeped users to check
...
enum ISP_CMD_ENUM_FRMB {
	...
	ISP_CMD_REGISTER_IRQ_FRMB =
		ISP_CMD_SENSOR_FREQ_CTRL + 1, /* register for a specific irq */
	...
	ISP_CMD_REGISTER_IRQ_USER_KEY, /* register for a user key to do irq
					  operation */
	ISP_CMD_MARK_IRQ_REQUEST, /* mark for a specific register befor wait for
	...
	ISP_CMD_GET_DROP_FRAME_FRMB, /* dump current frame informaiton, 1 for
					drop frmae, 2 for last working frame */
};
/*  */
/* #define ISP_RESET           _IO  (ISP_MAGIC, ISP_CMD_RESET) */
/* #define ISP_RESET_BUF       _IO  (ISP_MAGIC, ISP_CMD_RESET_BUF) */
/* #define ISP_READ_REGISTER   _IOWR(ISP_MAGIC, ISP_CMD_READ_REG,
 * ISP_REG_IO_STRUCT) */
/* #define ISP_WRITE_REGISTER  _IOWR(ISP_MAGIC, ISP_CMD_WRITE_REG,
 * ISP_REG_IO_STRUCT) */
/* #define ISP_HOLD_REG_TIME   _IOW (ISP_MAGIC, ISP_CMD_HOLD_TIME,
 * ISP_HOLD_TIME_ENUM) */
/* #define ISP_HOLD_REG        _IOW (ISP_MAGIC, ISP_CMD_HOLD_REG,      bool) */
/* #define ISP_WAIT_IRQ        _IOW (ISP_MAGIC, ISP_CMD_WAIT_IRQ,
 * ISP_WAIT_IRQ_STRUCT) */
/* #define ISP_READ_IRQ        _IOR (ISP_MAGIC, ISP_CMD_READ_IRQ,
 * ISP_READ_IRQ_STRUCT) */
/* #define ISP_CLEAR_IRQ       _IOW (ISP_MAGIC, ISP_CMD_CLEAR_IRQ,
 * ISP_CLEAR_IRQ_STRUCT) */
/* #define ISP_DUMP_REG        _IO  (ISP_MAGIC, ISP_CMD_DUMP_REG) */
/* #define ISP_SET_USER_PID    _IOW (ISP_MAGIC, ISP_CMD_SET_USER_PID,
 * unsigned long) */
/* #define ISP_BUFFER_CTRL     _IOWR(ISP_MAGIC, ISP_CMD_RT_BUF_CTRL,
 * ISP_BUFFER_CTRL_STRUCT) */
/* #define ISP_REF_CNT_CTRL    _IOWR(ISP_MAGIC, ISP_CMD_REF_CNT,
 * ISP_REF_CNT_CTRL_STRUCT) */
#define ISP_DEBUG_FLAG_FRMB                                                    \
	_IOW(ISP_MAGIC, ISP_CMD_DEBUG_FLAG_FRMB, unsigned long)
/* #define ISP_SENSOR_FREQ_CTRL  _IOW (ISP_MAGIC, ISP_CMD_SENSOR_FREQ_CTRL,
 * unsigned long) */
#define ISP_REGISTER_IRQ_FRMB                                                  \
	_IOW(ISP_MAGIC, ISP_CMD_REGISTER_IRQ_FRMB,                             \
	     struct ISP_WAIT_IRQ_STRUCT_FRMB)
#define ISP_UNREGISTER_IRQ_FRMB                                                \
	_IOW(ISP_MAGIC, ISP_CMD_UNREGISTER_IRQ_FRMB,                           \
	     struct ISP_WAIT_IRQ_STRUCT_FRMB)
#define ISP_WAIT_IRQ_FRMB                                                      \
	_IOW(ISP_MAGIC, ISP_CMD_WAIT_IRQ_FRMB, struct ISP_WAIT_IRQ_STRUCT_FRMB)
#define ISP_ED_QUEBUF_CTRL_FRMB                                                \
	_IOWR(ISP_MAGIC, ISP_CMD_ED_QUEBUF_CTRL_FRMB,                          \
	      struct ISP_ED_BUFQUE_STRUCT_FRMB)
#define ISP_UPDATE_REGSCEN_FRMB                                                \
	_IOWR(ISP_MAGIC, ISP_CMD_UPDATE_REGSCEN_FRMB, unsigned int)
#define ISP_QUERY_REGSCEN_FRMB                                                 \
	_IOR(ISP_MAGIC, ISP_CMD_QUERY_REGSCEN_FRMB, unsigned int)
#define ISP_UPDATE_BURSTQNUM_FRMB                                              \
	_IOW(ISP_MAGIC, ISP_CMD_UPDATE_BURSTQNUM_FRMB, int)
#define ISP_QUERY_BURSTQNUM_FRMB                                               \
	_IOR(ISP_MAGIC, ISP_CMD_QUERY_BURSTQNUM_FRMB, int)
#define ISP_DUMP_ISR_LOG_FRMB _IO(ISP_MAGIC, ISP_CMD_DUMP_ISR_LOG_FRMB)
#define ISP_GET_CUR_SOF_FRMB                                                   \
	_IOWR(ISP_MAGIC, ISP_CMD_GET_CUR_SOF_FRMB, unsigned long)
#define ISP_GET_DMA_ERR_FRMB                                                   \
	_IOWR(ISP_MAGIC, ISP_CMD_GET_DMA_ERR_FRMB, unsigned int)
#define ISP_GET_INT_ERR_FRMB                                                   \
	_IOR(ISP_MAGIC, ISP_CMD_GET_INT_ERR_FRMB, unsigned long)
#ifdef T_STAMP_2_0
#define ISP_SET_FPS_FRMB _IOW(ISP_MAGIC, ISP_CMD_SET_FPS_FRMB, unsigned int)
#endif
#define ISP_GET_DROP_FRAME_FRMB                                                \
	_IOWR(ISP_MAGIC, ISP_CMD_GET_DROP_FRAME_FRMB, unsigned int)
#define ISP_REGISTER_IRQ_USER_KEY                                              \
	_IOWR(ISP_MAGIC, ISP_CMD_REGISTER_IRQ_USER_KEY,                        \
	      struct ISP_REGISTER_USERKEY_STRUCT_FRMB)
...
#ifdef CONFIG_COMPAT
#define COMPAT_ISP_DEBUG_FLAG_FRMB                                             \
	_IOW(ISP_MAGIC, ISP_CMD_DEBUG_FLAG_FRMB, compat_uptr_t)
#define COMPAT_ISP_REGISTER_IRQ_USER_KEY                                       \
	_IOWR(ISP_MAGIC, ISP_CMD_REGISTER_IRQ_USER_KEY,                        \
	      struct compat_ISP_REGISTER_USERKEY_STRUCT_FRMB)
#define COMPAT_ISP_GET_DMA_ERR_FRMB                                            \
	_IOWR(ISP_MAGIC, ISP_CMD_GET_DMA_ERR_FRMB, compat_uptr_t)
#define COMPAT_ISP_GET_INT_ERR_FRMB                                            \
	_IOR(ISP_MAGIC, ISP_CMD_GET_INT_ERR_FRMB, compat_uptr_t)
#define COMPAT_ISP_WAIT_IRQ                                                    \
	_IOW(ISP_MAGIC, ISP_CMD_WAIT_IRQ_FRMB,                                 \
	     struct compat_ISP_WAIT_IRQ_STRUCT_FRMB)
...
#define COMPAT_ISP_FLUSH_IRQ_REQUEST                                           \
	_IOW(ISP_MAGIC, ISP_CMD_FLUSH_IRQ_REQUEST,                             \
	     struct compat_ISP_WAIT_IRQ_STRUCT_FRMB)
#define COMPAT_ISP_GET_CUR_SOF_FRMB                                            \
	_IOWR(ISP_MAGIC, ISP_CMD_GET_CUR_SOF_FRMB, compat_uptr_t)
#endif
...
/* int32_t ISP_BeginGCECallback(uint32_t taskID, uint32_t *regCount, uint32_t
 * **regAddress); */
/* int32_t ISP_EndGCECallback(uint32_t taskID, uint32_t regCount, uint32_t
 * *regValues); */
/*  */
 ...
 */
/* static int ISP_RTBC_DEQUE_FRMB(int dma,ISP_DEQUE_BUF_INFO_STRUCT*
 * pdeque_buf); */
/* static long ISP_Buf_CTRL_FUNC_FRMB(unsigned int Param); */
/* static int ISP_SOF_Buf_Get_FRMB(eISPIrq irqT,unsigned long long sec,unsigned
 * long usec,bool bDrop); */
/* static int ISP_DONE_Buf_Time_FrmB(eISPIrq irqT,unsigned long long
 * sec,unsigned long usec); */
