...
#define CMDQ_MAX_HIGH_PRIORITY_THREAD_COUNT                                    \
	(7) /* Thread that are high-priority (display threads) */
...
enum CMDQ_HW_THREAD_PRIORITY_ENUM {
	...
	CMDQ_THR_PRIO_DISPLAY_TRIGGER =
		2, /* trigger loop (enables display mutex) */
...
enum CMDQ_SCENARIO_ENUM {
	...
	/* for screen capture to wait for RDMA-done without blocking config
	 * thread
	 */
	...
	/* [phased out]client from user space, so the cmd buffer is in user
	 * space.
	 */
	...
	CMDQ_SCENARIO_HIGHP_TRIGGER_LOOP =
		35, /* for primary trigger loop enable pre-fetch usage */
	CMDQ_SCENARIO_LOWP_TRIGGER_LOOP =
		36, /* for low priority monitor loop to polling bus status */
...
struct cmdqReadRegStruct {
	...
	/* an array of 32-bit register addresses
	 * (uint32_t)
	 */
...
struct cmdqRegValueStruct {
	...
	/* in the same order as struct cmdqReadRegStruct */
...
struct cmdqReadAddressStruct {
	...
	/*  */
	/* indeed param dmaAddresses should be UNSIGNED LONG type for 64 bit
	 * kernel.
	 */
	...
	cmdqU32Ptr_t
		values; /* [OUT] uint32_t values that dmaAddresses point into */
};
...
enum CMDQ_SEC_ADDR_METADATA_TYPE {
	...
	CMDQ_SAM_DDP_REG_HDCP =
		3, /* DDP register needs to set opposite value when HDCP fail */
	CMDQ_SAM_NMVA2_MVA_REVERSE =
		4, /* reverse map normal MVA to secure world */
};

struct cmdqSecAddrMetadataStruct {
	/* [IN]_d, index of instruction. Update its arg_b value to real PA/MVA
	 * in secure world
	 */
	uint32_t instrIndex;
...
struct cmdqSecDataStruct {
	/* [IN]true for secure command */
	bool is_secure;

	/* address metadata, used to translate secure buffer PA related
	 * instruction in secure world
	 */
	/* [IN] count of element in addrList */
	uint32_t addrMetadataCount;

	/* [IN] array of struct cmdqSecAddrMetadataStruct */
	...
	/* task index in thread's tasklist. -1 for not in
	 * tasklist.
	 */
	int32_t waitCookie;
...
struct cmdqProfileMarkerStruct {
	uint32_t count;
	/* i.e. cmdqBackupSlotHandle, physical start address of
	 * backup slot
	 */
	long long hSlot;
...
struct cmdqCommandStruct {
	...
	/* [Reserved] This is for CMDQ driver usage itself. Not for client. Do
	 * not access this field from User Space
	 */
...
enum CMDQ_CAP_BITS {
	/* bit 0: TRUE if WFE instruction support is ready. FALSE if we need to
	 * POLL instead.
	 */
	CMDQ_CAP_WFE = 0,
};
