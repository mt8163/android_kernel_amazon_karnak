					      ...
					      const uint32_t size)
{
	void *pNewBuf = NULL;
	void *pOriginalBuf =
		(void *)CMDQ_U32_PTR(handle->secData.addrMetadatas);
	const uint32_t originalSize = sizeof(struct cmdqSecAddrMetadataStruct) *
	...
	handle->secData.addrMetadataMaxCount =
		size / sizeof(struct cmdqSecAddrMetadataStruct);
	...
	cmdqRecHandle *pHandle)
{
	...
	*pHandle = NULL;
	...
	/* CMD */
	if (0 !=
	    cmdq_rec_realloc_cmd_buffer(handle, CMDQ_INITIAL_CMD_BLOCK_SIZE)) {
		kfree(handle);
		return -ENOMEM;
	}
...
int32_t cmdq_check_before_append(cmdqRecHandle handle)
{
	...
	if ((handle->blockSize + 32) >= handle->bufferSize) {
		if (0 !=
		    cmdq_rec_realloc_cmd_buffer(handle, handle->bufferSize * 2))
			return -ENOMEM;
	}
 ...
 *     same as cmdq_append_command
 */
static int32_t cmdq_append_wpr_command(cmdqRecHandle handle,
				       enum CMDQ_CODE_ENUM code, uint32_t arg_a,
				       uint32_t arg_b, uint32_t arg_a_type,
				       uint32_t arg_b_type)
{
	int32_t status = 0;
	int32_t subsys;
	...
	if (arg_a_type == 0) {
		...
		if (subsys == CMDQ_SPECIAL_SUBSYS_ADDR) {
			...
			*p_command++ = (CMDQ_CODE_MOVE << 24) |
			...
			new_arg_a =
				((CMDQ_DATA_REG_DEBUG & 0x1f) << subsys_bit);
			if (arg_a & 0x1) {
	...
	/* new_arg_a is the HW register address to access from or GPR value
	 * store the HW register address
	 */
			    ...
			    uint32_t arg_b_type)
{
	...
	if (code != CMDQ_CODE_EOC &&
		...
		int32_t threadNo = cmdq_get_func()->getThreadID(
	...
	/* we must re-calculate current PC because we may already insert MARKER
	 * inst.
	 */
	p_command =
		(uint32_t *)((uint8_t *)handle->pBuffer + handle->blockSize);
	...
	CMDQ_VERBOSE(
		"REC: 0x%p CMD: 0x%p, op: 0x%02x, arg_a: 0x%08x, arg_b: 0x%08x\n",
		handle, p_command, code, arg_a, arg_b);

	switch (code) {
	case CMDQ_CODE_READ:
	case CMDQ_CODE_WRITE:
	case CMDQ_CODE_POLL:
		/* Because read/write/poll have similar format, handle them
		 * together
		 */
...
int32_t cmdq_task_set_secure_mode(cmdqRecHandle handle,
...
int32_t cmdq_task_secure_enable_dapc(cmdqRecHandle handle,
...
int32_t cmdq_task_secure_enable_port_security(cmdqRecHandle handle,
					      const uint64_t engineFlag)
{
#ifdef CMDQ_SECURE_PATH_SUPPORT
	if (handle == NULL)
				 ...
				 uint32_t size, uint32_t port)
{
	...
	writeInstrIndex =
		(handle->blockSize) / CMDQ_INST_SIZE - 1; /* start from 0 */
...
int32_t cmdq_op_poll(cmdqRecHandle handle, uint32_t addr, uint32_t value,
		     uint32_t mask)
{
	int32_t status;
...
int32_t cmdq_op_wait(cmdqRecHandle handle, enum CMDQ_EVENT_ENUM event)
{
	int32_t arg_a;
...
int32_t cmdq_op_wait_no_clear(cmdqRecHandle handle, enum CMDQ_EVENT_ENUM event)
{
	int32_t arg_a;
	...
	return cmdq_append_command(handle, CMDQ_CODE_WAIT_NO_CLEAR, arg_a, 0, 0,
				   0);
}

int32_t cmdq_op_clear_event(cmdqRecHandle handle, enum CMDQ_EVENT_ENUM event)
{
	int32_t arg_a;
...
int32_t cmdq_op_set_event(cmdqRecHandle handle, enum CMDQ_EVENT_ENUM event)
{
	int32_t arg_a;
			  ...
			  uint32_t slot_index, uint32_t *value)
{
	...
	*value = cmdqCoreReadWriteAddress(h_backup_slot +
			   ...
			   uint32_t slot_index, uint32_t value)
{
	...
	status = cmdqCoreWriteWriteAddress(
		h_backup_slot + slot_index * sizeof(uint32_t), value);
	return status;
				...
				uint32_t slot_index, uint32_t addr)
{
	...
	const dma_addr_t dramAddr =
		h_backup_slot + slot_index * sizeof(uint32_t);
	uint32_t highAddr = 0;
	...
	if (cmdq_core_subsys_from_phys_addr(addr) != CMDQ_SPECIAL_SUBSYS_ADDR) {
		...
		cmdq_append_command(handle, CMDQ_CODE_READ, addr, valueRegId, 0,
				    1);
	} else {
		/*
		...
		/* set GPR to address */
		cmdq_append_command(handle, CMDQ_CODE_MOVE, valueRegId, addr, 0,
				    0);

		/* read data from address in GPR to GPR */
		cmdq_append_command(handle, CMDQ_CODE_READ, valueRegId,
				    valueRegId, 1, 1);
	}
	...
	cmdq_append_command(handle, CMDQ_CODE_MOVE,
			    highAddr | ((destRegId & 0x1f) << 16) | (4 << 21),
			    (uint32_t)dramAddr, 0, 0);

	/* write value in GPR to memory pointed by GPR */
	cmdq_append_command(handle, CMDQ_CODE_WRITE, destRegId, valueRegId, 1,
			    1);
				...
				uint32_t slot_index, uint32_t addr)
{
	...
	CMDQ_GET_HIGH_ADDR(dramAddr, highAddr);
	cmdq_append_command(handle, CMDQ_CODE_MOVE,
			    highAddr | ((addrRegId & 0x1f) << 16) | (4 << 21),
			    (uint32_t)dramAddr, 0, 0); /* arg_a is GPR */

	/* 2. read value from src address, which is stroed in GPR, to valueRegId
	 */
	cmdq_append_command(handle, CMDQ_CODE_READ, addrRegId, valueRegId, 1,
			    1);
...
int32_t cmdq_op_write_mem(cmdqRecHandle handle,
			  cmdqBackupSlotHandle h_backup_slot,
			  uint32_t slot_index, uint32_t value)
{
#ifdef CMDQ_GPR_SUPPORT
	...
	arg_a = (CMDQ_CODE_MOVE << 24) | (valueRegId << 16) |
		(4 << 21); /* arg_a is GPR */
...
int32_t cmdq_op_finalize_command(cmdqRecHandle handle, bool loop)
{
	...
	if (!handle->finalized) {
		...
		/* no generate IRQ for loop thread to save
		 * power
		 */
		if (loop == true &&
		    cmdq_get_func()->force_loop_irq(handle->scenario) == false)
			arg_b = 0x0;
#endif

		status = cmdq_append_command(handle, CMDQ_CODE_EOC, 0, arg_b, 0,
					     0);
		 ...
		 */
		status = cmdq_append_command(
			handle, CMDQ_CODE_JUMP, 0, /* not absolute */
...
int32_t cmdq_task_flush(cmdqRecHandle handle)
{
	...
	CMDQ_MSG(
		"Submit task scenario: %d, priority: %d, engine: 0x%llx, buffer: 0x%p, size: %d\n",
		handle->scenario, handle->priority, handle->engineFlag,
		handle->pBuffer, handle->blockSize);
...
int32_t cmdq_task_flush_and_read_register(cmdqRecHandle handle,
					  uint32_t regCount,
					  uint32_t *addrArray,
					  uint32_t *valueArray)
{
...
int32_t cmdq_task_flush_async_callback(cmdqRecHandle handle,
				       CmdqAsyncFlushCB callback,
				       uint32_t userData)
{
	int32_t status = 0;
	struct cmdqCommandStruct desc = {0};
...
{
	return cmdq_task_start_loop_callback(handle, &cmdq_dummy_irq_callback,
					     0);
}

int32_t cmdq_task_start_loop_callback(cmdqRecHandle handle,
				      ...
				      unsigned long loopData)
{
	...
	CMDQ_MSG(
		...
		handle->scenario, handle->priority, handle->engineFlag);
	CMDQ_MSG(
		"Submit task loop: buffer: 0x%p, size: %d, callback: 0x%p, data: %ld\n",
	...
	status = cmdqCoreSubmitTaskAsync(&desc, loopCB, loopData,
					 &handle->pRunningTask);
	return status;
}
...
int32_t cmdq_op_profile_marker(cmdqRecHandle handle, const char *tag)
{
	...
	do {
		...
		/* those timestamp info will copy to record strute after task
		 * execute done
		 */
		if ((handle->profileMarker.count == 0) &&
		    (handle->profileMarker.hSlot == 0)) {
		...
		cmdq_op_read_reg_to_mem(handle, hSlot, index,
					CMDQ_APXGPT2_COUNT);
...
int32_t cmdq_task_dump_command(cmdqRecHandle handle)
{
	...
	if (pTask) {
		...
		CMDQ_LOG("REC 0x%p command buffer TRANSLATED:\n",
			 handle->pBuffer);
		for (i = 0, pCmd = handle->pBuffer; i < handle->blockSize;
		     i += 8, pCmd += 2) {
...
int32_t cmdq_task_estimate_command_exec_time(const cmdqRecHandle handle)
{
	...
	time = cmdq_prof_estimate_command_exe_time(handle->pBuffer,
						   handle->blockSize);
...
int32_t cmdq_op_set_nop(cmdqRecHandle handle, uint32_t index)
{
	...
	if (handle == NULL ||
	    offsetIndex > (handle->blockSize - CMDQ_INST_SIZE))
		return -EFAULT;
			       ...
			       enum CMDQ_EVENT_ENUM event)
{
	...
	for (QueryIndex = startIndex; QueryIndex < MaxIndex; QueryIndex++) {
		p_command = (uint32_t *)((uint8_t *)handle->pBuffer +
					 QueryIndex * CMDQ_INST_SIZE);
...
int32_t cmdq_resource_acquire(cmdqRecHandle handle,
...
int32_t cmdq_resource_acquire_and_write(cmdqRecHandle handle,
					enum CMDQ_EVENT_ENUM resourceEvent,
...
int32_t cmdq_resource_release(cmdqRecHandle handle,
			      enum CMDQ_EVENT_ENUM resourceEvent)
{
...
int32_t cmdq_resource_release_and_write(cmdqRecHandle handle,
					enum CMDQ_EVENT_ENUM resourceEvent,
					uint32_t addr, uint32_t value,
					uint32_t mask)
{
...
int32_t cmdqRecSecureEnablePortSecurity(cmdqRecHandle handle,
					const uint64_t engineFlag)
{
...
int32_t cmdqRecMark(cmdqRecHandle handle)
{
	...
	/* bit 48: do not increase CMD_COUNTER (because this is not the end of
	 * the task)
	 */
...
int32_t cmdqRecWrite(cmdqRecHandle handle, uint32_t addr, uint32_t value,
		     uint32_t mask)
{
...
int32_t cmdqRecPoll(cmdqRecHandle handle, uint32_t addr, uint32_t value,
		    uint32_t mask)
{
...
int32_t cmdqBackupAllocateSlot(cmdqBackupSlotHandle *p_h_backup_slot,
			       uint32_t slotCount)
{
...
int32_t cmdqRecFlushAsyncCallback(cmdqRecHandle handle,
				  CmdqAsyncFlushCB callback, uint32_t userData)
{
...
int32_t cmdqRecStartLoopWithCallback(cmdqRecHandle handle,
				     CmdqInterruptCB loopCB,
				     unsigned long loopData)
{
...
int32_t cmdqRecAcquireResource(cmdqRecHandle handle,
			       enum CMDQ_EVENT_ENUM resourceEvent)
{
	return cmdq_resource_acquire(handle, resourceEvent);
}

int32_t cmdqRecWriteForResource(cmdqRecHandle handle,
	enum CMDQ_EVENT_ENUM resourceEvent, uint32_t addr,
...
int32_t cmdqRecReleaseResource(cmdqRecHandle handle,
			       enum CMDQ_EVENT_ENUM resourceEvent)
{
	return cmdq_resource_release(handle, resourceEvent);
}

int32_t cmdqRecWriteAndReleaseResource(cmdqRecHandle handle,
				       enum CMDQ_EVENT_ENUM resourceEvent,
				       uint32_t addr, uint32_t value,
				       uint32_t mask)
{
