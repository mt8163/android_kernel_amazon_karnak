...
#define IMP_ENABLE_MDP_HW_CLOCK(FN_NAME, HW_NAME)                              \
	...
	{                                                                      \
		return cmdq_dev_enable_device_clock(                           \
			enable, gCmdqMdpModuleClock.clk_##HW_NAME,             \
			#HW_NAME "-clk");                                      \
	}
#define IMP_MDP_HW_CLOCK_IS_ENABLE(FN_NAME, HW_NAME)                           \
	bool cmdq_mdp_clock_is_enable_##FN_NAME(void)                          \
...
int32_t cmdq_mdp_reset_with_mmsys(const uint64_t engineToResetAgain)
{
	...
	static const int engineResetBit[32] = {
		...
		/* bit 13 : MDP_CROP, note that we use JPEG_ENC since they are
		 * tightly coupled
		 */
		CMDQ_ENG_JPEG_ENC,
		[14 ... 31] = -1};
...
#if 0
m4u_callback_ret_t cmdq_M4U_TranslationFault_callback(int port, unsigned
int	mva, void *data)
{
...
void cmdq_mdp_init_module_base_VA(void)
{
	...
	gCmdqMdpModuleBaseVA.MDP_RDMA0 = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8173-mdp_rdma0");
	gCmdqMdpModuleBaseVA.MDP_RDMA1 = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8173-mdp_rdma1");
	gCmdqMdpModuleBaseVA.MDP_RSZ0 = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8173-mdp_rsz0");
	gCmdqMdpModuleBaseVA.MDP_RSZ1 = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8173-mdp_rsz1");
	gCmdqMdpModuleBaseVA.MDP_RSZ2 = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8173-mdp_rsz2");
	gCmdqMdpModuleBaseVA.MDP_TDSHP0 = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8173-mdp_tdshp0");
	gCmdqMdpModuleBaseVA.MDP_TDSHP1 = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8173-mdp_tdshp1");
	gCmdqMdpModuleBaseVA.MDP_WROT0 = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8173-mdp_wrot0");
	gCmdqMdpModuleBaseVA.MDP_WROT1 = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8173-mdp_wrot1");
	gCmdqMdpModuleBaseVA.MDP_WDMA = cmdq_dev_alloc_module_base_VA_by_name(
		"mediatek,mt8173-mdp_wdma");
/* gCmdqMdpModuleBaseVA.MDP_COLOR =
 * cmdq_dev_alloc_module_base_VA_by_name("mediatek,mdp_color");
 */
/* gCmdqMdpModuleBaseVA.VENC =
 * cmdq_dev_alloc_module_base_VA_by_name("mediatek,mt6797-venc");
 */
#endif
}
...
{
	cmdq_dev_get_module_PA("mediatek,mt8173-disp_mutex", 0, startPA, endPA);
}
#endif
...
void cmdq_mdp_init_module_clk(void)
{
	...
	cmdq_dev_get_module_clock_by_name("mediatek,mt8173-gce",
					  "MT_CG_DISP0_CAM_MDP",
...
struct MODULE_BASE {
	uint64_t engine;
	/* considering 64 bit kernel, use long type to store base
	 * addr
	 */
	long base;
	const char *name;
...
int32_t cmdqMdpDumpInfo(uint64_t engineFlag, int logLevel)
{
	...
	if (logLevel >= 1) {
		...
		for (inner = 0; inner < (ARRAY_SIZE(bases));
		     ++inner) {
...
int32_t cmdqMdpResetEng(uint64_t engineFlag)
{
	...
	if (engineFlag & (1LL << CMDQ_ENG_MDP_RDMA0)) {
		...
		status = cmdq_mdp_loop_reset(
			CMDQ_ENG_MDP_RDMA0, MDP_RDMA0_BASE + 0x8,
	...
	if (engineFlag & (1LL << CMDQ_ENG_MDP_RDMA1)) {
		...
		status = cmdq_mdp_loop_reset(
			CMDQ_ENG_MDP_RDMA1, MDP_RDMA1_BASE + 0x8,
	...
	if (engineFlag & (1LL << CMDQ_ENG_MDP_WROT0)) {
		status = cmdq_mdp_loop_reset(
			CMDQ_ENG_MDP_WROT0, MDP_WROT0_BASE + 0x010,
	...
	if (engineFlag & (1LL << CMDQ_ENG_MDP_WROT1)) {
		status = cmdq_mdp_loop_reset(
			CMDQ_ENG_MDP_WROT1, MDP_WROT1_BASE + 0x010,
	...
	if (engineFlag & (1LL << CMDQ_ENG_MDP_WDMA)) {
		status = cmdq_mdp_loop_reset(
			CMDQ_ENG_MDP_WDMA, MDP_WDMA_BASE + 0x00C,
...
int32_t cmdqMdpClockOff(uint64_t engineFlag)
{
	...
	if (engineFlag & (1LL << CMDQ_ENG_MDP_WDMA)) {
		cmdq_mdp_loop_off(CMDQ_ENG_MDP_WDMA, MDP_WDMA_BASE + 0x00C,
				  MDP_WDMA_BASE + 0X0A0, 0x3FF, 0x1, false);
	}

	if (engineFlag & (1LL << CMDQ_ENG_MDP_WROT1)) {
		cmdq_mdp_loop_off(CMDQ_ENG_MDP_WROT1, MDP_WROT1_BASE + 0X010,
				  MDP_WROT1_BASE + 0X014, 0x1, 0x0, false);
	}

	if (engineFlag & (1LL << CMDQ_ENG_MDP_WROT0)) {
		cmdq_mdp_loop_off(CMDQ_ENG_MDP_WROT0, MDP_WROT0_BASE + 0X010,
				  MDP_WROT0_BASE + 0X014, 0x1, 0x0, false);
	}
	...
	if (engineFlag & (1LL << CMDQ_ENG_MDP_RDMA1)) {
		cmdq_mdp_loop_off(CMDQ_ENG_MDP_RDMA1, MDP_RDMA1_BASE + 0x008,
				  MDP_RDMA1_BASE + 0x408, 0x7FF00, 0x100,
				  false);
	}

	if (engineFlag & (1LL << CMDQ_ENG_MDP_RDMA0)) {
		cmdq_mdp_loop_off(CMDQ_ENG_MDP_RDMA0, MDP_RDMA0_BASE + 0x008,
				  MDP_RDMA0_BASE + 0x408, 0x7FF00, 0x100,
				  false);
	}
...
void testcase_clkmgr_mdp(void)
{
	...
	testcase_clkmgr_impl(
		CMDQ_ENG_MDP_WDMA, "CMDQ_TEST_MDP_WDMA",
	...
	testcase_clkmgr_impl(CMDQ_ENG_MDP_TDSHP0, "CMDQ_TEST_MDP_TDSHP0",
			     MDP_TDSHP0_BASE + 0x244, 0xAACCBBDD,
			     MDP_TDSHP0_BASE + 0x244, true);

	testcase_clkmgr_impl(CMDQ_ENG_MDP_TDSHP1, "CMDQ_TEST_MDP_TDSHP1",
			     MDP_TDSHP1_BASE + 0x244, 0xAACCBBDD,
			     MDP_TDSHP1_BASE + 0x244, true);
	...
	testcase_clkmgr_impl(CMDQ_ENG_MDP_RSZ2, "CMDQ_TEST_MDP_RSZ2",
			     MDP_RSZ2_BASE + 0x040, 0x00000001,
			     MDP_RSZ2_BASE + 0x044, false);
#endif /* defined(CMDQ_USE_CCF) */
}
