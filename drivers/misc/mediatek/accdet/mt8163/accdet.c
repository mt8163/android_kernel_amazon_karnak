...
#ifdef CONFIG_ACCDET_OMTP_CTIA_SWITCH_MANUAL
static void accdet_switch_manual(bool plug_in, int type)
{
/* -------------------------------------------------------------------------------------
...
static void accdet_switch_manual_work(void)
{
	int accdet_adc = 0;
...
static irqreturn_t accdet_eint_func(int irq, void *data)
{
	...
	if (cur_eint_state == EINT_PIN_PLUG_IN) {
		 ...
		 * we set the polarity back as we initialized. */
		 ...
		 * we set the opposite polarity to what we initialized. */
		if (accdet_eint_type == IRQ_TYPE_LEVEL_HIGH)
			irq_set_irq_type(accdet_irq, IRQ_TYPE_LEVEL_LOW);
		else
			irq_set_irq_type(accdet_irq, IRQ_TYPE_LEVEL_HIGH);
...
MODULE_DESCRIPTION("Audio accdet Driver");
MODULE_LICENSE("GPL");
