...
static int micbias1_individual_ctrl;
...
void accdet_auxadc_switch(int enable)
{
	...
	if (enable) {
		...
		/*currently mic_voltage_delay is 80ms,this is too long for us.*/
		/* As we checked, 20ms shoudl be okay on Karnak... anyway, */
		/* we use half of the mic_voltage_delay here */
		if (needDelay)
			msleep(mic_voltage_delay/2);
		/*ACCDET_DEBUG("ACCDET enable switch\n");*/
	} else {
		pmic_pwrap_write(ACCDET_RSV, micbias1_individual_ctrl ?
		 ACCDET_2V8_MODE_OFF_IND_CTL : ACCDET_2V8_MODE_OFF);
...
static bool accdet_eint_enabled(void)
{
	...
	enabled = (eint_accdet_sync_flag != 0);
...
static void accdet_FSA8049_enable(void)
{
	if (aud_gpios[GPIO_FSA8049_ON].gpio_prepare) {
		pinctrl_select_state(pinctrl_switchIC,
				 aud_gpios[GPIO_FSA8049_ON].gpioctrl);
...
static void accdet_FSA8049_disable(void)
{
	if (aud_gpios[GPIO_FSA8049_OFF].gpio_prepare) {
		pinctrl_select_state(pinctrl_switchIC,
				 aud_gpios[GPIO_FSA8049_OFF].gpioctrl);
...
bool Vcamaf_Regulator_PowerOnOFF(struct regulator *pwrreg, int IsOn, int vol)
{
	...
	if (IS_ERR(pwrreg)) {
		ACCDET_DEBUG("vcamaf power regulator is null, ret:%ld",
			 PTR_ERR(pwrreg));
		return false;
	}
	...
	if (IsOn) {		/* Power ON */
		...
		/* Need to reset Highest Voltage, if the power is used */
		/*by multi-user,otherwise, it would be bounding at */
		/*last time settings/Highest vol */
		voltage_count = regulator_count_voltages(pwrreg);
		if (voltage_count <= 0) {
			ACCDET_DEBUG("Fails to count, voltage_count = %d"
				voltage_count);
			return false;
		}
		...
		if (high_bound_voltage <= 0) {
			ACCDET_DEBUG("Fails to list, high_bound_voltage = %d",
				 high_bound_voltage);
			return false;
		}
		...
		if (ret != 0) {
			ACCDET_DEBUG("Fails to set vol = %d , ret = 0x%x",
				 vol, ret);
			return false;
		}
		...
		/*Need to reset Lowest/Highest Voltage, if the power */
		/*is used by multi-user,otherwise, it would be bounding*/
		/* at last time settings */
		high_bound_voltage = regulator_list_voltage(pwrreg,
			 voltage_count - 1);
		ACCDET_DEBUG("high_bound_voltage = %d", high_bound_voltage);
		...
		if (low_bound_voltage <= 0) {
			ACCDET_DEBUG("Fails to list, low_bound_voltage = %d",
				 low_bound_voltage);
			return false;
		}
		ret = regulator_set_voltage(pwrreg, low_bound_voltage,
			 high_bound_voltage);
		if (ret != 0) {
			ACCDET_DEBUG("Fails to set High(%d) ~ Low(%d) voltage",
...
/* to SLEEVE pin of SD80. However the HW connection on Karnak is */
...
#define MAX_INSERT_TIME  2500   /* wait for 2500ms */
	...
	do {
		...
		/* CTIA headset */
		if (accdet_adc < mic_dangling_voltage &&
			 accdet_adc > switch_mode_voltage)
			break;
		if (accdet_adc <= switch_mode_voltage) {
			omtp_continuous_times++;
			/*OMTP headset (sometimes CTIA gets the voltage in */
			/* this range) */
...
ACCDET_DEBUG("[Accdet]EINT func :plug-out, cur_eint_state = %d\n",
		...
		headset_plug_out();
#ifdef CONFIG_ACCDET_OMTP_CTIA_SWITCH_MANUAL
		/* We need to assure that accdet switch to default CTIA type */
		/* when plug out*/
...
static irqreturn_t accdet_eint_func(int irq, void *data)
{
	...
	if (ignore_early_headset_event
		 && (accdet_get_current_time() < 6000000000)) {
...
static inline int accdet_switch_manual_init(struct device_node *node)
{
	...
	ret = of_property_read_u32(node, "mic_dangling_vol",
			 &mic_dangling_voltage);
	if (ret < 0) {
	...
	ret = of_property_read_u32(node, "switch-mode-delay",
			 &switch_mode_delay);
	if (ret < 0) {
...
static inline int accdet_setup_eint(struct platform_device *accdet_device)
{
	...
	//	dev_err(&accdet_device->dev, "fwq Cannot find accdet
	//	 pinctrl default!\n");
	//}
	...
	if (IS_ERR(pins_eint_int)) {
		...
		dev_err(&accdet_device->dev,
			 "fwq Cannot find accdet pinctrl state_eint_int!\n");
		return ret;
	}
	...
	for (index_gpio = 0; index_gpio < ARRAY_SIZE(aud_gpios); index_gpio++) {
		aud_gpios[index_gpio].gpioctrl =
			 pinctrl_lookup_state(pinctrl_switchIC,
			 aud_gpios[index_gpio].name);
...
static void multi_key_detection(int current_status)
{
	...
	if (((pmic_pwrap_read(ACCDET_IRQ_STS) & IRQ_STATUS_BIT)
			 != IRQ_STATUS_BIT) && accdet_eint_enabled()) {
...
ACCDET_DEBUG("[Accdet]check_cable_type: Clear interrupt:Done[0x%x]!\n",
	...
	if (result) {
	ACCDET_DEBUG("%s -Failed to trigger type detection %d. Error=%d\n",
...
void accdet_get_dts_data(void)
{
	...
	if (node) {
		...
		of_property_read_u32(node, "accdet-micbias1-ind-ctl",
			 &micbias1_individual_ctrl);
...
#ifdef CONFIG_IDME
	ignore_early_headset_event = idme_get_dev_flags_value()
		 & IGNORE_EARLY_HEADSET_EVENT;
#endif
