...
static long accdet_unlocked_ioctl(struct file *file, unsigned int cmd,
	 unsigned long arg)
{
...
#ifdef CONFIG_COMPAT
static long accdet_compat_ioctl(struct file *file, unsigned int cmd,
	 unsigned long arg)
{
	long ret;
	...
	case GET_BUTTON_STATUS:
		return mt_accdet_unlocked_ioctl(cmd, arg);
	default:
		pr_info("[Accdet]accdet_ioctl : default!\n");
		ret = -1;
		break;
	}
	return ret;
