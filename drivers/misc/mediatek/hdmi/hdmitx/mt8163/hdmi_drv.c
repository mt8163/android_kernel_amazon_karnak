...
const char *hdmi_use_module_name_spy(enum HDMI_REF_MODULE_ENUM module)
{
	switch (module) {
	case AP_CCIF0:
		return "mediatek,mt8163-apmixedsys";	/* TVD//PLL */
	case TOPCK_GEN:
		return "mediatek,mt8163-topckgen";
	case INFRA_SYS:
		return "mediatek,mt8163-infracfg";
	case MMSYS_CONFIG:
		return "mediatek,mt8163-mmsys";
	case GPIO_REG:
		return "mediatek,mt8163-pinctrl";
	case DPI1_REG:
		return "mediatek,mt8163-disp_dpi1";
	case EFUSE_REG:
		return "mediatek,mt8163-disp_config2";
	case GIC_REG:
		return "mediatek,mt8163-mcucfg";
/*	case PERISYS_REG:
...
int hdmi_internal_power_on(void)
{
	...
	vWriteHdmiSYSMsk(HDMI_SYS_CFG20, 0x0 << 15, HDMI_SECURE_MODE);
...
int hdmi_internal_probe(struct platform_device *pdev)
{
	...
	np = of_find_compatible_node(NULL, NULL, "mediatek,mt8163-hdmitx");
#if 1
