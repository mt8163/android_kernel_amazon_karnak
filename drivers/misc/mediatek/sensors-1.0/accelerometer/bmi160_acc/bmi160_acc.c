...
#define BMI160_ACC_CALI_IDME           "/proc/idme/sensorcal"
#define BMI160_ACC_CALI_FILE           "/data/inv_cal_data.bin"

#define BMI160_DATA_BUF_NUM            3
#define BMI160_CALI_FILE_SIZE          24
#define BMI160_ACC_AXES_NUM        3
#define BMI160_CALI_DATA_NUM      100

#define POS_1G                          8192
#define NEG_1G                          -8192
#define ACCEL_XY_MAX_AVG_VAL            2048
#define ACCEL_XY_MIN_AVG_VAL            -2048
#define ACCEL_Z_POS_MAX_AVG_VAL         10240
#define ACCEL_Z_POS_MIN_AVG_VAL         6144
#define ACCEL_Z_NEG_MAX_AVG_VAL         -10240
#define ACCEL_Z_NEG_MIN_AVG_VAL         -6144
#define ACCEL_MAX_OFFSET_VAL  2048
#define ACCEL_MIN_OFFSET_VAL  -2048

static s16 accel_xyz_offset[BMI160_ACC_AXES_NUM] = {0};

extern int idme_get_sensorcal(s16 *data);
static int acc_self_test_flag = -1;
...
extern struct platform_device *accel_Platform_Dev;
...
static ssize_t show_sensorrawdata_value(struct device_driver *ddri, char *buf)
{
    ...
    if (sensor_power == false)
    {
        ...
        if (res)
        {
            GSE_ERR("Power on BMI160 error %d!\n", res);
        }
        msleep(50);
    }
...
BMI160_RETURN_FUNCTION_TYPE bmi160_set_command_register_bosch(u8 v_command_reg_u8)
{
	BMI160_RETURN_FUNCTION_TYPE com_rslt  = E_BMI160_COMM_RES;
	// check the p_bmi160 structure as NULL
	if (p_bmi160 == BMI160_NULL)
...
BMI160_RETURN_FUNCTION_TYPE bmi160_read_accel_x(s16 *v_accel_x_s16)
{
	/* variable used for return the status of communication result*/
	BMI160_RETURN_FUNCTION_TYPE com_rslt = E_BMI160_COMM_RES;
	/* Array contains the accel X lSB and MSB data		v_data_u8[0] - LSB		v_data_u8[1] - MSB*/
	u8 v_data_u8[BMI160_ACCEL_X_DATA_SIZE] = {BMI160_INIT_VALUE,	BMI160_INIT_VALUE};
	/* check the p_bmi160 structure as NULL*/
	if (p_bmi160 == BMI160_NULL)
	{
		return E_BMI160_NULL_PTR;
...
BMI160_RETURN_FUNCTION_TYPE bmi160_read_accel_y(s16 *v_accel_y_s16)
{
	/* variable used for return the status of communication result*/
	BMI160_RETURN_FUNCTION_TYPE com_rslt = E_BMI160_COMM_RES;
	/* Array contains the accel Y lSB and MSB data		v_data_u8[0] - LSB		v_data_u8[1] - MSB*/
	u8 v_data_u8[BMI160_ACCEL_Y_DATA_SIZE] = {BMI160_INIT_VALUE,	BMI160_INIT_VALUE};
...
BMI160_RETURN_FUNCTION_TYPE bmi160_read_accel_z(s16 *v_accel_z_s16)
{
	/* variable used for return the status of communication result*/
	BMI160_RETURN_FUNCTION_TYPE com_rslt = E_BMI160_COMM_RES;
	/* Array contains the accel Z lSB and MSB data		a_data_u8r[LSB_ZERO] - LSB		a_data_u8r[MSB_ONE] - MSB*/
	u8 a_data_u8r[BMI160_ACCEL_Z_DATA_SIZE] = {	BMI160_INIT_VALUE, BMI160_INIT_VALUE};
	/* check the p_bmi160 structure as NULL*/
	if (p_bmi160 == BMI160_NULL)
	{
		return E_BMI160_NULL_PTR;
...
static ssize_t set_accel_self_test(struct device_driver *ddri, char *buf)
{
	...
	acc_self_test_flag = -1;

	/*soft reset*/
	err = bmi160_set_command_register_bosch(CMD_RESET_USER_REG);
	if (err < 0) {
		GSE_ERR("[G Sensor] bmi160_set_command_register, CMD_RESET_USER_REG\n");
		acc_self_test_flag = 1;
		return BMI160_ACC_ERR_I2C;
	}
	msleep(70);

	err = bmi160_set_command_register_bosch(CMD_PMU_ACC_NORMAL);
	if (err < 0) {
		GSE_ERR("[G Sensor] bmi160_set_command_register, CMD_PMU_ACC_NORMAL\n");
		acc_self_test_flag = 2;
		return BMI160_ACC_ERR_I2C;
	}
	mdelay(80);

	/*databuf[0] = CMD_PMU_GYRO_NORMAL;
	err = bma_i2c_write_block(client, BMI160_CMD_COMMANDS__REG, &databuf[0], 1);
	if (err < 0) {
		GSE_ERR("BMI160_CMD_COMMANDS__REG, CMD_PMU_GYRO_NORMAL\n");
	...
	*/
	...
	if (err < 0) {
		GSE_ERR("[G Sensor] bmi160_set_accel_under_sampling_parameter failed\n");
		acc_self_test_flag = 3;
		return err;
	}

	//databuf[0] = BMI160_ACCEL_ODR_1600HZ;
	//err = bma_i2c_write_block(client, BMI160_USER_ACC_CONF_ODR__REG, &databuf[0], 1);
	//if (err < 0) {
	//	GSE_ERR("BMI160_USER_ACC_CONF_ODR__REG, BMI160_ACCEL_ODR_1600HZ\n");
	//	return BMI160_ACC_ERR_I2C;
	//}

	err = bmi160_set_accel_output_data_rate(BMI160_ACCEL_OUTPUT_DATA_RATE_1600HZ);
	if (err < 0) {
		GSE_ERR("[G Sensor] set acc bandwidth failed, BMI160_ACCEL_ODR_1600HZ\n");
		acc_self_test_flag = 4;
		return err;
	}
	...
	if (err < 0) {
		GSE_ERR("[G Sensor] set acc range failed, BMI160_ACCEL_RANGE_8G\n");
		acc_self_test_flag = 5;
		return err;
	}
	...
	if (err < 0) {
		GSE_ERR("[G Sensor] bmi160_set_accel_selftest_amp, BMI_SELFTEST_AMP_HIGH\n");
		acc_self_test_flag = 6;
		return err;
	}
	...
	for (i = X_AXIS; i < AXIS_MAX; i++)
	{
		...
		* so we need to +1*/
		...
		if (err < 0) {
			GSE_ERR("[G Sensor] bmi160_set_accel_selftest_axis, i + 1\n");
			acc_self_test_flag = 7;
			return err;
		}
		msleep(50);
		...
		case X_AXIS:
			...
			err += bmi160_get_accel_selftest_sign(&acc_selftest_sign);
			...
			if (err < 0) {
				GSE_ERR("[G Sensor] bmi160_read_accel_x, axis_n_value %x\n", axis_n_value);
				acc_self_test_flag = 8;
				return err;
			}
			...
			if (err < 0) {
				GSE_ERR("[G Sensor] bmi160_read_accel_x, axis_p_value %x\n", axis_p_value);
				acc_self_test_flag = 9;
				return err;
			}

			diff_axis[i] = abs(axis_p_value - axis_n_value);

			GSE_ERR("[G Sensor] X axis_n_value %x, axis_p_value %x\n", axis_n_value, axis_p_value);
		...
		case Y_AXIS:
			...
			if (err < 0) {
				GSE_ERR("[G Sensor] bmi160_read_accel_y, axis_n_value %x\n", axis_n_value);
				acc_self_test_flag = 10;
				return err;
			}
			...
			if (err < 0) {
				GSE_ERR("[G Sensor] bmi160_read_accel_y, axis_p_value %x\n", axis_p_value);
				acc_self_test_flag = 11;
				return err;
			}

			diff_axis[i] = abs(axis_p_value - axis_n_value);

			GSE_ERR("[G Sensor] Y axis_n_value %x, axis_p_value %x\n", axis_n_value, axis_p_value);
		...
		case Z_AXIS:
			...
			if (err < 0) {
				GSE_ERR("[G Sensor] bmi160_read_accel_z, axis_n_value %x\n", axis_n_value);
				acc_self_test_flag = 12;
				return err;
			}
			...
			if (err < 0) {
				GSE_ERR("[G Sensor] bmi160_read_accel_z, axis_p_value %x\n", axis_p_value);
				acc_self_test_flag = 13;
				return err;
			}

			diff_axis[i] = abs(axis_p_value - axis_n_value);

			GSE_ERR("[G Sensor] Z axis_n_value %x, axis_p_value %x\n", axis_n_value, axis_p_value);

			break;
		default:
			err += -EINVAL;
			acc_self_test_flag = 14;
			break;
		}
	...
	* but bmi_result bit4 0 is successful, 1 is failed*/
	...
	if(bmi_selftest != 0)
	{
		return sprintf(buf, "[G Sensor] set_accel_self_test FAIL, %x\n", bmi_selftest);
	}
	...
	//return count;
	acc_self_test_flag = 0;

	return sprintf(buf, "[G Sensor] set_accel_self_test PASS\n");
}

static ssize_t get_accel_self_test(struct device_driver *ddri, char *buf)
{
    if (acc_self_test_flag==0)
    {
        return sprintf(buf, "Accelmeter self test\nPass\n");
    }
    else if (acc_self_test_flag == 8 || acc_self_test_flag == 9)
    {
        return sprintf(buf, "Accel X out of range\nFail\n");
    }
    else if (acc_self_test_flag == 10 || acc_self_test_flag == 11)
    {
        return sprintf(buf, "Accel Y out of range\nFail\n");
    }
    else if (acc_self_test_flag == 12 || acc_self_test_flag == 13)
    {
        return sprintf(buf, "Accel Z out of range\nFail\n");
    }
    else
    {
        return sprintf(buf, "Accelmeter self test\nFail\n");
...
static int acc_store_offset_in_file(const char *filename, s16 *offset, int data_valid)
{
    ...
    else
    {
        ...
        cali_file->f_op->write(cali_file, (void *)w_buf, BMI160_DATA_BUF_NUM*sizeof(s16)*2, &cali_file->f_pos);
...
static void get_accel_idme_cali(void)
{
    ...
    mm_segment_t fs;
    int offset[BMI160_DATA_BUF_NUM]= {0};

    cali_file = filp_open(BMI160_ACC_CALI_IDME, O_RDONLY, 0);
    if(IS_ERR(cali_file))
    {
        GSE_LOG("read %s  ERROR\nFail\n", BMI160_ACC_CALI_IDME );
        return;
    }
    else
    {
...
static ssize_t set_accel_cali(struct device_driver *ddri, char *buf)
{
    int err;
    int i;
    u8 data[BMI160_CALI_DATA_NUM][6] = {{0}};
    static int xyz[BMI160_CALI_DATA_NUM][3] = {{0}};
    ...
    if (err)
    {
        ...
        return sprintf(buf, "[Sensor] enable power fail\n");
    }

    /* wait 200 for stable output */
    msleep(200);

    for (i=0; i<BMI160_CALI_DATA_NUM ; i++)
    {
        err = bma_i2c_read_block(client, addr, data[i], BMI160_ACC_DATA_LEN);
        if (err < 0) {
            GSE_ERR("read data failed.\n");
        } else {
            /* Convert sensor raw data to 16-bit integer */
    ...
    if (err)
    {
        ...
        return sprintf(buf, "[Sensor] disable power fail\n");
    }
    ...
    {
        return sprintf(buf, "[Sensor] %s: BMI160 X axis out of range %d\n", __FUNCTION__, avg[0]);
    }
    else if (avg[1]>ACCEL_XY_MAX_AVG_VAL || avg[1]<ACCEL_XY_MIN_AVG_VAL)
    {
        return sprintf(buf, "[Sensor] %s: BMI160 Y axis out of range %d\n", __FUNCTION__, avg[1]);
    }
    ...
    {
        return sprintf(buf, "[Sensor] %s: BMI160 Z axis out of range %d\n", __FUNCTION__, avg[2]);
    }
    ...
    if(acc_store_offset_in_file(BMI160_ACC_CALI_FILE, accel_xyz_offset, 1))
        {
            return sprintf(buf, "[G Sensor] BMI160 set_accel_cali ERROR %d, %d, %d\n",
        accel_xyz_offset[0],
        accel_xyz_offset[1],
        accel_xyz_offset[2]);
        }

    return sprintf(buf, "[G Sensor] set_accel_cali PASS  %d, %d, %d\n",
        accel_xyz_offset[0],
        accel_xyz_offset[1],
        accel_xyz_offset[2]);
}
...
static DRIVER_ATTR(accelgetselftest, S_IWUSR | S_IRUGO, get_accel_self_test, NULL);
static DRIVER_ATTR(accelsetselftest, S_IWUSR | S_IRUGO, set_accel_self_test, NULL);
static DRIVER_ATTR(accelgetcali, S_IWUSR | S_IRUGO, get_accel_cali, NULL);
static DRIVER_ATTR(accelsetcali, S_IWUSR | S_IRUGO, set_accel_cali, NULL);
static DRIVER_ATTR(accelgetidme, S_IWUSR | S_IRUGO, get_idme_cali, NULL);
...
static long bmi160_acc_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_GSENSOR_IOCTL_INIT:
		...
		err =
		    file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_INIT,
					       (unsigned long)arg32);
		if (err) {
	...
	case COMPAT_GSENSOR_IOCTL_READ_GAIN:
		...
		err =
		    file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_READ_GAIN,
					       (unsigned long)arg32);
		if (err) {
...
static int bmi160_acc_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	gpDMABuf_va = (u8 *) dma_alloc_coherent(&client->dev, ACCEL_DMA_MAX_TRANSACTION_LENGTH, &gpDMABuf_pa, GFP_KERNEL | GFP_DMA);	/* DMA size for customer */
	if (!gpDMABuf_va)
	...
	GSE_ERR("idme_get_sensorcal() \n");
	ret = idme_get_sensorcal(idmedata);

	GSE_ERR("bmi160_acc_i2c_probe idmedata %x, %x, %x, %x, %x, %x\n",
	...
	obj->cali_sw[0] = accel_xyz_offset[0];
	obj->cali_sw[1] = accel_xyz_offset[1];
	obj->cali_sw[2] = accel_xyz_offset[2];
