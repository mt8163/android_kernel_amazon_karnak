...
mtk_btif g_btif[BTIF_PORT_NR] = {
	{
	 ...
	 .p_btif_info = NULL,
	 .btif_buf.p_buf = NULL,
	 .tx_log.p_queue[0] = NULL,
	 .rx_log.p_queue[0] = NULL,
	 },
};
...
int btif_ftrace_print(const char *str, ...)
{
...
static int btif_rx_thread(void *p_data)
{
	...
	while (1) {
		...
		btif_ftrace_print("%s:brtif_rx_data_consummer start!\n", __func__);
		btif_rx_data_consummer(p_btif);
		btif_ftrace_print("%s:brtif_rx_data_consummer done!\n", __func__);
	}
	return 0;
}
...
static int _btif_rx_btm_init(p_mtk_btif p_btif)
{
...
#ifdef CONFIG_BISCUIT
	{
		...
		i_ret = sched_setscheduler(p_btif->p_task, SCHED_FIFO, &param);
		if (0 != i_ret)
			BTIF_WARN_FUNC(
				"set RT to psm workqueue failed %d\n", i_ret);
		else
			BTIF_INFO_FUNC(
				"set RT to btif_rxd workqueue succeed\n");
	}
#endif
...
err_exit1_5:
	...
	kfree(p_rx_queue);
	for (index = 0; index < BTIF_PORT_NR; index++) {
		kfree(g_btif[index].btif_buf.p_buf);
		kfree(g_btif[index].tx_log.p_queue[0]);
		kfree(g_btif[index].rx_log.p_queue[0]);
	}
