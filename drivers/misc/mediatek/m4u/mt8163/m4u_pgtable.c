...
static inline int m4u_get_pt_type(struct m4u_domain_t *domain, unsigned int mva)
{
	struct imu_pgd_t *pgd;
	struct imu_pte_t *pte;
	int ret;
	...
	while (mva < end_plus_1) {
		...
		if (F_PGD_TYPE_IS_PAGE(*pgd)) {
			...
			if (!pte) {
				...
				mva = m4u_calc_next_mva(mva,
					end_plus_1, MMU_SECTION_SIZE);
				continue;
			}

			next_mva = m4u_calc_next_mva(mva,
					end_plus_1, MMU_SECTION_SIZE);
...
int m4u_pte_allocator_init(void)
{
	gM4u_pte_kmem = kmem_cache_create("m4u_pte",
			IMU_BYTES_PER_PTE, IMU_BYTES_PER_PTE, 0, NULL);
	M4UINFO("%s: gM4u_pte_kmem = 0x%p, IMU_BYTES_PER_PTE = 0x%zx\n",
		__func__, gM4u_pte_kmem, IMU_BYTES_PER_PTE);
 ...
 * @author K Zhang      @date 2013/11/18
 ***********************************************************/
int m4u_alloc_pte(struct m4u_domain_t *domain, struct imu_pgd_t *pgd,
		unsigned int pgprot)
{
	void *pte_new_va;
	phys_addr_t pte_new;
	...
	if (likely(!imu_pgd_val(*pgd))) {
		...
		M4ULOG_L(
			"%s: pgd: 0x%p, pte_va:0x%p, pte_pa: 0x%lx, value: 0x%x\n",
			__func__, pgd, pte_new_va, (unsigned long)pte_new,
		...
		/* kfree(__va(pte_new)); */
		M4ULOG_L("m4u pte allocated by others: pgd=0x%p\n", pgd);
		kmem_cache_free(gM4u_pte_kmem, (void *)pte_new_va);
		return 1;
	}
}

int m4u_free_pte(struct m4u_domain_t *domain, struct imu_pgd_t *pgd)
{
		...
		unsigned long pa, unsigned int prot)
{
	...
	M4ULOG_L(
		"%s: mva: 0x%x, pgd: 0x%p (0x%p + 0x%x), pa: 0x%lx, value: 0x%x\n",
		...
		unsigned long pa, unsigned int prot)
{
	...
	M4ULOG_L(
		"%s: mva: 0x%x, pte: 0x%p (0x%p + 0x%x), pa: 0x%lx, value: 0x%x\n",
...
static inline int m4u_map_phys_align(struct m4u_domain_t *m4u_domain,
		unsigned int iova, unsigned long paddr,
		       ...
		       unsigned int prot)
{
	...
	if (!IS_ALIGNED(iova | (unsigned int)paddr | size, min_pagesz)) {
		M4UMSG(
			"unaligned: iova 0x%x pa 0x%lx size 0x%x min_pagesz 0x%x\n",
	...
	while (size) {
		...
		M4ULOG_L("mapping: iova 0x%x pa 0x%lx pgsize %lu\n",
			iova, paddr, pgsize);
		    ...
		    unsigned int prot)
{
	...
	for_each_sg(sg_table->sgl, sg, sg_table->nents, i) {
		...
		M4ULOG_L("%s: for_each_sg i: %d, len: %d, mva: 0x%x\n",
		...
		if (len == SZ_4K)	/* for most cases */
			...
			ret = m4u_map_phys_range(m4u_domain, map_mva,
						pa, len, prot);
...
int m4u_check_free_pte(struct m4u_domain_t *domain, struct imu_pgd_t *pgd)
{
	struct imu_pte_t *pte;
	int i;
...
int m4u_unmap(struct m4u_domain_t *domain, unsigned int mva, unsigned int size)
{
	...
	do {
		...
		if (F_PGD_TYPE_IS_PAGE(*pgd)) {
			...
			mva = (mva + MMU_SUPERSECTION_SIZE)
				& (~(MMU_SUPERSECTION_SIZE - 1));
		} else {
			mva += MMU_SECTION_SIZE;
		}
	} while (mva < end_plus_1 && mva);
	/*if the last mva is 0xfff00000, the new mva may be 0 */
