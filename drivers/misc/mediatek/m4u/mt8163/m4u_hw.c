...
static int larb_backup(unsigned int larb_idx)
{
	...
	if (larb_idx == 0) {	/*only support disp */
		...
		if (0 == (M4U_ReadReg32(DispPmReg, 0) & BIT(3))) {
			pr_err("larb_backup error1 already power down\n");
			WARN_ON(1);
		}
		...
		if (0 == (M4U_ReadReg32(DispPmReg, 0) & BIT(3))) {
			pr_err("larb_backup error2 already power down\n");
			WARN_ON(1);
		}

		if (ret != TZ_RESULT_SUCCESS) {
			pr_err("m4u reg backup ServiceCall error 0x%x\n", ret);
			WARN_ON(1);
			return -1;
		}
	}
...
static int larb_restore(unsigned int larb_idx)
{
	...
	if (larb_idx == 0) {	/*only support disp */
		...
		paramTypes = TZ_ParamTypes1(TZPT_VALUE_INPUT);
		if (0 == (M4U_ReadReg32(DispPmReg, 0) & BIT(3))) {
			pr_err("larb_restore error1 already power down\n");
			WARN_ON(1);
		}
		ret = KREE_TeeServiceCall(m4u_session,
					  M4U_TZCMD_LARB_REG_RESTORE, paramTypes, param);
		if (0 == (M4U_ReadReg32(DispPmReg, 0) & BIT(3))) {
			pr_err("larb_restore error2 already power down\n");
			WARN_ON(1);
		}
		if (ret != TZ_RESULT_SUCCESS) {
			pr_err("m4u reg backup ServiceCall error 0x%x\n", ret);
		...
		if (IntrSrc & F_INT_TRANSLATION_FAULT(m4u_slave_id)) {
			...
			if (gM4uPort[m4u_port].enable_tf == 1) {
				...
				if (!ptepa) {
					...
					pr_err_ratelimited("[M4U]TF: port=%s, mva=0x%x, pa=0x%x, pa@pgt=0x%x\n",
