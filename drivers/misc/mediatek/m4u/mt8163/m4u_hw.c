		    ...
		    unsigned int mva_end)
{
	...
	if (m4u_id == 2 && gGpuPowerCallback.gpu_power_state_freeze) {
		if (gGpuPowerCallback.gpu_power_state_freeze(false) == 0) {
			M4ULOG_H("gpu clock disable, skip invalid tlb\n");
			return -1;
		}
	}
#endif
...
void m4u_invalid_tlb_sec(int m4u_id, int L2_en, int isInvAll,
			unsigned int mva_start, unsigned int mva_end)
{
	unsigned int reg = 0;
	unsigned long m4u_base = gM4UBaseAddr[m4u_id];
	...
	if (isInvAll) {
		...
		M4U_WriteReg32(m4u_base,
			REG_MMU_INVLD_SEC, F_MMU_INV_SEC_RANGE);
	}
...
static inline void m4u_clear_intr(unsigned int m4u_id)
{
	m4uHw_set_field_by_mask(gM4UBaseAddr[m4u_id],
		REG_MMU_INT_L2_CONTROL,	F_INT_L2_CLR_BIT, F_INT_L2_CLR_BIT);
}
 ...
 * @param   vir          -- virtual monitor enable ?
	(if enable we will monitor mva, or else monitor PA)
 * @param   io           -- I/O use mau at input or output of RS.
	0 for input, 1 for output
	input: mau @ RS input, can monitor mva or pa (bypass m4u);
	output:mau @ RS output, can monitor pa to emi
		(bypass m4u, or after mva translation)
 * @param   bit32        -- enable bit32 monitor?
 * @param   start        -- start address of monitor
	(can be any address without alignment)
 * @param   end          -- end address of monitor
	(can be any address without alignment)
...
int config_mau(struct M4U_MAU_STRUCT mau)
{
	...
	for (i = 0; i < M4U0_MAU_NR; i++) {
		if (gM4u0_mau[i].Enabled != 0) {
			if (MVAStart >= gM4u0_mau[i].MVAStart
				&& MVAEnd <= gM4u0_mau[i].MVAEnd) {
				/* no overlap */
				if (mau.enable == 0) {
					gM4u0_mau[i].Enabled = 0;
					mau_start_monitor(0, 0, i, 0, 0, 0,
							0, 0, 0, 0, 0);
					continue;
				}
			}
	...
	mau_start_monitor(m4u_id, larb_2_m4u_slave_id(larb), free_id,
			(int)mau.write, 1, 0, 0, MVAStart, MVAEnd,
			1 << m4u_port_2_larb_port(mau.port), 1 << larb);
	return free_id;
}

/* notes:
 *you must fill cfg->m4u_id/m4u_slave_id/mau_set before call this func.
 */
...
int __mau_dump_status(int m4u_id, int m4u_slave_id, int mau)
{
	...
	if (status & (1 << mau)) {
		...
		M4ULOG_H("id=0x%x(%s), addr=0x%x, b32=0x%x\n", assert_id,
			    m4u_get_port_name(larb_port_2_m4u_port(larb, port)),
		...
		M4ULOG_H(
			...
			mau_cfg.virt, mau_cfg.io, mau_cfg.write_monitor,
			mau_cfg.start_bit32, mau_cfg.end_bit32);

	} else {
		M4ULOG_M("mau no assert in set %d\n", mau);
	}
...
int m4u_dump_reg(int m4u_index, unsigned int start)
{
	...
	for (i = 0; i < 368 / 8; i += 4) {
		...
		M4UINFO(
			...
			j,
			M4U_ReadReg32(gM4UBaseAddr[m4u_index], j + 4 * 0),
			M4U_ReadReg32(gM4UBaseAddr[m4u_index], j + 4 * 1),
			M4U_ReadReg32(gM4UBaseAddr[m4u_index], j + 4 * 2),
			M4U_ReadReg32(gM4UBaseAddr[m4u_index], j + 4 * 3),
			M4U_ReadReg32(gM4UBaseAddr[m4u_index], j + 4 * 4),
			M4U_ReadReg32(gM4UBaseAddr[m4u_index], j + 4 * 5),
			M4U_ReadReg32(gM4UBaseAddr[m4u_index], j + 4 * 6),
			M4U_ReadReg32(gM4UBaseAddr[m4u_index], j + 4 * 7));
	}
...
void m4u_get_main_tlb(int m4u_id, int m4u_slave_id,
			int idx, struct mmu_tlb_t *pTlb)
{
...
unsigned int m4u_get_pfh_tlb(int m4u_id, int set, int page,
			int way, struct mmu_tlb_t *pTlb)
{
	unsigned int regValue = 0;
	unsigned long m4u_base = gM4UBaseAddr[m4u_id];

	regValue = F_READ_ENTRY_EN | F_READ_ENTRY_PFH
...
static unsigned int imu_pfh_tag_to_va(int mmu, int set,
				int way, unsigned int tag)
{
	unsigned int tmp;
...
int m4u_confirm_main_range_invalidated(int m4u_index, int m4u_slave_id,
				...
				unsigned int MVAStart, unsigned int MVAEnd)
{
	...
	/* /> check Main TLB part */
	result = m4u_confirm_main_range_invalidated(m4u_index, 0,
						MVAStart, MVAEnd);
	if (result < 0)
		return -1;

	if (m4u_index == 0) {
		result = m4u_confirm_main_range_invalidated(m4u_index, 1,
							MVAStart, MVAEnd);
		if (result < 0)
			return -1;
	}
	...
	for (way = 0; way < way_nr; way++) {
		for (set = 0; set < set_nr; set++) {
			...
			if (regval & F_MMU_PFH_VLD_BIT(set, way)) {
				unsigned int tag =
					m4u_get_pfh_tag(m4u_index, set, 0, way);
				unsigned int tag_s, tag_e, sa, ea;
				int layer = tag & F_PFH_TAG_LAYER_BIT;
				int large = tag & F_PFH_TAG_16X_BIT;

				tag_s = imu_pfh_tag_to_va(m4u_index, set,
							way, tag);
...
int m4u_insert_seq_range(int port, unsigned int MVAStart, unsigned int MVAEnd)
{
	...
	M4ULOG_M(
		"m4u_insert_seq_range , module:%s, MVAStart:0x%x, MVAEnd:0x%x\n",
...
/* ================================================================== */
	...
	if (free_id == -1) {
		M4ULOG_M("warning: can not find available range\n");
		mutex_unlock(&gM4u_seq_mutex);
		return -1;
	}
...
static int _m4u_config_port(int port, int virt, int sec, int dis, int dir)
{
	...
	if (!m4u_base)
		return -1;
	if (0 == virt || 1 == sec)
		M4ULOG_H("config_port:%s,v%d,s%d\n",
	...
	if (m4u_index == 0) {
		...
		if (port == M4U_PORT_HSIC_DMA_1P)
			...
			M4UMSG("warning can't config virt for port %d\n", port);
			ret = -1;
			goto unlock_out;
		}
...
static inline void _m4u_port_clock_toggle(int m4u_index, int larb, int on)
{
	...
	if (m4u_index == 0) {
		...
		if (end - start > 50000000ULL) {	/* unit is ns */
			M4ULOG_H("warn: larb%d clock %d time: %lld ns\n",
				larb, on, end - start);
		}
	}
}
		...
		if (port_array->ports[port] & M4U_PORT_ATTR_EN) {
			...
			if (larb != last_larb) {
				if (last_larb != -1) /* clock off last larb */
					_m4u_port_clock_toggle(last_index,
							last_larb, 0);
...
int m4u_monitor_stop(int m4u_id)
{
	...
	M4ULOG_M(
		"[M4U%d-%d] total:%d, main miss:%d, pfh miss(walk):%d, auto pfh:%d\n",
...
int m4u_reg_backup(int m4u_id)
{
	...
	/* for(m4u_id=0; m4u_id<TOTAL_M4U_NUM; m4u_id++) */
	{
		...
		for (m4u_slave = 0; m4u_slave < M4U_SLAVE_NUM(m4u_id);
			m4u_slave++) {
...
static int larb_backup(unsigned int larb_idx)
{
	...
	__M4U_BACKUP(larb_base, SMI_LARB_MMU_EN,
		larb_reg_backup_buf[larb_idx][0]);
	__M4U_BACKUP(larb_base, SMI_LARB_SEC_EN,
		larb_reg_backup_buf[larb_idx][1]);
	__M4U_BACKUP(larb_base, SMI_LARB_DOMN_L,
		larb_reg_backup_buf[larb_idx][2]);
	__M4U_BACKUP(larb_base, SMI_LARB_DOMN_H,
		larb_reg_backup_buf[larb_idx][3]);
...
static int larb_restore(unsigned int larb_idx)
{
	...
	__M4U_RESTORE(larb_base, SMI_LARB_MMU_EN,
		larb_reg_backup_buf[larb_idx][0]);
	__M4U_RESTORE(larb_base, SMI_LARB_SEC_EN,
		larb_reg_backup_buf[larb_idx][1]);
	__M4U_RESTORE(larb_base, SMI_LARB_DOMN_L,
		larb_reg_backup_buf[larb_idx][2]);
	__M4U_RESTORE(larb_base, SMI_LARB_DOMN_H,
		larb_reg_backup_buf[larb_idx][3]);
...
#if 0
static int m4u_enable_prefetch(int PortID)
{
...
static int m4u_disable_prefetch(int PortID)
{
	unsigned long m4u_base = gM4UBaseAddr[m4u_port_2_m4u_id(PortID)];
...
irqreturn_t MTK_M4U_isr(int irq, void *dev_id)
{
...

	{
		...
		if (IntrSrc & F_INT_TRANS_F(m4u_slave_id)) {
			...
			MMProfileLogEx(M4U_MMP_Events[M4U_MMP_M4U_ERROR],
				MMProfileFlagPulse, m4u_port, fault_mva);
		}
		if (IntrSrc & F_INT_MAIN_MULTI_HIT_FAULT(m4u_slave_id)) {
			MMU_INT_REPORT(m4u_index, m4u_slave_id,
				F_INT_MAIN_MULTI_HIT_FAULT(m4u_slave_id));
		}
		if (IntrSrc & F_INT_INVALID_PA_F(m4u_slave_id)) {
			if (!(IntrSrc & F_INT_TRANS_F(m4u_slave_id))) {
				MMU_INT_REPORT(m4u_index, m4u_slave_id,
				F_INT_INVALID_PA_F(m4u_slave_id)
				);
			}
		}
...
int m4u_reg_init(struct m4u_domain_t *m4u_domain, unsigned long ProtectPA,
		int m4u_id)
{
	unsigned int regval;
...
int m4u_hw_init(struct m4u_device *m4u_dev, int m4u_id)
{
	...
	pM4URegBackUp = kmalloc(M4U_REG_BACKUP_SIZE * TOTAL_M4U_NUM,
			GFP_KERNEL | __GFP_ZERO);
