...
static void spm_register_init(void)
{
	...
	if (!node)
		spm_err("find mt8163-scpsys node failed\n");
	...
	/* cksys_base */
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt8163-topckgen");
	if (!node)
	...
	/* mcucfg */
	node = of_find_compatible_node(NULL, NULL, "mediatek,mt8163-mcucfg");
	if (!node)
