...
int interpolate_2d(struct sptbl_t *tab, int v1, int v2, int t1, int t2
...
void interpolate_table(struct sptbl_t *spt, int c1, int c2, int c3
	, struct sptbl_t *tab1, struct sptbl_t *tab2)
{
	int v, t;
...
int mt_spower_make_table(struct sptbl_t *spt, struct spower_raw_t *spower_raw
	, int wat, int voltage, int degree)
{
	int i;
	int c1, c2, c =  -1;
	 ...
	 * Thus, sptab takes use of the container
	 * which raw data is not used anymore.
	 **/
	if (wat == c) {
		/** just match **/
		tab1 = tab2 = &tab[i];
...
#define DEVINFO_GPU_BIT (24)
	...
	mt_spower_make_table(&sptab[MT_SPOWER_CA7], &ca7_spower_raw
		, mp0, 1150, 30);
	mt_spower_make_table(&sptab[MT_SPOWER_GPU], &gpu_spower_raw
		, gpu, 1150, 30);
