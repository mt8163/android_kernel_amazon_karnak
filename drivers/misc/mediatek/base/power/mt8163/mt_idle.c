...
static u32 dpidle_condition_mask[NR_GRPS] = {
	...
	0xffffffff,		/* DISP0: */
	0x0000ffff,		/* DISP1: */
	0x000003f1,		/* IMAGE: use SPM MTCMOS off as condition: */
	0x0000000f,		/* MFG: use SPM MTCMOS off as condition: */
	0x00000000,		/* AUDIO: use SPM MTCMOS off as condition: */
	0x00000001,		/* VDEC0: use SPM MTCMOS off as condition: */
	0x00000001,		/* VDEC1: use SPM MTCMOS off as condition: */
	0x00001111,		/* VENC_JPEG: use SPM MTCMOS off as condition */
};
...
#define CLK_CFG_UPDATE		TOPCKGEN_REG(0x004)
#define CLK_CFG_4		TOPCKGEN_REG(0x080)
#define CLK_CFG_6		TOPCKGEN_REG(0x0A0)
#define INFRA_PDN_STA0		INFRA_REG(0x0090)
#define INFRA_PDN_STA1		INFRA_REG(0x0094)
...
unsigned int __attribute__((weak)) spm_get_cpu_pwr_status(void)
{
	u32 val[2] = {0};
	u32 stat = 0;
	u32 cpu_mask = (CA7_CPU3_PWR_STA_MASK | CA7_CPU2_PWR_STA_MASK |
...
static void get_all_clock_state(u32 clks[NR_GRPS])
{
	...
	if (is_valid_reg(infrasys_base)) {
		clks[CG_INFRA0] = ~idle_readl(INFRA_PDN_STA0);	/* INFRA0 */
		clks[CG_INFRA1] = ~idle_readl(INFRA_PDN_STA1);	/* INFRA1 */
	}
	...
	if (sys_is_on(SYS_AUD) && is_valid_reg(audiosys_base))
	...
	if (sys_is_on(SYS_VEN) && is_valid_reg(vencsys_base))
		clks[CG_VENC] = idle_readl(VENC_CG_CON);	/* VENC */
}
...
static void backup_audintbus_26MHz(void)
{
	...
	idle_writel(CLK_CFG_4, clk_cfg_4 & 0xFCFFFFFF);
	idle_writel(CLK_CFG_UPDATE, BIT(18));
}
...
{
	if (en)
		slp_spm_deepidle_flags &= ~SPM_CPU_DVS_DIS;
	else
		slp_spm_deepidle_flags |= SPM_CPU_DVS_DIS;
}
...
{
	get_base_from_node("mediatek,mt8163-topckgen", &topckgen_base, 0);
	get_base_from_node("mediatek,mt8163-infracfg", &infrasys_base, 0);
	get_base_from_node("mediatek,mt8163-audiosys", &audiosys_base, 0);
	get_base_from_node("mediatek,mt8163-mfg", &mfgsys_base, 0);
	get_base_from_node("mediatek,mt8163-mmsys", &mmsys_base, 0);
	get_base_from_node("mediatek,mt8163-imgsys", &imgsys_base, 0);
	get_base_from_node("mediatek,mt8163-vdecsys", &vdecsys_base, 0);
	get_base_from_node("mediatek,mt8163-vencsys", &vencsys_base, 0);
	get_base_from_node("mediatek,mt8163-scpsys", &scpsys_base, 0);
}
				...
				size_t count, loff_t *f_pos)
{
	...
	if (!strncmp(cmd, "switch", sizeof("switch"))) {
				...
				size_t count, loff_t *f_pos)
{
	...
	if (!strncmp(cmd, "dpidle", sizeof("dpidle")))
		idle_switch[IDLE_TYPE_DP] = param;
	else if (!strncmp(cmd, "enable", sizeof("enable")))
		enable_dpidle_by_bit(param);
	else if (!strncmp(cmd, "disable", sizeof("disable")))
		disable_dpidle_by_bit(param);
	else if (!strncmp(cmd, "time", sizeof("time")))
		dpidle_time_critera = param;
	else if (!strncmp(cmd, "bypass", sizeof("bypass")))
				...
				size_t count, loff_t *f_pos)
{
	...
	if (!strncmp(cmd, "soidle", sizeof("soidle")))
				...
				size_t count, loff_t *f_pos)
{
	...
	if (!strncmp(cmd, "mcidle", sizeof("mcidle")))
				...
				size_t count, loff_t *f_pos)
{
	...
	if (!strncmp(cmd, "slidle", sizeof("slidle")))
