...
static int fh_dumpregs_proc_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "ARMCA7:0x%08x M:0x%08x MAIN:0x%08x MEM:0x%08x ",
			fh_read32(REG_ARMCA7PLL_CON0),	fh_read32(REG_MPLL_CON0),
	...
	seq_printf(m,  "MSDC:0x%08x MM:0x%08x VENC:0x%08x TVD:0x%08x VCODEC:0x%08x\r\n",
			...
			fh_read32(REG_VCODECPLL_CON0));

	seq_puts(m, "\r\nPLL_CON1 :\r\n");
	seq_printf(m, "ARMCA7:0x%08x M:0x%08x MAIN:0x%08x MEM:0x%08x ",
			fh_read32(REG_ARMCA7PLL_CON1),	fh_read32(REG_MPLL_CON1),
...
static int __reg_base_addr_init(void)
{
	...
	fhctl_node = of_find_compatible_node(NULL, NULL, "mediatek,mt8163-fhctl");
	if (!fhctl_node) {
	...
	apmixed_node = of_find_compatible_node(NULL, NULL, "mediatek,mt8163-apmixedsys");
	if (!fhctl_node) {
	...
	ddrphy_node = of_find_compatible_node(NULL, NULL, "mediatek,mt8163-ddrphy");
	if (!fhctl_node) {
		FH_MSG_DEBUG(" Error, Cannot find FHCTL device tree node");
		return -1;
	}
...
static int __fh_debug_proc_read(struct seq_file *m, void *v, fh_pll_t *pll)
{
	...
	seq_puts(m, "===============================================\r\n");
	seq_puts(m, "id=ARMCA7PLL=MPLL=MAINPLL=MEMPLL=MSDCPLL=MMPLL=VENCPLL=TVDPLL=VCODECPLL\r\n");
