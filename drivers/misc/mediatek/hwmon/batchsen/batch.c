				  ...
				  const char *buf, size_t count)
{
	...
	#else
	maxBatchReportLatencyNs = div_u64(maxBatchReportLatencyNs, 1000000);
	samplingPeriodNs = div_u64(samplingPeriodNs, 1000000);
	#endif
			...
			if (err)
				...
				if (batch_context_obj->dev_list.data_dev[handle].is_timestamp_supported == 0) {
					...
					if (pt->total_count == 0)
						...
						#else
				batch_sensors_data.data[i].time =
		div_u64(batch_sensors_data.data[i].time, pt->total_count);
						#endif
					}
...
exit_misc_register_failed:
	misc_deregister(&batch_context_obj->mdev);
