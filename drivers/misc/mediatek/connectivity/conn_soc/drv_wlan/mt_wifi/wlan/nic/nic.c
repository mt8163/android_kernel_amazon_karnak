...
WLAN_STATUS nicUpdateBss(IN P_ADAPTER_T prAdapter, IN ENUM_NETWORK_TYPE_INDEX_T eNetworkTypeIdx)
{
	...
	if ((kalStrnCmp(CONFIG_ARCH_MTK_PROJECT, "abh123", 7) == 0 ||
		...
		if (prBssInfo->eConnectionState == PARAM_MEDIA_STATE_CONNECTED &&
			...
			/* nicQmSetRxBASize(prAdapter, TRUE, 0x11); */
			/* Fix Rx BA Size to 2 */
...
WLAN_STATUS
nicQmSetRxBASize(IN P_ADAPTER_T prAdapter,
				BOOLEAN enable, UINT32 size)
{
...
#endif
WLAN_STATUS
nicUpdateP2PConfig(IN P_ADAPTER_T prAdapter,
				BOOLEAN IsLegacyCilentMCC)
{
	struct CMD_P2P_CONFIG_T rCmdP2PConfig;

	DEBUGFUNC("nicUpdateP2PConfig");
	ASSERT(prAdapter);
	kalMemZero(&rCmdP2PConfig, sizeof(rCmdP2PConfig));
	rCmdP2PConfig.fgIsLegacyClientMCC = IsLegacyCilentMCC;

	return wlanoidSendSetQueryP2PCmd(prAdapter,
				   CMD_ID_SET_P2P_CONFIG,
				   TRUE,
				   FALSE,
				   FALSE,
				   NULL, NULL,
				   sizeof(rCmdP2PConfig),
				   (PUINT_8)&rCmdP2PConfig,
				   NULL, 0);
}
