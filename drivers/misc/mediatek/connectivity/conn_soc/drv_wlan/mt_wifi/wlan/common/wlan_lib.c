...
WLAN_STATUS wlanLoadManufactureData(IN P_ADAPTER_T prAdapter, IN P_REG_INFO_T prRegInfo)
{
	...
	country = (((UINT_16) prRegInfo->au2CountryCode[0]) << 8) |
...
{
	P_MSG_AIS_ABORT_T prAisAbortMsg = NULL;
	P_MSG_P2P_CONNECTION_ABORT_T prDisconnectMsg = NULL;

	switch (ucNetTypeIndex) {
	case NETWORK_TYPE_AIS_INDEX:
		...
		break;
	case NETWORK_TYPE_P2P_INDEX:
		prDisconnectMsg = (P_MSG_P2P_CONNECTION_ABORT_T)
				  cnmMemAlloc(prAdapter,
					      RAM_TYPE_MSG,
