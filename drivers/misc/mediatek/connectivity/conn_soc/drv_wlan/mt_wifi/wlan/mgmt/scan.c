	...
	if (u4RemovePolicy & SCN_RM_POLICY_TIMEOUT) {
		...
		LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
			...
			if (prConnSettings->fgIsConnReqIssued &&
				...
				DBGLOG(SCN, LOUD,
					"Don't free scan BSS Descriptor due to it is the target BSS!!\n");
				continue;
...
P_BSS_DESC_T scanAddToBssDesc(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
...
#if CFG_SUPPORT_802_11K				/* CFG_SUPPORT_802_11D */
	...
	if (prCountryIE) {
		...
		if (ucRemainLen >= 3 && ucPowerConstraint > 0) {
...
P_BSS_DESC_T scanSearchBssDescByPolicy(IN P_ADAPTER_T prAdapter, IN ENUM_NETWORK_TYPE_INDEX_T eNetTypeIndex)
{
	...
	DBGLOG(SCN, INFO, "SEARCH: Bss Num: %d, Look for SSID: XXXXXX, %pM Band=%d, channel=%d\n",
			   (UINT_32) prBSSDescList->u4NumElem,
			   (prConnSettings->aucBSSID), eBand, ucChannel);
	DBGLOG(SSID, TRACE, "Look for SSID: %s\n", prConnSettings->aucSSID);
...
UINT_8 scanGetCurrentEssChnlList(P_ADAPTER_T prAdapter, PUINT_8 pucEssChnlInfo, UINT_8 ucMax)
{
	...
	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
		...
		pucEssChnlInfo[ucChnlCount] = prBssDesc->ucChannelNum;
		ucChnlCount++;
		if (ucChnlCount >= ucMax)
			break;
	}
	return ucChnlCount;
}
