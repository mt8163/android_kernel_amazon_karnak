...
{
	do {
#if CFG_SUPPORT_OOBE_AP_MODE
		BOOLEAN fgIsOOBEAP = TRUE;
#else
		BOOLEAN fgIsOOBEAP = FALSE;
#endif
		...
		if (prBssInfo->ucAllSupportedRatesLen == 0 || fgIsOOBEAP) {
...
#if CFG_SUPPORT_OOBE_AP_MODE
		/*Special config for P2P*/
		nicUpdateP2PConfig(prAdapter, TRUE);
#endif
		    ...
		    IN P_SW_RFB_T prSwRfb, IN PP_STA_RECORD_T pprStaRec, OUT PUINT_16 pu2StatusCode)
{
	...
	P_BSS_INFO_T prP2pBssInfo = (P_BSS_INFO_T) NULL;
	P_STA_RECORD_T prStaRec = (P_STA_RECORD_T) NULL;
	P_STA_RECORD_T prStaRec_tmp = NULL;
	...
	do {
		...
		*pu2StatusCode = STATUS_CODE_REQ_DECLINED;
		...
		/* Remove other StaRec that has	same mac address */
		for (i = NETWORK_TYPE_AIS_INDEX; i < NETWORK_TYPE_INDEX_NUM; i++) {
			if (i != NETWORK_TYPE_P2P_INDEX) {
				prStaRec_tmp = cnmGetStaRecByAddress(prAdapter,
								     (UINT_8)i,
								     prAuthFrame->aucSrcAddr);
				if (prStaRec_tmp) {
					disconnect_sta_by_network(prAdapter,
								  (UINT_8)i,
								  prAuthFrame->aucSrcAddr);
				}
			}
