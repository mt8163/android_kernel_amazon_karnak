...
/* [IOC_P2P_SET_PWR_MGMT_PARAM         - SIOCIWFIRSTPRIV]  = mtk_p2p_wext_set_pm_param, */
	...
	[IOC_P2P_GET_OP_CHAN - SIOCIWFIRSTPRIV]  = mtk_p2p_wext_get_op_chan,
};
...
static const struct ieee80211_iface_combination p2p_iface_comb_mcc[] = {
	{
		.limits = p2p_iface_limits_mcc,
		.n_limits = ARRAY_SIZE(p2p_iface_limits_mcc),
		.max_interfaces = 2,
    ...
    )
{
	...
	rStatus = wlanQueryInformation(prGlueInfo->prAdapter, wlanoidQueryP2pOpChannel, &chan, sizeof(UINT_32), &u4BufLen);
	if (rStatus == 0)
		wrqu->mode = nicChannelNum2Freq(chan)/1000; /* Hz->MHz */
...
} /* end of mtk_p2p_wext_get_op_chan() */
