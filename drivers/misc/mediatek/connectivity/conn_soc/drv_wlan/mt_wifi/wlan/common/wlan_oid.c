...
{
	struct PARAM_SCAN_REQUEST_ADV_T *prScanRequest;
	...
	*pu4SetInfoLen = 0;
	...
	if (u4SetBufferLen != sizeof(struct PARAM_SCAN_REQUEST_ADV_T))
		return WLAN_STATUS_INVALID_LENGTH;
		 ...
		 OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	int direct_query = *(int *)pvQueryBuffer;
#endif
		      ...
		      IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(PARAM_CUSTOM_SW_CTRL_STRUCT_T);
	...
	case 0x3000:
		{/* set_rx_ba_win */
			rCmdRxBASize.u2RxBAWinSize = (UINT_16)prSwCtrlInfo->u4Data;
			if (rCmdRxBASize.u2RxBAWinSize == 0)
				rCmdRxBASize.fgEnabled = FALSE;
			else
				rCmdRxBASize.fgEnabled = TRUE;
			DBGLOG(HAL, TRACE, "SetRxBaWin: %d, 0x%hx\n",
				rCmdRxBASize.fgEnabled,
				rCmdRxBASize.u2RxBAWinSize);
			rWlanStatus = wlanSendSetQueryCmd(prAdapter,
...
/*----------------------------------------------------------------------------*/
/*!
* \brief This routine is called to set RX FILTER
*
* \param[in] pvAdapter Pointer to the Adapter structure.
* \param[out] pvQueryBuf A pointer to the buffer that holds the result of
*						   the query.
				...
				IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	static struct CMD_RX_PATTERN_FILTER rCmdRxFilter; /*in case stack overflow when deep function call*/

	DEBUGFUNC("wlanoidSetRxFilter");
	DBGLOG(INIT, INFO, "%s\n", __func__);
	...
	*pu4QueryInfoLen = sizeof(struct PARAM_DRIVER_FW_RX_FILTER);

	if (u4QueryBufferLen < sizeof(struct PARAM_DRIVER_FW_RX_FILTER))
		return WLAN_STATUS_INVALID_LENGTH;

	kalMemCopy(&rCmdRxFilter, pvQueryBuffer, sizeof(struct CMD_RX_PATTERN_FILTER));

	pr_info("RxFilter cmd=%d id=%u IsWhiteList=%d IsEqual=%d Offset=%u Length=%u\r\n",
		rCmdRxFilter.eOpcode, rCmdRxFilter.ucPatternIndex, rCmdRxFilter.fsIsWhiteList,
		 rCmdRxFilter.fgIsEqual, rCmdRxFilter.u4Offset, rCmdRxFilter.u4Length);

	{
		rWlanStatus = wlanSendSetQueryCmd(prAdapter,
						CMD_ID_SET_PATTERN_FILTER,
						TRUE,
						FALSE,
						TRUE,
						nicCmdEventSetCommon,
						nicOidCmdTimeoutCommon,
						sizeof(struct CMD_RX_PATTERN_FILTER),
						(PUINT_8)&rCmdRxFilter,
						NULL, 0);
	}

	return rWlanStatus;
}
/* end of wlanoidSetRxFilter() */

/*----------------------------------------------------------------------------*/
/*!
* \brief This routine is called to set TX GENERATOR
*
* \param[in] pvAdapter Pointer to the Adapter structure.
* \param[out] pvQueryBuf A pointer to the buffer that holds the result of
*						   the query.
...
/*----------------------------------------------------------------------------*/
WLAN_STATUS
wlanoidSetTxHeartbeat(IN P_ADAPTER_T prAdapter,
				IN PVOID pvQueryBuffer,
				IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	WLAN_STATUS rWlanStatus = WLAN_STATUS_SUCCESS;
	static struct CMD_TX_PATTERN_GENERATOR rCmdTxGenerator; /*in case stack overflow when deep function call*/

	DEBUGFUNC("wlanoidSetTxHeartbeat");
	DBGLOG(INIT, INFO, "%s\n", __func__);
	...
	*pu4QueryInfoLen = sizeof(struct PARAM_DRIVER_FW_TX_GENERATOR);

	if (u4QueryBufferLen < sizeof(struct PARAM_DRIVER_FW_TX_GENERATOR))
		return WLAN_STATUS_INVALID_LENGTH;

	kalMemCopy(&rCmdTxGenerator, pvQueryBuffer, sizeof(struct CMD_TX_PATTERN_GENERATOR));

	pr_info("TxHeartbeat cmd=%d id=%u Length=%u u4Period=%u\r\n",
		rCmdTxGenerator.eOpcode, rCmdTxGenerator.ucPatternIndex,
		rCmdTxGenerator.u2EthernetLength, rCmdTxGenerator.u4Period);

	{
		rWlanStatus = wlanSendSetQueryCmd(prAdapter,
						CMD_ID_PATTERN_GENERATOR,
						TRUE,
						FALSE,
						TRUE,
						nicCmdEventSetCommon,
						nicOidCmdTimeoutCommon,
						sizeof(struct CMD_TX_PATTERN_GENERATOR),
						(PUINT_8)&rCmdTxGenerator,
						NULL, 0);
	}

	return rWlanStatus;
}
/* end of wlanoidSetTxHeartbeat() */

/*----------------------------------------------------------------------------*/
/*!
* \brief This routine is called to set TCP GENERATOR
*
* \param[in] pvAdapter Pointer to the Adapter structure.
* \param[out] pvQueryBuf A pointer to the buffer that holds the result of
*						   the query.
...
/*----------------------------------------------------------------------------*/
WLAN_STATUS
wlanoidSetTcpHeartbeat(IN P_ADAPTER_T prAdapter,
				IN PVOID pvQueryBuffer,
				IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	WLAN_STATUS rWlanStatus = WLAN_STATUS_SUCCESS;
	static struct CMD_TCP_PATTERN_GENERATOR rCmdTcpGenerator; /*in case stack overflow when deep function call*/

	DEBUGFUNC("wlanoidSetTcpHeartbeat");
	DBGLOG(INIT, INFO, "%s\n", __func__);
	...
	*pu4QueryInfoLen = sizeof(struct PARAM_DRIVER_FW_TCP_GENERATOR);

	if (u4QueryBufferLen < sizeof(struct PARAM_DRIVER_FW_TCP_GENERATOR))
		return WLAN_STATUS_INVALID_LENGTH;

	kalMemCopy(&rCmdTcpGenerator, pvQueryBuffer, sizeof(struct CMD_TCP_PATTERN_GENERATOR));

	pr_info("TcpHeartbeat cmd=%d id=%u Period=%u Timeout=%u ipId=%u Port=%u %u Seq=%u",
		rCmdTcpGenerator.eOpcode, rCmdTcpGenerator.ucPatternIndex, rCmdTcpGenerator.u4Period,
		rCmdTcpGenerator.u4Timeout,	rCmdTcpGenerator.ipId,
		rCmdTcpGenerator.u4SrcPort, rCmdTcpGenerator.u4DestPort, rCmdTcpGenerator.u4Seq);

	{
		rWlanStatus = wlanSendSetQueryCmd(prAdapter,
						CMD_ID_TCP_GENERATOR,
						TRUE,
						FALSE,
						TRUE,
						nicCmdEventSetCommon,
						nicOidCmdTimeoutCommon,
						sizeof(struct CMD_TCP_PATTERN_GENERATOR),
						(PUINT_8)&rCmdTcpGenerator,
						NULL, 0);
	}

	return rWlanStatus;
}
/* end of wlanoidSetTcpHeartbeat() */
			 ...
			 IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = 0;
	...
	DBGLOG(OID, LOUD, "u4AddressCount (%d)\n", prNetworkAddressList->u4AddressCount);
	for (i = 0; i < prNetworkAddressList->u4AddressCount; i++) {
		DBGLOG(OID, LOUD, "prNetworkAddress (%p)\n", prNetworkAddress);
        if (prNetworkAddress->u2AddressType == PARAM_PROTOCOL_ID_TCP_IP &&
			...
			prNetworkAddress = (P_PARAM_NETWORK_ADDRESS) ((ULONG)prNetworkAddress +
						prNetworkAddress->u2AddressLength + OFFSET_OF(PARAM_NETWORK_ADDRESS, aucAddress));
        } else if (prNetworkAddress->u2AddressType == PARAM_PROTOCOL_ID_TCP_IP &&
                prNetworkAddress->u2AddressLength == sizeof(PARAM_NETWORK_ADDRESS_IPV6)) {
			u4Ipv6AddressCount++;
			prNetworkAddress = (P_PARAM_NETWORK_ADDRESS) ((ULONG)prNetworkAddress +
						prNetworkAddress->u2AddressLength + OFFSET_OF(PARAM_NETWORK_ADDRESS, aucAddress));
        }
	...
	DBGLOG(OID, INFO, "u4IpAddressCount (%u), u4Ipv6AddressCount (%u)\n", u4IpAddressCount, u4Ipv6AddressCount);
    ...
    )
{
    WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
    UINT_32 i, j;
	P_CMD_SET_IPV6_NETWORK_ADDRESS_LIST prCmdIpv6NetworkAddressList;
    P_PARAM_NETWORK_ADDRESS_LIST prNetworkAddressList = (P_PARAM_NETWORK_ADDRESS_LIST)pvSetBuffer;
    P_PARAM_NETWORK_ADDRESS prNetworkAddress;
	P_PARAM_NETWORK_ADDRESS_IPV6 prNetAddrIpv6;
    UINT_32 u4IpAddressCount, u4CmdSize;
	UINT_32 u4Ipv6AddressCount = 0;
	PUINT_8 pucBuf = (PUINT_8)pvSetBuffer;
    ...
    *pu4SetInfoLen = 0;
	...
	if (prAdapter->fgEnArpFilter)
	{
		u4CmdSize = OFFSET_OF(CMD_SET_IPV6_NETWORK_ADDRESS_LIST, arNetAddress) +
			sizeof(IPV6_NETWORK_ADDRESS) * u4Ipv6AddressCount;
		if (u4Ipv6AddressCount == 0)
			u4CmdSize = sizeof (CMD_SET_IPV6_NETWORK_ADDRESS_LIST);

		prCmdIpv6NetworkAddressList = (P_CMD_SET_IPV6_NETWORK_ADDRESS_LIST) kalMemAlloc(u4CmdSize, VIR_MEM_TYPE);
		if (NULL == prCmdIpv6NetworkAddressList)
		{
			DBGLOG(OID, ERROR, "mem alloc for prCmdIpv6NetworkAddressList failed, size:%d\n", u4CmdSize);
			return WLAN_STATUS_FAILURE;
		}

		prCmdIpv6NetworkAddressList->ucNetTypeIndex = NETWORK_TYPE_AIS_INDEX;
		prCmdIpv6NetworkAddressList->ucAddressCount = (UINT_8)u4Ipv6AddressCount;
        prNetworkAddress = prNetworkAddressList->arAddress;

        DBGLOG(OID, INFO, "u4Ipv6AddressCount (%u)\n", u4Ipv6AddressCount);

		for (i = 0, j = 0 ; i < prNetworkAddressList->u4AddressCount ; i++) {
			DBGLOG(OID, INFO, "prNetworkAddress (%p)\n", prNetworkAddress);
			if (prNetworkAddress->u2AddressType == PARAM_PROTOCOL_ID_TCP_IP &&
				prNetworkAddress->u2AddressLength == sizeof(PARAM_NETWORK_ADDRESS_IP)){
				prNetworkAddress = (P_PARAM_NETWORK_ADDRESS) ((ULONG)prNetworkAddress +
					prNetworkAddress->u2AddressLength + OFFSET_OF(PARAM_NETWORK_ADDRESS, aucAddress));
					...
					prNetworkAddress->u2AddressLength == sizeof(PARAM_NETWORK_ADDRESS_IPV6)) {
				prNetAddrIpv6 = (P_PARAM_NETWORK_ADDRESS_IPV6)prNetworkAddress->aucAddress;
				DBGLOG(OID, INFO, "prNetAddrIpv6:%p\n", prNetAddrIpv6);
				DBGLOG(OID, INFO, "addr:%d.%d.%d.%d...\n", prNetAddrIpv6->addr[0], prNetAddrIpv6->addr[1], prNetAddrIpv6->addr[2], prNetAddrIpv6->addr[3]);
				kalMemCopy(prCmdIpv6NetworkAddressList->arNetAddress[j].aucIpAddr,
					prNetAddrIpv6->addr,
					sizeof (IPV6_NETWORK_ADDRESS));
				pucBuf = prCmdIpv6NetworkAddressList->arNetAddress[j].aucIpAddr;
				DBGLOG(OID, INFO, "prNetAddrIpv6->addr:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x:%2x\n", pucBuf[0], pucBuf[1],pucBuf[2],pucBuf[3],pucBuf[8],pucBuf[9],pucBuf[10],pucBuf[11],pucBuf[12],pucBuf[13],pucBuf[14],pucBuf[15]);
				prNetworkAddress = (P_PARAM_NETWORK_ADDRESS) ((ULONG)prNetworkAddress +
						prNetworkAddress->u2AddressLength + OFFSET_OF(PARAM_NETWORK_ADDRESS, aucAddress));
				j++;
			}
		...
		rStatus = wlanSendSetQueryCmd(prAdapter,
			...
			nicOidCmdTimeoutCommon,
			u4CmdSize,
			(PUINT_8)prCmdIpv6NetworkAddressList,
			pvSetBuffer,
			u4SetBufferLen
			);
		      ...
		      IN PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = sizeof(UINT_16);
	*(PUINT_16) pvQueryBuffer = prAdapter->rWifiVar.rConnSettings.u2CountryCode;
...
#if CFG_SHOW_NVRAM
	prRegInfo->au2CountryCode[0] = ((UINT_16) pucCountry[0]);
	prRegInfo->au2CountryCode[1] = ((UINT_16) pucCountry[1]);
#endif

	if (NULL != pCountryPwrTable) {
		...
		if (0 != pCountryPwrTable->r2GBandEdgePwr.fg2G4BandEdgePwrUsed) {
			rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrCCK = pCountryPwrTable->r2GBandEdgePwr.cBandEdgeMaxPwrCCK;
			rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrOFDM20 = pCountryPwrTable->r2GBandEdgePwr.cBandEdgeMaxPwrOFDM20;
			rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrOFDM40 = pCountryPwrTable->r2GBandEdgePwr.cBandEdgeMaxPwrOFDM40;
		} else {
...
#if CFG_SHOW_NVRAM
		if (0 != pCountryPwrTable->r2GBandEdgePwr.fg2G4BandEdgePwrUsed) {
			prRegInfo->fg2G4BandEdgePwrUsed = pCountryPwrTable->r2GBandEdgePwr.fg2G4BandEdgePwrUsed;
		}
		prRegInfo->cBandEdgeMaxPwrCCK = rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrCCK;
		prRegInfo->cBandEdgeMaxPwrOFDM20 = rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrOFDM20;
		prRegInfo->cBandEdgeMaxPwrOFDM40 = rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrOFDM40;
#endif

		/*set CMD_ID_SET_EDGE_TXPWR_LIMIT_5G to chip*/
		if (0 != pCountryPwrTable->r5GBandEdgePwr.uc5GBandEdgePwrUsed && 0 != pCountryPwrTable->ucSupport5GBand) {
			rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrCCK = 0;
			rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrOFDM20 = pCountryPwrTable->r5GBandEdgePwr.c5GBandEdgeMaxPwrOFDM20;
			rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrOFDM40 = pCountryPwrTable->r5GBandEdgePwr.c5GBandEdgeMaxPwrOFDM40;
		} else {
			rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrCCK = 0;
			rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrOFDM20 = MAX_TX_POWER;
			rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrOFDM40 = MAX_TX_POWER;
...
#if CFG_SHOW_NVRAM
		if (0 != pCountryPwrTable->r5GBandEdgePwr.uc5GBandEdgePwrUsed && 0 != pCountryPwrTable->ucSupport5GBand) {
			prRegInfo->aucEFUSE[OFFSET_fg5GBandEdgePwrUsed] = pCountryPwrTable->r5GBandEdgePwr.uc5GBandEdgePwrUsed;
		}
		prRegInfo->aucEFUSE[OFFSET_c5GBandEdgeMaxPwrOFDM20] = rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrOFDM20;
		prRegInfo->aucEFUSE[OFFSET_c5GBandEdgeMaxPwrOFDM40] = rCmdEdgeTxPwrLimit.cBandEdgeMaxPwrOFDM40;
#endif

		/*set CMD_ID_SET_TX_PWR to chip*/
		if (0 != pCountryPwrTable->ucTxPwrValid){
			wlanSendSetQueryCmd(prAdapter,
			CMD_ID_SET_TX_PWR,
			TRUE,
			FALSE, FALSE, NULL, NULL, sizeof(CMD_TX_PWR_T), (PUINT_8)(&pCountryPwrTable->rTxPwr), NULL, 0);

#if CFG_SHOW_NVRAM
			prRegInfo->ucTxPwrValid	= pCountryPwrTable->ucTxPwrValid;
			prRegInfo->rTxPwr.cTxPwr2G4Cck	= pCountryPwrTable->rTxPwr.cTxPwr2G4Cck;
			prRegInfo->rTxPwr.cTxPwr2G4OFDM_BPSK	= pCountryPwrTable->rTxPwr.cTxPwr2G4OFDM_BPSK;
			prRegInfo->rTxPwr.cTxPwr2G4OFDM_QPSK	= pCountryPwrTable->rTxPwr.cTxPwr2G4OFDM_QPSK;
			prRegInfo->rTxPwr.cTxPwr2G4OFDM_16QAM	= pCountryPwrTable->rTxPwr.cTxPwr2G4OFDM_16QAM;
			prRegInfo->rTxPwr.cTxPwr2G4OFDM_48Mbps	= pCountryPwrTable->rTxPwr.cTxPwr2G4OFDM_48Mbps;
			prRegInfo->rTxPwr.cTxPwr2G4OFDM_54Mbps	= pCountryPwrTable->rTxPwr.cTxPwr2G4OFDM_54Mbps;
			prRegInfo->rTxPwr.cTxPwr2G4HT20_BPSK	= pCountryPwrTable->rTxPwr.cTxPwr2G4HT20_BPSK;
			prRegInfo->rTxPwr.cTxPwr2G4HT20_QPSK	= pCountryPwrTable->rTxPwr.cTxPwr2G4HT20_QPSK;
			prRegInfo->rTxPwr.cTxPwr2G4HT20_16QAM	= pCountryPwrTable->rTxPwr.cTxPwr2G4HT20_16QAM;
			prRegInfo->rTxPwr.cTxPwr2G4HT20_MCS5	= pCountryPwrTable->rTxPwr.cTxPwr2G4HT20_MCS5;
			prRegInfo->rTxPwr.cTxPwr2G4HT20_MCS6	= pCountryPwrTable->rTxPwr.cTxPwr2G4HT20_MCS6;
			prRegInfo->rTxPwr.cTxPwr2G4HT20_MCS7	= pCountryPwrTable->rTxPwr.cTxPwr2G4HT20_MCS7;
			prRegInfo->rTxPwr.cTxPwr2G4HT40_BPSK	= pCountryPwrTable->rTxPwr.cTxPwr2G4HT40_BPSK;
			prRegInfo->rTxPwr.cTxPwr2G4HT40_QPSK	= pCountryPwrTable->rTxPwr.cTxPwr2G4HT40_QPSK;
			prRegInfo->rTxPwr.cTxPwr2G4HT40_16QAM	= pCountryPwrTable->rTxPwr.cTxPwr2G4HT40_16QAM;
			prRegInfo->rTxPwr.cTxPwr2G4HT40_MCS5	= pCountryPwrTable->rTxPwr.cTxPwr2G4HT40_MCS5;
			prRegInfo->rTxPwr.cTxPwr2G4HT40_MCS6	= pCountryPwrTable->rTxPwr.cTxPwr2G4HT40_MCS6;
			prRegInfo->rTxPwr.cTxPwr2G4HT40_MCS7	= pCountryPwrTable->rTxPwr.cTxPwr2G4HT40_MCS7;
			prRegInfo->rTxPwr.cTxPwr5GOFDM_BPSK 	= pCountryPwrTable->rTxPwr.cTxPwr5GOFDM_BPSK;
			prRegInfo->rTxPwr.cTxPwr5GOFDM_QPSK 	= pCountryPwrTable->rTxPwr.cTxPwr5GOFDM_QPSK;
			prRegInfo->rTxPwr.cTxPwr5GOFDM_16QAM	= pCountryPwrTable->rTxPwr.cTxPwr5GOFDM_16QAM;
			prRegInfo->rTxPwr.cTxPwr5GOFDM_48Mbps	= pCountryPwrTable->rTxPwr.cTxPwr5GOFDM_48Mbps;
			prRegInfo->rTxPwr.cTxPwr5GOFDM_54Mbps	= pCountryPwrTable->rTxPwr.cTxPwr5GOFDM_54Mbps;
			prRegInfo->rTxPwr.cTxPwr5GHT20_BPSK 	= pCountryPwrTable->rTxPwr.cTxPwr5GHT20_BPSK;
			prRegInfo->rTxPwr.cTxPwr5GHT20_QPSK 	= pCountryPwrTable->rTxPwr.cTxPwr5GHT20_QPSK;
			prRegInfo->rTxPwr.cTxPwr5GHT20_16QAM	= pCountryPwrTable->rTxPwr.cTxPwr5GHT20_16QAM;
			prRegInfo->rTxPwr.cTxPwr5GHT20_MCS5 	= pCountryPwrTable->rTxPwr.cTxPwr5GHT20_MCS5;
			prRegInfo->rTxPwr.cTxPwr5GHT20_MCS6 	= pCountryPwrTable->rTxPwr.cTxPwr5GHT20_MCS6;
			prRegInfo->rTxPwr.cTxPwr5GHT20_MCS7 	= pCountryPwrTable->rTxPwr.cTxPwr5GHT20_MCS7;
			prRegInfo->rTxPwr.cTxPwr5GHT40_BPSK 	= pCountryPwrTable->rTxPwr.cTxPwr5GHT40_BPSK;
			prRegInfo->rTxPwr.cTxPwr5GHT40_QPSK 	= pCountryPwrTable->rTxPwr.cTxPwr5GHT40_QPSK;
			prRegInfo->rTxPwr.cTxPwr5GHT40_16QAM	= pCountryPwrTable->rTxPwr.cTxPwr5GHT40_16QAM;
			prRegInfo->rTxPwr.cTxPwr5GHT40_MCS5 	= pCountryPwrTable->rTxPwr.cTxPwr5GHT40_MCS5;
			prRegInfo->rTxPwr.cTxPwr5GHT40_MCS6 	= pCountryPwrTable->rTxPwr.cTxPwr5GHT40_MCS6;
			prRegInfo->rTxPwr.cTxPwr5GHT40_MCS7 	= pCountryPwrTable->rTxPwr.cTxPwr5GHT40_MCS7;
#endif
		}
	    ...
	    OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	UINT_8 pref5gMed = 0;
	UINT_8 pref5gLo = 0;
	PUINT_8 pucQueryBuffer = (PUINT_8)pvQueryBuffer;
	...
	*pu4QueryInfoLen = 0;
#if AMZN_5GHZ_PREF
	scanGet5gRoamingPreference(&rangeHi, &rangeMed, &rangeLo, &pref5gHi, &pref5gMed, &pref5gLo);
#endif
	*pu4QueryInfoLen = kalSprintf(pucQueryBuffer, "rssiRangeHi:%d\n"
		"rssiRangeMed:%d\nrssiRangeLo:%d\npref5GhzHi:%d\npref5GhzMed:%d\n"
		"pref5GhzLow:%d\n", rangeHi, rangeMed, rangeLo, pref5gHi, pref5gMed, pref5gLo);
	pucQueryBuffer += *pu4QueryInfoLen;
	*pu4QueryInfoLen += kalSprintf(pucQueryBuffer, "roaming_enable: %s\n"
		"RcpiAdjustStep: %d\nRcpiLowThreshold:%d\nRoamRetryCount:%d\n"
		"RoamingStableTimeout:%d\n", prRoamCtrl->fgEnable ? "true":"false",
		prRoamCtrl->ucRcpiAdjustStep, prRoamCtrl->u2RcpiLowThr, prRoamCtrl->ucRoamingRetryCount,
		prRoamCtrl->ucRoamingStableTimeout);
	return WLAN_STATUS_SUCCESS;
}
...
wlanoidSetRoamParams(IN P_ADAPTER_T prAdapter,
			IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	PUINT_8 pucSetBuf = (PUINT_8)pvSetBuffer;
	INT_32 i4Status = 0;
	P_ROAMING_INFO_T prRoamInfo = &prAdapter->rWifiVar.rRoamingInfo;
	ASSERT(prAdapter);
	ASSERT(pvSetBuffer);
	if (kalStrnCmp(pucSetBuf, "roam_5g_bias ", 13) == 0) {
		INT_32 rangeHi = 0;
		INT_32 rangeMed = 0;
		INT_32 rangeLo = 0;
		UINT_32 pref5gHi = 0;
		...
		UINT_32 pref5gLo = 0;
		pucSetBuf += 13;
		i4Status = sscanf(pucSetBuf, "%d,%d,%d,%u,%u,%u", &rangeHi, &rangeMed,
				&rangeLo, &pref5gHi, &pref5gMed, &pref5gLo);
		if (i4Status != 6)
			return WLAN_STATUS_INVALID_DATA;
#if AMZN_5GHZ_PREF
		scanSet5gRoamingPreference(rangeHi, rangeMed, rangeLo,
					(UINT_8)pref5gHi, (UINT_8)pref5gMed, (UINT_8)pref5gLo);
#endif
		return WLAN_STATUS_SUCCESS;
	} else if (kalStrnCmp(pucSetBuf, "roam_param ", 11) == 0)
		pucSetBuf += 11;
	else
		return WLAN_STATUS_INVALID_DATA;

	i4Status = sscanf(pucSetBuf, "%hhu,%hhu,%hu,%hhu,%hhu", &prRoamInfo->rRoamCtrl.fgEnable,
		&prRoamInfo->rRoamCtrl.ucRcpiAdjustStep, &prRoamInfo->rRoamCtrl.u2RcpiLowThr,
		&prRoamInfo->rRoamCtrl.ucRoamingRetryCount, &prRoamInfo->rRoamCtrl.ucRoamingStableTimeout);

	DBGLOG(REQ, INFO, "roaming_params: %d,%d,%d,%d,%d\n", prRoamInfo->rRoamCtrl.fgEnable,
		prRoamInfo->rRoamCtrl.ucRcpiAdjustStep, prRoamInfo->rRoamCtrl.u2RcpiLowThr,
		prRoamInfo->rRoamCtrl.ucRoamingRetryCount, prRoamInfo->rRoamCtrl.ucRoamingStableTimeout);

	if (i4Status != 5) {
		DBGLOG(REQ, ERROR, "fail\n");
		return WLAN_STATUS_INVALID_DATA;
	}
	return wlanSendSetQueryCmd(prAdapter,
				   ...
				   nicCmdEventSetCommon,
				   nicOidCmdTimeoutCommon,
				   sizeof(CMD_ROAMING_CTRL_T),
				   (PUINT_8)&prRoamInfo->rRoamCtrl, pvSetBuffer, u4SetBufferLen);
}
			...
			IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	DBGLOG(ROAMING, EVENT, "Current Time = %d\n", kalGetTimeTick());
...
#ifdef CONFIG_MTK_WIFI_ANTENNA_SELECT
WLAN_STATUS
wlanoid_set_antenna_switch(IN P_ADAPTER_T prAdapter,
			   IN PVOID pvSetBuffer,
			   IN UINT_32 u4SetBufferLen,
			   OUT PUINT_32 pu4SetInfoLen)
{
	unsigned int ant_num = *(unsigned int *)pvSetBuffer;
	WLAN_STATUS ret = WLAN_STATUS_SUCCESS;

	switch (ant_num) {
	case 0:
		ret = antennaSwitch(prAdapter, ANT1, TRUE);
		break;
	case 1:
		ret = antennaSwitch(prAdapter, ANT2, TRUE);
		break;
	default:
		break;
	}
	...
	DBGLOG(OID, INFO,
	       "antenna switch : ant_num = %u\n", ant_num);

	return ret;
}

WLAN_STATUS
wlanoid_set_antenna_rssi(IN P_ADAPTER_T prAdapter,
			 IN PVOID pvSetBuffer,
			 IN UINT_32 u4SetBufferLen,
			 OUT PUINT_32 pu4SetInfoLen)
{
	...
	struct antenna_select_param *params =
		(struct antenna_select_param *)pvSetBuffer;

	ant_sel = &(prAdapter->rWifiVar.ant_select_info);

	memcpy(&ant_sel->params, params, sizeof(ant_sel->params));

	DBGLOG(OID, INFO,
	       "antenna_select_param : ant_num = %u, query_time = %u\n",
...
WLAN_STATUS
wlanoid_get_antenna_rssi(IN P_ADAPTER_T prAdapter,
			 OUT PVOID pvQueryBuffer,
			 IN UINT_32 u4QueryBufferLen,
			 OUT PUINT_32 pu4QueryInfoLen)
{
	PARAM_RSSI *avg_rssi = (PARAM_RSSI *)pvQueryBuffer;
	int ret = 0;
	...
	*pu4QueryInfoLen = sizeof(PARAM_RSSI);
	...
	ret = antenna_select_start(prAdapter, avg_rssi);

	if (ret) {
		DBGLOG(OID, ERROR, "Fail(%d)\n", ret);
		return WLAN_STATUS_FAILURE;
	}
	return WLAN_STATUS_PENDING;
	/* return WLAN_STATUS_SUCCESS; */
...
WLAN_STATUS
wlanoid_update_antenna_config(IN P_ADAPTER_T prAdapter,
			      IN PVOID pvSetBuffer,
			      IN UINT_32 u4SetBufferLen,
			      OUT PUINT_32 pu4SetInfoLen)
{
	return antenna_select_update_config(prAdapter, TRUE);
}

WLAN_STATUS
wlanoid_get_antenna_config(IN P_ADAPTER_T prAdapter,
			   OUT PVOID pvQueryBuffer,
			   IN UINT_32 u4QueryBufferLen,
			   OUT PUINT_32 pu4QueryInfoLen)
{
	...
	struct antenna_select_config *config =
		(struct antenna_select_config *)pvQueryBuffer;
	...
	*pu4QueryInfoLen = sizeof(struct antenna_select_config);

	ant_sel = &(prAdapter->rWifiVar.ant_select_info);
	config->ant_num = ant_sel->config.ant_num;
	config->rssi = ant_sel->config.rssi;

	DBGLOG(OID, INFO, "ant_num=%u, rssi=%d\n",
...
WLAN_STATUS
wlanoid_set_antenna_moving_avg_mode(IN P_ADAPTER_T prAdapter,
				    IN PVOID pvSetBuffer,
				    IN UINT_32 u4SetBufferLen,
				    OUT PUINT_32 pu4SetInfoLen)
{
	struct antenna_select_info *ant_sel;
	PARAM_CUSTOM_SW_CTRL_STRUCT_T sw_ctrl_info;
	unsigned char moving_avg_mode = *(unsigned char *)pvSetBuffer;

	ant_sel = &(prAdapter->rWifiVar.ant_select_info);

	sw_ctrl_info.u4Id = 0xa0350000;
	sw_ctrl_info.u4Data = moving_avg_mode;

	ant_sel->moving_avg_mode = moving_avg_mode;

	DBGLOG(OID, INFO, "moving_avg_mode=%u", moving_avg_mode);

	return wlanoidSetSwCtrlWrite(prAdapter,
				     &sw_ctrl_info,
				     sizeof(sw_ctrl_info),
				     pu4SetInfoLen);
}
...
)
{
	...
	*pu4QueryInfoLen = sizeof(CMD_SW_DBG_CTRL_T);

	if (u4QueryBufferLen < sizeof(CMD_SW_DBG_CTRL_T))
		return WLAN_STATUS_INVALID_LENGTH;
	kalMemZero(&rCmdSwCtrl, sizeof(CMD_SW_DBG_CTRL_T));

	rCmdSwCtrl.u4Id = 0xb000 << 16;
	rCmdSwCtrl.u4Data = 0;
	...
	return wlanSendSetQueryCmd(prAdapter,	/* prAdapter */
					...
					TRUE,	/* fgNeedResp */
					TRUE,	/* fgIsOid */
					nicCmdEventDbgCntr, /* pfCmdDoneHandler */
	...
	)
{
	...
	DEBUGFUNC("wlanoidSetResetCounter");
	DBGLOG(OID, LOUD, "\n");
			 ...
			 IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	struct BSS_TRANSITION_MGT_PARAM_T *prBtmMgt = &prAdapter->rWifiVar.rAisSpecificBssInfo.rBTMParam;
	PUINT_8 pucQueryReason = (PUINT_8)pvSetBuffer;

	if (prAdapter->rWifiVar.arBssInfo[NETWORK_TYPE_AIS_INDEX].eConnectionState !=
		PARAM_MEDIA_STATE_CONNECTED && prAisFsmInfo->prTargetStaRec &&
		prAisFsmInfo->prTargetStaRec->fgSupportBTM) {
		prBtmMgt->ucDialogToken = wnmGetBtmToken();
		if (pucQueryReason)
			prBtmMgt->ucQueryReason = *pucQueryReason;
		else
