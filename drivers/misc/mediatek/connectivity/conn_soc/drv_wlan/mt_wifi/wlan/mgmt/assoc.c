				       ...
				       IN PUINT_8 pucBuffer, IN UINT_8 aucMACAddress[], IN OUT PUINT_16 pu2PayloadLen)
{
	...
	/* Dynamic adjust the listen interval, just ensure that the minimal listen interval * beacon interval no less than 600ms*/
	DBGLOG(SAA, TRACE, "Use Dynamic DTIM skip count\n");
	u2ListenInterval = MAX_DTIM_SKIP_COUNT;
	if (IS_STA_IN_AIS(prStaRec)) {
		prTargetBssDesc = scanSearchBssDescByBssid(prAdapter, prAssocFrame->aucBSSID);
