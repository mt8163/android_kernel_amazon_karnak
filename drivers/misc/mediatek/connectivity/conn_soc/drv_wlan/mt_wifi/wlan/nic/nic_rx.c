...
VOID nicRxProcessDataPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	BOOLEAN fIsDummy = FALSE, fgIs1xFrame = FALSE;
	...
	if (secCheckClassError(prAdapter,
			prSwRfb,
			prStaRec,
			fgIs1xFrame) == TRUE &&
...
*/
			...
			if (prResults)
				DBGLOG(SCN, INFO, "ssid=XXXXXX, rssi=%d, channel=%d \r\n",
...
*/
/*----------------------------------------------------------------------------*/
VOID nicRxProcessEventPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb, IN int is_wakeup)
{
	...
	case EVENT_ID_UPDATE_BCM_DEBUG:
		...
		break;
#endif
	case EVENT_ID_HEARTBEAT_INFO:
		{
			struct EVENT_HEARTBEAT_INFO_T *prHeartbeatInfo;
			prHeartbeatInfo = (struct EVENT_HEARTBEAT_INFO_T *) (prEvent->aucBuffer);

			kalHandleTcpHeartbeatTimeout(prAdapter->prGlueInfo, prHeartbeatInfo);
		}
		break;
...
VOID nicRxProcessMgmtPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
	...
	if (nicUpdateWakeupStatistics(prAdapter, RX_MGMT_INT) != 0) {
		...
		if (aucDebugModule[DBG_RX_IDX] & DBG_CLASS_INFO)
			dumpMemory8((PUINT_8) prSwRfb->pvHeader, (UINT_32)prSwRfb->u2PacketLen);
	}
#endif
...
#if	CFG_SUPPORT_WAKEUP_STATISTICS
				prEvent = (P_WIFI_EVENT_T)prSwRfb->pucRecvBuff;
				if (prEvent->ucEID != EVENT_ID_GET_AIS_BSS_INFO &&
					prEvent->ucEID != EVENT_ID_SLEEPY_NOTIFY) {
					is_wakeup = nicUpdateWakeupStatistics(prAdapter, RX_EVENT_INT);
				}
#endif
				nicRxProcessEventPacket(prAdapter, prSwRfb, is_wakeup);
				break;
