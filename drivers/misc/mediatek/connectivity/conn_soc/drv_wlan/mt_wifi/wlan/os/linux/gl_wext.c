...
static const struct iw_priv_args rIwPrivTable[] = {
	...
	{IOCTL_SET_INTS, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 4, 0, ""},
	...
	{PRIV_CMD_ANT_SELECT, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 4, 0,
	 "set_ant_query"},
	{PRIV_CMD_ANT_SELECT, 0, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1,
	 "get_ant_query"},
#endif
...
wext_set_scan(IN struct net_device *prNetDev,
	      ...
	      IN struct iw_scan_req *prIwScanReq, IN char *pcExtra)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	UINT_32 u4BufLen = 0;
...
static int wext_set_country(IN struct net_device *prNetDev, IN struct iwreq *iwr)
{
	...
	rStatus = kalIoctl(prGlueInfo, wlanoidUpdatePowerTable, &aucCountry[0], 2, FALSE, FALSE, TRUE, FALSE, &u4BufLen);
	if (rStatus != WLAN_STATUS_SUCCESS) {
...
int wext_support_ioctl(IN struct net_device *prDev, IN struct ifreq *prIfReq, IN int i4Cmd)
{
	...
	kalMemCopy(&iwctl, iwr, sizeof(struct iwreq));
	iwctl.u.data.pointer = compat_ptr(iwp_compat->pointer);
	iwctl.u.data.length = iwp_compat->length;
	iwctl.u.data.flags = iwp_compat->flags;
	iwr = &iwctl;
#endif
	...
	case SIOCSIWSCAN:	/* 0x8B18, scan request */
		if (iwr->u.data.pointer == NULL)
			...
			if (copy_from_user(prIwScanReq, iwr->u.data.pointer,
					   iwr->u.data.length)) {
				ret = -EFAULT;
			} else {
				if (prIwScanReq->essid_len > IW_ESSID_MAX_SIZE)
					prIwScanReq->essid_len = IW_ESSID_MAX_SIZE;
				ret = wext_set_scan(prDev, NULL, prIwScanReq,
						    &(prIwScanReq->essid[0]));
			}

			kalMemFree(prIwScanReq, VIR_MEM_TYPE,
				   iwr->u.data.length);
			prIwScanReq = NULL;
		}
#endif
...
#ifdef CONFIG_COMPAT
	iwp_compat->pointer = ptr_to_compat(iwctl.u.data.pointer);
	iwp_compat->length = iwctl.u.data.length;
	iwp_compat->flags = iwctl.u.data.flags;
#endif
	return ret;
}				/* wext_support_ioctl */
