...
long WMT_unlocked_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case WMT_IOCTL_SET_PATCH_NUM:{
			...
			pPatchInfo = kcalloc(arg, sizeof(WMT_PATCH_INFO), GFP_ATOMIC);
			if (!pPatchInfo) {
...
static long WMT_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
		...
		case COMPAT_WMT_IOCTL_SEND_BGW_DS_CMD:
			...
			break;
		case WMT_IOCTL_SET_STP_MODE:
		case WMT_IOCTL_FUNC_ONOFF_CTRL:
		case WMT_IOCTL_LPBK_POWER_CTRL:
		case WMT_IOCTL_GET_CHIP_INFO:
		case WMT_IOCTL_SET_LAUNCHER_KILL:
		case WMT_IOCTL_SET_PATCH_NUM:
		...
		case WMT_IOCTL_WMT_COREDUMP_CTRL:
			ret = WMT_unlocked_ioctl(filp, cmd, arg); //int
			break;
		default:
			WMT_ERR_FUNC("Unsupported cmd error !! cmd[0x%x]\n", cmd);
			printk(KERN_ERR "Unsupported cmd error !! cmd[0x%x]\n", cmd);
			ret = -1;
			break;
		}
		return ret;
