...
INT32 stp_dbg_nl_send(char *aucMsg, UINT8 cmd, INT32 len)
{
...
INT32 stp_dbg_soc_put_emi_dump_to_nl(PUINT8 data_buf, INT32 dump_len)
{
...
INT32 stp_dbg_set_fw_info(UINT8 *issue_info, UINT32 len, ENUM_STP_FW_ISSUE_TYPE issue_type)
{
	...
	if ((STP_FW_ASSERT_ISSUE == issue_type) ||
		...
		if (netdev == NULL) {
			STP_DBG_ERR_FUNC("Fail to get %s net device\n", ifname);
		} else {
			STP_DBG_INFO_FUNC("Send FW Crash Event!\n");
