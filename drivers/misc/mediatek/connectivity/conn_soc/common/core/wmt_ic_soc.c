...
/* static UINT8 WMT_COEX_SETTING_CUSTOM_FEATURE_CMD[] = { 0x01, 0x10, 0x04,
 * 0x00, 0x17, 0x00, 0x00,0x00 }; */
static UINT8 WMT_COEX_SETTING_CUSTOM_FEATURE_EVT[] = {
...
#ifdef CONFIG_MTK_WIFI_ANTENNA_SWITCH
	INIT_CMD(NULL,
		 WMT_COEX_SETTING_CUSTOM_FEATURE_EVT,
		 "coex_custom_feature"),
#endif
};
...
#ifdef CONFIG_MTK_WIFI_ANTENNA_SWITCH
	iRet = wmt_core_reg_rw_raw(1, ANTENNA_SELECTION_REG, &val, 0x1);
	if (iRet) {
		WMT_ERR_FUNC("set antenna selection fail (%d)\n", iRet);
		return -20;
	}
#endif
...
static INT32 wmt_stp_init_coex(VOID)
{
	...
	WMT_GEN_CONF *pWmtGenConf;
#ifdef CONFIG_MTK_WIFI_ANTENNA_SWITCH
	struct coex_custom_feature custom_cmd = {0};
	UINT8 buffer[256];
	UINT32 idx = 0;
	UINT32 i;
#endif
...
#ifdef CONFIG_MTK_WIFI_ANTENNA_SWITCH
	custom_cmd.type = 0x01;
	custom_cmd.cmd_opcode = 0x10;
	custom_cmd.reserve = 0x00;
	custom_cmd.data_opcode = 0x17;

	for (i = 0; i < COEX_CUSTOM_MAX; i++) {
		struct coex_custom_feature_tlv custom_data;

		switch (i) {
		case COEX_CUSTOM_MANUAL_ANT_CTRL:
			if (pWmtGenConf->coex_custom_feature_manual_ant_ctrl) {
				custom_data.type = i;
				custom_data.len = 1;

				if (custom_data.data && idx +
				    custom_data.len <= sizeof(buffer)) {
					buffer[idx] = custom_data.type;
					idx++;
					buffer[idx] = custom_data.len;
					idx++;

					custom_data.data[0] = 1;

					osal_memcpy(&buffer[idx],
						    custom_data.data,
						    custom_data.len);
	...
	custom_cmd.data_len = idx;

	if (custom_cmd.data_len) {
		osal_memcpy(custom_cmd.data, buffer, custom_cmd.data_len);

		coex_table[COEX_CUSTOM_FEATURE].cmd = (UINT8 *)&custom_cmd;
		/* total data length + data length + data_opcode */
		custom_cmd.cmd_len = custom_cmd.data_len + 2;
		coex_table[COEX_CUSTOM_FEATURE].cmdSz =
			custom_cmd.cmd_len + 4;

		if (gWmtDbgLvl >= WMT_LOG_DBG)
			wmt_core_dump_data(&coex_table[COEX_CUSTOM_FEATURE].cmd[0],
					   coex_table[COEX_CUSTOM_FEATURE].str,
					   coex_table[COEX_CUSTOM_FEATURE].cmdSz);
	}
#endif

	iRet = wmt_core_init_script(coex_table, sizeof(coex_table) /
				    sizeof(coex_table[0]));
#ifdef CONFIG_MTK_WIFI_ANTENNA_SWITCH
	coex_table[COEX_CUSTOM_FEATURE].cmd = NULL;
#endif
