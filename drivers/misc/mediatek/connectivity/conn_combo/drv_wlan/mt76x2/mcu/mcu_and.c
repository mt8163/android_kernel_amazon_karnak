...
static void andes_rx_process_cmd_msg(RTMP_ADAPTER *ad, struct cmd_msg *rx_msg)
{
	...
	if (rx_info->evt_type == RSP_EVT_TYPE_COEX_RSP_EVENT) {
		...
		pRxRspEvtPayload = GET_OS_PKT_DATAPTR(net_pkt) + sizeof(*rx_info);
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s: RSP_EVT_TYPE_COEX_RSP_EVENT, RspEvtType = %d (len=%d)\n",
			  __func__, rx_info->evt_type, rx_info->pkt_len));

		pDW = (PUINT32) (&pRxRspEvtPayload[0]);
		DBGPRINT(RT_DEBUG_TRACE, ("%s: 0x%d\n", __func__, *pDW));

		pDW = (PUINT32) (&pRxRspEvtPayload[4]);
		DBGPRINT(RT_DEBUG_TRACE, ("%s: 0x%d\n", __func__, *pDW));

		pDW = (PUINT32) (&pRxRspEvtPayload[8]);
		DBGPRINT(RT_DEBUG_TRACE, ("%s: 0x%d\n", __func__, *pDW));
		pDW = (PUINT32) (&pRxRspEvtPayload[0]);

		switch (*pDW) {
		case RSP_UpdateCOEXMode:
			DBGPRINT(RT_DEBUG_TRACE, ("Receive COEX OPERATION RSP_UpdateCOEXMode\n"));
			pDW = (PUINT32) (&pRxRspEvtPayload[4]);
			ad->CoexMode.UpdateMode = (*pDW);
			DBGPRINT(RT_DEBUG_WARN, ("Receive COEX update mode %d\n", *pDW));
			break;

		case RSP_BT_DEVICE_LINK_STATUS:
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Receive COEX OPERATION RSP_BT_DEVICE_LINK_STATUS\n"));
			pDW = (PUINT32) (&pRxRspEvtPayload[4]);
			ad->BTStatusFlags = (*pDW);
			CoexParseBTStatus(ad);
			DBGPRINT(RT_DEBUG_WARN,
				 ("Receive COEX BT_DEVICE_LINK_STATUS = 0x%x\n", *pDW));
			break;
		default:
			DBGPRINT(RT_DEBUG_ERROR,
				 ("Receive COEX update packets (%d) BUT is a unkonw COEX Operation\n",
				  *pDW));
			break;
		     ...
		     u8 tim_byte_offset, u8 tim_byte_pattern)
{
	...
	if (op == RADIO_ON_ADVANCE) {
		/* Listen interval, Pre-TBTT, TIM info, PS Mode */
		var_len += 16;
	}
...
static void andes_traffic_switch_callback(struct cmd_msg *msg,
					  char *rsp_payload,
					  u16 rsp_payload_len)
{
	DBGPRINT(RT_DEBUG_TRACE, ("%s\n", __func__));
...
{				/* , RTMP_ADAPTER *ad, u32 cal_id, u32 param) */
	...
	if (!RTMP_TEST_FLAG(ad, fRTMP_ADAPTER_START_UP)) {
		printk("RTMP_ADAPTER_START_UP flag has been clear !\n");
		goto error;
	}
#ifdef NEW_WOW_SUPPORT
	...
	wow_param.Value = TRUE;	/* Parameter */
	...
	ret = andes_send_cmd_msg(ad, msg);
	if (ret) {
		DBGPRINT(RT_DEBUG_ERROR, ("WOW send command Step1 fail\n"));
		goto error;
	}
	RtmpOsMsDelay(1);

	/* Step 2 : Set WOW mask configuration  */
	var_len = sizeof(mask_cfg);
	NdisZeroMemory(&mask_cfg, var_len);
	/* WOW config */
	mask_cfg.Config_Type = WOW_MASK_CFG;	/* detect mask config */
	mask_cfg.Function_Enable = 3;	/* Enabel GTK re-key enable for bit0 and ARP/NS offload for bit 1 */
	...
	ret = andes_send_cmd_msg(ad, msg);
	if (ret) {
		DBGPRINT(RT_DEBUG_ERROR, ("WOW send command Step2 fail\n"));
		goto error;
	}
	RtmpOsMsDelay(1);
	...
	if (ad->WOW_Cfg.TcpPortCntV4 != 0 || ad->WOW_Cfg.TcpPortCntV6 != 0) {
		...
		/*
		   for(i=0;i<4;i++)
		   tcp_cfg.IP[i] = ad->WOW_Cfg.WowIP[i];
		 */
		...
		if (tcp_cfg.TcpPortCountV6 != 0) {
			...
			for (i = 0; i < tcp_cfg.TcpPortCountV6; i++) {
				if (ad->WOW_Cfg.TcpPortV6[i] == 0) {
					...
					tcp_cfg.TcpPortCountV6 = i;
					break;
				}
				tcp_cfg.TcpPortV6[i] = ad->WOW_Cfg.TcpPortV6[i];
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Set TCP port v6:%d\n", tcp_cfg.TcpPortV6[i]));
			}
		}
		...
		ret = andes_send_cmd_msg(ad, msg);
		if (ret) {
			DBGPRINT(RT_DEBUG_ERROR, ("WOW send command set TCP wake up fail\n"));
			goto error;
		}
		/*
		   DBGPRINT(RT_DEBUG_TRACE, ("TCP IP:%d.%d.%d.%d\nTCP port count v4:%d\nTCP port count v6:%d\n",
		   tcp_cfg.IP[0], tcp_cfg.IP[1], tcp_cfg.IP[2], tcp_cfg.IP[3],tcp_cfg.TcpPortCountV4,tcp_cfg.TcpPortCountV6));
		 */
		RtmpOsMsDelay(1);
	}
	...
	if (ad->WOW_Cfg.UdpPortCntV4 != 0 || ad->WOW_Cfg.UdpPortCntV6 != 0) {
		...
		if (udp_cfg.UdpPortCountV6 != 0) {
			...
			for (i = 0; i < udp_cfg.UdpPortCountV6; i++) {
				if (ad->WOW_Cfg.UdpPortV6[i] == 0) {
					...
					udp_cfg.UdpPortCountV6 = i;
					break;
				}
				udp_cfg.UdpPortV6[i] = ad->WOW_Cfg.UdpPortV6[i];
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Set UDP port v6:%d\n", udp_cfg.UdpPortV6[i]));
			}
		}
		...
		ret = andes_send_cmd_msg(ad, msg);
		if (ret) {
			DBGPRINT(RT_DEBUG_ERROR, ("WOW send command set UDP wake up fail\n"));
			goto error;
		}
		DBGPRINT(RT_DEBUG_TRACE,
			 ("UDP port count v4:%d\nUDP port count v6:%d\n", udp_cfg.UdpPortCountV4,
	...
	pEntry = &ad->MacTab.Content[BSSID_WCID];
	/* Offload ARP */
	if (NdisCmpMemory(zeroIP, ad->WOW_Cfg.ArpIP, sizeof(zeroIP)) != 0) {
		var_len = sizeof(offload_arp_cfg);
		NdisZeroMemory(&offload_arp_cfg, var_len);
		offload_arp_cfg.Config_Type = WOW_ARP_CFG;
		offload_arp_cfg.IpIdx = 0;
		offload_arp_cfg.IsEnable = 1;
		offload_arp_cfg.OffloadType = 0;	/* offload ARP */
		for (i = 0; i < 4; i++)
	...
	/* Step 3 : Set WOW security configuration if encrypted */
	if (ad->StaCfg.wdev.AuthMode >= Ndis802_11AuthModeWPAPSK) {
		var_len = sizeof(sec_cfg);

		NdisZeroMemory(&sec_cfg, var_len);
		/* Security config */
		...
		pEntry = &ad->MacTab.Content[BSSID_WCID];
#ifdef RT_CFG80211_SUPPORT
		NdisCopyMemory(sec_cfg.PTK, ad->WOW_Cfg.PTK, 64);
		...
		ret = andes_send_cmd_msg(ad, msg);
		if (ret) {
			DBGPRINT(RT_DEBUG_ERROR, ("WOW send command Step3 fail\n"));
			goto error;
		}
	...
	/* Step 4 : Set WOW Infra configuration */
	var_len = sizeof(infra_cfg);
	NdisZeroMemory(&infra_cfg, var_len);
	/* WOW config */
	...
	if (OPSTATUS_TEST_FLAG(ad, fOP_STATUS_MEDIA_STATE_CONNECTED)) {
		...
		DBGPRINT(RT_DEBUG_ERROR, ("WOW config AP status:TRUEl\n"));
	} else {
		infra_cfg.AP_Status = FALSE;
		DBGPRINT(RT_DEBUG_ERROR, ("WOW config AP status:FALSEl\n"));
	}
	...
	ret = andes_send_cmd_msg(ad, msg);
	if (ret) {
		DBGPRINT(RT_DEBUG_ERROR, ("WOW send command Step4 fail\n"));
		goto error;
	}
	...
	if (ad->WOW_Cfg.extMode & WOW_FAKE_SUSPEND) {
		...
		if (ad->WOW_Cfg.nHoldTime == 0) {
			wow_param.GPIO_Action_1 = ((ad->WOW_Cfg.bHighActive << 31) | 0);
			wow_param.GPIO_Action_2 = ((ad->WOW_Cfg.bHighActive << 31) | 0);
			wow_param.GPIO_Action_3 = ((ad->WOW_Cfg.bHighActive << 31) | 0);
		} else {
			wow_param.GPIO_Action_1 = ((ad->WOW_Cfg.bHighActive << 31) | 0);
			wow_param.GPIO_Action_2 =
			    ((ad->WOW_Cfg.bHighActive << 31) | ad->WOW_Cfg.nHoldTime);
			wow_param.GPIO_Action_3 = (((ad->WOW_Cfg.bHighActive ^ 1) << 31) | 0);
		}
	...
	ret = andes_send_cmd_msg(ad, msg);
	if (ret) {
		DBGPRINT(RT_DEBUG_ERROR, ("WOW send command Step5 fail\n"));
		goto error;
	}

	RtmpOsMsDelay(1);

	/*Step 5.1: Set Always Trigger Wakeup */
	if (ad->WOW_Cfg.bAlwaysTrigger) {
		if ((ad->WOW_Cfg.bInBand == 0) && (ad->WOW_Cfg.nHoldTime != 0)) {
			var_len = sizeof(wow_trig_cfg);

			NdisZeroMemory(&wow_trig_cfg, var_len);
			...
			andes_init_cmd_msg(msg, CMD_WOW_FEATURE,
			    FALSE, 0, TRUE, FALSE, 0, NULL, NULL);

			/* WOW config */
			data = (PUINT32)&wow_trig_cfg;
			for (i = 0; i < (var_len/4); i++) {
				value = cpu2le32(data[i]);
				andes_append_cmd_msg(msg, (char *)&value, 4);
			}
			...
			ret = andes_send_cmd_msg(ad, msg);
			if (ret) {
				DBGPRINT(RT_DEBUG_ERROR, ("WOW send command step5.1 fail\n"));
				goto error;
			}
	...
	var_len = sizeof(wow_aid_channel);

	NdisZeroMemory(&wow_aid_channel, var_len);
	/* WOW config */
	...
	ret = andes_send_cmd_msg(ad, msg);
	if (ret) {
		DBGPRINT(RT_DEBUG_ERROR, ("WOW send command Step5.2 fail\n"));
		goto error;
	}
	...
	ret = andes_send_cmd_msg(ad, msg);
	if (ret) {
		DBGPRINT(RT_DEBUG_ERROR, ("WOW send command Step6 fail\n"));
		goto error;
	}
	count = 0;
	while (count < WAIT_EMPTY_RETRY_COUNT) {
		if (cmdCbk) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("%s: %dms for waiting traffic swtich from host to fw operation\n",
				  __func__, count));
			break;
		}

		count++;
		if (count == WAIT_EMPTY_RETRY_COUNT && (cmdCbk != 1))
			DBGPRINT(RT_DEBUG_ERROR,
				 ("%s: traffic swtich from host to fw operation fail\n",
				  __func__));
	}
...
int andes_wow_disable(struct _RTMP_ADAPTER *ad)
{
	NEW_WOW_PARAM_STRUCT wow_param;
#ifdef WOW_BUF_PKT_SUPPORT
	struct NEW_WOW_BUF_PKT_CFG_STRUCT buf_pkt;
#endif /* WOW_BUF_PKT_SUPPORT */
	struct cmd_msg *msg;
	unsigned int var_len;
	u32 value, i;
...
/* INT32 Value; */
	...
	var_len = sizeof(buf_pkt);
	NdisZeroMemory(&buf_pkt, var_len);
	...
	andes_init_cmd_msg(msg, CMD_WOW_QUERY, FALSE, 0, TRUE, FALSE, 0, NULL, NULL);
	data = (PUINT32)&buf_pkt;
	for (i = 0; i < (var_len/4); i++) {
		value = cpu2le32(data[i]);
		andes_append_cmd_msg(msg, (char *)&value, 4);
	}
	...
	while (count < WAIT_RETRY_COUNT) {
		...
		if (count == WAIT_RETRY_COUNT
		    && ((Value & (BIT2 | BIT3)) != (BIT2 | BIT3)))
			DBGPRINT(RT_DEBUG_ERROR,
				 ("%s: MAC Tx/Rx not enabled\n", __func__));
	}
	...
	while (count < WAIT_RETRY_COUNT) {
		RtmpusecDelay(WAIT_DELAY_TIME_US);	/* 1 ms */
		read_reg(ad, 0x41, 0x080c, &Value);

		if ((Value & (BIT24 | BIT25)) == (BIT24 | BIT25))
			break;

		DBGPRINT(RT_DEBUG_ERROR,
			 ("%s: Unable to switch traffic [%X]\n", __func__,
			  Value));
		Value |= (BIT24 | BIT25);
		write_reg(ad, 0x41, 0x080c, Value);

		count++;
		if (count == WAIT_RETRY_COUNT
		    && ((Value & (BIT24 | BIT25)) != (BIT24 | BIT25)))
			DBGPRINT(RT_DEBUG_ERROR,
