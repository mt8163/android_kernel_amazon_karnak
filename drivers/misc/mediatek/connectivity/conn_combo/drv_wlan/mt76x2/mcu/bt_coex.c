...
static INT SendAndesCoexFrameInfo(IN PRTMP_ADAPTER pAd, IN UINT32 TriggerNumber)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("%s: Triggernumber = %u, Valid = %u, NodeType = %u, BssHashID = %u, ",
		  __func__, coexProtectionFrameInfo.Triggernumber,
		  coexProtectionFrameInfo.Valid, coexProtectionFrameInfo.NodeType,
		  coexProtectionFrameInfo.BssHashID));
	DBGPRINT(RT_DEBUG_ERROR,
		 ("FrameType = %u, CmdParametersLength = %d\n",
		 ...
		 IN UCHAR Channel, IN UCHAR CentralChannel, IN BOOLEAN Disable, IN UINT32 BssHashID)
{
	...
	DBGPRINT(RT_DEBUG_WARN, ("%s: LinkStatus = %u, BW = %u, Channel = %u, ",
				  __func__, coexAFH.LinkStatus, coexAFH.BW,
				  coexAFH.Channel));
	DBGPRINT(RT_DEBUG_WARN, ("BssHashID = %u PktLength = %d\n",
	      ...
	      IN UCHAR BBPCurrentBW, IN UCHAR Channel, IN UCHAR CentralChannel, IN BOOLEAN Disable)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s: COEX AFH Start Ch = %d, AFH End Ch = %d, Channel = %d, "
				  "CentralChannel = %d\n",
				  __func__,
				   ...
				   IN PUCHAR pAddr, IN UINT32 OPMode, IN PMAC_TABLE_ENTRY pEntry)
{
	...
	TXWI_STRUCT ProtectionFrameTxWI;
	TXWI_STRUCT *pTxWI;
	UCHAR *ptr = NULL;
	UINT i = 0;
	/* PHY_CFG PhyCfg = {0}; */
	...
	NdisZeroMemory(&ProtectionFrameTxWI, sizeof(TXWI_STRUCT));
	...
	pTxWI = (TXWI_STRUCT *) &ProtectionFrameTxWI;
	RTMPWriteTxWI(pAd,
		      pTxWI,
		      FALSE,
		      FALSE,
...
VOID EstablishFrameBundle(IN PRTMP_ADAPTER pAd,
			  IN PUCHAR pAddr, IN PUCHAR bssid, UINT32 OPMode, IN PMAC_TABLE_ENTRY pEntry)
{
	UCHAR n0, n1, n2, n3 = 0;

	DBGPRINT(RT_DEBUG_OFF, ("%s, OwnMac:%02x:%02x:%02x:%02x:%02x:%02x\n"
			, __func__, PRINT_MAC(pAddr)));
	n0 = PF_FRONT_0;
	if ((n0 != NULLFRAMESPACE) && !(pAd->CoexMode.LinkStatus&COEX_GO_LINK)) {
		DBGPRINT(RT_DEBUG_OFF, ("COEX: CTS2Self Trigger#:%d\n", n0));
		...
		DBGPRINT(RT_DEBUG_OFF, ("COEX, %s: Op mode:%x, p2p GO On:%x\n"
				, __func__
				, OPMode
	...
	if ((n1 != NULLFRAMESPACE) && (OPMode == COEX_OPMODE_STA)) {
		DBGPRINT(RT_DEBUG_OFF, ("COEX: PS1 Trigger#:%d\n", n1));
	...
	if ((n2 != NULLFRAMESPACE) && !(pAd->CoexMode.LinkStatus&COEX_GO_LINK)) {
		DBGPRINT(RT_DEBUG_OFF, ("COEX: CFE Trigger#:%d\n", n2));
	...
	if ((n3 != NULLFRAMESPACE) && (OPMode == COEX_OPMODE_STA)) {
		DBGPRINT(RT_DEBUG_OFF, ("COEX: PS0 Trigger#:%d\n", n3));
			...
			IN UCHAR WlanStatus, IN UINT32 PrivilegeTime, IN UINT32 BssHashID)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s: CoexOperation = %u, WlanStatus = %u, PrivilegeTime = %u,",
				  __func__,
...
VOID InitBTCoexistence(IN PRTMP_ADAPTER pAd)
{
	...
	if (IS_MT7650(pAd) || IS_MT7630(pAd) || IS_MT76x2(pAd)) {
		...
		if (IS_MT7662(pAd)) {
			...
			if (pAd->EEPROMImage[EXTERN_COMPONENT_SETTING_3] == COEX_TDD_ANT_SET)
...
VOID MLMEHook(IN PRTMP_ADAPTER pAd, IN UCHAR WlanStatus, IN UINT32 BssHashID)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("COEX: unit = %u, WlanStatus:%x\n", unit, WlanStatus));
...
VOID COEXLinkDown(IN PRTMP_ADAPTER pAd, PUCHAR pAddr)
{
	...
	if (pAd->CoexMode.LinkStatus&COEX_GO_LINK) {
		...
		EstablishFrameBundle(pAd, pAd->CurrentAddress, pAd->CommonCfg.Bssid, COEX_OPMODE_STA,
...
VOID CoexLinkUpHandler(RTMP_ADAPTER *pAd, struct COEX_LINKUP_INFO info)
{
	...
	if (IS_MT76XXBTCOMBO(pAd)) {
		...
		if (info.Channel > 14) {
			...
			UINT8 eeprom = pAd->EEPROMImage[EXTERN_COMPONENT_SETTING_3];
			EstablishFrameBundle(pAd, info.OwnMac, info.Bssid, info.coexType, info.pEntry);
		...
		DBGPRINT(RT_DEBUG_OFF, ("%s, type:%x, BW:%x, ch:%x, ctrl_ch:%x, link stat:%x\n"
			    , __func__, info.coexType, info.BBPCurrentBW
			    , info.Channel, info.CentralChannel, pAd->CoexMode.LinkStatus));
		DBGPRINT(RT_DEBUG_OFF, ("%s, BSSID:%02x:%02x:%02x:%02x:%02x:%02x, Hash:%x\n"
			    , __func__, PRINT_MAC(info.Bssid), QueryHashID(pAd, info.Bssid, FALSE)));
	}
}
