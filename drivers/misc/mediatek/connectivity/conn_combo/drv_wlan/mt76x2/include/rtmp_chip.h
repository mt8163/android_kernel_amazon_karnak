...
#define IS_MT7600(_pAd)		(((_pAd)->ChipID & 0xffff0000) == 0x76000000)
...
#define PMF_ENCRYPT_MODE_1      1	/* Data packets do hardware encryption, management packet do software encryption. */
	...
	UINT32 fw_len;
#define fNONSUPPORT_EDCCA_UPDATE 1
	UINT32 fw_non_support;	/* non-support function on selected firmware */
...
struct _RTMP_CHIP_OP_ {
	...
	/* ITxBf calibration */
	int (*fITxBfDividerCalibration) (struct _RTMP_ADAPTER *pAd, int calFunction, int calMethod,
					 UCHAR *divPhase);
	void (*fITxBfLNAPhaseCompensate) (struct _RTMP_ADAPTER *pAd);
	int (*fITxBfCal) (struct _RTMP_ADAPTER *pAd, PSTRING arg);
	int (*fITxBfLNACalibration) (struct _RTMP_ADAPTER *pAd, int calFunction, int calMethod,
				     BOOLEAN gBand);
...
#ifdef CONFIG_ANDES_SUPPORT
	int (*sendCommandToAndesMcu) (struct _RTMP_ADAPTER *pAd, UCHAR QueIdx, UCHAR cmd,
				      UCHAR *pData, USHORT DataLen, BOOLEAN FlgIsNeedLocked);
#endif				/* CONFIG_ANDES_SUPPORT */
	...
	int (*ATEReadExternalTSSI) (struct _RTMP_ADAPTER *pAd, PSTRING arg);
	 ...
	 VOID(*AsicTxAlcGetAutoAgcOffset) (IN struct _RTMP_ADAPTER *pAd,
					   IN PCHAR pDeltaPwr,
	...
	/* Antenna */
	 VOID(*AsicAntennaDefaultReset) (struct _RTMP_ADAPTER *pAd,
					 union _EEPROM_ANTENNA_STRUC *pAntenna);
	...
	int (*sc_rf_random_write) (struct _RTMP_ADAPTER *ad, BANK_RF_CR_REG *table, u32 num,
				   u32 flags);
	int (*Calibration) (struct _RTMP_ADAPTER *ad, UINT32 CalibrationID,
	...
	int (*AsicWOWEnable) (struct _RTMP_ADAPTER *ad);
	int (*AsicWOWDisable) (struct _RTMP_ADAPTER *ad);
...
VOID RtmpChipWriteHighMemory(IN struct _RTMP_ADAPTER *pAd,
			     IN USHORT Offset, IN UINT32 Value, IN UINT8 Unit);

VOID RtmpChipWriteMemory(IN struct _RTMP_ADAPTER *pAd,
			 IN USHORT Offset, IN UINT32 Value, IN UINT8 Unit);
