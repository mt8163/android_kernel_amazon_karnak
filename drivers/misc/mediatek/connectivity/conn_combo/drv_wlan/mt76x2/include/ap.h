...
BOOLEAN APBridgeToWirelessSta(IN PRTMP_ADAPTER pAd,
			      IN PUCHAR pHeader,
			      IN UINT HdrLen,
			      IN PUCHAR pData, IN UINT DataLen, IN ULONG fromwdsidx);
...
BOOLEAN APChkCls2Cls3Err(IN RTMP_ADAPTER *pAd, IN UCHAR Wcid, IN HEADER_802_11 *pHeader);

VOID RTMPDescriptorEndianChange(UCHAR *pData, ULONG DescriptorType);

VOID RTMPFrameEndianChange(IN RTMP_ADAPTER *pAd,
			   IN UCHAR *pData, IN ULONG Dir, IN BOOLEAN FromRxDoneInt);

/* ap_assoc.c */

VOID APAssocStateMachineInit(IN PRTMP_ADAPTER pAd,
...
/* ap_sync.c */
VOID APSyncStateMachineInit(IN PRTMP_ADAPTER pAd,
			    IN STATE_MACHINE * Sm, OUT STATE_MACHINE_FUNC Trans[]);
...
VOID SupportRate(IN PUCHAR SupRate,
		 ...
		 IN PUCHAR ExtRate,
		 IN UCHAR ExtRateLen,
		 OUT PUCHAR *Rates, OUT PUCHAR RatesLen, OUT PUCHAR pMaxSupportRate);
...
/* ap_mlme.c */
VOID APDetectOverlappingExec(IN PVOID SystemSpecific1,
			     IN PVOID FunctionContext,
			     IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

VOID APMlmePeriodicExec(RTMP_ADAPTER *pAd);

BOOLEAN APMsgTypeSubst(IN PRTMP_ADAPTER pAd,
...
MAC_TABLE_ENTRY *APSsPsInquiry(IN PRTMP_ADAPTER pAd,
			       IN PUCHAR pAddr,
			       OUT SST * Sst,
...
BOOLEAN PeerAssocReqCmmSanity(IN PRTMP_ADAPTER pAd,
			      IN BOOLEAN isRessoc,
			      IN VOID *Msg, IN INT MsgLen, IN IE_LISTS *ie_lists);
...
/* Define in ap.c */
BOOLEAN DOT1X_InternalCmdAction(IN RTMP_ADAPTER *pAd, IN MAC_TABLE_ENTRY *pEntry, IN UINT8 cmd);
