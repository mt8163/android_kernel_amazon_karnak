...
typedef union GNU_PACKED _TXWI_STRUCT {
#ifdef RLT_MAC
...
typedef union GNU_PACKED _TXINFO_STRUCT {
#ifdef RTMP_MAC
...
*/
typedef union GNU_PACKED _RXWI_STRUCT {
#ifdef RLT_MAC
...
#ifdef RT_BIG_ENDIAN
typedef struct GNU_PACKED _RXINFO_STRUCT {
...
#ifdef RT_BIG_ENDIAN
typedef union _AC_TXOP_CSR0_STRUCT {
	struct {
...
#ifdef RT_BIG_ENDIAN
typedef union _EDCA_AC_CFG_STRUCT {
	struct {
		UINT32:12;
...
#ifdef RT_BIG_ENDIAN
typedef union _TX_RTY_CFG_STRUCT {
	struct {
		UINT32 rsv:1;
...
#ifdef RT_BIG_ENDIAN
typedef union _TX_LINK_CFG_STRUCT {
	struct {
...
#define PROT_AUTO_TX_CFG		0x1648
...
/* RX_STA_CNT1_STRUCT: RX False CCA count & RX LONG frame count */
#define RX_STA_CNT1		0x1704
#ifdef RT_BIG_ENDIAN
typedef union _RX_STA_CNT1_STRUCT {
	struct {
		UINT16 PlcpErr;
		UINT16 FalseCca;
	} field;
...
#ifdef RT_BIG_ENDIAN
typedef union _TX_STA_CNT0_STRUCT {
	struct {
...
#ifdef RT_BIG_ENDIAN
typedef union _TX_STA_CNT1_STRUCT {
	struct {
		UINT16 TxRetransmit;
		UINT16 TxSuccess;
	} field;
...
INT get_pkt_phymode_by_rxwi(struct _RTMP_ADAPTER *pAd, RXWI_STRUCT *rxwi);
INT get_pkt_rssi_by_rxwi(struct _RTMP_ADAPTER *pAd, RXWI_STRUCT *rxwi, INT size, CHAR *rssi);
INT get_pkt_snr_by_rxwi(struct _RTMP_ADAPTER *pAd, RXWI_STRUCT *rxwi, INT size, UCHAR *snr);
...
INT rtmp_mac_pbf_init(struct _RTMP_ADAPTER *pAd);

INT rtmp_mac_init(struct _RTMP_ADAPTER *pAd);

#ifdef MCS_LUT_SUPPORT
INT set_lut_phy_rate(struct _RTMP_ADAPTER *pAd, UINT8 wcid,
