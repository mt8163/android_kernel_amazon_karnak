...
#define BT_STATUS_TEST_LINK_UP(_pAd)		(BT_STATUS_TEST_FLAG(_pAd, fBTSTATUS_BT_ACTIVE) && \
						(BT_STATUS_TEST_FLAG(pAd, fBTSTATUS_BT_BWL) || BT_STATUS_TEST_FLAG(pAd, fBTSTATUS_BT_BE) \
						|| BT_STATUS_TEST_FLAG(pAd, fBTSTATUS_BT_SYNC) || BT_STATUS_TEST_FLAG(pAd, fBTSTATUS_BT_SNIFF)))
...
VOID EstablishFrameBundle(IN struct _RTMP_ADAPTER *pAd,
			  IN PUCHAR pAddr, IN PUCHAR bssid, IN UINT32 OPMode, IN struct _MAC_TABLE_ENTRY *pEntry);
