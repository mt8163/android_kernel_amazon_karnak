...
VOID InitLookupTable(IN PRTMP_ADAPTER pAd)
{
	...
	for (band = IEEE80211_BAND_2G; band < band_nums; band++) {
		...
		for (pos_idx = 2; pos_idx <= pos_bound;) {
			...
			LUT_POSITIVE(band, pos_idx) =
			    LUT_POSITIVE(band,
					 pos_idx - 1) +
			    comp_val /* (Step[band] - (step_cnt+1) + 1) */;
		...
		for (neg_idx = 1; neg_idx <= neg_bound;) {
			...
			if (MinusStepNum[band][step_cnt] != 0 || step_cnt >= 8) {
				if ((neg_idx + 1) >= IdxTmp + MinusStepNum[band][step_cnt]
				    && step_cnt < 8) {
...
VOID AsicGetAutoAgcOffsetForTemperatureSensor(IN RTMP_ADAPTER *pAd,
					      IN CHAR *pDeltaPwr,
					      ...
					      IN CHAR *pDeltaPowerByBbpR1)
{
	...
	*pTotalDeltaPwr = 0;
	...
	if (bAutoTxAgc) {
		...
		if (tx_temp_dbg) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("[temp. compensation] current temp = %d(0x%x), pAd->curr_temp=%d, temper_val=%d\n",
		...
		if (tx_temp_dbg) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("[temp. compensation] index = %d\n",
				  pAd->TxPowerCtrl.LookupTableIndex));
			DBGPRINT(RT_DEBUG_TRACE,
				 ("[temp. compensation] f(%d)= %d\n",
				  pAd->TxPowerCtrl.LookupTableIndex - 1,
				  LookupTable[LookupTableIndex - 1]));
			DBGPRINT(RT_DEBUG_TRACE,
				 ("[temp. compensation] f(%d)= %d\n",
				  pAd->TxPowerCtrl.LookupTableIndex,
		...
		if (CurrentTemp >
		    RefTemp + LookupTable[LookupTableIndex + 1] +
		    ((LookupTable[LookupTableIndex + 1] - LookupTable[LookupTableIndex]) >> 2)
			...
			pAd->TxPowerCtrl.LookupTableIndex++;
			   ...
			   RefTemp + LookupTable[LookupTableIndex] -
			   ((LookupTable[LookupTableIndex] -
			     LookupTable[LookupTableIndex - 1]) >> 2) && LookupTableIndex > 0) {
			if (tx_temp_dbg) {
				DBGPRINT(RT_DEBUG_TRACE, ("[temp. compensation] --\n"));
			}
			LookupTableIndex--;
		...
		if (tx_temp_dbg) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("[temp. compensation] idxTxPowerTable=%d, idxTxPowerTable2=%d, TuningTableUpperBound=%d\n",
				  pAd->TxPowerCtrl.idxTxPowerTable +
				  pAd->TxPowerCtrl.LookupTableIndex,
				  pAd->TxPowerCtrl.idxTxPowerTable2 +
				  pAd->TxPowerCtrl.LookupTableIndex, TuningTableUpperBound));
		}
...
#else
		TuningTableIndex0 = pAd->TxPowerCtrl.idxTxPowerTable
		    + pAd->TxPowerCtrl.LookupTableIndex + pAd->hw_cfg.cur_ch_pwr[0];
#endif /* E3_DBG_FALLBACK */
		/* The boundary verification */
		TuningTableIndex0 =
		    (TuningTableIndex0 >
		     TuningTableUpperBound) ? TuningTableUpperBound : TuningTableIndex0;
		TuningTableIndex0 =
		    (TuningTableIndex0 <
		     LOWERBOUND_TX_POWER_TUNING_ENTRY) ? LOWERBOUND_TX_POWER_TUNING_ENTRY :
		    TuningTableIndex0;
		TxPowerTuningTableEntry0 =
		    &TxPowerTuningTable[TuningTableIndex0 + TX_POWER_TUNING_ENTRY_OFFSET];
...
#else
		TuningTableIndex1 = pAd->TxPowerCtrl.idxTxPowerTable2
		    + pAd->TxPowerCtrl.LookupTableIndex + pAd->hw_cfg.cur_ch_pwr[1];
#endif /* E3_DBG_FALLBACK */
		/* The boundary verification */
		TuningTableIndex1 =
		    (TuningTableIndex1 >
		     TuningTableUpperBound) ? TuningTableUpperBound : TuningTableIndex1;
		TuningTableIndex1 =
		    (TuningTableIndex1 <
		     LOWERBOUND_TX_POWER_TUNING_ENTRY) ? LOWERBOUND_TX_POWER_TUNING_ENTRY :
		    TuningTableIndex1;
		TxPowerTuningTableEntry1 =
		    &TxPowerTuningTable[TuningTableIndex1 + TX_POWER_TUNING_ENTRY_OFFSET];
		...
		RFValue |=
		    ((TxPowerTuningTableEntry0->RF_TX_ALC >
		      rf_txpwr_bnd) ? rf_txpwr_bnd : TxPowerTuningTableEntry0->RF_TX_ALC);
		RT30xxWriteRFRegister(pAd, RF_R49, RFValue);
		if (tx_temp_dbg) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("[temp. compensation] (tx0)RF_TX_ALC = %x, MAC_PowerDelta = %d, TuningTableIndex = %d\n",
				  TxPowerTuningTableEntry0->RF_TX_ALC,
				  TxPowerTuningTableEntry0->MAC_PowerDelta, TuningTableIndex0));
			DBGPRINT(RT_DEBUG_TRACE,
				 ("\tUpdate RF_R49[0:5] to 0x%x\n", (RFValue & 0x3f)));
		}
		...
		if (tx_temp_dbg) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("[temp. compensation] (tx1)RF_TX_ALC = %x, MAC_PowerDelta = %d, TuningTableIndex = %d\n",
				  TxPowerTuningTableEntry1->RF_TX_ALC,
				  TxPowerTuningTableEntry1->MAC_PowerDelta, TuningTableIndex1));
			DBGPRINT(RT_DEBUG_TRACE,
				 ("\tUpdate RF_R50[0:5] to 0x%x\n", (RFValue & 0x3f)));
		}
	...
	*pAgcCompensate = *pTxAgcCompensate;
	...
	if (tx_temp_dbg) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("<--%s(): pDeltaPwr=%d, TotalDeltaPwr=%d, AgcCompensate=%d, DeltaPowerByBbpR1=%d\n",
			  __func__, *pDeltaPwr, *pTotalDeltaPwr, *pAgcCompensate,
			  *pDeltaPowerByBbpR1));
	}
}
...
VOID AsicGetTxPowerOffset(RTMP_ADAPTER *pAd, ULONG *TxPwr)
{
...
#ifdef DOT11_VHT_AC
	if (pAd->CommonCfg.BBPCurrentBW == BW_80 && pAd->CommonCfg.Channel > 14) {
	...
	if (pAd->CommonCfg.BBPCurrentBW == BW_40) {
		if (pAd->CommonCfg.CentralChannel > 14) {
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[0].MACRegisterOffset = TX_PWR_CFG_0;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[0].RegisterValue =
			    pAd->Tx40MPwrCfgABand[0];
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[1].MACRegisterOffset = TX_PWR_CFG_1;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[1].RegisterValue =
			    pAd->Tx40MPwrCfgABand[1];
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[2].MACRegisterOffset = TX_PWR_CFG_2;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[2].RegisterValue =
			    pAd->Tx40MPwrCfgABand[2];
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[3].MACRegisterOffset = TX_PWR_CFG_3;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[3].RegisterValue =
			    pAd->Tx40MPwrCfgABand[3];
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[4].MACRegisterOffset = TX_PWR_CFG_4;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[4].RegisterValue =
			    pAd->Tx40MPwrCfgABand[4];
		} else {
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[0].MACRegisterOffset = TX_PWR_CFG_0;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[0].RegisterValue =
			    pAd->Tx40MPwrCfgGBand[0];
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[1].MACRegisterOffset = TX_PWR_CFG_1;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[1].RegisterValue =
			...
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[3].MACRegisterOffset = TX_PWR_CFG_3;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[3].RegisterValue =
			    pAd->Tx40MPwrCfgGBand[3];
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[4].MACRegisterOffset = TX_PWR_CFG_4;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[4].RegisterValue =
			    pAd->Tx40MPwrCfgGBand[4];
		}
	} else {
		if (pAd->CommonCfg.CentralChannel > 14) {
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[0].MACRegisterOffset = TX_PWR_CFG_0;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[0].RegisterValue =
			    pAd->Tx20MPwrCfgABand[0];
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[1].MACRegisterOffset = TX_PWR_CFG_1;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[1].RegisterValue =
			...
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[3].MACRegisterOffset = TX_PWR_CFG_3;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[3].RegisterValue =
			    pAd->Tx20MPwrCfgABand[3];
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[4].MACRegisterOffset = TX_PWR_CFG_4;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[4].RegisterValue =
			    pAd->Tx20MPwrCfgABand[4];
		} else {
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[0].MACRegisterOffset = TX_PWR_CFG_0;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[0].RegisterValue =
			    pAd->Tx20MPwrCfgGBand[0];
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[1].MACRegisterOffset = TX_PWR_CFG_1;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[1].RegisterValue =
			...
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[3].MACRegisterOffset = TX_PWR_CFG_3;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[3].RegisterValue =
			    pAd->Tx20MPwrCfgGBand[3];
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[4].MACRegisterOffset = TX_PWR_CFG_4;
			CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[4].RegisterValue =
			    pAd->Tx20MPwrCfgGBand[4];
		}
	}
...
VOID AsicAdjustTxPower(RTMP_ADAPTER *pAd)
{
	...
	RTMP_CHIP_ASIC_AUTO_AGC_OFFSET_GET(pAd, &DeltaPwr, &TotalDeltaPower, &TxAgcCompensate,
					   &DeltaPowerByBbpR1);

	DBGPRINT(RT_DEBUG_INFO,
		 ("%s(): DeltaPwr=%d, TotalDeltaPower=%d, TxAgcCompensate=%d, DeltaPowerByBbpR1=%d\n",
		  __func__, DeltaPwr, TotalDeltaPower, TxAgcCompensate, DeltaPowerByBbpR1));
	...
	if (pAd->CommonCfg.bSKUMode == TRUE) {
		...
		GetSingleSkuDeltaPower(pAd, &TotalDeltaPower, (PULONG) &SingleSKUTotalDeltaPwr,
				       &SingleSKUBbpR1Offset);
		...
		for (i = 0; i < CfgOfTxPwrCtrlOverMAC.NumOfEntries; i++) {
			...
			if (reg_val != 0xffffffff) {
				for (j = 0; j < 8; j++) {
					...
					if (pAd->CommonCfg.bSKUMode == TRUE) {
						TotalDeltaPower =
						    SingleSKUBbpR1Offset + TotalDeltaPowerOri -
						    (CHAR) ((SingleSKUTotalDeltaPwr[i] >> j *
							     4) & 0x0F);
					...
					if ((pAd->TxPowerCtrl.bInternalTxALC ==
					     TRUE) ^ (pAd->chipCap.bTempCompTxALC == TRUE)) {
						switch (0x1314 + (i * 4)) {
						case 0x1314:
#ifdef RT8592
							if (IS_RT8592(pAd))
...
#ifdef E3_DBG_FALLBACK
					pTxPwrEntry->RegisterValue =
					    (reg_val & ~(0x0000000F << j * 4)) | (Value << j * 4);
#else
					reg_val =
					    (reg_val & ~(0x0000000F << j * 4)) | (Value << j * 4);
#endif /* E3_DBG_FALLBACK */
...
#endif /* E3_DBG_FALLBACK */
				RTMP_IO_WRITE32(pAd, pTxPwrEntry->MACRegisterOffset,
						pTxPwrEntry->RegisterValue);
			    ...
			    INOUT UCHAR *pSingleSKUBbpR1Offset)
{
	...
	criterion =
	    (UCHAR) ((CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[0].RegisterValue & 0x000F0000) >> 16);
	...
	for (i = 0; i < CfgOfTxPwrCtrlOverMAC.NumOfEntries; i++) {
		if (i == 0) {
			for (j = 0; j < 8; j++) {
				Value =
				    (CHAR) ((CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[i].
					     RegisterValue >> j * 4) & 0x0F);

				if (j < 4) {
					AdjustMaxTxPwr[i * 8 + j] = TxPwrInEEPROM + (Value - criterion) + 4;	/* CCK has 4dBm larger than OFDM */
				} else {
					AdjustMaxTxPwr[i * 8 + j] =
					    TxPwrInEEPROM + (Value - criterion);
				}

				DBGPRINT(RT_DEBUG_TRACE,
					 ("%s: offset = 0x%04X, i/j=%d/%d, (Default)Value=%d, %d\n",
					  __func__,
					  CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[i].
					  MACRegisterOffset, i, j, Value,
					  AdjustMaxTxPwr[i * 8 + j]));
			}
		} else {
			for (j = 0; j < 8; j++) {
		...
		if (CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[i].RegisterValue != 0xffffffff) {
			for (j = 0; j < 8; j++) {
				...
				if (AdjustMaxTxPwr[i * 8 + j] > CountryTxPwr) {
					...
					*(pSingleSKUTotalDeltaPwr + i) =
					    (*(pSingleSKUTotalDeltaPwr + i) &
					     ~(0x0000000F << j * 4)) | (Value << j * 4);

					DBGPRINT(RT_DEBUG_TRACE,
						 ("%s: offset = 0x%04X, i/j=%d/%d, (Exceed)Value=%d, %d\n",
						  __func__,
						  CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[i].
						  MACRegisterOffset, i, j, Value,
						  AdjustMaxTxPwr[i * 8 + j]));
					...
					DBGPRINT(RT_DEBUG_TRACE,
						 ("%s: offset = 0x%04X, i/j=%d/%d, Value=%d, %d, no change\n",
						  __func__,
						  CfgOfTxPwrCtrlOverMAC.TxPwrCtrlOverMAC[i].
						  MACRegisterOffset, i, j, Value,
						  AdjustMaxTxPwr[i * 8 + j]));
...
VOID AsicCompensatePowerViaBBP(RTMP_ADAPTER *pAd, CHAR *pTotalDeltaPower)
{
	...
	DBGPRINT(RT_DEBUG_INFO,
		 ("%s: <Before> TotalDeltaPower = %d dBm\n", __func__, *pTotalDeltaPower));
	...
	if (IS_RT65XX(pAd)) {
		...
		DBGPRINT(RT_DEBUG_INFO,
			 ("%s: <After> TotalDeltaPower = %d dBm, TXBE_R4 = 0x%0x\n", __func__,
			  *pTotalDeltaPower, bbp_val));
	}
#endif /* RT65xx */
	...
	if (pAd->chipCap.hif_type == HIF_RTMP) {
		...
		DBGPRINT(RT_DEBUG_INFO,
			 ("%s: <After> TotalDeltaPower = %d dBm, BbpR1 = 0x%02X\n", __func__,
			  *pTotalDeltaPower, BbpR1));
	}
#endif /* RTMP_BBP */
}
...
VOID RTMPReadTxPwrPerRate(RTMP_ADAPTER *pAd)
{
	...
	/* For default one, go here!! */
	{
		...
		for (i = 0; i < 5; i++) {
			...
			RT28xx_EEPROM_READ16(pAd, EEPROM_TXPOWER_BYRATE_20MHZ_2_4G + i * 4 + 2,
					     value);
...
VOID RTMPReadChannelPwr(RTMP_ADAPTER *pAd)
{
	...
	for (i = 0; i < 7; i++) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s: TxPower[%d].Power = 0x%02X, TxPower[%d].Power = 0x%02X\n",
				  __func__, i * 2, pAd->TxPower[i * 2].Power, i * 2 + 1,
				  pAd->TxPower[i * 2 + 1].Power));

			if (IS_RT5392(pAd) || IS_RT5592(pAd)) {
				DBGPRINT(RT_DEBUG_TRACE,
					 ("%s: TxPower[%d].Power2 = 0x%02X, TxPower[%d].Power2 = 0x%02X\n",
					  __func__, i * 2, pAd->TxPower[i * 2].Power2,
					  i * 2 + 1, pAd->TxPower[i * 2 + 1].Power2));
			}
		} else
...

	{
		...
		/* 2.2 Fill up power */
		for (i = 0; i < 8; i++) {
			RT28xx_EEPROM_READ16(pAd, EEPROM_A_TX_PWR_OFFSET + (choffset - 14) + i * 2,
					     Power.word);
			RT28xx_EEPROM_READ16(pAd, EEPROM_A_TX2_PWR_OFFSET + (choffset - 14) + i * 2,
					     Power2.word);
