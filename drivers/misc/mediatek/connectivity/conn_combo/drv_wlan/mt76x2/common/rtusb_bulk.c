		     ...
		     IN PTX_CONTEXT pTxContext, IN UCHAR BulkOutPipeId, IN usb_complete_t Func)
{
	...
	RTUSB_FILL_TX_BULK_URB(pUrb,
			       ...
			       bulkOutAddr,
			       pSrc,
			       pTxContext->BulkOutSize, Func, pTxContext, pTxContext->data_dma);
}
		       ...
		       IN UCHAR BulkOutPipeId, IN ULONG BulkOutSize, IN usb_complete_t Func)
{
...
#ifdef USB_BULK_BUF_ALIGMENT
	pSrc =
	&pTxContext->TransferBuffer[idx]->field.WirelessPacket[pTxContext->NextBulkOutPosition];
#else
	...
	if (BulkOutPipeId == QID_HCCA) {	/* Use EP9 when MCC is turn on for p2p */
		...
		RTUSB_FILL_TX_BULK_URB(pUrb,
				       ...
				       Func, pTxContext, pTxContext->data_dma);
		...
		if (RTMPEqualMemory(pHTTXContext->SavedPad, allzero, 4)) {
			...
			DBGPRINT_RAW(RT_DEBUG_ERROR,
				     ("WARNING-Zero-3:%02x%02x%02x%02x%02x%02x%02x%02x,CWPos=%ld, CWRPos=%ld, bCW=%d, NBPos=%ld, TBPos=%ld, TBSize=%ld\n",
				      ...
				      pBuf[7], pHTTXContext->CurWritePosition,
				      pHTTXContext->CurWriteRealPos, pHTTXContext->bCurWriting,
				      pHTTXContext->NextBulkOutPosition, TmpBulkEndPos,
				      ThisBulkSize));

			pBuf = &pWirelessPkt[pHTTXContext->CurWritePosition];
			DBGPRINT_RAW(RT_DEBUG_ERROR,
				     ("\tCWPos=%02x%02x%02x%02x%02x%02x%02x%02x\n", pBuf[0],
				      pBuf[1], pBuf[2], pBuf[3], pBuf[4], pBuf[5], pBuf[6],
				      pBuf[7]));
		}
...
VOID DoBulkIn(IN RTMP_ADAPTER *pAd)
{
	...
	pUrb = pRxContext->pUrb;
	pUrb->transfer_flags |= URB_RX_REQ_MODE1_ENABLE;
	ret = RTUSB_SUBMIT_URB(pUrb);
	if (ret != 0) {	/* fail */
#ifdef MULTI_URB
		RTMP_IRQ_UNLOCK(&pAd->SubmitBulkInLock, IrqFlags_SubmitBulkInLock);
...
USBHST_STATUS RTUSBBulkRxComplete(URBCompleteStatus Status, purbb_t pURB, pregs *pt_regs)
{
	...
	if (BulkInStatus == USB_ST_NOERROR) {
		...
		if (pRxContext->BulkInOffset) {	/* As jan's comment, it may bulk-in success but size is zero. */
			pRxContext->Readable = TRUE;	/* TODO: if not set TRUE here, how would we deal with the URB not in use and not readable? */
		}
	} else {		/* STATUS_OTHER */

		pAd->BulkInCompleteFail++;
		/* Still read this packet although it may comtain wrong bytes. */
		pRxContext->Readable = FALSE;	/* TODO: if not set TRUE here, how would we deal with the URB not in use and not readable? */
		...
		if ((!RTMP_TEST_FLAG(pAd, (fRTMP_ADAPTER_RESET_IN_PROGRESS |
			...
			DBGPRINT_RAW(RT_DEBUG_ERROR,
				     ("Bulk In Failed. Status=%d, BIIdx=0x%x, BIRIdx=0x%x, actual_length= 0x%x\n",
				      BulkInStatus, pAd->NextRxBulkInIndex,
