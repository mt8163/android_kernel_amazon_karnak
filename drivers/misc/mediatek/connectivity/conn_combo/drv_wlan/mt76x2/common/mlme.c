...
/*
	...
	==========================================================================
 */
static VOID MlmeHandler(RTMP_ADAPTER *pAd)
{
	...
	while (!MlmeQueueEmpty(&pAd->Mlme.Queue)) {
		...
		if (MlmeDequeue(&pAd->Mlme.Queue, &Elem)) {
			...
			case APCLI_AUTH_STATE_MACHINE:
				...
				if (isValidApCliIf(apcliIfIndex)) {
					...
					if (apcliIfIndex >= 64) {
						...
						AuthCurrState =
						    pAd->ApCfg.ApCliTab[apcliIfIndex].
						    RepeaterCli[CliIdx].AuthCurrState;
					} else
#endif /* MAC_REPEATER_SUPPORT */
						AuthCurrState =
			...
			case APCLI_ASSOC_STATE_MACHINE:
				...
				if (isValidApCliIf(apcliIfIndex)) {
					...
					if (apcliIfIndex >= 64) {
						...
						AssocCurrState =
						    pAd->ApCfg.ApCliTab[apcliIfIndex].
						    RepeaterCli[CliIdx].AssocCurrState;
					}
#endif /* MAC_REPEATER_SUPPORT */
			...
			case APCLI_SYNC_STATE_MACHINE:
				apcliIfIndex = Elem->Priv;
				if (isValidApCliIf(apcliIfIndex))
					StateMachinePerformAction(pAd, &pAd->Mlme.ApCliSyncMachine,
								  Elem,
								  (pAd->ApCfg.ApCliTab
								   [apcliIfIndex].SyncCurrState));
				break;
			...
			case APCLI_CTRL_STATE_MACHINE:
				...
				if (isValidApCliIf(apcliIfIndex)) {
					...
					if (apcliIfIndex >= 64) {
						...
						CtrlCurrState =
						    pAd->ApCfg.ApCliTab[apcliIfIndex].
						    RepeaterCli[CliIdx].CtrlCurrState;
					}
#endif /* MAC_REPEATER_SUPPORT */
					StateMachinePerformAction(pAd, &pAd->Mlme.ApCliCtrlMachine,
								  Elem, CtrlCurrState);
				}
				break;
#endif /* APCLI_SUPPORT */
			...
			case WPA_STATE_MACHINE:
				StateMachinePerformAction(pAd, &pAd->Mlme.WpaMachine, Elem,
							  pAd->Mlme.WpaMachine.CurrState);
				break;
#ifdef WSC_INCLUDED
			case WSC_STATE_MACHINE:
				if (pAd->pWscElme) {
					RTMP_SEM_LOCK(&pAd->WscElmeLock);
					NdisMoveMemory(pAd->pWscElme, Elem,
			...
			case IWSC_STATE_MACHINE:
				StateMachinePerformAction(pAd, &pAd->Mlme.IWscMachine, Elem,
...
NDIS_STATUS MlmeInit(RTMP_ADAPTER *pAd)
{
	...
	do {
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			...
			if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_ADVANCE_POWER_SAVE_PCIE_DEVICE)) {
				...
				RTMPInitTimer(pAd, &pAd->Mlme.PsPollTimer,
					      GET_TIMER_FUNCTION(PsPollWakeExec), pAd, FALSE);
				RTMPInitTimer(pAd, &pAd->Mlme.RadioOnOffTimer,
					      GET_TIMER_FUNCTION(RadioOnExec), pAd, FALSE);
			}
#endif /* PCIE_PS_SUPPORT */

			RTMPInitTimer(pAd, &pAd->Mlme.LinkDownTimer,
				      GET_TIMER_FUNCTION(LinkDownExec), pAd, FALSE);
			RTMPInitTimer(pAd, &pAd->StaCfg.StaQuickResponeForRateUpTimer,
				      GET_TIMER_FUNCTION(StaQuickResponeForRateUpExec), pAd, FALSE);
			pAd->StaCfg.StaQuickResponeForRateUpTimerRunning = FALSE;
			RTMPInitTimer(pAd, &pAd->StaCfg.WpaDisassocAndBlockAssocTimer,
				      GET_TIMER_FUNCTION(WpaDisassocApAndBlockAssoc), pAd, FALSE);

#ifdef RTMP_MAC_USB
			RTMPInitTimer(pAd, &pAd->Mlme.AutoWakeupTimer,
				      GET_TIMER_FUNCTION(RtmpUsbStaAsicForceWakeupTimeout), pAd,
				      FALSE);
		...
		RTMPInitTimer(pAd, &pAd->Mlme.PeriodicTimer, GET_TIMER_FUNCTION(MlmePeriodicExec),
			      pAd, TRUE);
		...
		RTMPInitTimer(pAd, &pAd->Mlme.RxAntEvalTimer,
			      GET_TIMER_FUNCTION(AsicRxAntEvalTimeout), pAd, FALSE);

#ifdef CONFIG_AP_SUPPORT
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			/* Init APSD periodic timer */
			RTMPInitTimer(pAd, &pAd->Mlme.APSDPeriodicTimer,
				      GET_TIMER_FUNCTION(APSDPeriodicExec), pAd, TRUE);
			RTMPSetTimer(&pAd->Mlme.APSDPeriodicTimer, 50);

			/* Init APQuickResponseForRateUp timer. */
			RTMPInitTimer(pAd, &pAd->ApCfg.ApQuickResponeForRateUpTimer,
				      GET_TIMER_FUNCTION(APQuickResponeForRateUpExec), pAd, FALSE);
		...
		RTMPInitTimer(pAd, &pAd->cfg80211_ctrl.P2pCTWindowTimer,
			      GET_TIMER_FUNCTION(CFG80211_P2PCTWindowTimer), pAd, FALSE);
		RTMPInitTimer(pAd, &pAd->cfg80211_ctrl.P2pSwNoATimer,
			      GET_TIMER_FUNCTION(CFG80211_P2pSwNoATimeOut), pAd, FALSE);
		RTMPInitTimer(pAd, &pAd->cfg80211_ctrl.P2pPreAbsenTimer,
			      GET_TIMER_FUNCTION(CFG80211_P2pPreAbsenTimeOut), pAd, FALSE);
...
VOID MlmeHalt(RTMP_ADAPTER *pAd)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		for (idx = 0; idx < MAX_APCLI_NUM; idx++) {
			...
			RTMPCancelTimer(&pApCliEntry->MlmeAux.WpaDisassocAndBlockAssocTimer,
					&Cancelled);

#ifdef WSC_AP_SUPPORT
			if (pApCliEntry->WscControl.WscProfileRetryTimerRunning) {
				pApCliEntry->WscControl.WscProfileRetryTimerRunning = FALSE;
				RTMPCancelTimer(&pApCliEntry->WscControl.WscProfileRetryTimer,
						&Cancelled);
			}
...
VOID MlmeResetRalinkCounters(RTMP_ADAPTER *pAd)
{
	...
	if (!ATE_ON(pAd))
		...
		NdisZeroMemory(&pAd->RalinkCounters, offsetof(COUNTER_RALINK, OneSecEnd));
		      ...
		      IN PVOID FunctionContext, IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if (pAd->CommonCfg.MO_Cfg.bEnable) {
		...
		if (stage == MO_MEAS_PERIOD) {
			...
			for (Idx = MO_MEAS_PERIOD + 1; Idx < MO_IDLE_PERIOD + 1; Idx++)
				pAd->CommonCfg.MO_Cfg.nFalseCCACnt +=
				    pAd->RalinkCounters.FalseCCACnt_100MS[Idx];
	...
	if ((RTMP_TEST_FLAG(pAd, (fRTMP_ADAPTER_HALT_IN_PROGRESS |
				  ...
				  fRTMP_ADAPTER_RESET_IN_PROGRESS | fRTMP_ADAPTER_NIC_NOT_EXIST))))
		return;
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		if ((pAd->Mlme.PeriodicRound & 0x1) &&
			    (STA_TGN_WIFI_ON(pAd)) && ((pAd->MACVersion & 0xffff) == 0x0101)) {
			UINT32 txop_cfg;
...
/*	RECBATimerTimeout(SystemSpecific1,FunctionContext,SystemSpecific2,SystemSpecific3);*/
...
#ifdef MT76x2
	if ((pAd->Mlme.OneSecPeriodicRound % 1 == 0) && IS_MT76x2(pAd) && INFRA_ON(pAd)) {
				...
				*/
		if (!RTMP_TEST_SUSPEND_FLAG(pAd,
		     fRTMP_ADAPTER_SUSPEND_STATE_SUSPENDING))
	...
	if ((pAd->ra_interval) && ((pAd->Mlme.PeriodicRound % (pAd->ra_interval / 100)) == 0)
	...
	if (pAd->Mlme.PeriodicRound % MLME_TASK_EXEC_MULTIPLE == 0) {
		...
		if (IS_MT76x0(pAd) &&
		    ...
		    (RTMP_TEST_FLAG
		     (pAd,
		      fRTMP_ADAPTER_RADIO_OFF | fRTMP_ADAPTER_DISABLE_DEQUEUEPACKET) == FALSE)) {
			if ((pAd->Mlme.OneSecPeriodicRound % 1) == 0) {
				/* TSSI compensation */
			...
			if ((INFRA_ON(pAd)
				...
				*/
				if (!RTMP_TEST_SUSPEND_FLAG(pAd,
				     fRTMP_ADAPTER_SUSPEND_STATE_SUSPENDING)) {
#endif /* NEW_WOW_SUPPORT */
					/* if infra is connected or if P2P-GO or P2P-GC on */
					mt76x2_tssi_compensation(pAd, pAd->hw_cfg.cent_ch);
				}
			}
#endif /* CONFIG_STA_SUPPORT */
#ifdef HOSTAPD_SUPPORT
			else if (pApCliEntry->Hostapd != Hostapd_Disable) {
				/* if hostapd is running */
				mt76x2_tssi_compensation(pAd, pAd->hw_cfg.cent_ch);
			}
#endif /* HOSTAPD_SUPPORT */
#if defined(CONFIG_AP_SUPPORT) && !defined(CONFIG_STA_SUPPORT)
			/* if only pure AP mode is enabled */
			mt76x2_tssi_compensation(pAd, pAd->hw_cfg.cent_ch);
#endif
		}
		...
		if (RTMP_TEST_FLAG
		    (pAd, fRTMP_ADAPTER_RADIO_OFF | fRTMP_ADAPTER_DISABLE_DEQUEUEPACKET) == FALSE) {
			if ((pAd->Mlme.OneSecPeriodicRound % 10) == 0) {
				{
				}
			}
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
...
#endif /* RTMP_MAC_USB */
					{
						...
						if (((MacReg & 0x20000000) && (MacReg & 0x80))
							...
							if (MacReg)
								RTMP_IO_WRITE32(pAd, MAC_SYS_CTRL,
										MacReg);
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		if ((pAd->ApCfg.bPartialScanning == TRUE) && (pAd->ApCfg.PartialScanChannelNum == DEFLAUT_PARTIAL_SCAN_CH_NUM)) {	/* pAd->ApCfg.PartialScanChannelNum == DEFLAUT_PARTIAL_SCAN_CH_NUM means that one partial scan is finished */
			if (((pAd->ApCfg.PartialScanBreakTime++) %
			     DEFLAUT_PARTIAL_SCAN_BREAK_TIME) == 0)
...
VOID STAMlmePeriodicExec(RTMP_ADAPTER *pAd)
{
	...
	TxTotalCnt = pAd->RalinkCounters.OneSecTxNoRetryOkCount +
	    pAd->RalinkCounters.OneSecTxRetryOkCount + pAd->RalinkCounters.OneSecTxFailCount;
		...
		if (pAd->CommonCfg.bHTProtect) {
			...
			if (pAd->CommonCfg.IOTestParm.bRTSLongProtOn == FALSE) {
				AsicUpdateProtect(pAd,
						  (USHORT) pAd->CommonCfg.AddHTInfo.AddHtInfo2.
			...
			if ((pAd->Antenna.field.TxPath == 1) &&
				...
				if ((pAd->RalinkCounters.OneSecOsTxCount[QID_AC_VO] == 0) &&
					...
					if (Ac2Cfg.field.Aifsn != 0xc) {
						...
						RTUSBWriteMACRegister(pAd, EDCA_AC2_CFG,
								      Ac2Cfg.word, FALSE);
					}
					...
					if (Ac0Cfg.field.AcTxop != 0x07) {
						...
						RTUSBWriteMACRegister(pAd, EDCA_AC0_CFG,
								      Ac0Cfg.word, FALSE);
					}
			...
			if (pAd->StaCfg.UapsdInfo.bAPSDCapable
				...
				if ((pAd->Mlme.OneSecPeriodicRound % 20) == 8) {
					RTMPSendNullFrame(pAd, pAd->CommonCfg.TxRate, TRUE,
							  pAd->CommonCfg.
							  bAPSDForcePowerSave ? PWR_SAVE : pAd->
							  StaCfg.Psm);
				}
			} else {
				...
				if ((pAd->Mlme.OneSecPeriodicRound % 10) == 8) {
					RTMPSendNullFrame(pAd,
							  ...
							  (pAd->CommonCfg.bWmmCapable & pAd->
							   CommonCfg.APEdcaParm.bValid),
							  pAd->CommonCfg.
							  bAPSDForcePowerSave ? PWR_SAVE : pAd->
							  StaCfg.Psm);
				}
...
/* should mark this two function, because link down alse will call this function */
		...
		if ((pAd->StaCfg.bAutoReconnect == TRUE)
			...
			if ((pAd->ScanTab.BssNr == 0)
				...
				if (RTMP_TIME_AFTER
				    (pAd->Mlme.Now32, pAd->StaCfg.LastScanTime + (10 * OS_HZ))
				    || (pAd->StaCfg.bNotFirstScan == FALSE)) {
					DBGPRINT(RT_DEBUG_TRACE,
						 ("%s:CNTL - ScanTab.BssNr==0, start a new ACTIVE scan SSID[%s]\n",
						  __func__, pAd->MlmeAux.AutoReconnectSsid));
					if (pAd->StaCfg.BssType == BSS_ADHOC)
						pAd->StaCfg.bNotFirstScan = TRUE;
					ScanParmFill(pAd, &ScanReq,
						     (PSTRING) pAd->MlmeAux.AutoReconnectSsid,
						     pAd->MlmeAux.AutoReconnectSsidLen, BSS_ANY,
						     SCAN_ACTIVE);
...
VOID MlmeAutoScan(RTMP_ADAPTER *pAd)
{
	...
	if (pAd->Mlme.CntlMachine.CurrState == CNTL_IDLE) {
		DBGPRINT(RT_DEBUG_ERROR, ("MMCHK - Driver auto scan\n"));
		MlmeEnqueue(pAd,
			    MLME_CNTL_STATE_MACHINE,
			    OID_802_11_BSSID_LIST_SCAN,
			    pAd->MlmeAux.AutoReconnectSsidLen, pAd->MlmeAux.AutoReconnectSsid, 0);
		RTMP_MLME_HANDLER(pAd);
	}
}
...
VOID MlmeAutoReconnectLastSSID(RTMP_ADAPTER *pAd)
{
	...
	if ((pWscControl->WscConfMode != WSC_DISABLE) &&
	    ...
	    (pWscControl->WscMode == WSC_PBC_MODE) && (pWscControl->WscPBCBssCount != 1))
		return;

	if ((pWscControl->WscConfMode != WSC_DISABLE) && (pWscControl->WscState >= WSC_STATE_START)) {
		ULONG ApIdx = 0;

		ApIdx = WscSearchWpsApBySSID(pAd,
					     pWscControl->WscSsid.Ssid,
					     pWscControl->WscSsid.SsidLength, pWscControl->WscMode);

		if ((ApIdx != BSS_NOT_FOUND) && (pAd->StaCfg.BssType == BSS_INFRA)) {
	...
	if (pAd->StaCfg.bAutoConnectByBssid) {
		...
		MlmeEnqueue(pAd,
			    MLME_CNTL_STATE_MACHINE,
			    OID_802_11_BSSID, MAC_ADDR_LEN, pAd->MlmeAux.Bssid, 0);
	...
	/* check CntlMachine.CurrState to avoid collision with NDIS SetOID request */
		 ...
		 (MlmeValidateSSID
		  (pAd->MlmeAux.AutoReconnectSsid, pAd->MlmeAux.AutoReconnectSsidLen) == TRUE)) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Driver auto reconnect to last OID_802_11_SSID setting - %s, len - %d\n",
			  pAd->MlmeAux.AutoReconnectSsid, pAd->MlmeAux.AutoReconnectSsidLen));
		MlmeEnqueue(pAd, MLME_CNTL_STATE_MACHINE, OID_802_11_SSID, sizeof(NDIS_802_11_SSID),
			    &OidSsid, 0);
		RTMP_MLME_HANDLER(pAd);
	}
}
...
VOID MlmeCheckForRoaming(RTMP_ADAPTER *pAd, ULONG Now32)
{
	...
	run = min(pAd->ScanTab.BssNr, (UCHAR) ARRAY_SIZE(pAd->ScanTab.BssEntry));
	for (i = 0; i < run; i++) {
...
BOOLEAN MlmeCheckForFastRoaming(RTMP_ADAPTER *pAd)
{
	...
	for (i = 0; i < run; i++) {
		...
		if (MAC_ADDR_EQUAL(pBss->Bssid, pAd->CommonCfg.Bssid))
			continue;	/* skip current AP */
		if (!SSID_EQUAL
		    (pBss->Ssid, pBss->SsidLen, pAd->CommonCfg.Ssid, pAd->CommonCfg.SsidLen))
			continue;	/* skip different SSID */
		max_rssi =
		    RTMPMaxRssi(pAd, pAd->StaCfg.RssiSample.LastRssi0,
				pAd->StaCfg.RssiSample.LastRssi1, pAd->StaCfg.RssiSample.LastRssi2);
...
VOID MlmeCheckPsmChange(RTMP_ADAPTER *pAd, ULONG Now32)
{
	...
	if (INFRA_ON(pAd) &&
	    (PowerMode != Ndis802_11PowerModeCAM) && (pAd->StaCfg.Psm == PWR_ACTIVE) &&
/*		(! RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS))*/
	    (pAd->Mlme.CntlMachine.CurrState == CNTL_IDLE)
#ifdef PCIE_PS_SUPPORT
...
/*===========================================================================*/
	do {
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			...
			if ((pAd->StaCfg.BssType == BSS_ADHOC) &&
			    WMODE_EQUAL(pAd->CommonCfg.PhyMode, WMODE_B) && (MaxDesire > RATE_11)) {
				MaxDesire = RATE_11;
			}
			break;
		}
	...
	if (bLinkUp && (pAd->OpMode == OPMODE_STA)) {
		...
		if (WMODE_CAP(pAd->CommonCfg.PhyMode, WMODE_B) && pAd->CommonCfg.Channel <= 14) {
...
#ifdef MCAST_RATE_SPECIFIC
		{
			...
			if (memcmp
			    (&pAd->CommonCfg.MCastPhyMode, &tPhyMode,
			     sizeof(HTTRANSMIT_SETTING)) == 0) {
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 (" %s(): (MaxDesire=%d, MaxSupport=%d, MaxTxRate=%d, MinRate=%d, Rate Switching =%d)\n",
		  ...
		  RateIdToMbps[pAd->CommonCfg.MaxTxRate], RateIdToMbps[pAd->CommonCfg.MinTxRate],
		  /*OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_TX_RATE_SWITCH_ENABLED) */
		  *auto_rate_cur_p));
	DBGPRINT(RT_DEBUG_TRACE,
		 (" %s(): (TxRate=%d, RtsRate=%d, BasicRateBitmap=0x%04lx)\n", __func__,
		  RateIdToMbps[pAd->CommonCfg.TxRate], RateIdToMbps[pAd->CommonCfg.RtsRate],
		  BasicRateBitmap));
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s(): (MlmeTransmit=0x%x, MinHTPhyMode=%x, MaxHTPhyMode=0x%x, HTPhyMode=0x%x)\n",
		  __func__, pAd->CommonCfg.MlmeTransmit.word,
...
VOID MlmeUpdateHtTxRates(RTMP_ADAPTER *pAd, UCHAR apidx)
{
	...
	if (wdev) {
		...
		DBGPRINT(RT_DEBUG_ERROR, ("%s: wdev is NULL!\n", __func__));
		return;
	}
#ifdef CONFIG_STA_SUPPORT
	if ((ADHOC_ON(pAd) || INFRA_ON(pAd)) && (pAd->OpMode == OPMODE_STA)
		...
		BasicMCS =
		    pAd->MlmeAux.AddHtInfo.MCSSet[0] + (pAd->MlmeAux.AddHtInfo.MCSSet[1] << 8) +
		    (StbcMcs << 16);
...
#endif /* CONFIG_STA_SUPPORT */
	{
		...
		BasicMCS =
		    pAd->CommonCfg.AddHTInfo.MCSSet[0] + (pAd->CommonCfg.AddHTInfo.MCSSet[1] << 8) +
		    (StbcMcs << 16);
	...
	if (pMaxHtPhy->field.BW == BW_20)
		pMaxHtPhy->field.ShortGI =
		    (pAd->CommonCfg.DesiredHtPhy.ShortGIfor20 & pRtHtCap->ShortGIfor20);
	else
		pMaxHtPhy->field.ShortGI =
		    (pAd->CommonCfg.DesiredHtPhy.ShortGIfor40 & pRtHtCap->ShortGIfor40);
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 (" %s():<---.AMsduSize = %d\n", __func__, pAd->CommonCfg.DesiredHtPhy.AmsduSize));
	DBGPRINT(RT_DEBUG_TRACE,
...
ULONG BssSsidTableSearchBySSID(BSS_TABLE *Tab, UCHAR *pSsid, UCHAR SsidLen)
{
	UCHAR i;
	UCHAR run;
	run = min(Tab->BssNr, (UCHAR) MAX_LEN_OF_BSS_TABLE);
	for (i = 0; i < run; i++) {
...
VOID BssTableDeleteEntry(BSS_TABLE *Tab, UCHAR *pBssid, UCHAR Channel)
{
	...
	run = min(Tab->BssNr, (UCHAR) ARRAY_SIZE(Tab->BssEntry));
	for (i = 0; i < run; i++) {
		if ((Tab->BssEntry[i].Channel == Channel) &&
		    (MAC_ADDR_EQUAL(Tab->BssEntry[i].Bssid, pBssid))) {
			UCHAR *pOldAddr = NULL;
			...
			for (j = i; j < Tab->BssNr - 1; j++) {
				...
				NdisMoveMemory(&(Tab->BssEntry[j]), &(Tab->BssEntry[j + 1]),
					       sizeof(BSS_ENTRY));
				if (pOldAddr) {
					RTMPZeroMemory(pOldAddr, MAX_VIE_LEN);
 ...
 *	\post
 */
static VOID BssEntrySet(IN RTMP_ADAPTER *pAd,
			OUT BSS_ENTRY *pBss,
...
#ifdef RT_CFG80211_SUPPORT
				/* fix vht channel been set to zero when connected with 11ac AP and doing scan */
				if (INFRA_ON(pAd) && CFG80211DRV_OpsScanRunning(pAd)) {
					/* DBGPRINT(RT_DEBUG_TRACE, ("%s:Infra-ON and doing scan, donot update vht_cent_ch\n", __FUNCTION__)); */
				} else
#endif /* RT_CFG80211_SUPPORT */
				{
					cent_ch =
					    vht_cent_ch_freq(pAd, ie_list->AddHtInfo.ControlChan);
					DBGPRINT(RT_DEBUG_TRACE,
						 ("%s():VHT cent_ch=%d, vht_op_info->center_freq_1=%d, Bss->CentCh=%d, change from CentralChannel to cent_ch!\n",
						  __func__, cent_ch,
...
#define WPS_AP		0x01
...
#ifdef WSC_INCLUDED
					pBss->WpsAP |= WPS_AP;
					WscCheckWpsIeFromWpsAP(pAd, pEid,
			...
			case IE_WAPI:
				if (NdisEqualMemory(pEid->Octet + 4, WAPI_OUI, 3)) {
					UCHAR idx;
					snprintf((PSTRING) pBss->WapiIE.IE, sizeof(pBss->WapiIE.IE),
						 "wapi_ie=%02x%02x", pEid->Eid, pEid->Len);
					for (idx = 0; idx < pEid->Len; idx++) {
						snprintf((PSTRING) pBss->WapiIE.IE,
							 sizeof(pBss->WapiIE.IE), "%s%02x",
							 pBss->WapiIE.IE,
							 (unsigned char)pEid->Octet[idx]);
					}
		       ...
		       IN USHORT LengthVIE, IN PNDIS_802_11_VARIABLE_IEs pVIE)
{
	...
	if (Idx == BSS_NOT_FOUND) {
		if (Tab->BssNr >= MAX_LEN_OF_BSS_TABLE) {
			...
			for (i = 0; i < pAd->ApCfg.ApCliNum; i++) {
				...
				if (MAC_ADDR_EQUAL(pApCliEntry->MlmeAux.Bssid, ie_list->Bssid)
				    || SSID_EQUAL(pApCliEntry->MlmeAux.Ssid,
						  pApCliEntry->MlmeAux.SsidLen, ie_list->Ssid,
						  ie_list->SsidLen)) {
					bInsert = TRUE;
					break;
				}
			...
			if (!OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED) ||
				...
				if (MAC_ADDR_EQUAL(pAd->MlmeAux.Bssid, ie_list->Bssid) ||
				    SSID_EQUAL(pAd->MlmeAux.Ssid, pAd->MlmeAux.SsidLen,
				    ...
				    || SSID_EQUAL(pAd->cfg80211_ctrl.Cfg_pending_Ssid,
					...
					BssEntrySet(pAd, &Tab->BssEntry[Idx], ie_list, Rssi,
						    pMeshScanInfo, LengthVIE, pVIE);
					Tab->BssOverlapNr += 1;
					Tab->BssOverlapNr =
					    Tab->BssOverlapNr % MAX_LEN_OF_BSS_TABLE;
#ifdef RT_CFG80211_SUPPORT
					pAd->cfg80211_ctrl.Cfg_pending_SsidLen = 0;
					NdisZeroMemory(pAd->cfg80211_ctrl.Cfg_pending_Ssid,
						       MAX_LEN_OF_SSID + 1);
#endif /* RT_CFG80211_SUPPORT */
				}
				return Idx;
		...
		Idx = Tab->BssNr;
		BssEntrySet(pAd, &Tab->BssEntry[Idx], ie_list, Rssi, pMeshScanInfo, LengthVIE,
			    pVIE);
		Tab->BssNr++;
	} else {
		ASSERT(Idx < ARRAY_SIZE(Tab->BssEntry));
		if (Idx >= ARRAY_SIZE(Tab->BssEntry))
			return BSS_NOT_FOUND;
		      ...
		      OUT BSS_TABLE *OutTab, IN CHAR Ssid[], IN UCHAR SsidLen)
{
	...
	if ((SsidLen == 0) && (pAd->StaCfg.bAutoConnectIfNoSSID == FALSE))
		return;
	...
	for (i = 0; i < run; i++) {
		...
		if (((pAd->CommonCfg.bIEEE80211H == 1) &&
		     (pAd->MlmeAux.Channel > 14) && RadarChannelCheck(pAd, pInBss->Channel))
#ifdef CARRIER_DETECTION_SUPPORT	/* Roger sync Carrier             */
		...
		if (OutTab->BssNr >= ARRAY_SIZE(OutTab->BssEntry))
			break;
		...
		if ((pInBss->BssType == pAd->StaCfg.BssType) &&
			...
			if (((wdev->AuthMode == Ndis802_11AuthModeWPA2)
				...
				NdisMoveMemory(&RsnCap, &pInBss->WPA2.RsnCapability,
					       sizeof(RSN_CAPABILITIES));
				RsnCap.word = cpu2le16(RsnCap.word);
				if ((pAd->StaCfg.PmfCfg.MFPR) && (RsnCap.field.MFPC == FALSE))
					continue;

				if (((pAd->StaCfg.PmfCfg.MFPC == FALSE)
				     && (RsnCap.field.MFPC == FALSE))
				    || (pAd->StaCfg.PmfCfg.MFPC && RsnCap.field.MFPC
					&& (pAd->StaCfg.PmfCfg.MFPR == FALSE)
					&& (RsnCap.field.MFPR == FALSE))) {
					if ((pAd->StaCfg.PmfCfg.PMFSHA256)
					    && (pInBss->IsSupportSHA256KeyDerivation == FALSE))
						continue;
				}
			...
			/* 2.4G/5G N only mode */
			if ((pInBss->HtCapabilityLen == 0) && WMODE_HT_ONLY(pAd->CommonCfg.PhyMode)) {
				DBGPRINT(RT_DEBUG_TRACE,
					 ("STA is in N-only Mode, this AP don't have Ht capability in Beacon.\n"));
				continue;
			}
	...
	for (i = 0; i < OutTab->BssNr - 1; i++) {
		for (j = i + 1; j < OutTab->BssNr; j++) {
			if (OutTab->BssEntry[j].Rssi > OutTab->BssEntry[i].Rssi ?
				...
				if (OutTab->BssEntry[j].Rssi != OutTab->BssEntry[i].Rssi) {
					NdisMoveMemory(pTmpBss, &OutTab->BssEntry[j],
						       sizeof(BSS_ENTRY));
					NdisMoveMemory(&OutTab->BssEntry[j], &OutTab->BssEntry[i],
						       sizeof(BSS_ENTRY));
					NdisMoveMemory(&OutTab->BssEntry[i], pTmpBss,
						       sizeof(BSS_ENTRY));
				}
			}
		}
...
VOID BssCipherParse(BSS_ENTRY *pBss)
{
	...
	if (pBss->VarIELen > sizeof(pBss->VarIEs)) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s: VarIELen > array size of VarIEs\n", __func__));
		return;
	}
	...
	while (Length > 0) {
		...
		case IE_RSN:
			...
			if (!RTMPEqualMemory(pCipher->Oui, RSN_OUI, sizeof(RSN_OUI)))
				break;
...
VOID MgtMacHeaderInitExt(IN RTMP_ADAPTER *pAd,
			 ...
			 IN UCHAR SubType,
			 IN UCHAR ToDs, IN UCHAR *pAddr1, IN UCHAR *pAddr2, IN UCHAR *pAddr3)
{
	NdisZeroMemory(pHdr80211, sizeof(HEADER_802_11));
 ...
 *	\post
 */
BOOLEAN MlmeEnqueueForRecv(IN RTMP_ADAPTER *pAd,
			   IN ULONG Wcid,
			   ...
			   IN UCHAR Rssi1,
			   IN UCHAR Rssi2,
			   IN ULONG MsgLen, IN VOID *Msg, IN UCHAR Signal, IN UCHAR OpMode)
{
...
#endif /* P2P_SUPPORT || RT_CFG80211_P2P_SUPPORT */
	{
		...
		do {
			...
			for (i = 0; i < pAd->ApCfg.ApCliNum; i++) {
				if (MAC_ADDR_EQUAL
				    (pAd->ApCfg.ApCliTab[i].MlmeAux.Bssid, pFrame->Hdr.Addr2)) {
					bToApCli = TRUE;
					break;
				}
			}
	...
	if (pAd->ApCfg.bMACRepeaterEn) {
		for (CliIdx = 0; CliIdx < MAX_EXT_MAC_ADDR_SIZE; CliIdx++) {
			if (MAC_ADDR_EQUAL
			    (pAd->ApCfg.ApCliTab[ApCliIdx].RepeaterCli[CliIdx].CurrentAddress,
			     pFrame->Hdr.Addr1)) {
				Queue->Entry[Tail].Priv =
...
VOID StateMachineSetAction(IN STATE_MACHINE * S,
			   IN ULONG St, IN ULONG Msg, IN STATE_MACHINE_FUNC Func)
{
	ULONG MsgIdx;
...
BOOLEAN RTMPCheckHt(IN RTMP_ADAPTER *pAd,
		    IN UCHAR Wcid, IN HT_CAPABILITY_IE *pHtCap, IN ADD_HT_INFO_IE *pAddHtInfo)
{
	MAC_TABLE_ENTRY *sta;
	...
	pAd->MlmeAux.HtCapability.HtCapInfo.ChannelWidth =
	    pAddHtInfo->AddHtInfo.RecomWidth & pAd->CommonCfg.DesiredHtPhy.ChannelWidth;
	...
	if (pAd->MlmeAux.HtCapability.HtCapInfo.ChannelWidth == BW_40) {
		if (RTMPCheckChannel(pAd, pAd->MlmeAux.CentralChannel, pAd->MlmeAux.Channel) ==
		    FALSE) {
	...
	pAd->MlmeAux.HtCapability.HtCapInfo.GF =
	    pHtCap->HtCapInfo.GF & pAd->CommonCfg.DesiredHtPhy.GF;

	/* Send Assoc Req with my HT capability. */
	pAd->MlmeAux.HtCapability.HtCapInfo.AMsduSize = pAd->CommonCfg.DesiredHtPhy.AmsduSize;
	pAd->MlmeAux.HtCapability.HtCapInfo.MimoPs = pAd->CommonCfg.DesiredHtPhy.MimoPs;
	pAd->MlmeAux.HtCapability.HtCapInfo.ShortGIfor20 =
	    (pAd->CommonCfg.DesiredHtPhy.ShortGIfor20) & (pHtCap->HtCapInfo.ShortGIfor20);
	...
	pAd->MlmeAux.HtCapability.HtCapInfo.TxSTBC =
	    (pAd->CommonCfg.DesiredHtPhy.TxSTBC) & (pHtCap->HtCapInfo.RxSTBC);
	pAd->MlmeAux.HtCapability.HtCapInfo.RxSTBC =
	    (pAd->CommonCfg.DesiredHtPhy.RxSTBC) & (pHtCap->HtCapInfo.TxSTBC);
...
/*
	...
	========================================================================
*/
BOOLEAN RTMPCheckVht(IN PRTMP_ADAPTER pAd,
		     IN UCHAR Wcid, IN VHT_CAP_IE * vht_cap, IN VHT_OP_IE *vht_op)
{
	...
	if (SIGMA_ON(pAd)) {
		DBGPRINT(RT_DEBUG_OFF, ("[%s] Allow VHT TXBF for %dx%d AP\n", __func__,
			vht_cap_info->num_snd_dimension, vht_cap_info->num_snd_dimension));
	} else {
...
VOID AsicEvaluateRxAnt(RTMP_ADAPTER *pAd)
{
	...
	if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_RESET_IN_PROGRESS |
			   ...
			   fRTMP_ADAPTER_RADIO_OFF |
			   fRTMP_ADAPTER_NIC_NOT_EXIST | fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS)
	    || OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_DOZE)
	    )
		return;
...
/*
    ...
    ========================================================================
*/
BOOLEAN RTMPCheckEntryEnableAutoRateSwitch(IN RTMP_ADAPTER *pAd, IN MAC_TABLE_ENTRY *pEntry)
{
	BOOLEAN result = TRUE;

	if (!pEntry) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s(): entry(%p) is NULL!\n", __func__, pEntry));
		return FALSE;
	}
	if (!pEntry->wdev) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s(): wdev(%p) is NULL!\n", __func__, pEntry->wdev));
		return FALSE;
	}
#ifdef CONFIG_AP_SUPPORT
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
...
UCHAR RTMPStaFixedTxMode(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		...
		if (IS_ENTRY_CLIENT(pEntry))
			tx_mode =
			    (UCHAR) pAd->ApCfg.MBSSID[pEntry->apidx].wdev.DesiredTransmitSetting.
			    field.FixedTxMode;
#ifdef WDS_SUPPORT
		else if (IS_ENTRY_WDS(pEntry))
			tx_mode =
			    (UCHAR) pAd->WdsTab.WdsEntry[pEntry->wdev_idx].wdev.
			    DesiredTransmitSetting.field.FixedTxMode;
#endif /* WDS_SUPPORT */
#ifdef APCLI_SUPPORT
		else if (IS_ENTRY_APCLI(pEntry))
			tx_mode =
			    (UCHAR) pAd->ApCfg.ApCliTab[pEntry->wdev_idx].wdev.
			    DesiredTransmitSetting.field.FixedTxMode;
#endif /* APCLI_SUPPORT */
	}
#endif /* CONFIG_AP_SUPPORT */
...
VOID RTMPUpdateLegacyTxSetting(UCHAR fixed_tx_mode, MAC_TABLE_ENTRY *pEntry)
{
	...
	if ((fixed_tx_mode != FIXED_TXMODE_CCK) && (fixed_tx_mode != FIXED_TXMODE_OFDM)
#ifdef DOT11_VHT_AC
