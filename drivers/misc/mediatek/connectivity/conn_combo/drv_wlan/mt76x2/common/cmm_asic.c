		...
		case 1:	/* OFDM */
			{
				...
				case 0:
					LgCfg0.field.OFDMMCS0FBK =
					    (pNextTxRate->Mode ==
					     MODE_OFDM) ? (pNextTxRate->CurrMCS +
							   8) : pNextTxRate->CurrMCS;
					break;
				case 1:
		...
		case 3:	/* HT-GF */
			{
				if ((pNextTxRate->Mode >= MODE_HTMIX)
				    && (pCurrTxRate->CurrMCS != pNextTxRate->CurrMCS)) {
...
#ifdef AGS_SUPPORT
						if ((bUseAGS == TRUE) &&
						    (pCurrTxRate->CurrMCS >= 16)
						    && (pCurrTxRate->CurrMCS <= 23)) {
							switch (pCurrTxRate->CurrMCS) {
							case 16:
								Ht3SSCfg0.field.HTMCS16FBK =
								    pNextTxRate->CurrMCS;
								break;
							case 17:
								Ht3SSCfg0.field.HTMCS17FBK =
								    pNextTxRate->CurrMCS;
								break;
							case 18:
								Ht3SSCfg0.field.HTMCS18FBK =
								    pNextTxRate->CurrMCS;
								break;
							case 19:
								Ht3SSCfg0.field.HTMCS19FBK =
								    pNextTxRate->CurrMCS;
								break;
							case 20:
								Ht3SSCfg1.field.HTMCS20FBK =
								    pNextTxRate->CurrMCS;
								break;
							case 21:
								Ht3SSCfg1.field.HTMCS21FBK =
								    pNextTxRate->CurrMCS;
								break;
							case 22:
								Ht3SSCfg1.field.HTMCS22FBK =
								    pNextTxRate->CurrMCS;
								break;
							case 23:
								Ht3SSCfg1.field.HTMCS23FBK =
								    pNextTxRate->CurrMCS;
								break;
							}
					} else
...
#if defined(DOT11_VHT_AC) && defined(CERTIFICATION_SIGMA_SUPPORT)
	if (SIGMA_ON(pAd)) {
		/* VHT-5.2.35: Disable RTS/CTS for VHT 80 */
		if (INFRA_ON(pAd) && (pAd->CommonCfg.BBPCurrentBW == BW_80)) {
			RTMP_IO_READ32(pAd, TX_PROT_CFG8, &vht_port_cfg.word);
			vht_port_cfg.field.ProtectCtrl = 0;
			vht_port_cfg.field.ProtectNav = ASIC_SHORTNAV;
			vht_port_cfg.field.ProtectRate = protect_rate;
			RTMP_IO_WRITE32(pAd, TX_PROT_CFG8, vht_port_cfg.word);
			DBGPRINT(RT_DEBUG_OFF, ("%s():VHT-5.2.35 CFG8 0x%x\n", __func__, vht_port_cfg.word));
		}
	}
#endif /* DOT11_VHT_AC */
...
/*
	...
	==========================================================================
 */
VOID AsicSleepThenAutoWakeup(IN PRTMP_ADAPTER pAd, IN USHORT TbttNumToNextWakeUp)
{
...
#if defined(P2P_SUPPORT) || defined(RT_CFG80211_P2P_CONCURRENT_DEVICE) || defined(MESH_SUPPORT)
	{
	...
	if ((pEdcaParm == NULL) || (pEdcaParm->bValid == FALSE)) {
		...
		for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
			if (IS_ENTRY_CLIENT(&pAd->MacTab.Content[i])
				...
				CLIENT_STATUS_CLEAR_FLAG(&pAd->MacTab.Content[i],
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			...
			if (pAd->CommonCfg.bWiFiTest && pEdcaParm->Aifsn[QID_AC_VI] == 10)
		...
		if (pAd->CommonCfg.bWiFiTest) {
			if (Ac3Cfg.field.AcTxop == 102) {
				Ac0Cfg.field.AcTxop =
				    pEdcaParm->Txop[QID_AC_BE] ? pEdcaParm->Txop[QID_AC_BE] : 10;
		...
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
			...
			if (INFRA_ON(pAd))
				CLIENT_STATUS_SET_FLAG(&pAd->MacTab.Content[BSSID_WCID],
						       fCLIENT_STATUS_WMM_CAPABLE);
		}
#endif /* CONFIG_STA_SUPPORT */
		...
		if (!ADHOC_ON(pAd)) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("     AC_BE      %2d     %2d     %2d      %4d     %d\n",
				  pEdcaParm->Aifsn[0], pEdcaParm->Cwmin[0], pEdcaParm->Cwmax[0],
				  pEdcaParm->Txop[0] << 5, pEdcaParm->bACM[0]));
			DBGPRINT(RT_DEBUG_TRACE,
				 ("     AC_BK      %2d     %2d     %2d      %4d     %d\n",
				  pEdcaParm->Aifsn[1], pEdcaParm->Cwmin[1], pEdcaParm->Cwmax[1],
				  pEdcaParm->Txop[1] << 5, pEdcaParm->bACM[1]));
			DBGPRINT(RT_DEBUG_TRACE,
				 ("     AC_VI      %2d     %2d     %2d      %4d     %d\n",
				  pEdcaParm->Aifsn[2], pEdcaParm->Cwmin[2], pEdcaParm->Cwmax[2],
				  pEdcaParm->Txop[2] << 5, pEdcaParm->bACM[2]));
			DBGPRINT(RT_DEBUG_TRACE,
				 ("     AC_VO      %2d     %2d     %2d      %4d     %d\n",
				  pEdcaParm->Aifsn[3], pEdcaParm->Cwmin[3], pEdcaParm->Cwmax[3],
				  pEdcaParm->Txop[3] << 5, pEdcaParm->bACM[3]));
		}
	}
...
VOID RTMPGetTxTscFromAsic(RTMP_ADAPTER *pAd, UCHAR apidx, UCHAR *pTxTsc)
{
	...
	if (pAd->ApCfg.MBSSID[apidx].wdev.GroupKeyWepStatus == Ndis802_11AESEnable) {	/* AES */
		*pTxTsc = IvEiv[0];
		*(pTxTsc + 1) = IvEiv[1];
		*(pTxTsc + 2) = IvEiv[4];
		*(pTxTsc + 3) = IvEiv[5];
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("RTMPGetTxTscFromAsic : WCID(%d) TxTsc 0x%02x-0x%02x-0x%02x-0x%02x-0x%02x-0x%02x\n",
		  Wcid, *pTxTsc, *(pTxTsc + 1), *(pTxTsc + 2), *(pTxTsc + 3), *(pTxTsc + 4),
		  *(pTxTsc + 5)));
	...
	if (pRxMic) {
		DBGPRINT_RAW(RT_DEBUG_TRACE,
			     ("	Rx MIC Key = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
			      pRxMic[0], pRxMic[1], pRxMic[2], pRxMic[3], pRxMic[4],
			      pRxMic[5], pRxMic[6], pRxMic[7]));
	}
	if (pTxMic) {
		DBGPRINT_RAW(RT_DEBUG_TRACE,
			     ("	Tx MIC Key = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n",
			      pTxMic[0], pTxMic[1], pTxMic[2], pTxMic[3], pTxMic[4],
			      pTxMic[5], pTxMic[6], pTxMic[7]));
	}
/*============================================================================================*/
...
/*	IRQL = DISPATCH_LEVEL*/
VOID AsicRemoveSharedKeyEntry(IN PRTMP_ADAPTER pAd, IN UCHAR BssIndex, IN UCHAR KeyIdx)
{
	/*ULONG SecCsr0; */
	SHAREDKEY_MODE_STRUC csr1;
	...
	/* If wpa_supplicant has enabled 11W PMF function, it may set key with idx 0~5 */
	ASSERT(BssIndex < 6);
	ASSERT(KeyIdx < 6);
...
VOID AsicUpdateWCIDIVEIV(IN PRTMP_ADAPTER pAd, IN USHORT WCID, IN ULONG uIV, IN ULONG uEIV)
{
	ULONG offset;
	UINT32 iveiv_tb_base = 0, iveiv_tb_size = 0;
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s: wcid(%d) 0x%08lx, 0x%08lx\n", __func__, WCID, uIV, uEIV));
}

VOID AsicUpdateRxWCIDTable(IN PRTMP_ADAPTER pAd, IN USHORT WCID, IN PUCHAR pAddr)
{
	ULONG offset;
	ULONG Addr;
...
/*
	...
	========================================================================
*/
VOID AsicUpdateWcidAttributeEntry(IN PRTMP_ADAPTER pAd,
				  IN UCHAR BssIdx,
				  IN UCHAR KeyIdx,
				  IN UCHAR CipherAlg, IN UINT8 Wcid, IN UINT8 KeyTabFlag)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s:WCID #%d, KeyIdx #%d, WCIDAttri=0x%x, Alg=%s\n",
				  __func__, Wcid, KeyIdx, WCIDAttri.word, CipherName[CipherAlg]));
}
...
/*
	...
	========================================================================
*/
VOID AsicAddPairwiseKeyEntry(IN PRTMP_ADAPTER pAd, IN UCHAR WCID, IN PCIPHER_KEY pCipherKey)
{
	INT i;
	ULONG offset;
	...
	if (pRxMic) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("	Rx MIC Key = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n", pRxMic[0],
			  pRxMic[1], pRxMic[2], pRxMic[3], pRxMic[4], pRxMic[5], pRxMic[6],
			  pRxMic[7]));
	}
	if (pTxMic) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("	Tx MIC Key = %02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x\n", pTxMic[0],
			  pTxMic[1], pTxMic[2], pTxMic[3], pTxMic[4], pTxMic[5], pTxMic[6],
			  pTxMic[7]));
	}
}
...
/*
	...
	========================================================================
*/
VOID AsicRemovePairwiseKeyEntry(IN PRTMP_ADAPTER pAd, IN UCHAR Wcid)
{
	/* Set the specific WCID attribute entry as OPEN-NONE */
	AsicUpdateWcidAttributeEntry(pAd, BSS0, 0, CIPHER_NONE, Wcid, PAIRWISEKEYTABLE);
}
...
BOOLEAN AsicSendCommandToMcuBBP(IN PRTMP_ADAPTER pAd,
				IN UCHAR Command,
				IN UCHAR Token,
...
#ifdef WAPI_SUPPORT
VOID AsicUpdateWAPIPN(IN PRTMP_ADAPTER pAd, IN USHORT WCID, IN ULONG pn_low, IN ULONG pn_high)
{
		       ...
		       IN PUCHAR pMacAddr, IN INT chainIdx, IN BOOLEAN bEnabled)
{
	...
	RTMP_IO_WRITE32(pAd, regAddr,
			(UINT32) (pMacAddr[0]) |
			(UINT32) (pMacAddr[1] << 8) |
			(UINT32) (pMacAddr[2] << 16) | (UINT32) (pMacAddr[3] << 24));

	RTMP_IO_READ32(pAd, regAddr + 4, &regVal);
	regVal &= (~0x000f0000);
	RTMP_IO_WRITE32(pAd, regAddr + 4,
			(regVal | streamWord) |
			(UINT32) (pMacAddr[4]) | (UINT32) (pMacAddr[5] << 8));
...
VOID AsicMeasureFalseCCA(IN PRTMP_ADAPTER pAd)
{
	if (pAd->chipOps.AsicMeasureFalseCCA)
		pAd->chipOps.AsicMeasureFalseCCA(pAd);
}

VOID AsicMitigateMicrowave(IN PRTMP_ADAPTER pAd)
{
...
/*
	...
	==========================================================================
 */
VOID AsicSetApCliBssid(IN PRTMP_ADAPTER pAd, IN PUCHAR pBssid, IN UCHAR index)
{
	UINT32 Addr4 = 0;
...
VOID RT28xxAndesWOWEnable(IN PRTMP_ADAPTER pAd)
{
	...
	if (pAd->StaCfg.AuthMode >= Ndis802_11AuthModeWPAPSK) {
		...
		if (Ret != NDIS_STATUS_SUCCESS) {
			printk("\x1b[31m%s: send WOW config command failed(%d/%d)!!\x1b[m\n",
			       __func__, CmdUnit.u.ANDES.Type, sec_cfg.Config_Type);
			return;
		}
...
VOID RT28xxAndesWOWDisable(IN PRTMP_ADAPTER pAd)
{
	...
	printk("\x1b[31m%s: ...\x1b[m", __func__);
...
VOID thermal_protection(IN RTMP_ADAPTER *pAd)
{
	...
	DBGPRINT(RT_DEBUG_INFO, ("%s - current temp=%d, temp diff=%d\n",
				 __func__, current_temp, temp_diff));
...
VOID asic_set_drop_mask(PRTMP_ADAPTER ad, USHORT wcid, BOOLEAN enable)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s(%u):, wcid = %u, reg_id = 0x%08x, mac_reg = 0x%08x, group_index = %u, drop_mask = 0x%08x\n",
		  __func__, enable, wcid, reg_id, mac_reg, group_index, drop_mask));
}
...
VOID asic_change_tx_retry(IN PRTMP_ADAPTER pAd, IN USHORT num)
{
...
VOID pkt_aggr_num_change(IN PRTMP_ADAPTER pAd, IN USHORT num)
{
	if (IS_RT6352(pAd)) {
		if (num < 5) {
			/* use default */
...
VOID asic_tune_be_wmm(IN PRTMP_ADAPTER pAd, IN USHORT num)
{
