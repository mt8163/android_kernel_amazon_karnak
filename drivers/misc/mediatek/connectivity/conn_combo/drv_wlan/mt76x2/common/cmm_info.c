...
INT Set_CountryRegion_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) {
		...
		CountryRegion_SetDuringIfDown = simple_strtol(arg, 0, 10);
...
INT Set_CountryRegionABand_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) {
		...
		CountryRegionABand_SetDuringIfDown = simple_strtol(arg, 0, 10);
...
{
	pAd->cfg80211_ctrl.bSigmaEnabledFlag = (UCHAR) simple_strtol(arg, 0, 10);
	DBGPRINT(RT_DEBUG_OFF,
				...
				*(pEnd) = '\0';
				...
				if (count != 5) {
					...
					inChDesp.FirstChannel =
					    (UCHAR) simple_strtol(tempBuff[0], 0, 10);
					inChDesp.NumOfCh =
					    (UCHAR) simple_strtol(tempBuff[1], 0, 10);
					inChDesp.MaxTxPwr =
					    (UCHAR) simple_strtol(tempBuff[2], 0, 10);
					inChDesp.Geography =
...
INT Set_MCC_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	MCC_Enable = simple_strtol(arg, 0, 10);
	...
	for (i = 0; i < DBQ_LENGTH; i++) {
		...
		case 0x78:	/* RXWI - 2 longs, MSB to LSB */
			...
			if (decode && oldTail->type == 0x70) {
				...
				DBGPRINT(RT_DEBUG_OFF, ("%c s=%03X %02X %s-", (oldTail->data[0] & 0x10) ? 'A' : '_',	/* AMPDU */
				...
				DBGPRINT(RT_DEBUG_OFF, (" s=%03X %02X %02X%01X-", (oldTail->data[3] * 256 + oldTail->data[2]) & 0xFFF,	/* Size */
							oldTail->data[0],	/* WCID */
		...
		case 0x73:	/* TX STAT FIFO */
			...
			DBGPRINT(RT_DEBUG_OFF, ("TxFI %02X%02X%02X%02X=%c%c%c%c%c M%02d/%02d%c%c", oldTail->data[3], oldTail->data[2], oldTail->data[1], oldTail->data[0], (phyRate & 0x0100) ? 'S' : 'L',	/* Guard Int:    S or L */
						(phyRate & 0x0080) ? '4' : '2',	/* BW:                   4 or 2 */
						(phyRate & 0x0200) ? 'S' : 's',	/* STBC:         S or s */
...
INT Show_MacTable_Proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	printk("\n%-19s%-4s%-4s%-4s%-4s%-8s", "MAC", "AID", "BSS", "PSM", "WMM", "MIMOPS");
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if ((IS_ENTRY_CLIENT(pEntry) || (IS_ENTRY_APCLI(pEntry)
			...
			if (pAd->CommonCfg.RxStream == 3)
				printk("%-7d%-7d%-7d", pEntry->RssiSample.AvgRssi0,
				       pEntry->RssiSample.AvgRssi1, pEntry->RssiSample.AvgRssi2);
			else if (pAd->CommonCfg.RxStream == 2)
				printk("%-7d%-7d", pEntry->RssiSample.AvgRssi0,
			...
			if (pEntry->HTPhyMode.field.MODE == MODE_VHT)
				printk("%dS-M%-2d", ((pEntry->HTPhyMode.field.MCS >> 4) + 1),
...
/* +++Add by shiang for debug */
			...
			if (pEntry->MaxHTPhyMode.field.MODE == MODE_VHT)
				printk("%dS-M%d", ((pEntry->MaxHTPhyMode.field.MCS >> 4) + 1),
...
#ifdef DOT11_VHT_AC
	if (pEntry->HTPhyMode.field.MODE == MODE_VHT)
		printk("%dS-M%d", ((pEntry->HTPhyMode.field.MCS >> 4) + 1),
		       (pEntry->HTPhyMode.field.MCS & 0xf));
	else
#endif /* DOT11_VHT_AC */
		printk("%-6d", pEntry->HTPhyMode.field.MCS);
	...
	for (McsIdx = 0; McsIdx < 16; McsIdx++) {
		...
		for (i = start; i != stop; i = (i + 1) % DIAGNOSE_TIME) {
			printk("%d(%d%%)  ", pDiag->diag_info[i].TxAMPDUCnt[McsIdx],
			       pDiag->diag_info[i].TxAMPDUCnt[McsIdx] ? (pDiag->diag_info[i].
									 TxAMPDUCnt[McsIdx] * 100 /
									 pDiag->diag_info[i].
									 TxAggCnt) : 0);
		}
		printk("\n");
