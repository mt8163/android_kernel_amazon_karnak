...
VOID dump_rxinfo(RTMP_ADAPTER *pAd, RXINFO_STRUCT *pRxInfo)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("RxInfo Fields:\n"));

	DBGPRINT(RT_DEBUG_TRACE, ("\tBA=%d\n", pRxInfo->BA));
	DBGPRINT(RT_DEBUG_TRACE, ("\tDATA=%d\n", pRxInfo->DATA));
	DBGPRINT(RT_DEBUG_TRACE, ("\tNULLDATA=%d\n", pRxInfo->NULLDATA));
	DBGPRINT(RT_DEBUG_TRACE, ("\tFRAG=%d\n", pRxInfo->FRAG));
	DBGPRINT(RT_DEBUG_TRACE, ("\tU2M=%d\n", pRxInfo->U2M));
	DBGPRINT(RT_DEBUG_TRACE, ("\tMcast=%d\n", pRxInfo->Mcast));
	DBGPRINT(RT_DEBUG_TRACE, ("\tBcast=%d\n", pRxInfo->Bcast));
	DBGPRINT(RT_DEBUG_TRACE, ("\tMyBss=%d\n", pRxInfo->MyBss));
	DBGPRINT(RT_DEBUG_TRACE, ("\tCrc=%d\n", pRxInfo->Crc));
	DBGPRINT(RT_DEBUG_TRACE, ("\tCipherErr=%d\n", pRxInfo->CipherErr));
	DBGPRINT(RT_DEBUG_TRACE, ("\tAMSDU=%d\n", pRxInfo->AMSDU));
	DBGPRINT(RT_DEBUG_TRACE, ("\tHTC=%d\n", pRxInfo->HTC));
	DBGPRINT(RT_DEBUG_TRACE, ("\tRSSI=%d\n", pRxInfo->RSSI));
	DBGPRINT(RT_DEBUG_TRACE, ("\tL2PAD=%d\n", pRxInfo->L2PAD));
	DBGPRINT(RT_DEBUG_TRACE, ("\tAMPDU=%d\n", pRxInfo->AMPDU));
	DBGPRINT(RT_DEBUG_TRACE, ("\tDecrypted=%d\n", pRxInfo->Decrypted));
	DBGPRINT(RT_DEBUG_TRACE, ("\tBssIdx3=%d\n", pRxInfo->BssIdx3));
	DBGPRINT(RT_DEBUG_TRACE, ("\twapi_kidx=%d\n", pRxInfo->wapi_kidx));
	DBGPRINT(RT_DEBUG_TRACE, ("\tpn_len=%d\n", pRxInfo->pn_len));
	DBGPRINT(RT_DEBUG_TRACE, ("\tsw_fc_type0=%d\n", pRxInfo->action_wanted));
	DBGPRINT(RT_DEBUG_TRACE, ("\tsw_fc_type1=%d\n", pRxInfo->sw_fc_type1));
	DBGPRINT(RT_DEBUG_TRACE, ("\tprobe_rsp=%d\n", pRxInfo->probe_rsp));
	DBGPRINT(RT_DEBUG_TRACE, ("\tbeacon=%d\n", pRxInfo->beacon));
	DBGPRINT(RT_DEBUG_TRACE, ("\tdisasso=%d\n", pRxInfo->disasso));
	DBGPRINT(RT_DEBUG_TRACE, ("\tdeauth=%d\n", pRxInfo->deauth));
	DBGPRINT(RT_DEBUG_TRACE, ("\taction_wanted=%d\n", pRxInfo->action_wanted));
	DBGPRINT(RT_DEBUG_TRACE, ("\trsv=%d\n", pRxInfo->rsv));
...
VOID dump_txinfo(RTMP_ADAPTER *pAd, TXINFO_STRUCT *pTxInfo)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("TxInfo Fields:\n"));
...
VOID dump_txwi(RTMP_ADAPTER *pAd, TXWI_STRUCT *pTxWI)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("TxWI Fields:\n"));
#ifdef RLT_MAC
	if (pAd->chipCap.hif_type == HIF_RLT)
...
VOID dump_rxwi(RTMP_ADAPTER *pAd, RXWI_STRUCT *pRxWI)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("RxWI Fields:\n"));
#ifdef RLT_MAC
	if (pAd->chipCap.hif_type == HIF_RLT)
...
#ifdef DBG_DIAGNOSE
#if defined(DBG_TXQ_DEPTH) || defined(DBG_TX_RING_DEPTH)
static VOID dump_txblk(TX_BLK *pTxBlk)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("\tTotalFrameNum=%u!\n", pTxBlk->TxPacketList.Number));
	...
	for (i = 0; i < frameNum; i++) {
		...
		if (pPacket) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("\t\t[%d]:ptr=0x%x, Len=%d!\n", i,
				  (UINT32) (ULONG) (GET_OS_PKT_DATAPTR(pPacket)),
...
#endif /* defined(DBG_TXQ_DEPTH) || defined(DBG_TX_RING_DEPTH) */
#endif /* DBG_DIAGNOSE */
...
VOID dump_rxblk(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("\t\tpRxPacket=0x%p, len=%d\n", pRxBlk->pRxPacket, pRxBlk->MPDUtotalByteCnt));
	DBGPRINT(RT_DEBUG_TRACE, ("\t\tpHeader=0x%p\n", pRxBlk->pHeader));
	hex_dump("Dump RxPacket in dump_rxblk", (UCHAR *) pRxBlk->pHeader,
		 pRxBlk->MPDUtotalByteCnt > 48 ? 48 : pRxBlk->MPDUtotalByteCnt);
		...
		if (pEntry && (pEntry->NoBADataCountDown == 0) && IS_HT_STA(pEntry)) {
			...
			if (IS_ENTRY_APCLI(pEntry)) {
				if (pEntry->apidx < MAX_APCLI_NUM) {
					if (pAd->ApCfg.ApCliTab[pEntry->apidx].MlmeAux.APRalinkIe !=
					    0)
						isRalink = TRUE;
				}
			}
			      ...
			      IN UCHAR QueIdx, IN UCHAR *pData, IN UINT Length)
{
	...
	do {
		...
		if ((FreeNum > 0)) {
			...
			Status =
			    RTMPAllocateNdisPacket(pAd, &pPacket, (PUCHAR) &rtmpHwHdr[0], hw_len,
						   pData, Length);
...
/*
	...
	========================================================================
*/
static void AP_QueuePsActionPacket(IN PRTMP_ADAPTER pAd,
				   IN MAC_TABLE_ENTRY *pMacEntry,
				   IN PNDIS_PACKET pPacket,
				   IN BOOLEAN FlgIsDeltsFrame,
				   IN BOOLEAN FlgIsLocked, IN UCHAR MgmtQid)
{
#ifdef UAPSD_SUPPORT
			     ...
			     IN BOOLEAN FlgDataQForce, IN BOOLEAN FlgIsLocked)
{
	...
	if ((pHeader_802_11->FC.Type == FC_TYPE_DATA) || (pHeader_802_11->FC.Type == FC_TYPE_MGMT)) {
	...
	if ((pEntry != NULL) &&
		...
		AP_QueuePsActionPacket(pAd, pEntry, pPacket, FlgIsDeltsFrame, FlgIsLocked, MgmtQid);
				     ...
				     IN UCHAR QueIdx, IN PNDIS_PACKET pPacket)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		if (WMODE_EQUAL(pAd->CommonCfg.PhyMode, WMODE_A | WMODE_B | WMODE_G)
		    || WMODE_EQUAL(pAd->CommonCfg.PhyMode,
				   WMODE_A | WMODE_B | WMODE_G | WMODE_AN | WMODE_GN)
#ifdef DOT11_VHT_AC
	...
	if (pAd->CommonCfg.MlmeTransmit.field.MODE == MODE_CCK &&
	    NdisEqualMemory(pAd->cfg80211_ctrl.P2PCurrentAddress, pHeader_802_11->Addr2,
			    MAC_ADDR_LEN)) {
		if (pAd->CommonCfg.MlmeTransmit.field.MODE == MODE_CCK) {
			pAd->CommonCfg.MlmeTransmit.field.MODE = MODE_OFDM;
			pAd->CommonCfg.MlmeTransmit.field.MCS = MCS_RATE_6;
		...
		if (pHeader_802_11->Addr1[0] & 0x01) {	/* MULTICAST, BROADCAST */
...
#endif /* SOFT_SOUNDING */
			{
				...
				if ((pHeader_802_11->FC.SubType == SUBTYPE_PROBE_RSP)
					...
					UINT32 apidx =
					    get_apidx_by_addr(pAd, pHeader_802_11->Addr2);

					if (!(apidx >= pAd->ApCfg.BssidNum) && (pAd->ApCfg.MBSSID[apidx].TxPwrAdj != -1)	/* &&
																   (pAd->CommonCfg.MlmeTransmit.field.MODE == MODE_CCK) &&
	...
	if (pAd->cfg80211_ctrl.IsNeedTxStatus) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s, send tx status WCID(%d), MgmtID(%d), Seq(%d), Timeout(%d)\n",
			  __func__, TXStatus->WCID, TXStatus->MgmtID, TXStatus->Seq,
			  TXStatus->Timeout));
		wcid = pAd->cfg80211_ctrl.TxStatusCtrl.WCID;
	}
#endif /* endif */
	...
	if (((pHeader_802_11->FC.Type == FC_TYPE_DATA)
		...
		RTMPWriteTxWI(pAd, pFirstTxWI, FALSE, FALSE, bInsertTimestamp, FALSE, bAckRequired,
			      FALSE, 0, wcid, (SrcBufLen - TXINFO_SIZE - TXWISize - TSO_SIZE), PID,
			      0, tx_rate, IFS_PIFS, transmit);
		...
		DBGPRINT(RT_DEBUG_OFF,
			 ("%s():Kick Sounding to %02x:%02x:%02x:%02x:%02x:%02x, dataRate(PhyMode:%s, BW:%sHz, %dSS, MCS%d)\n",
			  ...
			  get_phymode_str(transmit->field.MODE), get_bw_str(transmit->field.BW),
...
#endif /* SOFT_SOUNDING */
	{
		RTMPWriteTxWI(pAd, pFirstTxWI, FALSE, FALSE, bInsertTimestamp, FALSE, bAckRequired,
			      FALSE, 0, wcid, (SrcBufLen - TXINFO_SIZE - TXWISize - TSO_SIZE), PID,
			      0, tx_rate, IFS_BACKOFF, transmit);
	...
	if (Wcid == MCAST_WCID) {	/* Handle for RA is Broadcast/Multicast Address. */
...
#endif /* RT_CFG80211_P2P_SUPPORT */
		{
			...
			if (apidx < pAd->ApCfg.BssidNum)
				...
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s(%d): WARN! pTxBlk->wdev not assigned!\n",
					  __func__, __LINE__));
			}
		}
#endif /* CONFIG_AP_SUPPORT */
...
BOOLEAN RTMP_FillTxBlkInfo(RTMP_ADAPTER *pAd, TX_BLK *pTxBlk)
{
	...
	if (pTxBlk->wdev && pTxBlk->wdev->if_dev)
		pNetDev = pTxBlk->wdev->if_dev;
	...
	if (pTxBlk->Wcid == MCAST_WCID) {
...

		{
...
#endif /* P2P_SUPPORT || RT_CFG80211_P2P_SUPPORT */
			{
				...
				if (IS_ENTRY_APCLI(pMacEntry)) {
					...
					if ((pMacEntry->bReptCli) && (pAd->ApCfg.bMACRepeaterEn)) {
						...
						pAd->MatCfg.bMACRepeaterEn =
						    pAd->ApCfg.bMACRepeaterEn;
						if (pAd->ApCfg.MACRepeaterOuiMode != 1) {
							tmpIdx =
							    (64 +
							     (MAX_EXT_MAC_ADDR_SIZE *
							      pMacEntry->wdev_idx) +
							     pMacEntry->MatchReptCliIdx);
							apCliPkt =
							    (PNDIS_PACKET) MATEngineTxHandle(pAd,
											     pPacket,
											     tmpIdx,
											     pTxBlk->
											     OpMode);
...
#endif /* MAC_REPEATER_SUPPORT */
					{
						...
						apCliPkt =
						    (PNDIS_PACKET) MATEngineTxHandle(pAd, pPacket,
										     pMacEntry->
										     wdev_idx,
										     pTxBlk->
										     OpMode);
					...
					if (apCliPkt) {
						RELEASE_NDIS_PACKET(pAd, pPacket,
								    NDIS_STATUS_SUCCESS);
						pPacket = apCliPkt;
						RTMP_QueryPacketInfo(pPacket, &PacketInfo,
...
#endif /* MAT_SUPPORT */
					pTxBlk->pApCliEntry =
					    &pAd->ApCfg.ApCliTab[pMacEntry->wdev_idx];
...
#endif /* P2P_SUPPORT || RT_CFG80211_P2P_SUPPORT */
			{
				...
				if ((pAd->MeshTab.OpMode & MESH_POTAL) && MESH_ON(pAd)) {
					PNDIS_PACKET brigePkt2 = NULL;
					/* For MESH_STA_WIRELESS_BRIDGE ARP. */
					brigePkt2 =
					    (PNDIS_PACKET) MATEngineTxHandle(pAd, pPacket,
		...
		if (pTxBlk->TxFrameType == TX_LEGACY_FRAME) {
			if (((RTMP_GET_PACKET_LOWRATE(pPacket))
#ifdef UAPSD_SUPPORT
			     && (!(pMacEntry->bAPSDFlagSPStart))
#endif /* UAPSD_SUPPORT */
			    ) ||
			    ((pAd->OpMode == OPMODE_AP) &&
			     (pMacEntry->MaxHTPhyMode.field.MODE == MODE_CCK)
			     && (pMacEntry->MaxHTPhyMode.field.MCS == RATE_1))
			    ) {
				BOOLEAN IsGOPkt = FALSE;
				if (pNetDev && (RT_CFG80211_GET_IF_TYPE(pNetDev) == RT_CMD_80211_IFTYPE_P2P_GO))
					IsGOPkt = TRUE;
				...
				/* Temp solution, fixme! */
				/* If it is P2P EAPOL or other low rate data, we should use OFDM rate (6Mbps) */
				if (RTMP_GET_PACKET_LOWRATE(pPacket)
				    && ((pAd->ApCfg.ApCliTab[0].CtrlCurrState == APCLI_CTRL_CONNECTED)
				    || IsGOPkt)) {
					/* Follow Mgmt rate */
...
static BOOLEAN CanDoAggregateTransmit(IN RTMP_ADAPTER *pAd,
				      IN NDIS_PACKET *pPacket, IN TX_BLK *pTxBlk)
{
	...
	if ((INFRA_ON(pAd)) && (pAd->OpMode == OPMODE_STA))	/* must be unicast to AP */
		...
		/* CFG_TODO */
		if ((MAC_ADDR_EQUAL
		     (GET_OS_PKT_DATAPTR(pTxBlk->pPacket), GET_OS_PKT_DATAPTR(pPacket)))
...
VOID RTMPDeQueuePacket(IN RTMP_ADAPTER *pAd,
		       IN BOOLEAN bIntContext, IN UCHAR QIdx, IN INT Max_Tx_Packets)
{
	...
	if (QIdx == NUM_OF_TX_RING) {
		...
		sQIdx = QID_AC_VO;
#endif /* endif */
		eQIdx = QID_AC_BE;	/* 4 ACs, start from 0. */
	} else {
		ASSERT(QIdx < NUM_OF_TX_RING);
		if (QIdx > NUM_OF_TX_RING) {
			DBGPRINT(RT_DEBUG_ERROR, ("QIdx=%d > 5 is out of bound\n", QIdx));
			return;
		}
		sQIdx = eQIdx = QIdx;
	}
	...
	for (QueIdx = sQIdx; QueIdx >= eQIdx; QueIdx--) {
		...
		while (1) {
			...
			if (pAd->CommonCfg.ChannelSwitchMode == 1 &&
			    pAd->StaCfg.bChannelSwitchCountingDown) {
				pEntry = RemoveHeadQueue(pQueue);
				pPacket = QUEUE_ENTRY_TO_PACKET(pEntry);
				RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_SUCCESS);
				DEQUEUE_UNLOCK(&pAd->irq_lock, bIntContext, IrqFlags);
				DBGPRINT(RT_DEBUG_ERROR, ("Go into silent mode due to CSA IE!\n"));
				continue;
			}
#endif /* endif */
			pTxBlk = &TxBlk;
			...
			if (RTMP_GET_PACKET_OPMODE(pPacket)) {
				...
				if (RTMP_GET_PACKET_MGMT_PKT(pPacket) == 1) {
					...
					pEntry = RemoveHeadQueue(pQueue);
						...
						status =
						    MlmeHardTransmitMgmtRing(pAd, QueIdx, pPacket);

					if (status != NDIS_STATUS_SUCCESS) {
						DBGPRINT(RT_DEBUG_ERROR,
							 ("tx queued mgmt frame error!\n"));
						RELEASE_NDIS_PACKET(pAd, pPacket,
								    NDIS_STATUS_FAILURE);
					}
					...
					if (IS_ENTRY_WDS(pMacEntry)) {
						...
						if (pMacEntry->LockEntryTx
						    && RTMP_TIME_BEFORE(Now32,
									pMacEntry->
									TimeStamp_toTxRing +
									WDS_ENTRY_RETRY_INTERVAL)) {
							pEntry = RemoveHeadQueue(pQueue);
							RELEASE_NDIS_PACKET(pAd, pPacket,
									    NDIS_STATUS_FAILURE);
...
#define ENTRY_RETRY_INTERVAL	(100 * OS_HZ / 1000)
			...
			hasTxDesc =
			    RTMP_HAS_ENOUGH_FREE_DESC(pAd, pTxBlk, FreeNumber[QueIdx], pPacket);
			if (!hasTxDesc) {
			...
			if (pTxBlk->TxFrameType & (TX_RALINK_FRAME | TX_AMSDU_FRAME)) {
				...
				if (NEED_QUEUE_BACK_FOR_AGG
				    (pAd, QueIdx, FreeNumber[QueIdx], pTxBlk->TxFrameType)) {
				...
				do {
					...
					hasTxDesc =
					    RTMP_HAS_ENOUGH_FREE_DESC(pAd, pTxBlk,
								      FreeNumber[QueIdx], pPacket);
					if ((hasTxDesc == FALSE)
					    || (CanDoAggregateTransmit(pAd, pPacket, pTxBlk) ==
						FALSE))
						break;
			...
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
				/*
				 * pTxBlk->QueIdx can not be QID_HCCA=4 because
				 * 1. It will causes AckPolicy[4] out of bound in ap/ap_data.c: AP_Legacy_Frame_Tx().
				 * 2. Mutichannel do not run on AP mode.
				 */
				ASSERT(pTxBlk->QueIdx < QID_HCCA);
				if (pTxBlk->QueIdx == QID_HCCA) {
					DBGPRINT(RT_DEBUG_ERROR,
						 ("pTxBlk->QueIdx=%d causes AckPolicy[4] out of bound\n",
						  pTxBlk->QueIdx));
					return;
				}
...
VOID RTMPResumeMsduTransmission(IN PRTMP_ADAPTER pAd)
{
	...
	/* Clear bOnlyScanActiveChannel after scan is done */
	pAd->StaCfg.bOnlyScanActiveChannel = FALSE;
				...
				IN PUCHAR pData, IN ULONG DataSize, IN UCHAR OpMode)
{
	...
	if ((FromWhichBSSID < pAd->ApCfg.BssidNum)
		...
		VLAN_Size =
		    (pAd->ApCfg.MBSSID[FromWhichBSSID].wdev.VLAN_VID != 0) ? LENGTH_802_1Q : 0;
#ifdef WDS_VLAN_SUPPORT
	else if ((FromWhichBSSID >= MIN_NET_DEVICE_FOR_WDS) &&
		 (FromWhichBSSID < (MIN_NET_DEVICE_FOR_WDS + MAX_WDS_ENTRY))) {
		VLAN_Size =
		    (pAd->WdsTab.WdsEntry[FromWhichBSSID - MIN_NET_DEVICE_FOR_WDS].wdev.VLAN_VID !=
		     0) ? LENGTH_802_1Q : 0;
	}
	...
	while (DataSize > LENGTH_802_3) {
		...
		if ((Header802_3[12] == 0x88) && (Header802_3[13] == 0x8E)
			...
			if (Elem != NULL) {
				memmove(Elem->Msg + (LENGTH_802_11 + LENGTH_802_1_H), pPayload,
					PayloadSize);
				Elem->MsgLen = LENGTH_802_11 + LENGTH_802_1_H + PayloadSize;
				REPORT_MGMT_FRAME_TO_MLME(pAd, BSSID_WCID, Elem->Msg, Elem->MsgLen,
							  0, 0, 0, 0, OPMODE_STA);
			...
			if (MAC_ADDR_EQUAL(pAd->ApCfg.ApCliTab[0].MlmeAux.Bssid, pSA)) {
				...
				if (Elem != NULL) {
					if (pRxBlk != NULL) {
						...
						memmove(Elem->Msg +
							(LENGTH_802_11 + LENGTH_802_1_H), pPayload,
							PayloadSize);
						Elem->MsgLen =
						    LENGTH_802_11 + LENGTH_802_1_H + PayloadSize;
						REPORT_MGMT_FRAME_TO_MLME(pAd,
									  apcli_entry->MacTabWCID,
...
#ifdef CONFIG_AP_SUPPORT
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
			    AP_ANNOUNCE_OR_FORWARD_802_3_PACKET(pAd, pClonePacket,
...
UINT BA_Reorder_AMSDU_Annnounce(IN PRTMP_ADAPTER pAd, IN PNDIS_PACKET pPacket, IN UCHAR OpMode)
{
	PUCHAR pData;
	USHORT DataSize;
	UINT nMSDU = 0;
...
VOID Indicate_AMSDU_Packet(IN PRTMP_ADAPTER pAd, IN RX_BLK *pRxBlk, IN UCHAR FromWhichBSSID)
{
	...
	nMSDU =
	    deaggregate_AMSDU_announce(pAd, pRxBlk, pRxBlk->pRxPacket, pRxBlk->pData,
				       pRxBlk->DataSize, pRxBlk->OpMode);
}
 ...
 */
static inline VOID Sniff2BytesFromNdisBuffer(IN PNDIS_BUFFER buf,
					     IN UCHAR offset, OUT UCHAR *p0, OUT UCHAR *p1)
{
...
BOOLEAN RTMPCheckEtherType(IN RTMP_ADAPTER *pAd,
			   IN PNDIS_PACKET pPacket,
			   IN MAC_TABLE_ENTRY *pMacEntry,
			   IN struct wifi_dev *wdev, OUT UCHAR *pUserPriority, OUT UCHAR *pQueIdx)
{
	UINT16 TypeLen;
			...
			if (*(pSrcBuf + 9) == IP_PROTO_UDP) {
				...
				if (wdev->wdev_type == WDEV_TYPE_AP) {
...
#ifdef CONFIG_HOTSPOT
					{
						...
						if (!HSIPv4Check
						    (pAd, &Wcid, pPacket, pSrcBuf, srcPort,
						     dstPort))
							return FALSE;
					}
			...
			if (wdev->wdev_type == WDEV_TYPE_AP) {
				...
				if (pMbss->WNMCtrl.ProxyARPEnable) {
					...
					if (IsIPv4ProxyARPCandidate(pAd, pSrcBuf - 2)) {
						...
						FoundProxyARPEntry =
						    IPv4ProxyARP(pAd, pMbss, pSrcBuf - 2, TRUE);
			...
			if (wdev->wdev_type == WDEV_TYPE_AP) {
				...
				if (pMbss->WNMCtrl.ProxyARPEnable) {
					...
					if (IsIPv6ProxyARPCandidate(pAd, pSrcBuf - 2)) {
						BOOLEAN FoundProxyARPEntry;
						FoundProxyARPEntry =
						    IPv6ProxyARP(pAd, pMbss, pSrcBuf - 2, TRUE);
						if (!FoundProxyARPEntry)
							DBGPRINT(RT_DEBUG_TRACE,
				...
				if ((pBAEntry->list.qlen > 0) &&
					...
					DBGPRINT(RT_DEBUG_OFF,
						 ("Indicate_Legacy_Packet():flush reordering_timeout_mpdus! RxWI->Flags=%d, pRxWI.TID=%d, RxD->AMPDU=%d!\n",
						  pRxBlk->Flags, pRxBlk->TID,
						  pRxBlk->pRxInfo->AMPDU));
					hex_dump("Dump the legacy Packet:",
						 GET_OS_PKT_DATAPTR(pRxBlk->pRxPacket), 64);
...
/* +++Add by shiang for debug */
	if (0) {
		hex_dump("After80211_2_8023", GET_OS_PKT_DATAPTR(pRxBlk->pRxPacket),
...
/* ---Add by shiang for debug */
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		if (pRxBlk->pRxInfo->Bcast && (pAd->ApCfg.bMACRepeaterEn)
			...
			if (*(pLayerHdr + 9) == 0x11) {
				...
				if (srcPort == 67 && dstPort == 68) {	/*It's a DHCP packet */
					...
					pReptEntry =
					    RTMPLookupRepeaterCliEntry(pAd, FALSE, pCliHwAddr);
					if (pReptEntry)
						NdisMoveMemory(pCliHwAddr,
							       pReptEntry->OriginalAddress,
							       MAC_ADDR_LEN);
					bHdrChanged = TRUE;
				}
...
/* Normal legacy Rx packet indication*/
VOID Indicate_Legacy_Packet_Hdr_Trns(IN RTMP_ADAPTER *pAd,
...
VOID CmmRxnonRalinkFrameIndicate(IN PRTMP_ADAPTER pAd, IN RX_BLK *pRxBlk, IN UCHAR FromWhichBSSID)
{
...
/* Normal, AMPDU or AMSDU*/
#ifdef HDR_TRANS_SUPPORT
VOID CmmRxnonRalinkFrameIndicate_Hdr_Trns(IN PRTMP_ADAPTER pAd,
					  IN RX_BLK *pRxBlk, IN UCHAR FromWhichBSSID)
{
	if (RX_BLK_TEST_FLAG(pRxBlk, fRX_AMPDU) && (pAd->CommonCfg.bDisableReordering == 0)) {
...
PNDIS_PACKET RTMPDeFragmentDataFrame(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	if (pHeader->Frag == 0) {	/* Frag. Number is 0 : First frag or only one pkt */
		...
		if (pHeader->FC.MoreFrag) {
			...
			NdisMoveMemory(pFragBuffer + RXWISize, pHeader,
				       pAd->FragFrame.RxSize - RXWISize);
	...
	if (bReassDone) {
		...
		if (pNewFragPacket) {
			...
			pRxBlk->pRxWI = (RXWI_STRUCT *) GET_OS_PKT_DATAPTR(pRetPacket);
...
VOID Indicate_EAPOL_Packet(IN RTMP_ADAPTER *pAd, IN RX_BLK *pRxBlk, IN UCHAR FromWhichBSSID)
{
	MAC_TABLE_ENTRY *pEntry = NULL;
...
#ifdef SOFT_ENCRYPT
BOOLEAN RTMPExpandPacketForSwEncrypt(IN RTMP_ADAPTER *pAd, IN TX_BLK *pTxBlk)
{
	PACKET_INFO PacketInfo;
	...
	RTMP_QueryPacketInfo(pTxBlk->pPacket, &PacketInfo, &pTxBlk->pSrcBufHeader,
			     &pTxBlk->SrcBufLen);

	return TRUE;
}

VOID RTMPUpdateSwCacheCipherInfo(IN RTMP_ADAPTER *pAd, IN TX_BLK *pTxBlk, IN UCHAR *pHdr)
{
	HEADER_802_11 *pHeader_802_11;
...
#endif /* DOT11Z_TDLS_SUPPORT || defined(CFG_TDLS_SUPPORT) */
		}
#endif /* CONFIG_STA_SUPPORT */
...
VOID RtmpPrepareHwNullFrame(IN PRTMP_ADAPTER pAd,
...
{
	UINT8 TXWISize = pAd->chipCap.TXWISize;
	TXWI_STRUCT *pTxWI = &pAd->NullTxWI;
	PUCHAR pNullFrame;
	NDIS_STATUS NState;
	HEADER_802_11 *pNullFr;
	ULONG Length;
	...
	if (NState == NDIS_STATUS_SUCCESS) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("@@@ %s : pNullFr->Addr1 =%02x:%02x:%02x:%02x:%02x:%02x\n", __func__,
			  PRINT_MAC(pNullFr->Addr1)));
		...
		if (IS_ENTRY_APCLI(pEntry)) {
			...
			/* yiwei */
			DBGPRINT(RT_DEBUG_TRACE,
				 ("@@@ %s : pNullFr->Addr2 =%02x:%02x:%02x:%02x:%02x:%02x\n",
				  __func__, PRINT_MAC(pNullFr->Addr2)));
		...
		pNullFr->Duration =
		    pAd->CommonCfg.Dsifs + RTMPCalcDuration(pAd, pAd->CommonCfg.TxRate, 14);
		...
		if (Index == 1) {
			...
			RTMPWriteTxWI(pAd,
				      ...
				      0, (UCHAR) Transmit.field.MCS, IFS_HTTXOP, &Transmit);
		} else {
			RTMPWriteTxWI(pAd,
				      pTxWI,
				      FALSE,
		...
		hex_dump("RtmpPrepareHwNullFrame ==>", pNullFrame, TXWISize + Length);

		DBGPRINT(RT_DEBUG_TRACE,
			 ("@@@ write to buffer pAd->NullBufOffset[%d]=0x%x\n", Index,
			  pAd->NullBufOffset[Index]));

		for (i = 0; i < Length; i += 4) {
			longValue =
			    *ptr + (*(ptr + 1) << 8) + (*(ptr + 2) << 16) + (*(ptr + 3) << 24);
			if (Index == 0)	/* for ra0 */
				RTMP_IO_WRITE32(pAd, pAd->chipCap.BcnBase[14] + TXWISize + i,
						longValue);
			else if (Index == 1)	/* for p2p0 */
				RTMP_IO_WRITE32(pAd, pAd->chipCap.BcnBase[15] + TXWISize + i,
						longValue);
...
VOID Disable_Tx2Q(RTMP_ADAPTER *pAd)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Disable_Tx2Q mac 404  value %x\n", Value));
}
...
VOID dev_rx_mgmt_frm(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		if (pAd->ApCfg.BANClass3Data == TRUE) {
			...
			if ((pHeader->FC.SubType == SUBTYPE_ASSOC_REQ)
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		...
		if ((pHeader->FC.SubType == SUBTYPE_BEACON) &&
		    (ADHOC_ON(pAd)) && (pRxBlk->wcid < MAX_LEN_OF_MAC_TABLE)) {
			MAC_TABLE_ENTRY *pEntry = NULL;
			pEntry = &pAd->MacTab.Content[pRxBlk->wcid];
	...
	REPORT_MGMT_FRAME_TO_MLME(pAd, pRxBlk->wcid, pHeader,
				  pRxBlk->DataSize,
				  pRxBlk->rssi[0], pRxBlk->rssi[1], pRxBlk->rssi[2],
				  min((CHAR) pRxBlk->snr[0], (CHAR) pRxBlk->snr[1]), op_mode);
	...
	if (pAd->chipCap.FlgHwTxBfCap) {
		...
		if (((pHeader->FC.SubType == SUBTYPE_ACTION)
		     || (pHeader->FC.SubType == SUBTYPE_ACTION_NO_ACK))
		    && (pRxBlk->pData[0] == CATEGORY_HT)
		    && ((pRxBlk->pData[1] == MIMO_N_BEACONFORM)
			|| (pRxBlk->pData[1] == MIMO_BEACONFORM))
		    ) {
	...
	case SUBTYPE_RTS:
		if (pAd->CommonCfg.vht_bw_signal && pRxBlk->wcid <= MAX_LEN_OF_MAC_TABLE) {
			...
			if ((rts->Addr1[0] & 0x1) == 0x1) {
				srv_field =
				    (PLCP_SERVICE_FIELD *) &pRxBlk->pRxWI->RXWI_N.bbp_rxinfo[15];
				if (srv_field->dyn_bw == 1) {
					DBGPRINT(RT_DEBUG_TRACE,
						 ("%02x:%02x:%02x:%02x:%02x:%02x, WCID:%d, DYN,BW=%d\n",
						  PRINT_MAC(rts->Addr1), pRxBlk->wcid,
						  srv_field->cbw_in_non_ht));
				}
			}
		}
		break;
...
BOOLEAN rtmp_rx_done_handle(RTMP_ADAPTER *pAd)
{
	...
	while (1) {
		...
		pRxBlk = &rxblk;
		pRxPacket =
		    GetPacketFromRxRing(pAd, pRxBlk, &bReschedule, &RxPending, &bCmdRspPacket, 0);
		...
		if (pAd->chipCap.hif_type == HIF_RLT) {
			...
			if ((pFceInfo->info_type != 0) || (pFceInfo->pkt_80211 != 1)) {
				DBGPRINT(RT_DEBUG_OFF,
					 ("==>%s(): GetFrameFromOtherPorts!\n", __func__));
				hex_dump("hw_rx_info", &rxblk.hw_rx_info[0],
					 sizeof(rxblk.hw_rx_info));
...
/* ---Add by shiang for debug */
		...
		if (RTMP_CFG80211_VIF_P2P_GO_ON(pAd) &&
		    (NdisEqualMemory
		     (pAd->cfg80211_ctrl.P2PCurrentAddress, pHeader->Addr1, MAC_ADDR_LEN)
		     || (pHeader->FC.SubType == SUBTYPE_PROBE_REQ))) {
			SET_PKT_OPMODE_AP(&rxblk);
		} else if (RTMP_CFG80211_VIF_P2P_CLI_ON(pAd) &&
			   (((pHeader->FC.SubType == SUBTYPE_BEACON
			      || pHeader->FC.SubType == SUBTYPE_PROBE_RSP)
			     && NdisEqualMemory(pAd->ApCfg.ApCliTab[MAIN_MBSSID].CfgApCliBssid,
						pHeader->Addr2, MAC_ADDR_LEN))
			    || (pHeader->FC.SubType == SUBTYPE_PROBE_REQ)
			    || NdisEqualMemory(pAd->ApCfg.ApCliTab[MAIN_MBSSID].MlmeAux.Bssid,
					       pHeader->Addr2, MAC_ADDR_LEN))) {
		...
		if (ATE_ON(pAd)) {
			...
			if ((pAd->chipCap.FlgHwTxBfCap) && (pHeader->FC.Type == FC_TYPE_MGMT)) {
				...
				if ((((pHeader->FC.SubType == SUBTYPE_ACTION)
				      || (pHeader->FC.SubType == SUBTYPE_ACTION_NO_ACK))
				     && (pRxBlk->pData)[0] == CATEGORY_HT && ((pRxBlk->pData)[1] == MIMO_N_BEACONFORM	/*non-compressed beamforming report */
									      || (pRxBlk->pData)[1] == MIMO_BEACONFORM))) {	/*compressed beamforming report */
					/* sounding frame */
					if (pAd->ate.sounding == 1) {
						int i, Nc = ((pRxBlk->pData)[2] & 0x3) + 1;
						pAd->ate.soundingSNR[0] =
						    (CHAR) ((pRxBlk->pData)[8]);
						pAd->ate.soundingSNR[1] =
						    (Nc < 2) ? 0 : (CHAR) ((pRxBlk->pData)[9]);
						pAd->ate.soundingSNR[2] =
						    (Nc < 3) ? 0 : (CHAR) ((pRxBlk->pData)[10]);
						pAd->ate.sounding = 2;
						pAd->ate.soundingRespSize = pRxBlk->DataSize;
						for (i = 0;
						     i < pRxBlk->DataSize
						     && i < MAX_SOUNDING_RESPONSE_SIZE; i++)
				...
				/* Roger Debug : Fix Me */
					...
					if (pHeader->FC.Order)
						DBGPRINT(RT_DEBUG_WARN,
							 ("fcsubtype=%x\ndata[0]=%x\ndata[1]=%x\n",
							  pHeader->FC.SubType, (pRxBlk->pData)[0],
							  (pRxBlk->pData)[1]));
				}
			}
#endif /* TXBF_SUPPORT */
		...
		if (MONITOR_ON(pAd) && pAd->monitor_ctrl.current_monitor_mode != MONITOR_MODE_OFF) {
			...
			if (pAd->monitor_ctrl.current_monitor_mode == MONITOR_MODE_REGULAR_RX) {
				...
				if ((pHeader->FC.Type == FC_TYPE_MGMT)
					...
					pClonePacket =
					    ClonePacket(pAd, rxblk.pRxPacket, rxblk.pData,
							rxblk.DataSize);
				...
				pTmpRxPacket = rxblk.pRxPacket;
				pClonePacket =
				    ClonePacket(pAd, rxblk.pRxPacket, rxblk.pData, rxblk.DataSize);
				rxblk.pRxPacket = pClonePacket;
				STA_MonPktSend(pAd, &rxblk);
		...
		if ((pHeader->FC.FrDs == 1) && (pHeader->FC.ToDs == 1)) {
			...
			if (MAC_ADDR_EQUAL(pHeader->Addr1, pAd->CurrentAddress))
				pEntry =
				    FindWdsEntry(pAd, pRxBlk->wcid, pHeader->Addr2,
						 pRxBlk->rx_rate.field.MODE);

#ifdef STATS_COUNT_SUPPORT
			if (pEntry) {
				pAd->WdsTab.WdsEntry[pEntry->wdev_idx].WdsCounter.
				    ReceivedByteCount += pRxBlk->MPDUtotalByteCnt;
				INC_COUNTER64(pAd->WdsTab.WdsEntry[pEntry->wdev_idx].WdsCounter.
					      ReceivedFragmentCount);

				if (IS_MULTICAST_MAC_ADDR(pHeader->Addr3))
					INC_COUNTER64(pAd->WdsTab.WdsEntry[pEntry->wdev_idx].
						      WdsCounter.MulticastReceivedFrameCount);
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			if (APCheckRxError(pAd, pRxInfo, pRxBlk) != NDIS_STATUS_SUCCESS) {
				...
				if (pHeader->FC.FrDs == 0 &&
				    pRxInfo->U2M && pRxBlk->bss_idx < pAd->ApCfg.BssidNum) {
					MULTISSID_STRUCT *pMbss =
					    &pAd->ApCfg.MBSSID[pRxBlk->bss_idx];
					pMbss->RxDropCount++;
					pMbss->RxErrorCount++;
				}
#ifdef WDS_SUPPORT
...
#endif /* RT_CFG80211_P2P_SUPPORT */
				/* Check for all RxD errors */
				if (RTMPCheckRxError(pAd, pHeader, &rxblk, pRxInfo) !=
				    NDIS_STATUS_SUCCESS) {
...
#ifdef CONFIG_AP_SUPPORT
#ifdef RT_CFG80211_P2P_SUPPORT
	if (pRxBlk != NULL && IS_PKT_OPMODE_AP(pRxBlk))
#else
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
#endif /* RT_CFG80211_P2P_SUPPORT */
	{
...
VOID drop_mask_init_per_client(PRTMP_ADAPTER ad, PMAC_TABLE_ENTRY entry)
{
	...
	RTMPInitTimer(ad, &entry->dropmask_timer, GET_TIMER_FUNCTION(drop_mask_timer_action), entry,
		      FALSE);
...
VOID drop_mask_release_per_client(PRTMP_ADAPTER ad, PMAC_TABLE_ENTRY entry)
{
	BOOLEAN cancelled = 0;
...
VOID drop_mask_per_client_reset(PRTMP_ADAPTER ad)
{
	INT i;
...
VOID set_drop_mask_per_client(PRTMP_ADAPTER ad, PMAC_TABLE_ENTRY entry, UINT8 type, BOOLEAN enable)
{
	BOOLEAN cancelled = 0;
...
VOID drop_mask_timer_action(IN PVOID SystemSpecific1,
