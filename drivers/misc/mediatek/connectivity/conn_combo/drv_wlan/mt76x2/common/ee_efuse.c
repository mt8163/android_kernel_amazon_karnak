...
VOID eFuseReadPhysical(IN PRTMP_ADAPTER pAd,
		       IN PUSHORT lpInBuffer,
		       IN ULONG nInBufferSize, OUT PUSHORT lpOutBuffer, IN ULONG nOutBufferSize);

static VOID eFusePhysicalWriteRegisters(IN PRTMP_ADAPTER pAd,
					IN USHORT Offset, IN USHORT Length, OUT USHORT *pData);

static NTSTATUS eFuseWriteRegisters(IN PRTMP_ADAPTER pAd,
				    IN USHORT Offset, IN USHORT Length, IN USHORT *pData);

static VOID eFuseWritePhysical(IN PRTMP_ADAPTER pAd,
			       PUSHORT lpInBuffer,
			       ULONG nInBufferSize, PUCHAR lpOutBuffer, ULONG nOutBufferSize);
					...
					IN USHORT Offset, IN USHORT Length, OUT USHORT *pData)
{
	...
	if ((Offset % 4) != 0) {
		eFuseDataBuffer[efuseDataOffset] =
		    (eFuseDataBuffer[efuseDataOffset] & 0xffff) | (data << 16);
	} else {
		eFuseDataBuffer[efuseDataOffset] =
				    ...
				    IN USHORT Offset, IN USHORT Length, IN USHORT *pData)
{
	...
	if (EFSROM_AOUT == 0x3f) {	/*find available logical address pointer        */
		...
		for (i = StartBlock; i >= EndBlock; i -= 2) {
			...
			if (((LogicalAddress >> 8) & 0xff) == 0) {	/*Not used logical address pointer */
			...
			if ((LogicalAddress & 0xff) == 0) {	/*Not used logical address pointer */
	...
	do {
		...
		tmpOffset |=
		    ((~
		      ((tmpOffset & 0x01) ^ (tmpOffset >> 1 & 0x01) ^ (tmpOffset >> 2 & 0x01) ^
		       (tmpOffset >> 3 & 0x01))) << 6) & 0x40;
		tmpOffset |=
		    ((~
		      ((tmpOffset >> 2 & 0x01) ^ (tmpOffset >> 3 & 0x01) ^ (tmpOffset >> 4 & 0x01) ^
		       (tmpOffset >> 5 & 0x01))) << 7) & 0x80;
...
int rtmp_ee_efuse_read16(IN RTMP_ADAPTER *pAd, IN USHORT Offset, OUT USHORT *pValue)
{
	eFuseReadRegisters(pAd, Offset, 2, pValue);
	return *pValue;
}

int rtmp_ee_efuse_write16(IN RTMP_ADAPTER *pAd, IN USHORT Offset, IN USHORT data)
{
...
INT eFuse_init(RTMP_ADAPTER *pAd)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("NVM is Efuse and its size =%x[%x-%x]\n", pAd->chipCap.EFUSE_USAGE_MAP_SIZE,
		  pAd->chipCap.EFUSE_USAGE_MAP_START, pAd->chipCap.EFUSE_USAGE_MAP_END));
