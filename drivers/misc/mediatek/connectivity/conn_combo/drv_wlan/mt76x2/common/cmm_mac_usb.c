	...
	do {
...
#ifdef OLD_PRE_ALLOC
			ASSERT(!pHTTXContext->TransferBuffer);
			pHTTXContext->TransferBuffer =
			    RTMPQMemAddr(sizeof(HTTX_BUFFER), &pHTTXContext->data_dma,
					 acidx + BLK_TX0);
		...
		for (i = 0; i < RX_RING_SIZE; i++) {
			...
			pRxContext->TransferBuffer =
			    RTMPQMemAddr(MAX_RXBULK_SIZE, &pRxContext->data_dma, i + BLK_RX0);
			if (pRxContext->TransferBuffer == NULL)
				goto err;
		...
		pCmdRspEventContext->CmdRspBuffer =
		    RTMPQMemAddr(CMD_RSP_BULK_SIZE, &pCmdRspEventContext->data_dma, BLK_CMD);
		if (!pCmdRspEventContext->CmdRspBuffer)
			goto err;
...
#endif /* endif */
		if (NdisEqualMemory(pBeaconSync->BeaconTxWI[bcn_idx], &pAd->BeaconTxWI, TXWISize) == FALSE) {	/* If BeaconTxWI changed, we need to rewrite the TxWI for the Beacon frames. */
			pBeaconSync->BeaconBitMap &= (~(BEACON_BITMAP_MASK & (1 << bcn_idx)));
