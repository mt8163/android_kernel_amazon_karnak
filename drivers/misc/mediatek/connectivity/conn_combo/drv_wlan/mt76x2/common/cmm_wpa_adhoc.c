...
VOID Adhoc_RTMPGetTxTscFromAsic(IN PRTMP_ADAPTER pAd, IN UCHAR apidx, OUT PUCHAR pTxTsc)
{
	...
	*(pTxTsc + 5) = EIV & 0xff000000;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("Adhoc_RTMPGetTxTscFromAsic : WCID(%d) TxTsc 0x%02x-0x%02x-0x%02x-0x%02x-0x%02x-0x%02x\n",
	...
	if (pEntry) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 (" PortSecured(%d), WpaState(%d), AuthMode(%d), PMKID_CacheIdx(%d)\n",
			  pEntry->PortSecured, pAuthenticator->WpaState, pEntry->AuthMode,
	...
	*((USHORT *) &EapolKeyInfo) = cpu2le16(*((USHORT *) &EapolKeyInfo));
	...
	if (MsgType == EAPOL_PAIR_MSG_1 || MsgType == EAPOL_PAIR_MSG_3 || MsgType == EAPOL_GROUP_MSG_1) {	/* For supplicant */
		...
		if ((RTMPCompareMemory
		     (pMsg->KeyDesc.ReplayCounter, p4WayProfile->ReplayCounter,
	...
	if (bReplayDiff) {
		...
		hex_dump("Receive replay counter ", pMsg->KeyDesc.ReplayCounter,
			 LEN_KEY_DESC_REPLAY);
		hex_dump("Current replay counter ", p4WayProfile->ReplayCounter,
			 LEN_KEY_DESC_REPLAY);
		return FALSE;
	}
	...
	if (MsgType != EAPOL_PAIR_MSG_1) {
		...
		if (EapolKeyInfo.KeyDescVer == KEY_DESC_TKIP) {	/* TKIP */
			RT_HMAC_MD5(p4WayProfile->PTK, LEN_PTK_KCK, (PUCHAR) pMsg, eapol_len, mic,
				    MD5_DIGEST_SIZE);
		} else if (EapolKeyInfo.KeyDescVer == KEY_DESC_AES) {	/* AES */
			RT_HMAC_SHA1(p4WayProfile->PTK, LEN_PTK_KCK, (PUCHAR) pMsg, eapol_len,
		...
		if (pEntry->Sst == SST_ASSOC) {
			...
			if ((peerKeyInfo.KeyAck == 1)
				...
				if ((peerKeyInfo.KeyMic == 1) &&
				    (peerKeyInfo.Request == 1) && (peerKeyInfo.Error == 1)) {
				...
				} else if ((peerKeyInfo.Request == 0) && (peerKeyInfo.Error == 0)
					   && (peerKeyInfo.KeyMic == 1)) {
...
VOID Adhoc_WpaStart4WayHS(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY *pEntry, IN ULONG TimeInterval)
{
	...
	if ((pAuthenticator->WpaState > AS_PTKSTART) || (pAuthenticator->WpaState < AS_INITPMK)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("[ERROR]WPAStart4WayHS : Not expect calling=%d\n",
	...
	/* EAPOL-Key(0,0,1,0,P,0,0,ANonce,0,DataKD_M1) */
	Adhoc_ConstructEapolMsg(pEntry, group_cipher, EAPOL_PAIR_MSG_1, 0,	/* Default key index */
				pAuthenticator->ANonce, NULL,	/* TxRSC */
				NULL,	/* GTK */
				NULL,	/* RSNIE */
				0,	/* RSNIE length */
				pAuthenticator, pEapolFrame);
			      ...
			      IN MAC_TABLE_ENTRY *pEntry, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (Adhoc_PeerWpaMessageSanity(pAd, pMsg1, MsgLen, EAPOL_PAIR_MSG_1, pSupplicant, pEntry) ==
	    FALSE)
		return;

	/* Store Replay counter, it will use to verify message 3 and construct message 2 */
	NdisMoveMemory(pSupplicant->ReplayCounter, pMsg1->KeyDesc.ReplayCounter,
		       LEN_KEY_DESC_REPLAY);
	...
	WpaDerivePTK(pAd,
		     pmk_ptr,
		     pSupplicant->ANonce,
		     pEntry->Addr, pSupplicant->SNonce, pCurrentAddr, PTK, LEN_PTK);
	...
	Adhoc_ConstructEapolMsg(pEntry, group_cipher, EAPOL_PAIR_MSG_2, 0,	/* DefaultKeyIdx */
				pSupplicant->SNonce, NULL,	/* TxRsc */
				NULL,	/* GTK */
				(UCHAR *) rsnie_ptr, rsnie_len, pSupplicant, pEapolFrame);
			      ...
			      IN MAC_TABLE_ENTRY *pEntry, IN MLME_QUEUE_ELEM *Elem)
{
	...
	/* Derive PTK */
	WpaDerivePTK(pAd, (UCHAR *) pmk_ptr, pAuthenticator->ANonce,	/* ANONCE */
		     pAd->CurrentAddress, pAuthenticator->SNonce,	/* SNONCE */
	...
	if (Adhoc_PeerWpaMessageSanity(pAd, pMsg2, MsgLen, EAPOL_PAIR_MSG_2, pAuthenticator, pEntry)
	    == FALSE)
		return;
	...
	do {
		...
		Adhoc_ConstructEapolMsg(pEntry,
					...
					(UCHAR *) gtk_ptr,
					(UCHAR *) rsnie_ptr,
					rsnie_len, pAuthenticator, pEapolFrame);
			      ...
			      IN MAC_TABLE_ENTRY *pEntry, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (Adhoc_PeerWpaMessageSanity(pAd, pMsg3, MsgLen, EAPOL_PAIR_MSG_3, pSupplicant, pEntry) ==
	    FALSE)
		return;

	/* Save Replay counter, it will use construct message 4 */
	NdisMoveMemory(pSupplicant->ReplayCounter, pMsg3->KeyDesc.ReplayCounter,
		       LEN_KEY_DESC_REPLAY);
	...
	Adhoc_ConstructEapolMsg(pEntry, group_cipher, EAPOL_PAIR_MSG_4, 0,	/* group key index not used in message 4 */
				NULL,	/* Nonce not used in message 4 */
				NULL,	/* TxRSC not used in message 4 */
				NULL,	/* GTK not used in message 4 */
				NULL,	/* RSN IE not used in message 4 */
				0, pSupplicant, pEapolFrame);
			      ...
			      IN MAC_TABLE_ENTRY *pEntry, IN MLME_QUEUE_ELEM *Elem)
{
	...
	do {
		...
		if (Adhoc_PeerWpaMessageSanity
		    (pAd, pMsg4, MsgLen, EAPOL_PAIR_MSG_4, pAuthenticator, pEntry) == FALSE)
			break;
			       ...
			       IN MAC_TABLE_ENTRY *pEntry, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (Adhoc_PeerWpaMessageSanity(pAd, pGroup, MsgLen, EAPOL_GROUP_MSG_1, pSupplicant, pEntry)
	    == FALSE)
		return;

	/* Save Replay counter, it will use to construct message 2 */
	NdisMoveMemory(pSupplicant->ReplayCounter, pGroup->KeyDesc.ReplayCounter,
		       LEN_KEY_DESC_REPLAY);
	...
	Adhoc_ConstructEapolMsg(pEntry, group_cipher, EAPOL_GROUP_MSG_2, default_key, NULL,	/* Nonce not used */
				NULL,	/* TxRSC not used */
				NULL,	/* GTK not used */
...
VOID Adhoc_Wpa4WayComplete(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY *pEntry)
{
	...
	DBGPRINT(RT_DEBUG_OFF,
		 ("Adhoc_Wpa4WayComplete - WPA2, AuthMode(%d)=%s, WepStatus(%d)=%s, GroupWepStatus(%d)=%s\n\n",
		  pEntry->AuthMode, GetAuthMode(pEntry->AuthMode), pEntry->WepStatus,
		  GetEncryptType(pEntry->WepStatus), pAd->StaCfg.GroupCipher,
			...
			IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if ((pEntry) && IS_ENTRY_CLIENT(pEntry)) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Adhoc_WPARetryExec---> ReTryCounter=%d, WpaState=%d\n",
		...
		case Ndis802_11AuthModeWPA2PSK:
			if (pAuthenticator->MsgRetryCounter == 0) {
...
#endif /* IWSC_SUPPORT // */
				{
					...
					DBGPRINT(RT_DEBUG_ERROR,
						 ("Adhoc_WPARetryExec::MSG1 timeout\n"));
				}

			} else if (pAuthenticator->MsgType == EAPOL_PAIR_MSG_1) {
				if ((pAuthenticator->WpaState == AS_PTKSTART)
				    || (pAuthenticator->WpaState == AS_INITPSK)
				    || (pAuthenticator->WpaState == AS_INITPMK)) {
					DBGPRINT(RT_DEBUG_TRACE,
						 ("Adhoc_WPARetryExec::ReTry MSG1 of 4-way Handshake\n"));
					Adhoc_WpaStart4WayHS(pAd, pEntry,
...
#endif /* IWSC_SUPPORT // */
				{
					MlmeDeAuthAction(pAd, pEntry, REASON_4_WAY_TIMEOUT, FALSE);
					DBGPRINT(RT_DEBUG_TRACE,
						 ("Adhoc_WPARetryExec::Retry MSG3, TIMEOUT\n"));
				}
			}
			break;
		default:
			     ...
			     IN PFOUR_WAY_HANDSHAKE_PROFILE p4WayProfile, OUT PEAPOL_PACKET pMsg)
{
	...
	KeyDescVer = (((pEntry->WepStatus == Ndis802_11Encryption3Enabled) ||
		       (GroupKeyWepStatus ==
			Ndis802_11Encryption3Enabled)) ? (KEY_DESC_AES) : (KEY_DESC_TKIP));
	...
	*(USHORT *) (pKeyInfo) = cpu2le16(*(USHORT *) (pKeyInfo));
	...
	if (bWPA2) {
		...
		if ((MsgType == EAPOL_PAIR_MSG_1) || (MsgType == EAPOL_PAIR_MSG_3))
			pKeyDesc->KeyLength[1] =
			    ((pEntry->WepStatus ==
			      Ndis802_11Encryption2Enabled) ? LEN_TKIP_TK : LEN_AES_TK);
	} else {
		if (MsgType >= EAPOL_GROUP_MSG_1) {
			/* the length of group key cipher */
			pKeyDesc->KeyLength[1] =
			    ((GroupKeyWepStatus ==
			      Ndis802_11Encryption2Enabled) ? LEN_TKIP_GTK : LEN_AES_GTK);
		} else {
	...
	Adhoc_ConstructEapolKeyData(pEntry,
				    ...
				    KeyDescVer,
				    MsgType,
				    DefaultKeyIdx, GTK, RSNIE, RSNIE_Len, p4WayProfile, pMsg);
	...
	if (GTK_Included) {
		if ((keyDescVer == KEY_DESC_AES)) {
			...
			AES_Key_Wrap(Key_Data, (UINT) data_offset,
				     &p4WayProfile->PTK[LEN_PTK_KCK], LEN_PTK_KEK, eGTK, &wrap_len);
