...
static VOID WpaEAPOLASFAlertAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID WpaEAPOLLogoffAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID WpaEAPOLStartAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);
...
/*
    ...
    ==========================================================================
 */
VOID WpaStateMachineInit(IN PRTMP_ADAPTER pAd, IN STATE_MACHINE * S, OUT STATE_MACHINE_FUNC Trans[])
{
	StateMachineInit(S, (STATE_MACHINE_FUNC *) Trans, MAX_WPA_PTK_STATE, MAX_WPA_MSG,
			 (STATE_MACHINE_FUNC) Drop, WPA_PTK, WPA_MACHINE_BASE);
	...
	if (Elem->MsgLen == 6)
		...
		if (pEntry &&
		    ...
		    ((pEntry->AuthMode == Ndis802_11AuthModeWPAPSK)
		     || (pEntry->AuthMode == Ndis802_11AuthModeWPA2PSK)) && pEntry->bWscCapable) {
	...
	if (pEntry) {
		DBGPRINT(RT_DEBUG_TRACE,
			 (" PortSecured(%d), WpaState(%d), AuthMode(%d), PMKID_CacheIdx(%d)\n",
			  pEntry->PortSecured, pEntry->WpaState, pEntry->AuthMode,
			  pEntry->PMKID_CacheIdx));
...
VOID WpaEAPOLKeyAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	NdisMoveMemory((PUCHAR) &peerKeyInfo, (PUCHAR) &pEapol_packet->KeyDesc.KeyInfo,
		       sizeof(KEY_INFO));
	...
	*((USHORT *) &peerKeyInfo) = cpu2le16(*((USHORT *) &peerKeyInfo));
	...
	do {
		...
		if (CliIdx != 0xFF)
			pEntry =
			    &pAd->MacTab.Content[pAd->ApCfg.ApCliTab[ifIndex].RepeaterCli[CliIdx].
						 MacTabWCID];
		else
#endif /* MAC_REPEATER_SUPPORT */
			pEntry = MacTableLookup(pAd, pHeader->Addr2);
		...
		if ((pEntry->Sst == SST_ASSOC) && (pEntry->WpaState >= AS_INITPSK)) {
			...
			if (peerKeyInfo.KeyAck == 1) {
				...
				if ((peerKeyInfo.Request == 0) &&
					    (peerKeyInfo.Error == 0) && (peerKeyInfo.KeyMic == 1)) {
					if (peerKeyInfo.Secure == 0
						...
						if (CONV_ARRARY_TO_UINT16
						    (pEapol_packet->KeyDesc.KeyDataLen) == 0)
					...
					if (peerKeyInfo.KeyType == GROUPKEY) {
						...
						DBGPRINT(RT_DEBUG_TRACE,
							 ("REQUEST=1, ERROR=0, update group key\n"));
						...
						wdev->DefaultKeyId =
						    (wdev->DefaultKeyId == 1) ? 2 : 1;
						WpaDeriveGTK(mbss->GMK, mbss->GNonce, wdev->bssid,
							     mbss->GTK, LEN_TKIP_GTK);

						for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
							if (IS_ENTRY_CLIENT(&pAd->MacTab.Content[i])
							    && (pAd->MacTab.Content[i].WpaState ==
								AS_PTKINITDONE)
							    && (pAd->MacTab.Content[i].apidx ==
								apidx)) {
								pAd->MacTab.Content[i].GTKState =
								    REKEY_NEGOTIATING;
								WPAStart2WayGroupHS(pAd,
										    &pAd->MacTab.
										    Content[i]);
								pAd->MacTab.Content[i].
								    ReTryCounter =
								    GROUP_MSG1_RETRY_TIMER_CTR;
								RTMPModTimer(&pAd->MacTab.
									     Content[i].RetryTimer,
						...
						DBGPRINT(RT_DEBUG_TRACE,
							 ("REQUEST=1, ERROR= 0, update pairwise key\n"));

						NdisZeroMemory(&pEntry->PairwiseKey,
							       sizeof(CIPHER_KEY));
		       ...
		       IN UINT HdrLen, IN PUCHAR pData, IN UINT DataLen, IN BOOLEAN bClearFrame)
{
	...
	do {
		Status =
		    RTMPAllocateNdisPacket(pAd, &pPacket, pHeader802_3, HdrLen, pData, DataLen);
	...
	*((USHORT *) &EapolKeyInfo) = cpu2le16(*((USHORT *) &EapolKeyInfo));
	...
	if (MsgType == EAPOL_PAIR_MSG_1 || MsgType == EAPOL_PAIR_MSG_3 || MsgType == EAPOL_GROUP_MSG_1) {	/* For supplicant */
		...
		if ((RTMPCompareMemory
		     (pMsg->KeyDesc.ReplayCounter, pEntry->R_Counter, LEN_KEY_DESC_REPLAY) != 1)
		    &&
		    (RTMPCompareMemory(pMsg->KeyDesc.ReplayCounter, ZeroReplay, LEN_KEY_DESC_REPLAY)
		     != 0)) {
			bReplayDiff = TRUE;
		}
	...
	if (bReplayDiff) {
		...
		RTMPSendWirelessEvent(pAd, IW_REPLAY_COUNTER_DIFF_EVENT_FLAG, pEntry->Addr,
				      pEntry->apidx, 0);
	...
	if (MsgType != EAPOL_PAIR_MSG_1) {
		...
		if (EapolKeyInfo.KeyDescVer == KEY_DESC_TKIP) {	/* TKIP */
			RT_HMAC_MD5(pEntry->PTK, LEN_PTK_KCK, (PUCHAR) pMsg, eapol_len, mic,
				    MD5_DIGEST_SIZE);
		} else if (EapolKeyInfo.KeyDescVer == KEY_DESC_AES) {	/* AES        */
			RT_HMAC_SHA1(pEntry->PTK, LEN_PTK_KCK, (PUCHAR) pMsg, eapol_len, digest,
				     SHA1_DIGEST_SIZE);
		...
		if (!NdisEqualMemory(rcvd_mic, mic, LEN_KEY_DESC_MIC)) {
			...
			RTMPSendWirelessEvent(pAd, IW_MIC_DIFF_EVENT_FLAG, pEntry->Addr,
					      pEntry->apidx, 0);
...
/*
    ...
    ==========================================================================
*/
VOID WPAStart4WayHS(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY *pEntry, IN ULONG TimeInterval)
{
	...
	if ((pEntry->AuthMode == Ndis802_11AuthModeWPA2)
		...
		NdisMoveMemory(&pEapolFrame->KeyDesc.KeyData[6],
			       &pMbss->PMKIDCache.BSSIDInfo[pEntry->PMKID_CacheIdx].PMKID,
			       LEN_PMKID);
		NdisMoveMemory(&pMbss->PMK,
			       &pMbss->PMKIDCache.BSSIDInfo[pEntry->PMKID_CacheIdx].PMK, PMK_LEN);
			...
			IN MAC_TABLE_ENTRY *pEntry, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (Elem->MsgLen <
	    (LENGTH_802_11 + LENGTH_802_1_H + LENGTH_EAPOL_H + MIN_LEN_OF_EAPOL_KEY_MSG))
		return;
...
#ifdef DOT11W_PMF_SUPPORT
	if ((CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_USE_SHA256))) {
		PMF_DerivePTK(pAd, (UCHAR *) pmk_ptr, pEntry->ANonce, pEntry->Addr, pEntry->SNonce, pCurrentAddr, PTK, LEN_AES_PTK);	/* Must is 48 bytes */
...
#endif /* DOT11W_PMF_SUPPORT */
	{
		...
		WpaDerivePTK(pAd,
			     pmk_ptr,
			     pEntry->ANonce,
			     pEntry->Addr, pEntry->SNonce, pCurrentAddr, PTK, LEN_PTK);
			...
			IN MAC_TABLE_ENTRY *pEntry, IN MLME_QUEUE_ELEM *Elem)
{
...
#ifdef DOT11W_PMF_SUPPORT
	if ((CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_USE_SHA256))) {
		PMF_DerivePTK(pAd, (UCHAR *) pmk_ptr, pEntry->ANonce, (UCHAR *) pBssid, pEntry->SNonce, pEntry->Addr, PTK, LEN_AES_PTK);	/* Must is 48 bytes */
...
#endif /* DOT11W_PMF_SUPPORT */
	{
		...
		if ((pmk_ptr == NULL) || (pBssid == NULL)) {
			DBGPRINT(RT_DEBUG_ERROR, ("%s: pmk_ptr or pBssid == NULL!\n", __func__));
			return;
		}
	...
	do {
		...
		ConstructEapolMsg(pEntry,
				  ...
				  pEntry->ANonce,
				  TxTsc,
				  (UCHAR *) gtk_ptr, (UCHAR *) rsnie_ptr, rsnie_len, pEapolFrame);
	...
	if (pEntry->AuthMode == Ndis802_11AuthModeWPA2PSK ||
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			if ((pAd->ApCfg.ApCliAutoConnectRunning == TRUE)
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Apcli auto connected:PeerPairMsg3Action()\n"));
	...
	do {
		...
		if (pEntry->AuthMode == Ndis802_11AuthModeWPA2 ||
			...
			if (pEntry->AuthMode == Ndis802_11AuthModeWPA2) {
				...
				NdisMoveMemory(&PMK_key[0], "PMK Name", 8);
				NdisMoveMemory(&PMK_key[8],
					       pAd->ApCfg.MBSSID[pEntry->apidx].wdev.bssid,
					       MAC_ADDR_LEN);
				NdisMoveMemory(&PMK_key[14], pEntry->Addr, MAC_ADDR_LEN);
				RT_HMAC_SHA1(pAd->ApCfg.MBSSID[pEntry->apidx].PMK, PMK_LEN, PMK_key,
					     20, digest, SHA1_DIGEST_SIZE);
				RTMPAddPMKIDCache(pAd, pEntry->apidx, pEntry->Addr, digest,
						  pAd->ApCfg.MBSSID[pEntry->apidx].PMK);
				DBGPRINT(RT_DEBUG_TRACE,
			...
			RTMPSendWirelessEvent(pAd, IW_SET_KEY_DONE_WPA2_EVENT_FLAG, pEntry->Addr,
					      pEntry->apidx, 0);
...
VOID WPAStart2WayGroupHS(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY *pEntry)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		...
		DBGPRINT(RT_DEBUG_WARN, ("%s : Operation mode is not match!\n", __func__));
		return;
	}
#endif /* CONFIG_AP_SUPPORT */
	...
	ConstructEapolMsg(pEntry,
			  ...
			  EAPOL_GROUP_MSG_1,
			  default_key,
			  (UCHAR *) gnonce_ptr, TxTsc, (UCHAR *) gtk_ptr, NULL, 0, pEapolFrame);
...
VOID MlmeDeAuthAction(IN PRTMP_ADAPTER pAd,
		      IN MAC_TABLE_ENTRY *pEntry, IN USHORT Reason, IN BOOLEAN bDataFrameFirst)
{
...
VOID PeerGroupMsg2Action(IN PRTMP_ADAPTER pAd,
			 IN MAC_TABLE_ENTRY *pEntry, IN VOID *Msg, IN UINT MsgLen)
{
	UINT Len;
	PUCHAR pData;
	BOOLEAN Cancelled;
	...
	do {
		...
		if ((pEntry->AuthMode == Ndis802_11AuthModeWPA2)
			...
			RTMPSendWirelessEvent(pAd, IW_SET_KEY_DONE_WPA1_EVENT_FLAG, pEntry->Addr,
					      pEntry->apidx, 0);
	 ...
	 IN INT prefix_len, IN UCHAR *data, IN INT data_len, OUT UCHAR *output, IN INT len)
{
	...
	for (i = 0; i < (len + 19) / 20; i++) {
		RT_HMAC_SHA1(key, key_len, input, total_len, &output[currentindex],
			     SHA1_DIGEST_SIZE);
		currentindex += 20;
...
static void F(char *password, unsigned char *ssid, int ssidlength, int iterations, int count,
...
VOID WpaDeriveGTK(IN UCHAR *GMK, IN UCHAR *GNonce, IN UCHAR *AA, OUT UCHAR *output, IN UINT len)
{
	UCHAR concatenation[76];
	UINT CurrPos = 0;
	UCHAR Prefix[19];
...
static VOID RTMPMakeRsnIeCipher(IN PRTMP_ADAPTER pAd,
				...
				IN UCHAR FlexibleCipher, OUT PUCHAR pRsnIe, OUT UCHAR *rsn_len)
{
	UCHAR PairwiseCnt;
		...
		case Ndis802_11AuthModeWPA1WPA2:
			...
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
				if (pAd->ApCfg.MBSSID[apidx].PmfCfg.MFPR) {
					NdisMoveMemory(pRsnie_auth->auth[0].oui, OUI_WPA2_1X_SHA256,
						       4);
					...
					NdisMoveMemory(pRsnie_auth->auth[0].oui + (4 * AkmCnt),
						       OUI_WPA2_1X_SHA256, 4);
					AkmCnt++;
					DBGPRINT(RT_DEBUG_WARN,
						 ("[PMF]%s: Insert 1X-SHA256 to AKM of RSNIE\n",
						  __func__));
			...
			IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
				...
				BssIdx =
				    BssTableSearchWithSSID(&pAd->MlmeAux.SsidBssTab,
				...
				if (BssIdx != BSS_NOT_FOUND) {
					...
					if (CLIENT_STATUS_TEST_FLAG
					    (pInBss, fCLIENT_STATUS_USE_SHA256)) {
...
#if defined(DOT1X_SUPPORT) || defined(DOT11W_PMF_SUPPORT)
			PMULTISSID_STRUCT pMbss = &pAd->ApCfg.MBSSID[apidx];
#endif /* DOT1X_SUPPORT || DOT11W_PMF_SUPPORT */
...
*/
#ifdef DOT11W_PMF_SUPPORT
static VOID RTMPInsertRsnIeZeroPMKID(IN PRTMP_ADAPTER pAd,
				     IN UCHAR ElementID,
				     IN UCHAR apidx, OUT PUCHAR pRsnIe, OUT UCHAR *rsn_len)
{
	PUINT8 pBuf;
...
VOID RTMPMakeRSNIE(RTMP_ADAPTER *pAd, UINT AuthMode, UINT WepStatus, UCHAR apidx)
{
	...
	do {
		...
		if (apidx >= MIN_NET_DEVICE_FOR_APCLI) {
			...
			if (pAd->ApCfg.ApCliTab[apcliIfidx].wpa_supplicant_info.WpaSupplicantUP !=
			...
			if (apcliIfidx < MAX_APCLI_NUM) {
				...
				NdisZeroMemory(pAd->ApCfg.ApCliTab[apcliIfidx].RSN_IE,
					       MAX_LEN_OF_RSNIE);
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			if (pAd->ApCfg.MBSSID[apidx].Hostapd != Hostapd_Disable)
				return;
	...
	if ((AuthMode == Ndis802_11AuthModeWPA) ||
		...
		PrimaryRsnie = WpaIe;
#ifdef WAPI_SUPPORT
	else if ((AuthMode == Ndis802_11AuthModeWAICERT) || (AuthMode == Ndis802_11AuthModeWAIPSK))
		PrimaryRsnie = WapiIe;
#endif /* WAPI_SUPPORT */
	else
		PrimaryRsnie = Wpa2Ie;
	...
	if (pRsnIe == NULL) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s: pRsnIe == NULL!\n", __func__));
		return;
	}
#ifdef WAPI_SUPPORT
...
#endif /* WAPI_SUPPORT */
	{
		...
		RTMPMakeRsnIeCipher(pAd, PrimaryRsnie, WepStatus, apidx, bMixCipher, FlexibleCipher,
				    pRsnIe, &p_offset);
	...
	*rsnielen_cur_p = p_offset;
	...
	if ((pAd->OpMode == OPMODE_AP)
		...
		if (bMixRsnIe) {
			/* 1. insert cipher suite */
			RTMPMakeRsnIeCipher(pAd, Wpa2Ie, WepStatus, apidx, bMixCipher,
					    FlexibleCipher, pRsnIe_ex, &s_offset);
...
/*
    ...
    ==========================================================================
*/
BOOLEAN RTMPCheckWPAframe(IN RTMP_ADAPTER *pAd,
			  IN MAC_TABLE_ENTRY *pEntry,
			  IN UCHAR *pData, IN ULONG DataByteCount, IN UCHAR FromWhichBSSID)
{
	ULONG Body_len;
	BOOLEAN Cancelled;
	...
	while (len > sizeof(RSNIE2)) {
		...
		/* WPA2 RSN IE, doesn't need to check RSNIE Capabilities field        */
			...
			if ((pEntry->AuthMode == Ndis802_11AuthModeWPA2
			    ...
			    &&
			    (NdisEqualMemory
			     (pEid->Octet, &pEntry->RSN_IE[2], pEntry->RSNIE_Len - 4))) {
						...
						if (pKDE->DataType == KDE_GTK) {
							...
							DBGPRINT(RT_DEBUG_TRACE,
							 ...
							 (CLIENT_STATUS_TEST_FLAG
							  (pEntry, fCLIENT_STATUS_PMF_CAPABLE))) {
		...
		{
			{
				...
				WPAInstallSharedKey(pAd,
						    ...
						    MCAST_WCID, FALSE, pAd->StaCfg.GTK, GTKLEN);
			}
		}
	}
#endif /* CONFIG_STA_SUPPORT */
...
VOID ConstructEapolMsg(IN PMAC_TABLE_ENTRY pEntry,
		       ...
		       IN UCHAR *KeyNonce,
		       IN UCHAR *TxRSC,
		       IN UCHAR *GTK, IN UCHAR *RSNIE, IN UCHAR RSNIE_Len, OUT PEAPOL_PACKET pMsg)
{
	BOOLEAN bWPA2 = FALSE;
	UCHAR KeyDescVer;
...
#endif /* DOT11W_PMF_SUPPORT */
	{
		...
		KeyDescVer = (((pEntry->WepStatus == Ndis802_11AESEnable) ||
			       (GroupKeyWepStatus ==
				Ndis802_11AESEnable)) ? (KEY_DESC_AES) : (KEY_DESC_TKIP));
	}
	...
	*(USHORT *) (&pMsg->KeyDesc.KeyInfo) = cpu2le16(*(USHORT *) (&pMsg->KeyDesc.KeyInfo));
	...
	if (bWPA2) {
		...
		if ((MsgType == EAPOL_PAIR_MSG_1) || (MsgType == EAPOL_PAIR_MSG_3))
			pMsg->KeyDesc.KeyLength[1] =
			    ((pEntry->WepStatus ==
			      Ndis802_11TKIPEnable) ? LEN_TKIP_TK : LEN_AES_TK);
	} else if (!bWPA2) {
		if (MsgType >= EAPOL_GROUP_MSG_1) {
			/* the length of group key cipher */
			pMsg->KeyDesc.KeyLength[1] =
			    ((GroupKeyWepStatus ==
			      Ndis802_11TKIPEnable) ? LEN_TKIP_GTK : LEN_AES_GTK);
		} else {
	...
	ConstructEapolKeyData(pEntry,
			      GroupKeyWepStatus,
			      KeyDescVer, MsgType, DefaultKeyIdx, GTK, RSNIE, RSNIE_Len, pMsg);
			   ...
			   IN UCHAR *RSNIE, IN UCHAR RSNIE_LEN, OUT PEAPOL_PACKET pMsg)
{
	...
	if (RSNIE_LEN && ((MsgType == EAPOL_PAIR_MSG_2) || (MsgType == EAPOL_PAIR_MSG_3))) {
		...
		RTMPInsertRSNIE(&Key_Data[data_offset],
				&data_offset, RSNIE, RSNIE_LEN, pmkid_ptr, pmkid_len);
	}
	...
	if ((MsgType == EAPOL_PAIR_MSG_3 && bWPA2Capable) || (MsgType == EAPOL_GROUP_MSG_1)) {
		...
		if ((bWPA2Capable)
			...
			PMF_InsertIGTKKDE(pEntry->pAd, pEntry->apidx, &Key_Data[data_offset],
					  &data_offset);
		}
#endif /* DOT11W_PMF_SUPPORT */
	...
	if (GTK_Included) {
		...
		if (
			...
			AES_Key_Wrap(Key_Data, (UINT) data_offset,
				     &pEntry->PTK[LEN_PTK_KCK], LEN_PTK_KEK, eGTK, &wrap_len);
			data_offset = wrap_len;

		} else {
			TKIP_GTK_KEY_WRAP(&pEntry->PTK[LEN_PTK_KCK],
					  pMsg->KeyDesc.KeyIv, Key_Data, data_offset, eGTK);
		}
...
CIPHER_KEY *RTMPSwCipherKeySelection(IN RTMP_ADAPTER *pAd,
				     IN UCHAR *pIV,
				     IN RX_BLK *pRxBlk, IN MAC_TABLE_ENTRY *pEntry)
{
	...
	if ((pEntry == NULL) ||
	    ...
	    (RX_BLK_TEST_FLAG(pRxBlk, fRX_WDS)) || (RX_BLK_TEST_FLAG(pRxBlk, fRX_MESH)))
		return NULL;
	...
	} else if ((CipherAlg == Ndis802_11TKIPEnable) || (CipherAlg == Ndis802_11AESEnable)) {
		if (pRxBlk->pRxInfo->U2M)
				     ...
				     INOUT PUCHAR pData, INOUT UINT16 *DataByteCnt)
{
	...
	case CIPHER_TKIP:
		...
		NStatus =
		    RTMPSoftDecryptTKIP(pAd, pHdr, UserPriority, pKey, pData, &(*DataByteCnt));
...
#endif /* endif */
			ret = RTMPSoftDecryptSMS4(pHdr, FALSE, pKey, pData, &(*DataByteCnt));
			if (ret != STATUS_SUCCESS) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("ERROR : SW decrypt SMS4 data fails(%d).\n", ret));
				/* give up this frame */
				return NDIS_STATUS_FAILURE;
			}
#ifdef RT_BIG_ENDIAN
			      ...
			      IN UCHAR KeyIdx, IN PCIPHER_KEY pKey, OUT UINT8 *ext_len)
{
	*ext_len = 0;
	...
	if (CipherAlg == CIPHER_SMS4) {
		...
		RTMPSoftEncryptSMS4(pHdr, pSrcBufData, SrcBufLen, KeyIdx, pKey->Key, pKey->TxTsc);
#ifdef RT_BIG_ENDIAN
		RTMPFrameEndianChange(pAd, pHdr, DIR_READ, FALSE);
#endif /* endif */
		*ext_len = LEN_WPI_MIC;
	...
	if ((CipherAlg == CIPHER_WEP64) || (CipherAlg == CIPHER_WEP128)) {
		...
		*ext_len = LEN_ICV;
		...
		/* Encrypt the MPDU data by software */
		RTMPSoftEncryptCCMP(pAd, pHdr, pKey->TxTsc, pKey->Key, pSrcBufData, SrcBufLen);
		     ...
		     IN UINT8 rsnie_len, IN PUINT8 pmkid_ptr, IN UINT8 pmkid_len)
{
	...
	if (rsnie_len != 0) {
		...
		MakeOutgoingFrame(pTmpBuf, &TempLen,
				  1, &ie_num, 1, &total_len, rsnie_len, rsnie_ptr, END_OF_ARGS);
		...
		*pFrameLen = *pFrameLen + TempLen;
		...
		if (ie_num == IE_RSN) {
			...
			if (extra_len > 0) {
				MakeOutgoingFrame(pTmpBuf, &TempLen,
						  2, &pmk_count, pmkid_len, pmkid_ptr, END_OF_ARGS);
	...
	if (pEntry->PairwiseKey.CipherAlg == CIPHER_TKIP) {
		if (bAE) {
			NdisMoveMemory(pEntry->PairwiseKey.TxMic,
				       &pEntry->PTK[OFFSET_OF_AP_TKIP_TX_MIC], LEN_TKIP_MIC);
			NdisMoveMemory(pEntry->PairwiseKey.RxMic,
				       &pEntry->PTK[OFFSET_OF_AP_TKIP_RX_MIC], LEN_TKIP_MIC);
		} else {
			NdisMoveMemory(pEntry->PairwiseKey.TxMic,
				       &pEntry->PTK[OFFSET_OF_STA_TKIP_TX_MIC], LEN_TKIP_MIC);
			NdisMoveMemory(pEntry->PairwiseKey.RxMic,
				       &pEntry->PTK[OFFSET_OF_STA_TKIP_RX_MIC], LEN_TKIP_MIC);
		}
	}
#ifdef SOFT_ENCRYPT
			 ...
			 UINT8 KeyIdx, UINT8 Wcid, BOOLEAN bAE, PUINT8 pGtk, UINT8 GtkLen)
{
	...
	if (Wcid != 0) {
		RTMPSetWcidSecurityInfo(pAd,
					BssIdx,
					KeyIdx, pSharedKey->CipherAlg, Wcid, SHAREDKEYTABLE);
	}
}
			     ...
			     UINT8 KeyIdx, UINT8 CipherAlg, UINT8 Wcid, UINT8 KeyTabFlag)
{
	...
	if (CipherAlg == CIPHER_WEP64 || CipherAlg == CIPHER_WEP128) {
		...
		IV_KEYID = (KeyIdx << 6);
		IV = (IV_KEYID << 24) | (TxTsc[2] << 16) | (TxTsc[1] << 8) | (TxTsc[0]);
