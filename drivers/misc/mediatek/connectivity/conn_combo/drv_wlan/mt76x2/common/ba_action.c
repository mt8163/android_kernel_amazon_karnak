...
static VOID Peer_DelBA_Tx_Adapt_Enable(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry);

static VOID Peer_DelBA_Tx_Adapt_Disable(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry);
...
DECLARE_TIMER_FUNCTION(BAOriSessionSetupTimeout);
BUILD_TIMER_FUNCTION(BAOriSessionSetupTimeout);
DECLARE_TIMER_FUNCTION(BARecSessionIdleTimeout);
...
#endif /* RT65xx */
	if (pAd->Antenna.field.TxPath == 3 && (pEntryPeer->HTCapability.MCSSet[2] != 0))
...
static void Announce_Reordering_Packet(RTMP_ADAPTER *pAd, struct reordering_mpdu *mpdu)
{
	...
	if (mpdu->bAMSDU) {
		...
		if (IS_OPMODE_AP(mpdu)) {
			AP_ANNOUNCE_OR_FORWARD_802_3_PACKET(pAd, pPacket,
...
static inline struct reordering_mpdu *ba_dequeue(struct reordering_list *list)
{
	...
	if (list == NULL) {
		DBGPRINT(RT_DEBUG_TRACE, ("%s():input list is null\n", __func__));
		ASSERT(list);
		return NULL;
	}
	ASSERT(!((list->next != NULL && list->qlen == 0)
		 || (list->next == NULL && list->qlen > 0)));
	if ((list->next != NULL && list->qlen == 0)
	    || (list->next == NULL && list->qlen > 0)) {
		DBGPRINT(RT_DEBUG_TRACE, ("%s():list is damaged\n", __func__));
		return NULL;
	}
	if (list->qlen > 0 && list->next != NULL) {
...
static USHORT ba_indicate_reordering_mpdus_in_order(IN PRTMP_ADAPTER pAd,
						    IN PBA_REC_ENTRY pBAEntry, IN USHORT StartSeq)
{
	struct reordering_mpdu *mpdu_blk;
	...
	while ((mpdu_blk = ba_reordering_mpdu_probe(&pBAEntry->list))) {
		...
		ASSERT(mpdu_blk);
		if (mpdu_blk == NULL) {
			DBGPRINT(RT_DEBUG_ERROR, ("%s mpdu_blk should not be null\n", __func__));
			break;
		}
		/* pass this frame up */
...
static void ba_indicate_reordering_mpdus_le_seq(IN PRTMP_ADAPTER pAd,
						IN PBA_REC_ENTRY pBAEntry, IN USHORT Sequence)
{
	struct reordering_mpdu *mpdu_blk;
	...
	*/
	...
	if (!(pEntry->TXBAbitmap & (1 << TID))) {
		RTMPInitTimer(pAd, &pBAEntry->ORIBATimer,
			      GET_TIMER_FUNCTION(BAOriSessionSetupTimeout), pBAEntry, FALSE);
	} else
...
static VOID BAOriSessionAdd(IN RTMP_ADAPTER *pAd,
			    IN MAC_TABLE_ENTRY *pEntry, IN FRAME_ADDBA_RSP * pFrame)
{
	BA_ORI_ENTRY *pBAEntry = NULL;
	BOOLEAN Cancelled;
	UCHAR TID;
	...
	if ((Idx != 0) && (pBAEntry->TID == TID) && (pBAEntry->ORI_BA_Status == Originator_WaitRes)) {
		...
		if (IS_ENTRY_APCLI(mac_entry)) {
			...
			if (pEntry->bReptCli)
				BarHeaderInit(pAd, &FrameBar, mac_entry->Addr,
					      pAd->ApCfg.ApCliTab[apidx].RepeaterCli[pEntry->
										     MatchReptCliIdx].
					      CurrentAddress);
			else
#endif /* MAC_REPEATER_SUPPORT */
				BarHeaderInit(pAd, &FrameBar, mac_entry->Addr,
					      mac_entry->wdev->if_addr);
		} else
#endif /* APCLI_SUPPORT */
		...
		if ((pAd->Mlme.bStartMcc == TRUE)
		    &&
		    (NdisEqualMemory
		     (mac_entry->wdev->if_addr, pAd->cfg80211_ctrl.P2PCurrentAddress,
		      MAC_ADDR_LEN))) {
			MiniportMMRequest(pAd, QID_HCCA, pOutBuffer2, FrameLen);
		} else
...
static BOOLEAN BARecSessionAdd(IN RTMP_ADAPTER *pAd,
			       IN MAC_TABLE_ENTRY *pEntry, IN FRAME_ADDBA_REQ *pFrame)
{
	...
	if (pBAEntry != NULL) {
		...
		if (pEntry->RXBAbitmap & (1 << TID))
			...
			RTMPInitTimer(pAd, &pBAEntry->RECBATimer,
				      GET_TIMER_FUNCTION(BARecSessionIdleTimeout), pBAEntry, TRUE);
...
VOID BAOriSessionTearDown(INOUT RTMP_ADAPTER *pAd,
...
VOID BASessionTearDownALL(RTMP_ADAPTER *pAd, UCHAR Wcid)
{
	...
	for (i = 0; i < NUM_OF_TID; i++) {
		BAOriSessionTearDown(pAd, Wcid, i, TRUE, FALSE);
		BARecSessionTearDown(pAd, Wcid, i, TRUE);
	}
}
			      ...
			      IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if ((pBAEntry->ORI_BA_Status == Originator_WaitRes)
		...
		MlmeEnqueue(pAd, ACTION_STATE_MACHINE, MT2_MLME_ADD_BA_CATE,
			    sizeof(MLME_ADDBA_REQ_STRUCT), (PVOID) & AddbaReq, 0);
			     ...
			     IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if ((pBAEntry->REC_BA_Status == Recipient_Accept)) {
		...
		if (RTMP_TIME_AFTER
		    ((unsigned long)Now32,
		     (unsigned long)(pBAEntry->LastIndSeqAtTimer + REC_BA_SESSION_IDLE_TIMEOUT))) {
			pAd = pBAEntry->pAdapter;
			/* flush all pending reordering mpdus */
			ba_refresh_reordering_mpdus(pAd, pBAEntry);
...
VOID PeerAddBAReqAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if (ADHOC_ON(pAd)
		...
		ActHeaderInit(pAd, &ADDframe.Hdr, pAddr, pAd->StaCfg.wdev.if_addr,
			      pAd->CommonCfg.Bssid);
	} else
#endif /* CONFIG_STA_SUPPORT */
	{
		if (pMacEntry->wdev == NULL) {
			DBGPRINT(RT_DEBUG_ERROR, ("%s: pMacEntry->wdev is NULL\n", __func__));
			if (pMacEntry->wdev->if_addr == NULL)
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: pMacEntry->wdev->if_addr is NULL\n", __func__));
			if (pMacEntry->wdev->bssid == NULL)
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: pMacEntry->wdev->bssid is NULL\n", __func__));
			goto error;
		}
		if (pAddr == NULL) {
			DBGPRINT(RT_DEBUG_ERROR, ("%s: pAddr is NULL\n", __func__));
			goto error;
		}
		ActHeaderInit(pAd, &ADDframe.Hdr, pAddr, pMacEntry->wdev->if_addr,
			      pMacEntry->wdev->bssid);
	}
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		if (IS_ENTRY_APCLI(pMacEntry)) {
			...
			if (pMacEntry->bReptCli)
				ActHeaderInit(pAd, &ADDframe.Hdr, pAddr,
					      pAd->ApCfg.ApCliTab[apidx].RepeaterCli
					      [pMacEntry->MatchReptCliIdx].CurrentAddress, pAddr);
			else
#endif /* MAC_REPEATER_SUPPORT */
				ActHeaderInit(pAd, &ADDframe.Hdr, pAddr,
					      pAd->ApCfg.ApCliTab[apidx].wdev.if_addr, pAddr);
		} else
#endif /* APCLI_SUPPORT */
#ifdef WDS_SUPPORT
		if (IS_ENTRY_WDS(pMacEntry)) {
			apidx = pMacEntry->wdev_idx;
			ActHeaderInit(pAd, &ADDframe.Hdr, pAddr,
				      pAd->ApCfg.MBSSID[MAIN_MBSSID].wdev.if_addr, pAddr);
		} else
#endif /* WDS_SUPPORT */
		{
...
#ifdef DOT11_VHT_AC
	if (IS_VHT_STA(pMacEntry) && (Status == 0))
	...
	*(USHORT *) (&ADDframe.BaParm) = cpu2le16(*(USHORT *) (&ADDframe.BaParm));
	...
	MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(FRAME_ADDBA_RSP), &ADDframe, END_OF_ARGS);
...
VOID PeerDelBAAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (PeerDelBAActionSanity(pAd, Elem->Wcid, Elem->Msg, Elem->MsgLen)) {
		...
		if (pDelFrame->DelbaParm.Initiator == ORIGINATOR) {
			...
			BAOriSessionTearDown(pAd, Elem->Wcid, pDelFrame->DelbaParm.TID, TRUE,
					     FALSE);
		}
	}
}
...
VOID SendSMPSAction(RTMP_ADAPTER *pAd, UCHAR Wcid, UCHAR smps)
{
	...
	MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(FRAME_SMPS_ACTION), &Frame, END_OF_ARGS);
	MiniportMMRequest(pAd, QID_AC_BE, pOutBuffer, FrameLen);
	MlmeFreeMemory(pAd, pOutBuffer);
...
VOID SendBeaconRequest(RTMP_ADAPTER *pAd, UCHAR Wcid)
{
	...
	ActHeaderInit(pAd, &Frame.Hdr, pAd->MacTab.Content[Wcid].Addr,
		      pAd->ApCfg.MBSSID[apidx].wdev.if_addr, pAd->ApCfg.MBSSID[apidx].wdev.bssid);
...
static VOID ba_enqueue_reordering_packet(IN RTMP_ADAPTER *pAd,
...
#ifdef HDR_TRANS_SUPPORT
static VOID ba_enqueue_reordering_packet_hdr_trns(IN PRTMP_ADAPTER pAd,
...
VOID Indicate_AMPDU_Packet(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk, UCHAR FromWhichBSSID)
{
	...
	if (SEQ_STEPONE(Sequence, pBAEntry->LastIndSeq, MAXSEQ)) {
		...
		INDICATE_LEGACY_OR_AMSDU(pAd, pRxBlk, FromWhichBSSID);
		LastIndSeq =
		    ba_indicate_reordering_mpdus_in_order(pAd, pBAEntry, pBAEntry->LastIndSeq);
	...
	/* IV. Receive Sequence within Window Size */
		 ...
		 (Sequence, (((pBAEntry->LastIndSeq + pBAEntry->BAWinSize + 1)) & MAXSEQ),
		  MAXSEQ)) {
...
#ifdef PEER_DELBA_TX_ADAPT
VOID Peer_DelBA_Tx_Adapt_Init(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry)
{
