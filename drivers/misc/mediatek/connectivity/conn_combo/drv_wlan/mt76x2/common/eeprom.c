...
INT RtmpChipOpsEepromHook(IN RTMP_ADAPTER *pAd, IN INT infType)
{
	...
	e2p_type = ((e2p_type != 0)
		    && (e2p_type < NUM_OF_E2P_MODE)) ? e2p_type : RtmpEepromGetDefault(pAd);
...
INT rtmp_ee_load_from_bin(IN PRTMP_ADAPTER pAd)
{
	...
	if (src && *src) {
		...
		if (IS_FILE_OPEN_ERR(srcf)) {
			...
			ret_val =
			    RtmpOSFileRead(srcf, (PSTRING) pAd->EEPROMImage,
...
INT rtmp_ee_load_from_default(IN PRTMP_ADAPTER pAd)
{
	INT result = TRUE;
	...
	DBGPRINT(RT_DEBUG_OFF, ("Load setting from default BIN\n"));
