...
/*
	...
	========================================================================
*/
static ULONG RTMPTkipGetUInt32(IN PUCHAR pMICKey)
{
	ULONG res = 0;
	INT i;
...
/*
	...
	========================================================================
*/
static VOID RTMPTkipPutUInt32(IN OUT PUCHAR pDst, IN ULONG val)
{
	INT i;
...
/*
	...
	========================================================================
*/
static VOID RTMPTkipSetMICKey(IN PTKIP_KEY_INFO pTkip, IN PUCHAR pMICKey)
{
	/* Set the key */
			   ...
			   IN PUCHAR pEncap, IN PCIPHER_KEY pKey, IN UCHAR apidx)
{
	...
	if (apidx >= MIN_NET_DEVICE_FOR_APCLI && ((apidx - MIN_NET_DEVICE_FOR_APCLI) < MAX_APCLI_NUM) && (pAd->OpMode == OPMODE_AP)) {	/* For packet which need to do MATConvert, we need to use the CurrentAddress of specific ApCli interface. */
		RTMPInitMICEngine(pAd,
				  pKey->Key,
				  pSrc,
				  pAd->ApCfg.ApCliTab[apidx -
						      MIN_NET_DEVICE_FOR_APCLI].wdev.if_addr,
				  UserPriority, pKey->TxMic);
	} else
#endif /* APCLI_SUPPORT */
		...
		RTMPInitMICEngine(pAd,
				  ...
				  pSrc + 6, UserPriority, pKey->TxMic);
	} else
	...
	{
		RTMPInitMICEngine(pAd, pKey->Key, pSrc, pSrc + 6, UserPriority, pKey->TxMic);
	}
...
static UINT tkip_sbox(UINT index)
{
	UINT index_low;
	UINT index_high;
	UINT left, right;
...
static VOID RTMPTkipMixKey(UCHAR *key, UCHAR *ta, ULONG pnl,	/* Least significant 16 bits of PN */
			   ...
			   UCHAR *rc4key, UINT *p1k)
{
	...
	for (i = 0; i < 8; i++) {
		j = 2 * (i & 1);
		p1k[0] =
		    (p1k[0] + tkip_sbox((p1k[4] ^ ((256 * key[1 + j]) + key[j])) % 65536)) % 65536;
		p1k[1] =
		    (p1k[1] +
		     tkip_sbox((p1k[0] ^ ((256 * key[5 + j]) + key[4 + j])) % 65536)) % 65536;
		p1k[2] =
		    (p1k[2] +
		     tkip_sbox((p1k[1] ^ ((256 * key[9 + j]) + key[8 + j])) % 65536)) % 65536;
		p1k[3] =
		    (p1k[3] +
		     tkip_sbox((p1k[2] ^ ((256 * key[13 + j]) + key[12 + j])) % 65536)) % 65536;
		p1k[4] =
		    (p1k[4] +
		     tkip_sbox((p1k[3] ^ (((256 * key[1 + j]) + key[j]))) % 65536)) % 65536;
		p1k[4] = (p1k[4] + i) % 65536;
	}
	...
	if (!NdisEqualMemory(MIC, TrailMIC, LEN_TKIP_MIC)) {
		...
		if (pAd->StaCfg.wpa_supplicant_info.WpaSupplicantUP) {
			WpaSendMicFailureToWpaSupplicant(pAd->net_dev, pFrame->Addr2,
							 ...
							 0
							 /*key id need be retrived by IV, actually supplicant didn't need it! */
							 , NULL);
		} else
