...
{
	StateMachineInit(S, (STATE_MACHINE_FUNC *) Trans, MAX_ACT_STATE, MAX_ACT_MSG,
			 (STATE_MACHINE_FUNC) Drop, ACT_IDLE, ACT_MACHINE_BASE);
...
VOID MlmeADDBAAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if (MlmeAddBAReqSanity(pAd, Elem->Msg, Elem->MsgLen, Addr) && VALID_WCID(pInfo->Wcid)) {
		...
		if (IS_ENTRY_APCLI(pEntry) && (pEntry->bReptCli == TRUE)) {
			...
			ActHeaderInit(pAd, &Frame.Hdr, pInfo->pAddr,
				      pAd->ApCfg.ApCliTab[idx].RepeaterCli[pEntry->MatchReptCliIdx].
				      CurrentAddress, pInfo->pAddr);
		} else
...
#ifdef UNALIGNMENT_SUPPORT
		{
			...
			NdisMoveMemory((PUCHAR) (&tmpBaParm), (PUCHAR) (&Frame.BaParm),
				       sizeof(BA_PARM));
			*(USHORT *) (&tmpBaParm) = cpu2le16(*(USHORT *) (&tmpBaParm));
			NdisMoveMemory((PUCHAR) (&Frame.BaParm), (PUCHAR) (&tmpBaParm),
				       sizeof(BA_PARM));
		}
#else
		*(USHORT *) (&(Frame.BaParm)) = cpu2le16((*(USHORT *) (&(Frame.BaParm))));
		...
		if ((pAd->Mlme.bStartMcc == TRUE)
		    &&
		    (NdisEqualMemory
		     (wdev->if_addr, pAd->cfg80211_ctrl.P2PCurrentAddress, MAC_ADDR_LEN))) {
			MiniportMMRequest(pAd, QID_HCCA, pOutBuffer, FrameLen);
		} else
#endif /* RT_CFG80211_SUPPORT */
#endif /* CONFIG_MULTI_CHANNEL */
			MiniportMMRequest(pAd, (MGMT_USE_QUEUE_FLAG | WMM_UP2AC_MAP[pInfo->TID]),
					  pOutBuffer, FrameLen);
...
VOID MlmeDELBAAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if (MlmeDelBAReqSanity(pAd, Elem->Msg, Elem->MsgLen) && VALID_WCID(pInfo->Wcid)) {
				  ...
				  IN UCHAR Wcid, IN UCHAR apidx, IN UCHAR InfoReq)
{
	...
	if (NStatus != NDIS_STATUS_SUCCESS) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("ACT - SendBSS2040CoexistMgmtAction() allocate memory failed\n"));
		return;
	}
	...
	MakeOutgoingFrame(pOutBuffer, &FrameLen,
			  ...
			  sizeof(BSS_2040_INTOLERANT_CH_REPORT), &BssIntolerantInfo, END_OF_ARGS);

#ifdef CONFIG_MULTI_CHANNEL
	if ((pAd->Mlme.bStartMcc == TRUE)
	    && (NdisEqualMemory(wdev->if_addr, pAd->cfg80211_ctrl.P2PCurrentAddress, MAC_ADDR_LEN))) {
		MiniportMMRequest(pAd, QID_HCCA, pOutBuffer, FrameLen);
	} else
#endif /* CONFIG_MULTI_CHANNEL */
		MiniportMMRequest(pAd, QID_AC_BE, pOutBuffer, FrameLen);
...
VOID StaPublicAction(RTMP_ADAPTER *pAd, BSS_2040_COEXIST_IE *pBssCoexIE)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("ACTION - StaPeerPublicAction  Bss2040Coexist = %x\n", *((PUCHAR) pBssCoexIE)));
...
VOID UpdateBssScanParm(IN PRTMP_ADAPTER pAd, IN OVERLAP_BSS_SCAN_IE APBssScan)
{
	...
	for (k = 0; k < 7; k++) {
		...
		for (i = 0; i < pAd->CommonCfg.TriggerEventTab.EventANo; i++) {
			if (pAd->CommonCfg.TriggerEventTab.EventA[i].bValid == TRUE) {
				if (pAd->CommonCfg.TriggerEventTab.EventA[i].RegClass ==
				    TmpRegClass) {
					for (j = 0; j < idx; j++) {
						if (ChannelList[j] ==
						    (UCHAR) pAd->CommonCfg.TriggerEventTab.
						    EventA[i].Channel)
							break;
					}
					if ((j == idx)) {
...
/*
	...
	==========================================================================
 */
VOID Update2040CoexistFrameAndNotify(IN PRTMP_ADAPTER pAd,
...
VOID Send2040CoexistAction(IN RTMP_ADAPTER *pAd, IN UCHAR Wcid, IN BOOLEAN bAddIntolerantCha)
{
...
#endif /* APCLI_SUPPORT */
		ActHeaderInit(pAd, &Frame.Hdr, pAd->MacTab.Content[Wcid].Addr, pAd->CurrentAddress,
			      pAd->CommonCfg.Bssid);
	...
	MakeOutgoingFrame(pOutBuffer, &FrameLen,
			  sizeof(FRAME_ACTION_HDR), &Frame,
			  1, &BssCoexistIe,
			  1, &HtLen, 1, &pAd->CommonCfg.BSSCoexist2040.word, END_OF_ARGS);
...
BOOLEAN ChannelSwitchSanityCheck(IN PRTMP_ADAPTER pAd,
...
VOID ChannelSwitchAction(IN PRTMP_ADAPTER pAd,
			 IN UCHAR Wcid, IN UCHAR NewChannel, IN UCHAR Secondary)
{
	...
	if (rf_channel != 0) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s(): %dMHz LINK UP, CtrlChannel=%d,  CentralChannel= %d\n",
			  __func__, (rf_bw == BW_40 ? 40 : 20), pAd->CommonCfg.Channel,
...
VOID PeerPublicAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	case ACTION_BSS_2040_COEXIST:	/* Format defined in IEEE 7.4.7a.1 in 11n Draf3.03 */
		{
			...
			if (Elem->MsgLen >=
			    (LENGTH_802_11 + sizeof(BSS_2040_COEXIST_ELEMENT) +
			     sizeof(BSS_2040_INTOLERANT_CH_REPORT))) {
				pIntolerantReport =
				    (BSS_2040_INTOLERANT_CH_REPORT *) ((PUCHAR) pCoexistInfo +
								       sizeof
			...
			IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
			{
				...
				if (!IS_ENTRY_APCLI(&pAd->MacTab.Content[Elem->Wcid])) {
					...
					if ((pBssCoexistIe->field.BSS20WidthReq == 1)
					...
					} else if ((pIntolerantReport)
						   && (pIntolerantReport->Len > 1)
						...
						for (i = 0; i < (pIntolerantReport->Len - 1);
						     i++, ptr++) {
						...
						if (retVal == TRUE) {
							...
							DBGPRINT(RT_DEBUG_TRACE,
								 ("Check IntolerantReport Channel List in our effectedChList(%d~%d)\n",
								  pAd->ChannelList
								  [coexChRange.effectChStart].
								  Channel,
								  pAd->ChannelList[coexChRange.
										   effectChEnd].
								  Channel));
							for (i = 0;
							     i < (pIntolerantReport->Len - 1);
							     i++, ptr++) {
								UCHAR chEntry;
					...
					if (bNeedFallBack) {
						...
						for (apidx = 0; apidx < pAd->ApCfg.BssidNum;
						     apidx++)
							SendBSS2040CoexistMgmtAction(pAd,
										     MCAST_WCID,
										     apidx, 0);
					}
#ifdef APCLI_SUPPORT
	...
	case ACTION_WIFI_DIRECT:
		...
		if ((pAd->StaCfg.BssType == BSS_ADHOC) &&
			...
			MlmeEnqueue(pAd, IWSC_STATE_MACHINE, IWSC_MT2_PEER_ACTION_FRAME,
				    Elem->MsgLen, Elem->Msg, 0);
			RTMP_MLME_HANDLER(pAd);
			break;
		}
#endif /* IWSC_SUPPORT // */
...
VOID SendNotifyBWActionFrame(RTMP_ADAPTER *pAd, UCHAR Wcid, UCHAR apidx)
{
	...
	MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(FRAME_ACTION_HDR), &Frame, END_OF_ARGS);
	...
	case ACT_VHT_OPMODE_NOTIFY:
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("\t RxNssType=%d, RxNss=%d, ChBW=%d\n",
						  op_mode->rx_nss_type, op_mode->rx_nss,
						  op_mode->ch_width));
...
VOID SendRefreshBAR(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	for (i = 0; i < NUM_OF_TID; i++) {
		...
		if (pBAEntry->ORI_BA_Status == Originator_Done) {
			...
			if (IS_ENTRY_APCLI(pEntry) && pEntry->bReptCli)
				BarHeaderInit(pAd, &FrameBar, pEntry->Addr,
					      pAd->ApCfg.ApCliTab[pEntry->wdev_idx].
					      RepeaterCli[pEntry->MatchReptCliIdx].CurrentAddress);
			else
#endif /* MAC_REPEATER_SUPPORT */
#endif /* APCLI_SUPPORT */
			...
			if ((pAd->Mlme.bStartMcc == TRUE)
			    &&
			    (NdisEqualMemory
			     (pEntry->wdev->if_addr, pAd->cfg80211_ctrl.P2PCurrentAddress,
			      MAC_ADDR_LEN))) {
				MiniportMMRequest(pAd, QID_HCCA, pOutBuffer, FrameLen);
			} else
#endif /* RT_CFG80211_SUPPORT */
...
VOID ActHeaderInit(IN RTMP_ADAPTER *pAd,
		   IN OUT PHEADER_802_11 pHdr80211, IN UCHAR *da, IN UCHAR *sa, IN UCHAR *bssid)
{
...
VOID BarHeaderInit(IN PRTMP_ADAPTER pAd, IN OUT PFRAME_BAR pCntlBar, IN PUCHAR pDA, IN PUCHAR pSA)
{
/*	USHORT	Duration;*/
...
VOID InsertActField(IN PRTMP_ADAPTER pAd,
		    OUT PUCHAR pFrameBuf, OUT PULONG pFrameLen, IN UINT8 Category, IN UINT8 ActCode)
{
	ULONG TempLen;
