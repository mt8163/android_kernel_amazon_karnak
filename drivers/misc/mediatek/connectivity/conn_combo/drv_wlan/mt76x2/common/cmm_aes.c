...
static VOID xor_32(IN PUCHAR a, IN PUCHAR b, OUT PUCHAR out)
{
	INT i;
...
static VOID xor_128(IN PUCHAR a, IN PUCHAR b, OUT PUCHAR out)
{
	INT i;
...
static UCHAR RTMPCkipSbox(IN UCHAR a)
{
	return SboxTable[(int)a];
}

static VOID next_key(IN PUCHAR key, IN INT round)
{
	UCHAR rcon;
	UCHAR sbox_key[4];
	UCHAR rcon_table[12] = {
...
static VOID byte_sub(IN PUCHAR in, OUT PUCHAR out)
{
	INT i;
...
static VOID shift_row(IN PUCHAR in, OUT PUCHAR out)
{
	out[0] = in[0];
	out[1] = in[5];
	out[2] = in[10];
...
static VOID mix_column(IN PUCHAR in, OUT PUCHAR out)
{
	INT i;
	UCHAR add1b[4];
	UCHAR add1bf7[4];
...
static void construct_mic_header1(unsigned char *mic_header1,
...
static void construct_mic_header2(unsigned char *mic_header2,
...
static void construct_mic_iv(unsigned char *mic_iv,
			     int qc_exists,
			     int a4_exists,
			     unsigned char *mpdu,
...
static void construct_ctr_preload(unsigned char *ctr_preload,
				  int a4_exists,
				  int qc_exists,
			   ...
			   IN PUCHAR pData, IN ULONG DataByteCnt, IN PCIPHER_KEY pWpaKey)
{
	...
	for (i = 0; i < num_blocks; i++) {
		construct_ctr_preload(ctr_preload, a4_exists, qc_exists, pData, PN, i + 1);
	...
	if (payload_remainder > 0) {
		construct_ctr_preload(ctr_preload, a4_exists, qc_exists, pData, PN, num_blocks + 1);
...
/*
	...
	========================================================================
*/
static VOID RTMPConstructCCMPAAD(IN PUCHAR pHdr,
				 IN BOOLEAN isDataFrame,
				 IN UINT8 a4_exists,
				 IN UINT8 qc_exists, OUT UCHAR *aad_hdr, OUT UINT *aad_len)
{
	UINT len = 0;
...
/*
	...
	========================================================================
*/
static VOID RTMPConstructCCMPNonce(IN PUCHAR pHdr,
				   IN UINT8 a4_exists,
				   IN UINT8 qc_exists,
				   IN BOOLEAN isMgmtFrame,
				   IN UCHAR *pn, OUT UCHAR *nonce_hdr, OUT UINT *nonce_hdr_len)
{
	UINT n_offset = 0;
	INT i;
			    ...
			    IN PUCHAR pIV, IN PUCHAR pKey, INOUT PUCHAR pData, IN UINT32 DataLen)
{
	...
	RTMPConstructCCMPAAD(pHdr,
			     (frame_type == FC_TYPE_DATA), a4_exists, qc_exists, aad_hdr, &aad_len);

	/* Construct NONCE header */
	RTMPConstructCCMPNonce(pHdr,
			       a4_exists,
			       qc_exists,
			       (frame_type == FC_TYPE_MGMT), pIV, nonce_hdr, &nonce_hdr_len);
			    ...
			    IN PCIPHER_KEY pKey, INOUT PUCHAR pData, INOUT UINT16 *DataLen)
{
	...
	RTMPConstructCCMPNonce(pHdr,
			       a4_exists,
			       qc_exists,
			       (frame_type == FC_TYPE_MGMT), pn, nonce_hdr, &nonce_hdr_len);
...
VOID CCMP_test_vector(IN PRTMP_ADAPTER pAd, IN INT input)
{
	...
	if (AES_CCM_Encrypt(res_buf, sizeof(P_TEXT_DATA),
			    ...
			    AAD, sizeof(AAD), 8, res_buf, &res_len) == 0) {
