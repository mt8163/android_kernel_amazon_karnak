...
VOID dump_vht_cap(RTMP_ADAPTER *pAd, VHT_CAP_IE *vht_ie)
{
	...
	DBGPRINT(RT_DEBUG_OFF,
		 ("\tCompressedSteeringNumOfBeamformerAnt=%d\n", vht_cap->cmp_st_num_bfer));
	DBGPRINT(RT_DEBUG_OFF,
		 ("\tNumber of Sounding Dimensions=%d\n", vht_cap->num_snd_dimension));
...
UCHAR vht_cent_ch_freq(RTMP_ADAPTER *pAd, UCHAR prim_ch)
{
	...
	if (pAd->CommonCfg.vht_bw < VHT_BW_80 || prim_ch < 36) {
		...
		DBGPRINT(RT_DEBUG_TRACE, ("%s:%d, set vht_cent_ch=0\n", __func__, __LINE__));
		return prim_ch;
	}

	while (vht_ch_80M[idx].ch_up_bnd != 0) {
		if (prim_ch >= vht_ch_80M[idx].ch_low_bnd && prim_ch <= vht_ch_80M[idx].ch_up_bnd) {
...
*/
static INT build_vht_op_ie(RTMP_ADAPTER *pAd, UCHAR *buf)
{
	VHT_OP_IE vht_op;
	UCHAR cent_ch;
#ifdef RT_BIG_ENDIAN
	...
	if (pAd->CommonCfg.Channel > 14 &&
	    (pAd->CommonCfg.bIEEE80211H == 1) && (pAd->Dot11_H.RDMode == RD_SWITCHING_MODE))
		...
		if (IS_MT76x2(pAd)) {
			vht_op.basic_mcs_set.mcs_ss2 = (((pAd->CommonCfg.vht_bw == VHT_BW_2040)
							 && (pAd->CommonCfg.RegTransmitSetting.
							     ...
							     BW_20)) ? VHT_MCS_CAP_8 :
							VHT_MCS_CAP_9);
			printk("2SS vht_op.basic_mcs_set.mcs_ss2: %d",
			       vht_op.basic_mcs_set.mcs_ss2);
		} else
#endif /* MT76x2 */
...
INT build_vht_cap_ie(RTMP_ADAPTER *pAd, UCHAR *buf)
{
	...
	if (SIGMA_ON(pAd)) {
		DBGPRINT(RT_DEBUG_TRACE, ("[%s] Enable TXBF capability for VHT IE\n", __func__));
		pAd->BeaconSndDimensionFlag = 0;
	}
#endif /* CERTIFICATION_SIGMA_SUPPORT */

	if ((pAd->chipCap.FlgHwTxBfCap) && (pAd->BeaconSndDimensionFlag == 0)) {
		vht_cap_ie.vht_cap.num_snd_dimension =
		    pAd->CommonCfg.vht_cap_ie.vht_cap.num_snd_dimension;
		vht_cap_ie.vht_cap.cmp_st_num_bfer =
		    pAd->CommonCfg.vht_cap_ie.vht_cap.cmp_st_num_bfer;
...
BOOLEAN vht80_channel_group(RTMP_ADAPTER *pAd, UCHAR channel)
{
	...
	while (vht_ch_80M[idx].ch_up_bnd != 0) {
		if (channel >= vht_ch_80M[idx].ch_low_bnd && channel <= vht_ch_80M[idx].ch_up_bnd) {
