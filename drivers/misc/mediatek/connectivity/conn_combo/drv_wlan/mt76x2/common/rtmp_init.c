...
#ifdef WCX_SUPPORT
		if (pAd->nvramValid == FALSE) {
			if (TRUE == pAd->chipOps.eeinit(pAd))
				pAd->nvramValid = TRUE;
#if !defined(CONFIG_IDME)
			else if (pAd->E2pAccessMode == E2P_BIN_MODE) {
				DBGPRINT(RT_DEBUG_ERROR, ("NVRAM not valid\n"));
				rtmp_ee_load_from_default(pAd);
			}
#endif /* !CONFIG_IDME */
		}
#else
...
#ifdef CONFIG_IDME
	/* Case1: Get WIFI MFG from idme , IDME success -> IDME
	   Case2: IDME fail, read nvram success -> NVRAM
	   Case3: IDME/NVRAM fail, use driver default value
	*/
	if (idme_get_wifi_mfg(pAd) && pAd->nvramValid == FALSE) {
		DBGPRINT(RT_DEBUG_ERROR, ("IDME WIFI CFG read fail.& NVRAM not valid\n"));
		rtmp_ee_load_from_default(pAd);
	} else
		DBGPRINT(RT_DEBUG_TRACE, ("Either Load IDME or NVRAM successful!\n"));
	/* 2. Write Mac address from idme, don't change sequence */
	idme_get_mac_addr(pAd);
#endif /* CONFIG_IDME */
...
VOID RTMPSwReset(RTMP_ADAPTER *pAd)
{
	...
	UINT32 MacReg = 0;
	UINT32 count = 0;
#define WAIT_EMPTY_RETRY_COUNT		(100)
#define WAIT_EMPTY_DELAY_TIME_US	(1000)	/* in micro-second */
	...
	RTMP_SET_SUSPEND_FLAG(pAd, fRTMP_ADAPTER_SUSPEND_STATE_SW_RESET);
	...
	MacReg = 0;
	write_reg(pAd, 0x41, MAC_SYS_CTRL, MacReg);
	...
	RTMP_IO_READ32(pAd, 0x9014, &MacReg);
	MacReg = (MacReg | (BIT6));
	RTMP_IO_WRITE32(pAd, 0x9014, MacReg);
	RtmpOsMsDelay(15);
	...
	RTMP_IO_WRITE32(pAd, 0x64, MacReg);
	RtmpOsMsDelay(15);
	RTMP_IO_READ32(pAd, 0x64, &MacReg);
	MacReg = (MacReg & ~(BIT21 | BIT22));
	RTMP_IO_WRITE32(pAd, 0x64, MacReg);
	...
	read_reg(pAd, 0x41, 0x400, &MacReg);
	MacReg |= (BIT2 | BIT3);
	write_reg(pAd, 0x41, 0x400, MacReg);
	RtmpOsMsDelay(15);
	read_reg(pAd, 0x41, 0x400, &MacReg);
	MacReg &= ~(BIT2 | BIT3);
	write_reg(pAd, 0x41, 0x400, MacReg);
	...
	/*
	   11. Driver restarts all Bulk OUT URBs
	 */
	DBGPRINT(RT_DEBUG_ERROR, ("%s: Enable Bulkout\n", __func__));
	RTMP_CLEAR_FLAG(pAd, fRTMP_ADAPTER_IDLE_RADIO_OFF);

	/*
	   12. Driver reloads FW (Without WLAN reset (0x40_0064, bit[19]))
	 */
	DBGPRINT(RT_DEBUG_ERROR, ("%s: Reload WiFi Firmware\n", __func__));
	NICLoadFirmware(pAd);
	/* After this, we need to send EP8 cmd, so we clear this state */
	RTMP_CLEAR_SUSPEND_FLAG(pAd, fRTMP_ADAPTER_SUSPEND_STATE_SW_RESET);
