...
*/
	...
	for (RoundIndex = 1; RoundIndex < NumberOfRound; RoundIndex++) {
		...
		for (RowIndex = 0; RowIndex < AES_STATE_ROWS; RowIndex++)
			for (ColumnIndex = 0; ColumnIndex < AES_STATE_COLUMNS; ColumnIndex++)
				paes_ctx->State[RowIndex][ColumnIndex] ^=
				    paes_ctx->KeyWordExpansion[RowIndex][(RoundIndex * ((UINT)
											AES_STATE_COLUMNS))
									 + ColumnIndex];
	}
		    ...
		    IN UINT MACLength, OUT UINT8 CipherText[], INOUT UINT *CipherTextLength)
{
	 ...
	 */
	AES_CCM_MAC(PlainText, PlainTextLength, Key, KeyLength, Nonce, NonceLength, AAD, AADLength,
		    MACLength, Block_MAC);
	...
	while (Cipher_Index < PlainTextLength) {
		Block_CTR[15] += 1;
		Temp_Length = sizeof(Block_CTR_Cipher);
		RT_AES_Encrypt(Block_CTR, AES_BLOCK_SIZES, Key, KeyLength, Block_CTR_Cipher,
			       &Temp_Length);
		...
		if (Copy_Length > AES_BLOCK_SIZES)
			Copy_Length = AES_BLOCK_SIZES;
		for (Temp_Index = 0; Temp_Index < Copy_Length; Temp_Index++)
			CipherText[Cipher_Index + Temp_Index] =
			    PlainText[Cipher_Index + Temp_Index] ^ Block_CTR_Cipher[Temp_Index];
...
{
	UINT8 Block_MAC[AES_BLOCK_SIZES], Block_MAC_From_Cipher[AES_BLOCK_SIZES] = { 0 };
	UINT8 Block_CTR[AES_BLOCK_SIZES], Block_CTR_Cipher[AES_BLOCK_SIZES];
	 ...
	 */
	Block_Index = 0;
	for (Temp_Index = (CipherTextLength - MACLength); Temp_Index < CipherTextLength;
	     Temp_Index++, Block_Index++)
		Block_MAC_From_Cipher[Block_Index] =
		    CipherText[Temp_Index] ^ Block_CTR_Cipher[Block_Index];
	...
	while (Cipher_Index < (CipherTextLength - MACLength)) {
		...
		for (Temp_Index = 0; Temp_Index < Copy_Length; Temp_Index++)
			PlainText[Cipher_Index + Temp_Index] =
			    CipherText[Cipher_Index + Temp_Index] ^ Block_CTR_Cipher[Temp_Index];
		Cipher_Index += Copy_Length;
	}
		     ...
		     IN UINT IVLength, OUT UINT8 CipherText[], INOUT UINT *CipherTextLength)
{
			...
			for (Index = 0; Index < AES_BLOCK_SIZES; Index++)
				Block[Index] =
				    PlainText[PlainBlockStart +
					      Index] ^ CipherText[CipherBlockStart -
								  ((UINT) AES_BLOCK_SIZES) + Index];
		}

		CipherBlockSize = *CipherTextLength - CipherBlockStart;
		RT_AES_Encrypt(Block, AES_BLOCK_SIZES, Key, KeyLength,
			       CipherText + CipherBlockStart, &CipherBlockSize);
	...
	NdisMoveMemory(Block, (&PlainText[0] + PlainBlockStart),
		       (PlainTextLength - PlainBlockStart));
	NdisFillMemory((Block + (((UINT) AES_BLOCK_SIZES) - PaddingSize)), PaddingSize,
		       (UINT8) PaddingSize);
	if (CipherBlockStart == 0) {
		for (Index = 0; Index < AES_BLOCK_SIZES; Index++)
		...
		for (Index = 0; Index < AES_BLOCK_SIZES; Index++)
			Block[Index] ^=
			    CipherText[CipherBlockStart - ((UINT) AES_BLOCK_SIZES) + Index];
	}
	CipherBlockSize = *CipherTextLength - CipherBlockStart;
	RT_AES_Encrypt(Block, AES_BLOCK_SIZES, Key, KeyLength, CipherText + CipherBlockStart,
		       &CipherBlockSize);
	CipherBlockStart += CipherBlockSize;
		     ...
		     IN UINT IVLength, OUT UINT8 PlainText[], INOUT UINT *PlainTextLength)
{
	...
	while ((CipherTextLength - CipherBlockStart) >= AES_BLOCK_SIZES) {
		...
		RT_AES_Decrypt(CipherText + CipherBlockStart, AES_BLOCK_SIZES, Key, KeyLength,
			       PlainText + PlainBlockStart, &PlainBlockSize);
		...
		if (PlainBlockStart == 0) {
			...
			for (Index = 0; Index < AES_BLOCK_SIZES; Index++)
				PlainText[PlainBlockStart + Index] ^=
				    CipherText[CipherBlockStart + Index - ((UINT) AES_BLOCK_SIZES)];
		}
...
*/
	if (pResult == NULL) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("AES_Key_Wrap: allocate %zu bytes memory failure.\n",
	...
	for (Index_j = 0; Index_j < 6; Index_j++) {
		for (Index_i = 0; Index_i < Number_Of_Block; Index_i++) {
			...
			RT_AES_Encrypt(Block_Input, AES_BLOCK_SIZES, Key, KeyLength, Block_B,
				       &Temp_Length);
...
*/
	if (pResult == NULL) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("AES_Key_Unwrap: allocate %zu bytes memory failure.\n",
	...
	for (Index_j = 5; Index_j >= 0; Index_j--) {
		for (Index_i = (Number_Of_Block - 1); Index_i >= 0; Index_i--) {
			...
			NdisMoveMemory(Block_Input + 8, pResult + (Index_i * 8), 8);
			Temp_Length = sizeof(Block_B);
			RT_AES_Decrypt(Block_Input, AES_BLOCK_SIZES, Key, KeyLength, Block_B,
				       &Temp_Length);
