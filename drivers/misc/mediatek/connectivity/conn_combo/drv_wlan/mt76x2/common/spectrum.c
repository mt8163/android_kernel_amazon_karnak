...
UINT8 GetRegulatoryMaxTxPwr(IN PRTMP_ADAPTER pAd, IN UINT8 channel)
{
	...
	for (RegulatoryClassLoop = 0;
	     RegulatoryClassLoop < MAX_NUM_OF_REGULATORY_CLASS; RegulatoryClassLoop++) {
		...
		if (RegulatoryClass >= MaxRegulatoryClassNum) {
			DBGPRINT(RT_DEBUG_ERROR, ("%s: %c%c Unknow Requlatory class (%d)\n",
						  __func__, pCountry[0], pCountry[1],
						  RegulatoryClass));
			return 0xff;
		}
			...
			OUT PULONG pFrameLen, IN PSTRING pCountry, IN UINT8 RegulatoryClass)
{
	...
	if (Len > 1) {
		MakeOutgoingFrame(pFrameBuf, &TempLen,
				  1, &IEId,
				  1, &Len, 1, &RegulatoryClass, Len - 1, pChListPtr, END_OF_ARGS);
...
/*
	...
	==========================================================================
 */
static VOID InsertTpcReqIE(IN PRTMP_ADAPTER pAd, OUT PUCHAR pFrameBuf, OUT PULONG pFrameLen)
{
	ULONG TempLen;
	UINT8 Len = 0;
	UINT8 ElementID = IE_TPC_REQUEST;

	MakeOutgoingFrame(pFrameBuf, &TempLen, 1, &ElementID, 1, &Len, END_OF_ARGS);
		       ...
		       OUT PULONG pFrameLen, IN UINT8 TxPwr, IN UINT8 LinkMargin)
{
	...
	MakeOutgoingFrame(pFrameBuf, &TempLen,
			  1, &ElementID, 1, &Len, Len, &TpcReportIE, END_OF_ARGS);
			       ...
			       IN UINT8 Len, IN PMEASURE_REQ_INFO pMeasureReqIE)
{
	...
	MakeOutgoingFrame(pFrameBuf, &TempLen,
			  1, &ElementID,
			  1, &Len, sizeof(MEASURE_REQ_INFO), pMeasureReqIE, END_OF_ARGS);
				  ...
				  IN UINT8 ReportLnfoLen, IN PUINT8 pReportInfo)
{
	...
	MakeOutgoingFrame(pFrameBuf, &TempLen,
			  1, &ElementID, 1, &Len, Len, pMeasureReportIE, END_OF_ARGS);
			   ...
			   IN UINT8 MeasureReqType, IN UINT8 ReportInfoLen, IN PUINT8 pReportInfo)
{
	...
	InsertMeasureReportIE(pAd, (pOutBuffer + FrameLen), &FrameLen, &MeasureRepIE, ReportInfoLen,
			      pReportInfo);
...
static VOID InsertChSwAnnIE(IN PRTMP_ADAPTER pAd,
			    OUT PUCHAR pFrameBuf,
			    OUT PULONG pFrameLen,
			    IN UINT8 ChSwMode, IN UINT8 NewChannel, IN UINT8 ChSwCnt)
{
	ULONG TempLen;
...
VOID EnqueueChSwAnn(IN PRTMP_ADAPTER pAd, IN PUCHAR pDA, IN UINT8 ChSwMode, IN UINT8 NewCh)
{
	...
	InsertActField(pAd, (pOutBuffer + FrameLen), &FrameLen, CATEGORY_SPECTRUM,
		       SPEC_CHANNEL_SWITCH);
...
static VOID StartDFSProcedure(IN PRTMP_ADAPTER pAd, IN UCHAR Channel, IN UINT8 ChSwMode)
{
	/* start DFS procedure */
...
static BOOLEAN PeerChSwAnnSanity(IN PRTMP_ADAPTER pAd,
				 IN VOID *pMsg, IN ULONG MsgLen, OUT PCH_SW_ANN_INFO pChSwAnnInfo)
{
...
static BOOLEAN PeerTpcReqSanity(IN PRTMP_ADAPTER pAd,
				IN VOID *pMsg, IN ULONG MsgLen, OUT PUINT8 pDialogToken)
{
	PFRAME_802_11 Fr = (PFRAME_802_11) pMsg;
	PUCHAR pFramePtr = Fr->Octet;
	BOOLEAN result = FALSE;
...
static BOOLEAN PeerTpcRepSanity(IN PRTMP_ADAPTER pAd,
				IN VOID *pMsg,
				IN ULONG MsgLen,
				OUT PUINT8 pDialogToken, OUT PTPC_REPORT_INFO pTpcRepInfo)
{
	PFRAME_802_11 Fr = (PFRAME_802_11) pMsg;
	PUCHAR pFramePtr = Fr->Octet;
	BOOLEAN result = FALSE;
...
static VOID PeerChSwAnnAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	/* ChSwAnn need check. */
	if ((pAd->OpMode == OPMODE_AP) && (DfsRequirementCheck(pAd, ChSwAnnInfo.Channel) == TRUE)) {
		NotifyChSwAnnToPeerAPs(pAd, pFr->Hdr.Addr1, pFr->Hdr.Addr2, ChSwAnnInfo.ChSwMode,
				       ChSwAnnInfo.Channel);
	...
	if (pAd->OpMode == OPMODE_STA) {
		...
		ASSERT(Bssidx < ARRAY_SIZE(pAd->ScanTab.BssEntry));
		if (Bssidx >= ARRAY_SIZE(pAd->ScanTab.BssEntry)) {
			DBGPRINT(RT_DEBUG_ERROR, ("Bssidx is out of ScanTab.BssEntry[]\n"));
			return;
		}
...
/*
	...
	==========================================================================
 */
static VOID PeerMeasureReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	PFRAME_802_11 pFr = (PFRAME_802_11) Elem->Msg;
	UINT8 DialogToken;
	...
	if (PeerMeasureReqSanity
	    (pAd, Elem->Msg, Elem->MsgLen, &DialogToken, &MeasureReqInfo, &MeasureReq)) {
		ReportMode.word = 0;
		ReportMode.field.Incapable = 1;
		EnqueueMeasurementRep(pAd, pFr->Hdr.Addr2, DialogToken, MeasureReqInfo.Token,
				      ReportMode.word, MeasureReqInfo.ReqType, 0, NULL);
	}
...
/*
	...
	==========================================================================
 */
static VOID PeerMeasureReportAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
...
/*	if ((pMeasureReportInfo = kmalloc(sizeof(MEASURE_RPI_REPORT), GFP_ATOMIC)) == NULL)*/
	...
	if (PeerMeasureReportSanity
	    (pAd, Elem->Msg, Elem->MsgLen, &DialogToken, &MeasureReportInfo, pMeasureReportInfo)) {
		do {
			PMEASURE_REQ_ENTRY pEntry = NULL;
...
VOID PeerSpectrumAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	case SPEC_CHANNEL_SWITCH:

		{
			...
			RTMPMoveMemory(&ChannelSwitch, &Elem->Msg[LENGTH_802_11 + 4],
				       sizeof(CHA_SWITCH_ANNOUNCE_IE));

			/* 802.11n D3.03 adds secondary channel offset element in the end. */
			if (Elem->MsgLen ==
			    (LENGTH_802_11 + 2 + sizeof(CHA_SWITCH_ANNOUNCE_IE) +
			     sizeof(SEC_CHA_OFFSET_IE))) {
				RTMPMoveMemory(&Secondary, &Elem->Msg[LENGTH_802_11 + 9],
					       sizeof(SEC_CHA_OFFSET_IE));
			} else {
				Secondary.SecondaryChannelOffset = 0;
			}

			if ((Elem->Msg[LENGTH_802_11 + 2] == IE_CHANNEL_SWITCH_ANNOUNCEMENT)
			    && (Elem->Msg[LENGTH_802_11 + 3] == 3)) {
				ChannelSwitchAction(pAd, Elem->Wcid, ChannelSwitch.NewChannel,
...
INT Set_MeasureReq_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	MakeMeasurementReqFrame(pAd, pOutBuffer, &FrameLen,
				...
				MeasureReqToken, MeasureReqMode.word, MeasureReqType, 1);
...

	{
		...
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TempLen,
				  sizeof(MEASURE_REQ), &MeasureReq, END_OF_ARGS);
		FrameLen += TempLen;
	}
...
#define PWR_CONSTRAION_TAB_SIZE \
	...
	for (Idx = 0; Idx < PWR_CONSTRAION_TAB_SIZE; Idx++) {
		if (DaltaPwr < PwrConstrainTab[Idx].Attenuation) {
			...
			pAd->CommonCfg.TxPowerPercentage = PwrConstrainTab[Idx].TxPowerPercentage;
	...
	if (Idx == PWR_CONSTRAION_TAB_SIZE) {
		DBGPRINT(RT_DEBUG_ERROR, ("Power constraint value be in range from 0 to 13dB\n"));
	}
...
static PDOT11_REGULATORY_INFORMATION GetRugClassRegion(IN PSTRING pCountryCode, IN UINT8 RugClass)
{
...
VOID RguClass_BuildBcnChList(IN PRTMP_ADAPTER pAd, OUT PUCHAR pBuf, OUT PULONG pBufLen)
{
	...
	for (loop = 0; loop < MAX_NUM_OF_REGULATORY_CLASS; loop++) {
		...
		pRguClassRegion = GetRugClassRegion((PSTRING) pAd->CommonCfg.CountryCode,
		...
		MakeOutgoingFrame(pBuf + *pBufLen, &TmpLen,
				  ...
				  1, &pChList->MaxTxPwr, END_OF_ARGS);
