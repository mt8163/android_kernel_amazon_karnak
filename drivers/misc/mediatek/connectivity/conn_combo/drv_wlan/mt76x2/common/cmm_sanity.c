...
BOOLEAN PeerBeaconAndProbeRspSanity_Old(IN PRTMP_ADAPTER pAd,
					IN VOID *Msg,
					IN ULONG MsgLen,
					IN UCHAR MsgChannel,
					OUT PUCHAR pAddr2,
					...
					OUT HT_CAPABILITY_IE *pHtCapability, OUT EXT_CAP_INFO_ELEMENT *pExtCapInfo, OUT UCHAR *AddHtInfoLen, OUT ADD_HT_INFO_IE *AddHtInfo, OUT UCHAR *NewExtChannelOffset,	/* Ht extension channel offset(above or below) */
	...
	while ((Length + 2 + pEid->Len) <= MsgLen) {
		...
		case IE_ADD_HT:
			if (pEid->Len >= sizeof(ADD_HT_INFO_IE)) {
				...
				*(USHORT *) (&AddHtInfo->AddHtInfo2) =
				    cpu2le16(*(USHORT *) (&AddHtInfo->AddHtInfo2));
				*(USHORT *) (&AddHtInfo->AddHtInfo3) =
				    cpu2le16(*(USHORT *) (&AddHtInfo->AddHtInfo3));
		...
		case IE_TIM:
			if (SubType == SUBTYPE_BEACON) {
				...
				if (INFRA_ON(pAd)
				    && NdisEqualMemory(pBssid, pAd->CommonCfg.Bssid, MAC_ADDR_LEN)) {
					GetTimBit((PCHAR) pEid, pAd->StaActive.Aid, &TimLen,
						  pBcastFlag, pDtimCount, pDtimPeriod,
						  pMessageToMe);
				}
			}
			break;
		...
		case IE_VENDOR_SPECIFIC:
			...
			/* Check broadcom-defiend pre-802.11nD1.0 OUI for HT related IE, including HT Capatilities IE and HT Information IE */
				...
				if ((pEid->Octet[3] == OUI_BROADCOM_HT) && (pEid->Len >= 30)
				    && (*pHtCapabilityLen == 0)) {
					NdisMoveMemory(pHtCapability, &pEid->Octet[4],
						       sizeof(HT_CAPABILITY_IE));
					*pPreNHtCapabilityLen = SIZE_HT_CAP_IE;
				}

				if ((pEid->Octet[3] == OUI_PREN_ADD_HT) && (pEid->Len >= 26)) {
					NdisMoveMemory(AddHtInfo, &pEid->Octet[4],
		...
		if (bFoundIWscIe && (pAd->StaCfg.BssType == BSS_ADHOC)) {
			...
			if ((pWscCtrl->WscConfMode == WSC_ENROLLEE) &&
				...
				if ((*pSsidLen) <= 32 && (*pSsidLen) != 0) {
					...
					NdisMoveMemory(pWscCtrl->WscSsid.Ssid, Ssid,
		...
		case IE_OPERATING_MODE_NOTIFY:
			if (pEid->Len == sizeof(OPERATING_MODE)) {
				...
				if (op_mode.rx_nss_type == 0) {
					...
					NdisMoveMemory(&pEntry->operating_mode, &op_mode,
...
BOOLEAN PeerDlsTearDownSanity(IN PRTMP_ADAPTER pAd,
			      IN VOID *Msg,
			      IN ULONG MsgLen, OUT PUCHAR pDA, OUT PUCHAR pSA, OUT USHORT *pReason)
{
	CHAR *Ptr;
	PFRAME_802_11 Fr = (PFRAME_802_11) Msg;
