...
========================================================================
*/
static VOID UAPSD_InsertTailQueueAc(IN RTMP_ADAPTER *pAd,
				    IN MAC_TABLE_ENTRY *pEntry,
				    IN QUEUE_HEADER *pQueueHeader, IN QUEUE_ENTRY *pQueueEntry)
{
		...
		if (pEntry->bAPSDFlagSPStart != 0) {
			...
			if (pEntry->pUAPSDEOSPFrame != NULL) {
				...
				DBGPRINT(RT_DEBUG_TRACE, ("uapsd> [3] Free EOSP (UP = %d)!\n",
							  RTMP_GET_PACKET_UP(QUEUE_ENTRY_TO_PACKET
									     (pEntry->
									      pUAPSDEOSPFrame))));
#endif /* UAPSD_DEBUG */
...
BOOLEAN UAPSD_SP_IsClosed(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	if ((pEntry) && (pEntry->PsMode == PWR_SAVE) && (pEntry->bAPSDFlagSPStart != 0)
	    )
		FlgIsSpClosed = FALSE;
...
VOID UAPSD_AllPacketDeliver(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	if (pEntry->pUAPSDEOSPFrame != NULL) {
		...
		QueId =
		    RTMP_GET_PACKET_UAPSD_QUE_ID(QUEUE_ENTRY_TO_PACKET(pEntry->pUAPSDEOSPFrame));
		...
		UAPSD_INSERT_QUEUE_AC(pAd, pEntry, &pAd->TxSwQueue[QueId], pEntry->pUAPSDEOSPFrame);
...
========================================================================
*/
VOID UAPSD_AssocParse(IN RTMP_ADAPTER *pAd,
		      IN MAC_TABLE_ENTRY *pEntry, IN UCHAR *pElm, IN BOOLEAN FlgApsdCapable)
{
	...
	if ((pEntry) && (FlgApsdCapable == TRUE)) {
		...
		if ((pEntry->bAPSDCapablePerAC[QID_AC_BE] == 1) &&
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("apsd> at least one AC is not UAPSD %d %d %d %d\n",
...
========================================================================
*/
VOID UAPSD_PacketEnqueue(IN PRTMP_ADAPTER pAd,
			 IN MAC_TABLE_ENTRY *pEntry, IN PNDIS_PACKET pPacket, IN UINT32 IdAc)
{
	/*
	...
	if (pQueUapsd->Number >= MAX_PACKETS_IN_UAPSD_QUEUE) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("uapsd> many(%u) WCID(%d) AC(%d)\n",
			  pQueUapsd->Number, RTMP_GET_PACKET_WCID(pPacket), IdAc));
		...
		if (RTMP_GET_PACKET_MGMT_PKT(pPacket) == 1) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("ps> data (0x%08lx) (AC%d) to uapsd queue (num of pkt = %ld)...\n",
				  (ULONG) pPacket, IdAc, pQueUapsd->Number));
		}
#endif /* UAPSD_DEBUG */
	}
}
...
========================================================================
*/
VOID UAPSD_SP_AUE_Handle(IN RTMP_ADAPTER *pAd, IN MAC_TABLE_ENTRY *pEntry, IN UCHAR FlgSuccess)
{
	...
	if (pEntry->PsMode == PWR_SAVE) {
		...
		if (pEntry->UAPSDTxNum > 0) {
...
#ifdef UAPSD_DEBUG
				DBGPRINT(RT_DEBUG_TRACE,
					 ("uapsd> aux: SP is suspend, keep SP if exists!\n"));
#endif /* UAPSD_DEBUG */
			...
			if ((pEntry->bAPSDFlagSPStart != 0) && (pAd->bAPSDFlagSPSuspend == 0)) {
				/* pEntry->bAPSDFlagSPStart = 0; */
				pEntry->bAPSDFlagEOSPOK = 0;
				UAPSD_SP_END(pAd, pEntry);
...
VOID UAPSD_SP_CloseInRVDone(RTMP_ADAPTER *pAd)
{
	...
	for (IdEntry = FirstWcid; IdEntry < MAX_LEN_OF_MAC_TABLE; IdEntry++) {
		...
		if ((pEntry->bAPSDFlagSPStart != 0) && (pEntry->bAPSDFlagEOSPOK != 0)) {
			/*
			   1. SP is started;
...
VOID UAPSD_TimingRecord(RTMP_ADAPTER *pAd, UINT32 Type)
{
	...
	case UAPSD_TIMING_RECORD_TX2AIR:
		...
		if ((Index % UAPSD_TIMING_RECORD_DISPLAY_TIMES) == 0) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("uapsd> Isr2Tasklet=%d, Trig2Queue=%d, Queue2Air=%d micro seconds\n",
				  gUAPSD_TimingSumIsr2Tasklet / UAPSD_TIMING_RECORD_DISPLAY_TIMES,
				  gUAPSD_TimingSumTrig2Txqueue / UAPSD_TIMING_RECORD_DISPLAY_TIMES,
				  gUAPSD_TimingSumTxqueue2Air / UAPSD_TIMING_RECORD_DISPLAY_TIMES));
			gUAPSD_TimingSumIsr2Tasklet = 0;
			gUAPSD_TimingSumTrig2Txqueue = 0;
			gUAPSD_TimingSumTxqueue2Air = 0;
		}
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("sam> Isr->Tasklet:%d, Trig->TxQueue:%d, TxQueue->TxDone:%d\n",
			  (UINT32) (gUAPSD_TimingTasklet[Index] - gUAPSD_TimingIsr[Index]),
			  (UINT32) (gUAPSD_TimingMov2Tx[Index] - gUAPSD_TimingTrgRcv[Index]),
			  (UINT32) (gUAPSD_TimingTx2Air[Index] - gUAPSD_TimingMov2Tx[Index])));
		break;
...
/*	ULONG    FlgIrq; */
...
#ifdef UAPSD_DEBUG
			DBGPRINT(RT_DEBUG_TRACE,
				 ("uapsd> SP period is too large so SP is closed first!"
				  " (%lu %lu %lu)!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n", TimeNow,
				  pEntry->UAPSDTimeStampLast,
	...
	if (pEntry->MaxSPLength >= 4) {
		...
		DBGPRINT(RT_DEBUG_TRACE, ("uapsd> MaxSPLength >= 4 (%d)!\n", pEntry->MaxSPLength));
...
/*	RTMP_IRQ_LOCK(FlgIrq); */
	...
	/*
	   ...
	   sent to the air.
	 */
	if ((pEntry->bAPSDFlagSpRoughUse == 0) && (pEntry->bAPSDFlagLegacySent != 0)) {
		pEntry->UAPSDTxNum++;
...
========================================================================
*/
VOID UAPSD_TagFrame(IN RTMP_ADAPTER *pAd,
		    IN NDIS_PACKET *pPkt, IN UCHAR Wcid, IN UINT32 PktOffset)
{
	...
	if (RTMP_GET_PACKET_UAPSD_Flag(pPkt) == TRUE) {
		...
		ASSERT(AcQueId <= ARRAY_SIZE(pEntry->UAPSDTagOffset));
		if (AcQueId >= ARRAY_SIZE(pEntry->UAPSDTagOffset)) {
			DBGPRINT(RT_DEBUG_ERROR, ("AcQueId is out of pEntry->UAPSDTagOffset[]\n"));
			return;
		}
...
========================================================================
*/
VOID UAPSD_UnTagFrame(IN RTMP_ADAPTER *pAd,
		      IN UCHAR AcQueId, IN UINT32 bulkStartPos, IN UINT32 bulkEnPos)
{
	...
	for (IdEntry = FirstWcid; IdEntry < MAX_LEN_OF_MAC_TABLE; IdEntry++) {
		...
		if ((IS_ENTRY_CLIENT(pEntry)
		    ...
		    && (pEntry->bAPSDFlagSPStart == 1) && (pEntry->UAPSDTagOffset[AcQueId] != 0)) {
#ifdef UAPSD_DEBUG
