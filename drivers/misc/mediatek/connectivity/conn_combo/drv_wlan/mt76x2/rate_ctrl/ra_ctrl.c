...
*/
VOID MlmeGetSupportedMcs(IN PRTMP_ADAPTER pAd, IN UCHAR *pTable, OUT CHAR mcs[])
{
	CHAR idx;
	...
	if (RTMP_TEST_MORE_FLAG(pAd, fASIC_CAP_MCS_LUT) && (pEntry->wcid < 128)) {
		...
		if (pAd->chipCap.hif_type == HIF_RLT) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("pEntry->HTPhyMode.field.eTxBF = %d, pEntry->HTPhyMode.field.iTxBF = %d\n",
...
#endif /* RTMP_MAC */
		{
			DBGPRINT(RT_DEBUG_ERROR,
				 ("\x1b[31m%s: HIF Type Error !!!\x1b[m\n", __func__));
			return;
		}
		...
		DBGPRINT(RT_DEBUG_INFO,
			 ("%s():MCS_LUT update, write to MAC=0x%08x, Value=0x%04x, WCID=%d\n",
			  __func__, wcid_offset, pEntry->HTPhyMode.word, pEntry->wcid));

		DBGPRINT_RAW(RT_DEBUG_INFO,
			     ("\tWcid=%d, APMlmeSetTxRate - CurrTxRateIdx=%d, MCS=%d, STBC=%d, ShortGI=%d, Mode=%d, BW=%d\n"
			      "\                            ETxBf=%d, ITxBf=%d\n\n", pEntry->wcid,
			      pEntry->CurrTxRateIndex, pEntry->HTPhyMode.field.MCS,
			      pEntry->HTPhyMode.field.STBC, pEntry->HTPhyMode.field.ShortGI,
...
VOID APMlmeSetTxRate(IN RTMP_ADAPTER *pAd,
		     IN MAC_TABLE_ENTRY *pEntry, IN RTMP_RA_LEGACY_TB *pTxRate)
{
	...
	if ((pAd->chipCap.phy_caps & fPHY_CAP_VHT) &&
		...
		if (pAd->CommonCfg.vht_bw_signal && tx_bw == BW_40 &&
		    pAdaptTbEntry->Mode == MODE_VHT && (pAd->MacTab.fAnyStation20Only == FALSE)) {
	...
	if (tx_mode == MODE_HTMIX || tx_mode == MODE_HTGREENFIELD) {
		...
		if ((pTxRate->ShortGI || pAd->WIFItestbed.bShortGI)
	...
	if ((pAd->WIFItestbed.bGreenField & pEntry->HTCapability.HtCapInfo.GF)
	...
	if (pAd->CommonCfg.bRcvBSSWidthTriggerEvents ||
	    (pEntry->MaxHTPhyMode.field.BW == BW_20) || (pAd->CommonCfg.BBPCurrentBW == BW_20))
	...
	if (pAd->CommonCfg.BBPCurrentBW == BW_80 &&
	    pEntry->MaxHTPhyMode.field.BW == BW_80 && pEntry->MaxHTPhyMode.field.MODE == MODE_VHT)
	...
	if ((pEntry->pTable == RateTableVht2S) ||
		...
		pEntry->HTPhyMode.field.MCS =
		    pAdaptTbEntry->CurrMCS | ((pAdaptTbEntry->dataRate - 1) << 4);
		...
		if ((bw_max != BW_10) && (bw_max > pAd->CommonCfg.BBPCurrentBW)) {
	...
	/* 20 MHz Fallback */
	if ((tx_mode == MODE_HTMIX || tx_mode == MODE_HTGREENFIELD) &&
	    pEntry->HTPhyMode.field.BW == BW_40 && ADAPT_RATE_TABLE(pEntry->pTable)) {
...
VOID MlmeSetTxRate(IN RTMP_ADAPTER *pAd,
		   IN MAC_TABLE_ENTRY *pEntry, IN RTMP_RA_LEGACY_TB *pTxRate)
{
	...
	if (pTxRate->STBC && (((pAd->StaCfg.wdev.MaxHTPhyMode.field.STBC)
			       && (tx_mode == MODE_HTMIX || tx_mode == MODE_HTGREENFIELD))
#ifdef DOT11_VHT_AC
			      ||
			      ((CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_VHT_RXSTBC_CAPABLE))
			       && (tx_mode == MODE_VHT))
	...
	if ((pEntry->pTable == RateTableVht2S) || (pEntry->pTable == RateTableVht2S_BW20) ||
		...
		pAd->StaCfg.wdev.HTPhyMode.field.MCS =
		    pAdaptTbEntry->CurrMCS | ((pAdaptTbEntry->dataRate - 1) << 4);
	...
	if (ADHOC_ON(pAd)) {
...
#ifdef DOT11_VHT_AC
		{
			...
			if ((pEntry->force_op_mode == TRUE)) {
				...
				if ((pEntry->HTPhyMode.field.BW != BW_10)
...
VOID MlmeSelectTxRateTable(IN PRTMP_ADAPTER pAd,
			   ...
			   IN UCHAR **ppTable, IN UCHAR *pTableSize, IN UCHAR *pInitTxRateIdx)
{
	do {
#ifdef DOT11_VHT_AC
		...
		if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode) &&
...
#ifdef WFA_VHT_PF
			if ((pAd->CommonCfg.vht_nss_cap > 0) && (ss > pAd->CommonCfg.vht_nss_cap))
			...
			if (pAd->rateAlg == RATE_ALG_GRP) {
				if (ss == 2) {
					...
					DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
						 ("%s(): Select RateTableVht%dS%s\n", __func__,
						  (ss == 2 ? 2 : 1),
						  ((*ppTable ==
						    RateTableVht1S_MCS9) ? "_MCS9" : "")));
				} else {
					DBGPRINT(RT_DEBUG_ERROR,
						 ("%s:unknow ss(%d)\n", __func__, ss));
				}
			...
			if ((IS_RT8592(pAd) && (bw != BW_40)) || (!IS_RT8592(pAd)))
				break;
			...
			if ((pAd->CommonCfg.vht_bw_signal == BW_SIGNALING_DYNAMIC) &&
			    (bw == BW_40) && (pAd->MacTab.fAnyStation20Only == FALSE))
				break;
#endif /* WFA_VHT_PF */
		}
		...
		if ((pAd->OpMode == OPMODE_STA) && ADHOC_ON(pAd)) {
			/* for ADHOC mode */
			if (WMODE_CAP_N(pAd->CommonCfg.PhyMode) && (pEntry->HTCapability.MCSSet[0] != 0x00) && ((pEntry->HTCapability.MCSSet[1] == 0x00) || (pAd->Antenna.field.TxPath == 1))) {	/* 11N 1S Adhoc */
			...
			} else if (WMODE_CAP_N(pAd->CommonCfg.PhyMode) && (pEntry->HTCapability.MCSSet[0] != 0x00) && (pEntry->HTCapability.MCSSet[1] != 0x00) && (((pAd->Antenna.field.TxPath == 3) && (pEntry->HTCapability.MCSSet[2] == 0x00)) || (pAd->Antenna.field.TxPath == 2))) {	/* 11N 2S Adhoc */
#ifdef AGS_SUPPORT
				if (SUPPORT_AGS(pAd)) {
		...
		if ((pEntry->SupportRateMode & (SUPPORT_OFDM_MODE)) && (pEntry->HTCapability.MCSSet[0] != 0x00) && ((pEntry->HTCapability.MCSSet[1] == 0x00) || (pAd->CommonCfg.TxStream == 1))) {	/* 11BGN 1S AP */
...
#ifdef AGS_SUPPORT
		/* only for station */
		if (SUPPORT_AGS(pAd) && (pEntry->HTCapability.MCSSet[0] != 0x00) && (pEntry->HTCapability.MCSSet[1] != 0x00) && (pEntry->HTCapability.MCSSet[2] != 0x00) && (pAd->CommonCfg.TxStream == 3)) {	/* 11N 3S */
			*ppTable = AGS3x3HTRateTable;
			break;
		}
#endif /* AGS_SUPPORT */
		...
		if ((pEntry->SupportRateMode & (SUPPORT_OFDM_MODE)) &&
		    ...
		    (((pAd->Antenna.field.TxPath == 3) && (pEntry->HTCapability.MCSSet[2] == 0x00)) || (pAd->CommonCfg.TxStream == 2))) {	/* 11BGN 2S AP */
#ifdef DOT11_VHT_AC
			if (CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_2G_256QAM_CAPABLE)
			    && (pAd->LatchRfRegs.Channel <= 14)) {
				if (pEntry->MaxHTPhyMode.field.BW == BW_20) {
		...
		if ((pEntry->SupportRateMode & (SUPPORT_OFDM_MODE)) &&
		    (pEntry->HTCapability.MCSSet[0] != 0x00) &&
		    (pEntry->HTCapability.MCSSet[1] != 0x00) &&
		    (pEntry->HTCapability.MCSSet[2] != 0x00) && (pAd->CommonCfg.TxStream == 3)) {
#ifdef NEW_RATE_ADAPT_SUPPORT
			if (pAd->rateAlg == RATE_ALG_GRP) {
		...
		if ((pEntry->HTCapability.MCSSet[0] != 0x00) &&
		    (pEntry->HTCapability.MCSSet[1] != 0x00) && (pAd->CommonCfg.TxStream == 2)
#ifdef THERMAL_PROTECT_SUPPORT
		    && (pAd->force_one_tx_stream == FALSE)
#endif /* THERMAL_PROTECT_SUPPORT */
		    ) {		/* 11N 2S AP */
		...
		if (pAd->CommonCfg.TxStream == 3) {
			if (pEntry->HTCapability.MCSSet[0] != 0x00) {
				if (pEntry->HTCapability.MCSSet[1] == 0x00) {	/* Only support 1SS */
					if (pEntry->RateLen > 0) {
#ifdef NEW_RATE_ADAPT_SUPPORT
						if (pAd->rateAlg == RATE_ALG_GRP)
							*ppTable = RateSwitchTableAdapt11N1S;
					...
					break;
				} else if (pEntry->HTCapability.MCSSet[2] == 0x00) {	/* Only support 2SS */
					if (pEntry->RateLen > 0) {
#ifdef NEW_RATE_ADAPT_SUPPORT
						if (pAd->rateAlg == RATE_ALG_GRP)
							*ppTable = RateSwitchTableAdapt11N2S;
			...
			if ((pEntry->HTCapability.MCSSet[0] == 0) && (pEntry->HTCapability.MCSSet[1] == 0)) {
				if (pAd->CommonCfg.MaxTxRate <= RATE_11) {
					*ppTable = RateSwitchTable11B;
					   ...
					   && (pAd->CommonCfg.MinTxRate > RATE_11)) {
					*ppTable = RateSwitchTable11G;
				} else {
					*ppTable = RateSwitchTable11BG;
				}
...
*/
UCHAR MlmeSelectTxRate(IN PRTMP_ADAPTER pAd,
		       IN PMAC_TABLE_ENTRY pEntry, IN CHAR mcs[], IN CHAR Rssi, IN CHAR RssiOffset)
{
	UCHAR TxRateIdx = 0;
	...
	if (pTable == RateTableVht2S || pTable == RateTableVht2S_BW20
	    || pTable == RateTableVht2S_BW40 || (pTable == RateTableVht2S_MCS7)
		...
		if (mcs[15] >= 0 && (Rssi >= (-70 + RssiOffset)) && (pEntry->SupportVHTMCS[MCS_15]))
			TxRateIdx = mcs[15];
			 ...
			 && (pEntry->SupportVHTMCS[MCS_14]))
			TxRateIdx = mcs[14];
		else if (mcs[13] >= 0 && (Rssi >= (-76 + RssiOffset))
			 && (pEntry->SupportVHTMCS[MCS_13]))
			TxRateIdx = mcs[13];
		else if (mcs[12] >= 0 && (Rssi >= (-78 + RssiOffset))
			 && (pEntry->SupportVHTMCS[MCS_12]))
			TxRateIdx = mcs[12];
		else if (mcs[4] >= 0 && (Rssi >= (-82 + RssiOffset))
			 && (pEntry->SupportVHTMCS[MCS_4]))
			TxRateIdx = mcs[4];
		else if (mcs[3] >= 0 && (Rssi >= (-84 + RssiOffset))
			 && (pEntry->SupportVHTMCS[MCS_3]))
			TxRateIdx = mcs[3];
		else if (mcs[2] >= 0 && (Rssi >= (-86 + RssiOffset))
			 && (pEntry->SupportVHTMCS[MCS_2]))
			TxRateIdx = mcs[2];
		else if (mcs[1] >= 0 && (Rssi >= (-88 + RssiOffset))
			 && (pEntry->SupportVHTMCS[MCS_1]))
			TxRateIdx = mcs[1];
		else
			TxRateIdx = mcs[0];
	} else if ((pTable == RateTableVht1S_MCS9)
		   || (pTable == RateTableVht1S_2G_BW20) || (pTable == RateTableVht1S_2G_BW40)) {	/*  VHT mode with 1SS */
		if (mcs[9] >= 0 && (Rssi > (-72 + RssiOffset)) && (pEntry->SupportVHTMCS[MCS_9]))
			TxRateIdx = mcs[8];
		else if (mcs[8] >= 0 && (Rssi > (-72 + RssiOffset))
			 && (pEntry->SupportVHTMCS[MCS_8]))
			TxRateIdx = mcs[8];
		else if (mcs[7] >= 0 && (Rssi > (-72 + RssiOffset))
			 && (pEntry->SupportVHTMCS[MCS_7]))
			TxRateIdx = mcs[7];
		else if (mcs[6] >= 0 && (Rssi > (-74 + RssiOffset))
			 && (pEntry->SupportVHTMCS[MCS_6]))
			TxRateIdx = mcs[6];
		else if (mcs[5] >= 0 && (Rssi > (-77 + RssiOffset))
			 && (pEntry->SupportVHTMCS[MCS_5]))
			TxRateIdx = mcs[5];
			...
			TxRateIdx = mcs[0];
	} else if (pTable == RateTableVht1S) {	/*  VHT mode with 1SS */
...
#endif /* NEW_RATE_ADAPT_SUPPORT */
#ifdef DOT11N_SS3_SUPPORT
		if ((pTable == RateSwitchTable11BGN3S) || (pTable == RateSwitchTable11N3S)
		...
		if (mcs[23] >= 0 && (Rssi >= (-66 + RssiOffset)) && (pEntry->SupportHTMCS[MCS_23]))
			TxRateIdx = mcs[23];
			 ...
			 && (pEntry->SupportHTMCS[MCS_22]))
			TxRateIdx = mcs[22];
		else if (mcs[21] >= 0 && (Rssi >= (-72 + RssiOffset))
			 && (pEntry->SupportHTMCS[MCS_21]))
			TxRateIdx = mcs[21];
		else if (mcs[20] >= 0 && (Rssi >= (-74 + RssiOffset))
			 && (pEntry->SupportHTMCS[MCS_20]))
			TxRateIdx = mcs[20];
		else if (mcs[13] >= 0 && (Rssi >= (-76 + RssiOffset))
			 && (pEntry->SupportHTMCS[MCS_13]))
			TxRateIdx = mcs[13];
		else if (mcs[12] >= 0 && (Rssi >= (-78 + RssiOffset))
			 && (pEntry->SupportHTMCS[MCS_12]))
			TxRateIdx = mcs[12];
		else if (mcs[4] >= 0 && (Rssi >= (-82 + RssiOffset))
			 && (pEntry->SupportHTMCS[MCS_4]))
			TxRateIdx = mcs[4];
		else if (mcs[3] >= 0 && (Rssi >= (-84 + RssiOffset))
			 && (pEntry->SupportHTMCS[MCS_3]))
			TxRateIdx = mcs[3];
		else if (mcs[2] >= 0 && (Rssi >= (-86 + RssiOffset))
			 && (pEntry->SupportHTMCS[MCS_2]))
			TxRateIdx = mcs[2];
		else if (mcs[1] >= 0 && (Rssi >= (-88 + RssiOffset))
			 && (pEntry->SupportHTMCS[MCS_1]))
			TxRateIdx = mcs[1];
		else
			TxRateIdx = mcs[0];
	} else
		...
		if (mcs[15] >= 0 && (Rssi >= (-70 + RssiOffset)) && (pEntry->SupportHTMCS[MCS_15]))
		   ...
		   (pTable == RateSwitchTable11N1S) || (pTable == RateSwitchTable11N1SForABand)
#ifdef NEW_RATE_ADAPT_SUPPORT
	    ...
	    ) {			/*  N mode with 1 stream */
		{
			if (mcs[7] >= 0 && (Rssi > (-72 + RssiOffset))
			    && (pEntry->SupportHTMCS[MCS_7]))
				TxRateIdx = mcs[7];
			else if (mcs[6] >= 0 && (Rssi > (-74 + RssiOffset))
				 && (pEntry->SupportHTMCS[MCS_6]))
				TxRateIdx = mcs[6];
			else if (mcs[5] >= 0 && (Rssi > (-77 + RssiOffset))
				 && (pEntry->SupportHTMCS[MCS_5]))
				TxRateIdx = mcs[5];
			else if (mcs[4] >= 0 && (Rssi > (-79 + RssiOffset))
				 && (pEntry->SupportHTMCS[MCS_4]))
				TxRateIdx = mcs[4];
...
*/
VOID MlmeRALog(IN PRTMP_ADAPTER pAd,
	       IN PMAC_TABLE_ENTRY pEntry,
	       IN RA_LOG_TYPE raLogType, IN ULONG TxErrorRatio, IN ULONG TxTotalCnt)
{
#ifdef TXBF_SUPPORT
	...
	UINT TxCount =
	    pEntry->TxBFCounters.TxSuccessCount + pEntry->TxBFCounters.TxFailCount + ETxCount +
	    ITxCount;
	ULONG bfRatio = 0;
#endif /*  TXBF_SUPPORT */
	...
	if (TxTotalCnt != 0 || raLogType == RAL_QUICK_DRS
		...
		if (raLogType == RAL_QUICK_DRS)
			tp = (100 -
			      TxErrorRatio) * TxTotalCnt * RA_INTERVAL / (100 *
			...
			tp = (100 -
			      TxErrorRatio) * TxTotalCnt * RA_INTERVAL / (100 * (RA_INTERVAL -
										 pAd->ra_fast_interval));
		...
		if ((TxCount - pEntry->LastTxCount) > 0) {
			if (pEntry->HTPhyMode.field.eTxBF)
				bfRatio =
				    100 * (ETxCount - pEntry->LastETxCount) / (TxCount -
									       pEntry->LastTxCount);
			else if (pEntry->HTPhyMode.field.iTxBF)
				bfRatio =
				    100 * (ITxCount - pEntry->LastITxCount) / (TxCount -
									       pEntry->LastTxCount);
		}
		...
		if ((pEntry->HTPhyMode.field.eTxBF || pEntry->HTPhyMode.field.iTxBF)
			...
			DBGPRINT_RAW(RT_DEBUG_ERROR,
				     ("%s[%d]: M=%d %c%c%c%c%c PER=%ld%% TP=%ld BF=%ld%% ",
				      raLogType == RAL_QUICK_DRS ? " Q" : (raLogType ==
									   RAL_NEW_DRS ? "\nRA" :
									   "\nra"), pEntry->wcid,
				      pEntry->HTPhyMode.field.MCS,
				      pEntry->HTPhyMode.field.MODE ==
				      MODE_CCK ? 'C' : (pEntry->HTPhyMode.
							field.ShortGI ? 'S' : 'L'),
				      pEntry->HTPhyMode.field.BW ? '4' : '2', stbc ? 'S' : 's',
				      csd ? 'C' : 'c',
				      pEntry->HTPhyMode.field.eTxBF ? 'E' : (pEntry->
									     HTPhyMode.field.iTxBF ?
									     'I' : '-'),
...
VOID MlmeRestoreLastRate(IN PMAC_TABLE_ENTRY pEntry)
{
...
VOID MlmeCheckRDG(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry)
{
	PUCHAR pTable = pEntry->pTable;

	/*  Turn off RDG when 3s and rx count > tx count*5 */
	if (((pTable == RateSwitchTable11BGN3S) ||
	     (pTable == RateSwitchTable11BGN3SForABand) || (pTable == RateSwitchTable11N3S)
#ifdef NEW_RATE_ADAPT_SUPPORT
	     || (pTable == RateSwitchTableAdapt11N3S)
#endif /* NEW_RATE_ADAPT_SUPPORT */
		...
		pTempTxRate =
		    (RTMP_RA_LEGACY_TB *) (&pTable[(pEntry->CurrTxRateIndex + 1) * TableStep]);
		RTMP_IO_READ32(pAd, TX_LINK_CFG, &TxLinkCfg.word);
		if (pAd->RalinkCounters.OneSecReceivedByteCount >
		    (pAd->RalinkCounters.OneSecTransmittedByteCount * 5)
...
#ifdef TXBF_SUPPORT
static VOID txbf_rate_adjust(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	if (ADAPT_RATE_TABLE(pTable))
		pNextTxRate =
		    (RTMP_RA_LEGACY_TB *) PTX_RA_GRP_ENTRY(pTable, pEntry->CurrTxRateIndex);
	else
#endif /*  NEW_RATE_ADAPT_SUPPORT */
...
static INT rtmp_get_rate_from_rate_tb(UCHAR *table, INT idx, RTMP_TX_RATE *tx_rate)
{
#ifdef NEW_RATE_ADAPT_SUPPORT
...
VOID MlmeNewTxRate(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	if (pAd->CommonCfg.StreamMode != 0 && pEntry->StreamModeMACReg != 0) {
		UINT streamWord;
		BOOLEAN mcsDisable;
		...
		RTMP_IO_WRITE32(pAd, pEntry->StreamModeMACReg + 4,
				streamWord | (ULONG) (pEntry->
						      Addr[4]) | (ULONG) (pEntry->Addr[5] << 8));
	}
#endif /* STREAM_MODE_SUPPORT */
}

VOID RTMPSetSupportMCS(IN PRTMP_ADAPTER pAd,
		       IN UCHAR OpMode,
		       IN PMAC_TABLE_ENTRY pEntry,
		       IN UCHAR SupRate[],
		       IN UCHAR SupRateLen, IN UCHAR ExtRate[], IN UCHAR ExtRateLen,
#ifdef DOT11_VHT_AC
		       IN UCHAR vht_cap_len, IN VHT_CAP_IE *vht_cap,
#endif				/* DOT11_VHT_AC */
		       IN HT_CAPABILITY_IE *pHtCapability, IN UCHAR HtCapabilityLen)
{
	...
	if (ExtRateLen > 0) {
		if ((SupRateLen + ExtRateLen) <= MAX_LEN_OF_SUPPORTED_RATES) {
			...
			NdisMoveMemory(&SupportedRates[SupRateLen], ExtRate,
				       MAX_LEN_OF_SUPPORTED_RATES - SupRateLen);
	...
	if (HtCapabilityLen) {
		...
		if (OpMode == OPMODE_AP) {
			...
			if (IS_ENTRY_WDS(pEntry))
				pDesired_ht_phy =
				    &pAd->WdsTab.WdsEntry[pEntry->apidx].wdev.DesiredHtPhyInfo;
			else
#endif /* WDS_SUPPORT */
#ifdef APCLI_SUPPORT
			if (IS_ENTRY_APCLI(pEntry))
				pDesired_ht_phy =
				    &pAd->ApCfg.ApCliTab[pEntry->apidx].wdev.DesiredHtPhyInfo;
			else
#endif /* APCLI_SUPPORT */
				pDesired_ht_phy =
				    &pAd->ApCfg.MBSSID[pEntry->apidx].wdev.DesiredHtPhyInfo;
		}
#endif /* CONFIG_AP_SUPPORT */
		...
		if ((vht_cap_len > 0) && (vht_cap != NULL) && pDesired_ht_phy->bVhtEnable) {
			...
			case 2:
				if (vht_cap->mcs_set.rx_mcs_map.mcs_ss2 < VHT_MCS_CAP_NA) {
					...
					} else if (vht_cap->mcs_set.rx_mcs_map.mcs_ss2 ==
						   VHT_MCS_CAP_9) {
						pEntry->SupportVHTMCS[18] = TRUE;
			...
			case 1:
				if (vht_cap->mcs_set.rx_mcs_map.mcs_ss1 < VHT_MCS_CAP_NA) {
					...
					} else if (vht_cap->mcs_set.rx_mcs_map.mcs_ss1 ==
						   VHT_MCS_CAP_9) {
						pEntry->SupportVHTMCS[8] = TRUE;
