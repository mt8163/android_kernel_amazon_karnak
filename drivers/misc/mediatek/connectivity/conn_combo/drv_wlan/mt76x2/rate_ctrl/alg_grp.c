...
#endif /* DOT11_VHT_AC */
	if (ADAPT_RATE_TABLE(pTable) || (pTable == RateSwitchTable11BGN3S) || (pTable == RateSwitchTable11BGN3SForABand)) {	/*  N mode with 3 stream */
		if ((pEntry->HTCapability.MCSSet[2] == 0xff) && (pAd->CommonCfg.TxStream == 3)) {
		...
		if (mcs[15] >= 0 && (Rssi >= (-70 + RssiOffset)))
			...
			TxRateIdx = mcs[0];
	} else if ((pTable == RateSwitchTable11BGN1S) || (pTable == RateSwitchTable11N1S) || (pTable == RateSwitchTable11N1SForABand)) {	/*  N mode with 1 stream */
		if (mcs[7] >= 0 && (Rssi > (-72 + RssiOffset)))
			TxRateIdx = mcs[7];
		else if (mcs[6] >= 0 && (Rssi > (-74 + RssiOffset)))
			TxRateIdx = mcs[6];
				     ...
				     IN ULONG TxRetransmit, IN ULONG TxFailCount)
{
	...
	if (TxTotalCnt <= 15) {
		...
		if (pEntry->lowTrafficCount >= pAd->CommonCfg.lowTrafficThrd) {
			...
			if (pEntry->LastSaveRateIdx > 0) {
				...
				if (pEntry->lowTrafficCount > 120) {	/* 60 second */
					pEntry->LastSaveRateIdx =
					    MlmeSelectTxRateAdapt(pAd, pEntry, mcs, Rssi,
								  RssiOffset);
				...
				/* Select the Tx rate based on the RSSI */
				TxRateIdx =
				    MlmeSelectTxRateAdapt(pAd, pEntry, mcs, Rssi, RssiOffset);
			}
			...
			/* if (TxRateIdx != pEntry->CurrTxRateIndex) */
			{
				...
				if (!pEntry->fLastSecAccordingRSSI) {
					DBGPRINT_RAW(RT_DEBUG_INFO,
						     ("DRS: TxTotalCnt <= 15, switch to MCS%d according to RSSI (%d), RssiOffset=%d\n",
						      pEntry->HTPhyMode.field.MCS, Rssi,
						      RssiOffset));
				}
			}
		...
		DBGPRINT(RT_DEBUG_TRACE, ("Low TX Traffic: Current Rate is %d\n", CurrRateIdx));
#if defined(DOT11_VHT_AC) && defined(CERTIFICATION_SIGMA_SUPPORT)
		/* bypass low traffic count */
		if (SIGMA_ON(pAd))
			DBGPRINT(RT_DEBUG_OFF, ("VHT5.2.62:bypass low traffic case\n"));
		else
#endif /* DOT11_VHT_AC */
			return;
	}
