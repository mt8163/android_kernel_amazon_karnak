...
VOID APMlmeDynamicTxRateSwitching(RTMP_ADAPTER *pAd)
{
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
				...
				if (pEntry->wcid >= 1 && pEntry->wcid <= 8) {
					...
					DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
						 ("%s()=>Wcid:%d, MCS:%d, CuTxRaIdx=%d,TxErrRatio(Hw:%ld-%ld%%, Sw:%ld-%ld%%)\n",
						  ...
						  pEntry->CurrTxRateIndex, HwTxCnt, HwErrRatio,
						  TxTotalCnt, TxErrorRatio));
			...
			if (TxRateIdx != pEntry->CurrTxRateIndex
				...
				if (!pEntry->fLastSecAccordingRSSI)
					DBGPRINT(RT_DEBUG_INFO,
						 ("DRS: TxTotalCnt <= 15, switch MCS according to RSSI (%d)\n",
						  Rssi));
			}
		...
		MlmeOldRateAdapt(pAd, pEntry, CurrRateIdx, UpRateIdx, DownRateIdx, TrainUp,
				 TrainDown, TxErrorRatio);
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
				...
				if ((pEntry->wcid >= 1) && (pEntry->wcid <= 8)) {
					...
					DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
						 ("%s()=>Wcid:%d, MCS:%d, TxErrRation(Hw:0x%lx-0x%lx, Sw:0x%lx-%lx)\n",
						  __func__, pEntry->wcid,
						  pEntry->HTPhyMode.field.MCS, HwTxCnt, HwErrRatio,
						  TxTotalCnt, TxErrorRatio));
		...
		do {
			...
			if ((pEntry->LastTimeTxRateChangeAction == RATE_NO_CHANGE)
				...
				ratio =
				    (RA_INTERVAL - pAd->ra_fast_interval) / pAd->ra_fast_interval;
			...
			if (pAd->MacTab.Size == 1) {
				...
				OneSecTxNoRetryOKRationCount =
				    pEntry->OneSecTxNoRetryOkCount * ratio +
				    (pEntry->OneSecTxNoRetryOkCount >> 1);
			}
...
/* TODO: gaa - use different criterion for train up in Old RA? */
				...
				if ((TxErrorRatio >= 50) && (TxErrorRatio >= TrainDown)) {
				} else if ((pEntry->LastTxOkCount + 2) >=
					   OneSecTxNoRetryOKRationCount) {
					MlmeRestoreLastRate(pEntry);
				} else {
					DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
...
VOID MlmeDynamicTxRateSwitching(IN PRTMP_ADAPTER pAd)
{
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
...
#ifdef NEW_RATE_ADAPT_SUPPORT
		if (ADAPT_RATE_TABLE(pTable)) {
			MlmeDynamicTxRateSwitchingAdapt(pAd, i, TxSuccess, TxRetransmit,
							TxFailCount);
			continue;
		}
#endif /* NEW_RATE_ADAPT_SUPPORT */
		...
		if ((pAd->MacTab.Size == 1) || IS_ENTRY_DLS(pEntry)) {
			...
			if (pAd->chipCap.FlgHwFifoExtCap) {
				if (pEntry->Aid >= 1 && pEntry->Aid <= 8) {
					...
					DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
						 ("%s():Aid:%d, MCS:%d, CuTxRaIdx=%d,TxErrRatio(Hw:%lu-%lu%%, Sw:%lu-%lu%%)\n",
						  __func__, pEntry->Aid,
						  pEntry->HTPhyMode.field.MCS,
						  pEntry->CurrTxRateIndex, HwTxCnt, HwErrRatio,
						  TxTotalCnt, TxErrorRatio));
		...
		if (AGS_IS_USING(pAd, pTable)) {
			...
			MlmeDynamicTxRateSwitchingAGS(pAd, pEntry, pTable, TableSize,
						      &AGSStatisticsInfo, InitTxRateIdx);
				  ...
				  IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if (pAd->MacTab.Size == 1) {
			...
			if (pAd->chipCap.FlgHwFifoExtCap) {
				if (pEntry->wcid >= 1 && pEntry->wcid <= 8) {
					...
					HwTxCnt =
					    wcidTxCnt.field.succCnt + wcidTxCnt.field.reTryCnt;
					if (HwTxCnt)
						HwErrRatio =
						    (wcidTxCnt.field.reTryCnt * 100) / HwTxCnt;

					DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
						 ("%s():TxErrRatio(wcid:%d, MCS:%d, Hw:%lu-%lu, Sw:%lu-%lu)\n",
						  __func__, pEntry->wcid,
						  pEntry->HTPhyMode.field.MCS, HwTxCnt, HwErrRatio,
						  TxTotalCnt, TxErrorRatio));
		...
		if (AGS_IS_USING(pAd, pTable)) {
			...
			StaQuickResponeForRateUpExecAGS(pAd, pEntry, pTable, TableSize,
							&AGSStatisticsInfo, InitTxRateIdx);
...
*/
VOID MlmeOldRateAdapt(IN PRTMP_ADAPTER pAd,
	...
	if (TxErrorRatio >= TrainDown) {
		...
		if (bTrainUp) {
			...
			if ((CurrRateIdx != UpRateIdx)
			    && (MlmeGetTxQuality(pEntry, UpRateIdx) <= 0)) {
				...
				if (pEntry->phyETxBf || pEntry->phyITxBf) {
					...
					RTMP_RA_LEGACY_TB *pUpRate =
					    PTX_RA_LEGACY_ENTRY(pTable, UpRateIdx);
					RTMP_RA_LEGACY_TB *pCurrTxRate =
					    PTX_RA_LEGACY_ENTRY(pTable, CurrRateIdx);

					/* First try Up Rate with BF */
					if ((CurrRateIdx != UpRateIdx)
					    && MlmeTxBfAllowed(pAd, pEntry, pUpRate)) {
