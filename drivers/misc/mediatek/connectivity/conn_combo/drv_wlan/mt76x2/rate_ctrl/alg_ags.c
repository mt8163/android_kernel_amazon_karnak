...
UCHAR AgsRssiOffsetTable[3][4] = {
	...
	/* ---------------------- */
	{0, 2, 4, 6}
	,			/* For VHT 1*1: BW20, BW40, BW80, BW160 */
	{2, 4, 6, 8}
	,			/* For VHT 2*2: BW20, BW40, BW80, BW160 */
...
INT Show_AGS_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
...
/*
	...
	Return Value:
		None
*/
VOID MlmeDynamicTxRateSwitchingAGS(IN PRTMP_ADAPTER pAd,
				   ...
				   IN UCHAR InitTxRateIdx)
{
	...
	DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
		 ("%s: QuickAGS: AccuTxTotalCnt = %d, TxSuccess = %d, TxRetransmit = %d, TxFailCount = %d, TxErrorRatio = %d\n",
		  __func__, pAGSStatisticsInfo->AccuTxTotalCnt, pAGSStatisticsInfo->TxSuccess,
		  pAGSStatisticsInfo->TxRetransmit, pAGSStatisticsInfo->TxFailCount,
	...
	pCurrTxRate =
	    (RTMP_RA_AGS_TB *) (&pTable[(CurrRateIdx + 1) * SIZE_OF_AGS_RATE_TABLE_ENTRY]);
			...
			default:
				{
					DBGPRINT_RAW(RT_DEBUG_ERROR,
						     ("%s: AGS: Incorrect MCS group, pEntry->AGSCtrl.MCSGroup = %d\n",
						      __func__, pEntry->AGSCtrl.MCSGroup));
				}
				break;
			}
	...
	if (pAGSStatisticsInfo->AccuTxTotalCnt <= 15) {
		...
		if ((pTable == Ags2x2VhtRateTable) || (pTable == Ags1x1VhtRateTable)) {
			...
			while (idx < pTable[0]) {
				pCurrTxRate =
				    (RTMP_RA_AGS_TB
				     *) (&pTable[(idx + 1) * SIZE_OF_AGS_RATE_TABLE_ENTRY]);
				if (pCurrTxRate->Mode == MODE_VHT) {
					if (pCurrTxRate->Nss == NSS_1)
						mcs_idx_offset = 0;
					else if ((pCurrTxRate->Nss == NSS_2)
						 && (pTable == Ags2x2VhtRateTable))
						mcs_idx_offset = 8;

					if ((pCurrTxRate->CurrMCS >= MCS_0)
					    && (pCurrTxRate->CurrMCS <= MCS_7))
 ...
 || (pTable == Ags2x2VhtRateTable)
		...
		pNextTxRate =
		    (RTMP_RA_LEGACY_TB
		     *) (&pTable[(pEntry->CurrTxRateIndex + 1) * SIZE_OF_AGS_RATE_TABLE_ENTRY]);
	...
	if (pEntry->fLastSecAccordingRSSI == TRUE) {
		...
		DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
			     ("%s: AGS: The MCS selection base on RSSI\n", __func__));

		return;
	}

	DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
		     ("%s: AGS: TrainUp:%d, TrainDown:%d\n", __func__, TrainUp, TrainDown));
	...
	do {
		...
		if (bTrainUpDown) {
			...
			} else if ((CurrRateIdx != UpRateIdx) && (pEntry->TxQuality[UpRateIdx] <= 0)) {	/* Upgrade Tx rate */
	...
	DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
		     ("%s: AGS: pEntry->CurrTxRateIndex = %d, CurrRateIdx = %d, pEntry->LastSecTxRateChangeAction = %d\n",
		      __func__, pEntry->CurrTxRateIndex, CurrRateIdx,
	...
	if ((bTxRateChanged == TRUE) && (!pAd->StaCfg.StaQuickResponeForRateUpTimerRunning)) {
	...
	if (INFRA_ON(pAd)) {
		if ((pAd->CommonCfg.bRdg == TRUE) && CLIENT_STATUS_TEST_FLAG(&pAd->MacTab.Content[BSSID_WCID], fCLIENT_STATUS_RDG_CAPABLE)) {	/* RDG capable */
			...
			if ((pAd->RalinkCounters.OneSecReceivedByteCount >
			    ...
			    ((pAd->RalinkCounters.OneSecReceivedByteCount +
			      pAd->RalinkCounters.OneSecTransmittedByteCount) >= (50 * 1024))) {
...
/*
	...
	Return Value:
		None
*/
VOID StaQuickResponeForRateUpExecAGS(IN PRTMP_ADAPTER pAd,
				     IN PMAC_TABLE_ENTRY pEntry,
				     IN PUCHAR pTable,
				     IN UCHAR TableSize,
				     IN PAGS_STATISTICS_INFO pAGSStatisticsInfo,
				     IN UCHAR InitTxRateIdx)
{
	...
	DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
		 ("%s: QuickAGS: AccuTxTotalCnt = %lu, TxSuccess = %lu, "
		  ...
		  pAGSStatisticsInfo->TxFailCount, pAGSStatisticsInfo->TxErrorRatio));
	...
	do {
		...
		if ((pEntry->LastSecTxRateChangeAction == 1) && (CurrRateIdx != DownRateIdx)) {
			...
			if (pAGSStatisticsInfo->TxErrorRatio > TrainDown) {	/* Poor quality */
				...
				DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
					     ("%s: QuickAGS: (UP) bad Tx ok count (Current PER:%d, NewMcs's TrainDown:%d)\n",
					      __func__, pAGSStatisticsInfo->TxErrorRatio,
					      TrainDown));
			} else {	/* Good quality */

				DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
					     ("%s: QuickAGS: (UP) keep rate-up (Current PER:%d, NewMcs's TrainDown:%d)\n",
					      __func__, pAGSStatisticsInfo->TxErrorRatio,
					      TrainDown));

				RTMPZeroMemory(pEntry->TxQuality,
					       sizeof(USHORT) * (MAX_TX_RATE_INDEX + 1));
...
/* Don't quick check within train down case */
	...
	if (pEntry->LastSecTxRateChangeAction == 1) {
		...
		if ((pEntry->CurrTxRateIndex != CurrRateIdx) && (pEntry->AGSCtrl.MCSGroup > 0)) {
			...
			pCurrTxRate =
			    (RTMP_RA_AGS_TB
			     *) (&pTable[(DownRateIdx + 1) * SIZE_OF_AGS_RATE_TABLE_ENTRY]);
		}
		...
		default:
			{
				DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
					     ("%s: QuickAGS: Incorrect MCS group, pEntry->AGSCtrl.MCSGroup = %d\n",
					      __func__, pEntry->AGSCtrl.MCSGroup));
			}
			break;
		}
		...
		DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
			     ("%s: QuickAGS: next MCS group,  pEntry->AGSCtrl.MCSGroup = %d\n",
			      __func__, pEntry->AGSCtrl.MCSGroup));

	}

	if ((pEntry->CurrTxRateIndex != CurrRateIdx) && (pEntry->LastSecTxRateChangeAction == 2)) {	/* Tx rate up */
		DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
			     ("%s: QuickAGS: ++TX rate from %d to %d\n", __func__, CurrRateIdx,
			      pEntry->CurrTxRateIndex));

		pEntry->TxRateUpPenalty = 0;
		pEntry->TxQuality[pEntry->CurrTxRateIndex] = 0;	/*restore the TxQuality from max to 0 */
		RTMPZeroMemory(pEntry->PER, sizeof(UCHAR) * (MAX_TX_RATE_INDEX + 1));
	} else if ((pEntry->CurrTxRateIndex != CurrRateIdx) && (pEntry->LastSecTxRateChangeAction == 1)) {	/* Tx rate down */
		DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
			     ("%s: QuickAGS: --TX rate from %d to %d\n", __func__, CurrRateIdx,
			      pEntry->CurrTxRateIndex));
...
/*
	...
	None
*/
VOID ApMlmeDynamicTxRateSwitchingAGS(IN RTMP_ADAPTER *pAd, IN INT idx)
{
	...
	DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
		 ("%s: QuickAGS: AccuTxTotalCnt = %ld, TxSuccess = %ld, TxRetransmit = %ld, TxFailCount = %ld, TxErrorRatio = %ld\n",
		  __func__, AGSStatisticsInfo.AccuTxTotalCnt, AGSStatisticsInfo.TxSuccess,
		  AGSStatisticsInfo.TxRetransmit, AGSStatisticsInfo.TxFailCount,
 ...
 || (pTable == Ags2x2VhtRateTable)
	...
	DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
		 ("%s: AGS: Rssi = %d, TxSuccess = %lu, TxRetransmit = %lu, TxFailCount = %lu, TxErrorRatio = %lu\n",
		  __func__, AGSStatisticsInfo.RSSI, AGSStatisticsInfo.TxSuccess,
		  AGSStatisticsInfo.TxRetransmit, AGSStatisticsInfo.TxFailCount,
		  AGSStatisticsInfo.TxErrorRatio));
	...
	DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
		     ("%s: AGS: Before - CurrTxRateIdx = %d, MCS = %d, STBC = %d, ShortGI = %d, Mode = %d, "
		      ...
		      pEntry->AGSCtrl.MCSGroup,
		      AGSStatisticsInfo.TxErrorRatio,
		      AGSStatisticsInfo.TxRetransmit, AGSStatisticsInfo.TxSuccess));
	...
	if (AGSStatisticsInfo.AccuTxTotalCnt <= 15) {
			...
			while (idx < pTable[0]) {
				...
				if (pCurrTxRate->Mode == MODE_VHT) {
					...
					if ((pCurrTxRate->CurrMCS <=
					     MCS_7) /* && (pCurrTxRate->CurrMCS >= MCS_0) */)
						MCS[pCurrTxRate->CurrMCS + mcs_idx_offset] = idx;
				}
				idx++;
			}
...
#endif /* DOT11_VHT_AC */
		{
			while (idx < pTable[0]) {
				...
				if (pCurrTxRate->CurrMCS <=
				    MCS_23 /* && pCurrTxRate->CurrMCS >= MCS_0 */)
					MCS[pCurrTxRate->CurrMCS] = idx;
				idx++;
			}
		}
	...
	/* Tx rate fast train up/down */
	if ((bTxRateChanged == TRUE) && (!pAd->ApCfg.ApQuickResponeForRateUpTimerRunning)) {
...
/*
	...
	Return Value:
		None
*/
VOID ApQuickResponeForRateUpExecAGS(IN RTMP_ADAPTER *pAd, IN INT idx)
{
	UCHAR *pTable, TableSize = 0;
	UCHAR UpRateIdx = 0, DownRateIdx = 0, CurrRateIdx = 0;
	RTMP_RA_AGS_TB *pCurrTxRate = NULL;
	...
	DBGPRINT(RT_DEBUG_INFO | DBG_FUNC_RA,
		 ("%s: QuickAGS: AccuTxTotalCnt = %lu, TxSuccess = %lu, "
		  ...
		  AGSStatisticsInfo.TxFailCount, AGSStatisticsInfo.TxErrorRatio));
			...
			if (AGSStatisticsInfo.TxErrorRatio > TrainDown) {	/* Poor quality */
				...
				DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
					     ("%s: QuickAGS: (UP) bad Tx ok count (Current PER:%ld, NewMcs's TrainDown:%d)\n",
					      __func__, AGSStatisticsInfo.TxErrorRatio, TrainDown));
			} else {	/* Good quality */
...
/* Don't quick check within train down case */
	...
	if ((pEntry->CurrTxRateIndex != CurrRateIdx) && (pEntry->LastSecTxRateChangeAction == 2)) {	/* Tx rate up */
		...
		bTxRateChanged = FALSE;
		DBGPRINT_RAW(RT_DEBUG_INFO | DBG_FUNC_RA,
			     ("%s: QuickAGS: rate is not changed\n", __func__));
	}
