...
static void mt76x2_ate_switch_channel(RTMP_ADAPTER *ad)
{
	...
	UINT32 tssi_backup_value_tx0, tssi_backup_value_tx1;
	UCHAR bbp_ch_idx = 0;
	BOOLEAN band_change = FALSE;
	u8 channel = 0;
	UINT32 eLNA_gain_from_e2p = 0;
	RTMP_REG_PAIR *mac_cr_table = NULL;
	RTMP_REG_PAIR *mac_pa_cr_table = NULL;
	UCHAR mac_cr_table_nums = 0;
	UCHAR mac_pa_cr_table_nums = 0;
...
*/
	...
	if (band_change) {
		...
		if (band == _G_BAND) {
			mac_cr_table = mt76x2_mac_g_band_cr_table;
			mac_cr_table_nums = mt76x2_mac_g_band_cr_nums;

			if (ad->chipCap.PAType & INT_PA_2G) {
				mac_pa_cr_table = mt76x2_mac_g_band_internal_pa_cr_table;
				mac_pa_cr_table_nums = mt76x2_mac_g_band_internal_pa_cr_nums;
			} else {
				mac_pa_cr_table = mt76x2_mac_g_band_external_pa_cr_table;
				mac_pa_cr_table_nums = mt76x2_mac_g_band_external_pa_cr_nums;
			}
		} else {
			mac_cr_table = mt76x2_mac_a_band_cr_table;
			mac_cr_table_nums = mt76x2_mac_a_band_cr_nums;

			if (ad->chipCap.PAType & INT_PA_5G) {
				mac_pa_cr_table = mt76x2_mac_a_band_internal_pa_cr_table;
				mac_pa_cr_table_nums = mt76x2_mac_a_band_internal_pa_cr_nums;
			} else {
				mac_pa_cr_table = mt76x2_mac_a_band_external_pa_cr_table;
				mac_pa_cr_table_nums = mt76x2_mac_a_band_external_pa_cr_nums;
			}
		}

#ifdef RTMP_USB_SUPPORT
		if (IS_USB_INF(ad)) {
			RANDOM_WRITE(ad, mac_cr_table, mac_cr_table_nums);
			RANDOM_WRITE(ad, mac_pa_cr_table, mac_pa_cr_table_nums);
		}
#endif /* endif */
	}
...
static INT mt76x2_set_ate_tx_bw_proc(IN PRTMP_ADAPTER ad, IN PSTRING arg)
{
	...
	if ((BBPCurrentBW == 0)) {
		...
		DBGPRINT(RT_DEBUG_ERROR, ("Set_ATE_TX_BW_Proc!! Error!! Unknown bw.\n"));
		return TRUE;
	}
...
static VOID mt76x2_ate_asic_adjust_tx_power(IN PRTMP_ADAPTER pAd)
{
	...
	if ((pAd->chipCap.tssi_enable) &&
		...
		if (mt76x2_ate_calibration_delay % 3 == 0) {
			DBGPRINT(RT_DEBUG_INFO,
				 ("mt76x2_ate_calibration_delay mod 3 == 0 \n"));
			DBGPRINT(RT_DEBUG_INFO,
				 ("mt76x2_tx0_tssi_small_pwr_adjust %d\n",
				  mt76x2_tx0_tssi_small_pwr_adjust));
			DBGPRINT(RT_DEBUG_INFO,
				 ("mt76x2_tx1_tssi_small_pwr_adjust %d\n",
