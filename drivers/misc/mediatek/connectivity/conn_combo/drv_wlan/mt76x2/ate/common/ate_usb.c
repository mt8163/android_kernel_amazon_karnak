...
static VOID ATEWriteTxWI(IN PRTMP_ADAPTER pAd, IN TXWI_STRUCT *pTxWI, IN BOOLEAN FRAG,
...
INT ATESetUpFrame(IN PRTMP_ADAPTER pAd, IN UINT32 TxIdx)
{
	...
	if (pNullContext->InUse == FALSE) {
		...
		RTMPZeroMemory(&pAd->NullContext.TransferBuffer->field.WirelessPacket[0],
			       TX_BUFFER_NORMSIZE);
		pTxInfo =
		    (TXINFO_STRUCT *) &pAd->NullContext.TransferBuffer->field.WirelessPacket[0];
		...
		pTxWI =
		    (TXWI_STRUCT *) &pAd->NullContext.TransferBuffer->field.
