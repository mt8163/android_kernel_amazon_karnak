...
CHAR ATEGetDesiredTSSI(IN PRTMP_ADAPTER pAd)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s: desiredTSSI = %d, Latest Tx setting: MODE = %d, MCS = %d, STBC = %d\n",
		  __func__, desiredTSSI, phy_mode, MCS, stbc));
...
static CHAR ATEConvertToRssi(IN PRTMP_ADAPTER pAd, IN CHAR Rssi, IN UCHAR RssiNumber)
{
...
VOID ATESampleRssi(IN RTMP_ADAPTER *pAd, IN RXWI_STRUCT *pRxWI)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	CHAR rssi[3] = { 0 };
	CHAR snr[3] = { 0 };
	...
	if (rssi[0] != 0) {
		...
		pATEInfo->AvgRssi0X8 =
		    (pATEInfo->AvgRssi0X8 - pATEInfo->AvgRssi0) + pATEInfo->LastRssi0;
	...
	if (rssi[1] != 0) {
		...
		pATEInfo->AvgRssi1X8 =
		    (pATEInfo->AvgRssi1X8 - pATEInfo->AvgRssi1) + pATEInfo->LastRssi1;
	...
	if (rssi[2] != 0) {
		...
		pATEInfo->AvgRssi2X8 =
		    (pATEInfo->AvgRssi2X8 - pATEInfo->AvgRssi2) + pATEInfo->LastRssi2;
...
#ifdef RT_RF
VOID RtmpRfIoWrite(IN PRTMP_ADAPTER pAd)
{
...
VOID ATEAsicSetTxRxPath(IN PRTMP_ADAPTER pAd)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
==========================================================================
*/
VOID ATEAsicSwitchChannel(IN PRTMP_ADAPTER pAd)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
VOID BbpSoftReset(IN PRTMP_ADAPTER pAd)
{
#ifdef RTMP_BBP
	if (pAd->chipCap.hif_type == HIF_RTMP) {
		UCHAR BbpData = 0;
...
#ifdef MT76x2
#ifdef TXBF_SUPPORT
static VOID ITxBfBbpInit(IN PRTMP_ADAPTER pAd)
{
...
static int CheckMCSValid(IN PRTMP_ADAPTER pAd, IN UCHAR Mode, IN UCHAR Mcs)
{
	int index;
	PCHAR pRateTab = NULL;
...
static INT ATETxPwrHandler(IN PRTMP_ADAPTER pAd, IN char index)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
========================================================================
*/
static VOID ATEDisableAsicProtect(IN PRTMP_ADAPTER pAd)
{
...
==========================================================================
*/
static VOID ATEAPStop(IN PRTMP_ADAPTER pAd)
{
	BOOLEAN Cancelled;
	UINT32 Value = 0;
	INT apidx = 0;
...
static VOID RTMPStationStop(IN PRTMP_ADAPTER pAd)
{
...
static VOID RTMPStationStart(IN PRTMP_ADAPTER pAd)
{
...
static NDIS_STATUS ATESTART(IN PRTMP_ADAPTER pAd)
{
	PATE_INFO pATEInfo = &(pAd->ate);
		...
		if (pChipStruct->bBBPStoreTXCARR == TRUE) {
			...
			if (pAd->chipCap.hif_type == HIF_RTMP) {
				...
				for (bbp_id = 0; bbp_id < ATE_BBP_REG_NUM; bbp_id++)
					ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, bbp_id,
								    restore_BBP[bbp_id]);
			}
#endif /* RTMP_BBP */
	 ...
	 */
		...
		if (atemode == ATE_TXCONT) {
			if (pChipStruct->bBBPStoreTXCONT == TRUE) {
				...
				/* Restore All BBP Value */
				for (bbp_index = 0; bbp_index < ATE_BBP_REG_NUM; bbp_index++)
					ATE_BBP_IO_WRITE8_BY_REG_ID(pAd, bbp_index,
								    restore_BBP[bbp_index]);
#endif /* RTMP_BBP */
...
/*	pAd->ContinBulkIn = FALSE; */
	...
	while ((pAd->PendingRx > 0)) {
		/* Fix ATE Rx out of resource issue */
	...
	if (pATEInfo->PeriodicTimer.State == TRUE) {
		...
		RTMPInitTimer(pAd, &pAd->ate.PeriodicTimer, GET_TIMER_FUNCTION(ATEPeriodicExec),
			      pAd, TRUE);
...
static NDIS_STATUS ATESTOP(IN PRTMP_ADAPTER pAd)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static NDIS_STATUS TXCARR(IN PRTMP_ADAPTER pAd)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	UINT32 MacData = 0;
	NDIS_STATUS Status = NDIS_STATUS_SUCCESS;
...
static NDIS_STATUS TXCONT(IN PRTMP_ADAPTER pAd)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	UINT32 MacData = 0;
	NDIS_STATUS Status = NDIS_STATUS_SUCCESS;
...
static NDIS_STATUS TXCARS(IN PRTMP_ADAPTER pAd)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	UINT32 MacData = 0;
	NDIS_STATUS Status = NDIS_STATUS_SUCCESS;
...
static NDIS_STATUS TXFRAME(IN PRTMP_ADAPTER pAd)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	UINT32 MacData = 0;
#ifdef RTMP_MAC_USB
	...
	/* To avoid tx done too fast to tx process stop */
	...
	if (pATEInfo->bTxBF == TRUE) {
		...
		value32 &= ~0x330;
		value32 = (pATEInfo->TxWI.TXWI_N.eTxBF == TRUE) ? (value32 | 0x210) : value32;
...
static NDIS_STATUS RXFRAME(IN PRTMP_ADAPTER pAd)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	UINT32 MacData = 0;
	NDIS_STATUS Status = NDIS_STATUS_SUCCESS;
...
==========================================================================
*/
static NDIS_STATUS ATECmdHandler(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
...
#ifdef MT76x2
#ifdef TXBF_SUPPORT
		pAd->ceBfCertificationFlg = FALSE;
...
#ifdef CONFIG_STA_SUPPORT
	DBGPRINT(RT_DEBUG_TRACE, ("Set_ATE_SA_Proc (SA = %02x:%02x:%02x:%02x:%02x:%02x)\n",
				  pATEInfo->Addr2[0], pATEInfo->Addr2[1], pATEInfo->Addr2[2],
...
#ifdef MT76x2
	/* to sync power DAC with QA tool , reload default DAC while set channel */
	if (IS_MT76x2(pAd)) {
		UCHAR bw = 0;
		if (pAd->CommonCfg.BBPCurrentBW == BW_80)
			...
			bw = 0;
		mt76x2_tx_pwr_gain(pAd, channel, bw);
	}
#endif /* endif */
...
==========================================================================
*/
static INT ATESetAntennaTxPower(IN PRTMP_ADAPTER pAd, IN PSTRING arg, IN INT Antenna)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	CHAR TxPower;
...
#endif /* RT65xx */
			if ((TxPower > (pATEInfo->MaxTxPowerBandA))
			    || (TxPower < (pATEInfo->MinTxPowerBandA))) {
			DBGPRINT_ERR(("Set_ATE_TX_POWER%d_Proc::Out of range! (Value=%d)\n", index,
				      TxPower));
...
INT Set_ATE_SET_AUTORESP_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (value > 0) {
		DBGPRINT(RT_DEBUG_TRACE, ("Enable Auto Responder\n"));
		RTMP_IO_READ32(pAd, AUTO_RSP_CFG, &reg);
		reg = reg | 0x00000001;
		RTMP_IO_WRITE32(pAd, AUTO_RSP_CFG, reg);
#ifdef CONFIG_AP_SUPPORT
		AsicSetDevMac(pAd, pATEInfo->Addr3);
#endif /* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
		AsicSetDevMac(pAd, pATEInfo->Addr2);
#endif /* CONFIG_STA_SUPPORT */
	} else if (value == 0) {
		DBGPRINT(RT_DEBUG_TRACE, ("Disable Auto Responder\n"));
		RTMP_IO_READ32(pAd, AUTO_RSP_CFG, &reg);
		reg = reg & 0xFFFFFFFE;
		RTMP_IO_WRITE32(pAd, AUTO_RSP_CFG, reg);
...
{
	CHAR value;
	/*TODO: fix me, need find an ioctl port which can carry return value */
	value = simple_strtol(arg, 0, 10);

	DBGPRINT(RT_DEBUG_TRACE, ("%s::value=%d\n", __func__, value));
	if (pAd->E2pAccessMode == E2P_EFUSE_MODE)
		return TRUE;
	return FALSE;
}
...
INT Set_ATE_GET_KFREE_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	/*TODO: fix me, need find an ioctl port which can carry return value */
	CHAR value;
	BOOLEAN is_cal_free = FALSE;
	value = simple_strtol(arg, 0, 10);
	DBGPRINT(RT_DEBUG_TRACE, ("%s::value=%d\n", __func__, value));
	RTMP_CAL_FREE_IC_CHECK(pAd, is_cal_free);
	return is_cal_free;
}
...
VOID DefaultATEAsicExtraPowerOverMAC(IN PRTMP_ADAPTER pAd)
{
	...
	if (IS_RT5392(pAd)) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Offset =0x13D8, TxPwr = 0x%08X, ", (UINT) ExtraPwrOverTxPwrCfg8));
	}

	DBGPRINT(RT_DEBUG_TRACE,
		 ("Offset = 0x13D4, TxPwr = 0x%08X, Offset = 0x13DC, TxPwr = 0x%08X\n",
		  (UINT) ExtraPwrOverTxPwrCfg7, (UINT) ExtraPwrOverTxPwrCfg9));
}
...
static VOID ATEAsicTemperCompensation(IN PRTMP_ADAPTER pAd)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
INT Set_ATE_TX_MODE_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (phy_mode > MODE_VHT) {
		...
		DBGPRINT(RT_DEBUG_OFF,
			 ("0: CCK, 1: OFDM, 2: HT_MIX, 3: HT_GREEN_FIELD, 4: VHT.\n"));
		return FALSE;
	}
#ifdef RLT_MAC
	if (pAd->chipCap.hif_type == HIF_RLT) {
...
INT Set_ATE_TEMP_SENSOR_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (value > 0) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("ATETEMPSENSOR = FALSE , temperature sensor disabled!\n"));
	}
...
INT Set_ATE_TXBF_DIVCAL_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (value == 0 || value == 1) {
		...
		DBGPRINT(RT_DEBUG_WARN, ("Divider Cal Done:\n"
					 ...
					 (UCHAR) (divParams.gBeg[0] -
						  divParams.gEnd[0]) * 360 / 256,
					 (UCHAR) (divParams.gBeg[1] -
						  divParams.gEnd[1]) * 360 / 256,
					 (UCHAR) (divParams.gBeg[0] -
						  divParams.gBeg[1]) * 360 / 256,
					 (UCHAR) (divParams.gEnd[0] -
						  divParams.gEnd[1]) * 360 / 256));
#endif /* endif */
	}
...
INT Set_ATE_TXBF_LNACAL_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_WARN,
		 ("%s : Time consumption : %ld sec\n", __func__,
		  (stTimeChk1 - stTimeChk0) * 1000 / OS_HZ));
...
==========================================================================
*/
static BOOLEAN rtmp_ate_txbf_cal_valid_ch(IN RTMP_ADAPTER *pAd, IN UCHAR channel)
{
	BOOLEAN bValidCh;
	...
	for (i = 0; i < calLoop; i++) {
		...
		DBGPRINT(RT_DEBUG_WARN,
			 ("%s : Time consumption : switch time = %ld sec\n", __func__,
			  (stTimeChk3 - stTimeChk2) * 1000 / OS_HZ));
...
INT Set_ATE_ForceBBP_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("Set_ATE_ForceBBP_Proc:(forceBBPReg value=%d)\n", pATEInfo->forceBBPReg));
...
INT Set_ATE_Fixed_Payload_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("Set_ATE_Fixed_Payload_Proc (Fixed Payload  = %d)\n", pATEInfo->bFixedPayload));
...
INT Set_ATE_Show_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_OFF,
		 ("Addr2=%02x:%02x:%02x:%02x:%02x:%02x\n", pATEInfo->Addr2[0], pATEInfo->Addr2[1],
		  pATEInfo->Addr2[2], pATEInfo->Addr2[3], pATEInfo->Addr2[4], pATEInfo->Addr2[5]));
	DBGPRINT(RT_DEBUG_OFF,
		 ("Addr3=%02x:%02x:%02x:%02x:%02x:%02x\n", pATEInfo->Addr3[0], pATEInfo->Addr3[1],
		  pATEInfo->Addr3[2], pATEInfo->Addr3[3], pATEInfo->Addr3[4], pATEInfo->Addr3[5]));
...
INT Set_ATE_Help_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_OFF,
		 ("ATETXANT, set TX antenna. 0:all, 1:antenna one, 2:antenna two, 3:antenna three.\n"));
#else
	DBGPRINT(RT_DEBUG_OFF,
	...
	if (IS_RT3350(pAd))
		DBGPRINT(RT_DEBUG_OFF,
			 ("ATEPABIAS, set power amplifier bias for EVM, range 0~15\n"));
#endif /* RT3350 */
#ifdef RTMP_RF_RW_SUPPORT
	...
	DBGPRINT(RT_DEBUG_OFF,
		 ("ATETXSOUNDING, Sounding mode 0:none, 1:Data sounding, 2:2 stream NDP, 3:3 stream NDP.\n"));
#endif /* TXBF_SUPPORT */
#ifdef RTMP_INTERNAL_TX_ALC
...
#if defined(RT3350) || defined(RT3352)
INT RT335x2_Set_ATE_TSSI_CALIBRATION_ENABLE_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
...
INT RT335xATETssiCalibrationExtend(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	CHAR TssiRefPerChannel[CFG80211_NUM_OF_CHAN_2GHZ],
	    TssiDeltaPerChannel[CFG80211_NUM_OF_CHAN_2GHZ];
	...
	NdisCopyMemory(TssiDeltaPerChannel, pAd->ate.TssiDeltaPerChannel,
	...
	for (CurrentChannel = 2; CurrentChannel < 7; CurrentChannel++)
		TssiRefPerChannel[CurrentChannel - 1] =
		    InsertTssi(CurrentChannel, 1, 7, TssiRefPerChannel[0], TssiRefPerChannel[6]);

	/* insert channel 8 to 12 TSSI values */
	for (CurrentChannel = 8; CurrentChannel < 13; CurrentChannel++)
		TssiRefPerChannel[CurrentChannel - 1] =
		    InsertTssi(CurrentChannel, 7, 13, TssiRefPerChannel[6], TssiRefPerChannel[12]);
	...
	for (CurrentChannel = 1; CurrentChannel <= 14; CurrentChannel++) {
		TssiDeltaPerChannel[CurrentChannel - 1] =
		    TssiRefPerChannel[CurrentChannel - 1] - TssiRefPerChannel[6];
		...
		DBGPRINT(RT_DEBUG_TRACE, ("Channel %d, TSSI= 0x%x, TssiDelta=0x%x\n",
					  CurrentChannel, TssiRefPerChannel[CurrentChannel - 1],
					  TssiDeltaPerChannel[CurrentChannel - 1]));
	}
	...
	for (CurrentChannel = 3; CurrentChannel <= 14; CurrentChannel += 4) {
		EEPData =
		    (TssiDeltaPerChannel[CurrentChannel + 2] << 12) |
		    (TssiDeltaPerChannel[CurrentChannel + 1] << 8)
		    | (TssiDeltaPerChannel[CurrentChannel] << 4) |
		    TssiDeltaPerChannel[CurrentChannel - 1];
		RT28xx_EEPROM_WRITE16(pAd,
				      (EEPROM_TX_POWER_OFFSET_OVER_CH_3 +
				       ((CurrentChannel - 3) / 2)), EEPData);
		RtmpusecDelay(10);
	}
...
INT Set_ATE_Load_CR_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (arg) {
		rv = sscanf(arg, "%d-%d-%d", &(mode), (int *)&(temp_level), (int *)&(channel));
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s():mode = %d, temp_level = %d, channel = %d\n", __func__, mode,
			  temp_level, channel));
		if (rv == 3) {
				   ...
				   IN UCHAR bbpId, IN CHAR bbpVal, IN RX_CHAIN_IDX rx_ch_idx)
{
	...
	if (((pAd->MACVersion & 0xffff0000) < 0x28830000) || (pAd->Antenna.field.RxPath == 1)) {
		...
		if (rx_ch_idx & 0x01) {
			...
			DBGPRINT(RT_DEBUG_INFO,
				 ("%s(Idx):Write(R%d,val:0x%x) to Chain(0x%x, idx:%d)\n",
				  __func__, bbpId, bbpVal, rx_ch_idx, idx));
		}
		rx_ch_idx >>= 1;
		idx++;
	}
			...
			if (IS_FILE_OPEN_ERR(srcf)) {
				...
				for (i = 0; i < 0x1000; i++) {
					...
					sprintf(msg, "%d %d %d %d %d %d\n",
						SMMValued.field.LOW_BYTE1,
						SMMValued.field.LOW_BYTE0, PKTValue1d.field.BYTE3,
						PKTValue1d.field.BYTE2, PKTValue1d.field.BYTE1,
						PKTValue1d.field.BYTE0);
					sprintf(msg1, "%d %d %d %d %d %d\n",
						SMMValued.field.LOW_BYTE1,
						SMMValued.field.LOW_BYTE0, PKTValue2d.field.BYTE3,
						PKTValue2d.field.BYTE2, PKTValue2d.field.BYTE1,
						PKTValue2d.field.BYTE0);

					retval = RtmpOSFileWrite(srcf, (PSTRING) msg, strlen(msg));
					retval =
					    RtmpOSFileWrite(srcf, (PSTRING) msg1, strlen(msg1));
				}
			}
		} else {
		     ...
		     IN PVOID FunctionContext, IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if (ATE_ON(pAd)) {
		...
		{
#if defined(DYNAMIC_VGA_SUPPORT) && defined(ATE_DYNAMIC_VGA_SUPPORT)
			DBGPRINT(RT_DEBUG_ERROR,
				 ("%s:ATE_DYNAMIC_VGA_SUPPORT enabled\n", __func__));
			if (IS_MT76x2(pAd))
				update_rssi_for_channel_model(pAd);

			/* Calculated Dynamic VGA gain per second (ATE_TASK_EXEC_INTV (100ms) * ATE_TASK_EXEC_MULTIPLE (10)) */
			/* TODO: Replace modulation (%) with more effective calculation method like ++ or -- */
			if (pATEInfo->PeriodicRound % (ATE_TASK_EXEC_MULTIPLE) == 0)
			...
			if (pATEInfo->bRxFER == 1) {
				...
				if (pATEInfo->RxAntennaSel == 0)
					ate_print(KERN_EMERG
						  "ATEPeriodicExec: Rx AvgRssi0=%d, AvgRssi1=%d, AvgRssi2=%d\n\n",
						  pATEInfo->AvgRssi0, pATEInfo->AvgRssi1,
						  pATEInfo->AvgRssi2);
				else
