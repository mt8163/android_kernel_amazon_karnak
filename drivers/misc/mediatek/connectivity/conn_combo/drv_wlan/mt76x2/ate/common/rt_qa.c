...
#ifdef RTMP_BBP
static VOID BubbleSort(INT32 size, INT32 array[])
{
		...
		for (column = 0; column < 10; column++) {
			...
			if (pAd->Antenna.field.RxPath >= 2) {	/* 2R */
				...
				if (BbpR51Rssi1 == 0) {
					...
					RSSI1 =
					    (INT32) (-12 - BbpR51Rssi1 - LNA_Gain - Rssi1Offset);
				}
				RSSI[1][column] = RSSI1;
			}
			...
			if (pAd->Antenna.field.RxPath >= 3) {	/* 3R */
				...
				if (BbpR52Rssi2 == 0)
					...
					RSSI2 =
					    (INT32) (-12 - BbpR52Rssi2 - LNA_Gain - Rssi2Offset);
...
static VOID ATE_BBPRead(IN PRTMP_ADAPTER pAd, IN UCHAR offset, IN UCHAR *pValue)
{
#ifdef RTMP_BBP
	if (ATE_ON(pAd)) {
...
static VOID ATE_BBPWrite(IN PRTMP_ADAPTER pAd, IN UCHAR offset, IN UCHAR value)
{
#ifdef RTMP_BBP
	if (ATE_ON(pAd)) {
...
static BOOLEAN SyncTxRxConfig(PRTMP_ADAPTER pAd, USHORT offset, UCHAR value)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static INT ResponseToGUI(IN struct ate_racfghdr *pRaCfg,
			 IN RTMP_IOCTL_INPUT_STRUCT *pwrq, IN INT Length, IN INT Status)
{
...
static INT DO_RACFG_CMD_ATE_START(IN PRTMP_ADAPTER pAd,
				  IN RTMP_IOCTL_INPUT_STRUCT *wrq, IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static INT DO_RACFG_CMD_ATE_STOP(IN PRTMP_ADAPTER pAd,
				 IN RTMP_IOCTL_INPUT_STRUCT *wrq, IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	INT32 ret;
...
static INT DO_RACFG_CMD_RF_WRITE_ALL(IN PRTMP_ADAPTER pAd,
...
static INT DO_RACFG_CMD_E2PROM_READ16(IN PRTMP_ADAPTER pAd,
				      IN RTMP_IOCTL_INPUT_STRUCT *wrq,
				      IN struct ate_racfghdr *pRaCfg)
{
...
static INT DO_RACFG_CMD_E2PROM_WRITE16(IN PRTMP_ADAPTER pAd,
				       IN RTMP_IOCTL_INPUT_STRUCT *wrq,
				       IN struct ate_racfghdr *pRaCfg)
{
	USHORT offset, value;
...
static INT DO_RACFG_CMD_E2PROM_READ_ALL(IN PRTMP_ADAPTER pAd,
					IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					IN struct ate_racfghdr *pRaCfg)
{
					 ...
					 IN struct ate_racfghdr *pRaCfg)
{
	...
	os_alloc_mem(NULL, (UCHAR **) &buffer, (EEPROM_SIZE >> 1) * sizeof(USHORT));
...
static INT DO_RACFG_CMD_IO_READ(IN PRTMP_ADAPTER pAd,
				IN RTMP_IOCTL_INPUT_STRUCT *wrq, IN struct ate_racfghdr *pRaCfg)
{
	UINT32 offset;
	UINT32 value;
...
static INT DO_RACFG_CMD_IO_WRITE(IN PRTMP_ADAPTER pAd,
				 IN RTMP_IOCTL_INPUT_STRUCT *wrq, IN struct ate_racfghdr *pRaCfg)
{
	UINT32 offset, value;
#ifdef RLT_RF
...
static INT DO_RACFG_CMD_IO_READ_BULK(IN PRTMP_ADAPTER pAd,
				     IN RTMP_IOCTL_INPUT_STRUCT *wrq,
				     IN struct ate_racfghdr *pRaCfg)
{
	UINT32 offset;
...
static INT DO_RACFG_CMD_BBP_READ8(IN PRTMP_ADAPTER pAd,
				  IN RTMP_IOCTL_INPUT_STRUCT *wrq, IN struct ate_racfghdr *pRaCfg)
{
	USHORT offset;
	UCHAR value;
...
static INT DO_RACFG_CMD_BBP_WRITE8(IN PRTMP_ADAPTER pAd,
				   IN RTMP_IOCTL_INPUT_STRUCT *wrq, IN struct ate_racfghdr *pRaCfg)
{
	USHORT offset;
	UCHAR value;
...
static INT DO_RACFG_CMD_BBP_READ_ALL(IN PRTMP_ADAPTER pAd,
				     IN RTMP_IOCTL_INPUT_STRUCT *wrq,
				     IN struct ate_racfghdr *pRaCfg)
{
	...
	ResponseToGUI(pRaCfg, wrq, sizeof(pRaCfg->status) + pAd->chipCap.MaxNumOfBbpId + 1,
		      NDIS_STATUS_SUCCESS);

	return NDIS_STATUS_SUCCESS;
}

static INT DO_RACFG_CMD_GET_NOISE_LEVEL(IN PRTMP_ADAPTER pAd,
					IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					IN struct ate_racfghdr *pRaCfg)
{
	UCHAR channel;
	...
	memcpy_exl(pAd, (UCHAR *) pRaCfg->data, (UCHAR *) &(buffer[0][0]),
		   (sizeof(INT32) * 3 * 10));

	ResponseToGUI(pRaCfg, wrq, sizeof(pRaCfg->status) + (sizeof(INT32) * 3 * 10),
		      NDIS_STATUS_SUCCESS);
...
static INT DO_RACFG_CMD_GET_COUNTER(IN PRTMP_ADAPTER pAd,
				    IN RTMP_IOCTL_INPUT_STRUCT *wrq,
				    IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
#ifdef MT76x2
static INT DO_RACFG_CMD_ATE_MT76x2_Calibration(IN PRTMP_ADAPTER pAd,
					       IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					       IN struct ate_racfghdr *pRaCfg)
{
	USHORT length;
	UINT32 size, cal_id = 0, param0 = 0;
...
static INT DO_RACFG_CMD_CLEAR_COUNTER(IN PRTMP_ADAPTER pAd,
				      IN RTMP_IOCTL_INPUT_STRUCT *wrq,
				      IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static INT DO_RACFG_CMD_TX_START(IN PRTMP_ADAPTER pAd,
				 IN RTMP_IOCTL_INPUT_STRUCT *wrq, IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	USHORT *p;
...
/*		p = (USHORT *)(&pRaCfg->data[TXWISize + TxInfoSize + 2]); */
		...
		NdisMoveMemory(&pATEInfo->Header, pRaCfg->data + (TXWISize + TxInfoSize + 2 * 3),
			       pATEInfo->HLen);

		pATEInfo->PLen =
		    OS_NTOHS(pRaCfg->length) - (pATEInfo->HLen + (TXWISize + TxInfoSize + 2 * 4));
		...
		NdisMoveMemory(&pATEInfo->Pattern,
			       pRaCfg->data + (TXWISize + TxInfoSize + 2 * 3) + pATEInfo->HLen,
			       pATEInfo->PLen);
...
static INT DO_RACFG_CMD_GET_TX_STATUS(IN PRTMP_ADAPTER pAd,
				      IN RTMP_IOCTL_INPUT_STRUCT *wrq,
				      IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static INT DO_RACFG_CMD_TX_STOP(IN PRTMP_ADAPTER pAd,
				IN RTMP_IOCTL_INPUT_STRUCT *wrq, IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static INT DO_RACFG_CMD_RX_START(IN PRTMP_ADAPTER pAd,
				 IN RTMP_IOCTL_INPUT_STRUCT *wrq, IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static INT DO_RACFG_CMD_RX_STOP(IN PRTMP_ADAPTER pAd,
				IN RTMP_IOCTL_INPUT_STRUCT *wrq, IN struct ate_racfghdr *pRaCfg)
{
...
static INT DO_RACFG_CMD_ATE_START_TX_CARRIER(IN PRTMP_ADAPTER pAd,
					     IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					     IN struct ate_racfghdr *pRaCfg)
{
...
static INT DO_RACFG_CMD_ATE_START_TX_CONT(IN PRTMP_ADAPTER pAd,
					  IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					  IN struct ate_racfghdr *pRaCfg)
{
...
static INT DO_RACFG_CMD_ATE_START_TX_FRAME(IN PRTMP_ADAPTER pAd,
					   IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					   IN struct ate_racfghdr *pRaCfg)
{
...
static INT DO_RACFG_CMD_ATE_START_TX_FRAME_V2(IN PRTMP_ADAPTER pAd,
					      IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					      IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("DO_RACFG_CMD_ATE_START_TX_FRAME_V2 (TxCount = %d)\n", pATEInfo->TxCount));
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("DO_RACFG_CMD_ATE_START_TX_FRAME_V2 (TxLength = %d)\n", pATEInfo->TxLength));
...
static INT DO_RACFG_CMD_ATE_SET_IPG(IN PRTMP_ADAPTER pAd,
				    IN RTMP_IOCTL_INPUT_STRUCT *wrq,
				    IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static INT DO_RACFG_CMD_ATE_SET_BW(IN PRTMP_ADAPTER pAd,
				   IN RTMP_IOCTL_INPUT_STRUCT *wrq, IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
	STRING str[LEN_OF_ARG];
...
static INT DO_RACFG_CMD_ATE_SET_TX_POWER0(IN PRTMP_ADAPTER pAd,
					  IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					  IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
...
static INT DO_RACFG_CMD_ATE_SET_TX_POWER1(IN PRTMP_ADAPTER pAd,
					  IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					  IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
...
static INT DO_RACFG_CMD_ATE_SET_TX_POWER2(IN PRTMP_ADAPTER pAd,
					  IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					  IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
...
static INT DO_RACFG_CMD_ATE_TX_POWER_EVAL(IN PRTMP_ADAPTER pAd,
					  IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					  IN struct ate_racfghdr *pRaCfg)
{
...
static INT DO_RACFG_CMD_ATE_SET_FREQ_OFFSET(IN PRTMP_ADAPTER pAd,
					    IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					    IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
...
static INT DO_RACFG_CMD_ATE_SET_TSSI_ON_OFF(IN PRTMP_ADAPTER pAd,
					    IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					    IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static INT DO_RACFG_CMD_ATE_GET_STATISTICS(IN PRTMP_ADAPTER pAd,
					   IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					   IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	...
	memcpy_exl(pAd, &pRaCfg->data[12], (UCHAR *) &pAd->WlanCounters.RTSSuccessCount.u.LowPart,
		   4);
	memcpy_exl(pAd, &pRaCfg->data[16], (UCHAR *) &pAd->WlanCounters.RTSFailureCount.u.LowPart,
		   4);
	memcpy_exl(pAd, &pRaCfg->data[20],
		   (UCHAR *) &pAd->WlanCounters.ReceivedFragmentCount.QuadPart, 4);
	memcpy_exl(pAd, &pRaCfg->data[24], (UCHAR *) &pAd->WlanCounters.FCSErrorCount.u.LowPart,
		   4);
	memcpy_exl(pAd, &pRaCfg->data[28], (UCHAR *) &pAd->Counters8023.RxNoBuffer, 4);
	memcpy_exl(pAd, &pRaCfg->data[32],
		   (UCHAR *) &pAd->WlanCounters.FrameDuplicateCount.u.LowPart, 4);
...
static INT DO_RACFG_CMD_ATE_RESET_COUNTER(IN PRTMP_ADAPTER pAd,
					  IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					  IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static INT DO_RACFG_CMD_ATE_SEL_TX_ANTENNA(IN PRTMP_ADAPTER pAd,
					   IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					   IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
...
static INT DO_RACFG_CMD_ATE_SEL_RX_ANTENNA(IN PRTMP_ADAPTER pAd,
					   IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					   IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
...
static INT DO_RACFG_CMD_ATE_SET_PREAMBLE(IN PRTMP_ADAPTER pAd,
					 IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					 IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
...
static INT DO_RACFG_CMD_ATE_SET_CHANNEL(IN PRTMP_ADAPTER pAd,
					IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
...
static INT DO_RACFG_CMD_ATE_SET_ADDR1(IN PRTMP_ADAPTER pAd,
				      IN RTMP_IOCTL_INPUT_STRUCT *wrq,
				      IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static INT DO_RACFG_CMD_ATE_SET_ADDR2(IN PRTMP_ADAPTER pAd,
				      IN RTMP_IOCTL_INPUT_STRUCT *wrq,
				      IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static INT DO_RACFG_CMD_ATE_SET_ADDR3(IN PRTMP_ADAPTER pAd,
				      IN RTMP_IOCTL_INPUT_STRUCT *wrq,
				      IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static INT DO_RACFG_CMD_ATE_SET_FIXED_PAYLOAD(IN PRTMP_ADAPTER pAd,
					      IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					      IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
...
static INT DO_RACFG_CMD_ATE_SET_RATE(IN PRTMP_ADAPTER pAd,
				     IN RTMP_IOCTL_INPUT_STRUCT *wrq,
				     IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
...
#ifdef DOT11_VHT_AC
static INT DO_RACFG_CMD_ATE_SET_VHT_NSS(IN PRTMP_ADAPTER pAd,
					IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
...
static INT DO_RACFG_CMD_ATE_SET_TX_FRAME_LEN(IN PRTMP_ADAPTER pAd,
					     IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					     IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
...
static INT DO_RACFG_CMD_ATE_SET_TX_FRAME_COUNT(IN PRTMP_ADAPTER pAd,
					       IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					       IN struct ate_racfghdr *pRaCfg)
{
	USHORT value = 0;
...
static INT DO_RACFG_CMD_ATE_START_RX_FRAME(IN PRTMP_ADAPTER pAd,
					   IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					   IN struct ate_racfghdr *pRaCfg)
{
	DBGPRINT(RT_DEBUG_TRACE, ("RACFG_CMD_RX_START\n"));
...
static INT DO_RACFG_CMD_ATE_E2PROM_READ_BULK(IN PRTMP_ADAPTER pAd,
					     IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					     IN struct ate_racfghdr *pRaCfg)
{
	USHORT offset;
	...
	os_alloc_mem(NULL, (UCHAR **) &buffer, (EEPROM_SIZE >> 1) * sizeof(USHORT));
	if (buffer == NULL) {
		DBGPRINT_ERR(("%s - Error for allocate size %zu\n", __func__,
			      (EEPROM_SIZE >> 1) * sizeof(USHORT)));
		return NDIS_STATUS_FAILURE;
	}
...
static INT DO_RACFG_CMD_ATE_E2PROM_WRITE_BULK(IN PRTMP_ADAPTER pAd,
					      IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					      IN struct ate_racfghdr *pRaCfg)
{
	USHORT offset;
	...
	if ((offset + len) <= EEPROM_SIZE) {
		...
		if (pAd->E2pAccessMode == E2P_FLASH_MODE) {
			...
			if (pAd->chipCap.ee_inited) {
				memcpy_exs(pAd, (UCHAR *) (pAd->eebuf + offset),
					   (UCHAR *) pRaCfg->data + 2, len);

#ifdef MULTIPLE_CARD_SUPPORT
				DBGPRINT(RT_DEBUG_TRACE,
					 ("rtmp_ee_flash_write:pAd->MC_RowID = %d\n",
					  pAd->MC_RowID));
				DBGPRINT(RT_DEBUG_TRACE,
					 ("E2P_OFFSET = 0x%08x\n",
					  pAd->E2P_OFFSET_IN_FLASH[pAd->MC_RowID]));
				if ((pAd->E2P_OFFSET_IN_FLASH[pAd->MC_RowID] == 0x48000)
				    || (pAd->E2P_OFFSET_IN_FLASH[pAd->MC_RowID] == 0x40000))
					RtmpFlashWrite(pAd->eebuf,
						       pAd->E2P_OFFSET_IN_FLASH[pAd->MC_RowID],
						       EEPROM_SIZE);
#else
			...
			rt_ee_write_bulk(pAd, (USHORT *) (((UCHAR *) buffer) + offset), offset,
					 len);
	} else {
...
static INT DO_RACFG_CMD_ATE_IO_WRITE_BULK(IN PRTMP_ADAPTER pAd,
					  IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					  IN struct ate_racfghdr *pRaCfg)
{
	...
	start = sizeof(offset) - sizeof(pRaCfg->status) + sizeof(len);

/* DBGPRINT(RT_DEBUG_WARN,("\n\n")); */
	for (pos = 0; pos < len; pos += 4) {
		if (start + pos + 4 > sizeof(pRaCfg->data))	/* Reserved at least 4 byte available data */
			break;
		memcpy_exl(pAd, (UCHAR *) &value, pRaCfg->data + start + pos, 4);
		RTMP_IO_WRITE32(pAd, ((offset + pos) & (0xffff)), value);
/* DBGPRINT(RT_DEBUG_WARN,("mac w 0x%04X=0x%08X\n", offset + pos, value)); */
		DBGPRINT(RT_DEBUG_WARN,
			 ("iwpriv %s0 mac %04X=%08X\n", INF_MAIN_DEV_NAME, offset + pos, value));
	}
/* DBGPRINT(RT_DEBUG_WARN,("\n\n")); */
...
static INT DO_RACFG_CMD_ATE_BBP_READ_BULK(IN PRTMP_ADAPTER pAd,
					  IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					  IN struct ate_racfghdr *pRaCfg)
{
	USHORT offset;
...
static INT DO_RACFG_CMD_ATE_BBP_WRITE_BULK(IN PRTMP_ADAPTER pAd,
					   IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					   IN struct ate_racfghdr *pRaCfg)
{
	USHORT offset;
	...
	DBGPRINT(RT_DEBUG_WARN, ("\n\n"));
	for (pos = offset; pos < (offset + len); pos++) {
		curr_pos = (pos - offset) + sizeof(len);
		if (curr_pos >= sizeof(pRaCfg->data))
			break;
		value = pRaCfg->data + curr_pos;
...
#ifndef RLT_RF
static INT DO_RACFG_CMD_ATE_RF_READ_BULK(IN PRTMP_ADAPTER pAd,
					 IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					 IN struct ate_racfghdr *pRaCfg)
{
	USHORT offset;
...
static INT DO_RACFG_CMD_ATE_RF_WRITE_BULK(IN PRTMP_ADAPTER pAd,
					  IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					  IN struct ate_racfghdr *pRaCfg)
{
	USHORT offset;
...
#ifdef RLT_RF
static INT DO_RACFG_CMD_ATE_RF_READ_BULK_BANK(IN PRTMP_ADAPTER pAd,
					      IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					      IN struct ate_racfghdr *pRaCfg)
{
	USHORT offset;
...
static INT DO_RACFG_CMD_ATE_RF_WRITE_BULK_BANK(IN PRTMP_ADAPTER pAd,
					       IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					       IN struct ate_racfghdr *pRaCfg)
{
	USHORT offset;
...
#ifdef MT_RF
static INT DO_RACFG_CMD_ATE_MT_RF_READ_BULK_BANK(IN PRTMP_ADAPTER pAd,
						 IN RTMP_IOCTL_INPUT_STRUCT *wrq,
						 IN struct ate_racfghdr *pRaCfg)
{
	UINT32 offset;
...
/* , bank, pos, pRaCfg->data[pos - offset])); */
		DBGPRINT(RT_DEBUG_TRACE,
			 ("rf bank%u r 0x%04X=0x%04X\n", bank, pos,
			  *(UINT32 *) &pRaCfg->data[pos - offset]));
	}
/* DBGPRINT(RT_DEBUG_WARN,("\n\n")); */
...
static INT DO_RACFG_CMD_ATE_MT_RF_WRITE_BULK_BANK(IN PRTMP_ADAPTER pAd,
						  IN RTMP_IOCTL_INPUT_STRUCT *wrq,
						  IN struct ate_racfghdr *pRaCfg)
{
	UINT32 offset;
...
/* DBGPRINT(RT_DEBUG_WARN,("\n\n")); */
	for (pos = offset; pos < (offset + len); pos += 4) {
		value =
		    (UINT32 *) (pRaCfg->data + 2 + sizeof(offset) + sizeof(len) + (pos - offset));
...
static INT DO_RACFG_CMD_TX_START_V2(IN PRTMP_ADAPTER pAd,
				    IN RTMP_IOCTL_INPUT_STRUCT *wrq,
				    IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
/*		RTMPDescriptorEndianChange((PUCHAR) &pATEInfo->TxInfo, TYPE_TXINFO); */
		...
		if (TXWISize > sizeof(pATEInfo->TxWI)) {
			DBGPRINT_ERR(("%s : pATEInfo->TxWILen > size of TXWI\n", __func__));
			DBGPRINT_ERR(("pATEInfo->TxWILen = %d\n", pATEInfo->TxWILen));
			err = 2;
		...
		NdisMoveMemory(&pATEInfo->Header, pRaCfg->data + (4 + 2 + TXWISize + 4 + 2 + 2),
			       pATEInfo->HLen);
		pATEInfo->PLen =
		    OS_NTOHS(pRaCfg->length) - (pATEInfo->HLen +
						(4 + 2 + TXWISize + 4 + 2 + 2 + 2));
		...
		NdisMoveMemory(&pATEInfo->Pattern,
			       pRaCfg->data + (4 + 2 + TXWISize + 4 + 2 + 2) + pATEInfo->HLen,
			       pATEInfo->PLen);
...
#ifdef TXBF_SUPPORT
static INT DO_RACFG_CMD_ATE_TXBF_DUT_INIT(IN PRTMP_ADAPTER pAd,
					  IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					  IN struct ate_racfghdr *pRaCfg)
{
...
static INT DO_RACFG_CMD_ATE_TXBF_LNA_CAL(IN PRTMP_ADAPTER pAd,
					 IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					 IN struct ate_racfghdr *pRaCfg)
{
	USHORT band;
...
static INT DO_RACFG_CMD_ATE_TXBF_DIV_CAL(IN PRTMP_ADAPTER pAd,
					 IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					 IN struct ate_racfghdr *pRaCfg)
{
	USHORT band;
...
static INT DO_RACFG_CMD_ATE_TXBF_PHASE_CAL(IN PRTMP_ADAPTER pAd,
					   IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					   IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
...
static INT DO_RACFG_CMD_ATE_TXBF_GOLDEN_INIT(IN PRTMP_ADAPTER pAd,
					     IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					     IN struct ate_racfghdr *pRaCfg)
{
	SHORT value = 0;
...
static INT DO_RACFG_CMD_ATE_TXBF_VERIFY(IN PRTMP_ADAPTER pAd,
					IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static INT DO_RACFG_CMD_ATE_TXBF_VERIFY_NOCOMP(IN PRTMP_ADAPTER pAd,
					       IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					       IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
static INT DO_RACFG_CMD_ATE_GET_HW_COUNTER(IN PRTMP_ADAPTER pAd,
					   IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					   IN struct ate_racfghdr *pRaCfg)
{
...
static INT32 DO_RACFG_CMD_ATE_SHOW_PARAM(IN PRTMP_ADAPTER pAd,
					 IN RTMP_IOCTL_INPUT_STRUCT *wrq,
					 IN struct ate_racfghdr *pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
...
typedef INT(*RACFG_CMD_HANDLER) (IN PRTMP_ADAPTER pAd,
				 IN RTMP_IOCTL_INPUT_STRUCT *wrq, IN struct ate_racfghdr *pRaCfg);
...
static INT32 RACfgCMDHandler(IN PRTMP_ADAPTER pAd,
			     IN RTMP_IOCTL_INPUT_STRUCT *wrq, IN pRACFGHDR pRaCfg)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	INT32 Status = NDIS_STATUS_SUCCESS;
	USHORT Command_Id;
...
INT RtmpDoAte(IN PRTMP_ADAPTER pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq, IN PSTRING wrq_name)
{
	INT32 Status = NDIS_STATUS_SUCCESS;
...
VOID ATE_QA_Statistics(IN RTMP_ADAPTER *pAd,
		       IN RXWI_STRUCT *pRxWI, IN RXINFO_STRUCT *pRxInfo, IN PHEADER_802_11 pHeader)
{
	PATE_INFO pATEInfo = &(pAd->ate);
	CHAR rssi[3] = { 0 };
	UCHAR snr[3] = { 0 };
...
INT Set_TxStop_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
...
INT Set_RxStop_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
