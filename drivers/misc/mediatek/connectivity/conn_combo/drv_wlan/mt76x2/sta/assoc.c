...
VOID MlmeAssocReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	/* check sanity first */
		...
		VarIesOffset += 1;
		NdisMoveMemory(pAd->StaCfg.ReqVarIEs + VarIesOffset, pAd->MlmeAux.Ssid,
			       pAd->MlmeAux.SsidLen);
		...
		VarIesOffset += 1;
		NdisMoveMemory(pAd->StaCfg.ReqVarIEs + VarIesOffset, pAd->MlmeAux.SupRate,
		...
		MakeOutgoingFrame(pOutBuffer, &FrameLen,
				  ...
				  pAd->MlmeAux.SupRateLen, pAd->MlmeAux.SupRate, END_OF_ARGS);
		...
		if ((pAd->MlmeAux.HtCapabilityLen > 0)
			...
			*(USHORT *) (&HtCapabilityTmp.ExtHtCapInfo) =
			...
			if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode) &&
			    (pAd->MlmeAux.Channel > 14) && (pAd->MlmeAux.vht_cap_len)
			    ) {
				...
				Idx =
				    BssTableSearch(&pAd->ScanTab, pAd->MlmeAux.Bssid,
						   pAd->MlmeAux.Channel);
				if (Idx != BSS_NOT_FOUND && (Idx < MAX_LEN_OF_BSS_TABLE)) {
					pAd->BeaconSndDimensionFlag = 0;
					if (pAd->ScanTab.BssEntry[Idx].vht_cap_ie.vht_cap.
					    num_snd_dimension >= 2) {
		...
		if (((wdev->AuthMode == Ndis802_11AuthModeWPAPSK) ||
...
#endif /* WPA_SUPPLICANT_SUPPORT */
			{
				...
				if (wdev->AuthMode == Ndis802_11AuthModeWPA2) {
					...
					for (idx = 0; idx < PMKID_NO; idx++) {
						if (NdisEqualMemory
						    (ApAddr, &pAd->StaCfg.SavedPMK[idx].BSSID, 6)) {
							FoundPMK = TRUE;
							break;
						}
					}
					...
					if (FoundPMK) {
						if (pAd->StaCfg.RSNIE_Len <=
						    (MAX_LEN_OF_RSNIE - 19)) {
							/* Set PMK number */
							*(PUSHORT) &pAd->StaCfg.RSN_IE[pAd->StaCfg.
											RSNIE_Len]
							    = 1;
							NdisMoveMemory(&pAd->StaCfg.
								       RSN_IE[pAd->StaCfg.
									      RSNIE_Len + 2],
								       &pAd->StaCfg.SavedPMK[idx].
								       PMKID, 16);
			 ...
			 */
/* #ifdef SIOCSIWGENIE */
			if ((pAd->StaCfg.wpa_supplicant_info.
...
/* #endif */
#endif /* WPA_SUPPLICANT_SUPPORT */
			{
				...
				NdisMoveMemory(pAd->StaCfg.ReqVarIEs + VarIesOffset,
					       pAd->StaCfg.RSN_IE, pAd->StaCfg.RSNIE_Len);
...
/* #ifdef SIOCSIWGENIE */
		...
		if ((pAd->StaCfg.wpa_supplicant_info.WpaSupplicantUP & WPA_SUPPLICANT_ENABLE) &&
			...
			DBGPRINT(RT_DEBUG_TRACE, ("%s %d  WpaSupplicantUP 0x%x ie_len\n",
					__func__, __LINE__,
...
/* #endif */
		...
		if (pAd->StaCfg.WfdCfg.bSuppInsertWfdIe) {
			...
			WfdIeBitmap =
			    (0x1 << SUBID_WFD_DEVICE_INFO) | (0x1 << SUBID_WFD_ASSOCIATED_BSSID) |
...
VOID MlmeDisassocReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
		...
		if (pAd->StaCfg.bRadio == TRUE) {
			...
			for (idx = 0; idx < MAX_NUM_OF_TDLS_ENTRY; idx++) {
				...
				if (pTDLS->Valid && (pTDLS->Status >= TDLS_MODE_CONNECTED)) {
					...
					MacTableDeleteEntry(pAd, pTDLS->MacTabMatchWCID,
							    pTDLS->MacAddr);
				} else if (pTDLS->Valid) {
					pTDLS->Status = TDLS_MODE_NONE;
					pTDLS->Valid = FALSE;
	...
	WAPI_InternalCmdAction(pAd,
			       pAd->StaCfg.AuthMode, BSS0, pDisassocReq->Addr, WAI_MLME_DISCONNECT);
#endif /* WAPI_SUPPORT */
	...
	MgtMacHeaderInit(pAd, &DisassocHdr, SUBTYPE_DISASSOC, 0, pDisassocReq->Addr, pAd->CurrentAddress, pDisassocReq->Addr);	/* patch peap ttls switching issue */
	MakeOutgoingFrame(pOutBuffer, &FrameLen,
...
VOID PeerAssocRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (PeerAssocRspSanity(pAd, Elem->Msg, Elem->MsgLen,
			       ...
			       &NewExtChannelOffset, &EdcaParm, &ExtCapInfo, &CkipFlag, ie_list)) {
		/* The frame is for me ? */
		if (MAC_ADDR_EQUAL(Addr2, pAd->MlmeAux.Bssid)) {
			...
			if (Status == MLME_SUCCESS) {
				...
				MaxSupportedRateIn500Kbps =
				    dot11_max_sup_rate(SupRateLen, &SupRate[0], ExtRateLen,
						       &ExtRate[0]);
				/* go to procedure listed on page 376 */
				AssocPostProc(pAd, Addr2, CapabilityInfo, Aid,
					      SupRate, SupRateLen, ExtRate,
					      ExtRateLen, &EdcaParm,
					      ie_list, &HtCapability, HtCapabilityLen, &AddHtInfo);
				...
				StaUpdateMacTableEntry(pAd,
						       ...
						       AddHtInfoLen, ie_list, CapabilityInfo);
				...
				if (pAd->StaCfg.BssType == BSS_INFRA) {
					if (pAd->MlmeAux.BssIdx < pAd->MlmeAux.SsidBssTab.BssNr) {
						...
						pInBss =
						    &pAd->MlmeAux.SsidBssTab.BssEntry[pAd->MlmeAux.
										      BssIdx];
						if (CLIENT_STATUS_TEST_FLAG
						    (pInBss, fCLIENT_STATUS_PMF_CAPABLE))
			...
			if (Status == MLME_SUCCESS) {
				...
				if (pFrame && ie_len > 0) {
					...
					os_alloc_mem(pAd, (UCHAR **) &(cfg80211_ctrl->pAssocRspIe),
						     ie_len);
					if (cfg80211_ctrl->pAssocRspIe) {
...
VOID PeerReassocRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (PeerAssocRspSanity(pAd, Elem->Msg, Elem->MsgLen, Addr2,
		...
		DBGPRINT(RT_DEBUG_TRACE, ("REASSOC - %s() sanity check fail\n", __func__));
	}
...
{				/* AP might use this additional ht info IE */
	...
	if (Idx == BSS_NOT_FOUND) {
		...
		if ((wdev->AuthMode >= Ndis802_11AuthModeWPA && (Idx < MAX_LEN_OF_BSS_TABLE)
...
VOID PeerDisassocAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if (PeerDisassocSanity(pAd, Elem->Msg, Elem->MsgLen, Addr2, &Reason)) {
		DBGPRINT(RT_DEBUG_ERROR, ("ASSOC - PeerDisassocAction() Reason = %d\n", Reason));
		if (INFRA_ON(pAd)
			...
			WAPI_InternalCmdAction(pAd,
					       pAd->StaCfg.AuthMode,
					       BSS0, Addr2, WAI_MLME_DISCONNECT);
#endif /* WAPI_SUPPORT */
...
VOID Cls3errAction(RTMP_ADAPTER *pAd, UCHAR *pAddr)
{
	...
	MgtMacHeaderInit(pAd, &DisassocHdr, SUBTYPE_DISASSOC, 0, pAddr, pAd->CurrentAddress, pAd->CommonCfg.Bssid);	/* patch peap ttls switching issue */
	MakeOutgoingFrame(pOutBuffer, &FrameLen,
			   ...
			   IN STATE_MACHINE *S, OUT STATE_MACHINE_FUNC Trans[])
{
	...
	StateMachineSetAction(S, ASSOC_WAIT_RSP, MT2_MLME_DISASSOC_REQ, (STATE_MACHINE_FUNC)
	...
	StateMachineSetAction(S, REASSOC_WAIT_RSP, MT2_MLME_REASSOC_REQ,
			      (STATE_MACHINE_FUNC) InvalidStateWhenReassoc);
	StateMachineSetAction(S, REASSOC_WAIT_RSP, MT2_MLME_DISASSOC_REQ, (STATE_MACHINE_FUNC)
			      InvalidStateWhenDisassociate);
	...
	StateMachineSetAction(S, DISASSOC_WAIT_RSP, MT2_MLME_REASSOC_REQ,
			      (STATE_MACHINE_FUNC) InvalidStateWhenReassoc);
	StateMachineSetAction(S, DISASSOC_WAIT_RSP, MT2_MLME_DISASSOC_REQ, (STATE_MACHINE_FUNC)
			      InvalidStateWhenDisassociate);
