...
VOID RTMPReportMicError(RTMP_ADAPTER *pAd, PCIPHER_KEY pWpaKey)
{
	...
	MlmeEnqueue(pAd,
		    ...
		    OID_802_11_MIC_FAILURE_REPORT_FRAME, 1, &unicastKey, 0);
...
VOID WpaMicFailureReportFrame(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	*((USHORT *) &pPacket->KeyDesc.KeyInfo) =
	...
	if (pAd->StaCfg.wdev.WepStatus == Ndis802_11AESEnable) {	/* AES */
		UCHAR digest[20] = { 0 };
		RT_HMAC_SHA1(pAd->StaCfg.PTK, LEN_PTK_KCK, pOutBuffer, FrameLen, digest,
			     SHA1_DIGEST_SIZE);
		NdisMoveMemory(Mic, digest, LEN_KEY_DESC_MIC);
	} else {		/* TKIP */
		RT_HMAC_MD5(pAd->StaCfg.PTK, LEN_PTK_KCK, pOutBuffer, FrameLen, Mic,
			    MD5_DIGEST_SIZE);
	}
...
VOID WpaStaPairwiseKeySetting(RTMP_ADAPTER *pAd)
{
	...
	NdisMoveMemory(pEntry->PairwiseKey.TxMic, &pAd->StaCfg.PTK[48 + LEN_TKIP_MIC],
		       LEN_TKIP_MIC);
	pEntry->PairwiseKey.CipherAlg = pSharedKey->CipherAlg;

	/* Update pairwise key information to ASIC Shared Key Table      */
	RTMP_ASIC_SHARED_KEY_TABLE(pAd, BSS0, 0, pSharedKey);

	/* Update ASIC WCID attribute table and IVEIV table */
	RTMP_SET_WCID_SEC_INFO(pAd, BSS0, 0, pSharedKey->CipherAlg, BSSID_WCID, SHAREDKEYTABLE);
...
VOID WpaStaGroupKeySetting(RTMP_ADAPTER *pAd)
{
	...
	RTMP_ASIC_SHARED_KEY_TABLE(pAd, BSS0, pAd->StaCfg.wdev.DefaultKeyId, pSharedKey);
