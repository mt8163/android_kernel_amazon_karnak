...
static struct {
	...
	"SkuProfileInfo", RTMPShowSingleSKUParameters}, {
	"SkuLoadProfile", RTMPLoadSKUProfile},
#endif /* endif */
	{
	...
	"wow_ip", Set_WOW_IP}, {
	"wow_awake_for_rc", Set_WOW_Awake}, {
	"wow_suspend_1x1", Set_WOW_Suspend1x1},
	...
	{"agc_th", Set_Agc_Threshold},
	{"agc_lowerbound", Set_Agc_Lowerbound},
#endif /* RT_CFG80211_SUPPORT */
	{"P2PIdleTimeout", Set_Sta_IdleTimeout},
	{"ScanChTime", Set_Scan_FastScanChTime},
	{"OpChTime", Set_Scan_StayOpChTime},
	{"ScanDisable", Set_Scan_Disable},
	{
	NULL,}
};
...
INT RTMPSTAPrivIoctlSet(IN RTMP_ADAPTER *pAd, IN PSTRING SetProcName, IN PSTRING ProcArg)
{
	...
	for (PRTMP_PRIVATE_SET_PROC = RTMP_PRIVATE_SUPPORT_PROC; PRTMP_PRIVATE_SET_PROC->name;
		...
		if (strcmp(SetProcName, PRTMP_PRIVATE_SET_PROC->name) == 0) {
			if (!PRTMP_PRIVATE_SET_PROC->set_proc(pAd, ProcArg)) {	/*FALSE:Set private failed then return Invalid argument */
	...
	IN	PSTRING			arg)
{
	...
	if (kstrtol(arg, 10, &APSDCapable) == 0) {
		if (APSDCapable == 1)
...
#endif /* endif */
	start = (UINT32) simple_strtol(arg, 0, 10);
	if (start > 0) {
		...
		if (pAd->Mlme.channel_1st_channel <= 14) {
			...
			tssi_slope_offset_1st &= ~TSSI_PARAM2_SLOPE0_MASK;
			tssi_slope_offset_1st |= TSSI_PARAM2_SLOPE0(cap->tssi_0_slope_a_band[(INT32)
											     get_chl_grp
											     (pAd->
											      Mlme.channel_1st_channel)]);
			tssi_slope_offset_1st &= ~TSSI_PARAM2_SLOPE1_MASK;
			tssi_slope_offset_1st |= TSSI_PARAM2_SLOPE1(cap->tssi_1_slope_a_band[(INT32)
											     get_chl_grp
											     (pAd->
											      Mlme.channel_1st_channel)]);
			tssi_slope_offset_1st &= ~TSSI_PARAM2_OFFSET0_MASK;
		...
		if (pAd->Mlme.channel_2nd_channel <= 14) {
			...
			tssi_slope_offset_2nd &= ~TSSI_PARAM2_SLOPE0_MASK;
			tssi_slope_offset_2nd |= TSSI_PARAM2_SLOPE0(cap->tssi_0_slope_a_band[(INT32)
											     get_chl_grp
											     (pAd->
											      Mlme.channel_2nd_channel)]);
			tssi_slope_offset_2nd &= ~TSSI_PARAM2_SLOPE1_MASK;
			tssi_slope_offset_2nd |= TSSI_PARAM2_SLOPE1(cap->tssi_1_slope_a_band[(INT32)
											     get_chl_grp
											     (pAd->
											      Mlme.channel_2nd_channel)]);
			tssi_slope_offset_2nd &= ~TSSI_PARAM2_OFFSET0_MASK;
...
VOID StaSiteSurvey(IN PRTMP_ADAPTER pAd, IN PNDIS_802_11_SSID pSsid, IN UCHAR ScanType)
{
	...
	if (SCAN_ONLINE && pAd->StaCfg.bImprovedScan &&
		RTMP_CFG80211_VIF_P2P_GO_ON(pAd) && CFG80211DRV_OpsScanRunning(pAd)) {
		ScanType = FAST_SCAN_ACTIVE;
		DBGPRINT(RT_DEBUG_TRACE, ("%s SCAN_ONLINE, FAST_SCAN_ACTIVE\n", __func__));
	}
...
INT Set_WOW_Awake(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	ULONG val = 0;
	INT ret = TRUE;
	ret = kstrtol(arg, 10, &val);
	if (ret < 0)
		return FALSE;

	pAd->WOW_Cfg.awakeTime = (UCHAR)val;

	DBGPRINT(RT_DEBUG_ERROR, ("%s, wakeTime:%d\n", __func__, (UCHAR)val));
	return TRUE;
}
...
INT Set_WOW_Suspend1x1(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (ret < 0)
		return FALSE;
	if (val)
		pAd->WOW_Cfg.extMode |= WOW_1X1_ENABLE;
	else
		pAd->WOW_Cfg.extMode &= ~WOW_1X1_ENABLE;

	DBGPRINT(RT_DEBUG_ERROR, ("%s, ext Mode:%x\n", __func__, pAd->WOW_Cfg.extMode));
	return TRUE;
}
/* set WOW enable */
...
INT Set_WOW_Hold(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("WOW_Hold = %d ms\n", pAd->WOW_Cfg.nHoldTime));
...
INT Set_WOW_HighActive(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("WOW_HighActive = %d\n", pAd->WOW_Cfg.bHighActive));
...
INT Set_WOW_TcpPort_v4(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	while ((this_char = strsep((char **)&arg, ",")) != NULL && i < WOW_MAX_PORT_NUM_TCPV4) {
		...
		port[i] = simple_strtol(this_char, 0, 10);
...
INT Set_WOW_IP(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	while ((this_char = strsep((char **)&arg, ".")) != NULL && i < 4) {
		pAd->WOW_Cfg.WowIP[i] = simple_strtol(this_char, 0, 10);
...
INT Set_UsbWOWSuspend(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (Flag == TRUE) {
		...
		pAd->WOW_Cfg.extMode |= WOW_FAKE_SUSPEND;
		DBGPRINT(RT_DEBUG_ERROR,
...
INT Set_UsbWOWResume(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	RTMP_DRIVER_ADAPTER_RT28XX_WOW_STATUS(pAd, &Flag);
	if (Flag == TRUE) {
		DBGPRINT(RT_DEBUG_TRACE, ("Resuming to Normal mode...\n"));
		...
		pAd->WOW_Cfg.extMode &= ~WOW_FAKE_SUSPEND;
		pAd->WOW_Cfg.extMode &= ~WOW_NEED_RESUME;
	}
	return TRUE;
#else
	DBGPRINT(RT_DEBUG_ERROR, ("Driver does not support WOW!\n"));
...
INT Set_Register_Dump(IN PRTMP_ADAPTER pAd, IN INT arg)
{
	...
	UINT32 Value = 0;
	if (RTDebugLevel < RT_DEBUG_WARN && arg == 0)
		return 0;
	/* Toggle Error */
...
{
	Set_Register_Dump(pAd, 1);
	return os_set_chip_reset();
}
...
/*            Now = jiffies; */
	...
	case OID_802_11_SET_PASSPHRASE:
		...
		if (ppassphrase == NULL) {
			...
			Status =
			    copy_from_user(ppassphrase, wrq->u.data.pointer, wrq->u.data.length);
	...
	case RT_OID_802_11_STA_CONFIG:
		if (wrq->u.data.length != sizeof(RT_802_11_STA_CONFIG))
			...
			Status =
			    copy_from_user(&StaConfig, wrq->u.data.pointer, wrq->u.data.length);
	...
	case RT_OID_802_11_TX_POWER_LEVEL_1:
		if (wrq->u.data.length < sizeof(ULONG))
			...
			Status =
			    copy_from_user(&PowerTemp, wrq->u.data.pointer, wrq->u.data.length);
			if (PowerTemp > 100)
	...
	case RT_OID_802_11_ADD_WPA:
		...
		if ((pKey->Length != wrq->u.data.length) || (pKey->KeyLength > LEN_PMK)) {
			...
			} else if ((wdev->AuthMode == Ndis802_11AuthModeWPAPSK) || (wdev->AuthMode == Ndis802_11AuthModeWPA2PSK) || (wdev->AuthMode == Ndis802_11AuthModeWPANone)) {	/* Only for WPA PSK mode */
	...
	case OID_802_11_CONFIGURATION:
		if (wrq->u.data.length != sizeof(NDIS_802_11_CONFIGURATION))
			...
			if (pConfig != NULL) {
				Status =
				    copy_from_user(pConfig, wrq->u.data.pointer,
						   wrq->u.data.length);
/*                pConfig = &Config; */
	...
	case RT_OID_802_11_SET_HT_PHYMODE:
		if (wrq->u.data.length != sizeof(OID_SET_HT_PHYMODE))
			...
			Status =
			    copy_from_user(&HT_PhyMode, wrq->u.data.pointer, wrq->u.data.length);
			DBGPRINT(RT_DEBUG_TRACE,
		...
		if (Status) {
			...
			if (KeyIdx >= 4) {
				...
				if (pWepKey->KeyIndex & 0x80000000) {
#ifdef WPA_SUPPLICANT_SUPPORT
					NdisZeroMemory(&pAd->StaCfg.
						       wpa_supplicant_info.DesireSharedKey[KeyIdx],
						       sizeof(CIPHER_KEY));
					...
					pAd->StaCfg.wpa_supplicant_info.
					    DesireSharedKey[KeyIdx].CipherAlg = CipherAlg;
#endif /* WPA_SUPPLICANT_SUPPORT */
					wdev->DefaultKeyId = (UCHAR) KeyIdx;
				}
#ifdef WPA_SUPPLICANT_SUPPORT
		...
		if (wrq->u.data.length != sizeof(BOOLEAN))
			...
			Status =
			    copy_from_user(&IEEE8021x_required_keys, wrq->u.data.pointer,
					   wrq->u.data.length);
			pAd->StaCfg.wpa_supplicant_info.IEEE8021x_required_keys =
	...
	case OID_BW_CAPABILITY:
		{
			if (wrq->u.data.length != sizeof(INT))
				...
				INT HtBw = 0;	/* PIN or PBC */
				Status =
				    copy_from_user(&HtBw, wrq->u.data.pointer, wrq->u.data.length);
				if (Status == 0) {
	...
	case OID_HE_SET_PTK:
		{
			if (wrq->u.data.length != 48 && wrq->u.data.length != 64)
				Status = -EINVAL;
			else {
				Status =
				    copy_from_user(&pAd->WOW_Cfg.PTK, wrq->u.data.pointer,
						   wrq->u.data.length);
				if (Status == 0) {
					DBGPRINT(RT_DEBUG_TRACE, ("Set::OID_HE_SET_PTK ok\n"));
				}
			}
			break;
		}
	case OID_HE_SET_REPLAY_COUNTER:
		{
			if (wrq->u.data.length != LEN_KEY_DESC_REPLAY)
				Status = -EINVAL;
			else {
				Status =
				    copy_from_user(&pAd->WOW_Cfg.ReplayCounter, wrq->u.data.pointer,
						   wrq->u.data.length);
				if (Status == 0) {
					DBGPRINT(RT_DEBUG_TRACE,
						 ("Set::OID_HE_SET_REPLAY_COUNTER ok\n"));
				}
			}
			break;
		}
...
INT RTMPQueryInformation(IN PRTMP_ADAPTER pAd, IN OUT RTMP_IOCTL_INPUT_STRUCT *rq, IN INT cmd)
{
	...
	case RT_OID_802_11_QUERY_TXBF_TABLE:
		if (pAd->chipCap.FlgHwTxBfCap) {
			...
			pMacTab =
			    (RT_802_11_TXBF_TABLE *) kmalloc(sizeof(RT_802_11_TXBF_TABLE),
							     MEM_ALLOC_FLAG);
			if (pMacTab) {
				pMacTab->Num = 0;
				for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
	...
	case OID_HE_GET_PRODUCT_INFO:
		{
			...
			RtmpOsGetUsbDevManufacturerName(((POS_COOKIE) pAd->OS_Cookie)->pUsb_Dev,
							manufacturerName, sizeof(manufacturerName));
			RtmpOsGetUsbDevProductName(((POS_COOKIE) pAd->OS_Cookie)->pUsb_Dev,
						   productName, sizeof(productName));
			RtmpOsGetUsbDevSerialNumber(((POS_COOKIE) pAd->OS_Cookie)->pUsb_Dev,
			...
			Status =
			    copy_to_user(wrq->u.data.pointer, &productInfo, sizeof(productInfo));
			DBGPRINT(RT_DEBUG_TRACE,
		   ...
		   IN UINT32 subcmd, IN VOID *pData, IN ULONG Data)
{
	...
	case SHOW_DRVIER_VERION:
		...
		snprintf(extra, size, "DRI VER-%s FW VER-%X.%X.%X-%s\n", STA_DRIVER_VERSION,
			   ...
			   IN INT keyIdx, IN UCHAR CipherAlg, IN BOOLEAN bGTK, IN UCHAR *pKey)
{
	...
	if (!bGTK) {
		...
		AsicAddSharedKeyEntry(pAd, BSS0 + APCLI_BSSID_IDX, keyIdx,
					&pAd->SharedKey[BSS0][keyIdx]);
#endif
	}
}
...
static INT RtmpIoctl_rt_ioctl_siwencodeext(RTMP_ADAPTER *pAd, VOID *pData, ULONG Data)
{
	...
	if (pIoctlSec->flags == RT_CMD_STA_IOCTL_SECURITY_DISABLED) {
		...
		AsicRemoveSharedKeyEntry(pAd, 0, (UCHAR) keyIdx);
#ifdef STA_P2P_CONNCURRENT
		AsicRemoveSharedKeyEntry(pAd, APCLI_BSSID_IDX, (UCHAR) keyIdx);
#endif
		NdisZeroMemory(&pAd->SharedKey[BSS0][keyIdx], sizeof(CIPHER_KEY));
...
RtmpIoctl_rt_private_get_statistics(IN RTMP_ADAPTER *pAd, IN VOID *pData, IN ULONG Data)
{
...
#endif /* RALINK_ATE */
	{
		sprintf(extra + strlen(extra), "RSSI                            = %ld %ld\n",
		...
		sprintf(extra + strlen(extra), "NoiseLevel-A                   = %ld\n",
			(LONG)(pAd->StaCfg.RssiSample.AvgRssi0 - pAd->StaCfg.RssiSample.AvgSnr0));
		sprintf(extra + strlen(extra), "NoiseLevel-B                   = %ld\n\n",
		...
		if (RTMP_CFG80211_VIF_P2P_GO_ON(pAd) || RTMP_CFG80211_VIF_P2P_CLI_ON(pAd)) {
			sprintf(extra + strlen(extra), "P2P SNR-A                      = %ld\n",
				(LONG)(pAd->ApCfg.RssiSample.AvgSnr0));
			sprintf(extra + strlen(extra), "P2P SNR-B (if available)       = %ld\n",
				(LONG)(pAd->ApCfg.RssiSample.AvgSnr1));

			sprintf(extra + strlen(extra), "P2P NoiseLevel-A               = %ld\n",
				(LONG)(pAd->ApCfg.RssiSample.AvgRssi0 - pAd->ApCfg.RssiSample.AvgSnr0));
			sprintf(extra + strlen(extra), "P2P NoiseLevel-B               = %ld\n\n",
		...
		if (pAd->MacTab.Size > 0) {
			...
			int MacTabInfoSize = 0;
			int MacTabInfoSize_Calced = 0;
			int len_start = strlen(extra);
			int i;

			for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
				PMAC_TABLE_ENTRY pEntry = &(pAd->MacTab.Content[i]);
				CHAR BbpRssiToDbmDelta = pAd->BbpRssiToDbmDelta;
				if ((strlen(extra) + MacTabInfoSize + 20) > Data)
					break;
				if ((IS_ENTRY_CLIENT(pEntry) || IS_ENTRY_APCLI(pEntry)
					...
					if (pEntry->HTPhyMode.field.MODE == MODE_VHT)
						sprintf(extra+strlen(extra), "PhyMode: 802.11ac");
					else if (pEntry->HTPhyMode.field.MODE == MODE_CCK)
						sprintf(extra+strlen(extra), "PhyMode: 802.11b");
					else if  (pEntry->HTPhyMode.field.MODE == MODE_OFDM)
						sprintf(extra+strlen(extra), "PhyMode: 802.11g");
					else
						sprintf(extra+strlen(extra), "PhyMode: 802.11n");

					sprintf(extra+strlen(extra), " %shz PhyRate:%dMbps",
						get_bw_str(pEntry->HTPhyMode.field.BW),
						phyRate/1000000);

					if (pEntry->HTPhyMode.field.ShortGI)
						sprintf(extra+strlen(extra), " SGI\n");
					else
						sprintf(extra+strlen(extra), " LGI\n");

					sprintf(extra+strlen(extra),
					"RSSI				 = %ld %ld\n",
					(LONG)(pEntry->RssiSample.AvgRssi0 - BbpRssiToDbmDelta),
					(LONG)(pEntry->RssiSample.AvgRssi1 - BbpRssiToDbmDelta));
					...
					if (!MacTabInfoSize_Calced) {
						MacTabInfoSize = strlen(extra) - len_start;
						MacTabInfoSize_Calced = 1;
					}
	...
	/* iwpriv stat data may exceed limit when this precompiler is enabled*/
#ifdef WPA_SUPPLICANT_SUPPORT
				...
				if (pCredential->AuthType &
					...
					if (RTMPCheckStrPrintAble
						...
						for (idx = 0; idx < pCredential->KeyLength; idx++)
							sprintf(extra + strlen(extra), "%02X",
								pCredential->Key[idx]);
						sprintf(extra + strlen(extra), "\n");
					}
				}
	...
	if (pAd->CoexMode.dbgEnable) {
		...
		sprintf(extra + strlen(extra), "### Coex Info ###\n");
		sprintf(extra + strlen(extra),
				"Current Mode: %x\n",
...
INT Set_DynamicVGA_Enable(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT32 Disabled = (INT32) simple_strtol(arg, 0, 10);
...
{
	LONG Timeout;
	if (kstrtol(arg, 0, &Timeout) == 0) {/*unit sec*/
		pAd->ApCfg.StaIdleTimeout = (UINT32)Timeout;
		DBGPRINT(RT_DEBUG_OFF,
			("set Idle Timeout = %ld\n", Timeout));
	}
	return TRUE;
...
{
	LONG Time;
	if (kstrtol(arg, 0, &Time) == 0) {/*unit msec*/
		pAd->MlmeAux.FastScanChannelTime = (UINT)Time;
		DBGPRINT(RT_DEBUG_OFF,
			("set FastScanChannelTime = %ld\n", Time));
	}
	return TRUE;
}
...
INT Set_Scan_StayOpChTime(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	LONG Time;
	if (kstrtol(arg, 0, &Time) == 0) {/*unit msec*/
		pAd->MlmeAux.OpChannelTime = (ULONG)Time;
		DBGPRINT(RT_DEBUG_OFF,
			("set ScnOpChannelTime = %ld\n", Time));
	}
	return TRUE;
}
...
{
	LONG Disable;
	if (kstrtol(arg, 0, &Disable) == 0) {/*unit msec*/
		pAd->MlmeAux.ScanDisable = (BOOLEAN)Disable;
		DBGPRINT(RT_DEBUG_OFF,
			("set ScanDisable = %ld\n", Disable));
	}
	return TRUE;
