			...
			if ((pAd->StaCfg.wdev.IEEE8021X == TRUE) &&
			    ...
			    (EAP_CODE_SUCCESS ==
			     WpaCheckEapCode(pAd, pRxBlk->pData,
					     pRxBlk->DataSize, LENGTH_802_1_H))) {
				PUCHAR Key;
				UCHAR CipherAlg;
				int idx = 0;
				...
				if (pAd->StaCfg.wpa_supplicant_info.IEEE8021x_required_keys ==
				    FALSE) {
					...
					if (pAd->StaCfg.wpa_supplicant_info.DesireSharedKey[idx].
					    KeyLen > 0) {
			...
			if (pRxBlk->pHeader->FC.Wep) {
				...
				if (wdev->WepStatus == Ndis802_11EncryptionDisabled) {
					...
					RELEASE_NDIS_PACKET(pAd,
							    pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
					return;
				}
			} else {
...
VOID STARxDataFrameAnnounce_Hdr_Trns(IN PRTMP_ADAPTER pAd,
				     IN MAC_TABLE_ENTRY *pEntry,
				     IN RX_BLK *pRxBlk, IN UCHAR FromWhichBSSID)
{
...
/* ---Add by shiangf for debug */
	...
	if ((pHeader->FC.FrDs == 1) && (pHeader->FC.ToDs == 1)) {
		...
		if (pAd->StaCfg.UapsdInfo.bAPSDCapable
		    && pAd->CommonCfg.APEdcaParm.bAPSDCapable && (pHeader->FC.SubType & 0x08)) {
			UCHAR *pData;
			...
			if ((*pData >> 4) & 0x01) {
				...
				if (pAd->StaCfg.FlgPsmCanNotSleep == TRUE) {
					DBGPRINT(RT_DEBUG_TRACE,
						 ("tdls uapsd> Rcv EOSP frame but we can not sleep!\n"));
				} else
				...
				if ((CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_APSD_CAPABLE))
					...
					if ((*(pRxBlk->pData + LENGTH_802_11) & 0x10) == 0) {
						...
						if (OldPwrMgmt == PWR_SAVE) {
							...
							UAPSD_TriggerFrameHandle(pAd, pEntry,
										 OldUP);
						}
					} else {
						DBGPRINT(RT_DEBUG_TRACE,
		...
		if ((pEntry) && (pEntry->bIAmBadAtheros == FALSE) && (pRxInfo->AMPDU == 1)
		    && (pHeader->FC.Retry)) {
...
#endif /* defined(DOT11Z_TDLS_SUPPORT) || defined(CFG_TDLS_SUPPORT) */
		{
			if ((RtmpPktPmBitCheck(pAd) == TRUE) && (pHeader->FC.MoreData == 1)) {
				if ((((UserPriority == 0) || (UserPriority == 3))
				     && pAd->CommonCfg.bAPSDAC_BE == 0) || (((UserPriority == 1)
									     || (UserPriority == 2))
									    && pAd->CommonCfg.
									    bAPSDAC_BK == 0)
				    || (((UserPriority == 4) || (UserPriority == 5))
					&& pAd->CommonCfg.bAPSDAC_VI == 0) || (((UserPriority == 6)
										|| (UserPriority ==
										    7))
									       && pAd->CommonCfg.
									       bAPSDAC_VO == 0)) {
	...
	if (pRxInfo->Bcast || pRxInfo->Mcast) {
		...
		if ((pAd->StaCfg.WfdCfg.bWfdEnable) &&
		    pRxInfo->Bcast && TDLS_CheckTDLSframe(pAd, pRxBlk->pData, pRxBlk->DataSize)) {
...
INT STASendPacket(RTMP_ADAPTER *pAd, PNDIS_PACKET pPacket)
{
	...
	if (((wdev->AuthMode == Ndis802_11AuthModeWAICERT) ||
		...
		DBGPRINT(RT_DEBUG_TRACE, ("%s():Drop packet before port secured!\n", __func__));
		RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
...

	{
		...
		if (NumberOfFrag > 1)
			...
			RTSRequired =
			    (PacketInfo.TotalPacketLength > pAd->CommonCfg.RtsThreshold) ? 1 : 0;
	...
	RTMP_SET_PACKET_UP(pPacket, UserPriority);
		...
		if ((pMacEntry != NULL) &&
			...
			if (RtmpInsertPsQueue(pAd, pPacket, pMacEntry, QueIdx) !=
			    NDIS_STATUS_SUCCESS)
				return NDIS_STATUS_FAILURE;
		} else
...
NDIS_STATUS RTMPFreeTXDRequest(IN PRTMP_ADAPTER pAd,
			       IN UCHAR QueIdx, IN UCHAR NumberRequired, IN PUCHAR FreeNumberIs)
{
	/*ULONG         FreeNumber = 0; */
...
VOID STAFindCipherAlgorithm(RTMP_ADAPTER *pAd, TX_BLK *pTxBlk)
{
...
#ifdef WAPI_SUPPORT
	if (pMacEntry && (pMacEntry->WepStatus == Ndis802_11EncryptionSMS4Enabled)) {
...
#endif /* WAPI_SUPPORT */
	{
		...
		} else if ((Cipher == Ndis802_11TKIPEnable) || (Cipher == Ndis802_11AESEnable)) {
...
VOID STABuildCacheWifiInfo(IN RTMP_ADAPTER *pAd, IN TX_BLK *pTxBlk, IN UCHAR *pWiInfo)
{
	PWIFI_INFO_STRUCT pWI;
...
static VOID STABuildCache802_11Header(IN RTMP_ADAPTER *pAd, IN TX_BLK *pTxBlk, IN UCHAR *pHeader)
{
	MAC_TABLE_ENTRY *pMacEntry;
...
static inline PUCHAR STA_Build_ARalink_Frame_Header(IN RTMP_ADAPTER *pAd, IN TX_BLK *pTxBlk)
{
...
static inline PUCHAR STA_Build_AMSDU_Frame_Header(IN RTMP_ADAPTER *pAd, IN TX_BLK *pTxBlk)
{
	PUCHAR pHeaderBufPtr;
	HEADER_802_11 *pHeader_802_11;
	UINT8 TXWISize = pAd->chipCap.TXWISize;
...
static VOID STA_AMPDU_Frame_Tx(IN PRTMP_ADAPTER pAd, IN TX_BLK *pTxBlk)
{
	HEADER_802_11 *pHeader_802_11;
	PUCHAR pHeaderBufPtr;
	USHORT FreeNumber = 0;
	...
	while (pTxBlk->TxPacketList.Head) {
...
#endif /* VENDOR_FEATURE1_SUPPORT */
		{
			...
			*(pHeaderBufPtr + 1) = 0;
			...
			if (pMacEntry->TxSndgType >= SNDG_TYPE_SOUNDING) {
				DBGPRINT(RT_DEBUG_TRACE,
					 ("--Sounding in AMPDU: TxSndgType=%d, MCS=%d\n",
					  ...
					  pMacEntry->TxSndgType ==
					  SNDG_TYPE_NDP ? pMacEntry->sndgMcs : pTxBlk->pTransmit->
					  field.MCS));
...
#endif /* endif */
			if ((pMacEntry->HTCapability.ExtHtCapInfo.MCSFeedback >= MCSFBK_MRQ) && (pTxBlk->TxSndgPkt == SNDG_TYPE_DISABLE)) {	/* because the signal format of sounding frmae may be different from normal data frame, which may result in different MFB */
				if (bHTCPlus == FALSE) {
					bHTCPlus = TRUE;
					NdisZeroMemory(pHeaderBufPtr, sizeof(HT_CONTROL));
				}
			...
			if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bSwEncrypt)) {
				...
				RTMPSoftEncryptionAction(pAd, pTxBlk->CipherAlg, (PUCHAR)
							 pHeader_802_11,
							 pTxBlk->pSrcBufData,
							 pTxBlk->SrcBufLen,
		...
		if ((pMacEntry->isCached)
			...
			RTMPWriteTxWI_Cache(pAd, (TXWI_STRUCT *) (&pTxBlk->HeaderBuf[TXINFO_SIZE]),
					    pTxBlk);
		} else {
			RTMPWriteTxWI_Data(pAd, (TXWI_STRUCT *) (&pTxBlk->HeaderBuf[TXINFO_SIZE]),
					   pTxBlk);

#ifndef VENDOR_FEATURE1_SUPPORT	/* 20140915_zhilin: modify for memory occupied */
		...
		/* calculate Transmitted AMPDU count and ByteCount  */
		{
			...
			pAd->RalinkCounters.TransmittedOctetsInAMPDUCount.QuadPart +=
			    pTxBlk->SrcBufLen;
		}
#endif /* STATS_COUNT_SUPPORT */
...
#ifdef HDR_TRANS_SUPPORT
VOID STA_AMPDU_Frame_Tx_Hdr_Trns(IN PRTMP_ADAPTER pAd, IN TX_BLK *pTxBlk)
{
	HEADER_802_11 *pHeader_802_11;
	UCHAR *pWiBufPtr;
	USHORT FreeNumber = 0;
...
static VOID STA_AMSDU_Frame_Tx(IN PRTMP_ADAPTER pAd, IN TX_BLK *pTxBlk)
{
	PUCHAR pHeaderBufPtr;
	USHORT FreeNumber = 0;
...
#ifdef HDR_TRANS_SUPPORT
VOID STA_Legacy_Frame_Tx_Hdr_Trns(IN PRTMP_ADAPTER pAd, IN TX_BLK *pTxBlk)
{
	PUCHAR pHeaderBufPtr;
	USHORT FreeNumber = 0;
	BOOLEAN bVLANPkt;
...
static VOID STA_ARalink_Frame_Tx(IN PRTMP_ADAPTER pAd, IN TX_BLK *pTxBlk)
{
	PUCHAR pHeaderBufPtr;
	USHORT freeCnt = 0;
	USHORT totalMPDUSize = 0;
...
static VOID STA_Fragment_Frame_Tx(IN RTMP_ADAPTER *pAd, IN TX_BLK *pTxBlk)
{
	HEADER_802_11 *pHeader_802_11;
	PUCHAR pHeaderBufPtr;
	USHORT freeCnt = 0;
	...
	if (pTxBlk->CipherAlg == CIPHER_TKIP) {
		...
		RTMP_QueryPacketInfo(pTxBlk->pPacket, &PacketInfo,
				     &pTxBlk->pSrcBufHeader, &pTxBlk->SrcBufLen);
	}
	...
	if (pTxBlk->CipherAlg == CIPHER_TKIP) {
		RTMPCalculateMICValue(pAd, pTxBlk->pPacket,
				      pTxBlk->pExtraLlcSnapEncap, pTxBlk->pKey, 0);
	...
	do {
		FreeMpduSize =
		    pAd->CommonCfg.FragmentThreshold - LENGTH_CRC - pTxBlk->MpduHeaderLen;
...
VOID Sta_Announce_or_Forward_802_3_Packet(IN RTMP_ADAPTER *pAd,
