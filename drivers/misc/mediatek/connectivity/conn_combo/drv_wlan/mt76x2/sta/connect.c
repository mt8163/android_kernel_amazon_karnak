...
/*
	...
	==========================================================================
*/
VOID MlmeCntlInit(IN PRTMP_ADAPTER pAd, IN STATE_MACHINE *S, OUT STATE_MACHINE_FUNC Trans[])
{
	...
	case CNTL_WAIT_OID_LIST_SCAN:
		if (Elem->MsgType == MT2_SCAN_CONF) {
...
#ifdef WPA_SUPPLICANT_SUPPORT
					RtmpOSWrielessEventSend(pAd->net_dev, RT_WLAN_EVENT_SCAN,
								-1, NULL, NULL, 0);
#endif /* WPA_SUPPLICANT_SUPPORT */
				}
			}
			...
			if (pAd->StaCfg.BssType == BSS_ADHOC) {
				MlmeEnqueue(pAd, IWSC_STATE_MACHINE, IWSC_MT2_MLME_SCAN_DONE, 0,
					    NULL, 0);
...
*/
			if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS)) {
				...
				CFG80211DBG(RT_DEBUG_ERROR,
					    ("[%s] BSS_SCAN_IN_PROGRESS, Set CntlMachine to CNTL_WAIT_OID_LIST_SCAN state\n",
					     __func__));
			} else {
				pAd->Mlme.CntlMachine.CurrState = CNTL_IDLE;
			}
...
/* Disable scanning when ATE is running. */
	...
	if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED)) {
		...
		if (BssIdx != BSS_NOT_FOUND && BssIdx < MAX_LEN_OF_BSS_TABLE) {
			NdisMoveMemory(pCurrBss, &pAd->ScanTab.BssEntry[BssIdx], sizeof(BSS_ENTRY));
		}
	}

		ScanParmFill(pAd, &ScanReq, (PSTRING) Elem->Msg, Elem->MsgLen, BSS_ANY, Elem->Priv);
...
VOID CntlOidSsidProc(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (INFRA_ON(pAd) &&
	    ...
	    && NdisEqualMemory(pAd->CommonCfg.Ssid, pAd->MlmeAux.SsidBssTab.BssEntry[0].Ssid,
		 ...
		 */
		if (!SSID_EQUAL
		    (pAd->CommonCfg.Ssid, pAd->CommonCfg.SsidLen, pAd->MlmeAux.Ssid,
		     pAd->MlmeAux.SsidLen)) {
		...
		if ((pAd->MlmeAux.SsidBssTab.BssNr == 0) &&
		    (pAd->StaCfg.bAutoReconnect == TRUE) && (((pAd->MlmeAux.BssType == BSS_INFRA)
							      &&
							      (!RTMP_TEST_FLAG
							       (pAd,
			...
			if ((pAd->StaCfg.ConnectinfoChannel != 0)
			    && (pAd->StaCfg.Connectinfoflag == TRUE)) {
...
#ifdef RT_CFG80211_SUPPORT
			{
				...
				if (BSSIDScanning || pAd->cfg80211_ctrl.Cfg80211FreqScanning) {
					MLME_SCAN_REQ_STRUCT ScanReq;
					DBGPRINT(RT_DEBUG_TRACE,
					("%s:Can't find BSSID(%02x:%02x:%02x:%02x:%02x:%02x) or freq scan triggered\n",
					 __func__, PRINT_MAC(pAd->cfg80211_ctrl.Cfg80211_ConnReqBssid)));

					/* Case-study:
					   The requested BSSID is not in the ScaTab, it could be caused by newtork
					   containing too many APs exceeding driver's max size (128). In such case,
					   we will need to perform a new scan immediately again with specific SSID
					   to minize the scan results.
					   Then after this scan, we shall try to connect again if the BSSID was found.
					 */
		...
		if ((pAd->MlmeAux.SsidBssTab.BssNr == 0) && (pAd->MlmeAux.BssType == BSS_INFRA)
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("CFG80211_MLME: No matching BSS, Report cfg80211_layer SM to Idle --> %lu\n",
				  pAd->Mlme.CntlMachine.CurrState));
			...
			NdisMoveMemory(pAd->cfg80211_ctrl.Cfg_pending_Ssid, pAd->MlmeAux.Ssid,
				       pAd->MlmeAux.SsidLen);

			RT_CFG80211_CONN_RESULT_INFORM(pAd, pAd->MlmeAux.Bssid, NULL, 0, NULL, 0,
						       0);
		}
#endif /* RT_CFG80211_SUPPORT */
	}
	...
	if ((pAd->StaCfg.WscControl.WscState >= WSC_STATE_START) &&
		...
		if (pAd->StaCfg.BssType == BSS_INFRA) {
			MlmeEnqueue(pAd,
				    ...
				    OID_802_11_BSSID,
				    MAC_ADDR_LEN, pAd->StaCfg.WscControl.WscBssid, 0);
		} else {
			MlmeEnqueue(pAd,
				    MLME_CNTL_STATE_MACHINE,
				    OID_802_11_SSID,
...
/* No need to perform this routine when ATE is running. */
	...
	if (BssIdx < MAX_LEN_OF_BSS_TABLE) {
		...
		DBGPRINT(RT_DEBUG_ERROR, ("%s: BssIdx >= MAX_LEN_OF_BSS_TABLE", __func__));
		return;
	}
...
VOID CntlMlmeRoamingProc(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
...

	{
		...
		if (IS_MT76XXBTCOMBO(pAd)) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("COEX: ROAMING_START, Remove old protection frame and AFH information\n"));
			pAd->MlmeAux.bStaCanRoam = TRUE;
			pAd->CoexMode.LinkStatus &= ~COEX_STA_LINK;
...
/*
	...
	==========================================================================
*/
VOID CntlOidDLSSetupProc(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (INFRA_ON(pAd)) {
		for (i = 0; i < MAX_NUM_OF_DLS_ENTRY; i++) {
			if (pDLS->Valid && pAd->StaCfg.DLSEntry[i].Valid
			    ...
			    && (pDLS->TimeOut == pAd->StaCfg.DLSEntry[i].TimeOut)
...
VOID CntlWaitJoinProc(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (Elem->MsgType == MT2_JOIN_CONF) {
		...
		if (Reason == MLME_SUCCESS) {
			...
			pAd->MlmeAux.BssIdx++;
#ifdef MT76XX_BTCOEX_SUPPORT
			MLMEHook(pAd, WLAN_CONNECTION_JOIN_FAIL,
				 QueryHashID(pAd, pAd->MlmeAux.Bssid, TRUE));
#endif /*MT76XX_BTCOEX_SUPPORT */
			IterateOnBssTab(pAd);
		}
	}
		...
		if (Result == MLME_SUCCESS) {
			...
			if (WMODE_CAP_N(pAd->CommonCfg.PhyMode)
				...
				pAd->MlmeAux.CentralChannel = get_cent_ch_by_htinfo(pAd,
										    ...
										    &pAd->CommonCfg.
										    HtCapability);
				NdisMoveMemory(&pAd->MlmeAux.AddHtInfo, &pAd->CommonCfg.AddHTInfo,
					       sizeof(ADD_HT_INFO_IE));
				RTMPCheckHt(pAd, BSSID_WCID, &pAd->CommonCfg.HtCapability,
				...
				if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode) &&
					...
					rt_vht_cap->vht_txstbc =
					    pAd->MlmeAux.vht_cap.vht_cap.tx_stbc;
					rt_vht_cap->vht_rxstbc =
					    pAd->MlmeAux.vht_cap.vht_cap.rx_stbc;
					rt_vht_cap->vht_htc =
					    pAd->MlmeAux.vht_cap.vht_cap.htc_vht_cap;
				}
#endif /* DOT11_VHT_AC */
			} else {
		...
		if (Reason == MLME_SUCCESS) {
			...
			DBGPRINT(RT_DEBUG_TRACE, ("CNTL - AUTH FAIL, try again...\n"));
			{
				if ((pAd->StaCfg.wdev.AuthMode == Ndis802_11AuthModeShared)
					...
					/* either Ndis802_11AuthModeShared or Ndis802_11AuthModeAutoSwitch, try shared key first */
					AuthParmFill(pAd, &AuthReq,
						     pAd->MlmeAux.Bssid, AUTH_MODE_KEY);
				} else {
					AuthParmFill(pAd, &AuthReq,
						     pAd->MlmeAux.Bssid, AUTH_MODE_OPEN);
				}
...
#endif /* RT_CFG80211_SUPPORT */
#ifdef MT76XX_BTCOEX_SUPPORT
				MLMEHook(pAd, WLAN_CONNECTION_AUTH_FAIL,
					 QueryHashID(pAd, pAd->MlmeAux.Bssid, TRUE));
#endif /*MT76XX_BTCOEX_SUPPORT */
			}
		}
	...
	if (Elem->MsgType == MT2_ASSOC_CONF) {
		...
		if (Reason == MLME_SUCCESS) {
...
#ifdef RT_CFG80211_SUPPORT
			{
				...
				/* We should let wpa_supplicant do the roaming */
				DBGPRINT(RT_DEBUG_TRACE,
					 ("%s: ASSOC FAIL, DO NOT try next BSS\n", __func__));
				RTMPCancelTimer(&pAd->MlmeAux.BeaconTimer, &TimerCancelled);
				pAd->Mlme.CntlMachine.CurrState = CNTL_IDLE;
				pAd->Mlme.SyncMachine.CurrState = SYNC_IDLE;
...
#ifdef MT76XX_BTCOEX_SUPPORT
			MLMEHook(pAd, WLAN_CONNECTION_ASSOC_FAIL,
				 QueryHashID(pAd, pAd->MlmeAux.Bssid, TRUE));
#endif /*MT76XX_BTCOEX_SUPPORT */
...
static VOID AdhocTurnOnQos(RTMP_ADAPTER *pAd)
{
...
VOID LinkUp(RTMP_ADAPTER *pAd, UCHAR BssType)
{
	...
	pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BeaconSynced = FALSE;
	pAd->StaCfg.wpa_supplicant_info.CFG_Tdls_info.BeaconCount = 0;
	...
	if (BssType == BSS_ADHOC) {
		...
		if (pAd->MlmeAux.bStaCanRoam) {
			MLMEHook(pAd, WLAN_ROAMING_COMPLETE,
...
/* wdev->bw = pAd->CommonCfg.RegTransmitSetting.field.BW; */
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("link uppAd->CommonCfg.RegTransmitSetting.field.BW %d pAd->CommonCfg.BBPCurrentBW %d\n",
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("linkup bw %d  common %d central %d\n", wdev->bw, wdev->channel,
		  wdev->CentralChannel));
	...
	if (BssType == BSS_ADHOC) {
				...
				if (pAd->SharedKey[BSS0][idx].KeyLen > 0) {
					...
					if (idx == wdev->DefaultKeyId) {
						...
						for (cnt = 0; cnt < LEN_WEP_TSC; cnt++)
							pAd->SharedKey[BSS0][idx].TxTsc[cnt] =
							    RandomByte(pAd);
		...
		/* Fill in Shared Key Table(offset: 0x6c00) and Shared Key Mode(offset: 0x7000) */
			...
			/* Set key material and cipherAlg to Asic */
			AsicAddSharedKeyEntry(pAd, BSS0, 0, &pAd->SharedKey[BSS0][0]);
			...
			WpaDeriveGTK(pAd->StaCfg.GMK,
				     pAd->StaCfg.GNonce,
				     pAd->CurrentAddress, pAd->StaCfg.GTK, LEN_TKIP_GTK);
			...
			if (pAd->StaCfg.GroupCipher == Ndis802_11AESEnable) {
				...
				AsicAddPairwiseKeyEntry(pAd, (UCHAR) Wcid, &pAd->StaCfg.TxGTK);
		...
		while (Cancelled == TRUE) {
			if (pAd->CommonCfg.LastSsidLen == pAd->CommonCfg.SsidLen) {
				if (RTMPCompareMemory
				    (pAd->CommonCfg.LastSsid, pAd->CommonCfg.Ssid,
...
/*			RTMPWPARemoveAllKeys(pAd);*/
		...
		if (wdev->AuthMode < Ndis802_11AuthModeWPA) {
			...
			if (((pAd->StaCfg.wpa_supplicant_info.WpaSupplicantUP)
			    ...
			    ||
			    ((pAd->StaCfg.wpa_supplicant_info.WpaSupplicantUP ==
			      WPA_SUPPLICANT_DISABLE) && (wdev->WepStatus == Ndis802_11WEPEnabled))
		...
		if ((pAd->StaCfg.WscControl.WscConfMode != WSC_DISABLE) &&
			...
			NdisZeroMemory(pAd->CommonCfg.Ssid, MAX_LEN_OF_SSID);
			NdisMoveMemory(pAd->CommonCfg.Ssid, tmpWscSsid, tmpWscSsidLen);
...
#endif /* RT_CFG80211_P2P_SUPPORT */
	{
		...
		MlmeSelectTxRateTable(pAd, pEntry,
				      &pEntry->pTable, &TableSize, &pAd->CommonCfg.TxRateIndex);
	...
	if (wdev->bAutoTxRateSwitch == FALSE) {
		...
		if (pEntry->HTPhyMode.field.MODE <= MODE_OFDM)
			RTMPUpdateLegacyTxSetting((UCHAR) wdev->DesiredTransmitSetting.field.
						  FixedTxMode, pEntry);
	} else
	...
	RtmpPrepareHwNullFrame(pAd,
			       ...
			       FALSE, FALSE, 0, OPMODE_STA, PWR_SAVE, 0, 0);
...
/* start MCC when Infra connect */
	if (p2p_wdev && ((wdev->AuthMode == Ndis802_11AuthModeOpen) && !(pAd->StaCfg.wpa_supplicant_info.WpaSupplicantUP & WPA_SUPPLICANT_ENABLE_WPS))	/* WPS not under goining! */
	    ) {
		DBGPRINT(RT_DEBUG_TRACE,
		...
		if (RTMP_CFG80211_VIF_P2P_GO_ON(pAd)) {
			if ((wdev->bw != p2p_wdev->bw) && ((wdev->channel == p2p_wdev->channel))) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("#####pAd->Mlme.Now32 %lu pAd->StaCfg.LastBeaconRxTime %lu\n",
...
#ifdef MT76XX_BTCOEX_SUPPORT
	{
		...
		info.coexType = COEX_OPMODE_STA;
		info.BBPCurrentBW = pAd->CommonCfg.BBPCurrentBW;
		info.Channel = pAd->CommonCfg.Channel;
		info.CentralChannel = pAd->CommonCfg.CentralChannel;
		info.OwnMac = pAd->CurrentAddress;
		info.Bssid = pAd->CommonCfg.Bssid;
		info.wdev = wdev;
		info.pEntry = pEntry;
		if (BssType == BSS_INFRA)
			info.coexType = COEX_OPMODE_STA;
		else if (BssType == BSS_ADHOC)
			info.coexType = COEX_OPMODE_AD_HOC;
		CoexLinkUpHandler(pAd, info);
	}
#endif /*MT76XX_BTCOEX_SUPPORT */
}
...
VOID LinkDown(IN PRTMP_ADAPTER pAd, IN BOOLEAN IsReqFromAP)
{
	...
	struct wifi_dev *wdev = &pAd->StaCfg.wdev;
#ifdef RT_CFG80211_SUPPORT
	INT32 needInform = !MAC_ADDR_EQUAL(pAd->CommonCfg.Bssid, ZERO_MAC_ADDR);
#endif /* RT_CFG80211_SUPPORT */
...
/*||	RTMP_TEST_PSFLAG(pAd, fRTMP_PS_SET_PCI_CLK_OFF_COMMAND) */
	...
	if (ADHOC_ON(pAd)) {	/* Adhoc mode link down */
			...
			for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
				if (IS_ENTRY_CLIENT(&pAd->MacTab.Content[i]))
					MlmeDeAuthAction(pAd,
							 ...
							 REASON_DEAUTH_STA_LEAVING, FALSE);
			}
		}
#endif /* ADHOC_WPA2PSK_SUPPORT */
		...
		if (pAd->MlmeAux.CurrReqIsFromNdis == TRUE) {
			...
			if ((wdev->PortSecured == WPA_802_1X_PORT_SECURED)
				 ...
				 */
				BssTableDeleteEntry(&pAd->ScanTab,
						    pAd->CommonCfg.Bssid, pAd->CommonCfg.Channel);
			}
		}
	...
	if (wdev->WepStatus == Ndis802_11EncryptionSMS4Enabled) {
		/* Cancel rekey timer */
		RTMPCancelWapiRekeyTimerAction(pAd, &pAd->MacTab.Content[BSSID_WCID]);
	}
#endif /* WAPI_SUPPORT */
...
#ifdef DOT11_VHT_AC
	NdisZeroMemory(&pAd->MlmeAux.vht_cap, sizeof(VHT_CAP_IE));
	NdisZeroMemory(&pAd->MlmeAux.vht_op, sizeof(VHT_OP_IE));
	pAd->MlmeAux.vht_cap_len = 0;
	pAd->MlmeAux.vht_op_len = 0;
#endif /* DOT11_VHT_AC */
	...
	if (pAd->StaCfg.wpa_supplicant_info.WpaSupplicantUP &&
	    (wdev->WepStatus == Ndis802_11WEPEnabled) && (wdev->IEEE8021X == FALSE)) {
		wdev->PortSecured = WPA_802_1X_PORT_SECURED;
	} else
#endif /* WPA_SUPPLICANT_SUPPORT */
	{
...
#ifdef RT_CFG80211_SUPPORT
	/* It is workaround to avoid reporting deauth to
	 * wpa_supplicant too early but AP WPS is not ready.
	 */
	if (pAd->StaCfg.wpa_supplicant_info.WpaSupplicantUP & WPA_SUPPLICANT_ENABLE_WPS) {
		DBGPRINT(RT_DEBUG_TRACE, ("wait 2 seconds for AP WPS ready\n"));
		mdelay(2000);
	}
...
/* pAd->MlmeAux.InfraChannel = 0; */
	...
	if (pAd->Mlme.bStartMcc == TRUE) {
		...
		if (((pMacEntry->PairwiseKey.KeyLen == LEN_TK) && IS_ENTRY_APCLI(pMacEntry)) || RTMP_CFG80211_VIF_P2P_GO_ON(pAd)) {	/* yiwei : PairwiseKey seems not clear in MacTableDeleteEntry() */
	...
	if (pAd->Mlme.bStartScc == TRUE) {
		...
		if (RTMP_CFG80211_VIF_P2P_GO_ON(pAd)) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("link down and switch to GO bw %d   pWdev->CentralChanne  %d\n",
				  pWdev->bw, pWdev->CentralChannel));
...
/*if p2p_cli*/
	...
	pAd->CommonCfg.ChannelSwitchMode = 0;
	pAd->StaCfg.bChannelSwitchCountingDown = FALSE;
	pAd->CommonCfg.ExtChannelSwitchMode = 0;
	pAd->StaCfg.bExtChannelSwitchCountingDown = FALSE;
...
VOID IterateOnBssTab(RTMP_ADAPTER *pAd)
{
	...
	if (pAd->StaCfg.BssType == BSS_ADHOC && BssIdx < MAX_LEN_OF_BSS_TABLE) {
	...
	} else if ((BssIdx < pAd->MlmeAux.SsidBssTab.BssNr) && (pAd->StaCfg.BssType == BSS_INFRA)) {
		pInBss = &pAd->MlmeAux.SsidBssTab.BssEntry[BssIdx];
		/* Check cipher suite, AP must have more secured cipher than station setting */
		/* Set the Pairwise and Group cipher to match the intended AP setting */
		/* We can only connect to AP with less secured cipher setting */
...
/* It seems not necessary to remove BssEntry that we just associated with,
	and it will cause one more scan when doing WPS */
...
VOID IterateOnBssTab2(RTMP_ADAPTER *pAd)
{
	...
	if (BssIdx < pAd->MlmeAux.RoamTab.BssNr) {
		...
		AssocParmFill(pAd, &ReassocReq, pBss->Bssid,
			      pBss->CapabilityInfo, ASSOC_TIMEOUT, pAd->StaCfg.DefaultListenCount);
...
/*
	...
	==========================================================================
*/
VOID JoinParmFill(IN PRTMP_ADAPTER pAd, IN OUT MLME_JOIN_REQ_STRUCT *JoinReq, IN ULONG BssIdx)
{
...
/*
	...
	==========================================================================
*/
VOID DlsParmFill(IN PRTMP_ADAPTER pAd,
...
{
	MlmeEnqueue(pAd, MLME_CNTL_STATE_MACHINE, MsgType, sizeof(USHORT), &Msg, 0);
}
#endif /* RTMP_MAC_USB */
...
ULONG MakeIbssBeacon(IN PRTMP_ADAPTER pAd)
{
	...
	MakeOutgoingFrame(pBeaconFrame, &FrameLen,
			  ...
			  1, &IbssIe, 1, &IbssLen, 2, &pAd->StaActive.AtimWin, END_OF_ARGS);
	...
	if (ExtRateLen) {
		...
		MakeOutgoingFrame(pBeaconFrame + FrameLen, &tmp,
				  3, LocalErpIe,
				  1, &ExtRateIe, 1, &ExtRateLen, ExtRateLen, ExtRate, END_OF_ARGS);
		FrameLen += tmp;
	}
	...
	if ((wdev->AuthMode == Ndis802_11AuthModeWPANone)
		...
		MakeOutgoingFrame(pBeaconFrame + FrameLen, &tmp,
				  ...
				  pAd->StaCfg.RSNIE_Len, pAd->StaCfg.RSN_IE, END_OF_ARGS);
		FrameLen += tmp;
	}
#ifdef WSC_STA_SUPPORT
	...
	if (WMODE_CAP_N(pAd->CommonCfg.PhyMode)
		...
		*(USHORT *) (&addHTInfoTmp.AddHtInfo3) =
		...
		MakeOutgoingFrame(pBeaconFrame + FrameLen, &TmpLen,
				  ...
				  1, &AddHtInfoIe, 1, &HtLen1, HtLen1, &addHTInfoTmp, END_OF_ARGS);
#endif /* RT_BIG_ENDIAN */
		FrameLen += TmpLen;
	}
	...
	if (pAd->CommonCfg.Channel > 14) {
		RTMPWriteTxWI(pAd, pTxWI, FALSE, FALSE, TRUE, FALSE, FALSE,
			      ...
			      RATE_1, IFS_HTTXOP, &pAd->CommonCfg.MlmeTransmit);
	} else {
...
/* IRQL = DISPATCH_LEVEL */
VOID MaintainBssTable(IN PRTMP_ADAPTER pAd,
		      IN OUT BSS_TABLE * Tab, IN ULONG MaxRxTimeDiff, IN UCHAR MaxSameRxTimeCount)
{
...
#if defined(RT_CFG80211_P2P_SUPPORT) && defined(SUPPORT_ACS_ALL_CHANNEL_RANK) && defined(SUPPORT_ACS_BY_SCAN)
	{
		...
		ch = ACS_PerformAlgorithm(pAd, pAd->ApCfg.AutoChannelAlg);
		if (ch <= 0) {
			       ...
			       IN BOOLEAN BandWidth, IN UCHAR PriCh, IN UCHAR ExtraCh)
{
	...
	*pBwFallBack = 0;
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s():CentralChannel=%d, Channel=%d, ChannelWidth=%d\n",
				  __func__, ExtraCh, PriCh, BandWidth));
	...
	DBGPRINT(RT_DEBUG_OFF,
		 ("%s(): Input BW=%d, rf_channel=%d, vht_bw=%d, Channel=%d, vht_cent_ch=%d!\n",
		  __func__, rf_bw, rf_channel, pAd->CommonCfg.vht_bw, pAd->CommonCfg.Channel,
	...
	pAd->WOW_Cfg.CentralChannel = rf_channel;
	pAd->WOW_Cfg.ExtChSetting = ext_ch;
#endif /* NEW_WOW_SUPPORT */

	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s():BW_%s, RF-Ch=%d, CtrlCh=%d, HT-CentralCh=%d\n",
		  __func__, (rf_bw == BW_80 ? "80" : (rf_bw == BW_40 ? "40" : "20")),
		  pAd->LatchRfRegs.Channel, pAd->CommonCfg.Channel, pAd->CommonCfg.CentralChannel));
#ifdef DOT11_VHT_AC
