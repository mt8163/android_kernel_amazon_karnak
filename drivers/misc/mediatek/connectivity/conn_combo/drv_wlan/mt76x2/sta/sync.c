...
VOID MlmeForceScanReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (MlmeScanReqSanity(pAd,
		...
		if (pAd->cfg80211_ctrl.Cfg80211FreqScanning) {
			pAd->MlmeAux.Channel = pAd->cfg80211_ctrl.Cfg80211ConnectChanIndex;
			pAd->cfg80211_ctrl.Cfg80211ConnectChanIndex = 0;
			DBGPRINT(RT_DEBUG_TRACE, ("MlmeForceScanReqAction -- Now scan connection channel %d,",
						   pAd->MlmeAux.Channel));
		}
...
/*			CFG80211_BeaconCountryRegionParse(pAd, pVIE, LenVIE); */
			...
			if (ADHOC_ON(pAd) && (CAP_IS_IBSS_ON(bcn_ie_list->CapabilityInfo))) {
				 ...
				 */
				if ((ADHOC_ON(pAd)
				     && ((!pEntry) || (pEntry && IS_ENTRY_NONE(pEntry)))) || (pEntry
											      &&
											      RTMP_TIME_AFTER
											      (Now,
											       pEntry->
				...
				if (bcn_ie_list->MessageToMe) {
					...
					if ((pAd->CommonCfg.bACMAPSDTr[QID_AC_VO]) ||
						...
						if (!OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_DOZE)) {
							...
							pAd->StaCfg.ThisTbttNumToNextWakeUp =
							    TbttNumToNextWakeUp;
...
VOID ScanTimeoutAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (pAd->cfg80211_ctrl.Cfg80211FreqScanning) {
		...
		DBGPRINT(RT_DEBUG_TRACE, ("%s --- Set the MlmeAux.Channel=%d, stop the next Timer Scan\n", __func__,
					   pAd->MlmeAux.Channel));
	}
