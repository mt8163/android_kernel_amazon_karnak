...
NDIS_STATUS os_alloc_mem(IN VOID *pReserved, OUT UCHAR **mem, IN ULONG size)
{
...
NDIS_STATUS os_alloc_mem_suspend(IN VOID *pReserved, OUT UCHAR **mem, IN ULONG size)
{
...
/* pAd MUST allow to be NULL */
NDIS_STATUS os_free_mem(IN VOID *pReserved, IN PVOID mem)
{
	ASSERT(mem);
	kfree(mem);
...
int os_set_chip_reset(void)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("No chip reset implementation\n"));
...
VOID RtmpDrvAllMacPrint(IN VOID *pReserved,
			IN UINT32 *pBufMac,
			IN UINT32 AddrStart, IN UINT32 AddrEnd, IN UINT32 AddrStep)
{
	struct file *file_w;
