	...
	if (pKeyInfo->KeyType == RT_CMD_80211_KEY_WEP40
		...
		if (IoctlSec.ext_flags == RT_CMD_STA_IOCTL_SECURTIY_EXT_SET_TX_KEY) {
			if (RTMP_CFG80211_VIF_P2P_GO_ON(pAd)) {
				...
				if (pMacEntry) {
					if (IS_ENTRY_APCLI(pMacEntry) && (pMacEntry->PairwiseKey.KeyLen == LEN_TK)) {
...
VOID CFG80211_LostApInform(IN VOID *pAdCB)
{
	...
	PRTMP_ADAPTER pAd = (PRTMP_ADAPTER) pAdCB;
	DBGPRINT(RT_DEBUG_OFF, ("80211> CFG80211_LostApInform ==> %d\n",
	...
	for (band_id = start_band; band_id < sup_band; band_id++) {
		...
		for (ch_id = 0; ch_id < num_ch; ch_id++) {
			...
			if (idx != Num_Cfg80211_Chan) {
				if (!(ch->flags & flags_updated[idx].flags & IEEE80211_CHAN_NO_IR)
					&& (ch->flags & IEEE80211_CHAN_NO_IR)) {
					ch->flags &= ~IEEE80211_CHAN_NO_IR;
					pAd->ChannelList[drv_idx].Flags &= ~IEEE80211_CHAN_NO_IR;
					CFG80211DBG(RT_DEBUG_TRACE,
						("IEEE80211_CHAN_NO_IR update beacon flags %x ch:%u\n"
				  ...
				  IN UCHAR *pRspIe, IN UINT32 RspIeLen, IN UCHAR FlgIsSuccess)
{
	...
	CFG80211DBG(RT_DEBUG_OFF, ("80211> CFG80211_ConnectResultInform ==> FlgIsSuccess(%d)\n", FlgIsSuccess));
	...
	if (bss) {
		...
		*/
		...
		DBGPRINT(RT_DEBUG_OFF,
			("%s found BSS [0x%2X:0x%2X:0x%2X:0x%2X:0x%2X:0x%2X] rssi(%d)in kernel, CH freq %d\n"
			, __func__, PRINT_MAC(bss->bssid), bss->signal, bss->channel->center_freq));
