		...
		if (TRUE == pAd->cfg80211_ctrl.IsNeedTxStatus) {
			...
			ret =
			    CFG80211_TxStatusCmd(pAd, TXS_TIMEOUT,
						 (UCHAR) pCfg80211_ctrl->TxStatusSeq, TXS_P2P_WCID,
						 TXS_P2P_MgmtID, Data);
			DBGPRINT(RT_DEBUG_TRACE,
			...
			if (!RTMP_OS_WAIT_FOR_COMPLETION_TIMEOUT
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("%s,  can't get tx status Timeout(%d)ms\n", __func__,
					  (TXS_TIMEOUT * 2)));
				CFG80211_SendMgmtFrameDone(pAd, pCfg80211_ctrl->TxStatusSeq, FALSE);
			}
			DBGPRINT(RT_DEBUG_TRACE,
...
BOOLEAN CFG80211_IsNeedTxStatus(IN VOID *pAdOrg, IN UCHAR *pFrame, UINT32 FrameLength)
{
	BOOLEAN IsNeedTxStatus = FALSE;
	...
	if (ieee80211_is_action(mgmt->frame_control)) {
		...
		if ((pP2PFrame->p80211Header.FC.SubType == SUBTYPE_ACTION) &&
			...
			IsNeedTxStatus = TRUE;
			switch (pP2PFrame->Subtype) {
			case GO_NEGOCIATION_REQ:
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: (%02X:%02X:%02X:%02X:%02X:%02X) -> (%02X:%02X:%02X:%02X:%02X:%02X) GO_NEGOTIATION_REQ %d\n",
					  __func__, PRINT_MAC(mgmt->sa), PRINT_MAC(mgmt->da),
					  pAd->LatchRfRegs.Channel));
				break;

			case GO_NEGOCIATION_RSP:
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: (%02X:%02X:%02X:%02X:%02X:%02X) -> (%02X:%02X:%02X:%02X:%02X:%02X) GO_NEGOTIATION_RSP %d\n",
					  __func__, PRINT_MAC(mgmt->sa), PRINT_MAC(mgmt->da),
					  pAd->LatchRfRegs.Channel));
				break;
			...
			case GO_NEGOCIATION_CONFIRM:
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: (%02X:%02X:%02X:%02X:%02X:%02X) -> (%02X:%02X:%02X:%02X:%02X:%02X) GO_NEGOTIATION_CONFIRM %d\n",
					  __func__, PRINT_MAC(mgmt->sa), PRINT_MAC(mgmt->da),
					  pAd->LatchRfRegs.Channel));
				break;
			case P2P_PROVISION_REQ:
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: (%02X:%02X:%02X:%02X:%02X:%02X) -> (%02X:%02X:%02X:%02X:%02X:%02X) P2P_PROVISION_REQ %d\n",
					  __func__, PRINT_MAC(mgmt->sa), PRINT_MAC(mgmt->da),
					  pAd->LatchRfRegs.Channel));
				break;
			...
			case P2P_PROVISION_RSP:
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: (%02X:%02X:%02X:%02X:%02X:%02X) -> (%02X:%02X:%02X:%02X:%02X:%02X) P2P_PROVISION_RSP %d\n",
					  __func__, PRINT_MAC(mgmt->sa), PRINT_MAC(mgmt->da),
					  pAd->LatchRfRegs.Channel));
				break;
			...
			case P2P_INVITE_REQ:
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: (%02X:%02X:%02X:%02X:%02X:%02X) -> (%02X:%02X:%02X:%02X:%02X:%02X) P2P_INVITE_REQ %d\n",
					  __func__, PRINT_MAC(mgmt->sa), PRINT_MAC(mgmt->da),
					  pAd->LatchRfRegs.Channel));
				break;
			...
			case P2P_INVITE_RSP:
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: (%02X:%02X:%02X:%02X:%02X:%02X) -> (%02X:%02X:%02X:%02X:%02X:%02X) P2P_INVITE_RSP %d\n",
					  __func__, PRINT_MAC(mgmt->sa), PRINT_MAC(mgmt->da),
					  pAd->LatchRfRegs.Channel));
				break;
			case P2P_DEV_DIS_REQ:
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: (%02X:%02X:%02X:%02X:%02X:%02X) -> (%02X:%02X:%02X:%02X:%02X:%02X) P2P_DEV_DIS_REQ %d\n",
					  __func__, PRINT_MAC(mgmt->sa), PRINT_MAC(mgmt->da),
					  pAd->LatchRfRegs.Channel));
				break;
			case P2P_DEV_DIS_RSP:
				DBGPRINT(RT_DEBUG_ERROR,
					 ("%s: (%02X:%02X:%02X:%02X:%02X:%02X) -> (%02X:%02X:%02X:%02X:%02X:%02X) P2P_DEV_DIS_RSP %d\n",
					  __func__, PRINT_MAC(mgmt->sa), PRINT_MAC(mgmt->da),
					  pAd->LatchRfRegs.Channel));
				break;

			default:
				IsNeedTxStatus = FALSE;
				DBGPRINT(RT_DEBUG_ERROR,
