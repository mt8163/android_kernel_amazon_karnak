...
static INT rlt_bbp_set_ctrlch(struct _RTMP_ADAPTER *pAd, UINT8 ext_ch)
{
	...
	if (pAd->CommonCfg.BBPCurrentBW == BW_80 &&
	    pAd->CommonCfg.Channel >= 36 && pAd->CommonCfg.vht_cent_ch) {
...
static INT rlt_bbp_set_agc(struct _RTMP_ADAPTER *pAd, UCHAR agc, RX_CHAIN_IDX chain)
{
	...
	if (((pAd->MACVersion & 0xf0000000) < 0x28830000) || (pAd->Antenna.field.RxPath == 1)) {
		chain = RX_CHAIN_0;
	}
		...
		if (idx & 0x01) {
			...
			DBGPRINT(RT_DEBUG_INFO,
				 ("%s(Idx):Write(R%d,val:0x%x) to Chain(0x%x, idx:%d)\n",
				  __func__, bbp_reg, bbp_val, chain, idx));
		}
		chain >>= 1;
		bbp_reg += 4;
		idx++;
...
static UCHAR rlt_bbp_get_random_seed(RTMP_ADAPTER *pAd)
{
	...
	seed = (UCHAR) ((value & 0xff) ^ ((value & 0xff00) >> 8) ^ ((value & 0xff0000) >> 16));
