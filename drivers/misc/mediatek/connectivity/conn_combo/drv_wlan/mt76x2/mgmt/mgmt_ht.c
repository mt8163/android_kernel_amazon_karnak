		   ...
		   RT_HT_CAPABILITY *my)
{
	...
	if ((peer->HtCapInfo.ChannelWidth) && (my->ChannelWidth)) {
		...
		pEntry->MaxHTPhyMode.field.ShortGI =
		    ((my->ShortGIfor40) & (peer->HtCapInfo.ShortGIfor40));
	} else {
		pEntry->MaxHTPhyMode.field.BW = BW_20;
		pEntry->MaxHTPhyMode.field.ShortGI =
		    ((my->ShortGIfor20) & (peer->HtCapInfo.ShortGIfor20));
...
UCHAR get_cent_ch_by_htinfo(RTMP_ADAPTER *pAd, ADD_HT_INFO_IE *ht_op, HT_CAPABILITY_IE *ht_cap)
{
	...
	if (ht_cap->HtCapInfo.ChannelWidth != BW_20) {
		if (RTMP_GetCustomizedChannelBw(pAd, ht_op->ControlChan) == BW_20)
			return ht_op->ControlChan;
	}
...
VOID RTMPSetHT(IN RTMP_ADAPTER *pAd, IN OID_SET_HT_PHYMODE * pHTPhyMode)
{
...
#if defined(CONFIG_AP_SUPPORT) || defined(MESH_SUPPORT)	/* zhilin_20140904 added */
	...
	if (CHAN_PropertyCheck(pAd, pAd->CommonCfg.Channel,
		...
		if (CHAN_PropertyCheck(pAd, pAd->CommonCfg.Channel, CHANNEL_NO_FAT_BELOW) == TRUE) {
			pHTPhyMode->ExtOffset = EXTCHA_ABOVE;
		}
	} else if (pHTPhyMode->ExtOffset == EXTCHA_ABOVE) {
		/* extension channel above this channel is not allowed */
		if (CHAN_PropertyCheck(pAd, pAd->CommonCfg.Channel, CHANNEL_NO_FAT_ABOVE) == TRUE) {
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("RTMPSetHT : HT_mode(%d), ExtOffset(%d), MCS(%d), BW(%d), STBC(%d), SHORTGI(%d)\n",
		  pHTPhyMode->HtMode, pHTPhyMode->ExtOffset, pHTPhyMode->MCS, pHTPhyMode->BW,
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("RTMPSetHT : AMsduSize = %d, MimoPs = %d, MpduDensity = %d, MaxRAmpduFactor = %d\n",
		  rt_ht_cap->AmsduSize, rt_ht_cap->MimoPs, rt_ht_cap->MpduDensity,
	...
	if (pHTPhyMode->BW == BW_40) {
		...
		pAd->CommonCfg.AddHTInfo.AddHtInfo.ExtChanOffset =
		    (pHTPhyMode->ExtOffset == EXTCHA_BELOW) ? (EXTCHA_BELOW) : EXTCHA_ABOVE;
		...
		if ((pAd->OpMode == OPMODE_AP) || INFRA_ON(pAd) || ADHOC_ON(pAd)
		    ...
		    || (RTMP_CFG80211_VIF_P2P_GO_ON(pAd) && (wdev->bw == BW_40))
...
#ifdef DOT11_VHT_AC
	if (pHTPhyMode->BW == BW_40 &&
	    pAd->CommonCfg.vht_bw == VHT_BW_80 && pAd->CommonCfg.vht_cent_ch)
		bw = BW_80;
#endif /* DOT11_VHT_AC */
...
VOID RTMPSetIndividualHT(RTMP_ADAPTER *pAd, UCHAR apidx)
{
	...
	do {
		...
		if (apidx >= MIN_NET_DEVICE_FOR_CFG80211_VIF_P2P_GO) {
			...
			pAd->ApCfg.MBSSID[idx].wdev.bAutoTxRateSwitch =
		...
		if (apidx >= MIN_NET_DEVICE_FOR_APCLI) {
			...
			if (idx < MAX_APCLI_NUM) {
				...
				DesiredMcs =
				    pAd->ApCfg.ApCliTab[idx].wdev.DesiredTransmitSetting.field.MCS;
				...
				DBGPRINT(RT_DEBUG_ERROR,
					 ("RTMPSetIndividualHT: invalid idx(%d)\n", idx));
				return;
			}
		}
#endif /* APCLI_SUPPORT */
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			if (apidx >= MIN_NET_DEVICE_FOR_WDS) {
				...
				if (idx < MAX_WDS_ENTRY) {
					pDesired_ht_phy =
					    &pAd->WdsTab.WdsEntry[idx].wdev.DesiredHtPhyInfo;
					DesiredMcs =
					    pAd->WdsTab.WdsEntry[idx].wdev.DesiredTransmitSetting.
					    field.MCS;
					/*encrypt_mode = pAd->WdsTab.WdsEntry[idx].WepStatus; */
					pAd->WdsTab.WdsEntry[idx].wdev.bAutoTxRateSwitch =
					    (DesiredMcs == MCS_AUTO) ? TRUE : FALSE;
					break;
				} else {
					DBGPRINT(RT_DEBUG_ERROR,
			...
			if ((apidx < pAd->ApCfg.BssidNum) && (apidx < MAX_MBSSID_NUM(pAd))
				...
				if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode)
					...
					pAd->ApCfg.MBSSID[apidx].DesiredTransmitSetting.field.
...
#ifdef CONFIG_STA_SUPPORT
	if ((pAd->OpMode == OPMODE_STA) && (pAd->StaCfg.BssType == BSS_INFRA)
...
VOID RTMPDisableDesiredHtInfo(RTMP_ADAPTER *pAd)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		...
		for (idx = 0; idx < MAX_WDS_ENTRY; idx++) {
			RTMPZeroMemory(&pAd->WdsTab.WdsEntry[idx].wdev.DesiredHtPhyInfo,
				       sizeof(RT_PHY_INFO));
		}
#endif /* WDS_SUPPORT */
#ifdef APCLI_SUPPORT
		for (idx = 0; idx < MAX_APCLI_NUM; idx++) {
			RTMPZeroMemory(&pAd->ApCfg.ApCliTab[idx].wdev.DesiredHtPhyInfo,
				       sizeof(RT_PHY_INFO));
		}
#endif /* APCLI_SUPPORT */
	}
