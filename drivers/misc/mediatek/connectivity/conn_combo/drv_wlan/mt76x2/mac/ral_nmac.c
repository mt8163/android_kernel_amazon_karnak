...
INT rlt_get_rxwi_phymode(RXWI_STRUCT *rxwi)
{
	return rxwi->RXWI_N.phy_mode;
}

INT rlt_get_rxwi_rssi(RXWI_STRUCT *rxwi, INT size, CHAR *rssi)
{
...
INT rlt_get_rxwi_snr(RTMP_ADAPTER *pAd, RXWI_STRUCT *rxwi, INT size, UCHAR *snr)
{
	if (IS_MT76x2(pAd)) {
...
VOID dump_rlt_txwi(RTMP_ADAPTER *pAd, TXWI_STRUCT *pTxWI)
{
...
VOID dump_rlt_rxwi(RTMP_ADAPTER *pAd, RXWI_STRUCT *pRxWI)
{
...
VOID dump_rlt_txinfo(RTMP_ADAPTER *pAd, TXINFO_STRUCT *pTxInfo)
{
