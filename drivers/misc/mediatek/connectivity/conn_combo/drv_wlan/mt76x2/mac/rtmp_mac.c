...
/*
	...
	========================================================================
*/
VOID RTMPWriteTxWI(IN RTMP_ADAPTER *pAd, IN TXWI_STRUCT *pOutTxWI, IN BOOLEAN FRAG, IN BOOLEAN CFACK, IN BOOLEAN InsTimestamp,
			IN BOOLEAN AMPDU,
			IN BOOLEAN Ack,
			...
			IN UCHAR TxRate, IN UCHAR Txopmode, IN HTTRANSMIT_SETTING *pTransmit)
{
	...
	UINT TxEAPId_Cal;
	UCHAR eTxBf, iTxBf, sounding = 0, ndp_rate = 0, stbc, bw, mcs, sgi, phy_mode, mpdu_density =
	    0, mimops = 0, ldpc = 0;
	...
	if (bw)
		bw = (pAd->CommonCfg.AddHTInfo.AddHtInfo.RecomWidth ==
		      0) ? (BW_20) : (pTransmit->field.BW);
	...
	TxEAPId_Cal =
	    WCID + (UINT) ((pAd->CommonCfg.Bssid[5] >> 4) ^ (pAd->CommonCfg.Bssid[5] & 0x0F)) * 32;
		...
		if (pMac->TxSndgType == SNDG_TYPE_NDP || pMac->TxSndgType == SNDG_TYPE_SOUNDING) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s : BSSID[5] = %x, txwi_n->TxEAPId = %d\n", __func__,
				  pAd->CommonCfg.Bssid[5], TxEAPId_Cal));
		}
	}
...
VOID RTMPWriteTxWI_Data(RTMP_ADAPTER *pAd, TXWI_STRUCT *pTxWI, TX_BLK *pTxBlk)
{
	...
	if (pMacEntry && IS_ENTRY_DLS(pMacEntry) && (pAd->StaCfg.BssType == BSS_INFRA))
		wcid = BSSID_WCID;
	else
	...
	if (pAd->Mlme.bpkt_dbg) {
		DBGPRINT(RT_DEBUG_TRACE,
			 (" @@@@ write_tmac_info_Data: wcid=%d , phy_mode=%d, pTransmit->field.BW=%u\n",
			  pTxBlk->Wcid, phy_mode, pTransmit->field.BW));
	}
#endif /* CONFIG_MULTI_CHANNEL */
	...
	if (pTxBlk->TxSndgPkt == SNDG_TYPE_SOUNDING) {
		...
		DBGPRINT(RT_DEBUG_TRACE, ("ETxBF in %s(): sending normal sounding, eTxBF=%d\n",
					  __func__, pTransmit->field.eTxBF));
		iTxBf = 0;
	...
	if (pMacEntry) {
		if ((pMacEntry->MmpsMode == MMPS_DYNAMIC) && (mcs > 7))
			...
			if ((pTransmit->field.MODE == MODE_HTMIX
			     || pTransmit->field.MODE == MODE_HTGREENFIELD) && (mcs > 7)) {
				mcs = 7;
				mimops = 0;
			}
		}
...
#ifdef DBG_TX_MCS
		if (pTransmit->field.MODE == MODE_HTMIX
		    || pTransmit->field.MODE == MODE_HTGREENFIELD) {
			if (mcs < MAX_MCS_SET)
	...
	if ((RTMP_TEST_MORE_FLAG(pAd, fASIC_CAP_MCS_LUT)) &&
	    (wcid < 128) && (pMacEntry && pMacEntry->bAutoTxRateSwitch == TRUE)) {
	...
	if (pAd->chipCap.hif_type == HIF_RLT) {
		...
		if (pAd->Mlme.bpkt_dbg)
			DBGPRINT(RT_DEBUG_TRACE,
				 ("!! @@@@ write_tmac_info_Data : wcid %d mcs   %d bw  %d  basize %d\n",
				  pTxBlk->Wcid, mcs, bw, basize));
#endif /* CONFIG_MULTI_CHANNEL */
		txwi_n->MCS = mcs;
		txwi_n->PHYMODE = phy_mode;
		txwi_n->BW = bw;
	...
	/* Update LastTxRate here */
	if (pMacEntry)
		pMacEntry->LastTxRate = (UINT32) pTransmit->word;
}

VOID RTMPWriteTxWI_Cache(RTMP_ADAPTER *pAd, TXWI_STRUCT *pTxWI, TX_BLK *pTxBlk)
{
	...
	if (pAd->Mlme.bpkt_dbg)
		DBGPRINT(RT_DEBUG_TRACE,
			 (" $$$$ write_tmac_info_Cache: wcid=%d , phy_mode=%d, pTransmit->field.BW=%u\n",
			  pTxBlk->Wcid, phy_mode, pTransmit->field.BW));
#endif /* CONFIG_MULTI_CHANNEL */
	...
	lut_enable = FALSE;
	if (RTMP_TEST_MORE_FLAG(pAd, fASIC_CAP_MCS_LUT) &&
	    (pTxBlk->Wcid < 128) && (pMacEntry && pMacEntry->bAutoTxRateSwitch == TRUE)) {
		HTTRANSMIT_SETTING rate_ctrl;
	...
	if (pAd->Mlme.bpkt_dbg)
		DBGPRINT(RT_DEBUG_TRACE,
			 ("$$$$ write_tmac_info_Cache: wcid %d mcs   %d bw  %d  basize %d\n",
			  pTxBlk->Wcid, mcs, bw, basize));
#endif /* CONFIG_MULTI_CHANNEL */
...
INT get_pkt_rssi_by_rxwi(RTMP_ADAPTER *pAd, RXWI_STRUCT *rxwi, INT size, CHAR *rssi)
{
	INT status = 0;
...
INT get_pkt_snr_by_rxwi(RTMP_ADAPTER *pAd, RXWI_STRUCT *rxwi, INT size, UCHAR *snr)
{
	INT status = 0;
...
INT get_pkt_phymode_by_rxwi(RTMP_ADAPTER *pAd, RXWI_STRUCT *rxwi)
{
	INT status = 0;
...
#ifdef MCS_LUT_SUPPORT
INT set_lut_phy_rate(RTMP_ADAPTER *pAd, UINT8 wcid,
		     UINT8 mcs, UINT8 bw, UINT8 gi, UINT8 stbc, UINT8 mode)
{
	UINT32 mac_reg = 0;
...
VOID rtmp_mac_bcn_buf_init(IN RTMP_ADAPTER *pAd)
{
	...
	for (idx = 0; idx < pChipCap->BcnMaxHwNum; idx++) {
		DBGPRINT(RT_DEBUG_TRACE, ("\t\tBcnBase[%d] = 0x%x, pAd->BeaconOffset[%d]=0x%x\n",
					  idx, pChipCap->BcnBase[idx], idx,
					  pAd->BeaconOffset[idx]));
	}
	...
	if (pAd->chipCap.hif_type == HIF_RTMP) {
		{
			RTMP_REG_PAIR bcn_legacy_reg_tb[] = {
				...
				,	/* 0x3800, 0x3A00, 0x3C00, 0x3E00, 512B for each beacon */
...
RTMP_REG_PAIR MACRegTable[] = {
	...
	,			/*  Basic rate set bitmap */
	{HT_BASIC_RATE, 0x00004003}
	,			/* Basic HT rate set , 20M, MCS=3, MM. Format is the same as in TXWI. */
	{MAC_SYS_CTRL, 0x00}
	,			/* 0x1004, , default Disable RX */
	{RX_FILTR_CFG, 0x17f97}
	,			/*0x1400  , RX filter control,  */
	{BKOFF_SLOT_CFG, 0x209}
	,			/* default set short slot time, CC_DELAY_TIME should be 2        */
	{TX_SW_CFG1, 0x80606}
	,			/* Gary,2006-08-23 */
	{TX_LINK_CFG, 0x1020}
	,			/* Gary,2006-08-23 */
	...
	,			/* CCK has some problem. So increase timieout value. 2006-10-09 MArvek RT , Modify for 2860E ,2007-08-01 */
...
/*	{TX_RTY_CFG,			0x6bb80101},	 sample*/
	...
	,			/* Jan, 2006/11/16, Set TxWI->ACK =0 in Probe Rsp Modify for 2860E ,2007-08-03 */

	{AUTO_RSP_CFG, 0x00000013}
	,			/* Initial Auto_Responder, because QA will turn off Auto-Responder */
	{CCK_PROT_CFG, 0x05740003 /*0x01740003 */ }
	,			/* Initial Auto_Responder, because QA will turn off Auto-Responder. And RTS threshold is enabled. */
	{OFDM_PROT_CFG, 0x05740003 /*0x01740003 */ }
	,			/* Initial Auto_Responder, because QA will turn off Auto-Responder. And RTS threshold is enabled. */
	...
	,			/* set 19:18 --> Short NAV for MIMO PS */
...
RTMP_REG_PAIR MACRegTable_RTMP[] = {
	...
	,			/* 0x3018, MAX frame length. Max PSDU = 16kbytes. */
	{PWR_PIN_CFG, 0x3}
	,			/* patch for 2880-E */
	/*{TX_SW_CFG0,          0x40a06},  Gary,2006-08-23 */
	{TX_SW_CFG0, 0x0}
	,			/* Gary,2008-05-21 for CWC test */
};
#endif /* RTMP_MAC */
...
RTMP_REG_PAIR APMACRegTable[] = {
	...
	{TX_CHAIN_ADDR0_L, 0xFFFFFFFF}
	,			/* Broadcast frames are in stream mode */
...
INT rtmp_mac_init(RTMP_ADAPTER *pAd)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
		for (idx = 0; idx < NUM_AP_MAC_REG_PARMS; idx++) {
			RTMP_IO_WRITE32(pAd, APMACRegTable[idx].Register, APMACRegTable[idx].Value);
		}
	}
#endif /* CONFIG_AP_SUPPORT */
	...
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		for (idx = 0; idx < NUM_STA_MAC_REG_PARMS; idx++) {
			RTMP_IO_WRITE32(pAd,
					STAMACRegTable[idx].Register, STAMACRegTable[idx].Value);
		}
	}
#endif /* CONFIG_STA_SUPPORT */
