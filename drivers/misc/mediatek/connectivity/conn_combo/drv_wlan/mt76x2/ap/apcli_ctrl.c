...
static VOID ApCliCtrlJoinReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliCtrlJoinReqTimeoutAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliCtrlProbeRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliCtrlAuthRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliCtrlAuth2RspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliCtrlAuthReqTimeoutAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliCtrlAuth2ReqTimeoutAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliCtrlAssocRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliCtrlDeAssocRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliCtrlAssocReqTimeoutAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliCtrlDisconnectReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliCtrlPeerDeAssocReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliCtrlDeAssocAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliCtrlDeAuthAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliWpaMicFailureReportFrame(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

#ifdef APCLI_CERT_SUPPORT
static VOID ApCliCtrlScanDoneAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);
#endif /* APCLI_CERT_SUPPORT */

#ifdef APCLI_CONNECTION_TRIAL
static VOID ApCliCtrlTrialConnectAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);
			       ...
			       IN STATE_MACHINE * Sm, OUT STATE_MACHINE_FUNC Trans[])
{
	...
	StateMachineSetAction(Sm, APCLI_CTRL_PROBE, APCLI_CTRL_JOIN_REQ_TIMEOUT,
	...
	StateMachineSetAction(Sm, APCLI_CTRL_AUTH, APCLI_CTRL_AUTH_REQ_TIMEOUT,
	...
	StateMachineSetAction(Sm, APCLI_CTRL_ASSOC, APCLI_CTRL_PEER_DISCONNECT_REQ,
			      (STATE_MACHINE_FUNC) ApCliCtrlPeerDeAssocReqAction);
	...
	/* connected state */
	StateMachineSetAction(Sm, APCLI_CTRL_CONNECTED, APCLI_CTRL_DISCONNECT_REQ,
...
#endif /* APCLI_CERT_SUPPORT */
#ifdef APCLI_CONNECTION_TRIAL
	StateMachineSetAction(Sm, APCLI_CTRL_CONNECTED, APCLI_CTRL_TRIAL_CONNECT,
			      (STATE_MACHINE_FUNC) ApCliCtrlTrialConnectAction);
	StateMachineSetAction(Sm, APCLI_CTRL_DISCONNECTED, APCLI_CTRL_TRIAL_CONNECT,
			      (STATE_MACHINE_FUNC) ApCliCtrlTrialConnectAction);
	StateMachineSetAction(Sm, APCLI_CTRL_TRIAL_TRIGGERED, /* for retry */
						APCLI_CTRL_JOIN_REQ_TIMEOUT,
						(STATE_MACHINE_FUNC) ApCliCtrlTrialConnectAction);
#endif /* APCLI_CONNECTION_TRIAL */
	...
	for (i = 0; i < MAX_APCLI_NUM; i++) {
		...
		for (j = 0; j < MAX_EXT_MAC_ADDR_SIZE; j++) {
			...
			pAd->ApCfg.ApCliTab[i].RepeaterCli[j].CtrlCurrState =
				     ...
				     IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("ApCli_SYNC - TrialConnectTimeout, Jump back to original Channel\n"));
	...
	if (*pCurrState == APCLI_CTRL_ASSOC) {
		...
		if (pApCliEntry->NewRootApRetryCnt >= 10) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s, RetryCnt:%d, pCurrState = %d,\n", __func__,
				  pApCliEntry->NewRootApRetryCnt, *pCurrState));
		...
		} else/* Disconnected State will bring the next probe req, auth req. */
					   ...
					   IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("ApCli_SYNC - %s,Jump back to trial channel:%d",
		__func__, pApCliEntry->TrialCh));
	DBGPRINT(RT_DEBUG_TRACE, ("to issue Assoc Req to new root AP\n"));
	...
	AssocParmFill(pAd, &AssocReq, pAd->ApCfg.ApCliTab[pApCliEntry->ifIndex].MlmeAux.Bssid,
	...
	if ((pWpsCtrl->WscConfMode != WSC_DISABLE) && (pWpsCtrl->bWscTrigger == TRUE)) {
		...
		if (pWpsCtrl->WscMode == 1) {	/* PIN */
			...
			if (bss_idx == BSS_NOT_FOUND) {
				...
				UCHAR channel = pAd->CommonCfg.Channel, RootApChannel =
				    pAd->ScanTab.BssEntry[bss_idx].Channel;
				UCHAR RootApCentralChannel =
				...
				if ((RootApChannel != channel) ||
					...
					snprintf(ChStr, sizeof(ChStr), "%d",
						 pAd->ScanTab.BssEntry[bss_idx].Channel);
					Set_Channel_Proc(pAd, ChStr);
					/*
					   ApStop will call WscStop, 
					   we need to reset WscConfMode,
					   WscMode & bWscTrigger here.
					 */
	...
	if (pApCliEntry->CfgSsidLen != 0) {
		...
		if (bss_idx == BSS_NOT_FOUND) {
			...
			*pCurrState = APCLI_CTRL_PROBE;
			...
			RT_CFG80211_P2P_CLI_CONN_RESULT_INFORM(pAd, JoinReq.Bssid, NULL, 0, NULL, 0,
							       0);
		...
		*pCurrState = APCLI_CTRL_ASSOC;
		...
		if (pApCliEntry->TrialCh == 0) {
			...
			AssocParmFill(pAd, &AssocReq, pApCliEntry->MlmeAux.Bssid,
				      pApCliEntry->MlmeAux.CapabilityInfo, ASSOC_TIMEOUT, 5);
#ifdef APCLI_CONNECTION_TRIAL
		}
#endif /* APCLI_CONNECTION_TRIAL */
...
static VOID ApCliCtrlAssocReqTimeoutAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	*pCurrState = APCLI_CTRL_ASSOC;
	...
	AssocParmFill(pAd, &AssocReq, pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Bssid,
		      pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.CapabilityInfo, ASSOC_TIMEOUT, 5);
...
static VOID ApCliCtrlDeAssocAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	DisassocParmFill(pAd, &DisassocReq, pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Bssid,
...
VOID ApCliWpaMicFailureReportFrame(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	SET_UINT16_TO_ARRARY(pPacket->Body_Len, MIN_LEN_OF_EAPOL_KEY_MSG)
	    ...
	    /* Key Replay Count */
	    NdisMoveMemory(pPacket->KeyDesc.ReplayCounter, apcli_entry->ReplayCounter,
			   LEN_KEY_DESC_REPLAY);
	...
	*((USHORT *) &pPacket->KeyDesc.KeyInfo) =
	...
	if (wdev->WepStatus == Ndis802_11Encryption3Enabled) {	/* AES */
		UCHAR digest[20] = { 0 };

		RT_HMAC_SHA1(apcli_entry->PTK, LEN_PTK_KCK, pOutBuffer, FrameLen, digest,
			     SHA1_DIGEST_SIZE);
		NdisMoveMemory(Mic, digest, LEN_KEY_DESC_MIC);
	} else {		/* TKIP */
		RT_HMAC_MD5(apcli_entry->PTK, LEN_PTK_KCK, pOutBuffer, FrameLen, Mic,
			    MD5_DIGEST_SIZE);
	}
	...
	RTMPToWirelessSta(pAd, &pAd->MacTab.Content[Wcid],
			  ...
			  (PUCHAR) pPacket, CONV_ARRARY_TO_UINT16(pPacket->Body_Len) + 4, FALSE);
	...
	if ((pAd->CommonCfg.BSSCoexist2040.field.InfoReq == 1)
		...
		for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
			if (IS_ENTRY_APCLI(&pAd->MacTab.Content[i])
			    && (pAd->MacTab.Content[i].apidx == ifIndex)) {
