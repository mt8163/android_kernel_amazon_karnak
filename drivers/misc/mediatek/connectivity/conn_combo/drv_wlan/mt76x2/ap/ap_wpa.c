...
/*
    ...
    ==========================================================================
*/
static BOOLEAN RTMPCheckMcast(IN PRTMP_ADAPTER pAd,
			      IN PEID_STRUCT eid_ptr, IN MAC_TABLE_ENTRY *pEntry)
{
	UCHAR apidx;
	...
	if (eid_ptr->Len >= 6) {
		...
		if (eid_ptr->Eid == IE_WPA) {
			...
			if (NdisEqualMemory
			    (&eid_ptr->Octet[6], &pAd->ApCfg.MBSSID[apidx].RSN_IE[0][6], 4))
				return TRUE;
			...
			if (NdisEqualMemory
			    (&eid_ptr->Octet[2], &pAd->ApCfg.MBSSID[apidx].RSN_IE[IE_Idx][2], 4))
				return TRUE;
		}
	}
...
/*
    ...
    ==========================================================================
*/
static BOOLEAN RTMPCheckUcast(IN PRTMP_ADAPTER pAd,
			      IN PEID_STRUCT eid_ptr, IN MAC_TABLE_ENTRY *pEntry)
{
	PUCHAR pStaTmp;
	USHORT Count;
	...
	if (eid_ptr->Len < 16) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("[ERROR]RTMPCheckUcast : the length is too short(%d)\n", eid_ptr->Len));
		return FALSE;
	}
	...
	if (eid_ptr->Len >= 16) {
		if (eid_ptr->Eid == IE_WPA) {
			if (wdev->WepStatus == Ndis802_11TKIPAESMix) {	/* multiple cipher (TKIP/CCMP) */
				...
				while (Count > 0) {
					...
					if (MIX_CIPHER_WPA_TKIP_ON(wdev->WpaMixPairCipher)) {
						...
						if (RTMPEqualMemory
						    (pStaTmp,
						     &pAd->ApCfg.MBSSID[apidx].RSN_IE[0][12], 4)) {
				...
				while (Count > 0) {
					if (RTMPEqualMemory
					    (pStaTmp, &pAd->ApCfg.MBSSID[apidx].RSN_IE[0][12], 4))
						return TRUE;
			...
			if (wdev->WepStatus == Ndis802_11TKIPAESMix) {	/* multiple cipher (TKIP/CCMP) */
				...
				while (Count > 0) {
					...
					if (MIX_CIPHER_WPA2_TKIP_ON(wdev->WpaMixPairCipher)) {
						...
						if (RTMPEqualMemory
						    (pStaTmp,
						     &pAd->ApCfg.MBSSID[apidx].RSN_IE[IE_Idx][8],
						     4)) {
							pEntry->WepStatus = Ndis802_11TKIPEnable;
							return TRUE;
						}
				...
				while (Count > 0) {
					if (RTMPEqualMemory
					    (pStaTmp, &pAd->ApCfg.MBSSID[apidx].RSN_IE[IE_Idx][8],
					     4))
						return TRUE;
...
/*
    ...
    ==========================================================================
*/
static BOOLEAN RTMPCheckAUTH(IN PRTMP_ADAPTER pAd,
			     IN PEID_STRUCT eid_ptr, IN MAC_TABLE_ENTRY *pEntry)
{
	PUCHAR pStaTmp;
	USHORT Count;
	...
	if (eid_ptr->Len < 16) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("RTMPCheckAUTH ==> WPAIE len is too short(%d)\n", eid_ptr->Len));
		return FALSE;
	}
	...
	if (eid_ptr->Eid == IE_WPA2) {
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("RTMPCheckAUTH ==> Unknown WPAIE, WPAIE=%d\n", eid_ptr->Eid));
		return FALSE;
	}
	...
	if (eid_ptr->Len >= 16) {
		if (eid_ptr->Eid == IE_WPA) {
			while (Count > 0) {
				if (RTMPCheckAKM
				    (pStaTmp, &pAd->ApCfg.MBSSID[apidx].RSN_IE[0][0], 0))
					return TRUE;
			...
			while (Count > 0) {
				if (RTMPCheckAKM
				    (pStaTmp, &pAd->ApCfg.MBSSID[apidx].RSN_IE[IE_Idx][0], 1))
					return TRUE;
...
VOID HandleCounterMeasure(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry)
{
	...
	/* record which entry causes this MIC error,
	*if this entry sends disauth/disassoc,
	*AP doesn't need to log the CM
	*/
	...
	if (pAd->ApCfg.CMTimerRunning == TRUE) {
		...
		/* renew GTK */
		GenRandom(pAd, pAd->ApCfg.MBSSID[pEntry->apidx].wdev.bssid,
			  pAd->ApCfg.MBSSID[pEntry->apidx].GNonce);
		  ...
		  IN PVOID FunctionContext, IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if ((pEntry) && IS_ENTRY_CLIENT(pEntry)) {
		...
		pEntry->ReTryCounter++;
		DBGPRINT(RT_DEBUG_TRACE,
			 ("WPARetryExec---> ReTryCounter=%d, WpaState=%d\n", pEntry->ReTryCounter,
			  pEntry->WpaState));
		...
		case Ndis802_11AuthModeWPA2PSK:
			...
			if (pEntry->ReTryCounter > (GROUP_MSG1_RETRY_TIMER_CTR + 1)) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("WPARetryExec::Group Key HS exceed retry count"));
				DBGPRINT(RT_DEBUG_TRACE,
					 (", Disassociate client, pEntry->ReTryCounter %d\n",
					  pEntry->ReTryCounter));
			...
			/* 2. Retry GTK. */
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("WPARetryExec::ReTry 2-way group-key Handshake\n"));
				if (pEntry->GTKState == REKEY_NEGOTIATING) {
					WPAStart2WayGroupHS(pAd, pEntry);
					RTMPSetTimer(&pEntry->RetryTimer,
			...
			/* 3. 4-way message 1 retried more than three times. Disconnect client */
				...
				RTMPSendWirelessEvent(pAd, IW_PAIRWISE_HS_TIMEOUT_EVENT_FLAG,
						      pEntry->Addr, pEntry->apidx, 0);

				DBGPRINT(RT_DEBUG_TRACE,
					 ("WPARetryExec::MSG1 timeout, pEntry->ReTryCounter = %d\n",
					  pEntry->ReTryCounter));
			...
			/* 4. Retry 4 way message 1, the last try, the timeout is 3 sec for EAPOL-Start */
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("WPARetryExec::Retry MSG1, the last try\n"));
			...
			/* 4. Retry 4 way message 1 */
				...
				if ((pEntry->WpaState == AS_PTKSTART)
					...
					DBGPRINT(RT_DEBUG_TRACE,
						 ("WPARetryExec::ReTry MSG1 of 4-way Handshake\n"));
		...
		if (pEntry->AuthMode == Ndis802_11AuthModeWPA
			...
			if (pEntry->wdev_idx < MAX_APCLI_NUM) {
				...
				if ((pEntry->bReptCli) && (pAd->ApCfg.bMACRepeaterEn == TRUE))
					ifIndex =
					    (64 + ifIndex * MAX_EXT_MAC_ADDR_SIZE +
					     pEntry->MatchReptCliIdx);

				if ((pAd->ApCfg.bMACRepeaterEn == TRUE) && (pEntry->bReptCli)) {

					RTMPRemoveRepeaterDisconnectEntry(pAd, pEntry->wdev_idx,
									  pEntry->MatchReptCliIdx);
					RTMPRemoveRepeaterEntry(pAd, pEntry->wdev_idx,
								pEntry->MatchReptCliIdx);
				} else
			...
			IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if (temp_counter > (pMbss->WPAREKEY.ReKeyInterval)) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Rekey Interval Excess, GKeyDoneStations=%d\n", pMbss->StaCount));
		...
		if ((pMbss->StaCount) > 0) {
			...
			for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
				...
				if (IS_ENTRY_CLIENT(pEntry) &&
					...
					WPAStart2WayGroupHS(pAd, pEntry);
					DBGPRINT(RT_DEBUG_TRACE,
						 ("Rekey interval excess, Update Group Key for"));
					DBGPRINT(RT_DEBUG_TRACE,
						 (" %x %x %x  %x %x %x , DefaultKeyId= %x\n",
	...
	if (pMbss->RekeyCountDown > 0) {
		...
		if (pMbss->RekeyCountDown == 0) {
			...
			WPAInstallSharedKey(pAd,
					    ...
					    apidx,
					    wdev->DefaultKeyId,
					    Wcid, TRUE, pMbss->GTK, LEN_TKIP_GTK);
		}
	}
...
/*
	...
	These frames come from Authenticator daemon.
	...
	if (RTMPEqualMemory((pPacket + 12), EAPOL, 2)) {
		...
		case EAP_CODE_SUCCESS:
			if ((pEntry->AuthMode >= Ndis802_11AuthModeWPA)
				...
				pEntry->WpaState = AS_PTKINITDONE;
				pAdapter->ApCfg.MBSSID[pEntry->apidx].wdev.PortSecured =
				    WPA_802_1X_PORT_SECURED;
				pEntry->PortSecured = WPA_802_1X_PORT_SECURED;
#ifdef WSC_AP_SUPPORT
				if (pAdapter->ApCfg.MBSSID[pEntry->apidx].WscControl.WscConfMode !=
				    WSC_DISABLE)
...
VOID RTMPAddPMKIDCache(IN PRTMP_ADAPTER pAd,
		       IN INT apidx, IN PUCHAR pAddr, IN UCHAR *PMKID, IN UCHAR *PMK)
{
	INT i, CacheIdx;

	/* Update PMKID status */
	CacheIdx = RTMPSearchPMKIDCache(pAd, apidx, pAddr;
	if (CacheIdx != -1) {
		NdisGetSystemUpTime(&
				    (pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[CacheIdx].
				     RefreshTime));
		NdisMoveMemory(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[CacheIdx].PMKID,
			       PMKID, LEN_PMKID);
	...
	for (i = 0; i < MAX_PMKID_COUNT; i++) {
		if (!pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[i].Valid) {
			...
			NdisGetSystemUpTime(&
					    (pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[i].
					     RefreshTime));
			COPY_MAC_ADDR(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[i].MAC, pAddr);
			NdisMoveMemory(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[i].PMKID,
				       PMKID, LEN_PMKID);
			NdisMoveMemory(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[i].PMK, PMK,
				       PMK_LEN);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("RTMPAddPMKIDCache add %02x:%02x:%02x:%02x:%02x:%02x cache(%d) from IF(ra%d)\n",
	...
	if (i == MAX_PMKID_COUNT) {
		...
		NdisGetSystemUpTime(&
				    (pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[idx].
				     RefreshTime));
		COPY_MAC_ADDR(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[idx].MAC, pAddr);
		NdisMoveMemory(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[idx].PMKID, PMKID,
			       LEN_PMKID);
		NdisMoveMemory(&pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[idx].PMK, PMK,
			       PMK_LEN);
		DBGPRINT(RT_DEBUG_TRACE,
			 ("RTMPAddPMKIDCache add %02x:%02x:%02x:%02x:%02x:%02x cache(%ld) from IF(ra%d)\n",
			  PRINT_MAC(pAddr), idx, apidx));
	}
}

INT RTMPSearchPMKIDCache(IN PRTMP_ADAPTER pAd, IN INT apidx, IN PUCHAR pAddr)
{
	INT i = 0;
	...
	for (i = 0; i < MAX_PMKID_COUNT; i++) {
		if ((pAd->ApCfg.MBSSID[apidx].PMKIDCache.BSSIDInfo[i].Valid)
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("RTMPSearchPMKIDCache %02x:%02x:%02x:%02x:%02x:%02x cache(%d) from IF(ra%d)\n",
				  PRINT_MAC(pAddr), i, apidx));
			break;
		}
	}
...
VOID RTMPDeletePMKIDCache(IN PRTMP_ADAPTER pAd, IN INT apidx, IN INT idx)
{
	...
	if (pInfo->Valid) {
		pInfo->Valid = FALSE;
		DBGPRINT(RT_DEBUG_TRACE,
			 ("RTMPDeletePMKIDCache(IF(%d), del PMKID CacheIdx=%d\n", apidx, idx));
	}
}

VOID RTMPMaintainPMKIDCache(IN PRTMP_ADAPTER pAd)
{
	INT i, j;
	ULONG Now;
	...
	for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
		...
		if ((wdev->WepStatus == Ndis802_11TKIPEnable) ||
			...
			if ((pMbss->WPAREKEY.ReKeyInterval != 0)
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 (" %s : Group rekey method= %ld , interval = 0x%lx\n",
					  __func__, pMbss->WPAREKEY.ReKeyMethod,
...
VOID RTMPHandleSTAKey(IN PRTMP_ADAPTER pAd, IN PMAC_TABLE_ENTRY pEntry, IN MLME_QUEUE_ELEM *Elem)
{
	ULONG FrameLen = 0;
...
/*        Offset += 2; */
	...
	NdisMoveMemory((PUCHAR) &peerKeyInfo, (PUCHAR) &pSTAKey->KeyDesc.KeyInfo,
		       sizeof(KEY_INFO));
	*((USHORT *) &peerKeyInfo) = cpu2le16(*((USHORT *) &peerKeyInfo));
	...
	if (!RTMPEqualMemory
	    (pSTAKey->KeyDesc.ReplayCounter, pEntry->R_Counter, LEN_KEY_DESC_REPLAY)) {
	...
	/*Benson add for big-endian 20081016 --> */
	if (peerKeyInfo.KeyMic && peerKeyInfo.Secure && peerKeyInfo.Request) {
	...
	NdisZeroMemory(pAd->ApCfg.MBSSID[pEntry->apidx].DlsPTK,
		       sizeof(pAd->ApCfg.MBSSID[pEntry->apidx].DlsPTK));
	...
	if (pEntry->WepStatus == Ndis802_11TKIPEnable) {
		RT_HMAC_MD5(pAd->ApCfg.MBSSID[pEntry->apidx].DlsPTK, LEN_PTK_KCK, (PUCHAR) pSTAKey,
			    MICMsgLen, mic, MD5_DIGEST_SIZE);
	} else {
		RT_HMAC_SHA1(pAd->ApCfg.MBSSID[pEntry->apidx].DlsPTK, LEN_PTK_KCK, (PUCHAR) pSTAKey,
			     MICMsgLen, digest, SHA1_DIGEST_SIZE);
	...
	if (pEntry->bDlsInit && !peerKeyInfo.Request) {	/*Benson add for big-endian 20081016 --> */
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Receive init STA's STAKey Message-2, STAKey handshake finished\n"));
		return;
	}
	...
	if (RTMPEqualMemory(&pSTAKey->KeyDesc.KeyData[2], OUI_WPA2_WEP40, 3)) {
		DBGPRINT(RT_DEBUG_WARN,
			 ("Receive a STAKey message which not support currently, just drop it\n"));
		return;
	}
	...
	do {
		...
		MAKE_802_3_HEADER(Header802_3, pDaEntry->Addr,
				  pAd->ApCfg.MBSSID[pDaEntry->apidx].wdev.bssid, EAPOL);
...
/*        NdisMoveMemory(pDaEntry->R_Counter, pAd->ApCfg.R_Counter, sizeof(pDaEntry->R_Counter)); */
		NdisMoveMemory(pOutPacket->KeyDesc.ReplayCounter, pDaEntry->R_Counter,
			       LEN_KEY_DESC_REPLAY);
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("STAKey handshake for peer STA %02x:%02x:%02x:%02x:%02x:%02x\n",
			  PRINT_MAC(DA)));

		if ((pDaEntry->AuthMode == Ndis802_11AuthModeWPA)
		    || (pDaEntry->AuthMode == Ndis802_11AuthModeWPAPSK)) {
			pOutPacket->KeyDesc.Type = WPA1_KEY_DESC;

			DBGPRINT(RT_DEBUG_TRACE,
				 ("pDaEntry->AuthMode == Ndis802_11AuthModeWPA/WPAPSK\n"));
		} else if ((pDaEntry->AuthMode == Ndis802_11AuthModeWPA2)
			   || (pDaEntry->AuthMode == Ndis802_11AuthModeWPA2PSK)) {
			pOutPacket->KeyDesc.Type = WPA2_KEY_DESC;
			pOutPacket->KeyDesc.KeyDataLen[1] = 0;

			DBGPRINT(RT_DEBUG_TRACE,
				 ("pDaEntry->AuthMode == Ndis802_11AuthModeWPA2/WPA2PSK\n"));
		}
		...
		if (pDaEntry->WepStatus == Ndis802_11AESEnable) {
			...
			NdisMoveMemory(&Key_Data[14 + LEN_TK], pEntry->PairwiseKey.TxMic,
				       LEN_TKIP_MIC);
			NdisMoveMemory(&Key_Data[14 + LEN_TK + LEN_TKIP_MIC],
				       pEntry->PairwiseKey.RxMic, LEN_TKIP_MIC);
		}
		...
		NdisZeroMemory(temp, 64);
		NdisMoveMemory(temp, "IEEE802.11 WIRELESS ACCESS POINT", 32);
		WpaDerivePTK(pAd, temp, temp, pAd->ApCfg.MBSSID[pEntry->apidx].wdev.bssid, temp, DA,
			     pAd->ApCfg.MBSSID[pEntry->apidx].DlsPTK, LEN_PTK);
		...
		*(USHORT *) (&pOutPacket->KeyDesc.KeyInfo) =
		    cpu2le16(*(USHORT *) (&pOutPacket->KeyDesc.KeyInfo));

		MakeOutgoingFrame(pOutBuffer, &FrameLen,
				  pOutPacket->Body_Len[1] + 4, pOutPacket, END_OF_ARGS);

		/* Calculate MIC */
		if (pDaEntry->WepStatus == Ndis802_11AESEnable) {
			RT_HMAC_SHA1(pAd->ApCfg.MBSSID[pEntry->apidx].DlsPTK, LEN_PTK_KCK,
				     pOutBuffer, FrameLen, digest, SHA1_DIGEST_SIZE);
			NdisMoveMemory(pOutPacket->KeyDesc.KeyMic, digest, LEN_KEY_DESC_MIC);
		} else {
			RT_HMAC_MD5(pAd->ApCfg.MBSSID[pEntry->apidx].DlsPTK, LEN_PTK_KCK,
				    pOutBuffer, FrameLen, mic, MD5_DIGEST_SIZE);
			NdisMoveMemory(pOutPacket->KeyDesc.KeyMic, mic, LEN_KEY_DESC_MIC);
		}

		RTMPToWirelessSta(pAd, pDaEntry, Header802_3, LENGTH_802_3, (PUCHAR) pOutPacket,
				  pOutPacket->Body_Len[1] + 4, FALSE);
...
VOID ApcliWpaSendEapolStart(IN PRTMP_ADAPTER pAd,
			    IN PUCHAR pBssid,
...
VOID ApCliRTMPReportMicError(IN PRTMP_ADAPTER pAd, IN PCIPHER_KEY pWpaKey, IN INT ifIndex)
{
	...
	MlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE, APCLI_MIC_FAILURE_REPORT_FRAME, 1, &unicastKey,
		    ifIndex);

	if (pAd->ApCfg.ApCliTab[ifIndex].MicErrCnt == 2) {
		DBGPRINT(RT_DEBUG_TRACE, (" MIC Error count = 2 Trigger Block timer....\n"));
		DBGPRINT(RT_DEBUG_TRACE,
			 (" pAd->ApCfg.ApCliTab[%d].LastMicErrorTime = %ld\n", ifIndex,
