...
static VOID ApCliAssocTimeout(IN PVOID SystemSpecific1,
			      IN PVOID FunctionContext,
			      IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);

#ifdef MAC_REPEATER_SUPPORT
static VOID ApCliAssocTimeoutExt(IN PVOID SystemSpecific1,
				 IN PVOID FunctionContext,
				 IN PVOID SystemSpecific2, IN PVOID SystemSpecific3);
...
static VOID ApCliMlmeDisassocReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliPeerAssocRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliPeerDisassocAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliAssocTimeoutAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliInvalidStateWhenAssoc(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliInvalidStateWhenDisassociate(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem);

static VOID ApCliAssocPostProc(IN PRTMP_ADAPTER pAd,
			       IN PUCHAR pAddr2,
				...
				IN STATE_MACHINE * S, OUT STATE_MACHINE_FUNC Trans[])
{
	...
	StateMachineInit(S, (STATE_MACHINE_FUNC *) Trans,
			 ...
			 (STATE_MACHINE_FUNC) Drop, APCLI_ASSOC_IDLE, APCLI_ASSOC_MACHINE_BASE);

	/* first column */
	StateMachineSetAction(S, APCLI_ASSOC_IDLE, APCLI_MT2_MLME_ASSOC_REQ,
			      (STATE_MACHINE_FUNC) ApCliMlmeAssocReqAction);
	StateMachineSetAction(S, APCLI_ASSOC_IDLE, APCLI_MT2_MLME_DISASSOC_REQ,
			      (STATE_MACHINE_FUNC) ApCliMlmeDisassocReqAction);
	StateMachineSetAction(S, APCLI_ASSOC_IDLE, APCLI_MT2_PEER_DISASSOC_REQ,
	...
	StateMachineSetAction(S, APCLI_ASSOC_WAIT_RSP, APCLI_MT2_PEER_DISASSOC_REQ,
			      (STATE_MACHINE_FUNC) ApCliPeerDisassocAction);
	StateMachineSetAction(S, APCLI_ASSOC_WAIT_RSP, APCLI_MT2_PEER_ASSOC_RSP,
			      (STATE_MACHINE_FUNC) ApCliPeerAssocRspAction);
	StateMachineSetAction(S, APCLI_ASSOC_WAIT_RSP, APCLI_MT2_ASSOC_TIMEOUT,
			      (STATE_MACHINE_FUNC) ApCliAssocTimeoutAction);
...
static VOID ApCliMlmeAssocReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (apcli_entry->bBlockAssoc == TRUE) {
		...
		*pCurrState = APCLI_ASSOC_IDLE;
	    ...
	    if (MlmeAssocReqSanity
		(pAd, Elem->Msg, Elem->MsgLen, ApAddr, &CapabilityInfo, &Timeout, &ListenIntv)) {
		/* RTMPCancelTimer(&apcli_entry->MlmeAux.ApCliAssocTimer, &Cancelled); */
#ifdef MAC_REPEATER_SUPPORT
		if (CliIdx != 0xFF)
			RTMPCancelTimer(&apcli_entry->RepeaterCli[CliIdx].ApCliAssocTimer,
					&Cancelled);
		else
#endif /* MAC_REPEATER_SUPPORT */
			RTMPCancelTimer(&apcli_entry->MlmeAux.ApCliAssocTimer, &Cancelled);
		...
		ApCliMgtMacHeaderInit(pAd, &AssocHdr, SUBTYPE_ASSOC_REQ, 0, ApAddr, ApAddr,
				      ifIndex);

#ifdef MAC_REPEATER_SUPPORT
		if (CliIdx != 0xFF)
			COPY_MAC_ADDR(AssocHdr.Addr2,
				      apcli_entry->RepeaterCli[CliIdx].CurrentAddress);
#endif /* MAC_REPEATER_SUPPORT */
		...
		if (apcli_entry->MlmeAux.ExtRateLen != 0) {
			MakeOutgoingFrame(pOutBuffer + FrameLen, &tmp,
					  ...
					  apcli_entry->MlmeAux.ExtRateLen,
					  apcli_entry->MlmeAux.ExtRate, END_OF_ARGS);
			FrameLen += tmp;
		}
		...
		if ((apcli_entry->MlmeAux.HtCapabilityLen > 0) &&
			...
			NdisMoveMemory(&HtCapabilityTmp, &apcli_entry->MlmeAux.HtCapability,
				       apcli_entry->MlmeAux.HtCapabilityLen);
#ifdef DOT11N_SS3_SUPPORT
			HtCapabilityTmp.MCSSet[2] =
			    (apcli_entry->MlmeAux.HtCapability.MCSSet[2] & apcli_entry->
			     RxMcsSet[2]);
			...
			*(USHORT *) (&HtCapabilityTmp.ExtHtCapInfo) =
			...
			if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode) &&
			    (pAd->CommonCfg.Channel > 14) && (apcli_entry->MlmeAux.vht_cap_len)) {
				FrameLen +=
		...
		if (pAd->CommonCfg.bAggregationCapable) {
			...
			if ((pAd->CommonCfg.bPiggyBackCapable)
			    && ((apcli_entry->MlmeAux.APRalinkIe & 0x00000003) == 3)) {
				ULONG TmpLen;
				UCHAR RalinkIe[9] = {
					IE_VENDOR_SPECIFIC, 7, 0x00, 0x0c, 0x43, 0x03, 0x00, 0x00, 0x00};
				MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen, 9, RalinkIe,
						  END_OF_ARGS);
				FrameLen += TmpLen;
			} else
			...
			if (apcli_entry->MlmeAux.APRalinkIe & 0x00000001) {
				ULONG TmpLen;
				UCHAR RalinkIe[9] = {
					IE_VENDOR_SPECIFIC, 7, 0x00, 0x0c, 0x43, 0x01, 0x00, 0x00, 0x00};
				MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen, 9, RalinkIe,
						  END_OF_ARGS);
				FrameLen += TmpLen;
			}
			...
			ULONG TmpLen;
			UCHAR RalinkIe[9] = {
					IE_VENDOR_SPECIFIC, 7, 0x00, 0x0c, 0x43, 0x06, 0x00, 0x00, 0x00};
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen, 9, RalinkIe, END_OF_ARGS);
			FrameLen += TmpLen;
		}
		...
		if (apcli_entry->MlmeAux.APEdcaParm.bValid) {
			...
			MakeOutgoingFrame(pOutBuffer + FrameLen, &tmp, 9, &WmeIe[0], END_OF_ARGS);
			FrameLen += tmp;
		}
		...
		if ((apcli_entry->wpa_supplicant_info.WpaSupplicantUP & 0x7F) ==
		    WPA_SUPPLICANT_ENABLE) {
			ULONG TmpWpaAssocIeLen = 0;

			DBGPRINT(RT_DEBUG_TRACE,
				 ("%s:: APCLI WPA_ASSOC_IE FROM SUPPLICANT\n", __func__));
			...
			if (wdev->AuthMode == Ndis802_11AuthModeWPA2) {
				...
				for (idx = 0; idx < PMKID_NO; idx++) {
					if (NdisEqualMemory
					    (ApAddr, &apcli_entry->SavedPMK[idx].BSSID, 6)) {
						FoundPMK = TRUE;
						break;
					}
				}
				...
				if ((wdev->AuthMode == Ndis802_11AuthModeWPA2) &&
				    (NdisEqualMemory
				     (pAd->MlmeAux.Bssid, pAd->CommonCfg.LastBssid,
				      MAC_ADDR_LEN))) {
					FoundPMK = FALSE;
				}
				...
				if (FoundPMK) {
					/* Set PMK number */
					*(PUSHORT) & apcli_entry->RSN_IE[apcli_entry->RSNIE_Len] =
					    1;
					NdisMoveMemory(&apcli_entry->RSN_IE
						       [apcli_entry->RSNIE_Len + 2],
						       &apcli_entry->SavedPMK[idx].PMKID, 16);
					apcli_entry->RSNIE_Len += 18;
				}
			}
#ifdef SIOCSIWGENIE
			if ((apcli_entry->wpa_supplicant_info.
			     WpaSupplicantUP & WPA_SUPPLICANT_ENABLE)
...
/*			if( (pWscBuf = kmalloc(512, GFP_ATOMIC)) != NULL) */
			if (pWscBuf != NULL) {
				...
				WscBuildAssocReqIE(&pAd->ApCfg.ApCliTab[ifIndex].WscControl,
						   pWscBuf, &WscIeLen);
...
static VOID ApCliMlmeDisassocReqAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	ApCliMgtMacHeaderInit(pAd, &DisassocHdr, SUBTYPE_DISASSOC, 0, pDisassocReq->Addr,
			      pDisassocReq->Addr, ifIndex);

#ifdef MAC_REPEATER_SUPPORT
	if (CliIdx != 0xFF)
		COPY_MAC_ADDR(DisassocHdr.Addr2,
...
static VOID ApCliPeerAssocRspAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (ApCliPeerAssocRspSanity
	    (pAd, Elem->Msg, Elem->MsgLen, Addr2, &CapabilityInfo, &Status, &Aid, SupRate,
	     &SupRateLen, ExtRate, &ExtRateLen, &HtCapability, &AddHtInfo, &HtCapabilityLen,
	     &AddHtInfoLen, &NewExtChannelOffset, &EdcaParm, &CkipFlag, ie_list)) {
		/* The frame is for me ? */
		if (MAC_ADDR_EQUAL(Addr2, pAd->ApCfg.ApCliTab[ifIndex].MlmeAux.Bssid)) {
			PFRAME_802_11 pFrame = (PFRAME_802_11) (Elem->Msg);

			DBGPRINT(RT_DEBUG_TRACE,
				 ("APCLI_ASSOC - receive ASSOC_RSP to me (status=%d)\n", Status));
#ifdef RT_CFG80211_P2P_CONCURRENT_DEVICE
			...
			NdisCopyMemory(pAd->ApCfg.ApCliTab[ifIndex].ResVarIEs, &pFrame->Octet[6],
				       pAd->ApCfg.ApCliTab[ifIndex].ResVarIELen);
#endif /* RT_CFG80211_P2P_CONCURRENT_DEVICE */
...
#endif /* MAC_REPEATER_SUPPORT */
					{
						ApCliAssocPostProc(pAd, Addr2, CapabilityInfo, ifIndex,
								   SupRate, SupRateLen, ExtRate, ExtRateLen,
								   &EdcaParm, &HtCapability,
						...
						if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode)
							...
							DBGPRINT(RT_DEBUG_TRACE,
								 ("vht ie in AssocRsp ifIdx=%d vht_cap_len=%d\n",
...
VOID ApcliSendAssocIEsToWpaSupplicant(IN RTMP_ADAPTER *pAd, IN UINT ifIndex)
{
	...
	if ((pAd->ApCfg.ApCliTab[ifIndex].ReqVarIELen + 17) <= IW_CUSTOM_MAX) {
		...
		NdisMoveMemory(custom + 17, pAd->ApCfg.ApCliTab[ifIndex].ReqVarIEs,
			       pAd->ApCfg.ApCliTab[ifIndex].ReqVarIELen);
		RtmpOSWrielessEventSend(pAd->net_dev, RT_WLAN_EVENT_CUSTOM, RT_REQIE_EVENT_FLAG,
					NULL, (PUCHAR) custom,
					pAd->ApCfg.ApCliTab[ifIndex].ReqVarIELen + 17);

		RtmpOSWrielessEventSend(pAd->net_dev, RT_WLAN_EVENT_CUSTOM, RT_ASSOCINFO_EVENT_FLAG,
					NULL, NULL, 0);
	} else
		DBGPRINT(RT_DEBUG_TRACE,
			 ("pAd->ApCfg.ApCliTab[%d].ReqVarIELen + 17 > MAX_CUSTOM_LEN\n", ifIndex));
}
#endif /*WPA_SUPPLICANT_SUPPORT */
#endif /* APCLI_SUPPORT */
