...
static VOID ApCliPeerProbeRspAtJoinAction(IN PRTMP_ADAPTER pAd, IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (PeerBeaconAndProbeRspSanity(pAd,
		...
		/* user doesn't indicate any bssid or ssid.
		* AP-Clinet will auto pick a AP to join by most strong siganl strength.
		*/
		...
		if (matchFlag) {
			...
			/* if the Bssid doesn't be indicated then you need to decide
			* which AP to connect by most strong Rssi signal strength.
			*/
				     ...
				     IN UCHAR SsidLen, OUT PCHAR Ssid, IN USHORT ifIndex)
{
	...
	if (NState != NDIS_STATUS_SUCCESS) {
		...
		if (MAC_ADDR_EQUAL(pAd->ApCfg.ApCliTab[ifIndex].CfgApCliBssid, ZERO_MAC_ADDR))
			...
			if (SIGMA_ON(pAd)) {
				/* Fix WFD Certification 6.1.21B, broadcom will
				* not reply probe response if we send the probe request by unicast
				*/
				DBGPRINT(RT_DEBUG_TRACE,
