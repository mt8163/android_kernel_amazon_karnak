...
INT ApAllowToSendPacket(IN RTMP_ADAPTER *pAd,
			IN struct wifi_dev *wdev, IN PNDIS_PACKET pPacket, IN UCHAR *pWcid)
{
...
INT APSendPacket(RTMP_ADAPTER *pAd, PNDIS_PACKET pPacket)
{
...
#ifdef CONFIG_MULTI_CHANNEL	/* we use alci0 for p2p client */
	UINT map_to_edca2 = 0;
	...
	if (/*(RTMP_CFG80211_VIF_P2P_CLI_ON(pAd) && INFRA_ON(pAd)) || */ pAd->Mlme.bStartMcc ==
	    TRUE) {
		map_to_edca2 = 1;
		if (pAd->Mlme.bpkt_dbg) {
			DBGPRINT(RT_DEBUG_ERROR,
				 ("@@@@ : %s map_to_edca2 , Wcid=%u, pMacEntry->apidx=%u\n",
				  __func__, wdev->if_dev, Wcid, pMacEntry->apidx));
		}
	}
#endif /* CONFIG_MULTI_CHANNEL */
...
/**************************************
     Michael [Plan A]
     EP4: AC00             (match_ap[0]: BE/BK)
     EP5: AC01/11  => (match_ap[0]: VI/VO)
     EP6: AC02/12
     EP7: AC03/13  => (match_ap[1]: VI/VO)
     EP9: AC10             (match_ap[1]: BE/BK)
  ***************************************/
	if (map_to_edca2 == 1)
		QueIdx = QID_HCCA;
#endif /* CONFIG_MULTI_CHANNEL */
	...
	if (IS_VALID_ENTRY(pMacEntry) || (Wcid == MCAST_WCID)) {
		...
		/* TODO: shiang-6590, how to disinguish the MCAST/BCAST from different BSS
		 *if we don't have RTMP_GET_PACKET_NET_DEVICE_MBSSID??
		 */
		...
		if (wdev->wdev_type == WDEV_TYPE_AP) {
			...
			if (pMbss && (Wcid == MCAST_WCID) &&
			    pMbss->HotSpotCtrl.HotSpotEnable && pMbss->HotSpotCtrl.DGAFDisable) {
		...
		if (((wdev->AuthMode >= Ndis802_11AuthModeWPA)
			...
			} else {	/* Process for unicast frame */
		...
		DBGPRINT(RT_DEBUG_ERROR, ("%s(%s):Drop unknown packet\n",
					  __func__, RtmpOsGetNetDevName(wdev->if_dev)));
		RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
		return NDIS_STATUS_FAILURE;
	}
	 ...
	 */
	if (pMacEntry
	    && (IS_ENTRY_WDS(pMacEntry) || IS_ENTRY_APCLI(pMacEntry) || IS_ENTRY_MESH(pMacEntry))) {
#ifdef WDS_SUPPORT
		ULONG Now32;
		...
		if (pAd->TxSwQueue[QueIdx].Number >= pAd->TxSwQMaxLen) {
			...
			InsertTailQueueAc(pAd, pMacEntry, &pAd->TxSwQueue[QueIdx],
	...
	/* M/BCAST frames are put to PSQ as long as there's any associated STA in power-save mode */
		...
		/*
		   ...
		   This design is similar to "BROADCAST throttling in most manageable
		   Ethernet Switch chip.
		 */
		if (pAd->MacTab.McastPsQueue.Number >= MAX_PACKETS_IN_MCAST_PS_QUEUE) {
			RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("M/BCAST PSQ(=%d) full, drop it!\n",
	...
	/* 3. otherwise, transmit the frame */
		...
		if (((InIgmpGroup == IGMP_IN_GROUP)
		     && pGroupEntry && (IgmpMemberCnt(&pGroupEntry->MemberList) > 0)
		    )
		    || (InIgmpGroup == IGMP_PKT)
		    ) {
			NDIS_STATUS PktCloneResult =
			    IgmpPktClone(pAd, pPacket, InIgmpGroup, pGroupEntry,
		...
		if (RTMP_GET_PACKET_WAI(pTxBlk->pPacket)) {
			...
			if (CipherAlg == CIPHER_SMS4) {
				...
				if (CLIENT_STATUS_TEST_FLAG
				    (pMacEntry, fCLIENT_STATUS_SOFTWARE_ENCRYPT)) {
					TX_BLK_SET_FLAG(pTxBlk, fTX_bSwEncrypt);
					/* TSC increment pre encryption transmittion */
...
static inline VOID APBuildCache802_11Header(IN RTMP_ADAPTER *pAd,
...
#endif /* WDS_SUPPORT */
#ifdef APCLI_SUPPORT
	if (IS_ENTRY_APCLI(pMacEntry)) {	/* The addr3 of Ap-client packet is Destination Mac address. */
		COPY_MAC_ADDR(pHeader80211->Addr3, pTxBlk->pSrcBufHeader);
	} else
#endif /* APCLI_SUPPORT */
...
static inline VOID APBuildCacheWifiInfo(IN RTMP_ADAPTER *pAd,
					IN TX_BLK *pTxBlk, IN UCHAR *pWiInfo)
{
	MAC_TABLE_ENTRY *pMacEntry;
...
#ifdef HDR_TRANS_TX_SUPPORT
static inline VOID APBuildWifiInfo(IN PRTMP_ADAPTER pAd, IN TX_BLK *pTxBlk)
{
	...
	pWI = (WIFI_INFO_STRUCT *) &pTxBlk->HeaderBuf[TXINFO_SIZE + TXWISize];
...
static inline VOID APBuildCommon802_11Header(RTMP_ADAPTER *pAd, TX_BLK *pTxBlk)
{
	...
	wifi_hdr->FC.SubType =
	    ((TX_BLK_TEST_FLAG(pTxBlk, fTX_bWMM)) ? SUBTYPE_QDATA : SUBTYPE_DATA);
...
static inline BOOLEAN BuildHtcField(IN RTMP_ADAPTER *pAd,
				    IN TX_BLK *pTxBlk,
				    IN MAC_TABLE_ENTRY *pMacEntry, IN PUCHAR pHeaderBufPtr)
{
...
static inline PUCHAR AP_Build_AMSDU_Frame_Header(IN RTMP_ADAPTER *pAd, IN TX_BLK *pTxBlk)
{
	...
	if (pTxBlk == NULL) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s: pTxBlk is NULL\n", __func__));
		return NULL;
	}
	...
	if (pTxBlk->pMacEntry && pAd->chipCap.FlgHwTxBfCap) {
		...
		if (pMacEntry->TxSndgType >= SNDG_TYPE_SOUNDING) {
			...
			if (pMacEntry->TxSndgType == SNDG_TYPE_SOUNDING) {
				...
				if (pAd->CommonCfg.ETxBfNoncompress == 0 &&
				    (pMacEntry->HTCapability.TxBFCap.ExpComBF > 0) &&
				    (pMacEntry->HTCapability.TxBFCap.ComSteerBFAntSup >=
				     (pMacEntry->sndgMcs / 8))
				    )
					((PHT_CONTROL) pHeaderBufPtr)->CSISTEERING = 3;
				else
...
static VOID AP_AMPDU_Frame_Tx(RTMP_ADAPTER *pAd, TX_BLK *pTxBlk)
{
	HEADER_802_11 *pHeader_802_11;
	UCHAR *pHeaderBufPtr;
	USHORT freeCnt = 1;
	...
	if ((pMacEntry->isCached)
		...
		NdisMoveMemory((PUCHAR) (&pTxBlk->HeaderBuf[TXINFO_SIZE]),
		...
		*(pHeaderBufPtr + 1) = 0;
		...
		if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bSwEncrypt)) {
			...
			RTMPSoftConstructIVHdr(pTxBlk->CipherAlg,
					       pTxBlk->KeyIdx,
					       pTxBlk->pKey->TxTsc, pHeaderBufPtr, &iv_offset);
			...
			RTMPSoftEncryptionAction(pAd,
						 ...
						 pTxBlk->pSrcBufData,
						 pTxBlk->SrcBufLen,
						 pTxBlk->KeyIdx, pTxBlk->pKey, &ext_offset);
	...
	if (pMacEntry && IS_ENTRY_WDS(pMacEntry)) {
		INC_COUNTER64(pAd->WdsTab.WdsEntry[pMacEntry->wdev_idx].WdsCounter.
			      TransmittedFragmentCount);
		pAd->WdsTab.WdsEntry[pMacEntry->wdev_idx].WdsCounter.TransmittedByteCount +=
		    pTxBlk->SrcBufLen;
	}
#endif /* WDS_SUPPORT */
#endif /* STATS_COUNT_SUPPORT */
	...
	if (pAd->CommonCfg.DebugFlags & DBF_DBQ_TXFRAME)
		dbQueueEnqueueTxFrame((UCHAR *) (&pTxBlk->HeaderBuf[TXINFO_SIZE]),
...
#ifdef HDR_TRANS_TX_SUPPORT
VOID AP_AMPDU_Frame_Tx_Hdr_Trns(IN PRTMP_ADAPTER pAd, IN TX_BLK *pTxBlk)
{
	PUCHAR pWiBufPtr;
...
*/
	...
	if ((pMacEntry->isCached)
		...
		NdisMoveMemory((PUCHAR)
			       ...
			       (PUCHAR) (&pMacEntry->CachedBuf[0]), TXWISize + WIFI_INFO_SIZE);
...
static VOID AP_AMSDU_Frame_Tx(RTMP_ADAPTER *pAd, TX_BLK *pTxBlk)
{
	...
	while (pTxBlk->TxPacketList.Head) {
		...
		if (frameNum == 0) {
			...
			if (pHeaderBufPtr == NULL) {
				DBGPRINT(RT_DEBUG_ERROR, ("%s: pHeaderBufPtr is NULL\n", __func__));
				return;
			}
			...
			if (pAd->force_amsdu) {
				...
				if (pMacEntry) {
					RABAOriIdx =
					    pMacEntry->BAOriWcidArray[pTxBlk->UserPriority];
					if (((pMacEntry->TXBAbitmap & (1 << pTxBlk->UserPriority))
					     != 0)
					    && (pAd->BATable.BAOriEntry[RABAOriIdx].amsdu_cap ==
						TRUE))
						TX_BLK_SET_FLAG(pTxBlk, fTX_AmsduInAmpdu);
				}
			}
#endif /* WFA_VHT_PF */
			RTMPWriteTxWI_Data(pAd, (TXWI_STRUCT *) (&pTxBlk->HeaderBuf[TXINFO_SIZE]),
					   pTxBlk);
			...
			padding =
			    ROUND_UP(AMSDU_SUBHEAD_LEN + subFramePayloadLen,
				     4) - (AMSDU_SUBHEAD_LEN + subFramePayloadLen);
		...
		if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bApCliPacket)) {
			...
			if (pTxBlk->pMacEntry->bReptCli) {
				pReptEntry =
				    &pAd->ApCfg.ApCliTab[pTxBlk->pMacEntry->wdev_idx].
				    RepeaterCli[pTxBlk->pMacEntry->MatchReptCliIdx];
				if (pReptEntry->CliValid)
					NdisMoveMemory(&subFrameHeader[6],
						       pReptEntry->CurrentAddress, 6);
			} else
#endif /* MAC_REPEATER_SUPPORT */
			{
...
#ifdef WAPI_SUPPORT
				if (pTxBlk->pMacEntry->WapiUskRekeyTimerRunning
				    && pAd->CommonCfg.wapi_usk_rekey_method == REKEY_METHOD_PKT)
		...
		if (pTxBlk->pMacEntry && IS_ENTRY_WDS(pTxBlk->pMacEntry)) {
			INC_COUNTER64(pAd->WdsTab.WdsEntry[pTxBlk->pMacEntry->wdev_idx].WdsCounter.
				      TransmittedFragmentCount);
			pAd->WdsTab.WdsEntry[pTxBlk->pMacEntry->wdev_idx].WdsCounter.
			    TransmittedByteCount += pTxBlk->SrcBufLen;
		}
#endif /* WDS_SUPPORT */
#endif /* STATS_COUNT_SUPPORT */
	}
...
static VOID AP_Legacy_Frame_Tx(RTMP_ADAPTER *pAd, TX_BLK *pTxBlk)
{
	HEADER_802_11 *wifi_hdr;
	UCHAR *pHeaderBufPtr;
	USHORT freeCnt = 1;
...
#ifdef TXBF_SUPPORT
		if (pAd->chipCap.FlgHwTxBfCap &&
		    (pTxBlk->pMacEntry) && (pTxBlk->pTransmit->field.MODE >= MODE_HTMIX)) {
			MAC_TABLE_ENTRY *pMacEntry = pTxBlk->pMacEntry;
			BOOLEAN bHTCPlus = FALSE;
	...
	if (pTxBlk->pMacEntry && IS_ENTRY_WDS(pTxBlk->pMacEntry)) {
		INC_COUNTER64(pAd->WdsTab.WdsEntry[pTxBlk->pMacEntry->wdev_idx].WdsCounter.
			      TransmittedFragmentCount);
		pAd->WdsTab.WdsEntry[pTxBlk->pMacEntry->wdev_idx].WdsCounter.TransmittedByteCount +=
		    pTxBlk->SrcBufLen;
	}
#endif /* WDS_SUPPORT */
#endif /* STATS_COUNT_SUPPORT */
...
#ifdef HDR_TRANS_TX_SUPPORT
VOID AP_Legacy_Frame_Tx_Hdr_Trns(IN PRTMP_ADAPTER pAd, IN TX_BLK *pTxBlk)
{
/*	UCHAR			QueIdx = pTxBlk->QueIdx; */
	USHORT FreeNumber = 1;	/* no use */
	BOOLEAN bVLANPkt;
...
*/
	...
	pWI = (PWIFI_INFO_STRUCT) &pTxBlk->HeaderBuf[TXINFO_SIZE + TXWISize];
	pTxBlk->pSrcBufData = pTxBlk->pSrcBufHeader;
	if (bVLANPkt)
		pWI->field.VLAN = TRUE;
...
static VOID AP_Fragment_Frame_Tx(RTMP_ADAPTER *pAd, TX_BLK *pTxBlk)
{
	HEADER_802_11 *pHeader_802_11;
	UCHAR *pHeaderBufPtr;
	USHORT freeCnt = 1;	/* no use */
	...
	if (pTxBlk->CipherAlg == CIPHER_TKIP) {
		RTMPCalculateMICValue(pAd, pTxBlk->pPacket, pTxBlk->pExtraLlcSnapEncap,
				      pTxBlk->pKey, pTxBlk->apidx);

		/*
		   NOTE: DON'T refer the skb->len directly after following copy. Because the length is not adjust
		   to correct length, refer to pTxBlk->SrcBufLen for the packet length in following progress.
		 */
		NdisMoveMemory(pTxBlk->pSrcBufData + pTxBlk->SrcBufLen, &pAd->PrivateInfo.Tx.MIC[0],
			       8);
		pTxBlk->SrcBufLen += 8;
	...
	else if (pTxBlk->CipherAlg == CIPHER_TKIP)/*TKIP: IV[4] + EIV[4] + ICV[4]*/
		EncryptionOverhead = 12;/*, MIC will be added to TotalPacketLength */
	...
	if (pTxBlk->TxRate <= RATE_LAST_OFDM_RATE)
		AckDuration =
		    RTMPCalcDuration(pAd, pAd->CommonCfg.ExpectedACKRate[pTxBlk->TxRate], 14);
	else
	...
	do {
		...
		if (SrcRemainingBytes <= FreeMpduSize) {
			...
			pHeader_802_11->Duration =
			    (3 * pAd->CommonCfg.Dsifs) + (2 * AckDuration) + RTMPCalcDuration(pAd,
											      pTxBlk->
											      TxRate,
											      NextMpduSize
											      +
		...
		if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bSwEncrypt)) {
			...
			NdisMoveMemory(pTxBlk->pSrcBufData, tmp_ptr + buf_offset,
				       pTxBlk->SrcBufLen);
...
static VOID AP_ARalink_Frame_Tx(RTMP_ADAPTER *pAd, TX_BLK *pTxBlk)
{
	UCHAR *pHeaderBufPtr;
	USHORT freeCnt = 1;	/* no use */
	USHORT totalMPDUSize = 0;
	...
	while (pTxBlk->TxPacketList.Head) {
		...
		/* For first frame, we need to create the 802.11 header + padding(optional)
		* + RA-AGG-LEN + SNAP Header
		*/
		if (frameNum == 0) {
...
static VOID AP_NDPA_Frame_Tx(RTMP_ADAPTER *pAd, TX_BLK *pTxBlk)
{
	UCHAR *buf;
...
/*
	...
	========================================================================
*/
INT APCheckRxError(RTMP_ADAPTER *pAd, RXINFO_STRUCT *pRxInfo, RX_BLK *pRxBlk)
{
	if (pRxInfo->Crc || pRxInfo->CipherErr) {
			...
			if (pAd->DiagStruct.inited) {
				...
				if (pRxBlk->rx_rate.field.MODE == MODE_HTMIX ||
					...
					if (pRxBlk->rx_rate.field.MCS < MAX_MCS_SET)
						diag_info->RxCrcErrCnt_HT[pRxBlk->rx_rate.field.
									  MCS]++;
				}
#ifdef DOT11_VHT_AC
		...
		if ((pRxInfo->U2M)
			...
			DBGPRINT(RT_DEBUG_INFO,
				 ("%s(): pRxInfo:Crc=%d, CipherErr=%d, U2M=%d, Wcid=%d\n",
				  __func__, pRxInfo->Crc, pRxInfo->CipherErr, pRxInfo->U2M,
				  pRxBlk->wcid));
			return NDIS_STATUS_FAILURE;
		}
	}
...
BOOLEAN APChkCls2Cls3Err(RTMP_ADAPTER *pAd, UCHAR wcid, HEADER_802_11 *hdr)
{
	...
	if (wcid >= MAX_LEN_OF_MAC_TABLE) {
		...
		DBGPRINT(RT_DEBUG_WARN,
			 ("%s():Rx a frame from %02x:%02x:%02x:%02x:%02x:%02x with seq %d, WCID(%d) > %d\n",
			  __func__, PRINT_MAC(hdr->Addr2), hdr->Sequence, wcid, MAX_LEN_OF_MAC_TABLE));
...
/* ---Add by shiang for debug */
			...
			OS_WAIT(10);/*sometimes we got auth but not handle yet so it has second chance to check*/
			pEntry = MacTableLookup(pAd, hdr->Addr2);
			if (pEntry)
				return FALSE;
		}
...
*/
/*static UCHAR is_on; */
VOID detect_wmm_traffic(IN RTMP_ADAPTER *pAd, IN UCHAR UserPriority, IN UCHAR FlgIsOutput)
{
	if (pAd == NULL)
		return;
	...
	if (UserPriority > 3) {
		...
		if (pAd->OneSecondnonBEpackets > 100 && pAd->MacTab.fAnyStationMIMOPSDynamic) {
			if (!pAd->is_on) {
#ifdef RTMP_MAC_USB
				RTEnqueueInternalCmd(pAd, CMDTHREAD_AP_ADJUST_EXP_ACK_TIME, NULL,
						     0);
#endif /* RTMP_MAC_USB */
				pAd->is_on = 1;
			}
			...
			if (pAd->is_on) {
#ifdef RTMP_MAC_USB
				RTEnqueueInternalCmd(pAd, CMDTHREAD_AP_RECOVER_EXP_ACK_TIME, NULL,
						     0);
#endif /* RTMP_MAC_USB */
				pAd->is_on = 0;
			}
...
/*
	Write non-zero value to AC0 TXOP to boost performace
...
VOID dynamic_tune_be_tx_op(RTMP_ADAPTER *pAd, ULONG nonBEpackets)
{
	...
	if (pAd == NULL) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s: pAd is NULL parameter. in function.\n", __func__));
		return;
	}

	if (pAd->CommonCfg.bEnableTxBurst || pAd->CommonCfg.bRdg || pAd->CommonCfg.bRalinkBurstMode) {

		if ((pAd->WIFItestbed.bGreenField && pAd->MacTab.fAnyStationNonGF == TRUE) ||
		    ((pAd->OneSecondnonBEpackets > nonBEpackets)
...
VOID APRxErrorHandle(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	if (pRxInfo->CipherErr) {
...
#ifdef WPA_SUPPLICANT_SUPPORT
					if (pAd->ApCfg.ApCliTab[pEntry->wdev_idx].
			...
			if (pRxInfo->U2M) {
				...
				if ((pEntry->WepStatus == Ndis802_11TKIPEnable)
					...
					if (pAd->ApCfg.MBSSID[pEntry->apidx].Hostapd == Hostapd_EXT)
						ieee80211_notify_michael_failure(pAd,
										 pRxBlk->pHeader,
										 (UINT32)
										 pRxBlk->key_idx,
										 0);
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Rx u2me Cipher Err(MPDUsize=%d, WCID=%d, CipherErr=%d)\n",
...
static BOOLEAN APCheckVaildDataFrame(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
...
/* +++Add by shiang for debug */
		...
		if (pAd->chipCap.hif_type == HIF_RLT) {
			if (pRxBlk->wcid >= MAX_LEN_OF_MAC_TABLE) {
				...
				DBGPRINT(RT_DEBUG_WARN,
					 ("ErrWcidPkt: seq=%d, ts=0x%02x%02x%02x%02x\n",
					  pHeader->Sequence, pRxBlk->pRxWI->RXWI_N.rssi[0],
				...
				DBGPRINT(RT_DEBUG_WARN,
					 ("NextValidWcidPkt: seq=%d, ts=0x%02x%02x%02x%02x\n",
					  pHeader->Sequence, pRxBlk->pRxWI->RXWI_N.rssi[0],
					  pRxBlk->pRxWI->RXWI_N.rssi[1],
					  pRxBlk->pRxWI->RXWI_N.rssi[2],
					  pRxBlk->pRxWI->RXWI_N.rssi[3]));
...
static BOOLEAN APCheckTkipMICValue(IN PRTMP_ADAPTER pAd,
				   IN MAC_TABLE_ENTRY *pEntry, IN RX_BLK *pRxBlk)
{
	...
	if (RTMPTkipCompareMICValue(pAd,
				    pData,
				    pDA, pSA, pWpaKey->RxMic, UserPriority, DataSize) == FALSE) {
		DBGPRINT_RAW(RT_DEBUG_ERROR, ("Rx MIC Value error 2\n"));

#ifdef APCLI_SUPPORT
#ifdef WPA_SUPPLICANT_SUPPORT
		if (IS_ENTRY_APCLI(pEntry)
		    && pAd->ApCfg.ApCliTab[pEntry->wdev_idx].wpa_supplicant_info.WpaSupplicantUP) {
			WpaSendMicFailureToWpaSupplicant(pAd->net_dev, pHeader->Addr2,
							 (pWpaKey->Type ==
							  PAIRWISEKEY) ? TRUE : FALSE,
...
VOID APRxEAPOLFrameIndicate(IN PRTMP_ADAPTER pAd,
			    ...
			    IN RX_BLK *pRxBlk, IN UCHAR FromWhichBSSID)
{
	...
	if (pEntry && IS_ENTRY_APCLI(pEntry)) {
		...
		eapcode = WpaCheckEapCode(pAd, pRxBlk->pData, pRxBlk->DataSize, LENGTH_802_1_H);
		...
		if (apcli_entry->wpa_supplicant_info.WpaSupplicantUP &&
			...
			if (sup_info->IEEE8021x_required_keys == FALSE) {
				...
				if (sup_info->DesireSharedKey[idx].KeyLen > 0) {
					...
					RTMP_SET_WCID_SEC_INFO(pAd, BssIdx, idx, CipherAlg,
					...
					apcli_entry->SharedKey[idx].KeyLen =
	...
	if ((pEntry) && (pEntry->WpaState < AS_INITPMK) &&
		...
		if ((pAd->ApCfg.MBSSID[pEntry->apidx].WscControl.WscConfMode != WSC_DISABLE) &&
		    (!MAC_ADDR_EQUAL
		     (pAd->ApCfg.MBSSID[pEntry->apidx].WscControl.EntryAddr, ZERO_MAC_ADDR))) {
...
static VOID APRxDataFrameAnnounce(IN RTMP_ADAPTER *pAd,
				  IN MAC_TABLE_ENTRY *pEntry,
				  IN RX_BLK *pRxBlk, IN UCHAR FromWhichBSSID)
{
	...
	if (!RTMPCheckWPAframe(pAd, pEntry, pRxBlk->pData, pRxBlk->DataSize, FromWhichBSSID)) {
		 ...
		 */
		if ((pEntry) && pEntry->PrivacyFilter == Ndis802_11PrivFilter8021xWEP) {
			/*
			...
			if (IS_ENTRY_CLIENT(pEntry) && pRxBlk->pHeader->FC.Wep &&
				...
				DBGPRINT(RT_DEBUG_WARN,
					 ("==> De-Auth this STA(%02x:%02x:%02x:%02x:%02x:%02x)\n",
...
#ifdef IGMP_SNOOP_SUPPORT
		if (pEntry && (IS_ENTRY_CLIENT(pEntry) || IS_ENTRY_WDS(pEntry))
			...
			PUCHAR pData = NdisEqualMemory(SNAP_802_1H, pRxBlk->pData,
						       6) ? (pRxBlk->pData + 6) : pRxBlk->pData;
...
VOID APRxDataFrameAnnounce_Hdr_Trns(IN RTMP_ADAPTER *pAd,
				    IN MAC_TABLE_ENTRY *pEntry,
				    IN RX_BLK *pRxBlk, IN UCHAR FromWhichBSSID)
{
...
{
	RXINFO_STRUCT *pRxInfo = NULL;
	RXWI_STRUCT *pRxWI = NULL;
	...
	FRAME_CONTROL *pFmeCtrl = NULL;
	COUNTER_RALINK *pCounter = NULL;
#ifdef APCLI_SUPPORT
	PAPCLI_STRUCT pApCliEntry = NULL;
#endif /* endif */

	if (pAd == NULL) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s: pAd is NULL at function.\n", __func__));
		return;
	}

	if (pRxBlk == NULL) {
		DBGPRINT(RT_DEBUG_ERROR, ("%s: pRxBlk is NULL at function.\n", __func__));
		return;
	}
	...
	if ((pFmeCtrl->FrDs == 1) && (pFmeCtrl->ToDs == 1)) {
		do {
			...
			/* handle WDS */
			{
				...
				if (pEntry && (pEntry->LockEntryTx == TRUE)) {
					DBGPRINT(RT_DEBUG_TRACE,
						 ("Receive WDS packet, disable TX lock state!\n"));
		...
		if (pEntry && IS_ENTRY_APCLI(pEntry)) {
			...
			if (pRxInfo->Mcast || pRxInfo->Bcast) {
				...
				if (!ApCliHandleRxBroadcastFrame
				    (pAd, pRxBlk, pEntry, FromWhichBSSID))
		...
		if (pHeader->FC.FrDs == 0 && pRxInfo->U2M && FromWhichBSSID < pAd->ApCfg.BssidNum) {
			MULTISSID_STRUCT *pMbss = pEntry->pMbss;
...
/* TODO: shiang@PF#2, is this atheros protection still necessary here???? */
	...
	if (pAd->ApCfg.MBSSID[pEntry->apidx].RssiLowForStaKickOut != 0) {
		pEntry->curLastDataRssiIndex =
		    pEntry->curLastDataRssiIndex % MAX_LAST_DATA_RSSI_LEN;
		pEntry->LastDataRssi[pEntry->curLastDataRssiIndex] =
		    RTMPMaxRssi(pAd, pEntry->RssiSample.LastRssi0, pEntry->RssiSample.LastRssi1,
				pEntry->RssiSample.LastRssi2);
	...
	if (pFmeCtrl->SubType & 0x08) {
		...
		if ((*pRxBlk->pData) & 0x80) {
			...
			pCounter->ReceivedOctesInAMSDUCount.QuadPart +=
			    (pRxBlk->DataSize + hdr_len);
		}
...
#ifdef TXBF_SUPPORT
	if (pAd->chipCap.FlgHwTxBfCap && (pHeader->FC.SubType & 0x08) && pHeader->FC.Order)
...
#ifdef HDR_TRANS_SUPPORT
VOID APHandleRxDataFrame_Hdr_Trns(IN PRTMP_ADAPTER pAd, IN RX_BLK *pRxBlk)
{
	RXINFO_STRUCT *pRxInfo = pRxBlk->pRxInfo;
...
/* ---Add by shiangf for debug */
	...
	/* 1. skip 802.11 HEADER */
#if defined(WDS_SUPPORT) || defined(MESH_SUPPORT)
	if (FALSE
#ifdef WDS_SUPPORT
	    || bWdsPacket
#endif /* WDS_SUPPORT */
...
#endif /* WDS_SUPPORT || MESH_SUPPORT */
	{
		pData += LENGTH_802_11;
	}
...
BOOLEAN APFowardWirelessStaToWirelessSta(IN RTMP_ADAPTER *pAd,
					 IN PNDIS_PACKET pPacket, IN ULONG FromWhichBSSID)
{
	...
	if (pHeader802_3[0] & 0x01) {
		...
		if (pEntry && (pEntry->Sst == SST_ASSOC) && IS_ENTRY_CLIENT(pEntry)) {
			...
			if (FromWhichBSSID == pEntry->apidx) {	/* STAs in same SSID */
				if (pEntry->apidx < HW_BEACON_MAX_NUM) {
					if ((pAd->ApCfg.MBSSID[pEntry->apidx].
					     IsolateInterStaTraffic == 1)) {
						/* release the packet */
						bDirectForward = FALSE;
						bAnnounce = FALSE;
					}
				}
			} else {	/* STAs in different SSID */
				if (pAd->ApCfg.IsolateInterStaTrafficBTNBSSID == 1 ||
				    ((FromWhichBSSID < MAX_MBSSID_NUM(pAd)) &&
				     (FromWhichBSSID < HW_BEACON_MAX_NUM) &&
				     (pAd->ApCfg.MBSSID[pEntry->apidx].wdev.VLAN_VID !=
				      pAd->ApCfg.MBSSID[FromWhichBSSID].wdev.VLAN_VID)))
			...
			return bAnnounce;
		{
			...
			if (!pEntry) {
				...
				if (/*(FromWhichBSSID >= 0) && */
					   ...
					   (FromWhichBSSID < MAX_MBSSID_NUM(pAd)) &&
					   (FromWhichBSSID < HW_BEACON_MAX_NUM)) {
					RTMP_SET_PACKET_WDEV(pForwardPacket,
							     pAd->ApCfg.MBSSID[FromWhichBSSID].wdev.
							     wdev_idx);
				}
			}
...
========================================================================
*/
NDIS_STATUS APInsertPsQueue(IN PRTMP_ADAPTER pAd,
...
#ifdef APCLI_SUPPORT
VOID ApCliRTMPSendNullFrame(IN PRTMP_ADAPTER pAd,
			    IN UCHAR TxRate,
			    IN BOOLEAN bQosNull, IN PMAC_TABLE_ENTRY pMacEntry, IN USHORT PwrMgmt)
{
	UCHAR NullFrame[48];
	ULONG Length;
	...
	if (((wdev->AuthMode == Ndis802_11AuthModeWPA) ||
	     ...
	     (wdev->AuthMode == Ndis802_11AuthModeWPA2PSK) || (wdev->IEEE8021X == TRUE)
#ifdef WAPI_SUPPORT
