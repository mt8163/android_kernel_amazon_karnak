			...
			if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS) &&
				...
				if (RTMP_TIME_AFTER
					...
					DBGPRINT(RT_DEBUG_ERROR,
						("MMCHK - APCli BeaconRxTime adjust wrong"));
					DBGPRINT(RT_DEBUG_ERROR,
						("(BeaconRx=0x%lx, Now=0x%lx)\n",
...
VOID ApCliLinkDown(RTMP_ADAPTER *pAd, UCHAR ifIndex)
{
	...
	if ((pApCliEntry->Valid == FALSE)
		...
		return;
	if (pApCliEntry->LastDeauthReason != REASON_DRIVER_DEFAULT)
	...
	for (ifIndex = 0; ifIndex < MAX_APCLI_NUM; ifIndex++) {
		...
		if (APCLI_IF_UP_CHECK(pAd, ifIndex)
			...
			MlmeEnqueue(pAd, APCLI_CTRL_STATE_MACHINE, APCLI_CTRL_JOIN_REQ, 0, NULL,
			 ...
			 && (pApCliEntry->TrialCh != 0)
			 /* && NdisCmpMemory(pApCliEntry->ApCliMlmeAux.Ssid,
			 pApCliEntry->CfgSsid, pApCliEntry->SsidLen) != 0 */
			   ...
			   IN PEID_STRUCT pEid_ptr, IN USHORT eid_len, IN USHORT idx)
{
	...
	while (len > 0) {
		...
		case IE_RSN:
			...
			/* 0. Version must be 1 */
			/*  The pRsnHeader->Version exists in native little-endian order,
			* so we may need swap it for RT_BIG_ENDIAN systems.
			*/
