...
static void ap_assoc_info_debugshow(IN RTMP_ADAPTER *pAd,
				    IN BOOLEAN isReassoc,
				    IN MAC_TABLE_ENTRY *pEntry, IN IE_LISTS * ie_list)
{
#ifdef DBG
	...
	if (ie_list->ht_cap_len && WMODE_CAP_N(pAd->CommonCfg.PhyMode)) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("\n%s - Update AP OperaionMode=%d, ",
			  sAssoc, pAd->CommonCfg.AddHTInfo.AddHtInfo2.OperaionMode));
		DBGPRINT(RT_DEBUG_TRACE,
			 ("fAnyStationIsLegacy=%d, fAnyStation20Only=%d, fAnyStationNonGF=%d\n",
			  pAd->MacTab.fAnyStationIsLegacy, pAd->MacTab.fAnyStation20Only,
			  pAd->MacTab.fAnyStationNonGF));

#ifdef DOT11_VHT_AC
		if ((ie_list->vht_cap_len) &&
		    WMODE_CAP_N(pAd->CommonCfg.PhyMode) && (pAd->CommonCfg.Channel > 14)) {
		...
		if (pEntry->ext_cap.operating_mode_notification) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("\t\t\tChannelWidth(%d), RxNss(%d), RxNssType(%d), ForceOpMode(%d)\n",
				  pEntry->operating_mode.ch_width, pEntry->operating_mode.rx_nss,
				  pEntry->operating_mode.rx_nss_type, pEntry->force_op_mode));
		}
#endif /* DOT11_VHT_AC */
	} else {
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("\tAuthMode=%d, WepStatus=%d, WpaState=%d, GroupKeyWepStatus=%d\n",
		  pEntry->AuthMode, pEntry->WepStatus, pEntry->WpaState, wdev->GroupKeyWepStatus));

	DBGPRINT(RT_DEBUG_TRACE,
		 ("\tWMMCap=%d, RalinkAgg=%d, PiggyBack=%d, RDG=%d, TxAMSDU=%d, IdleTimeout=%d\n",
...
static USHORT update_associated_mac_entry(IN RTMP_ADAPTER *pAd,
					  IN MAC_TABLE_ENTRY *pEntry,
					  IN IE_LISTS * ie_list, IN UCHAR MaxSupportedRate)
{
	struct wifi_dev *wdev;
#ifdef TXBF_SUPPORT
	...
	if ((pEntry->AuthMode == Ndis802_11AuthModeWPAPSK)
		...
		pEntry->WpaState = AS_INITPSK;
		 ...
		 (pEntry->AuthMode == Ndis802_11AuthModeWPA2) || (wdev->IEEE8021X == TRUE)) {
		pEntry->PrivacyFilter = Ndis802_11PrivFilter8021xWEP;
	...
	/* There are some situation to need to encryption by software
	   1. The Client support PMF. It shall only support AES cipher.
	...
	if ((ie_list->ht_cap_len != 0) &&
	    (wdev->DesiredHtPhyInfo.bHtEnable) && WMODE_CAP_N(pAd->CommonCfg.PhyMode)) {
		...
		if (wdev->DesiredTransmitSetting.field.MCS != MCS_AUTO) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("@@@ IF-ra%d DesiredTransmitSetting.field.MCS = %d\n",
				  pEntry->apidx, wdev->DesiredTransmitSetting.field.MCS));
		...
		pEntry->MaxHTPhyMode.field.STBC =
		    (ie_list->HTCapability.HtCapInfo.RxSTBC & (pAd->CommonCfg.DesiredHtPhy.TxSTBC));
		...
		NdisMoveMemory(&pEntry->HTCapability, &ie_list->HTCapability,
		...
		if (ie_list->operating_mode_len == sizeof(OPERATING_MODE) &&
			...
			DBGPRINT(RT_DEBUG_OFF,
				 ("%s(): Peer's OperatingMode=>RxNssType: %d, RxNss: %d, ChBW: %d\n",
				  ...
				  pEntry->operating_mode.rx_nss, pEntry->operating_mode.ch_width));
		} else
	...
	if (wdev->bAutoTxRateSwitch == TRUE) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s(SS=%d): pMbss(FixedTxMode=%d, MCS=%d), pEntry(Mode=%d, MCS=%d)\n",
			  __func__, pAd->CommonCfg.TxStream,
			  wdev->DesiredTransmitSetting.field.FixedTxMode,
			  wdev->DesiredTransmitSetting.field.MCS, pEntry->HTPhyMode.field.MODE,
			  pEntry->HTPhyMode.field.MCS));
	}
...
/*
    ...
    ==========================================================================
 */
static USHORT APBuildAssociation(IN RTMP_ADAPTER *pAd,
				 IN MAC_TABLE_ENTRY *pEntry,
				 IN IE_LISTS * ie_list,
				 IN UCHAR MaxSupportedRateIn500Kbps, OUT USHORT *pAid)
{
	...
	if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode) &&
	    (pAd->CommonCfg.Channel > 14) &&
	    (ie_list->vht_cap_len == 0) && (pAd->CommonCfg.bNonVhtDisallow))
		return MLME_ASSOC_REJ_DATA_RATE;
#endif /* DOT11_VHT_AC */
		...
		*pAid = pEntry->Aid;
		...
		if (pEntry->bWscCapable == FALSE)
#endif /* WSC_AP_SUPPORT */
			{
				...
				StatusCode = APValidateRSNIE(pAd, pEntry, &ie_list->RSN_IE[0],
							ie_list->RSNIE_Len);
		...
		if (*pAid == 0)
		...
		else if ((pEntry->RSNIE_Len == 0) && (wdev->AuthMode >= Ndis802_11AuthModeWPA)
#ifdef HOSTAPD_SUPPORT
			...
			if (((wsc_ctrl->WscConfMode != WSC_DISABLE) && pEntry->bWscCapable
			...
			StatusCode =
			    update_associated_mac_entry(pAd, pEntry, ie_list, MaxSupportedRate);
		}
...
static VOID ap_cmm_peer_assoc_req_action(IN PRTMP_ADAPTER pAd,
					 IN MLME_QUEUE_ELEM * Elem, IN BOOLEAN isReassoc)
{
	...
	if (pEntry->Aid != 0 && (pEntry->WepStatus >= Ndis802_11TKIPEnable
#ifdef DOT1X_SUPPORT
				 || wdev->IEEE8021X
#endif /* DOT1X_SUPPORT */
	    )) {
		...
		if (pEntry->AuthMode == Ndis802_11AuthModeWPA ||
		    pEntry->AuthMode == Ndis802_11AuthModeWPA2 || wdev->IEEE8021X)
...
#ifdef WSC_V2_SUPPORT
	if ((wsc_ctrl->WscV2Info.bEnableWpsV2) && (wsc_ctrl->WscV2Info.bWpsEnable == FALSE))
		;
	else
#endif /* WSC_V2_SUPPORT */
	{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s - MBSS(%d), receive %s request from %02x:%02x:%02x:%02x:%02x:%02x\n", sAssoc,
		  pEntry->apidx, sAssoc, PRINT_MAC(ie_list->Addr2)));
	...
	RTMPSetSupportMCS(pAd,
			  OPMODE_AP,
			  pEntry, ie_list->SupportedRates, ie_list->SupportedRatesLen, NULL, 0,
#ifdef DOT11_VHT_AC
			  ie_list->vht_cap_len, &ie_list->vht_cap,
#endif /* DOT11_VHT_AC */
			  &ie_list->HTCapability, ie_list->ht_cap_len);
...
/* +++Add by shiang for debug */
	if (ie_list->vht_cap_len) {
		if (WMODE_CAP_AC(pAd->CommonCfg.PhyMode)) {
			...
			NdisMoveMemory(&pEntry->ext_cap, &ie_list->ExtCapInfo,
...
SendAssocResponse:
...
#endif /* WSC_V2_SUPPORT */
	{
		if ((wsc_ctrl->WscConfMode != WSC_DISABLE) && (ie_list->CapabilityInfo & 0x0010))
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s: ASSOC_FAIL_REQ Threshold = %d, ASSOC_NO_RSP_REQ Threshold = %d,PktMaxRssi=%d\n",
	...
	if ((pMbss->AssocReqFailRssiThreshold != 0) && (rssi < pMbss->AssocReqFailRssiThreshold)) {
		...
		bAssocSkip = TRUE;
	} else if ((pMbss->AssocReqNoRspRssiThreshold != 0)
		   && (rssi < pMbss->AssocReqNoRspRssiThreshold)) {
	...
	if (bACLReject == TRUE || bAssocSkip || bAssocNoRsp) {
		if (!bAssocNoRsp) {
			...
			MakeOutgoingFrame(pOutBuffer, &FrameLen,
					  ...
					  1, &SupRateIe,
					  1, &SupRateLen,
					  SupRateLen, pAd->CommonCfg.SupRate, END_OF_ARGS);
		...
		RTMPSendWirelessEvent(pAd, IW_MAC_FILTER_LIST_EVENT_FLAG, ie_list->Addr2,
				      pEntry->apidx, 0);
	...
	MgtMacHeaderInit(pAd, &AssocRspHdr, SubType, 0, ie_list->Addr2, wdev->if_addr, wdev->bssid);
	...
	if ((pAd->CommonCfg.ExtRateLen) && (PhyMode != WMODE_B) && (FlgIs11bSta == 0)) {
		...
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
				  ...
				  pAd->CommonCfg.ExtRateLen, pAd->CommonCfg.ExtRate, END_OF_ARGS);
		FrameLen += TmpLen;
	}
	...
	if (wdev->bWmmCapable && CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_WMM_CAPABLE)) {
		...
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen, 26, WmeParmIe, END_OF_ARGS);
		FrameLen += TmpLen;
	}
#ifdef DOT11W_PMF_SUPPORT
	if (StatusCode == MLME_ASSOC_REJ_TEMPORARILY) {
		...
		MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
				  1, &IEType, 1, &IELen, 1, &TIType, 4, &units, END_OF_ARGS);
		FrameLen += TmpLen;
	}
#endif /* DOT11W_PMF_SUPPORT */
		...
		*(USHORT *) (&addHTInfoTmp.AddHtInfo3) =
		...
		if ((ie_list->RalinkIe) == 0 || (pAd->bBroadComHT == TRUE)) {
			...
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
					  ...
					  1, &epigram_ie_len,
					  4, &BROADCOM_HTC[0],
					  ie_list->ht_cap_len, &HtCapabilityRsp, END_OF_ARGS);
#else
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
					  1, &WpaIe,
					  1, &epigram_ie_len,
					  4, &BROADCOM_HTC[0],
					  ie_list->ht_cap_len, &HtCapabilityTmp, END_OF_ARGS);
#endif /* endif */
			...
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
					  ...
					  HtLen1, &pAd->CommonCfg.AddHTInfo, END_OF_ARGS);
#else
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
					  1, &WpaIe,
					  1, &epigram_ie_len,
		...
		if (WMODE_CAP_N(pAd->CommonCfg.PhyMode) &&
			...
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
					  1, &OverlapScanIE,
					  1, &ScanIELen, ScanIELen, &OverlapScanParam, END_OF_ARGS);
	...
	/* 7.3.2.27 Extended Capabilities IE */
	{
		...
		if (bNeedAppendExtIE == TRUE) {
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
					  1, &ExtCapIe,
					  1, &extInfoLen, extInfoLen, &extCapInfo, END_OF_ARGS);
			FrameLen += TmpLen;
		}
	}
	...
	if (pAd->ApCfg.MBSSID[pEntry->apidx].AssocRespExtraIeLen) {
		...
		if (RTMPIsValidIEs(Ies, IesLen)) {
			...
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen, IesLen, Ies, END_OF_ARGS);
			FrameLen += TmpLen;
		} else
...
/*		SendSingalToDaemon(SIGUSR2, pObj->IappPid); */
		...
		if (TRUE) {	/*CFG_TODO */
...
#endif /* RT_CFG80211_P2P_CONCURRENT_DEVICE */
			{
				if (pEntry->WepStatus == Ndis802_11WEPEnabled) {
					...
					CipherAlg =
					    pAd->SharedKey[pEntry->func_tb_idx][KeyIdx].CipherAlg;
		...
		if ((pEntry->AuthMode == Ndis802_11AuthModeWPAPSK) ||
			...
			if (pEntry->bWscCapable || (ie_list->RSNIE_Len == 0)) {
				...
				pEntry->Receive_EapolStart_EapRspId =
				    (WSC_ENTRY_GET_EAPOL_START | WSC_ENTRY_GET_EAP_RSP_ID);
...
#ifdef DOT1X_SUPPORT
		/*else if (isReassoc && */
		else if ((pEntry->AuthMode == Ndis802_11AuthModeWPA2) && (pPmkid != NULL)) {	/* Key cache */
			INT CacheIdx = RTMPSearchPMKIDCache(pAd, pEntry->apidx, pEntry->Addr);

			if ((CacheIdx != -1)
			    &&
			    (RTMPEqualMemory
			     (pPmkid, &pMbss->PMKIDCache.BSSIDInfo[CacheIdx].PMKID, LEN_PMKID))) {
				/* Enqueue a EAPOL-start message with the pEntry for WPAPSK State Machine */
				if ((pEntry->EnqueueEapolStartTimerRunning == EAPOL_START_DISABLE
#ifdef HOSTAPD_SUPPORT
				     && wdev->Hostapd == Hostapd_Disable
			...
			if (pEntry->WepStatus == Ndis802_11WEPEnabled) {
				...
				RTMP_SET_WCID_SEC_INFO(pAd,
						       pEntry->apidx,
						       KeyIdx,
						       CipherAlg, pEntry->wcid, SHAREDKEYTABLE);
			}
		}

	}
#ifdef DELAYED_TCP_ACK
	RTMPInitTimer(pAd, &pEntry->QueueAckTimer, GET_TIMER_FUNCTION(RTMPQueueAckPeriodicExec),
		      pEntry, FALSE);
...
/*
    ...
    ==========================================================================
 */
static VOID APPeerAssocReqAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
...
/*
    ...
    ==========================================================================
 */
static VOID APPeerReassocReqAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
...
/*
    ...
    ==========================================================================
 */
static VOID APPeerDisassocReqAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
...
VOID APMlmeKickOutSta(RTMP_ADAPTER *pAd, UCHAR *pStaAddr, UCHAR Wcid, USHORT Reason)
{
	...
	if (Aid < MAX_LEN_OF_MAC_TABLE) {
		...
		MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(HEADER_802_11), &DisassocHdr, 2,
				  &Reason, END_OF_ARGS);
#ifdef CONFIG_MULTI_CHANNEL
		if ((pAd->Mlme.bStartMcc == TRUE) && RTMP_CFG80211_VIF_P2P_GO_ON(pAd)) {
			/*when MCC STAR, map MGMPACKET to HCCA */
...
VOID APMlmeKickOutAllSta(RTMP_ADAPTER *pAd, UCHAR apidx, USHORT Reason)
{
	...
	if ((apidx < pAd->ApCfg.BssidNum) && (pPmfCfg)) {
		...
		if (NStatus != NDIS_STATUS_SUCCESS)
			return;
		DBGPRINT(RT_DEBUG_ERROR,
			 ("Send DISASSOC Broadcast frame(%d) with ra%d\n", Reason, apidx));
		...
		MakeOutgoingFrame(pOutBuffer, &FrameLen,
				  sizeof(HEADER_802_11), &DisassocHdr, 2, &Reason, END_OF_ARGS);
		...
		if (pPmfCfg->MFPC == TRUE) {
			...
			ELen = LEN_PMF_MMIE;
			MakeOutgoingFrame(pOutBuffer + FrameLen, &TmpLen,
					  1, &EID, 1, &ELen, LEN_PMF_MMIE, res_buf, END_OF_ARGS);
			FrameLen += TmpLen;
...
/*
    ...
    ==========================================================================
 */
static VOID APMlmeDisassocReqAction(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem)
{
...
VOID APCls3errAction(RTMP_ADAPTER *pAd, ULONG Wcid, HEADER_802_11 *pHeader)
{
	...
	MgtMacHeaderInit(pAd, &DisassocHdr, SUBTYPE_DISASSOC, 0, pHeader->Addr2, pHeader->Addr1,
			 pHeader->Addr1);
	MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(HEADER_802_11), &DisassocHdr, 2, &Reason,
			  END_OF_ARGS);
...
/*
    ...
    ==========================================================================
 */
VOID APAssocStateMachineInit(IN RTMP_ADAPTER *pAd,
			     IN STATE_MACHINE * S, OUT STATE_MACHINE_FUNC Trans[])
{
	StateMachineInit(S, (STATE_MACHINE_FUNC *) Trans, AP_MAX_ASSOC_STATE, AP_MAX_ASSOC_MSG,
			 (STATE_MACHINE_FUNC) Drop, AP_ASSOC_IDLE, AP_ASSOC_MACHINE_BASE);

	StateMachineSetAction(S, AP_ASSOC_IDLE, APMT2_MLME_DISASSOC_REQ,
			      (STATE_MACHINE_FUNC) APMlmeDisassocReqAction);
	StateMachineSetAction(S, AP_ASSOC_IDLE, APMT2_PEER_DISASSOC_REQ,
			      (STATE_MACHINE_FUNC) APPeerDisassocReqAction);
	StateMachineSetAction(S, AP_ASSOC_IDLE, APMT2_PEER_ASSOC_REQ,
			      (STATE_MACHINE_FUNC) APPeerAssocReqAction);
	StateMachineSetAction(S, AP_ASSOC_IDLE, APMT2_PEER_REASSOC_REQ,
			      (STATE_MACHINE_FUNC) APPeerReassocReqAction);
