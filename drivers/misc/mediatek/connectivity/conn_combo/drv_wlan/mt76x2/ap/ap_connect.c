			...
			for (idx = 0; idx < MAX_WDS_ENTRY; idx++) {
				if ((pAd->WdsTab.WdsEntry[idx].wdev.if_dev != NULL)
				    ...
				    (RTMP_OS_NETDEV_STATE_RUNNING
				     (pAd->WdsTab.WdsEntry[idx].wdev.if_dev))) {
					result = TRUE;
					break;
				}
			}
...
VOID APMakeBssBeacon(RTMP_ADAPTER *pAd, INT apidx)
{
	...
	HTTRANSMIT_SETTING BeaconTransmit = {.word = 0 };
	...
	/* AP Channel Report */
	{
		...
		if (PhyMode == (WMODE_B | WMODE_G | WMODE_GN)) {
			MakeOutgoingFrame(pBeaconFrame + FrameLen, &TmpLen,
					  ...
					  rclasslen, rclass32,
					  1, &APChannelReportIe,
					  1, &rclasslen, rclasslen, rclass33, END_OF_ARGS);
			FrameLen += TmpLen;
		}
	}
	...
	RTMPWriteTxWI(pAd, &pAd->BeaconTxWI, FALSE, FALSE, TRUE, FALSE, FALSE, TRUE, 0,
		      BSS0Mcast_WCID, FrameLen, PID_MGMT, 0, 0, IFS_HTTXOP, &BeaconTransmit);
	...
	*(ptr + 4) = TimFirst;
	...
	if ((wdev->AuthMode == Ndis802_11AuthModeWPA1WPA2) ||
		...
		MakeOutgoingFrame(pBeaconFrame + FrameLen, &TmpLen,
				  ...
				  1, &RSNIe2,
				  1, &pMbss->RSNIE_Len[1],
				  pMbss->RSNIE_Len[1], pMbss->RSN_IE[1], END_OF_ARGS);
		FrameLen += TmpLen;
		...
		MakeOutgoingFrame(pBeaconFrame + FrameLen, &TmpLen,
				  1, &RSNIe,
				  1, &pMbss->RSNIE_Len[0],
				  pMbss->RSNIE_Len[0], pMbss->RSN_IE[0], END_OF_ARGS);
		FrameLen += TmpLen;
	}
	...
	if ((pMbss->WscControl.WscConfMode != WSC_DISABLE) &&
		...
		MakeOutgoingFrame(pBeaconFrame + FrameLen, &TempLen,
				  7, PROVISION_SERVICE_IE, END_OF_ARGS);
		FrameLen += TempLen;
	}
#endif /* WSC_AP_SUPPORT */
	/* Update ERP */
	...
	if ((pComCfg->Channel > 14)
		...
		*(ptr + 4) = (pAd->Dot11_H.CSPeriod - pAd->Dot11_H.CSCount - 1);
		...
		if (pComCfg->bExtChannelSwitchAnnouncement) {
			...
			NdisMoveMemory(ptr, &HtExtChannelSwitchIe,
				       sizeof(HT_EXT_CHANNEL_SWITCH_ANNOUNCEMENT_IE));
				...
				*(ptr + 1) = sizeof(WIDE_BW_CH_SWITCH_ELEMENT);
				...
				if (pComCfg->vht_bw == VHT_BW_80) {
					wb_info.center_freq_1 =
					    vht_cent_ch_freq(pAd, pComCfg->Channel);
...
/*		USHORT	b2lTmp, b2lTmp2; // no use */
		...
		MakeOutgoingFrame(pBeaconFrame + FrameLen, &TmpLen,
				  ...
				  1, &AddHtInfoIe,
				  1, &HtLen1, HtLen1, &pComCfg->AddHTInfo, END_OF_ARGS);
#else
		...
		*(USHORT *) (&addHTInfoTmp.AddHtInfo3) =
		 ...
		 */
		if ((pComCfg->Channel <= 14) && (pComCfg->HtCapability.HtCapInfo.ChannelWidth == 1)) {
			...
			OverlapScanParam.ScanPassiveDwell =
			    cpu2le16(pComCfg->Dot11OBssScanPassiveDwell);
			OverlapScanParam.ScanActiveDwell =
			    cpu2le16(pComCfg->Dot11OBssScanActiveDwell);
			OverlapScanParam.TriggerScanInt =
			    cpu2le16(pComCfg->Dot11BssWidthTriggerScanInt);
			OverlapScanParam.PassiveTalPerChannel =
			    cpu2le16(pComCfg->Dot11OBssScanPassiveTotalPerChannel);
			OverlapScanParam.ActiveTalPerChannel =
			    cpu2le16(pComCfg->Dot11OBssScanActiveTotalPerChannel);
			OverlapScanParam.DelayFactor =
			    cpu2le16(pComCfg->Dot11BssWidthChanTranDelayFactor);
		...
		if (WMODE_CAP_AC(PhyMode) && (pComCfg->Channel > 14)) {
			int _len =
			    build_vht_ies(pAd, (UCHAR *) (pBeaconFrame + FrameLen), SUBTYPE_BEACON);
			FrameLen += _len;
		}
#endif /* DOT11_VHT_AC */
	}
	...
	/* 7.3.2.27 Extended Capabilities IE */
	{
		...
		if (WMODE_CAP_N(PhyMode) && (pComCfg->Channel <= 14) &&
		    (pMbss->wdev.DesiredHtPhyInfo.bHtEnable) && (pComCfg->bBssCoexEnable == TRUE)
		    ) {
	...
	if (pAd->force_vht_op_mode == TRUE) {
		...
		MakeOutgoingFrame(pBeaconFrame + FrameLen, &TmpLen,
				  1, &operating_ie,
				  1, &operating_len, 1, &operating_mode, END_OF_ARGS);
		FrameLen += TmpLen;
	}
#endif /* WFA_VHT_PF */
	...
	if (pMbss->wdev.bWmmCapable) {
		...
		MakeOutgoingFrame(pBeaconFrame + FrameLen, &TmpLen, 26, WmeParmIe, END_OF_ARGS);
		FrameLen += TmpLen;
	}
	...
	if (((pComCfg->Channel > 14) && pComCfg->bIEEE80211H == TRUE)
		...
		MakeOutgoingFrame(pBeaconFrame + FrameLen, &TmpLen,
				  1, &PwrConstraintIE,
				  1, &PwrConstraintLen, 1, &PwrConstraint, END_OF_ARGS);
		FrameLen += TmpLen;
		...
		if (WMODE_CAP_AC(PhyMode)) {
			...
			MakeOutgoingFrame(pBeaconFrame + FrameLen, &TmpLen,
					  1, &vht_txpwr_env_ie,
					  1, &ie_len, ie_len, &txpwr_env, END_OF_ARGS);
			FrameLen += TmpLen;
		}
#endif /* DOT11_VHT_AC */
	...
	if (WMODE_CAP_N(PhyMode) && (wdev->DesiredHtPhyInfo.bHtEnable)) {
		...
		if (pAd->bBroadComHT == TRUE) {
			...
			MakeOutgoingFrame(pBeaconFrame + FrameLen, &TmpLen,
					  ...
					  1, &epigram_ie_len,
					  4, &BROADCOM_HTC[0],
					  HtLen, &pComCfg->HtCapability, END_OF_ARGS);
#else
			NdisMoveMemory(&HtCapabilityTmp, &pComCfg->HtCapability, HtLen);
			*(USHORT *) (&HtCapabilityTmp.HtCapInfo) =
			    SWAP16(*(USHORT *) (&HtCapabilityTmp.HtCapInfo));
			...
			*(USHORT *) (&HtCapabilityTmp.ExtHtCapInfo) =
			...
			MakeOutgoingFrame(pBeaconFrame + FrameLen, &TmpLen,
					  ...
					  HtLen1, &pComCfg->AddHTInfo, END_OF_ARGS);
#else
			NdisMoveMemory(&addHTInfoTmp, &pComCfg->AddHTInfo, HtLen1);
			*(USHORT *) (&addHTInfoTmp.AddHtInfo2) =
			    SWAP16(*(USHORT *) (&addHTInfoTmp.AddHtInfo2));
	...
	RTMPWriteTxWI(pAd, &pAd->BeaconTxWI, FALSE, FALSE, TRUE, FALSE, FALSE, TRUE, 0,
		      RESERVED_WCID, FrameLen, PID_MGMT, 0 /*QID_MGMT */ , 0, IFS_HTTXOP,
		      &BeaconTransmit);
	...
	if ((pAd->ApCfg.DtimCount == 0) &&
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("\tNow RecomWidth=%d, ExtChanOffset=%d, prevBW=%d, prevExtOffset=%d\n",
			  pAd->CommonCfg.AddHTInfo.AddHtInfo.RecomWidth,
			  pAd->CommonCfg.AddHTInfo.AddHtInfo.ExtChanOffset, prevBW,
			  prevExtChOffset));
