...
NDIS_STATUS APInitialize(RTMP_ADAPTER *pAd)
{
	...
	for (i = 0; i < MAX_MBSSID_NUM(pAd); i++)
		RTMPInitTimer(pAd, &pAd->ApCfg.MBSSID[i].REKEYTimer,
			      GET_TIMER_FUNCTION(GREKEYPeriodicExec), pAd, TRUE);

	RTMPInitTimer(pAd, &pAd->ApCfg.CounterMeasureTimer, GET_TIMER_FUNCTION(CMTimerExec), pAd,
		      FALSE);
#ifdef RTMP_MAC_USB
	...
	RTMPInitTimer(pAd, &pAd->ApCfg.IDSTimer, GET_TIMER_FUNCTION(RTMPIdsPeriodicExec), pAd,
		      FALSE);
...
/*			pMbss->CapabilityInfo |= 0x0200; */
		...
		if ((wdev->AuthMode == Ndis802_11AuthModeWPA2 ||
		    ...
		    (wdev->WepStatus == Ndis802_11AESEnable) && (pMbss->PmfCfg.Desired_MFPC)) {
		...
		DBGPRINT(RT_DEBUG_ERROR, ("[PMF]%s:: apidx=%d, MFPC=%d, MFPR=%d, SHA256=%d\n",
					  __func__, idx, pMbss->PmfCfg.MFPC,
		...
		if (pMbss->WscControl.WscV2Info.bEnableWpsV2) {
			...
			if ((wdev->WepStatus == Ndis802_11WEPEnabled) ||
			    (wdev->WepStatus == Ndis802_11TKIPEnable) || (pMbss->bHideSsid))
...
/* +++Add by shiang for debug */
	DBGPRINT(RT_DEBUG_OFF,
		 ("%s(): AP Set CentralFreq at %d(Prim=%d, HT-CentCh=%d, VHT-CentCh=%d, BBP_BW=%d)\n",
		  __func__, pAd->hw_cfg.cent_ch, pAd->CommonCfg.Channel,
...
/* ---Add by shiang for debug */
	...
	for (idx = 0; idx < pAd->ApCfg.BssidNum; idx++) {
		...
		if (wdev->WepStatus == Ndis802_11WEPEnabled) {
				...
				if (pAd->SharedKey[idx][key_idx].KeyLen > 0) {
					...
					if (key_idx == wdev->DefaultKeyId) {
						...
						for (i = 0; i < LEN_WEP_TSC; i++)
							pAd->SharedKey[idx][key_idx].TxTsc[i] =
							    RandomByte(pAd);
			...
			WAPIInstallSharedKey(pAd,
					     wdev->GroupKeyWepStatus,
					     idx, wdev->DefaultKeyId, Wcid, pMbss->GTK);
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("### BSS(%d) AuthMode(%d)=%s, WepStatus(%d)=%s, AccessControlList.Policy=%ld\n",
			  idx, wdev->AuthMode, GetAuthMode(wdev->AuthMode), wdev->WepStatus,
			  GetEncryptType(wdev->WepStatus), pMbss->AccessControlList.Policy));
	}
	...
	/* Add wds key information to ASIC */
...
/*
	...
	==========================================================================
 */
VOID APStop(IN PRTMP_ADAPTER pAd)
{
	BOOLEAN Cancelled;
	INT idx;
...
/*
	...
	==========================================================================
 */
VOID APCleanupPsQueue(IN PRTMP_ADAPTER pAd, IN PQUEUE_HEADER pQueue)
{
	PQUEUE_ENTRY pEntry;
...
{
	int i, startWcid;
	/*Default set minimum AMPDU Size to 2, i.e. 32K */
	ULONG MinimumAMPDUSize = pAd->CommonCfg.DesiredHtPhy.MaxRAmpduFactor;
	BOOLEAN bRdgActive;
	...
	for (i = startWcid; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if (IS_ENTRY_APCLI(pEntry) && (pEntry->PortSecured == WPA_802_1X_PORT_SECURED)) {
			...
			if ((pAd->Mlme.OneSecPeriodicRound % 10) == 8) {
				...
				ApCliRTMPSendNullFrame(pAd, pEntry->CurrTxRate, ApclibQosNull,
						       pEntry, PWR_ACTIVE);
		...
		if ((pEntry->Sst != SST_ASSOC)
			...
			if (NdisEqualMemory
			    (pEntry->Addr, pMbss->WscControl.EntryAddr, MAC_ADDR_LEN))
		...
		if (pEntry->bIAmBadAtheros) {
			...
			if (pAd->CommonCfg.IOTestParm.bRTSLongProtOn == FALSE)
				AsicUpdateProtect(pAd, 8, ALLN_SETPROTECT, FALSE,
		...
		if ((pMbss->StationKeepAliveTime > 0) &&
			...
			if (pEntry->StationKeepAliveCount++ == 0) {
					if (pEntry->PsMode == PWR_SAVE) {
						...
						WLAN_MR_TIM_BIT_SET(pAd, pEntry->apidx,
								    pEntry->Aid);
					} else {
						/* use Null or QoS Null to detect the ACTIVE station */
						...
						if (CLIENT_STATUS_TEST_FLAG
						    (pEntry, fCLIENT_STATUS_WMM_CAPABLE))
							bQosNull = TRUE;

						RtmpEnqueueNullFrame(pAd, pEntry->Addr,
								     pEntry->CurrTxRate,
								     pEntry->Aid, pEntry->apidx,
								     bQosNull, TRUE, 0);
					}
			} else {
...
#define CHECK_DATA_RSSI_UP_BOUND 3
			...
			if (overRssiThresCount >= CHECK_DATA_RSSI_UP_BOUND) {
				bDisconnectSta = TRUE;
				DBGPRINT(RT_DEBUG_WARN,
					 ("Dis STA %02x:%02x:%02x:%02x:%02x:%02x , RSSI Kickout Thres[%d]-[%d]Times\n",
		...
		if (bDisconnectSta) {
			...
			if (pEntry->Sst == SST_ASSOC) {
				...
				if ((pAd->ApCfg.bMACRepeaterEn == TRUE) && IS_ENTRY_CLIENT(pEntry)) {
					...
					pReptEntry =
					    RTMPLookupRepeaterCliEntry(pAd, TRUE, pEntry->Addr);
					if (pReptEntry && (pReptEntry->CliConnectState != 0)) {
						...
						RTMPRemoveRepeaterDisconnectEntry(pAd, apCliIdx,
										  CliIdx);
		...
		if ((pEntry->HTPhyMode.field.MCS != 32) && ((pEntry->HTPhyMode.field.MCS % 8 == 0)
...
#ifdef WFA_VHT_PF
	if (worst_rssi != NULL &&
	    ((pAd->Mlme.OneSecPeriodicRound % 10) == 5) && (worst_rssi_sta_idx >= 1)) {
		CHAR gain = 2;
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("%s():WorstRSSI for STA(%02x:%02x:%02x:%02x:%02x:%02x):%d,%d,%d, Set Gain as %s\n",
			  __func__, PRINT_MAC(pMacTable->Content[worst_rssi_sta_idx].Addr),
		...
		if (triggerBitMap != 0) {
			...
			if (pAd->ApCfg.fAllStatIsHighTraffic == TRUE) {
				...
				if ((triggerBitMap & 0x3) != 0) {
					...
					DBGPRINT(RT_DEBUG_WARN,
						 ("%s::condition (0.1) under HT/VHT mode\n",
						  __func__));
				}
				...
				DBGPRINT(RT_DEBUG_WARN,
					 ("%s::condition (1) under HT/VHT mode\n", __func__));
			} else {
				MAC_TABLE_ENTRY *pMacEntry = NULL;
				UINT32 i = 0;
				UINT32 RegValue;
				UCHAR txop_value = 0;
				BOOLEAN bEnableSpecificTxOp = FALSE;

#ifdef MULTI_CLIENT_SUPPORT
				if ((triggerBitMap & 0x3) != 0) {
					asic_change_tx_retry(pAd, 1);
					DBGPRINT(RT_DEBUG_WARN,
						 ("%s::condition (0.2) under HT/VHT mode\n",
						  __func__));
				}
#endif /* MULTI_CLIENT_SUPPORT */
			...
			for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
				...
				if (pMacEntry && IS_ENTRY_CLIENT(pMacEntry) &&
				    ...
				    (pMacEntry->ContinuelowTraffic == FALSE)) {
					if ((((pMacEntry->HTPhyMode.field.MODE ==
					       MODE_HTMIX)
						 ...
						 || ((pAd->CommonCfg.TxStream == 1)
						    ...
						    ((pMacEntry->HTPhyMode.field.MODE == MODE_VHT)
						     && (((pAd->CommonCfg.TxStream == 2)
							  && (pMacEntry->HTPhyMode.field.MCS >= 23))
							 || ((pAd->CommonCfg.TxStream == 1)
							     && (pMacEntry->HTPhyMode.field.MCS >=
								 7))))
#endif /* DOT11_VHT_AC */
						    ) {
							bEnableSpecificTxOp = TRUE;
						}
					}
				}
				...
				if (RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_RDG_ACTIVE))
					txop_value = 0x80;
				else if (bEnableSpecificTxOp == TRUE) {
					txop_value = 0x60;
				...
				RTMP_IO_WRITE32(pAd, EDCA_AC0_CFG, RegValue);
				DBGPRINT(RT_DEBUG_WARN,
					 ("%s::condition (2) under HT/VHT mode\n", __func__));
			}
		}
	}

	if (pAd->ApCfg.fDisableTrafficCnt == TRUE) {
#ifdef MULTI_CLIENT_SUPPORT
		asic_change_tx_retry(pAd, 1);
#endif /* MULTI_CLIENT_SUPPORT */
	}
...
UINT32 MacTableAssocStaNumGet(IN PRTMP_ADAPTER pAd)
{
	UINT32 num = 0;
	UINT32 i;
...
/*
	...
	==========================================================================
*/
MAC_TABLE_ENTRY *APSsPsInquiry(IN RTMP_ADAPTER *pAd,
			       IN UCHAR *pAddr,
			       OUT SST * Sst,
			       OUT USHORT *Aid, OUT UCHAR *PsMode, OUT UCHAR *Rate)
{
...
/*
	...
	==========================================================================
 */
VOID ApLogEvent(IN PRTMP_ADAPTER pAd, IN PUCHAR pAddr, IN USHORT Event)
{
...
/*
	...
	==========================================================================
*/
VOID APUpdateOperationMode(IN PRTMP_ADAPTER pAd)
{
	...
	if ((pAd->CommonCfg.AddHTInfo.AddHtInfo2.OperaionMode == 0) &&
	    (pAd->MacTab.fAnyStation20Only) && (pAd->CommonCfg.DesiredHtPhy.ChannelWidth == 1)) {
	...
	AsicUpdateProtect(pAd,
			  ...
			  (ALLN_SETPROTECT), bDisableBGProtect, bNonGFExist);
...
/*
	...
	==========================================================================
 */
VOID APUpdateCapabilityAndErpIe(IN PRTMP_ADAPTER pAd)
{
	...
	if ((pAd->CommonCfg.TxPreamble == Rt802_11PreambleLong)
	    || (ApCheckLongPreambleSTA(pAd) == TRUE))
...
/*
    ...
    ==========================================================================
 */
BOOLEAN ApCheckLongPreambleSTA(IN PRTMP_ADAPTER pAd)
{
	UCHAR i;
...
/*
	...
	==========================================================================
*/
BOOLEAN ApCheckAccessControlList(IN PRTMP_ADAPTER pAd, IN PUCHAR pAddr, IN UCHAR Apidx)
{
	BOOLEAN Result = TRUE;
	...
	if (pAd->ApCfg.MBSSID[Apidx].AccessControlList.Policy == 0)	/* ACL is disabled */
		...
		for (i = 0; i < pAd->ApCfg.MBSSID[Apidx].AccessControlList.Num; i++) {
			if (MAC_ADDR_EQUAL
			    (pAddr, pAd->ApCfg.MBSSID[Apidx].AccessControlList.Entry[i].Addr)) {
				Result = !Result;
				break;
			}
		}
...
VOID ApUpdateAccessControlList(RTMP_ADAPTER *pAd, UCHAR Apidx)
{
	...
	for (MacIdx = 0; MacIdx < MAX_LEN_OF_MAC_TABLE; MacIdx++) {
		...
		/* Do nothing for infra connection */
		if (INFRA_ON(pAd) && OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED) &&
		    MAC_ADDR_EQUAL(&pEntry->Addr[0], pAd->CommonCfg.Bssid))
			continue;
		...
		for (AclIdx = 0; AclIdx < pMbss->AccessControlList.Num; AclIdx++) {
			if (MAC_ADDR_EQUAL
			    (&pEntry->Addr[0], pMbss->AccessControlList.Entry[AclIdx].Addr)) {
				Matched = TRUE;
				break;
			}
		}
		...
		if (drop == TRUE) {
			...
			if (pEntry->Sst == SST_ASSOC) {
				...
				MgtMacHeaderInit(pAd, &DisassocHdr, SUBTYPE_DISASSOC, 0,
						 pEntry->Addr, pMbss->wdev.if_addr,
						 pMbss->wdev.bssid);
				MakeOutgoingFrame(pOutBuffer, &FrameLen, sizeof(HEADER_802_11),
						  &DisassocHdr, 2, &Reason, END_OF_ARGS);
				MiniportMMRequest(pAd, 0, pOutBuffer, FrameLen);
				MlmeFreeMemory(pAd, pOutBuffer);
	...
	For 2.4GHz,
		...
		where
			...
			OTi  is member i of the set of channels that comparises all channels that are members of the
				channel set C that were listed once in the Channel List fields of 20/40 BSS Intolerant
				Channel
				Report elements receved during the previous X seconds and all channels that are members
				of the channel set C and that are the primary operating channel of at least one
				20/40 MHz
				BSS that were detected within the AP's BSA during the previous X seconds.
			C	is the set of all channels that are allowed operating channels within the current
				operational regulatory domain and whose center frequency falls within the
				40 MHz affected channel
			...
			"==" means that the values on either side of the "==" are to be tested
				 for equaliy with a resulting boolean value.
				=>When the value of OPi is the empty set, then the expression
					(P == OPi for all values of i) is defined to be TRUE
				=>When the value of OTi is the empty set, then the expression
					(P == OTi for all values of i) is defined to be TRUE
				=>When the value of OSi is the empty set, then the expression
					(S == OSi for all values of i)	is defined to be TRUE
*/

INT GetBssCoexEffectedChRange(IN RTMP_ADAPTER *pAd, IN BSS_COEX_CH_RANGE * pCoexChRange)
{
	INT index, cntrCh = 0;
...
#ifdef A_BAND_SUPPORT
	if (pAd->CommonCfg.Channel > 14) {	/* For 5GHz band */
		...
		if (index < pAd->ChannelListNum) {
			...
			DBGPRINT(RT_DEBUG_TRACE,
				("5.0GHz: Found CtrlCh idx(%d) from the ChList ExtCh=%s, PriCh=[Idx:%d, CH:%d]",
				index,
				(pAd->CommonCfg.RegTransmitSetting.field.EXTCHA ==
				EXTCHA_ABOVE) ? "ABOVE" : "BELOW",
				pCoexChRange->primaryCh,
				pAd->ChannelList[pCoexChRange->primaryCh].Channel));
			DBGPRINT(RT_DEBUG_TRACE,
				(", SecCh=[Idx:%d, CH:%d], effected Ch=[CH:%d~CH:%d]!\n",
				pCoexChRange->secondaryCh,
				pAd->ChannelList[pCoexChRange->secondaryCh].Channel,
			...
			DBGPRINT(RT_DEBUG_ERROR,
				 ("5GHz: Cannot found the CtrlCh(%d) in ChList, something wrong?\n",
	...
	{			/* For 2.4GHz band */
		...
		if (index < pAd->ChannelListNum) {
			...
			if (cntrCh) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("2.4GHz: Found CtrlCh idx(%d) from the ChList, ExtCh=%s,  PrimaryCh=[Idx:%d,",
					 index,
					 (pAd->CommonCfg.RegTransmitSetting.field.EXTCHA ==
					    EXTCHA_ABOVE) ? "ABOVE" : "BELOW",
					    pCoexChRange->primaryCh));
				DBGPRINT(RT_DEBUG_TRACE,
					 (" CH:%d], SecondaryCh=[Idx:%d, CH:%d], effected Ch=[CH:%d~CH:%d]!\n",
					 pAd->ChannelList[pCoexChRange->primaryCh].Channel,
					  pCoexChRange->secondaryCh,
					  pAd->ChannelList[pCoexChRange->secondaryCh].Channel,
					  pAd->ChannelList[pCoexChRange->effectChStart].Channel,
...
VOID APOverlappingBSSScan(RTMP_ADAPTER *pAd)
{
	...
	if ((!WMODE_CAP_N(pAd->CommonCfg.PhyMode)) ||
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("The pAd->PhyMode=%d, BW=%d, didn't need channel adjustment!\n",
	...
	if (pAd->CommonCfg.Channel > 14) {	/* For 5GHz band */
		...
		if (index < pAd->ChannelListNum) {
			...
			/* It should not happened! */
			DBGPRINT(RT_DEBUG_ERROR,
				 ("5GHz: Cannot found the ControlChannel(%d) in ChannelList, something wrong?\n",
				  pAd->CommonCfg.Channel));
			return;
		}
	} else
	...
	{			/* For 2.4GHz band */
		...
		if (index < pAd->ChannelListNum) {
			...
			if (pAd->CommonCfg.RegTransmitSetting.field.EXTCHA == EXTCHA_ABOVE) {
				...
				curSecChIdx =
				    ((index + 4) <
				     pAd->ChannelListNum) ? (index + 4) : (pAd->ChannelListNum - 1);

				chStartIdx = (curPriChIdx >= 3) ? (curPriChIdx - 3) : 0;
				chEndIdx =
				    ((curSecChIdx + 3) <
				     pAd->ChannelListNum) ? (curSecChIdx +
							     3) : (pAd->ChannelListNum - 1);
			} else {
				curPriChIdx = index;
				...
				chEndIdx =
				    ((curPriChIdx + 3) <
				     pAd->ChannelListNum) ? (curPriChIdx +
							     3) : (pAd->ChannelListNum - 1);
			}
		} else {
			/* It should not happened! */
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Ready to do passive scanning for Channel[%d] to Channel[%d]!\n",
				  pAd->ChannelList[chStartIdx].Channel,
				  pAd->ChannelList[chEndIdx].Channel));
	...
	for (index = chStartIdx; index <= chEndIdx; index++) {
		DBGPRINT(RT_DEBUG_TRACE, ("Channel[Idx=%d, Ch=%d].bEffectedChannel=0x%x!\n",
					  index, pAd->ChannelList[index].Channel,
					  pAd->ChannelList[index].bEffectedChannel));
		if ((pAd->ChannelList[index].bEffectedChannel & (EFFECTED_CH_PRIMARY |
								 EFFECTED_CH_LEGACY))
		    && (index != curPriChIdx)) {
			needFallBack = TRUE;
			DBGPRINT(RT_DEBUG_TRACE, ("needFallBack=TRUE due to OP/OT!\n"));
		}
		if ((pAd->ChannelList[index].bEffectedChannel & EFFECTED_CH_SECONDARY)
		    && (index != curSecChIdx)) {
			needFallBack = TRUE;
...
/*
 ...
 ========================================================================
*/
BOOLEAN DOT1X_InternalCmdAction(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY *pEntry, IN UINT8 cmd)
{
...
/*
 ...
 ========================================================================
*/
BOOLEAN DOT1X_EapTriggerAction(IN PRTMP_ADAPTER pAd, IN MAC_TABLE_ENTRY *pEntry)
{
	INT apidx = MAIN_MBSSID;
	...
	if ((pEntry->AuthMode == Ndis802_11AuthModeWPA)
	    ...
	    || (pAd->ApCfg.MBSSID[apidx].wdev.IEEE8021X == TRUE)) {
		/* Init the frame buffer */
		NdisZeroMemory(FrameBuf, frame_len);
		...
		/* Prepare the 802.3 header */
		MAKE_802_3_HEADER(FrameBuf, pAd->ApCfg.MBSSID[apidx].wdev.bssid, pEntry->Addr,
				  EAPOL);
		offset += LENGTH_802_3;
