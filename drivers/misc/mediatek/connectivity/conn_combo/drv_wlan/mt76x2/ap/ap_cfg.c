...
COUNTRY_CODE_TO_COUNTRY_REGION allCountry[] = {
	...
	,
	{408, "KP", "KOREA DEMOCRATIC PEOPLE'S REPUBLIC OF", TRUE, A_BAND_REGION_5, TRUE,
	 G_BAND_REGION_1}
	,
...
INT RTMPAPPrivIoctlShow(IN RTMP_ADAPTER *pAd, IN RTMP_IOCTL_INPUT_STRUCT * pIoctlCmdStr)
{
	...
	while ((this_char = strsep((char **)&pIoctlCmdStr->u.data.pointer, ",")) != NULL) {
		...
		for (PRTMP_PRIVATE_SHOW_PROC = RTMP_PRIVATE_SHOW_SUPPORT_PROC;
		     PRTMP_PRIVATE_SHOW_PROC->name; PRTMP_PRIVATE_SHOW_PROC++) {
		...
		if (PRTMP_PRIVATE_SHOW_PROC->name == NULL) {	/*Not found argument */
			Status = -EINVAL;
			DBGPRINT(RT_DEBUG_TRACE,
				 ("IOCTL::(iwpriv) Command not Support [%s=%s]\n", this_char,
				  value));
...
VOID RTMPAPGetAssoMacTable(IN PRTMP_ADAPTER pAd, IN RTMP_IOCTL_INPUT_STRUCT * wrq)
{
	INT32 DataRate = 0;
	...
	os_alloc_mem(NULL, (UCHAR **) &msg,
		     sizeof(CHAR) * (MAX_LEN_OF_MAC_TABLE * ASSO_MAC_LINE_LEN));
	if (msg == NULL) {
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if ((IS_ENTRY_CLIENT(pEntry) || (IS_ENTRY_APCLI(pEntry)
			...
			if ((strlen(msg) + ASSO_MAC_LINE_LEN) >=
			    (MAX_LEN_OF_MAC_TABLE * ASSO_MAC_LINE_LEN))
				break;
			...
			sprintf(msg + strlen(msg), "%-4d",
				(int)CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_WMM_CAPABLE));
			sprintf(msg + strlen(msg), "%-8d", (int)pEntry->MmpsMode);

			if (pAd->CommonCfg.RxStream == 3)
				sprintf(msg + strlen(msg), "%-7d%-7d%-7d",
					pEntry->RssiSample.AvgRssi0, pEntry->RssiSample.AvgRssi1,
					pEntry->RssiSample.AvgRssi2);
			else if (pAd->CommonCfg.RxStream == 2)
				sprintf(msg + strlen(msg), "%-7d%-7d", pEntry->RssiSample.AvgRssi0,
...
INT RTMPAPSetInformation(IN PRTMP_ADAPTER pAd, INOUT RTMP_IOCTL_INPUT_STRUCT * rq, IN INT cmd)
{
	...
	case OID_802_11_SET_IEEE8021X:
		...
		if (wrq->u.data.length != sizeof(BOOLEAN))
			...
			if (apcliEn == TRUE) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Set Apcli(%d)::OID_802_11_SET_IEEE8021X (=%d)\n",
	...
	case OID_802_11_SET_IEEE8021X_REQUIRE_KEY:
		...
		if (wrq->u.data.length != sizeof(BOOLEAN))
			...
			if (apcliEn == TRUE) {
				Status =
				    copy_from_user(&IEEE8021x_required_keys, wrq->u.data.pointer,
						   wrq->u.data.length);
				pAd->ApCfg.ApCliTab[ifIndex].wpa_supplicant_info.
				    IEEE8021x_required_keys = IEEE8021x_required_keys;
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Set Apcli(%d)::OID_802_11_SET_IEEE8021X_REQUIRE_KEY (%d)\n",
	...
	case OID_802_11_PMKID:
		...
		if (pPmkId->BSSIDInfoCount == 0)
			NdisZeroMemory(pAd->ApCfg.ApCliTab[ifIndex].SavedPMK,
				       sizeof(pAd->ApCfg.ApCliTab[ifIndex].SavedPMK));
		else {
			...
			for (BssIdx = 0; BssIdx < pPmkId->BSSIDInfoCount; BssIdx++) {
				...
				for (CachedIdx = 0;
				     CachedIdx < pAd->ApCfg.ApCliTab[ifIndex].SavedPMKNum;
				     CachedIdx++) {
					/* compare the BSSID */
					if (NdisEqualMemory
					    (pBssIdInfo->BSSID,
					     pAd->ApCfg.ApCliTab[ifIndex].SavedPMK[CachedIdx].BSSID,
				...
				if (CachedIdx < PMKID_NO) {
					DBGPRINT(RT_DEBUG_OFF,
						 ("Update OID_802_11_PMKID, idx = %d\n",
						  CachedIdx));
					NdisMoveMemory(&pAd->ApCfg.ApCliTab[ifIndex].
	...
	case RT_OID_WPA_SUPPLICANT_SUPPORT:
		...
		if (wrq->u.data.length != sizeof(UCHAR))
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("APCLI Set::RT_OID_WPA_SUPPLICANT_SUPPORT (=0x%02X)\n",
				  pAd->ApCfg.ApCliTab[ifIndex].wpa_supplicant_info.
				  WpaSupplicantUP));
		}
		break;
			...
			if (pAd->ApCfg.ApCliTab[ifIndex].wdev.AuthMode >= Ndis802_11AuthModeWPA) {
				...
				if (KeyIdx & 0x80000000) {
					...
					if (KeyIdx > 3) {
						...
						AsicRemoveSharedKeyEntry(pAd, BssIdx,
									 (UCHAR) KeyIdx);
						DBGPRINT(RT_DEBUG_TRACE,
	...
	case OID_802_11_ADD_WEP:
		...
		if (Status) {
			...
			if (KeyIdx > 4) {
				...
				NdisMoveMemory(pApCliEntry->SharedKey[KeyIdx].Key,
				...
				default:
					...
					DBGPRINT(RT_DEBUG_TRACE,
						 ("only support CIPHER_WEP64(len:5) & CIPHER_WEP128(len:13)\n"));
					Status = -EINVAL;
					break;
				}
				...
				if (pWepKey->KeyIndex & 0x80000000) {
					NdisZeroMemory(&pApCliEntry->wpa_supplicant_info.
						       DesireSharedKey[KeyIdx], sizeof(CIPHER_KEY));

					/* set key material and key length */
					pApCliEntry->wpa_supplicant_info.DesireSharedKey[KeyIdx].
					    KeyLen = (UCHAR) pWepKey->KeyLength;
					NdisMoveMemory(pApCliEntry->wpa_supplicant_info.
				...
				if ((pApCliEntry->wpa_supplicant_info.WpaSupplicantUP !=
					...
					AsicAddSharedKeyEntry(pAd, BssIdx, KeyIdx,
							      &pApCliEntry->SharedKey[KeyIdx]);

					if (pWepKey->KeyIndex & 0x80000000) {
						RTMPSetWcidSecurityInfo(pAd, BssIdx, KeyIdx,
									CipherAlg,
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Set ApCli::OID_802_11_ADD_WEP (id=0x%x, Len=%d-byte), %s\n",
					  pWepKey->KeyIndex, pWepKey->KeyLength,
					  pMacEntry->PortSecured ==
					  WPA_802_1X_PORT_SECURED ? "Port Secured" :
					  "Port NOT Secured"));
			}
		}
	...
	case OID_802_11_DISASSOCIATE:
		...
		DisassocParmFill(pAd, &DisassocReq, pAd->MlmeAux.Bssid,
	...
	case OID_802_11_DROP_UNENCRYPTED:
		...
		if (wrq->u.data.length != sizeof(int))
			...
			NdisReleaseSpinLock(&pAd->MacTabLock);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Set ApCLi::OID_802_11_DROP_UNENCRYPTED (=%d)\n", enabled));
		}
		break;
	...
	case OID_SET_COUNTERMEASURES:
		...
		if (wrq->u.data.length != sizeof(int))
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Set ApCli::OID_SET_COUNTERMEASURES bBlockAssoc=%s\n",
	...
	case OID_802_11_DEAUTHENTICATION:
		...
		if (wrq->u.data.length != sizeof(MLME_DEAUTH_REQ_STRUCT))
			...
			if (pObj->ioctl_if_type == INT_APCLI) {
				...
				os_alloc_mem(pAd, (UCHAR **) &pInfo,
					     sizeof(MLME_DEAUTH_REQ_STRUCT));
				Status =
				    copy_from_user(pInfo, wrq->u.data.pointer, wrq->u.data.length);
				...
				MlmeEnqueue(pAd,
					    ...
					    sizeof(MLME_DEAUTH_REQ_STRUCT), &DeAuthFrame, ifIndex);
...
#endif /*APCLI_SUPPORT */
			{
				if (Elem) {
					...
					Status =
					    copy_from_user(pInfo, wrq->u.data.pointer,
							   wrq->u.data.length);
					pEntry = MacTableLookup(pAd, pInfo->Addr);
					if (pEntry != NULL) {
						Elem->Wcid = pEntry->wcid;
						MlmeEnqueue(pAd, AP_AUTH_STATE_MACHINE,
							    APMT2_MLME_DEAUTH_REQ,
							    sizeof(MLME_DEAUTH_REQ_STRUCT), Elem,
							    0);
						DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_SET_IAPP_PID:
		{
			...
			if (copy_from_user(&IappPid, wrq->u.data.pointer, wrq->u.data.length))
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("RT_SET_APD_PID::(IappPid=%lu(0x%x))\n", IappPid,
					  pObj->IappPid));
			}
		}
		break;
	...
	case RT_SET_APD_PID:
		{
			...
			if (copy_from_user(&apd_pid, wrq->u.data.pointer, wrq->u.data.length))
				...
				RTMP_GET_OS_PID(apd_pid, apd_pid);
				pObj->apd_pid_nr = apd_pid;
				DBGPRINT(RT_DEBUG_TRACE,
					 ("RT_SET_APD_PID::(ApdPid=%lu(%p))\n", apd_pid,
					  pObj->apd_pid));
			}
		}
		break;
	case RT_SET_DEL_MAC_ENTRY:
		if (copy_from_user(Addr, wrq->u.data.pointer, wrq->u.data.length))
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("RT_SET_DEL_MAC_ENTRY::(%02x:%02x:%02x:%02x:%02x:%02x)\n",
...
/*					MacTableDeleteEntry(pAd, pEntry->wcid, Addr); */
	...
	case RT_OID_WSC_SET_CON_WPS_STOP:
		{
			...
			os_alloc_mem(NULL, (UCHAR **) &pWscUpnpBandStop,
				     sizeof(WSC_UPNP_CTRL_WSC_BAND_STOP));

			if (pWscUpnpBandStop) {
				Status =
				    copy_from_user(pWscUpnpBandStop, wrq->u.data.pointer,
						   wrq->u.data.length);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("CON_WPS BAND_STOP_CMD From[%s], isApCli[%d], is2gBand[%d]\n",
					  pWscUpnpBandStop->ifName, pWscUpnpBandStop->isApCli,
					  pWscUpnpBandStop->is2gBand));

				if (pWscUpnpBandStop->isApCli) {
					pWpsCtrl = &pAd->ApCfg.ApCliTab[BSS0].WscControl;
					DBGPRINT(RT_DEBUG_TRACE,
						 ("CON_WPS FROM IOCTL: Stop the ApCli WPS, state [%d]\n",
						  pWpsCtrl->WscState));
					...
					if ((pWpsCtrl->WscConfMode != WSC_DISABLE) &&
						...
						DBGPRINT(RT_DEBUG_TRACE,
							 ("FROM IOCTL CON_WPS[%d]: Stop the AP Wsc Machine\n",
							  apidx));
						WscBuildBeaconIE(pAd, IsAPConfigured, FALSE, 0, 0,
								 apidx, NULL, 0, AP_MODE);
						WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR,
								    IsAPConfigured, FALSE, 0, 0,
								    apidx, NULL, 0, AP_MODE);
	...
	case RT_OID_WSC_SET_SELECTED_REGISTRAR:
		{
			...
			if (pAd->ApCfg.MBSSID[apidx].Hostapd == Hostapd_EXT) {
				...
				if (upnpInfo) {
					...
					Status =
					    copy_from_user(upnpInfo, wrq->u.data.pointer,
							   wrq->u.data.length);
					if (Status == NDIS_STATUS_SUCCESS) {
						len = wrq->u.data.length;

						if ((pAd->ApCfg.MBSSID[apidx].WscControl.
						     WscConfMode & WSC_PROXY)) {
							WscSelectedRegistrar(pAd, upnpInfo, len,
									     apidx);
							if (pAd->ApCfg.MBSSID[apidx].WscControl.
							    Wsc2MinsTimerRunning == TRUE) {
								BOOLEAN Cancelled;

								RTMPCancelTimer(&pAd->ApCfg.MBSSID
										[apidx].WscControl.
										Wsc2MinsTimer,
										&Cancelled);
							}
							/* 2mins time-out timer */
							RTMPSetTimer(&pAd->ApCfg.MBSSID[apidx].
								     WscControl.Wsc2MinsTimer,
								     WSC_TWO_MINS_TIME_OUT);
			...
			if (pAd->ApCfg.MBSSID[apidx].Hostapd == Hostapd_EXT) {
				...
				if (pUPnPMsg == NULL)
					...
					HeaderLen =
					    ...
					    sizeof(IEEE8021X_FRAME) + sizeof(EAP_FRAME);
					...
					/* If AP is unconfigured,
					* WPS state machine will be triggered after received M2.
					*/
					if ((pWscControl->WscConfStatus == WSC_SCSTATE_UNCONFIGURED)
#ifdef WSC_V2_SUPPORT
					    && (pWscControl->WscV2Info.bWpsEnable
						...
						if (strstr(pWpsMsg, "SimpleConfig") &&
						    ...
						    if (WscRxMsgTypeFromUpnp
							(pAd, pWpsMsg, WpsMsgLen) == WSC_MSG_M2
							&& !pWscControl->EapMsgRunning
							&& !pWscControl->WscUPnPNodeInfo.
							bUPnPInProgress) {
							/* Check Enrollee Nonce of M2 */
							if (WscCheckEnrolleeNonceFromUpnp
							    (pAd, pWpsMsg, WpsMsgLen,
							     pWscControl)) {
								WscGetConfWithoutTrigger(pAd,
											 pWscControl,
											 TRUE);
	...
	case RT_OID_WSC_READ_UFD_FILE:
		if (wrq->u.data.length > 0) {
			...
			if (pWscUfdFileName) {
				...
				if (copy_from_user
				    (pWscUfdFileName, wrq->u.data.pointer, wrq->u.data.length))
					Status = -EFAULT;
				else {
					DBGPRINT(RT_DEBUG_TRACE,
					...
					if (pWscCtrl->WscConfStatus == WSC_SCSTATE_UNCONFIGURED) {
						if (WscReadProfileFromUfdFile
						    (pAd, apIdx, pWscUfdFileName)) {
			...
			if (pWscUfdFileName) {
				...
				if (copy_from_user
					...
					if (pWscCtrl->WscConfStatus == WSC_SCSTATE_CONFIGURED) {
						WscWriteProfileToUfdFile(pAd, apIdx,
									 pWscUfdFileName);
					} else {
						DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_WSC_UUID:
		if (wrq->u.data.length == (UUID_LEN_STR - 1)) {
			...
			Status =
			    copy_from_user(&pAd->ApCfg.MBSSID[apIdx].WscControl.Wsc_Uuid_Str[0],
	...
	case OID_802_11_WEPDEFAULTKEYVALUE:
		{
			...
			if (KeyIdx > 4)
				...
				pAd->SharedKey[pObj->ioctl_if][pAd->ApCfg.MBSSID[pObj->ioctl_if].
							       DefaultKeyId].KeyLen =
				    (UCHAR) pKey->KeyLength;
				NdisMoveMemory(&pAd->SharedKey[pObj->ioctl_if]
					       [pAd->ApCfg.MBSSID[pObj->ioctl_if].DefaultKeyId].Key,
	...
	case OID_802_11_WEPDEFAULTKEYID:
		...
		if (wrq->u.data.length != sizeof(UCHAR))
			...
			Status =
			    copy_from_user(&pAd->ApCfg.MBSSID[pObj->ioctl_if].DefaultKeyId,
					   wrq->u.data.pointer, wrq->u.data.length);
	...
	case OID_802_11_PORT_SECURE_STATE:
		if (wrq->u.data.length != sizeof(WAPI_PORT_SECURE_STRUCT))
			...
			if (Status == NDIS_STATUS_SUCCESS) {
				pEntry = MacTableLookup(pAd, wapi_port.Addr);
				if (pEntry != NULL) {
	...
	case OID_802_11_UCAST_KEY_INFO:
		if (wrq->u.data.length != sizeof(WAPI_UCAST_KEY_STRUCT))
			...
			if (Status == NDIS_STATUS_SUCCESS) {
				pEntry = MacTableLookup(pAd, wapi_ukey.Addr);
				if (pEntry != NULL) {
	...
	case OID_802_11_AUTHENTICATION_MODE:
		if (wrq->u.data.length != sizeof(NDIS_802_11_AUTHENTICATION_MODE))
				...
				if (apcliEn) {
					...
					for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
						if (IS_ENTRY_APCLI(&pAd->MacTab.Content[i])) {
							pAd->MacTab.Content[i].PortSecured =
					...
					if (pAd->ApCfg.ApCliTab[ifIndex].wdev.AuthMode >=
			...
			pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.PortSecured =
			    WPA_802_1X_PORT_NOT_SECURED;
			DBGPRINT(RT_DEBUG_TRACE,
			...
			if (pObj->ioctl_if_type == INT_APCLI) {
				...
				if (apcliEn) {
					if (WepStatus <= Ndis802_11GroupWEP104Enabled) {
						if (pAd->ApCfg.ApCliTab[ifIndex].wdev.WepStatus !=
						    WepStatus) {
							/* Config has changed */
						...
						pApCliEntry->PairCipher =
						    pApCliEntry->wdev.WepStatus;
						pApCliEntry->GroupCipher =
						    pApCliEntry->wdev.WepStatus;
						pApCliEntry->bMixCipher = FALSE;

						if (pApCliEntry->wdev.WepStatus >=
...
#endif /*APCLI_SUPPORT */
			{
				...
				if (WepStatus <= Ndis802_11GroupWEP104Enabled) {
					struct wifi_dev *wdev =
					    &pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev;
		...
		if (wrq->u.data.length != sizeof(NDIS_802_11_SSID))
			...
			if (Ssid.SsidLength > MAX_LEN_OF_SSID)
				...
				if (Ssid.SsidLength == 0)
					...
					os_alloc_mem(NULL, (UCHAR **) &pSsidString,
						     MAX_LEN_OF_SSID + 1);
					if (pSsidString) {
...
#endif /*APCLI_SUPPORT */
						{
							NdisZeroMemory((PCHAR) pAd->ApCfg.
								       MBSSID[pObj->ioctl_if].Ssid,
								       MAX_LEN_OF_SSID);
							strcpy((PCHAR) pAd->ApCfg.
							       MBSSID[pObj->ioctl_if].Ssid,
							       pSsidString);
	...
	case OID_802_11_PASSPHRASE:
		{
			...
			hex_dump("Set::OID_802_11_PASSPHRASE ==>", &tmpPSK.WPAKey[0],
				 tmpPSK.WPAKeyLen);

			pMBSSStruct = &pAd->ApCfg.MBSSID[pObj->ioctl_if];
			retval =
			    RT_CfgSetWPAPSKKey(pAd, &tmpPSK.WPAKey[0], tmpPSK.WPAKeyLen,
					       (PUCHAR) pMBSSStruct->Ssid, pMBSSStruct->SsidLen,
					       pMBSSStruct->PMK);
			if (retval == FALSE)
			...
			NdisMoveMemory(pMBSSStruct->WscControl.WpaPsk, &tmpPSK.WPAKey[0],
				       pMBSSStruct->WscControl.WpaPskLen);
	...
	case HOSTAPD_OID_SET_KEY:
		{
			...
			pObj = (POS_COOKIE) pAd->OS_Cookie;
			apidx = pObj->ioctl_if;
			if (wrq->u.data.length != sizeof(struct ieee80211req_key)
			    || !wrq->u.data.pointer)
				break;
			...
			if ((Key.ik_type == CIPHER_WEP64) || (Key.ik_type == CIPHER_WEP128)) {	/*dynamic wep with 1x */
				if (pEntry) {	/*pairwise key */
					...
					NdisMoveMemory(pEntry->PairwiseKey.Key, Key.ik_keydata,
						       Key.ik_keylen);
					...
					RTMPAddWcidAttributeEntry(pAd, pEntry->apidx, KeyIdx,
								  pEntry->PairwiseKey.CipherAlg,
								  pEntry);
				} else {	/*group key */

					pMbss = &pAd->ApCfg.MBSSID[apidx];
					DBGPRINT(RT_DEBUG_TRACE, ("Key.ik_keyix=%x\n", Key.ik_keyix));
					KeyIdx = Key.ik_keyix & 0x0fff;
					DBGPRINT(RT_DEBUG_TRACE, ("ra%d KeyIdx=%d\n", apidx, KeyIdx));
					DBGPRINT(RT_DEBUG_TRACE, ("Key.ik_keyix=%x\n", Key.ik_keyix));
					/* it is a shared key */
					if (KeyIdx < 4) {
						pAd->SharedKey[apidx][KeyIdx].KeyLen =
						    (UCHAR) Key.ik_keylen;
						NdisMoveMemory(pAd->SharedKey[apidx][KeyIdx].Key,
							       &Key.ik_keydata, Key.ik_keylen);
						...
						pAd->SharedKey[apidx][KeyIdx].CipherAlg =
						    Key.ik_type;
						AsicAddSharedKeyEntry(pAd, apidx, KeyIdx,
				...
				if (pEntry->WepStatus == Ndis802_11TKIPEnable) {
					...
					NdisMoveMemory(&pEntry->PTK[OFFSET_OF_PTK_TK],
						       Key.ik_keydata, Key.ik_keylen);
				...
				if (pEntry->WepStatus == Ndis802_11AESEnable) {
					pEntry->PairwiseKey.KeyLen = LEN_TK;
					NdisMoveMemory(&pEntry->PTK[OFFSET_OF_PTK_TK],
						       Key.ik_keydata, OFFSET_OF_PTK_TK);
					NdisMoveMemory(pEntry->PairwiseKey.Key,
						       &pEntry->PTK[OFFSET_OF_PTK_TK],
						       Key.ik_keylen);
				}
				...
				AsicAddPairwiseKeyEntry(pAd,
							(UCHAR) pEntry->wcid, &pEntry->PairwiseKey);
				...
				if (pAd->ApCfg.MBSSID[apidx].GroupKeyWepStatus ==
				    Ndis802_11TKIPEnable) {
					pAd->SharedKey[apidx][pMbss->DefaultKeyId].KeyLen = LEN_TK;
					NdisMoveMemory(pAd->SharedKey[apidx][pMbss->DefaultKeyId].
						       Key, Key.ik_keydata, 16);
					NdisMoveMemory(pAd->SharedKey[apidx][pMbss->DefaultKeyId].
						       RxMic, (Key.ik_keydata + 16 + 8), 8);
					NdisMoveMemory(pAd->SharedKey[apidx][pMbss->DefaultKeyId].
				...
				if (pAd->ApCfg.MBSSID[apidx].GroupKeyWepStatus ==
					...
					pAd->SharedKey[apidx][pMbss->DefaultKeyId].CipherAlg =
					    CIPHER_TKIP;
				else if (pAd->ApCfg.MBSSID[apidx].GroupKeyWepStatus ==
					 Ndis802_11AESEnable)
					pAd->SharedKey[apidx][pMbss->DefaultKeyId].CipherAlg =
					    CIPHER_AES;
	...
	case HOSTAPD_OID_DEL_KEY:
		...
		Status = -EINVAL;
		if (wrq->u.data.length != sizeof(struct ieee80211req_del_key)
		    || !wrq->u.data.pointer)
			break;
	...
	case HOSTAPD_OID_STATIC_WEP_COPY:
		{
			...
			if (wrq->u.data.length != sizeof(struct ieee80211req_mlme))
				...
				Status =
				    copy_from_user(&mlme, wrq->u.data.pointer, wrq->u.data.length);
				pEntry = MacTableLookup(pAd, mlme.im_macaddr);
				if (!pEntry)
					Status = -EINVAL;
				else {
					...
					DBGPRINT(RT_DEBUG_TRACE,
						...
						pEntry->WepStatus));
					if (pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.IEEE8021X !=
					    TRUE)
						break;
					...
					DBGPRINT(RT_DEBUG_TRACE, ("Woody HOSTAPD_OID_STATIC_WEP_COPY=%d\n", KeyIdx));
		...
		if (wrq->u.data.length != sizeof(BOOLEAN))
			...
			Status =
			    copy_from_user(&hostapd_enable, wrq->u.data.pointer,
					   wrq->u.data.length);
			DBGPRINT(RT_DEBUG_TRACE, ("OID_HOSTAPD_SUPPORT apidx=%d\n", apidx));
			pAd->ApCfg.MBSSID[apidx].Hostapd = hostapd_enable;
			MULTISSID_STRUCT *pMBSSStruct;

			for (v = 0; v < MAX_MBSSID_NUM(pAd); v++) {
				if (pAd->ApCfg.MBSSID[v].Hostapd == Hostapd_EXT)
					DBGPRINT(RT_DEBUG_TRACE, ("ApCfg->MBSSID[%d].Hostapd == TRUE\n", v));
				else
					DBGPRINT(RT_DEBUG_TRACE, ("ApCfg->MBSSID[%d].Hostapd == FALSE\n", v));
		...
		if (wrq->u.data.length != sizeof(BOOLEAN))
			...
			if (countermeasures_enable) {
				...
				for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
					...
					/* happened twice within 60 sec,
					*AP SENDS disaccociate all associated STAs.
					*All STA's transition to State 2
					*/
	...
	case HOSTAPD_OID_SET_WPS_BEACON_IE:	/*pure 1x is enabled. */
		...
		if (wrq->u.data.length != sizeof(WSC_LV_INFO))
			...
			Status =
			    copy_from_user(&WscIEBeacon, wrq->u.data.pointer, wrq->u.data.length);
			pMBSSStruct = &pAd->ApCfg.MBSSID[apidx];
			NdisMoveMemory(pMBSSStruct->WscIEBeacon.Value, WscIEBeacon.Value,
				       WscIEBeacon.ValueLen);
	...
	case HOSTAPD_OID_SET_WPS_PROBE_RESP_IE:	/*pure 1x is enabled. */
		...
		if (wrq->u.data.length != sizeof(WSC_LV_INFO)) {
			...
			Status =
			    copy_from_user(&WscIEProbeResp, wrq->u.data.pointer,
					   wrq->u.data.length);
			pMBSSStruct = &pAd->ApCfg.MBSSID[apidx];
			NdisMoveMemory(pMBSSStruct->WscIEProbeResp.Value, WscIEProbeResp.Value,
				       WscIEProbeResp.ValueLen);
	...
	case OID_802_11_HS_ANQP_RSP:
		{
			...
			Send_ANQP_Rsp(pAd,
				      ...
				      rsp_data->anqp_rsp, rsp_data->anqp_rsp_len);
	...
	case OID_802_11_HS_ONOFF:
		{
			...
			Set_HotSpot_OnOff(pAd, onoff->hs_onoff, onoff->event_trigger,
					  onoff->event_type);
			os_free_mem(NULL, Buf);
		}
		break;
...
INT RTMPAPQueryInformation(IN RTMP_ADAPTER *pAd, IN OUT RTMP_IOCTL_INPUT_STRUCT *rq, IN INT cmd)
{
	RTMP_IOCTL_INPUT_STRUCT *wrq = (RTMP_IOCTL_INPUT_STRUCT *) rq;
	INT Status = NDIS_STATUS_SUCCESS;
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	...
	case RT_OID_NEW_DRIVER:
		{
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Query apcli::RT_OID_NEW_DRIVER (=%d)\n", enabled));
			break;
		}
	...
	case OID_802_11_BSSID_LIST:
		...
		if ((pAd->ApCfg.ApCliTab[ifIndex].wpa_supplicant_info.WpaSupplicantUP & 0x7F) ==
		    WPA_SUPPLICANT_ENABLE) {
		...
		for (i = 0; i < pAd->ScanTab.BssNr; i++) {
			...
			if ((pAd->ScanTab.BssEntry[i].Hidden == 1)) {
				/*
				 * We must return this SSID during 4way handshaking,
				 * otherwise Aegis will failed to parse WPA information
	...
	case OID_802_3_CURRENT_ADDRESS:
		...
		Status =
		    copy_to_user(wrq->u.data.pointer, pApCliEntry->wdev.if_addr,
				 wrq->u.data.length);
		break;
	...
	case OID_802_11_BSSID:
		...
		if (INFRA_ON(pAd) || ADHOC_ON(pAd)) {
			Status =
			    copy_to_user(wrq->u.data.pointer, pApCliEntry->CfgApCliBssid,
					 sizeof(NDIS_802_11_MAC_ADDRESS));

			DBGPRINT(RT_DEBUG_INFO,
				 ("IOCTL::SIOCGIWAP(=%02x:%02x:%02x:%02x:%02x:%02x)\n",
				  pApCliEntry->CfgApCliBssid[0], pApCliEntry->CfgApCliBssid[1],
				  pApCliEntry->CfgApCliBssid[2], pApCliEntry->CfgApCliBssid[3],
				  pApCliEntry->CfgApCliBssid[4], pApCliEntry->CfgApCliBssid[5]));
	...
	case OID_802_11_SSID:
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Query Apcli::OID_802_11_SSID (Len=%d, ssid=%s)\n", Ssid.SsidLength,
			  Ssid.Ssid));
		break;
#endif /*APCLI_WPA_SUPPLICANT_SUPPORT */
#endif /*APCLI_SUPPORT */
	...
	case RT_OID_802_11_PHY_MODE:
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Query::RT_OID_802_11_PHY_MODE (=%ld), %s\n", ulInfo,
			  wmode_2_str(pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.PhyMode)));
		break;
	...
	case OID_802_11_CHANNEL_WIDTH:
		{
			...
			Status = copy_to_user(wrq->u.data.pointer, &ulInfo, wrq->u.data.length);
			DBGPRINT(RT_DEBUG_TRACE,
				 ("Query::OID_802_11_CHANNEL_WIDTH (%lu)\n", ulInfo));
			break;
		}
		break;
	...
	case OID_802_11_BSSID:
		{
			...
			if (pObj->ioctl_if_type == INT_APCLI) {
				...
				if (pApCliEntry->Valid) {
					Status =
					    copy_to_user(wrq->u.data.pointer, wdev->bssid,
							 sizeof(NDIS_802_11_MAC_ADDRESS));
...
#endif /*APCLI_SUPPORT */
			{
				...
				wrq->u.data.length = MAC_ADDR_LEN;
				Status =
				    copy_to_user(wrq->u.data.pointer, &wdev->bssid[0],
						 wrq->u.data.length);
				DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_802_11_PREAMBLE:
		...
		Status = copy_to_user(wrq->u.data.pointer, &ulInfo, wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Query::RT_OID_802_11_PREAMBLE(=%ld)\n", pAd->CommonCfg.TxPreamble));
		break;
	...
	case OID_802_11_TX_PACKET_BURST:
		wrq->u.data.length = sizeof(BOOLEAN);
		Status =
		    copy_to_user(wrq->u.data.pointer, &pAd->CommonCfg.bEnableTxBurst,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case OID_802_11_COEXISTENCE:
		wrq->u.data.length = sizeof(BOOLEAN);
		Status =
		    copy_to_user(wrq->u.data.pointer, &pAd->CommonCfg.bBssCoexEnable,
				 wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE,
	...
	case RT_OID_WSC_MAC_ADDRESS:
		...
		wrq->u.data.length = MAC_ADDR_LEN;
		if (copy_to_user
		    (wrq->u.data.pointer, pAd->ApCfg.MBSSID[apidx].wdev.bssid,
		     wrq->u.data.length))
			Status = -EFAULT;
		break;
	case RT_OID_WSC_CONFIG_STATUS:
		...
		wrq->u.data.length = sizeof(UCHAR);
		if (copy_to_user
		    (wrq->u.data.pointer, &pAd->ApCfg.MBSSID[apidx].WscControl.WscConfStatus,
		     wrq->u.data.length)) {
			Status = -EFAULT;
		}
		break;
	...
	case RT_OID_WSC_QUERY_PEER_INFO_ON_RUNNING:
		...
		if (pAd->ApCfg.MBSSID[apidx].WscControl.WscState > WSC_STATE_WAIT_M2) {
			...
			if (copy_to_user
			    (wrq->u.data.pointer, &pAd->ApCfg.MBSSID[apidx].WscControl.WscPeerInfo,
			     wrq->u.data.length)) {
				Status = -EFAULT;
			}
		} else
	...
	case RT_OID_WSC_FRAGMENT_SIZE:
		...
		if (copy_to_user
		    (wrq->u.data.pointer, &pAd->ApCfg.MBSSID[apidx].WscControl.WscFragSize,
		     wrq->u.data.length)) {
			Status = -EFAULT;
		}
		break;
	...
	case RT_OID_NFC_STATUS:
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Query::RT_OID_NFC_STATUS (=%d)\n",
			  pAd->ApCfg.MBSSID[apidx].WscControl.NfcStatus));
		wrq->u.data.length = sizeof(UCHAR);
		if (copy_to_user
		    (wrq->u.data.pointer, &pAd->ApCfg.MBSSID[apidx].WscControl.NfcStatus,
		     wrq->u.data.length)) {
			Status = -EFAULT;
		}
		break;
		...
		if ((wrq->u.data.pointer == NULL) || (apidx != MAIN_MBSSID))
			...
			for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
				if (IS_ENTRY_CLIENT(&pAd->MacTab.Content[i])
					...
					COPY_MAC_ADDR(AssocTab.Entry[AssocTab.Num].Addr,
						      &pAd->MacTab.Content[i].Addr);
					AssocTab.Entry[AssocTab.Num].phyMode =
					    pAd->CommonCfg.PhyMode;
					AssocTab.Entry[AssocTab.Num].MOR =
					    RateIdToMbps[pAd->ApCfg.MBSSID[apidx].MaxTxRate] * 2;
					AssocTab.Num += 1;
				}
			}
	...
	case RT_OID_802_11_MAC_ADDRESS:
		wrq->u.data.length = MAC_ADDR_LEN;
		Status =
		    copy_to_user(wrq->u.data.pointer, &pAd->ApCfg.MBSSID[apidx].wdev.bssid,
				 wrq->u.data.length);
		break;
	...
	case OID_802_11_WEPDEFAULTKEYVALUE:
		...
		valueLen =
		    pAd->SharedKey[pObj->ioctl_if][pAd->ApCfg.MBSSID[pObj->ioctl_if].DefaultKeyId].
		    KeyLen;
		NdisMoveMemory(pKeyIdxValue->Value,
			       &pAd->SharedKey[pObj->ioctl_if][pAd->ApCfg.MBSSID[pObj->ioctl_if].
							       DefaultKeyId].Key, valueLen);
	...
	case RT_OID_802_11_WEPKEYMAPPINGLENGTH:
		...
		Status = copy_to_user(wrq->u.data.pointer,
				      &pAd->SharedKey[pObj->ioctl_if][pAd->ApCfg.
	...
	case RT_OID_802_11_QUERY_TXBF_TABLE:
		if (!pAd->chipCap.FlgHwTxBfCap)
			...
			os_alloc_mem(NULL, (UCHAR **) &MacTab, sizeof(RT_802_11_TXBF_TABLE));
			if (!MacTab) {
				Status = -ENOMEM;
				break;
			}
			...
			for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
				if (IS_ENTRY_CLIENT(&pAd->MacTab.Content[i])
				    && (pAd->MacTab.Content[i].Sst == SST_ASSOC)) {
					memcpy(&MacTab->Entry[MacTab->Num],
	...
	case OID_802_11_WAPI_IE:
		...
		if (wrq->u.data.length != sizeof(WAPI_WIE_STRUCT))
			...
			if (pWapiEntry && IS_ENTRY_CLIENT(pWapiEntry)
			    && (pWapiEntry->RSNIE_Len > 0)) {
				wapi_ie.wie_len = pWapiEntry->RSNIE_Len;
				NdisMoveMemory(wapi_ie.wie, pWapiEntry->RSN_IE,
					       pWapiEntry->RSNIE_Len);
			}
	...
	case OID_802_11_MCAST_KEY_INFO:
		{
			...
			if (pMbss->sw_wpi_encrypt) {
				NdisMoveMemory(wapi_mkey.m_tx_iv,
					       pAd->SharedKey[pObj->ioctl_if][pMbss->wdev.
									      DefaultKeyId].TxTsc,
					       LEN_WAPI_TSC);
			} else {
				INT m_wcid;
			...
			NdisMoveMemory(wapi_mkey.key_announce, pMbss->key_announce_flag,
				       LEN_WAPI_TSC);
	...
	case HOSTAPD_OID_GETWPAIE:	/*report wpa ie of the new station to hostapd. */
		...
		if (wrq->u.data.length != sizeof(wpaie)) {
			...
			if ((pEntry->AuthMode == Ndis802_11AuthModeWPA)
				...
				p = wpaie.rsn_ie;
				hex_dump("HOSTAPD_OID_GETWPAIE woody==>pEntry->RSN_IE",
					 (unsigned char *)pEntry->RSN_IE, ielen);
	...
	case HOSTAPD_OID_GET_1X_GROUP_KEY:	/*report default group key to hostapd. */
		...
		if (wrq->u.data.length != sizeof(group_key))
			...
			if (pAd->SharedKey[apidx][pMbss->DefaultKeyId].KeyLen != 0
			    && pAd->SharedKey[apidx][pMbss->DefaultKeyId].Key != NULL) {
				group_key.ik_keyix = pMbss->DefaultKeyId;
				group_key.ik_keylen =
				    pAd->SharedKey[apidx][pMbss->DefaultKeyId].KeyLen;
				NdisMoveMemory(group_key.ik_keydata,
					       pAd->SharedKey[apidx][pMbss->DefaultKeyId].Key,
					       pAd->SharedKey[apidx][pMbss->DefaultKeyId].KeyLen);
				if (copy_to_user
				    (wrq->u.data.pointer, &group_key, sizeof(group_key)))
					Status = -EFAULT;
			}
		}
		break;
	...
	case OID_GEN_MEDIA_CONNECT_STATUS:
		{
			...
			if (pObj->ioctl_if_type != INT_APCLI) {
				...
				Status =
				    copy_to_user(wrq->u.data.pointer, &MediaState,
						 wrq->u.data.length);
			}
		}
		break;
	...
	case OID_802_11_ACL_LIST:
		if (wrq->u.data.length < sizeof(RT_802_11_ACL))
			...
			Status =
			    copy_to_user(wrq->u.data.pointer,
					 &pAd->ApCfg.MBSSID[pObj->ioctl_if].AccessControlList,
	...
	case OID_WIFI_TEST_RF_BANK:
		{
			...
			DBGPRINT(RT_DEBUG_ERROR, ("Info->rf_bank = %x\n", Info->rf_bank));
			DBGPRINT(RT_DEBUG_ERROR, ("Info->rf_start = %x\n", Info->rf_start));
			DBGPRINT(RT_DEBUG_ERROR, ("Info->rf_end = %x\n", Info->rf_end));

			for (Index = Info->rf_start; Index <= Info->rf_end; Index++) {
				UINT8 Value;

				rlt_rf_read(pAd, Info->rf_bank, Index, &Value);
				DBGPRINT(RT_DEBUG_ERROR, ("Offset = %x\n", Index));
				DBGPRINT(RT_DEBUG_ERROR, ("Value = %x\n", Value));
	...
	case OID_WIFI_TEST_MEM_MAP_INFO:
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("Info->base = %x\n", Info->base));
			DBGPRINT(RT_DEBUG_TRACE, ("Info->mem_map_start = %x\n", Info->mem_map_start));
			DBGPRINT(RT_DEBUG_TRACE, ("Info->mem_map_end = %x\n", Info->mem_map_end));
	...
	case OID_WIFI_TEST_E2P:
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("Info->e2p_start = %x\n", Info->e2p_start));
			DBGPRINT(RT_DEBUG_TRACE, ("Info->e2p_end = %x\n", Info->e2p_end));
	...
	case OID_WIFI_TEST_MAC:
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("Info->mac_start = %x\n", Info->mac_start));
			DBGPRINT(RT_DEBUG_TRACE, ("Info->mac_end = %x\n", Info->mac_end));
	...
	case OID_WIFI_TEST_RF_BANK_OFFSET:
		{
			...
			DBGPRINT(RT_DEBUG_TRACE, ("pAd->chipCap.RFBankNum = %d\n", pAd->chipCap.RFBankNum));
...
/*
    ...
    ==========================================================================
*/
INT Set_CountryCode_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
...
INT Set_ChGeography_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR,
		 ("Set_ChannelGeography_Proc:: Geography = %s\n",
		  pAd->CommonCfg.Geography == ODOR ? "out-door" : (pAd->CommonCfg.Geography ==
								   IDOR ? "in-door" : "both")));
	...
	if (strlen(arg) <= 38) {
		...
		for (index = 0; index < NUM_OF_COUNTRIES; index++) {
			...
			snprintf(name_buffer, sizeof(name_buffer), "\"%s\"",
				 (PSTRING) allCountry[index].pCountryName);

			if (strncmp((PSTRING) allCountry[index].pCountryName, arg, strlen(arg)) ==
			    0)
				break;
			...
			if (WMODE_CAP_2G(pAd->CommonCfg.PhyMode)) {
				if (allCountry[index].SupportGBand == TRUE) {
					pAd->CommonCfg.CountryRegion =
					    (UCHAR) allCountry[index].RegDomainNum11G;
					success = TRUE;
				} else {
					DBGPRINT(RT_DEBUG_TRACE,
			...
			if (WMODE_CAP_5G(pAd->CommonCfg.PhyMode)) {
				if (allCountry[index].SupportABand == TRUE) {
					pAd->CommonCfg.CountryRegionForABand =
					    (UCHAR) allCountry[index].RegDomainNum11A;
					success = TRUE;
				} else {
					DBGPRINT(RT_DEBUG_TRACE,
	...
	if (success == TRUE) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Set_CountryString_Proc::(CountryString=%s CountryRegin=%d CountryCode=%s)\n",
			  allCountry[index].pCountryName, pAd->CommonCfg.CountryRegion,
...
INT Set_ShowACKQParm_Proc(IN PRTMP_ADAPTER pAdapter, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Delayed TCP ACK queue patameters\n"));
	DBGPRINT(RT_DEBUG_TRACE, ("[enackq]  ACKQEN: %d\n", pComCfg->ACKQEN));
	DBGPRINT(RT_DEBUG_TRACE, ("[ackwait] AckWaitTime: %d\n", pComCfg->AckWaitTime));
	DBGPRINT(RT_DEBUG_TRACE, ("[acklen]  Acklen: %d\n", pComCfg->Acklen));
	DBGPRINT(RT_DEBUG_TRACE, ("ack timeout: = %d\n", pComCfg->AckTimeout));
	DBGPRINT(RT_DEBUG_TRACE, ("ack no timeout: = %d\n", pComCfg->AckNOTimeout));
	for (i = 0; i <= TCP_ACK_BURST_LEVEL; i++)
		DBGPRINT(RT_DEBUG_TRACE, ("ack%d: = %d\n", i, pComCfg->TcpAck[i]));
	return TRUE;
}
#endif /* DELAYED_TCP_ACK */
...
/*
    ...
    ==========================================================================
*/
INT Set_TxRate_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
...
INT Set_BasicRate_Proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("Set_BasicRate_Proc::(BasicRateBitmap=0x%08lx)\n",
...
/*
    ...
    ==========================================================================
*/
INT Set_BeaconPeriod_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	USHORT BeaconPeriod;
	INT success = FALSE;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("Set_BeaconPeriod_Proc::(BeaconPeriod=%d)\n", pAd->CommonCfg.BeaconPeriod));
...
/*
    ...
    ==========================================================================
*/
INT Set_DtimPeriod_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	USHORT DtimPeriod;
	INT success = FALSE;
...
/*
    ...
    ==========================================================================
*/
INT Set_AP_WmmCapable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	BOOLEAN bWmmCapable;
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
...
/*
    ...
    ==========================================================================
*/
INT Set_NoForwarding_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	ULONG NoForwarding;
	...
	DBGPRINT(RT_DEBUG_TRACE, ("IF(ra%d) Set_NoForwarding_Proc::(NoForwarding=%ld)\n",
				  pObj->ioctl_if,
				  pAd->ApCfg.MBSSID[pObj->ioctl_if].IsolateInterStaTraffic));

	return TRUE;
}

INT Set_NoForwardingMBCast_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
...
/*
    ...
    ==========================================================================
*/
INT Set_NoForwardingBTNSSID_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	ULONG NoForwarding;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("Set_NoForwardingBTNSSID_Proc::(NoForwarding=%ld)\n",
...
/*
    ...
    ==========================================================================
*/
INT Set_HideSSID_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	BOOLEAN bHideSsid;
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF(ra%d) Set_HideSSID_Proc::(HideSSID=%d)\n", pObj->ioctl_if,
...
/*
    ...
    ==========================================================================
*/
INT Set_VLANID_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	struct wifi_dev *wdev;
...
INT Set_VLANPriority_Proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF(ra%d) Set_VLANPriority_Proc::(VLAN_Priority=%d)\n", pObj->ioctl_if,
		  wdev->VLAN_Priority));
...
/*
    ...
    ==========================================================================
*/
INT Set_AP_AuthMode_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	ULONG i;
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
...
/*
    ...
    ==========================================================================
*/
INT Set_AP_EncrypType_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
	struct wifi_dev *wdev;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF(ra%d) Set_EncrypType_Proc::(EncrypType=%d)\n", apidx, wdev->WepStatus));
...
/*
    ...
    ==========================================================================
*/
INT Set_AP_WpaMixPairCipher_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
	struct wifi_dev *wdev;
	 ...
	 */
	wdev = &pAd->ApCfg.MBSSID[apidx].wdev;
	if ((strncmp(arg, "WPA_AES_WPA2_TKIPAES", 20) == 0)
	    || (strncmp(arg, "wpa_aes_wpa2_tkipaes", 20) == 0))
		wdev->WpaMixPairCipher = WPA_AES_WPA2_TKIPAES;
	else if ((strncmp(arg, "WPA_AES_WPA2_TKIP", 17) == 0)
		 || (strncmp(arg, "wpa_aes_wpa2_tkip", 17) == 0))
		wdev->WpaMixPairCipher = WPA_AES_WPA2_TKIP;
	else if ((strncmp(arg, "WPA_TKIP_WPA2_AES", 17) == 0)
		 || (strncmp(arg, "wpa_tkip_wpa2_aes", 17) == 0))
		wdev->WpaMixPairCipher = WPA_TKIP_WPA2_AES;
	else if ((strncmp(arg, "WPA_TKIP_WPA2_TKIPAES", 21) == 0)
		 || (strncmp(arg, "wpa_tkip_wpa2_tkipaes", 21) == 0))
		wdev->WpaMixPairCipher = WPA_TKIP_WPA2_TKIPAES;
	else if ((strncmp(arg, "WPA_TKIPAES_WPA2_AES", 20) == 0)
		 || (strncmp(arg, "wpa_tkipaes_wpa2_aes", 20) == 0))
		wdev->WpaMixPairCipher = WPA_TKIPAES_WPA2_AES;
	else if ((strncmp(arg, "WPA_TKIPAES_WPA2_TKIPAES", 24) == 0)
		 || (strncmp(arg, "wpa_tkipaes_wpa2_tkipaes", 24) == 0))
		wdev->WpaMixPairCipher = WPA_TKIPAES_WPA2_TKIPAES;
	else if ((strncmp(arg, "WPA_TKIPAES_WPA2_TKIP", 21) == 0)
		 || (strncmp(arg, "wpa_tkipaes_wpa2_tkip", 21) == 0))
		wdev->WpaMixPairCipher = WPA_TKIPAES_WPA2_TKIP;
	else
		return FALSE;

	DBGPRINT(RT_DEBUG_TRACE,
		 ("I/F(ra%d) Set_AP_WpaMixPairCipher_Proc=0x%02x\n", apidx,
...
/*
    ...
    ==========================================================================
*/
INT Set_AP_RekeyInterval_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
	INT32 val;
...
INT Set_AP_PROBE_RSP_TIMES(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
	INT input;
	...
	if ((input >= 0) && (input <= 10))
		...
		DBGPRINT(RT_DEBUG_ERROR, ("AP->ProbeRspTimes: Out of Range\n"));

	DBGPRINT(RT_DEBUG_TRACE,
		 ("AP[%d]->ProbeRspTimes: %d\n", apidx, pAd->ApCfg.MBSSID[apidx].ProbeRspTimes));
...
#ifdef SPECIFIC_TX_POWER_SUPPORT
INT Set_AP_PKT_PWR(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
	INT input;
...
/*
    ...
    ==========================================================================
*/
INT Set_AP_RekeyMethod_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
...
/*
    ...
    ==========================================================================
*/
INT Set_AP_PMKCachePeriod_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
...
/*
    ...
    ==========================================================================
*/
INT Set_AP_DefaultKeyID_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	ULONG KeyIdx;
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF(ra%d) Set_DefaultKeyID_Proc::(DefaultKeyID(0~3)=%d)\n", apidx,
...
/*
    ...
    ==========================================================================
*/
INT Set_AP_Key1_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj;
	UCHAR apidx;
	...
	if (retVal == TRUE) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("IF(ra%d) Set_Key1_Proc::(Key1=%s) success!\n", apidx, arg));
	}
...
/*
    ...
    ==========================================================================
*/
INT Set_AP_Key2_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj;
	UCHAR apidx;
	CIPHER_KEY *pSharedKey;
	...
	if (retVal == TRUE) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("IF(ra%d) Set_Key2_Proc::(Key2=%s) success!\n", apidx, arg));
	}
...
/*
    ...
    ==========================================================================
*/
INT Set_AP_Key3_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj;
	UCHAR apidx;
	CIPHER_KEY *pSharedKey;
	...
	if (retVal == TRUE) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("IF(ra%d) Set_Key3_Proc::(Key3=%s) success!\n", apidx, arg));
	}
...
/*
    ...
    ==========================================================================
*/
INT Set_AP_Key4_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (retVal == TRUE) {
		...
		DBGPRINT(RT_DEBUG_TRACE,
			 ("IF(ra%d) Set_Key4_Proc::(Key4=%s) success!\n", apidx, arg));
	}
...
/*
    ...
    ==========================================================================
*/
INT Set_AccessPolicy_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF(ra%d) Set_AccessPolicy_Proc::(AccessPolicy=%ld)\n", pObj->ioctl_if,
		  pAd->ApCfg.MBSSID[pObj->ioctl_if].AccessControlList.Policy));
...
/*
    ...
    ==========================================================================
*/
INT Set_ACLAddEntry_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
...
/*	RT_802_11_ACL			acl; */
	...
	NdisMoveMemory(pacl, &pAd->ApCfg.MBSSID[pObj->ioctl_if].AccessControlList,
		       sizeof(RT_802_11_ACL));
	...
	NdisMoveMemory(&pAd->ApCfg.MBSSID[pObj->ioctl_if].AccessControlList, pacl,
		       sizeof(RT_802_11_ACL));
	...
	for (i = 0; i < pAd->ApCfg.MBSSID[pObj->ioctl_if].AccessControlList.Num; i++) {
		...
		for (j = 0; j < MAC_ADDR_LEN; j++)
			DBGPRINT(RT_DEBUG_TRACE, ("%02X ",
			       pAd->ApCfg.MBSSID[pObj->ioctl_if].AccessControlList.Entry[i].
			       Addr[j]));
		DBGPRINT(RT_DEBUG_TRACE, ("\n"));
	}
#endif /* endif */
...
/*
    ...
    ==========================================================================
*/
INT Set_ACLDelEntry_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UCHAR macAddr[MAC_ADDR_LEN];
	...
	for (j = 0; j < acl.Num; j++) {
		if (memcmp(acl.Entry[j].Addr, &nullAddr, MAC_ADDR_LEN) == 0) {
			...
			NdisMoveMemory(&
				       (pAd->ApCfg.MBSSID[pObj->ioctl_if].AccessControlList.
					Entry[i++]), acl.Entry[j].Addr, MAC_ADDR_LEN);
		}
	}
...
/*
    ...
    ==========================================================================
*/
INT Set_ACLShowAll_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	RT_802_11_ACL acl;
	...
	DBGPRINT(RT_DEBUG_WARN, ("=============== Access Control Policy ===============\n"));
	DBGPRINT(RT_DEBUG_WARN, ("Policy is %ld : ", acl.Policy));
	DBGPRINT(RT_DEBUG_WARN, ("%s\n", pACL_PolicyMessage[acl.Policy]));

	/* Dump the entry in the list one by one */
	DBGPRINT(RT_DEBUG_WARN, ("===============  Access Control List  ===============\n"));
	for (i = 0; i < acl.Num; i++) {
		DBGPRINT(RT_DEBUG_WARN, ("Entry #%02d: ", i + 1));
		for (j = 0; j < MAC_ADDR_LEN; j++)
			DBGPRINT(RT_DEBUG_WARN, ("%02X ", acl.Entry[i].Addr[j]));
		DBGPRINT(RT_DEBUG_WARN, ("\n"));
	}
...
/*
    ...
    ==========================================================================
*/
INT Set_ACLClearAll_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
/*	RT_802_11_ACL			acl; */
	RT_802_11_ACL *pacl = NULL;
	...
	NdisZeroMemory(&(pAd->ApCfg.MBSSID[pObj->ioctl_if].AccessControlList),
		       sizeof(RT_802_11_ACL));
	NdisMoveMemory(&(pAd->ApCfg.MBSSID[pObj->ioctl_if].AccessControlList), pacl,
		       sizeof(RT_802_11_ACL));
...
#ifdef DBG
static void _rtmp_hexdump(int level, const char *title, const UINT8 *buf, size_t len, int show)
{
	size_t i;

	if (level < RTDebugLevel)
		return;
	DBGPRINT(RT_DEBUG_TRACE, ("%s - hexdump(len=%lu):", title, (unsigned long)len));
	if (show) {
		for (i = 0; i < len; i++)
			DBGPRINT(RT_DEBUG_TRACE, (" %02x", buf[i]));
	} else {
		DBGPRINT(RT_DEBUG_TRACE, (" [REMOVED]"));
	}
	DBGPRINT(RT_DEBUG_TRACE, ("\n"));
}
...
/*
    ...
    ==========================================================================
*/
INT Set_AP_WPAPSK_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
	INT retval;
	...
	pMBSSStruct = &pAd->ApCfg.MBSSID[apidx];
	retval =
	    RT_CfgSetWPAPSKKey(pAd, arg, strlen(arg), (PUCHAR) pMBSSStruct->Ssid,
			       pMBSSStruct->SsidLen, pMBSSStruct->PMK);
	if (retval == FALSE)
		return FALSE;
...
INT Set_RadioOn_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UCHAR radio;
...
INT Set_AutoChannelSel_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (strcmp(arg, "0") == 0)
		...
		pAd->ApCfg.AutoChannelAlg = ChannelAlgRandom;
	else {
...
INT Set_PartialScan_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("%s(): bPartialScanning = %u\n", __func__, pAd->ApCfg.bPartialScanning));
	return TRUE;
}
#endif /* AP_PARTIAL_SCAN_SUPPORT */
...
INT Set_AutoChannelSelCheckTime_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s(): ACSCheckTime=%u seconds(%u hours)\n",
				  __func__, pAd->ApCfg.ACSCheckTime, Hour));
	return TRUE;
}
#endif /* AP_SCAN_SUPPORT */
...
INT Show_StaCount_Proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("BackOff Slot      : %s slot time, BKOFF_SLOT_CFG(0x1104) = 0x%08x\n",
	       OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_SHORT_SLOT_INUSED) ? "short" : "long", RegValue));
...
INT Show_StaSecurityInfo_Proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
		wdev = &pAd->ApCfg.MBSSID[apidx].wdev;
		DBGPRINT(RT_DEBUG_OFF, (" BSS(%d) AuthMode(%d)=%s, WepStatus(%d)=%s, ",
		     apidx, wdev->AuthMode, GetAuthMode(wdev->AuthMode), wdev->WepStatus,
		     GetEncryptType(wdev->WepStatus)));
		DBGPRINT(RT_DEBUG_OFF, ("GroupWepStatus(%d)=%s, WPAMixPairCipher(0x%02X)\n",
			wdev->GroupKeyWepStatus,
			GetEncryptType(wdev->GroupKeyWepStatus), wdev->WpaMixPairCipher));
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		if (pEntry && IS_ENTRY_CLIENT(pEntry) && pEntry->Sst == SST_ASSOC) {
			...
			DBGPRINT(RT_DEBUG_OFF, ("%-15s", GetAuthMode(pEntry->AuthMode)));
			DBGPRINT(RT_DEBUG_OFF, ("%-12s", GetEncryptType(pEntry->WepStatus)));
			DBGPRINT(RT_DEBUG_OFF, ("\n"));
		}
	}
	...
	for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
		...
		for (j = 0; j < NUM_OF_TID; j++) {
			if (pEntry->BARecWcidArray[j] != 0) {
				...
				DBGPRINT(RT_DEBUG_OFF, ("TID=%d, BAWinSize=%d, LastIndSeq=%d, ReorderingPkts=%d\n",
		...
		for (j = 0; j < NUM_OF_TID; j++) {
			if (pEntry->BAOriWcidArray[j] != 0) {
				...
				DBGPRINT(RT_DEBUG_OFF, ("TID=%d, BAWinSize=%d, StartSeq=%d, CurTxSeq=%d\n", j,
				       pOriBAEntry->BAWinSize, pOriBAEntry->Sequence,
				       pEntry->TxSeq[j]));
			}
		}
		DBGPRINT(RT_DEBUG_OFF, ("\n\n"));
...
INT Show_Sat_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	/* Some 11n statistics variables */
	DBGPRINT(RT_DEBUG_OFF, ("TransmittedAMSDUCount = %ld\n",
	       (ULONG) pAd->RalinkCounters.TransmittedAMSDUCount.u.LowPart));
	DBGPRINT(RT_DEBUG_OFF, ("TransmittedOctetsInAMSDU = %ld\n",
	       (ULONG) pAd->RalinkCounters.TransmittedOctetsInAMSDU.QuadPart));
	DBGPRINT(RT_DEBUG_OFF, ("ReceivedAMSDUCount = %ld\n",
	       (ULONG) pAd->RalinkCounters.ReceivedAMSDUCount.u.LowPart));
	DBGPRINT(RT_DEBUG_OFF, ("ReceivedOctesInAMSDUCount = %ld\n",
...

	{
		...
		for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
			...
			if (IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC)) {
				...
				for (j = maxMcs; j >= 0; j--) {
					if ((pEntry->TXMCSExpected[j] != 0)
						|| (pEntry->TXMCSFailed[j] != 0)) {
						DBGPRINT(RT_DEBUG_OFF, ("MCS[%02d]: Expected %u, \n",
							j, pEntry->TXMCSExpected[j]));
						DBGPRINT(RT_DEBUG_OFF, ("Successful %u (%d%%), Failed %u\n",
							pEntry->TXMCSSuccessful[j],
							pEntry->TXMCSExpected[j] ? (100 *
							pEntry->TXMCSSuccessful[j])
						...
						for (k = maxMcs; k >= 0; k--) {
							if (pEntry->TXMCSAutoFallBack[j][k] != 0) {
								DBGPRINT(RT_DEBUG_OFF, ("AutoMCS[%02d]: %u (%d%%)\n",
									k, pEntry->TXMCSAutoFallBack[j]
									[k],
									(100 *
									pEntry->TXMCSAutoFallBack[j]
									[k]) /
	...
	for (apidx = 0; apidx < pAd->ApCfg.BssidNum; apidx++) {
		...
		for (srv_idx = 0; srv_idx < pMbss->radius_srv_num; srv_idx++) {
			if (pMbss->radius_srv_info[srv_idx].radius_ip != 0) {
				p1xBssInfo->radius_srv_info[srv_idx].radius_ip =
				    pMbss->radius_srv_info[srv_idx].radius_ip;
				p1xBssInfo->radius_srv_info[srv_idx].radius_port =
				    pMbss->radius_srv_info[srv_idx].radius_port;
				p1xBssInfo->radius_srv_info[srv_idx].radius_key_len =
				    pMbss->radius_srv_info[srv_idx].radius_key_len;
				if (pMbss->radius_srv_info[srv_idx].radius_key_len > 0) {
					NdisMoveMemory(p1xBssInfo->radius_srv_info[srv_idx].
						       radius_key,
						       pMbss->radius_srv_info[srv_idx].radius_key,
						       pMbss->radius_srv_info[srv_idx].
						       radius_key_len);
				}
			}
		}
		...
		if (p1xBssInfo->ieee8021xWEP) {
			...
			if (KeyLen == 5 || KeyLen == 13) {
				...
				NdisMoveMemory(p1xBssInfo->key_material,
					       pAd->SharedKey[apidx][keyidx].Key, KeyLen);
			}
		}
		...
		if (pAd->ApCfg.EAPifname_len[apidx] > 0) {
			...
			NdisMoveMemory(pConf->EAPifname[apidx], pAd->ApCfg.EAPifname[apidx],
				       pAd->ApCfg.EAPifname_len[apidx]);
		}

		/* get PreAuthifname */
		if (pAd->ApCfg.PreAuthifname_len[apidx] > 0) {
			pConf->PreAuthifname_len[apidx] = pAd->ApCfg.PreAuthifname_len[apidx];
			NdisMoveMemory(pConf->PreAuthifname[apidx], pAd->ApCfg.PreAuthifname[apidx],
				       pAd->ApCfg.PreAuthifname_len[apidx]);
		}
	...
	if (pAd->ApCfg.MBSSID[apidx].wdev.AuthMode >= Ndis802_11AuthModeWPA) {
		if ((pKey->KeyLength == 32) || (pKey->KeyLength == 64)) {
			...
			if (pEntry != NULL) {
				...
				NdisMoveMemory(pAd->ApCfg.MBSSID[apidx].PMK,
					       pKey->KeyMaterial + k_offset, 32);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("RTMPIoctlAddWPAKey-IF(ra%d) : ",
					  apidx));
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Add PMK=%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x....\n",
					  pAd->ApCfg.MBSSID[apidx].PMK[0],
					  pAd->ApCfg.MBSSID[apidx].PMK[1],
					  pAd->ApCfg.MBSSID[apidx].PMK[2],
					  pAd->ApCfg.MBSSID[apidx].PMK[3],
					  pAd->ApCfg.MBSSID[apidx].PMK[4],
					  pAd->ApCfg.MBSSID[apidx].PMK[5],
					  pAd->ApCfg.MBSSID[apidx].PMK[6],
					  pAd->ApCfg.MBSSID[apidx].PMK[7]));
			}
		}
	} else {		/* Old WEP stuff */
		...
		if (KeyIdx < 4) {
			...
			if (pKey->KeyIndex & 0x80000000) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("RTMPIoctlAddWPAKey-IF(ra%d) : Set Group Key\n", apidx));
				...
				if (pKey->KeyLength > 16) {
					DBGPRINT(RT_DEBUG_TRACE,
						 ("RTMPIoctlAddWPAKey-IF(ra%d) : Key length too long %d\n",
				...
				/* Set Group key material to Asic */
				AsicAddSharedKeyEntry(pAd, apidx, (UINT8) KeyIdx,
						      &pAd->SharedKey[apidx][KeyIdx]);
				...
				if (pEntry) {
					DBGPRINT(RT_DEBUG_TRACE,
						 ("RTMPIoctlAddWPAKey-IF(ra%d) : Set Pair-wise Key\n",
						  apidx));
...
VOID RTMPIoctlAddPMKIDCache(IN PRTMP_ADAPTER pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	if (pAd->ApCfg.MBSSID[apidx].wdev.AuthMode >= Ndis802_11AuthModeWPA2) {
		if (pKey->KeyLength == 32) {
			...
			NdisMoveMemory(&PMK_key[8], pAd->ApCfg.MBSSID[apidx].wdev.bssid,
				       MAC_ADDR_LEN);
			NdisMoveMemory(&PMK_key[14], pKey->addr, MAC_ADDR_LEN);
			RT_HMAC_SHA1(pKey->KeyMaterial, PMK_LEN, PMK_key, 20, digest,
				     SHA1_DIGEST_SIZE);
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("WPA2(pre-auth):(%02x:%02x:%02x:%02x:%02x:%02x)\n",
				  pKey->addr[0], pKey->addr[1], pKey->addr[2], pKey->addr[3],
				  pKey->addr[4], pKey->addr[5]));
			DBGPRINT(RT_DEBUG_TRACE,
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("PMK =%02x:%02x:%02x:%02x-%02x:%02x:%02x:%02x\n",
				  pKey->KeyMaterial[0], pKey->KeyMaterial[1], pKey->KeyMaterial[2],
				  pKey->KeyMaterial[3], pKey->KeyMaterial[4], pKey->KeyMaterial[5],
				  pKey->KeyMaterial[6], pKey->KeyMaterial[7]));
		} else
			DBGPRINT(RT_DEBUG_ERROR,
...
VOID RTMPIoctlStaticWepCopy(IN PRTMP_ADAPTER pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	if (wrq->u.data.length != sizeof(MacAddr)) {
		DBGPRINT(RT_DEBUG_ERROR,
			 ("RTMPIoctlStaticWepCopy: the length isn't match (%d)\n",
			  wrq->u.data.length));
		return;
...
#ifdef RT65xx
static VOID RTMPAPIoctlBBP32(RTMP_ADAPTER *pAd, RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	PSTRING this_char;
	PSTRING value;
	UINT32 regBBP = 0;
	...
	bAllowDump =
	    ((wrq->u.data.flags & RTPRIV_IOCTL_FLAG_NODUMPMSG) ==
	     RTPRIV_IOCTL_FLAG_NODUMPMSG) ? FALSE : TRUE;
	bCopyMsg =
	    ((wrq->u.data.flags & RTPRIV_IOCTL_FLAG_NOSPACE) ==
	     RTPRIV_IOCTL_FLAG_NOSPACE) ? FALSE : TRUE;
	...
	if (argLen > 0) {
		...
		while ((this_char = strsep((char **)&ptr, ",")) != NULL) {
			...
			if (!value || !*value) {
				...
				if ((sscanf(this_char, "%x", &(bbpId)) == 1)
				    && (sscanf(value, "%lx", &(bbpValue)) == 1)) {
					DBGPRINT(RT_DEBUG_TRACE,
						 ("bbpID=%04x, value=0x%lx\n", bbpId, bbpValue));
					if ((bbpId <= 0x2fff) && (bbpId >= 0x2000)) {
						/* according to Andy, Gary, David require. */
				...
				if (!value || !*value) {	/*Read */
					if (sscanf(this_char, "%d", &(bbpId)) == 1) {
						if (bbpId <= pAdapter->chipCap.MaxNumOfBbpId) {
							 ...
							 *  For RT2870 ATE mode,
							 *  ATE_BBP_IO_WRITE8(/READ8)_BY_REG_ID are redefined.
							 */
							if (ATE_ON(pAdapter)) {
								ATE_BBP_IO_READ8_BY_REG_ID(pAdapter,
											   bbpId,
											   &regBBP);
					...
					if ((sscanf(this_char, "%d", &(bbpId)) == 1)
						...
						if (bbpId <= pAdapter->chipCap.MaxNumOfBbpId) {
							...
							{
								/* according to Andy, Gary, David require. */
								/* the command bbp shall read/write
								 *BBP register directly for dubug.
								 */
								/* BBP_IO_READ8_BY_REG_ID(pAdapter, bbpId, &regBBP); */
								RTMP_BBP_IO_READ8_BY_REG_ID
								    (pAdapter, bbpId, &regBBP);
								BBP_IO_WRITE8_BY_REG_ID(pAdapter,
											(UCHAR)
											bbpId,
											(UCHAR)
											bbpValue);
	...
	if (bIsPrintAllBBP) {
		...
		for (bbpId = 0; bbpId <= pAdapter->chipCap.MaxNumOfBbpId; bbpId++) {
...
#endif /* RALINK_ATE */
			{
				...
				sprintf(msg + strlen(msg), "R%02d[0x%02X]:%02X    ", bbpId, bbpId,
					regBBP);
				if (bbpId % 5 == 4)
					sprintf(msg + strlen(msg), "\n");
			}
...
Arguments:
    ...
    ==========================================================================
*/
VOID RTMPAPIoctlMAC(IN RTMP_ADAPTER *pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
		...
		while ((this_char = strsep((char **)&ptr, ",")) != NULL) {
			...
			if (!value || !*value) {	/*Read */
...

				{
					...
					AtoH(temp2, temp, 4);
					macValue =
					    *temp * 256 * 256 * 256 + temp[1] * 256 * 256 +
					    temp[2] * 256 + temp[3];
					...
					if (macAddr == (HW_DEBUG_SETTING_BASE + 4)) {
						...
						if (macValue & 0x000000ff) {
							...
							/* todo
							* bbp_set_agc(pAd, (0x26 + GET_LNA_GAIN(pAd)), RX_CHAIN_ALL);
							*/
							if (!bFromUI)
								DBGPRINT(RT_DEBUG_OFF,
									 ("turn off R66 tuning, restore to 0x%02x\n",
									  R66));
						}
						goto done;
					}
...
#ifdef MT_RF
static VOID RTMPAPIoctlRF_mt(IN PRTMP_ADAPTER pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	UINT regRF = 0;
			...
			if (!value || !*value) {	/*Read */
				if (sscanf(this_char, "%d", &(rfId)) == 1) {
					if (rfId <= pAdapter->chipCap.MaxNumOfRfId) {
#ifdef RALINK_ATE
						if (ATE_ON(pAdapter)) {
								RT30xxReadRFRegister(pAdapter, rfId,
										     &regRF);
						...
						/* Invalid parameters, so default printk all RF */
						bIsPrintAllRF = TRUE;
						break;
					}
				} else {
				...
				if ((sscanf(this_char, "%d", &(rfId)) == 1)
					...
					if (rfId <= pAdapter->chipCap.MaxNumOfRfId) {
						...
						if (ATE_ON(pAdapter)) {
							{
								RT30xxReadRFRegister(pAdapter, rfId,
										     &regRF);
								RT30xxWriteRFRegister(pAdapter,
										      (UCHAR) rfId,
										      (UCHAR)
										      rfValue);
							}
						...
						{
							/* according to Andy, Gary, David require. */
							/* the command RF shall read/write RF register
							 * directly for dubug.
							*/
							/*
							* BBP_IO_READ8_BY_REG_ID(pAdapter, bbpId, &regBBP);
							*/
							/*BBP_IO_WRITE8_BY_REG_ID(pAdapter, (UCHAR)bbpId,
							* (UCHAR) bbpValue);
							*/
							{
								RT30xxReadRFRegister(pAdapter, rfId,
					...
					} else {	/* Invalid parameters, so default printk all RF */
						bIsPrintAllRF = TRUE;
						break;
					}
				} else {	/* Invalid parameters, so default printk all RF */
	...
	if (bIsPrintAllRF) {
		...
		for (rfId = 0; rfId <= maxRFIdx; rfId++) {
...
#endif /* RALINK_ATE */
			{
				...
				{
					RT30xxReadRFRegister(pAdapter, rfId, &regRF);
					sprintf(msg + strlen(msg), "R%02d[0x%02X]:%02X    ", rfId,
						rfId * 2, regRF);
					if (rfId % 5 == 4)
						sprintf(msg + strlen(msg), "\n");
				}
			...
			if (!value || !*value) {	/*Read */
				...
				if (strlen(this_char) == 4) {
					...
					if (eepAddr < 0xFFFF) {
						...
						sprintf(msg + strlen(msg), "[0x%04X]:0x%04X  ",
							eepAddr, eepValue);
					} else {	/*Invalid parameters, so default printk all bbp */
						break;
					}
				}
			} else {	/*Write */
...
/*	msg = (PSTRING)kmalloc(sizeof(CHAR)*(2048), MEM_ALLOC_FLAG); */
	...
	if (ATE_ON(pAd))
		per =
		    ...
		    (pAd->WlanCounters.FCSErrorCount.u.LowPart + rxCount);
	else
#endif /* RALINK_QA */
		per =
		    pAd->WlanCounters.ReceivedFragmentCount.u.LowPart ==
...
#ifdef RT65xx
					if (IS_RT65XX(pAd)) {
						ULONG TxTotalCnt, TxSuccess, TxRetransmit,
						    TxFailCount, TxErrorRatio = 0;
		...
		for (idx = 0; idx < pAd->ApCfg.BssidNum; idx++) {
			...
			if (pAd->ApCfg.MBSSID[idx].WscControl.WscEnrolleePinCodeLen == 8)
				sprintf(msg + strlen(msg),
					...
					pAd->ApCfg.MBSSID[idx].WscControl.WscEnrolleePinCode);
			else
				sprintf(msg + strlen(msg),
	...
	for (idx = 0; idx < pAd->ApCfg.ApCliTab[0].WscControl.WscProfile.ProfileCnt; idx++) {
		PWSC_CREDENTIAL pCredential =
		    &pAd->ApCfg.ApCliTab[0].WscControl.WscProfile.Profile[idx];
 ...
 * Therefore, we can parse them out in cfg_manager. --Trey */
	...
	if (pAd->CommonCfg.DebugFlags & DBF_SHOW_BF_STATS) {
		for (i = 0; i < MAX_LEN_OF_MAC_TABLE; i++) {
			...
			totalSuccess =
			    pCnt->TxSuccessCount + pCnt->ETxSuccessCount + pCnt->ITxSuccessCount;

			DBGPRINT(RT_DEBUG_OFF,
				 ("MacTable[%d]     Success    Retry/PER    Fail/PLR\n", i));
			if (totalTx == 0) {
			...
			if (totalNBF != 0) {
				DBGPRINT(RT_DEBUG_OFF,
					 ("   NonBF (%3lu%%): %7lu  %7lu (%2lu%%) %5lu (%1lu%%)\n",
					  100 * totalNBF / totalTx, pCnt->TxSuccessCount,
					  pCnt->TxRetryCount,
					  100 * pCnt->TxRetryCount / (pCnt->TxSuccessCount +
								      pCnt->TxRetryCount),
			...
			if (totalEBF != 0) {
				DBGPRINT(RT_DEBUG_OFF,
					 ("   ETxBF (%3lu%%): %7lu  %7lu (%2lu%%) %5lu (%1lu%%)\n",
					  100 * totalEBF / totalTx, pCnt->ETxSuccessCount,
					  pCnt->ETxRetryCount,
					  100 * pCnt->ETxRetryCount / (pCnt->ETxSuccessCount +
								       pCnt->ETxRetryCount),
			...
			if (totalIBF != 0) {
				DBGPRINT(RT_DEBUG_OFF,
					 ("   ITxBF (%3lu%%): %7lu  %7lu (%2lu%%) %5lu (%1lu%%)\n",
					  100 * totalIBF / totalTx, pCnt->ITxSuccessCount,
					  pCnt->ITxRetryCount,
					  100 * pCnt->ITxRetryCount / (pCnt->ITxSuccessCount +
								       pCnt->ITxRetryCount),
					  pCnt->ITxFailCount, 100 * pCnt->ITxFailCount / totalIBF));
			}

			DBGPRINT(RT_DEBUG_OFF,
				 ("   Total         %7lu  %7lu (%2lu%%) %5lu (%1lu%%)\n",
				  totalSuccess, totalRetry,
				  100 * totalRetry / (totalSuccess + totalRetry),
				  pCnt->TxFailCount + pCnt->ETxFailCount + pCnt->ITxFailCount,
				  100 * (pCnt->TxFailCount + pCnt->ETxFailCount +
					 pCnt->ITxFailCount) / totalTx));
		}
	}
#endif /* DBG_CTRL_SUPPORT */
...
VOID RTMPIoctlQueryBaTable(IN PRTMP_ADAPTER pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	for (i = 0; ((i < MAX_LEN_OF_MAC_TABLE) && (TotalEntry > 0)); i++) {
		...
		if (IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC) && (pEntry->TXBAbitmap)
			...
			for (j = 0; j < 8; j++) {
				if (pEntry->BAOriWcidArray[j] != 0)
					BAT->BAOriEntry[index].BufSize[j] =
	...
	for (i = 0; ((i < MAX_LEN_OF_MAC_TABLE) && (TotalEntry > 0)); i++) {
		...
		if (IS_ENTRY_CLIENT(pEntry) && (pEntry->Sst == SST_ASSOC) && (pEntry->RXBAbitmap)
			...
			for (j = 0; j < 8; j++) {
				if (pEntry->BARecWcidArray[j] != 0)
					BAT->BARecEntry[index].BufSize[j] =
...
INT Set_ApCli_Ssid_Proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	if (strlen(arg) <= MAX_LEN_OF_SSID) {
		...
		if (((wdev->AuthMode == Ndis802_11AuthModeWPAPSK) ||
		     (wdev->AuthMode == Ndis802_11AuthModeWPA2PSK)) && apcli_entry->PSKLen > 0) {
			...
			RT_CfgSetWPAPSKKey(pAd, (PSTRING) PskKey,
					   ...
					   apcli_entry->CfgSsidLen, apcli_entry->PMK);
		}
...
INT Set_ApCli_AuthMode_Proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF(apcli%d) Set_ApCli_AuthMode_Proc::(AuthMode=%d)\n", ifIndex, wdev->AuthMode));
	return TRUE;
}
...
/*
    ...
    ==========================================================================
*/
INT Set_ApCli_EncrypType_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR ifIndex;
	PAPCLI_STRUCT pApCliEntry = NULL;
	...
	RTMPMakeRSNIE(pAd, wdev->AuthMode, wdev->WepStatus, (ifIndex + MIN_NET_DEVICE_FOR_APCLI));
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF(apcli%d) Set_ApCli_EncrypType_Proc::(EncrypType=%d)\n", ifIndex,
		  wdev->WepStatus));
...
INT Set_ApCli_DefaultKeyID_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF(apcli%d) Set_DefaultKeyID_Proc::(DefaultKeyID(0~3)=%d)\n", pObj->ioctl_if,
		  wdev->DefaultKeyId));
...
INT Set_ApCli_WPAPSK_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	retval =
	    RT_CfgSetWPAPSKKey(pAd, arg, strlen(arg), (PUCHAR) pApCliEntry->CfgSsid,
			       pApCliEntry->CfgSsidLen, pApCliEntry->PMK);
	if (retval == FALSE)
		return FALSE;
...
INT Set_ApCli_Key1_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (retVal == TRUE)
		DBGPRINT(RT_DEBUG_TRACE,
			 ("IF(apcli%d) Set_ApCli_Key1_Proc::(Key1=%s) success!\n", ifIndex, arg));
...
INT Set_ApCli_Key2_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (retVal == TRUE)
		DBGPRINT(RT_DEBUG_TRACE,
			 ("IF(apcli%d) Set_ApCli_Key2_Proc::(Key2=%s) success!\n", ifIndex, arg));
...
INT Set_ApCli_Key3_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (retVal == TRUE)
		DBGPRINT(RT_DEBUG_TRACE,
			 ("IF(apcli%d) Set_ApCli_Key3_Proc::(Key3=%s) success!\n", ifIndex, arg));
...
INT Set_ApCli_Key4_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (retVal == TRUE)
		DBGPRINT(RT_DEBUG_TRACE,
			 ("IF(apcli%d) Set_ApCli_Key4_Proc::(Key4=%s) success!\n", ifIndex, arg));
...
INT Set_ApCli_TxMcs_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	wdev->DesiredTransmitSetting.field.MCS = RT_CfgSetTxMCSProc(arg, &wdev->bAutoTxRateSwitch);

	if (wdev->DesiredTransmitSetting.field.MCS == MCS_AUTO) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("I/F(apcli%d) Set_ApCli_TxMcs_Proc = AUTO\n", pObj->ioctl_if));
	} else {
...
INT Set_ApCli_IEEE8021X_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF(ra%d) Set_ApCli_IEEE8021X_Proc::(IEEE8021X=%d)\n", pObj->ioctl_if,
		  pApCliEntry->wdev.IEEE8021X));
...
INT Set_ApCli_AutoConnect_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("I/F(apcli%d) Set_ApCli_AutoConnect_Proc::(Len=%d,Ssid=%s)\n",
				  ifIndex, pApCfg->ApCliTab[ifIndex].CfgSsidLen,
				  pApCfg->ApCliTab[ifIndex].CfgSsid));
...
INT Set_ApCli_Cert_Enable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("I/F(apcli%d) Set_ApCli_Cert_Enable_Proc::(enable = %d)\n", ifIndex,
		  pAd->bApCliCertTest));
...
INT Set_ApCli_WMM_Enable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("I/F(apcli%d) Set_ApCli_WMM_Enable_Proc::(enable = %d)\n", ifIndex,
		  pApCliEntry->wdev.bWmmCapable));
...
#ifdef WSC_AP_SUPPORT
#ifdef CON_WPS
static INT WscPushConcurrentPBCAction(IN PRTMP_ADAPTER pAd,
				      IN PWSC_CTRL pWscControl, IN BOOLEAN bIsApCli)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
	INT idx;
	...
	if (bIsApCli) {
		...
		WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR, pWscControl->WscConfStatus, TRUE,
				    DEV_PASS_ID_PBC, pWscControl->WscConfigMethods, apidx, NULL, 0,
				    AP_MODE);
		APUpdateBeaconFrame(pAd, apidx);
...
INT Set_AP_WscConfMode_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if ((ConfModeIdx & WSC_ENROLLEE_PROXY_REGISTRAR) == WSC_DISABLE) {
		...
		if (bFromApCli) {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("IF(apcli%d) Set_WscConfMode_Proc:: WPS is disabled.\n", apidx));
		} else {
			DBGPRINT(RT_DEBUG_TRACE,
				 ("IF(ra%d) Set_WscConfMode_Proc:: WPS is disabled.\n", apidx));
		...
		if (bFromApCli) {
			if (ConfModeIdx == WSC_ENROLLEE) {
				...
				DBGPRINT(RT_DEBUG_TRACE,
					 ("IF(apcli%d) Set_WscConfMode_Proc:: ",
					  ConfModeIdx));
				DBGPRINT(RT_DEBUG_TRACE,
					 ("Ap Client only supports Enrollee mode.(ConfModeIdx=%d)\n",
					  ConfModeIdx));
				return FALSE;
			}
		} else
...
INT Set_AP_WscConfStatus_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if ((IsAPConfigured > 0) && (IsAPConfigured <= 2)) {
		...
		WscBuildBeaconIE(pAd, IsAPConfigured, IsSelectedRegistrar, WscMode, 0, apidx, NULL,
				 0, AP_MODE);
		WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR, IsAPConfigured,
				    IsSelectedRegistrar, WscMode, 0, apidx, NULL, 0, AP_MODE);
		APUpdateBeaconFrame(pAd, apidx);
	} else {
		DBGPRINT(RT_DEBUG_TRACE,
...
INT Set_AP_WscGetConf_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (bFromApCli) {
		...
		if (WscMode == DEV_PASS_ID_PIN) {
			...
			NdisMoveMemory(mac_addr, pAd->ApCfg.ApCliTab[apidx].wdev.if_addr,
				       MAC_ADDR_LEN);
		} else {
			pWscControl->WscSsid.SsidLength = 0;
			NdisZeroMemory(&pWscControl->WscSsid, sizeof(NDIS_802_11_SSID));
...
#endif /* APCLI_SUPPORT */
	{
		WscBuildBeaconIE(pAd, IsAPConfigured, TRUE, WscMode, pWscControl->WscConfigMethods,
				 apidx, NULL, 0, AP_MODE);
		WscBuildProbeRespIE(pAd, WSC_MSGTYPE_AP_WLAN_MGR, IsAPConfigured, TRUE, WscMode,
				    pWscControl->WscConfigMethods, apidx, NULL, 0, AP_MODE);
		APUpdateBeaconFrame(pAd, apidx);
	...
	if (!bFromApCli) {
		if (WscMode == DEV_PASS_ID_PIN) {
			...
			WscSendUPnPConfReqMsg(pAd, pWscControl->EntryIfIdx,
					      (PUCHAR) pAd->ApCfg.MBSSID[pWscControl->EntryIfIdx].
					      Ssid, pAd->ApCfg.MBSSID[apidx].wdev.bssid, 3, 0,
					      AP_MODE);
		} else {
			RTMP_SEM_LOCK(&pWscControl->WscPeerListSemLock);
			WscClearPeerList(&pWscControl->WscPeerList);
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF(%02X:%02X:%02X:%02X:%02X:%02X) ",
		  PRINT_MAC(mac_addr)));
	DBGPRINT(RT_DEBUG_TRACE,
		 ("Set_WscGetConf_Proc trigger WSC state machine, wscGetConfMode=%d\n",
		  wscGetConfMode));
...
/*
    ...
    ==========================================================================
*/
VOID RTMPIoctlWscProfile(IN PRTMP_ADAPTER pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	if (Profile.WscEncrypType == 2) {
		Profile.DefaultKeyIdx = wdev->DefaultKeyId + 1;
		{
			...
			Profile.WscWPAKey[(pAd->SharedKey[apidx][wdev->DefaultKeyId].KeyLen) * 2] =
			    '\0';
		}
...
/*	msg = (PSTRING)kmalloc(sizeof(CHAR)*(2048), MEM_ALLOC_FLAG); */
	...
	if (Profile.WscEncrypType == 1) {
		...
		sprintf(msg + strlen(msg), "%-12s%-33s%-12s%-12s%-13s%-26s\n", "Configured", "SSID",
			"AuthMode", "EncrypType", "DefaultKeyID", "Key");
	} else {
		sprintf(msg + strlen(msg), "%-12s%-33s%-12s%-12s%-64s\n", "Configured", "SSID",
			"AuthMode", "EncrypType", "Key");
	}
...
/*
    ...
    ==========================================================================
*/
VOID RTMPAR9IoctlWscProfile(IN PRTMP_ADAPTER pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	WSC_CONFIGURED_VALUE Profile;
	PSTRING msg;
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
...
VOID RTMPIoctlWscPINCode(IN PRTMP_ADAPTER pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	PSTRING msg;
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
	...
	if (pAd->ApCfg.MBSSID[apidx].WscControl.WscEnrolleePinCode) {
		if (pAd->ApCfg.MBSSID[apidx].WscControl.WscEnrolleePinCodeLen == 8)
			sprintf((PSTRING) tempPIN, "%08u",
				pAd->ApCfg.MBSSID[apidx].WscControl.WscEnrolleePinCode);
		else
			sprintf((PSTRING) tempPIN, "%04u",
				pAd->ApCfg.MBSSID[apidx].WscControl.WscEnrolleePinCode);
		sprintf(msg, "%s%s\n", msg, tempPIN);
	}
	wrq->u.data.length = strlen(msg);
...
VOID RTMPIoctlWscStatus(IN PRTMP_ADAPTER pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	PSTRING msg;
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
...
VOID RTMPIoctlGetWscDynInfo(IN PRTMP_ADAPTER pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	char *msg;
	PMULTISSID_STRUCT pMbss;
...
VOID RTMPIoctlGetWscRegsDynInfo(IN PRTMP_ADAPTER pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	char *msg;
	PMULTISSID_STRUCT pMbss;
	INT apidx;
				      ...
				      IN PSTRING pData, IN USHORT Length, IN PWSC_CTRL pWscControl)
{
	...
	/* We have to look for WSC_IE_MSG_TYPE to classify M2 ~ M8,
	*the remain size must large than 4
	*/
	while (Length > 4) {
		WSC_IE TLV_Recv;
...
UCHAR WscRxMsgTypeFromUpnp(IN PRTMP_ADAPTER pAdapter, IN PSTRING pData, IN USHORT Length)
{
	...
	{			/* Eap-Esp(Messages) */
		...
		/* Not Wsc Start, We have to look for WSC_IE_MSG_TYPE to classify M2 ~ M8
		*, the remain size must large than 4
		*/
		while (Length > 4) {
...
/*
	...
	==========================================================================
*/
INT Set_AP_WscSecurityMode_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
...
INT Set_WscUUID_STR_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (strlen(arg) == (UUID_LEN_STR - 1)) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Set_WscUUID_E_Proc[%d]::(arg=%s)\n", pObj->ioctl_if, arg));
		pAd->ApCfg.MBSSID[pObj->ioctl_if].WscControl.Wsc_Uuid_Str[UUID_LEN_STR - 1] = 0;
		NdisMoveMemory(&pAd->ApCfg.MBSSID[pObj->ioctl_if].WscControl.Wsc_Uuid_Str[0], arg,
			       strlen(arg));
		return TRUE;
...
INT Set_WscUUID_HEX_E_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (strlen(arg) == (UUID_LEN_HEX * 2)) {
		DBGPRINT(RT_DEBUG_TRACE,
			 ("Set_WscUUID_HEX_E_Proc[%d]::(arg=%s)\n", pObj->ioctl_if, arg));
		AtoH(arg, &pAd->ApCfg.MBSSID[pObj->ioctl_if].WscControl.Wsc_Uuid_E[0],
		     UUID_LEN_HEX);
		hex_dump("Set_WscUUID_HEX_E_Proc OK:",
			 &pAd->ApCfg.MBSSID[pObj->ioctl_if].WscControl.Wsc_Uuid_E[0], UUID_LEN_HEX);
		return TRUE;
...
INT Set_WscSetupLock_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (bEnable == 0) {
		...
		WscBuildBeaconIE(pAd,
				 ...
				 FALSE, 0, 0, pObj->ioctl_if, NULL, 0, AP_MODE);
...
INT Set_WscV2Support_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Set_WscV2Support_Proc::(bEnableWpsV2=%d)\n",
				  pAd->ApCfg.MBSSID[pObj->ioctl_if].WscControl.WscV2Info.
				  bEnableWpsV2));
	return TRUE;
}
...
INT Set_WscSetupLockTime_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("Set_WscSetupLockTime_Proc::(SetupLockTime=%d)\n", SetupLockTime));
...
INT Set_DisConnectAllSta_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (value == 2) {
		...
		for (i = 1; i < MAX_LEN_OF_MAC_TABLE; i++) {
			...
			if (IS_ENTRY_CLIENT(pEntry)) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("[PMF]%s: MacTableDeleteEntry %x:%x:%x:%x:%x:%x\n",
...
/*
    ...
    ==========================================================================
*/
INT Set_IEEE8021X_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	ULONG ieee8021x;
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	struct wifi_dev *wdev;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF(ra%d) Set_IEEE8021X_Proc::(IEEE8021X=%d)\n", pObj->ioctl_if,
		  wdev->IEEE8021X));
...
/*
    ...
    ==========================================================================
*/
INT Set_PreAuth_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	ULONG PreAuth;
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	...
	DBGPRINT(RT_DEBUG_TRACE,
		 ("IF(ra%d) Set_PreAuth_Proc::(PreAuth=%d)\n", pObj->ioctl_if,
		  pAd->ApCfg.MBSSID[pObj->ioctl_if].PreAuth));

	return TRUE;
}

INT Set_OwnIPAddr_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UINT32 ip_addr;
...
INT Set_EAPIfName_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT i;
	PSTRING macptr;

	for (i = 0, macptr = rstrtok(arg, ";"); (macptr && i < MAX_MBSSID_NUM(pAd));
...
INT Set_PreAuthIfName_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	INT i;
	PSTRING macptr;
...
INT Set_RADIUS_Server_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	for (count = 0, macptr = rstrtok(arg, ";"); (macptr && count < MAX_RADIUS_SRV_NUM);
	     macptr = rstrtok(NULL, ";"), count++) {
		if (rtinet_aton(macptr, &ip_addr)) {
			PRADIUS_SRV_INFO pSrvInfo =
			    &pAd->ApCfg.MBSSID[apidx].radius_srv_info[srv_cnt];

			pSrvInfo->radius_ip = ip_addr;
			srv_cnt++;
			DBGPRINT(RT_DEBUG_TRACE, ("IF(ra%d), radius_ip(seq-%d)=%s(%x)\n",
						  apidx, srv_cnt, macptr, pSrvInfo->radius_ip));
		}
	}
	...
	for (count = 0, macptr = rstrtok(arg, ";"); (macptr && count < MAX_RADIUS_SRV_NUM);
		...
		if (strlen(macptr) > 0 && srv_cnt < pAd->ApCfg.MBSSID[apidx].radius_srv_num) {
			...
			DBGPRINT(RT_DEBUG_TRACE, ("IF(ra%d), radius_key(seq-%d)=%s, len=%d\n",
						  apidx, srv_cnt,
						  pSrvInfo->radius_key, pSrvInfo->radius_key_len));
		}
	}
	return TRUE;
}
...
INT Set_McastMcs(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (Mcs > 15)
		DBGPRINT(RT_DEBUG_ERROR, ("Mcs must in range of 0 to 15\n"));
	...
	while ((thisChar = strsep((char **)&arg, "-")) != NULL) {
		...
		case 0:
			if (ObssScanValue < 5 || ObssScanValue > 1000) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Invalid OBSSScanParam for Dot11OBssScanPassiveDwell(%d),",
					  ObssScanValue));
				DBGPRINT(RT_DEBUG_ERROR,
					 (" should in range 5~1000\n"));
			} else {
		...
		case 1:
			if (ObssScanValue < 10 || ObssScanValue > 1000) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Invalid OBSSScanParam for Dot11OBssScanActiveDwell(%d)",
					  ObssScanValue));
				DBGPRINT(RT_DEBUG_ERROR,
					 (", should in range 10~1000\n"));
			} else {
		...
		case 3:
			if (ObssScanValue < 200 || ObssScanValue > 10000) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Invalid OBSSScanParam for Dot11OBssScanPassiveTotalPerChannel(%d)",
					  ObssScanValue));
				DBGPRINT(RT_DEBUG_ERROR,
					 (", should in range 200~10000\n"));
			} else {
				/* Unit : TU. 200~10000 */
				pAd->CommonCfg.Dot11OBssScanPassiveTotalPerChannel = ObssScanValue;
				DBGPRINT(RT_DEBUG_TRACE,
		...
		case 4:
			if (ObssScanValue < 20 || ObssScanValue > 10000) {
				DBGPRINT(RT_DEBUG_ERROR,
					 ("Invalid OBSSScanParam for Dot11OBssScanActiveTotalPerChannel(%d)",
					  ObssScanValue));
				DBGPRINT(RT_DEBUG_ERROR,
					 (", should in range 20~10000\n"));
			} else {
				/* Unit : TU. 20~10000 */
				pAd->CommonCfg.Dot11OBssScanActiveTotalPerChannel = ObssScanValue;	
				DBGPRINT(RT_DEBUG_TRACE,
		...
		case 6:
			/* Unit : percentage */
			pAd->CommonCfg.Dot11OBssScanActivityThre = ObssScanValue;
			DBGPRINT(RT_DEBUG_TRACE,
				 ("OBSSScanParam for Dot11BssWidthChanTranDelayFactor=%d\n",
				  ObssScanValue));
			break;
...
INT Set_AP2040ReScan_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
...
INT Set_EntryLifeCheck_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	if (LifeCheckCnt <= 65535)
		...
		DBGPRINT(RT_DEBUG_TRACE, ("LifeCheckCnt must in range of 0 to 65535\n"));
	DBGPRINT(RT_DEBUG_TRACE, ("EntryLifeCheck Cnt = %ld.\n", pAd->ApCfg.EntryLifeCheck));
	return TRUE;
}
...
INT Set_MemDebug_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("Number of Packet Allocated = %lu\n", OS_NumOfPktAlloc));
	DBGPRINT(RT_DEBUG_TRACE, ("Number of Packet Freed = %lu\n", OS_NumOfPktFree));
	DBGPRINT(RT_DEBUG_TRACE, ("Offset of Packet Allocated/Freed = %lu\n", OS_NumOfPktAlloc - OS_NumOfPktFree));
...
#ifdef APCLI_SUPPORT
#ifdef APCLI_WPA_SUPPLICANT_SUPPORT
VOID RTMPApCliAddKey(IN PRTMP_ADAPTER pAd, IN INT apidx, IN PNDIS_APCLI_802_11_KEY pKey)
{
	ULONG KeyIdx;
	MAC_TABLE_ENTRY *pEntry;
	INT ifIndex, BssIdx;
	...
	if (wdev->AuthMode >= Ndis802_11AuthModeWPA) {
		if (pKey->KeyIndex & 0x80000000) {
			if (wdev->AuthMode == Ndis802_11AuthModeWPANone) {
				...
				NdisMoveMemory(pApCliEntry->PMK, pKey->KeyMaterial,
					       pKey->KeyLength);
				goto end;
			}
			/* Update PTK */
			...
			if (pApCliEntry->PairCipher == Ndis802_11TKIPEnable) {
				NdisMoveMemory(pMacEntry->PairwiseKey.RxMic,
					       pKey->KeyMaterial + LEN_TK, LEN_TKIP_MIC);
				NdisMoveMemory(pMacEntry->PairwiseKey.TxMic,
					       pKey->KeyMaterial + LEN_TK + LEN_TKIP_MIC,
					       LEN_TKIP_MIC);
			} else {
				NdisMoveMemory(pMacEntry->PairwiseKey.TxMic,
			...
			AsicAddPairwiseKeyEntry(pAd,
						(UCHAR) pMacEntry->Aid, &pMacEntry->PairwiseKey);

			RTMPSetWcidSecurityInfo(pAd,
						BssIdx,
						0,
						pMacEntry->PairwiseKey.CipherAlg,
						(UCHAR) pMacEntry->Aid, PAIRWISEKEYTABLE);
			...
			NdisZeroMemory(&pApCliEntry->SharedKey[wdev->DefaultKeyId],
				       sizeof(CIPHER_KEY));
			pApCliEntry->SharedKey[wdev->DefaultKeyId].KeyLen = LEN_TK;
			NdisMoveMemory(pApCliEntry->SharedKey[wdev->DefaultKeyId].Key,
				       pKey->KeyMaterial, LEN_TK);

			if (pApCliEntry->GroupCipher == Ndis802_11TKIPEnable) {
				NdisMoveMemory(pApCliEntry->SharedKey[wdev->DefaultKeyId].RxMic,
					       pKey->KeyMaterial + LEN_TK, LEN_TKIP_MIC);
				NdisMoveMemory(pApCliEntry->SharedKey[wdev->DefaultKeyId].TxMic,
					       pKey->KeyMaterial + LEN_TK + LEN_TKIP_MIC,
					       LEN_TKIP_MIC);
			} else {
				NdisMoveMemory(pApCliEntry->SharedKey[wdev->DefaultKeyId].TxMic,
...
#ifdef HOSTAPD_SUPPORT
VOID RtmpHostapdSecuritySet(IN RTMP_ADAPTER *pAd, IN RTMP_IOCTL_INPUT_STRUCT *wrqin)
{
	if (wrqin->u.data.length > 20 && MAX_LEN_OF_RSNIE > wrqin->u.data.length
		...
		if (wrqin->u.data.length != RSNIE_Len[0] + 2) {
			...
			NdisMoveMemory(pAd->ApCfg.MBSSID[apidx].RSN_IE[1],
				       (UCHAR *) (wrqin->u.data.pointer) + 2, RSNIE_Len[1]);
		...
		NdisMoveMemory(pAd->ApCfg.MBSSID[apidx].RSN_IE[0],
			       ((UCHAR *) (wrqin->u.data.pointer)) + 2 + offset_next_ie,
			       RSNIE_Len[0]);
...
========================================================================
*/
INT RTMP_AP_IoctlHandle(IN VOID *pAdSrc,
			...
			IN INT cmd, IN USHORT subcmd, IN VOID *pData, IN ULONG Data)
{
	...
	case CMD_RTPRIV_IOCTL_AP_SIOCGIFHWADDR:
		if (pObj->ioctl_if < MAX_MBSSID_NUM(pAd))
			NdisCopyMemory((PSTRING) wrq->u.name,
				       (PSTRING) pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.bssid, 6);
		break;
	...
	case CMD_RTPRIV_IOCTL_AP_SIOCGIWESSID:
		{
			...
			if (pSSID->priv_flags == INT_APCLI) {
				if (pAd->ApCfg.ApCliTab[pObj->ioctl_if].Valid == TRUE) {
					...
					pSSID->pSsidStr =
					    (char *)&pAd->ApCfg.ApCliTab[pObj->ioctl_if].Ssid;
				} else {
					pSSID->length = 0;
					pSSID->pSsidStr = NULL;
				}
	...
	case CMD_RTPRIV_IOCTL_WSC_INIT:
		{
			...
			if (pApCliEntry->WscControl.pWscTxBuf)
				NdisZeroMemory(pApCliEntry->WscControl.pWscTxBuf,
					       MGMT_DMA_BUFFER_SIZE);
	...
	case CMD_RTPRIV_IOCTL_AP_SIOCGIWAP:
		{
			...
			if (Data == INT_APCLI) {
				if (pAd->ApCfg.ApCliTab[pObj->ioctl_if].Valid == TRUE)
					pBssidStr =
					    (PCHAR) &APCLI_ROOT_BSSID_GET(pAd,
									   pAd->ApCfg.
									   ApCliTab[pObj->ioctl_if].
									   MacTabWCID);
				else
					pBssidStr = NULL;
			} else
			...
			{
				pBssidStr =
				    (PCHAR) &pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.bssid[0];
			}

			if (pBssidStr != NULL) {
				COPY_MAC_ADDR(pBssidDest, pBssidStr);
				DBGPRINT(RT_DEBUG_TRACE,
					 ("IOCTL::SIOCGIWAP(=%02x:%02x:%02x:%02x:%02x:%02x)\n",
					  PRINT_MAC(pBssidStr)));
			} else {
...
#ifdef DYNAMIC_VGA_SUPPORT
INT Set_AP_DyncVgaEnable_Proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	UINT Enable;
	UINT32 bbp_val;
	...
	if (pAd->CommonCfg.lna_vga_ctl.bDyncVgaEnable == FALSE) {
		...
		if (IS_MT76x2(pAd)) {
			...
			bbp_val =
			    (bbp_val & 0xffff80ff) | (pAd->CommonCfg.lna_vga_ctl.
						      agc_vga_init_0 << 8);
...
INT set_dynamic_lna_trigger_timer_proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_OFF, ("%s::(lna trigger timer = %d)\n",
				__func__, pAd->chipCap.dynamic_lna_trigger_timer));
...
INT set_thermal_protection_criteria_proc(IN PRTMP_ADAPTER pAd, IN PSTRING arg)
{
	...
	DBGPRINT(RT_DEBUG_ERROR, ("%s::criteria=%d\n", __func__, pAd->thermal_pro_criteria));
	return TRUE;
}
#endif /* THERMAL_PROTECT_SUPPORT */

#ifdef CONFIG_HOTSPOT
static INT Set_AP_HS_IE(IN PRTMP_ADAPTER pAd, IN UINT8 EID, IN PSTRING IE, IN UINT32 IELen)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
...
static INT Set_AP_VENDOR_SPECIFIC_IE(IN PRTMP_ADAPTER pAd,
				     IN UINT8 OUIType, IN PSTRING IE, IN UINT32 IELen)
{
#ifdef CONFIG_HOTSPOT
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
...
#ifdef CONFIG_DOT11V_WNM
static INT Set_AP_WNM_IE(IN PRTMP_ADAPTER pAd, IN UINT8 EID, IN PSTRING IE, IN UINT32 IELen)
{
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
...
INT Send_ANQP_Rsp(IN PRTMP_ADAPTER pAd,
		  IN PSTRING PeerMACAddr, IN PSTRING ANQPRsp, IN UINT32 ANQPRspLen)
{
	UCHAR *Buf;
	POS_COOKIE pObj = (POS_COOKIE) pAd->OS_Cookie;
	UCHAR apidx = pObj->ioctl_if;
			...
			if (MAC_ADDR_EQUAL(GASPeerEntry->PeerMACAddr, PeerMACAddr)) {
				...
				if ((ANQPRspLen % pHSCtrl->MMPDUSize) == 0)
					GASPeerEntry->GASRspFragNum =
					    ANQPRspLen / pHSCtrl->MMPDUSize;
				else
					GASPeerEntry->GASRspFragNum =
					    (ANQPRspLen / pHSCtrl->MMPDUSize) + 1;

				GASPeerEntry->CurrentGASFragNum = 0;

				for (i = 0; i < GASPeerEntry->GASRspFragNum; i++) {
					os_alloc_mem(NULL, (UCHAR **) &GASQueryRspFrag,
						     sizeof(*GASQueryRspFrag));
					...
					if (i < (GASPeerEntry->GASRspFragNum - 1))
						...
						GASQueryRspFrag->FragQueryRspLen =
						    ANQPRspLen - (pHSCtrl->MMPDUSize * i);

					os_alloc_mem(NULL,
						     (UCHAR **) &GASQueryRspFrag->FragQueryRsp,
						     GASQueryRspFrag->FragQueryRspLen);
					GASPeerEntry->AllocResource++;
					...
					NdisMoveMemory(GASQueryRspFrag->FragQueryRsp,
						       &ANQPRsp[QueryRspOffset],
						       GASQueryRspFrag->FragQueryRspLen);
