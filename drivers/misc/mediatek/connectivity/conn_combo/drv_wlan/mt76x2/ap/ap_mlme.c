			   ...
			   IN PVOID SystemSpecific2, IN PVOID SystemSpecific3)
{
	...
	if (pAd->CommonCfg.bBssCoexEnable == FALSE) {
		/* TODO: Find a better way to handle this when the timer is fired
		* and we disable the bBssCoexEable support!!
		*/
		...
		for (loop = 0; loop < MAX_APCLI_NUM; loop++) {
			...
			if (pAd->ApCfg.ApCliTab[loop].bBlockAssoc == TRUE
			    ...
			    && RTMP_TIME_AFTER(Now32,
					       pAd->ApCfg.ApCliTab[loop].LastMicErrorTime +
					       (60 * OS_HZ)))
			...
			if ((pApCliEntry->Valid == TRUE)
				...
				MlmeCalculateChannelQuality(pAd,
							    &pAd->MacTab.Content[pApCliEntry->
										 MacTabWCID],
							    Now32);
				...
				if (pAd->bApCliCertTest == TRUE) {
					...
					if (wdev->DesiredHtPhyInfo.bHtEnable && IsUseBA == FALSE) {
						EDCA_AC_CFG_STRUCT Ac2Cfg, Ac1Cfg;

						RTMP_IO_READ32(pAd, EDCA_AC2_CFG, &Ac2Cfg.word);
						RTMP_IO_READ32(pAd, EDCA_AC1_CFG, &Ac1Cfg.word);

						if ((pAd->RalinkCounters.
						     OneSecOsTxCount[QID_AC_VO] == 0)
						    && (pAd->RalinkCounters.
							OneSecOsTxCount[QID_AC_BK] >= 1000)
						    && (pAd->RalinkCounters.
							OneSecOsTxCount[QID_AC_VI] == 0)) {
							/*5.2.27 T7 */
							if (Ac1Cfg.field.Aifsn != 0x1) {
								Ac1Cfg.field.Aifsn = 0x1;
								RTMP_IO_WRITE32(pAd, EDCA_AC1_CFG,
										Ac1Cfg.word);
								printk
								    ("Change EDCA_AC1_CFG to %x\n",
								     Ac1Cfg.word);
							}
						} else
						    if ((pAd->RalinkCounters.
							 OneSecOsTxCount[QID_AC_VO]
							 == 0)
							&& (pAd->RalinkCounters.OneSecOsTxCount
							    [QID_AC_BK] == 0)
							&& (pAd->RalinkCounters.OneSecOsTxCount
							    [QID_AC_VI] == 0)
							&& (pAd->RalinkCounters.OneSecOsTxCount
	...
	if (pAd->CommonCfg.bHTProtect) {
		...
		if (pAd->CommonCfg.IOTestParm.bRTSLongProtOn == FALSE) {
			AsicUpdateProtect(pAd,
					  (USHORT) pAd->CommonCfg.AddHTInfo.AddHtInfo2.OperaionMode,
	...
	if (APCLI_IF_UP_CHECK(pAd, 0) && (pAd->bApCliCertTest == TRUE)) {
		if ((OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_SCAN_2040)) &&
			...
			if ((pAd->RalinkCounters.LastOneSecTotalTxCount < 70)
				...
				ScanParmFill(pAd, &ScanReq, ZeroSsid, 0, BSS_ANY,
	...
	if (pFrame->Hdr.FC.Type == FC_TYPE_DATA) {
		...
		if (pEntry &&
			...
			if ((MAC_ADDR_EQUAL(wsc_ctrl->EntryAddr, pEntry->Addr) ||
			    ...
			    IS_ENTRY_CLIENT(pEntry) && (wsc_ctrl->WscConfMode != WSC_DISABLE)) {
...
*/
			...
			*MsgType = APMT2_PEER_AUTH_CONFIRM;
			...
			DBGPRINT(RT_DEBUG_TRACE,
				 ("wrong AUTH seq=%d Octet=%02x %02x %02x %02x %02x %02x %02x %02x\n",
				  Seq, pFrame->Octet[0], pFrame->Octet[1], pFrame->Octet[2],
				  pFrame->Octet[3], pFrame->Octet[4], pFrame->Octet[5],
	...
	case SUBTYPE_ACTION_NO_ACK:
		*Machine = ACTION_STATE_MACHINE;
		/*  Sometimes Sta will return with category bytes with MSB = 1,
		* if they receive catogory out of their support
		*/
