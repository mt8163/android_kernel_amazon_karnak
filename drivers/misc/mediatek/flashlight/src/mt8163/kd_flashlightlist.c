...
static struct pinctrl_state *flashlight_enable_high;
static struct pinctrl_state *flashlight_enable_low;
...
int flashlight_gpio_init(struct platform_device *pdev)
{
	...
	flashlight_enable_high = pinctrl_lookup_state(flashlight_pinctrl, "enable_high");
	if (IS_ERR(flashlight_enable_high)) {
		ret = PTR_ERR(flashlight_enable_high);
		logI("%s : pinctrl err, flashlight_enable_high\n", __func__);
	}

	flashlight_enable_low = pinctrl_lookup_state(flashlight_pinctrl, "enable_low");
	if (IS_ERR(flashlight_enable_low)) {
		ret = PTR_ERR(flashlight_enable_low);
		logI("%s : pinctrl err, flashlight_enable_low\n", __func__);
	}
	return ret;
}
...
{
	return pinctrl_select_state(flashlight_pinctrl, flashlight_enable_high);
}

int flashlight_gpio_enable_low(void)
{
	return pinctrl_select_state(flashlight_pinctrl, flashlight_enable_low);
}
...
/* @@    register_battery_oc_notify(&bat_oc_protection_powerlimit, BATTERY_OC_PRIO_FLASHLIGHT); */
	...
	logI("[flashlight_probe] done! ret:%d~", ret);
	return ret;
}
