...
static dma_addr_t gpd_virt_to_phys(void *vaddr, u8 isRx, u32 num)
{
	...
	if (isRx)
		paddr =
		    (dma_addr_t) ((u64) (unsigned long)vaddr -
				  Rx_gpd_Offset[num]);
	else
		paddr =
		    (dma_addr_t) ((u64) (unsigned long)vaddr -
...
static void *gpd_phys_to_virt(dma_addr_t paddr, u8 isRx, u32 num)
{
	...
	if (isRx)
		vaddr =
		    (void *)(unsigned long)((u64) paddr + Rx_gpd_Offset[num]);
	else
		vaddr =
		    (void *)(unsigned long)((u64) paddr + Tx_gpd_Offset[num]);
	QMU_INFO("%s[%d]phys=%p<->virt=%p\n", ((isRx == RXQ) ? "RQ" : "TQ"),
		 num, (void *)paddr, vaddr);

	return vaddr;
}

static void init_gpd_list(u8 isRx, int num, struct _TGPD *ptr,
	struct _TGPD *io_ptr, u32 size)
{
	if (isRx) {
		...
		Tx_gpd_Offset[num] =
		    (u64) (unsigned long)ptr - (u64) (unsigned long)io_ptr;
		ptr++;
		Tx_gpd_List[num].pNext = (struct _TGPD *)((u8 *)ptr +
							GPD_EXT_LEN);
...
int qmu_init_gpd_pool(struct device *dev)
{
	...
	for (i = isoc_ep_start_idx; i <= MAX_QMU_EP; i++) {
		if (isoc_ep_gpd_count > mtk_qmu_max_gpd_num)
			Rx_gpd_max_count[i] = Tx_gpd_max_count[i] =
			    isoc_ep_gpd_count;
		else
			Rx_gpd_max_count[i] = Tx_gpd_max_count[i] =
			    mtk_qmu_max_gpd_num;
	...
	QMU_INFO("sizeof(struct _TGPD):%d\n", gpd_sz);
	if (gpd_sz != GPD_SZ)
	...
	for (i = 1; i <= RXQ_NUM; i++) {
		...
		ptr =
		    (struct _TGPD *) dma_alloc_coherent(dev, size, &dma_handle,
						GFP_KERNEL);
		if (!ptr)
			return -ENOMEM;
		memset(ptr, 0, size);
...
void mtk_qmu_enable(struct musb *musb, u8 ep_num, u8 isRx)
{
	...
	if (isRx) {
		...
		MGC_WriteQMU32(base, MGC_O_QMU_RQSAR(ep_num),
			       gpd_virt_to_phys(Rx_gpd_end[ep_num], RXQ,
						ep_num));
		...
		MGC_WriteQIRQ32(base, MGC_O_QIRQ_QIMCR,
				DQMU_M_RX_DONE(ep_num) | DQMU_M_RQ_EMPTY |
				DQMU_M_RXQ_ERR | DQMU_M_RXEP_ERR);
		...
		MGC_WriteQMU32(base, MGC_O_QMU_TQSAR(ep_num),
			       gpd_virt_to_phys(Tx_gpd_end[ep_num], TXQ,
						ep_num));
		MGC_WriteQUCS32(base, MGC_O_QUCS_USBGCSR,
				MGC_ReadQUCS32(base,
					       MGC_O_QUCS_USBGCSR) |
		...
		MGC_WriteQMU32(base, MGC_O_QMU_QCR2,
			       QCR | DQMU_TX_MULTIPLE(ep_num));
#endif

		MGC_WriteQIRQ32(base, MGC_O_QIRQ_QIMCR,
				DQMU_M_TX_DONE(ep_num) | DQMU_M_TQ_EMPTY |
				DQMU_M_TXQ_ERR | DQMU_M_TXEP_ERR);
...
static void mtk_qmu_disable(u8 ep_num, u8 isRx)
{
	...
	if (isRx) {
		...
		/* different EP QMU EN is separated in MGC_O_QUCS_USBGCSR ?? */
		MGC_WriteQUCS32(base, MGC_O_QUCS_USBGCSR,
				MGC_ReadQUCS32(base,
					       MGC_O_QUCS_USBGCSR) &
				(~(USB_QMU_Rx_EN(ep_num))));
...
{
	QMU_INFO(
		"task ep_n: %d, isRx: %d, buf: %p, len: %d zlp: %d isioc: %d\n",
...
void qmu_done_rx(struct musb *musb, u8 ep_num)
{
	...
	struct _TGPD *gpd_current =
		(struct _TGPD *) (unsigned long)MGC_ReadQMU32(
	...
	gpd_current = (struct _TGPD *) gpd_phys_to_virt((dma_addr_t)gpd_current,
				RXQ, ep_num);
	...
	if (gpd == gpd_current) {
		...
		QMU_ERR(
			"[RXD][ERR]HWO=%d,N_GPD=%p,BufLen=%d,Buf=%p,RecLen=%d,Ep=%d\n",
	...
	if (!gpd || !gpd_current) {
		QMU_ERR(
		  "[RXD][ERR]EP%d,gpd=%p,gpd_cur=%p,ishwo=%d,gpd_last=%p,RQCPR=0x%x\n",
		     ep_num, gpd, gpd_current,
		     ((gpd == NULL) ? 999 : TGPD_IS_FLAGS_HWO(gpd)),
		     Rx_gpd_last[ep_num], MGC_ReadQMU32(base,
							MGC_O_QMU_RQCPR
							(ep_num)));
		return;
	}
	...
	while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		...
		QMU_INFO(
			"[RXD]gpd=%p ->HWO=%d, Next_GPD=%p,RcvLen=%d, BufLen=%d,pBuf=%p\n",
		     gpd, TGPD_GET_FLAG(gpd), TGPD_GET_NEXT(gpd), rcv_len,
		     buf_len, TGPD_GET_DATA(gpd));
	...
	if (gpd != gpd_current && TGPD_IS_FLAGS_HWO(gpd)) {
		...
		QMU_ERR(
			"[RXD][ERR]HWO=%d,Next_GPD=%p,BufLen=%d,Buf=%p,RecLen=%d,Ep=%d\n",
		     (u32) TGPD_GET_FLAG(gpd), TGPD_GET_NEXT(gpd),
		     (u32) TGPD_GET_DataBUF_LEN(gpd), TGPD_GET_DATA(gpd),
		     (u32) TGPD_GET_BUF_LEN(gpd), (u32) TGPD_GET_EPaddr(gpd));
	}
...
void qmu_done_tx(struct musb *musb, u8 ep_num)
{
	...
	while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		...
		QMU_INFO(
			"[TXD]gpd=%p HWO=%d,BPD=%d,Next_GPD=%p,Buf=%p,BufLen=%d req=%p\n",
	...
	if (gpd != gpd_current && TGPD_IS_FLAGS_HWO(gpd)) {
		...
		QMU_ERR(
			"[TXD][ERR]HWO=%d,BPD=%d,Next_GPD=%p,DataBuf=%p,BufLen=%d,Ep=%d\n",
		...
		if (request->length == 0) {
			...
			QMU_WARN(
				"[TXD]Giveback ZLP of EP%d,actual:%d, length:%d %p\n",
			     req->epnum, request->actual, request->length,
			     request);
			musb_g_giveback(musb_ep, request, 0);
		}
	}
...
void h_qmu_done_rx(struct musb *musb, u8 ep_num)
{
	...
	/* gpd_current should at least point to the next
...
void h_qmu_done_tx(struct musb *musb, u8 ep_num)
{
	...
	/*gpd_current should at least point to
	 *the next GPD to the previous last one.
	 */
	if (gpd == gpd_current)
		return;
	...
	while (gpd != gpd_current && !TGPD_IS_FLAGS_HWO(gpd)) {
		QMU_INFO(
			"[TXD]gpd=%p HWO=%d, BPD=%d,Next_GPD=%p,Buf=%p, BufLen=%d\n",
		     gpd, (u32) TGPD_GET_FLAG(gpd), (u32) TGPD_GET_FORMAT(gpd),
		     TGPD_GET_NEXT(gpd), TGPD_GET_DATA(gpd),
...
finished:
	{
...
#ifdef CONFIG_MEDIATEK_SOLUTION
		{
			...
			skip_val = val &
			    (MUSB_RXCSR_INCOMPRX
			     | MUSB_RXCSR_DATAERROR | MUSB_RXCSR_PID_ERR);
...
void mtk_qmu_err_recover(struct musb *musb, u8 ep_num, u8 isRx, bool is_len_err)
{
	...
	list_for_each_entry(request, &musb_ep->req_list, list) {
		QMU_ERR("request 0x%p length(0x%x) len_err(%d)\n", request,
			request->request.length, is_len_err);

		if (request->request.dma != DMA_ADDR_INVALID) {
			if (request->tx) {
				QMU_ERR("[TX] gpd=%p, epnum=%d, len=%d\n",
					Tx_gpd_end[ep_num], ep_num,
				...
				QMU_ERR("[RX] gpd=%p, epnum=%d, len=%d\n",
					...
					request->request.length);
				mtk_qmu_insert_task(request->epnum, isRx,
