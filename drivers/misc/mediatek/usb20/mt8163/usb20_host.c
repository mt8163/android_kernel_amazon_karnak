...
static struct musb_fifo_cfg fifo_cfg_host[] = {
	{.hw_ep_num = 1, .style = MUSB_FIFO_TX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 1, .style = MUSB_FIFO_RX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 2, .style = MUSB_FIFO_TX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 2, .style = MUSB_FIFO_RX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 3, .style = MUSB_FIFO_TX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 3, .style = MUSB_FIFO_RX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 4, .style = MUSB_FIFO_TX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 4, .style = MUSB_FIFO_RX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 5, .style = MUSB_FIFO_TX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 5, .style = MUSB_FIFO_RX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 6, .style = MUSB_FIFO_TX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 6, .style = MUSB_FIFO_RX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 7, .style = MUSB_FIFO_TX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 7, .style = MUSB_FIFO_RX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 8, .style = MUSB_FIFO_TX, .maxpacket = 512, .mode =
	 MUSB_BUF_SINGLE},
	{.hw_ep_num = 8, .style = MUSB_FIFO_RX, .maxpacket = 64, .mode =
	 MUSB_BUF_SINGLE},
};
...
#if defined OTG_BOOST_BY_SWITCH_CHARGER
		tbl_charger_otg_vbus((work_busy(&musb->id_pin_work.work) << 8) |
				     1);
#else
#ifdef CONFIG_OF
#if defined(CONFIG_MTK_LEGACY)
...
#endif
#else
		mt_set_gpio_mode(GPIO_OTG_DRVVBUS_PIN,
				 GPIO_OTG_DRVVBUS_PIN_M_GPIO);
...
{
#if !(defined(SWITCH_CHARGER)) && !(defined(OTG_BOOST_BY_SWITCH_CHARGER))
#ifdef CONFIG_OF
#if defined(CONFIG_MTK_LEGACY)
	...
	if (IS_ERR(pinctrl_drvvbus)) {
		...
		dev_err(mtk_musb->controller,
			"Cannot find usb pinctrl drvvbus\n");
	}
	...
	if (IS_ERR(pinctrl_drvvbus_low)) {
		...
		dev_err(mtk_musb->controller,
			"Cannot find usb pinctrl drvvbus_low\n");
	}
	...
	if (IS_ERR(pinctrl_drvvbus_high)) {
		...
		dev_err(mtk_musb->controller,
			"Cannot find usb pinctrl drvvbus_high\n");
	}
...
static bool musb_is_host(void)
{
	...
	/* this will cause A-device change back to B-device
	 * after A-cable plug out
	 */
...
void musb_session_restart(struct musb *musb)
{
	...
	musb_writeb(mbase, MUSB_DEVCTL,
		    (musb_readb(mbase, MUSB_DEVCTL) & (~MUSB_DEVCTL_SESSION)));
...
static int musb_set_dock_present(struct musb *musb, bool en)
{
	...
	val.intval = en ? 1 : 0;
	ret = musb->batt_psy->desc->set_property(musb->batt_psy,
...
static void musb_id_pin_work(struct work_struct *data)
{
	...
	/*switch_set_state((struct switch_dev *)&otg_state,
	 *mtk_musb->is_host);
	 */
	...
	if (mtk_musb->is_host) {
...
#ifndef CONFIG_MTK_USB_CMD_CHANGE_MODE
		...
		musb_stop(mtk_musb);
#else
		/*ALPS01688604, IDDIG noise caused by MHL init */
		mt_usb_check_reconnect();
#endif
...
irqreturn_t mt_usb_ext_iddig_int(int irq, void *dev_id)
{
	if (!mtk_musb->is_ready) {
		...
		schedule_delayed_work(&mtk_musb->id_pin_work,
				      sw_deboun_time * HZ / 1000);
	}
...
{
	disable_irq(usb_iddig_number);
	cancel_delayed_work_sync(&mtk_musb->id_pin_work);
...
static void otg_int_init(void)
{
	...
	mt_eint_registration(IDDIG_EINT_PIN, EINTF_TRIGGER_LOW,
			     mt_usb_ext_iddig_int, FALSE);
#else
	int ret = 0;
	...
	ret =
	    request_irq(usb_iddig_number, mt_usb_ext_iddig_int,
