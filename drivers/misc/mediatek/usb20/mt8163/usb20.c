...
static struct musb_fifo_cfg fifo_cfg[] = {
	...
	{.hw_ep_num = 1, .style = MUSB_FIFO_TX, .maxpacket = 512, .ep_mode =
	 EP_BULK, .mode = MUSB_BUF_SINGLE},
	{.hw_ep_num = 1, .style = MUSB_FIFO_RX, .maxpacket = 512, .ep_mode =
	 EP_BULK, .mode = MUSB_BUF_SINGLE},
	{.hw_ep_num = 2, .style = MUSB_FIFO_TX, .maxpacket = 512, .ep_mode =
	 EP_BULK, .mode = MUSB_BUF_SINGLE},
	{.hw_ep_num = 2, .style = MUSB_FIFO_RX, .maxpacket = 512, .ep_mode =
	 EP_BULK, .mode = MUSB_BUF_SINGLE},
	{.hw_ep_num = 3, .style = MUSB_FIFO_TX, .maxpacket = 512, .ep_mode =
	 EP_BULK, .mode = MUSB_BUF_DOUBLE},
	{.hw_ep_num = 3, .style = MUSB_FIFO_RX, .maxpacket = 512, .ep_mode =
	 EP_BULK, .mode = MUSB_BUF_DOUBLE},
	{.hw_ep_num = 4, .style = MUSB_FIFO_TX, .maxpacket = 512, .ep_mode =
	 EP_BULK, .mode = MUSB_BUF_DOUBLE},
	{.hw_ep_num = 4, .style = MUSB_FIFO_RX, .maxpacket = 512, .ep_mode =
	 EP_BULK, .mode = MUSB_BUF_DOUBLE},
	{.hw_ep_num = 5, .style = MUSB_FIFO_TX, .maxpacket = 512, .ep_mode =
	 EP_INT, .mode = MUSB_BUF_SINGLE},
	{.hw_ep_num = 5, .style = MUSB_FIFO_RX, .maxpacket = 512, .ep_mode =
	 EP_INT, .mode = MUSB_BUF_SINGLE},
	{.hw_ep_num = 6, .style = MUSB_FIFO_TX, .maxpacket = 512, .ep_mode =
	 EP_INT, .mode = MUSB_BUF_SINGLE},
	{.hw_ep_num = 6, .style = MUSB_FIFO_RX, .maxpacket = 512, .ep_mode =
	 EP_INT, .mode = MUSB_BUF_SINGLE},
	{.hw_ep_num = 7, .style = MUSB_FIFO_TX, .maxpacket = 512, .ep_mode =
	 EP_BULK, .mode = MUSB_BUF_SINGLE},
	{.hw_ep_num = 7, .style = MUSB_FIFO_RX, .maxpacket = 512, .ep_mode =
	 EP_BULK, .mode = MUSB_BUF_SINGLE},
	{.hw_ep_num = 8, .style = MUSB_FIFO_TX, .maxpacket = 512, .ep_mode =
	 EP_ISO, .mode = MUSB_BUF_SINGLE},
	{.hw_ep_num = 8, .style = MUSB_FIFO_RX, .maxpacket = 512, .ep_mode =
	 EP_ISO, .mode = MUSB_BUF_SINGLE},
#else
	{.hw_ep_num = 1, .style = MUSB_FIFO_TX, .maxpacket = 512, .ep_mode =
	 EP_BULK, .mode = MUSB_BUF_DOUBLE},
...
static void mt_usb_enable(struct musb *musb)
{
	...
	/* mask ID pin, so "open clock" and "set flag"
	 * won't be interrupted. ISR may call clock_disable.
	 */
...
void mt_usb_connect(void)
{
	DBG(0, "is ready %d is_host %d power %d\n", mtk_musb->is_ready,
	    mtk_musb->is_host, mtk_musb->power);
...
void mt_usb_disconnect(void)
{
	...
	DBG(0, "[MUSB] USB will disconnect\n");
...
{
	pr_notice("[MUSB]%s %d: weak function", __func__, __LINE__);
...
#if (defined CONFIG_MTK_KERNEL_POWER_OFF_CHARGING) && (defined CONFIG_NO_CHARGER_KPOC)
	if (get_boot_mode() == KERNEL_POWER_OFF_CHARGING_BOOT)
		return false;
#endif
...
/*ALPS00775710*/
#ifdef CONFIG_MTK_USB_CMD_CHANGE_MODE
	int iddig_state = 1;
	if (mtk_musb->force_mode == USB_FORCE_HOST) {
		pr_info("[USB]%s:%d is_host:%d\n",
		       __func__, __LINE__, mtk_musb->is_host);
		iddig_state = 0;
...
static irqreturn_t generic_interrupt(int irq, void *__hci)
{
	...
	musb->int_usb =
	    musb_readb(musb->mregs, MUSB_INTRUSB) & musb_readb(musb->mregs,
							       MUSB_INTRUSBE);
	musb->int_tx =
	    musb_readw(musb->mregs, MUSB_INTRTX) & musb_readw(musb->mregs,
							      MUSB_INTRTXE);
	musb->int_rx =
	    musb_readw(musb->mregs, MUSB_INTRRX) & musb_readw(musb->mregs,
							      MUSB_INTRRXE);
...
static irqreturn_t mt_usb_interrupt(int irq, void *dev_id)
{
	...
	DBG(1, "usb interrupt assert %x %x  %x %x %x\n", usb_l1_ints,
	    musb_readl(mtk_musb->mregs, USB_L1INTM), musb_readb(musb->mregs,
								MUSB_INTRUSBE),
	    musb_readw(musb->mregs, MUSB_INTRTX), musb_readw(musb->mregs,
							     MUSB_INTRTXE));
				  ...
				  const char *buf, size_t count)
{
	...
	if (!dev) {
		...
		if (cable_mode != cmode) {
			...
			cable_mode = cmode;
			mdelay(10);
			/* check that "if USB cable connected
			 * and than call mt_usb_connect"
			 */
			/* Then, the Bat_Thread won't be
			 * always wakeup while no USB/chatger
			 * cable and IPO mode
			 */
			usb_check_connect();
...
#ifdef CONFIG_USB_MTK_OTG
			if (cmode == CABLE_MODE_CHRG_ONLY) {
				/* shut down USB host for IPO */
				if (mtk_musb && mtk_musb->is_host) {
					if (mtk_musb->usb_lock->active)
						__pm_relax(mtk_musb->usb_lock);
					musb_platform_set_vbus(mtk_musb, 0);
					/*add sleep time to ensure vbus
					 *off and disconnect irq processed.
					 */
					msleep(50);
					musb_stop(mtk_musb);
					MUSB_DEV_MODE(mtk_musb);
					/* Think about IPO shutdown
					 * with A-cable,
					 * then switch to B-cable
					 * and IPO bootup.
...
static ssize_t mt_usb_show_tx(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	UINT8 var;
	UINT8 var2;
...
static ssize_t mt_usb_store_tx(struct device *dev,
			       ...
			       size_t count)
{
	...
	if (!dev) {
		...
		DBG(0,
		    "[MUSB]addr: 0x6E TX [AFTER WRITE], value after: %x - %x\n",
		    var, var2);
		sw_tx = var;
	}
	return count;
...
static ssize_t mt_usb_show_rx(struct device *dev, struct device_attribute *attr,
			      char *buf)
{
	UINT8 var;
	UINT8 var2;
...
static struct i2c_board_info usb_i2c_dev __initdata = {
	I2C_BOARD_INFO("XXXXXXX", 0x60) };
...
/*static int usb_i2c_detect(struct i2c_client *client, int kind,
...
#ifdef CONFIG_OF
	/*pr_info("usb probe irq: 0x%d usb phy address:
	 * 0x%lx usb mac : 0x%1x\n",
