...
static u32 msdc_cmd_find_resp(struct msdc_host *host,
		struct mmc_request *mrq, struct mmc_command *cmd)
{
	u32 resp;
			    ...
			    struct mmc_command *cmd, struct mmc_data *data)
{
	...
	WARN(host->data, "host data can't be NULL!\n");
	host->data = data;
...
static void msdc_request_done(struct msdc_host *host, struct mmc_request *mrq)
{
	...
	/**
	  * For the sdio command process, msdc_request_done is always invoked
	  * in interrupt context. But if it is in SDIO SW timeout mechanism,
	  * msdc_request_done is invoked in delayed work thread context.
	  * In the case, delayed work starts to work and it comes a CMD_DONE
	  * interrupt at the same time, the code here can guarantee the
	  * mmc_request_done is only invoked once. Otherwise, the flow of
	  * mmc core layer will become chaotic.
	  */
	if (!ret && in_interrupt()) {
		...
		struct mmc_request *mrq, struct mmc_command *cmd)
{
	...
	WARN(host->cmd, "host cmd can't be NULL\n");
	host->cmd = cmd;
...
static void msdc_ops_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	host->error = 0;
	WARN(host->mrq, "host mrq can't be NULL\n");
	host->mrq = mrq;
...
static int msdc_ops_switch_volt(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (!IS_ERR(mmc->supply.vqmmc)) {
		...
		if (ret) {
			dev_info(host->dev, "Regulator set error %d (%d)\n",
...
#define AUTOK_LATCH_CK_SDIO_TUNE_TIMES  (10)  /* 4.5IP SDIO 128fifo ZIZE */
