...
#define MMC_TUNE_FLAG     (MMC_TUNE_FLAG_RSP | MMC_TUNE_FLAG_DAT)
#endif
	...
	if (!sbc_error && !(events & MSDC_INT_CMDRDY)) {
		...
		if (events & MSDC_INT_RSPCRCERR) {
			...
			if (cmd->opcode == MMC_SEND_TUNING_BLOCK_HS200) {
				dev_err(host->dev, "%s: HW issue: CMD DAT TIMEOUT, cmd=%d\n",
						 __func__, cmd->opcode);
				BUG();
			}
		}
...
static void msdc_dump_reg(struct msdc_host *host)
{
	int i;
	int val;
	...
	for (i = 0; i < 0x118; i += 4) {
		if (i != 0x18 && i != 0x1c)
			dev_err(host->dev, "[0x%x]:%x\n", i, readl(host->base + i));
		else
			dev_err(host->dev, "skip read [0x%x]\n", i);
	}

	for (i = 1; i < 0x19; i++) {
		writel(i, host->base + 0xA0);
		val = readl(host->base + 0xA4);
...
skip_internal:
	...
	if ((host->first_tune & MMC_TUNE_FLAG_RSP) == 0)
	{
		if (host->dev_comp->async_fifo)
			...
			dev_info(host->dev, "[%d]CALIB_CMD: rise 0x%x,%d/%d, fall 0x%x,%d/%d, internal 0x%x,%d/%d\n", mmc->index,
				rise_delay, final_rise_delay.final_phase, final_rise_delay.maxlen,
				fall_delay, debug_fall_delay.final_phase, debug_fall_delay.maxlen,
...
static int msdc_tune_together(struct mmc_host *mmc, u32 opcode)
{
	...
	/* if rising edge has enough margin, then do not scan falling edge */
#ifdef CONFIG_MMC_TUNE_WINDOW_LOGS
	if ((final_rise_delay.maxlen >= 12) && (host->first_tune & MMC_TUNE_FLAG))
#else
	if (final_rise_delay.maxlen >= 12)
#endif
		goto skip_fall;
...
#ifdef CONFIG_MMC_TUNE_WINDOW_LOGS
	if ((host->first_tune & MMC_TUNE_FLAG) == 0)
		debug_fall_delay = get_best_delay(host, fall_delay);
	if (final_rise_delay.maxlen < 12)
#endif
		final_fall_delay = get_best_delay(host, fall_delay);
