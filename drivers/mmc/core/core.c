...
extern int mmc_sd_init_card(struct mmc_host *host, u32 ocr,
...
void mmc_init_erase(struct mmc_card *card)
{
	...
	if (mmc_card_sd(card) && card->ssr.au) {
		...
		if (sz < 128)
			...
			card->pref_erase = 1024 * 1024 * 1024 / 512;
...
#ifdef CONFIG_MMC_PARANOID_SD_INIT
	/*
	 * if card abnormal in ESD or other cases, try re-init to recover but not just set removed.
	 */
	if (ret) {
		while(retries) {
			...
			if (err) {
				...
				pr_info(KERN_ERR "%s(%s): Re-init card rc = %d (retries = %d)\n",
					__func__, mmc_hostname(host), err, retries);
				retries--;
				udelay(5);
				continue;
			}
